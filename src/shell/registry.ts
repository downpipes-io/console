...
// requireCap builds a `when` that gates a command by a CAPABILITY (the capability model), so a
// command the caller's role cannot exercise is never listed in the palette (the mirror of the
// engine's per-route capability gate; the engine is the enforcement point). New affordances gate
// with can(role, capability) rather than the legacy cumulative ladder, so the two narrow roles
// (restore-operator, access-admin) are gated by their real capabilities. A null caller (whoami
// pending) fails closed, and WITNESSES.
//
// The witness is one of three sites (alongside canDo and canCap in screens/common.ts) that records
// when a capability gate is decided without an identity to decide it from: a palette opened before
// whoami returns would otherwise hide every capability-gated command, restore included, and leave
// no row saying so.
...
// The day-one command set. Navigation is
...
export const COMMANDS: Command[] = [
  ...
  // Notifications + Security centre. Both screens are READ-visible
  ...
  // by capability, not the cumulative role rank, matters because a restore-operator holds both yet sits
  // off the rank ladder: a role-rank gate would hide the restore commands from the recovery-only role the
  // engine accepts. Two entries keep palette claims honest; the engine remains the enforcement point.
