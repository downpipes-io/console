...
// kept separate from mountShell so the entry stays the assembler; the methods read the shell
// state through deps.
function buildShellHandles(deps: ShellHandleDeps): Shell {
  ...
  // state, which waits for a rendering opportunity) rather than inline with the setMain call: the swap has
  // not run yet immediately after startViewTransition() returns (exercised against a real browser in the
  // accompanying test suite). A QUIET re-render (the identity re-render,
  ...
  // enough to win that race, the quiet swap paints the CORRECT content, and the earlier transition's deferred
  // callback then fires anyway and clobbers it back to the stale first-paint content. This races identically
  // on the break-glass panel and on /security's owner-only retire control: it is not a per-screen gap, the
  // shared re-render mechanism itself is unguarded against its own async sibling.
  ...
  return {
    setMain(node: Node, measure: "prose" | "wide" | "full" = "wide", opts?: { quiet?: boolean }): void {
      ...
      // A soft cross-fade between screens: the View Transitions API is a
      ...
      // notFound -> "/" double-fire, the setup-gate / role-landing redirects, post-reset churn) would
      // otherwise start a second transition while the first is still capturing, throwing "InvalidStateError:
      // Transition was aborted because of invalid state" and surfacing as an Uncaught (in promise). A
...
// buildNavLink + applyNavCount live in shell/nav-links.ts; re-exported here so the
// nav/shell validators (test/validate-nav.ts) and any other importer can import them
// from the shell entry.
