...
export function wireGlobalKeys(openPalette: () => void, onNavigate: (route: string) => void): void {
  ...
  document.addEventListener("keydown", (ev: KeyboardEvent) => {
    ...
    // the keyboard: a modal, drawer, confirm, or the command palette all mount through
    // dialog.ts's one overlay engine, so isOverlayOpen() is a single structural check
    // that covers every dialog shape in the console, not a per-shape enumeration. A
    // pending "g" is also reset so a dialog opened between the "g" and its letter
    // cannot be escaped by completing the chord after the dialog appears.
