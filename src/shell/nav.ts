...
// health and recovery posture ("Roles gate visibility, not just enabling" applies
// to controls; the rail items themselves stay visible to all). So no nav item
// carries a role gate here.
...
// The rail structure, top to bottom.
export const NAV: NavGroup[] = [
  ...
  },
  {
    label: "Operate",
    items: [
      // The water path, in dependency order (the setup-first IA): Destinations (where
      // backups land, nothing works without it), Sources (what to protect, discovered,
      // never typed), then Downpipes (the routes between them). Add-a-source is not
      // its own rail item; it lives at Sources' advanced sub-screen (/sources/advanced).
