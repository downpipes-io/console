...
// dot: nothing drives one (the app only ever knows "unknown"), so a dot here would be a
// permanently grey light that reads like a broken indicator. The real health verdict
...
export function renderAccount(slot: HTMLElement, caller: Caller | null, whoamiAvailable: boolean, onSignOut: () => void): void {
  ...
  if (caller && whoamiAvailable) {
    ...
    // account area). The verdict chip already carries the verified email, so no
    // second copy of it renders beside it.
    slot.appendChild(
      h(
        "div",
