...
// The recovery sheet is the printed artefact from the key ceremony. It records the
// fingerprints, the active posture, the out-of-band anti-rollback pin field, and the
// plain-language instruction that the break-glass private key is the only way to
// recover and must be kept offline. It deliberately does NOT contain any private key.
//
// WHAT THE SHEET IS NOT. The sheet carries FINGERPRINTS, not key material, so a customer
// holding only the printed sheet holds neither signer.pub nor recipient.pub. That matters
// because verify and restore both hard-require --signer and exit ExitUsage without it, and
// the signer public key is nowhere in the archive to fall back on. The sheet states plainly
// that the kit is two files and that the sheet alone is not sufficient. It carries no key
// material, which is deliberate.
//
...
function sheetHeadHTML(): string {
  return [
    ...
    // THE PAGE MARGIN IS DECLARED HERE, and `size` deliberately is NOT. Without an explicit @page
    // margin, the print block's zeroed body margin leaves the printed margin at whatever the browser
    // happens to default to, which varies by browser and is not a value we control. 16mm is ours and
    // is the same on every browser. `size` is left alone ON PURPOSE so the customer's own paper (A4
    // here, Letter elsewhere) is still respected: a recovery sheet is printed once, by a customer, on
    // whatever is in their tray, and forcing a size is how it comes out scaled or clipped.
    "  @page { margin: 16mm; }",
