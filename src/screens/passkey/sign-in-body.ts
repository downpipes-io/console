...
// kept in their own module for size, each taking its inputs explicitly.
//
...
//
// STRICT CSP: every node is built with the h() builder; no inline script/style/handler.
...
// recovery flow's enrolNext handed over.
export function parseSignInIntent(match: RouteMatch): SignInIntent {
  ...
  // returns the operator where they were going. Only same-origin in-app paths are honoured: the value
...
// buildCardHeader appends the heading and the honest sub-line to the card.
...
// buildEmailField builds the email Field for the sign-in / set-up card.
...
// slot, and wires every click to its flow. The status host + setStatus are owned by render (the aria-live
// region with the scrollIntoView guard) and handed in. Returns the controls the layout builders consume.
...
// flow's "set up a new passkey now"): set-up leads; signing in is the quiet alternative. Appends to the card
// in place.
...
// blocks. Appends to the card in place and renders the sign-in view.
...
// Built here so the layout module owns the navigate import; render() wires the click.
