...
// DO reads, so they are identical under a real PublicKeyCredential and the validator's fixture. These are the
...
export function passkeyErrorMessage(err: unknown, mode: "login" | "register"): string {
  ...
  // NO DOMEXCEPTION NAME IS NOT THE SAME THING AS NOTHING ANSWERING. Below this point, anything unnamed falls
  // through to the reachability sentence, and on this surface that is the sentence an operator quotes to
  // support: it sends them to the network, the tunnel and DNS. Most of what can actually reach here already
  // has an honest classification elsewhere:
  //
  ...
  //
  // Not one of those is fixed by checking the network, and the console holds the status that says so: the
  // engine client folds it into the throw, and classifyError already tells all of these apart. So this asks
  // engineAnswered, and on an answer hands over errorDetail's REVIEWED sentence rather than writing a further
  ...
  //
  // Same shape of split as errors.ts's malformed-2xx handling and onboarding/steps.ts's polling narration.
  // Only `network`, a fetch that threw carrying no status, keeps the line below, and it is still logged rather
  // than rendered: a fetch TypeError's message can include the engine's internal hostname.
  //
  ...
  //
  // An account holding more than 64 credentials makes register/begin emit an excludeCredentials list the
  // browser will not accept, and Chromium throws DOMException("...exceeds the maximum allowed size (64)")
  // named `RangeError` before it ever consults an authenticator. `RangeError` matches no named branch above and
  // carries no HTTP status, so testing by name alone would tell the operator to go and check the engine was
  // reachable, when the engine answered register/begin 200 moments earlier. It is the same shape of error the
  // named branches above already handle honestly; it simply carries a name none of them enumerate, and
  // enumerating names one at a time can never cover every DOMException a browser can throw.
  //
  // So the test below is the KIND of error, not its name: fetch rejects with a TypeError and never with a
  ...
  //
  // IT IS ASKED BEFORE engineAnswered, AND THE ORDER MATTERS. engineAnswered treats any thrown message that
  // does not match a browser's own fetch-failure wording as `console-fault` (an answer the console mishandled),
  // and a WebAuthn DOMException matches no such wording. Asking engineAnswered first would read every
  // DOMException as an engine answer and splice the DOMException's own message into errorDetail's
  // console-fault sentence, which can carry the rp id and other deployment detail these branches exist to keep
  // off the screen. Testing the DOMException kind first is the right order on engineAnswered's own reasoning:
...
//
// THE ORDERING IS THE WHOLE POINT. Enumerating the refusal names one at a time would let `RangeError` be
// reported as an unreachable engine, so this does the opposite: it names only the errors that mean THE PERSON
// OR PLATFORM DECLINED, and treats every OTHER DOMException as the browser refusing. A refusal nobody has met
// yet then lands on copy that is honest about what is known rather than on copy written for a different
// failure.
//
...
export function recoveryTransportMessage(err: unknown): string {
  ...
  // above and hands EVERYTHING else to this function, so without the split below the engine's hard
  // per-IP-and-per-email recovery limiter, its 501 with CONSOLE_ORIGIN unset, an Access page and the console
  // worker's own binding-absent 503 would all read as an unreachable engine. This is the break-glass path: the
  // operator is here BECAUSE the ordinary way in stopped working, and telling them at that moment to go and
  // check the engine is reachable is the worst possible minute to send them somewhere there is nothing to
  // find.
  //
...
//
// WHY A SILENT OR UNGUARDED FAILURE HERE WOULD BE THE WORST FAILURE ON THIS SCREEN. A capability guard that
// returns silently when URL/createObjectURL/Blob are missing, or an unguarded createObjectURL/a.click() that
// throws when the browser refuses the click, would both leave the customer pressing "Download .txt" on the
// one-time recovery codes with nothing arriving, nothing said on the panel, and no trace of it anywhere; if
// they then lose their passkey they are locked out of their engine permanently. So a missing capability and a
// refused one are the SAME answer here, false, and both are recorded in the ring that rides inside the
// support pack.
