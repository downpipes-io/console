...
// engine + navigator and assert the request bodies and the success/cancel/unknown handling. They import the
// shared ceremony leaf (./ceremony.ts) one way, so they never form a cycle.
//
...
export function renderAdminTokenBlock(): HTMLElement {
  ...
  tokenBtn.addEventListener("click", () => {
    ...
    // ASVS V7.2.2: the pasted secret is presented ONCE, to the exchange route, and the client then carries a
    // minted session cookie instead; the transport discards the secret on success. A refused exchange (a wrong
    // or retired token) lands in the catch below with the same copy as before.
    void client
      .bootstrapSession()
      .then(() => {
        tokenInput.value = "";
...
export async function runStepUpOutcome(engine: EngineClient, handles?: Partial<FlowHandles>): Promise<StepUpOutcome> {
  ...
  // A SINGLE BARE CATCH ACROSS A BROWSER REFUSAL, A TRANSPORT FAULT AND A GENUINE CANCEL WOULD REPORT ALL
  // THREE AS `no-assertion`, whose copy ends "this device has no passkey enrolled for you and you can enrol
  // one from Security". For an account whose credential list is over the browser's measured 64-entry
  // ceiling, no prompt appears, that advice is wrong, and following it adds a credential to an account that
  // already cannot get back under. See stepUpFailureMessage.
  //
  // So each await below says which failure it is. The cancel/timeout/no-credential merge is PRESERVED
  ...
  // An engine that answers no_passkey is not refusing to start the check: it has nothing to check with. The
  // operator holds no passkey on this account, which is the ordinary state of an SSO member who has not enrolled
  // one, and since an OIDC or SAML session must assert a passkey for every gated action (ASVS V6.8.4) the remedy
  // is enrolment, not a retry. Its own ending, so the copy can say so.
  if (!begin.ok) return end({ ok: false, reason: begin.reason === "no_passkey" ? "no-passkey" : "begin-refused" });
...
export async function runRegister(engine: EngineClient, email: string, handles: FlowHandles, inviteToken?: string): Promise<void> {
  ...
  try {
    ...
    if (codes !== undefined && codes.length > 0 && handles.showRecoveryCodes) {
      ...
      handles.setBusy(false);
      // recoveryCodesPending:true means these codes are
...
export async function runBootstrapSend(engine: EngineClient, handles: FlowHandles): Promise<void> {
  ...
  try {
    ...
    // unpressed button.
    //
    // WHY THIS NAMES MORE THAN THE EMAIL SERVICE. "The account needs Cloudflare Email Service set up" names
    // only the LEAST likely blocker on a stock deployment. The engine checks the owner address FIRST
    // (router-auth-flow.ts:627, skip "owner-email-not-configured"), and that address is a per-deployment
    // secret which is deliberately never committed, so a fresh engine fails there before the Email Service is
    // even reached. And past that, the sender address ships as a committed default on a domain the customer
    // does not own (wrangler.toml EMAIL_FROM), which passes the engine's own validator and then fails at the
    // platform with a sender-domain error. A single-cause banner would send an operator to the Cloudflare
    // dashboard to fix something that is probably not their problem, and give them nothing to check for the
    // two that are.
    //
