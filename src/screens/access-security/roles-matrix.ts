...
// These describe the model, not live data.
...
// myPermissions shows the caller's OWN effective capabilities from their real role,
...
export function myPermissions(): HTMLElement {
  ...
  // capability so no role is ever misrepresented: a combined row would conflate downpipe writes with
  // restore-operator's recovery-only drill, so they are kept split.
  const caps: Array<{ cap: Capability; label: string; requires: string }> = [
    ...
    // "create, edit, delete and trigger" row would over-report a downpipe.write-only custom role against the
