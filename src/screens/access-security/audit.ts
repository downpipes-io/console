...
// record with the verify-chain action, JSON and CSV export, server-side filters seeded
// from and reflected to the URL, expandable change-control detail, and the honest boundary
...
//
// WHY THIS IS ONE CONSTANT AND NOT THREE SENTENCES. The denial copy lives in exactly one place, the banner
// in load()'s isForbidden branch. The banner alone cannot reach the toolbar, because the toolbar is built
// ABOVE the async region and lives OUTSIDE it, so replacing the region's children can never remove or
// refuse a control that is not one of them. Left alone, that produces a screen that states "you do not have
// permission to view the audit log" while "Verify chain integrity", "Export (JSON)" and "Export (CSV)" sit
// above it, enabled, in the tab order, carrying no aria-disabled, no title and no description. A screen that
// names a refusal and then offers the refused actions is not a legible refusal.
//
...
/** The banner shown where the audit table would be, sourced from the shared remedy. */
...
export function renderAuditPanel(engine: EngineClient, query: URLSearchParams): HTMLElement {
  ...
  // The audit panel's section heading is correctly placed as the first h2 below
  ...
  // The audit boundary, stated NOW. It is the panel's ONE standing
  ...
  // Near-cap warning: fetch the engine status and surface a calm,
  ...
  // The filters are the SERVER-SIDE query the engine applies, seeded from the URL so a
  // filtered view is bookmarkable and survives a refresh. Every change re-fetches via
  // load() and is reflected back to the URL.
  ...
  // so a search reaches beyond the first page; it is debounced for the
  ...
  const load = (): void => {
    ...
    void engine
      ...
      .catch((err) => {
        ...
        // misleads a legitimately-denied viewer. The precedent for this shape is
        // sources-downpipes/detail-actions.ts's drillLatest, which names the permission in customer
        // language and gates its own control disabled-with-reason.
        //
        // AND REFUSE THE ACTIONS. The banner replaces `region`, and the three privileged buttons are built
        // ABOVE it and live OUTSIDE it, so no amount of replacing the region's children can reach them on
        // its own; the denial latch above is what refuses them. Every one of the three actions is gated by
        // the SAME capability at the engine (GET /audit, GET /audit/verify and GET /audit/export each gate
        // on audit.read, engine admin/router-rbac.ts), so a caller denied the list is denied all three.
        // Refusing them, rather than hiding them, is the house rule (disabled-with-reason, never
        // hidden-then-403), and it is the right one here: an action that vanishes teaches nothing, an
        // action that names the permission it needs and who can grant it is a governed refusal a customer
        // can act on.
        if (isForbidden(err)) {
          denial.deny();
        ...
        // The audit tile can say the engine's audit store is not live yet. On a 404/501 that is
        ...
        // and the customer must NOT be told the trail is merely "coming soon" while a real fault hides it. Route
        // a genuine fault to the honest block error with Retry, and keep pending-engine only for a genuine
        // not-wired build.
...
// auditNearCap is explicitly true, so the warning only ever reflects what the engine reports.
//
// The three outcomes are DISTINCT. A failed status read must not render exactly what "the log is fine"
// renders (nothing), because a customer whose audit log rolled past retention unexported would then never be
// warned, and nobody could say whether the console had checked at all. A read that failed, and an engine
// whose status carries no auditNearCap field to read, are both reported as an UNKNOWN capacity (a quiet hint
// with the export to hand), never as an all-clear. They are hints, not alarms: this is the absence of a
// check, not the presence of a fault.
...
//
// WHY A FOURTH STATE. The engine's retained audit count SATURATES at its retention cap: from the first
// rollover onwards it is pinned there, so auditNearCap is true both at the moment nothing has been lost and
// after ten thousand entries have been destroyed. auditNearCap alone cannot distinguish those two moments,
// and the "warn" card's copy is written in the future tense ("once the cap is reached, oldest entries are
// rolled over") and ends "Export now to retain the full history", a remedy the operator cannot take once
// entries are already gone. auditRolledOverCount separates the two states, so "warn" (nothing lost yet) and
// "rolling" (entries already gone) get different copy.
//
// A count of 0 is a REPORTED zero and stays "warn", which is correct: nothing has rolled yet and the export
// really does still retain everything. An ABSENT count on an older engine also reads "warn": a capacity
// warning that could not read the loss must not be silently upgraded to a claim about it.
...
function mountNearCapWarning(
  ...
  void engine
    ...
    .then((s) => {
      ...
      // POLITE, AND THE ROLE SAYS SO. role and aria-live must agree: role="alert" implies assertive, so
      // pairing it with aria-live="polite" would leave the two declarations disagreeing about whether to
      // interrupt, with the outcome depending on which one the assistive technology honours. This card uses
      // role="status" so the two agree.
      //
...
async function handleVerifyChain(engine: EngineClient, verifyBtn: HTMLButtonElement, verifyHost: HTMLElement): Promise<void> {
  ...
  try {
    ...
    // that raced the first load) needs its own message: "Chain integrity could not be determined ... retry
    // once the engine is healthy or check the engine logs" would send them in three false directions at
    // once, because the engine is healthy, waiting will not help, and a caller denied the audit log cannot
    // read the engine logs either. It gets the honest refusal instead, and the toolbar's own refusal is the
    // standing form.
    ...
    // UNHIDE. verifyHost starts `hidden` (an empty grid child leaves a dead band, see its own comment
    // above), so a failure verdict must clear it here too, not only on the success path above: otherwise
    // the verdict this branch builds is written into a hidden container and shown to nobody, for every
    // failure this branch covers, not only the denial.
    verifyHost.hidden = false;
  } finally {
...
async function runAuditExport(engine: EngineClient, format: "json" | "csv", btn: HTMLButtonElement): Promise<void> {
  ...
  try {
    ...
    if (isUnauthorised(err)) return goSignedOut();
    // The same three-way split as handleVerifyChain: GET /audit/export gates on audit.read, so a denied
    // caller needs its own message. "Nothing was downloaded; retry once the engine is healthy" would still
    // be half right: nothing was downloaded, but the rest sends them to wait on a fault that is not there.
    if (isForbidden(err)) {
...
// recordFeature emits a feature probe (see roles.ts). A 401 records nothing: a lapsed session is the ordinary
