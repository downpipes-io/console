...
export function renderChainVerdict(verdict: ChainVerdict): HTMLElement {
  ...
  // tamper-proof.
  // A DELETED TAIL IS A BREAK THE RECOMPUTE CANNOT SEE, so it is checked FIRST. verify walks the entries
  ...
  if (verdict.intact) {
    ...
    // A ROLLED-OVER CHAIN IS INTACT AND IS ALSO MISSING ITS BEGINNING. Saying only "the chain recomputes
    // cleanly from the earliest retained entry" does not say WHICH entry that is, so the reading is identical
    // on a log that begins at entry 1 and on one whose first two thousand entries have been destroyed. The
    // engine sends the rolled-over count and the earliest retained seq on this very response, so both are
    // used. This is not a break and is not toned as one: retention is working as designed, and what the
    // operator needs is the number, because an export can only ever carry what is still held.
    // THE FLAG IS THE FACT; THE COUNT IS ONLY HOW MUCH. Gating the rollover branch on `rolledOver === true &&
    // (count ?? 0) > 0` would make the count the trigger, so a rollover the engine reports WITHOUT a usable
    // count (rolledOverCount: 0, which the engine books as `rollover-record-lost`) would fall through to the
    // plain intact surface below and the operator would read the very sentence this branch exists to stop
    // them reading. Gating on `rolledOver === true` alone avoids that: a count is worth having and its
    // absence is not a reason to say nothing, so earliestSeq (which rides the same response) names WHERE the
    // retained chain begins even when the count itself has been lost.
...
export function renderAuditEvents(engine: EngineClient, page: AuditPage, filters: AuditFilters, reload: () => void): HTMLElement {
  ...
  // artefact most visits never use, but copyable in full via codeBlock for the auditor).
  {
  ...
  // the server-side bar above, so this table carries no free-text filter
...
function openAuditDetail(e: AuditEvent): void {
  ...
  // Change-control detail for a restore (a question auditors often ask).
