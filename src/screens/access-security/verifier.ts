// The live Cloudflare Zero Trust verifier for the enforcement sub-view: it reads the REAL
// whoami verdict, never a hardcoded green, and shows the role-basis note beneath it. The verdict
// helpers (deriveVerdict, reauthenticate) live in ./shared.ts so this module never imports a
// sibling panel; the METHOD-TO-VERDICT mapping itself is accessVerdictFromCaller (trust-chips.ts),
// the console's one read of it.
...
// CONSOLE_ORIGIN setup error, and never a hardcoded green. Returns its element plus
...
export function renderVerifier(engine: EngineClient): { el: HTMLElement; reverify: () => void } {
  ...
  const reverify = async (): Promise<void> => {
    ...
    // is most often the CONSOLE_ORIGIN footgun: surface it as a setup error, not
  ...
  // The honest no-anonymous-probe note: we do not stage a fake anonymous request; the browser
  // always carries the Access cookie. The factor-limit caveat ("passed Access policy", never
  // "used a second factor") is NOT restated here: accessVerdictPanel always renders it per
  // state (verdict.ts), so a standing copy of it here would just duplicate that.
  card.appendChild(
    noteLine(
      ICON_INFO,
...
export function verifierErrorTile(engine: EngineClient, err: unknown, reverify: () => void): HTMLElement {
  ...
  if (kind.kind === "network") {
    ...
    void healthWithTimeout
      .then(() => {
        ...
        // The authenticated call threw with no response AND the unauthenticated health probe answered
        // in the same breath: that pair is the CORS fingerprint, and it is the difference between "set
        // CONSOLE_ORIGIN on the engine" (a two-minute fix) and "your engine is down" (a support call).
        // The console computes it, paints it, and records it, so a customer who sees this tile and
        // closes the tab still leaves a trace behind.
      ...
      .catch(() => {
        ...
        // This is the OTHER half of the same fork, and it must be its own row: an engine that is genuinely
  ...
  if (kind.kind === "server") {
    // A whoami 5xx is presented to the operator as a benign "your session is present" degrade. It reads as a
    // healthy-but-quiet console when it is in fact a broken engine, which is why the verdict is recorded even
