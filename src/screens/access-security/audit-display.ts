...
//
// EVERY action the engine can write is offered. The filter is a server-side one (the engine's isAuditAction
// guard accepts any AUDIT_ACTIONS member and SILENTLY DROPS anything else), so an action absent from this list
// is simply unselectable: an operator whose backups stopped could not narrow 500 rows of routine traffic down
// to the engine-secret-absent event that says WHY. The completeness of AUDIT_ACTION_LABELS below is enforced
// by tsc; this list is checked against it by the access-security validator.
export const AUDIT_ACTION_GROUPS: Array<{ group: string; actions: AuditAction[] }> = [
  ...
  { group: "Downpipes and runs", actions: ["downpipe-create", "downpipe-delete", "downpipe-roster-reconcile", "run-trigger", "run-failed", "sources-attached", "sources-detached", "retention-prune", "retention-prune-request", "retention-prune-approve", "retention-prune-reject"] },
  ...
  { group: "Keys", actions: ["key-ceremony-intent", "keys-installed", "key-install-failed", "break-glass-rotated", "operational-added", "operational-removed", "key-removal-failed", "posture-acknowledged", "custody-share-emailed", "secret-rotation-confirmed"] },
  ...
  // HARNESS_TEST_FAULTS estate, and the question they answer, "was a fault armed when this verdict was
  // banked", is one a reviewer narrows to deliberately rather than meets while reading routine traffic.
  { group: "Test faults (harness estates only)", actions: ["test-fault-armed", "test-fault-disarmed", "test-fault-fired", "test-fault-control-plane-cleared"] },
...
const AUDIT_ACTION_LABELS: Record<AuditAction, string> = {
  ...
  // This is the loss event, named LOUDLY because it is not an engine-state curiosity: a tracked secret
  ...
  "secret-rotation-confirmed": "Secret rotation confirmed",
  ...
  "test-fault-control-plane-cleared": "Control-plane test fault cleared by the harness",
};
...
export function describeTarget(t: AuditTarget): HTMLElement {
  switch (t.kind) {
    ...
    case "access-policy": {
      ...
      // account policy moved and in WHICH direction (arming dual control kept distinct from disarming it),
      // which canary control was used, and whether the gated action ran via the bare break-glass token, which
      // is exempt from the dual-control auto-apply. Closed enums and a boolean.
      const parts: string[] = [];
    ...
    case "authn-attempt": {
      // A REFUSED authentication ceremony. The connection is named only when it resolved, and attempts says how
      // many coalesced failures this one row stands for, so a reader is never left taking a coalesced row for a
      // single event. Closed enums, the operator-chosen slug and an integer, never an assertion or a token.
      ...
      return h("span", { class: "mono" }, `failed ${t.ceremony}${via}${n}`);
    }
    case "secret-rotation":
      // The closed cadence id of the attested-rotated bearer or destination credential; never a value.
      return h("span", { class: "mono" }, `secret rotation confirmed ${t.secretId}`);
    ...
    case "test-fault": {
      ...
      // nouns rather than prose, and the operator is comparing them against a harness manifest.
    ...
    default: {
      ...
      //
      // The console shows "newer engine?" on screen, and this call also records it in the pack. This is the
      // loudest of the three skew fallbacks (a whole audit row the console cannot describe), and it is its own
      // family: an unknown TARGET is a different piece of engine vocabulary from an unknown METHOD or an
      // unlabelled FIELD, and folding them together would leave support unable to say WHAT the console is
      // behind on. The raw kind is rendered as a text node for the operator.
...
// Record, so tsc refuses to let a member be added without a label). Completeness matters because a partial
// mapping that falls through to "engine-observed" for any unlisted method would render a real oidc, saml or
// recovery actor in the Method column as THE ENGINE having done it: a human sign-in through the customer's IdP,
// and a break-glass recovery-code sign-in above all, would read on screen as the cron, with nothing flagging it
// because nothing would think anything had gone wrong.
...
//
// THE ROW MEANS WHAT ITS VOCABULARY SAYS. A contract-skew row asserts that a NEWER ENGINE used a value this
// build does not know. `recovery` is a member of this console's own union, which the engine has always been
// able to write and which a mistyped email on the public recovery form produces every time
// (scheduler-do-recovery.ts appends actorMethod "recovery" with a null actorEmail on the failed branch), so it
// must never be treated as skew. The test is membership of the console's OWN union, which is the only state
// that can mean the engine is ahead of this build.
