...
// in the Cloudflare dashboard, then the verifier confirms it took. The hand-off button calls
// the verifier's own reverify directly rather than DOM-spelunking for a button.
...
function noSecretNote(): HTMLElement {
  // The standing no-secret rule, restated where it matters.
  return h(
    "div",
...
function stepProtectHostnames(): HTMLElement {
  // No decorative step rail: the numbered wizardStep cards carry the sequence. A static rail
  // hardcoded to a fixed step and verify-state would contradict the live verifier and the
  // enabled hand-off button beneath it.
...
function wizardStep(num: string, title: string, desc: string, body: Node[]): HTMLElement {
  ...
  // engages because the pre is already as wide as its content. A long wrangler snippet can then run past the
  // card at narrow widths and give the whole document a horizontal scrollbar. Zeroing the track's floor lets
  // the block scroll inside its own frame, which is what §20's "tables + code scroll inside their frame,
  // never widen the page" intends.
