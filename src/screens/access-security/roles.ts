// Roles and access sub-view of the Access and security area: the in-app RBAC table, the
// last-Owner guard mirror (enforced server-side), the dual-control
...
// roles-members.ts, roles-matrix.ts and roles-groups.ts (the shared leaf helpers are in
// roles-helpers.ts).
...
export function renderRolesPanel(engine: EngineClient): HTMLElement {
  // The roles tabpanel gets a section-level h2 below the screen h1, so the
  ...
  // The honesty facts live in this one lead hint: the engine is the boundary, the table is
  // the mirror, and the bootstrap-Owner rule.
  wrap.appendChild(
    h(
      "div",
  ...
  const load = (): void => {
    ...
    void Promise.all([
      ...
      .catch((err) => {
        ...
        // The pending-vs-broken conflation is resolved: a 404 (the feature genuinely is not built) and a 500
        // (the engine is broken) are told apart rather than both reading as "not built yet", which would tell
        // a customer whose engine is broken a comforting falsehood, often for weeks. recordFeature classes it
        // for the pack; the render tells them apart too: a genuine 404/501 keeps the pending note, a real
        // fault (500/network) reads as the honest block error with Retry.
        ...
        // Pending: present the role model the engine implements, never a fake grid.
  ...
  // Demoted sections (design-system.md 7a: the members table and the matrix pair are the eager
...
// second sign-off, and config changes awaiting a second approver). It is the governance
// read, not the inboxes themselves (linked below). It degrades honestly: a 404/501 from a
// build without restore approvals shows the pending-engine note rather than faking a zero, and the
// config count is simply omitted on a build without the change gate. No-custody: the records carry
// names, counts and a reason only, never a key or a value.
// Exported for the validator: this card is the governance read for the mechanic that gates a
...
export function dualControlCard(engine: EngineClient): HTMLElement {
  ...
  void Promise.all([
    ...
    // fail-open stays (it must never degrade the restore-approvals read beside it); the verdict is recorded
    // regardless, so the pack can say which it actually was.
...
// recordFeature is the emit helper: it classifies a thrown engine read into the console's own verdict and
