...
// recovery codes and any unexpired registration invite, because deleting a role row does not touch any of the
// three sign-in stores. A column on the roster could never show those identities, because they are not rows
// in it.
//
...
// keeps the two separate because deleteRole cannot tell which one it is being asked for. This panel keeps
// that distinction on screen, in the confirm copy, rather than quietly re-joining them.
...
export function renderSignInFactorsPanel(engine: EngineClient): HTMLElement {
  ...
  const load = (): void => {
    ...
    void Promise.all([
      ...
      // non-Owner caller who tries to strip an Owner. That refusal is a BARE 403 { error: "forbidden" } with
      // no reason in it, which reaches the operator as
...
function buildFactorsTable(engine: EngineClient, listing: SignInFactorListing, roles: RoleEntry[], reload: () => void): HTMLElement {
  ...
  // is still refused an Owner target. Access-admin can act on a non-owner target, so the refusal is the
  // escalation guard specifically, not a blanket denial.
...
//
// IT NAMES THE STEP-UP PROMPT, because the engine's revoke route requires one (it is a member of the
// engine's STEPUP_SUBS). Silence here about re-authentication would leave a customer facing an unannounced
// passkey prompt in the middle of an irreversible offboarding action.
//
