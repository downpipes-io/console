// The server-side filter machinery for the audit log sub-view: seed the AuditFilters from
// the route query, the debounced filter-bar
...
// auditFilterBar is the SERVER-SIDE filter control row. A change to any control mutates
// the shared AuditFilters, reflects them to the URL, and re-fetches via load() so a
// search reaches the whole log, not just the loaded page.
...
export function auditFilterBar(filters: AuditFilters, load: () => void): HTMLElement {
  ...
  // control of the same height, so top-aligning lines the five controls up exactly. END-aligning does
  // not: it lines up the field BOXES, and the Actor field is the only one of the five carrying a hint,
  // so bottom-aligning boxes of unequal height lifts its control by exactly the hint's height, visibly
  // out of line with its neighbours. Whatever sits BELOW a control may differ between members; what
  // must line up is the control itself.
  // TWO ROWS, NOT ONE GRID. A Clear action as a sixth item in the same auto-fit grid as the five filter
  // fields would land BESIDE a control instead of below the bar wherever that grid wraps. An action is
  // not a field and does not belong in a track sized for one. The fields keep the grid; the actions get
  // their own row underneath it.
  ...
  // for a person reaches beyond the first page. Debounced at 300ms.
...
// audit view is deep-linkable. limit is fixed at the page default; the
...
// so the filtered view is shareable and survives a refresh without adding a history entry.
// The route base stays /access/audit.
