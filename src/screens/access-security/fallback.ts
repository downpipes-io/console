...
export function renderFallbackPanel(engine: EngineClient): HTMLElement {
  // The fallback tabpanel gets a section-level h2 below the screen h1, so the
  ...
  // Sessions and passkeys: self-service session termination + the
...
// tokenFallbackDisclosure demotes the shared token behind a disclosure with a loud posture warning.
// The require-Access affordance is an HONEST READ-ONLY posture status, not an interactive
...
// engine's authoritative verdict, and the console consults it before advising closing the token path.
//
// Why the consult is load-bearing rather than decorative. Deleting or disabling the token is an act
...
function tokenFallbackDisclosure(engine: EngineClient): HTMLElement {
  ...
  inner.appendChild(
    h(
      "p",
      { class: "field__hint" },
      "The shared admin token is entered on the sign-in screen, not here. The console presents it once, exchanges it for a minted session, and discards it from memory; the console never persists it. The engine never downgrades a present-but-invalid Access assertion to the token path.",
    ),
  );
  inner.appendChild(h("hr", { style: "border:none;border-top:1px solid var(--border-subtle);margin:var(--space-1) 0" }));

  // Require-Access posture comes from the engine's preflight endpoint rather than being inferred from the
  // caller's own Access verdict, which would not reflect the account-wide posture.
...
// expiry, with an honest re-auth + sign-out that credit the edge. Pre-whoami it
...
function sessionCard(): HTMLElement {
  ...
  // Expiry is omitted rather than faked when not known.
...
// authMethodBadge names THIS session's sign-in method on the session card. It has an arm for every
// verdict accessVerdictFromCaller can reach, because those states are real, live sign-in methods (the
// engine's own OIDC and SAML, and a recovery-code sign-in), each with its own line rather than a
// fall-through that would claim "Method not reported" about a session the engine reported perfectly well.
// The break-glass badge is worn only by the break-glass method; an unrecognised method says so plainly
// rather than claiming either posture.
