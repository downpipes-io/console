...
// the seed default and the pagination guards cannot drift.
...
//
// It delegates to accessVerdictFromCaller, the console's one read of the sign-in method. A second, private
// mapping here would risk diverging from it on the very screen the console sends a customer to when it wants
// them to harden their posture: a live OIDC session, a live SAML session, a recovery-code sign-in and a
// method this build has never heard of all need to be told apart from the shared break-glass token, since
// conflating any of them with "Token fallback in use ... anyone with this URL and the shared admin token can
// administer it" is the worst thing the console can say about a customer's security when it is not true. One
// mapping answers honestly for all six engine methods, and an unrecognised method answers `unknown` AND
// records the wire anomaly (the method string never rides).
...
// errText lives in lib/errors.ts; re-exported here so the access-security screens keep their local import.
...
//   - "missing": a NON-engine (human) event that genuinely lacks an IP (e.g. an older recorded entry, or a
//     path that could not capture one) -> the honest "not recorded" dash, distinct from the system case.
...
// ---------------------------------------------------------------------------
// Engine-state field label mapping. Exported for testing.
...
// EVERY member of the engine's engine-state field union is labelled here, and that completeness is
// load-bearing. "secret-absent" is a presence LOSS: a tracked secret VANISHED, which is the engine saying a
// deploy dropped the signer key, the break-glass key or the destination binding, and therefore why the
// backups stopped. A field missing from this table takes the unknown-field arm in describeTarget and records
// a contract-skew row whose vocabulary asserts "a NEWER engine is writing fields this build does not know" --
// a row byte-identical to a genuinely-newer engine's, on the one screen the "backups stopped" ticket lands
// on. That misreads the console's own gap as a version-skew problem and points support at the wrong fix. A
// field with no label here must mean what the row says it means.
