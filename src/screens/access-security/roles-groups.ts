...
// downpipes works fully without it. Writes are gated on the access.policy capability
// (Owner or access-admin).
...
export function groupRoleMappingPanel(engine: EngineClient): HTMLElement {
  // Gate on the access.policy CAPABILITY, not canDo("owner"): the engine's real gate for
  // POST/DELETE /admin/group-roles is access.policy, which access-admin also holds. canDo("owner")
  // maps access-admin to rank 0, which would wrongly deny it this whole panel.
  ...
  const load = (): void => {
    ...
    void Promise.all([
      ...
      .catch((err) => {
        ...
        // customer who cannot see (or fix) who their SSO is granting access to. Broken and unbuilt render as
        // the same state here, not two.
...
// with Retry rather than a second pending-engine note that would frame a real fault as unbuilt.
...
function groupRoleAddRow(engine: EngineClient, reload: () => void, customRoles: readonly CustomRole[] = []): HTMLElement {
  ...
  // The mapping picker offers custom roles alongside the built-ins: the engine's setGroupRole accepts a
  // customRole, assignGroupCustomRole exists in the client layer, and the mapping table renders the
  // custom-role badge. A custom role is offered only when the caller holds every capability it confers,
  // mirroring requireGrantWithinAuthority, which the engine applies to a group mapping exactly as it does
  // to a per-person grant.
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    // Decoded, not cast: the select carries built-in roles and custom roles in one closed list. A value
    ...
    try {
      ...
      // step-up ceremony throws "set group role: stepup-required: 401", and printing that raw would put an
      // internal marker and a bare status in front of the operator, reading as a lapsed session when the
      // session is fine.
          ...
          onClick: async () => {
            try {
              ...
              if (isPendingResult(res)) {
                // group-role-delete is change-control gated, so a 202 means the removal was QUEUED for a
                // second approver and the mapping still stands. Reading that 202 as a success would report
                // "mapping removed" for a change that has not landed; surface the queued state honestly and
...
// recordFeature is the feature-probe emit helper (see roles.ts). A 401 records nothing: a lapsed session is
// the ordinary state of a console left open overnight, and a fault row for each one would bury the true faults.
