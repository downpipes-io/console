// Sources: the catalogue of everything the engine can see, a first-class screen in its own
// right rather than a drawer inside Downpipes.
//
...
// account catalogue in ./sources/account.ts, with the public surface identical across the split.
//
...
// is mirrored (Owner sets the token; Operator+ creates), the engine enforces.
...
function screenActions(): ScreenAction[] {
  return [
    // One palette row for this destination: the connect-account keywords fold into the
    // navigation command.
    {
      id: "go-sources",
      title: "Go to Sources",
