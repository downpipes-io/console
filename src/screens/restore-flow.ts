// Restore, runs and actions: the COORDINATOR for the restore flow. It
...
// mirrors the engine's own logic exactly. The
...
export const restoreFlowScreen = defineScreen({
  // The restore route family (information-architecture.md). This screen is the
  ...
  // "/restore/break-glass" (the in-console break-glass restore panel) and its
  ...
  // retention-prune's dual-control inbox, the sibling of "/restore/approvals" above) is a fixed route and
  // MUST likewise precede "/restore/:runId" so it is never swallowed as runId="prune-approvals".
  route: [
    ROUTE_RESTORE,
  ...
  actions: [
    ...
    },
    {
      ...
      // (restore.request, POST /restore/request). Gated by CAPABILITY, not by a role-NAME list, so the
      // recovery-only restore-operator role, which holds BOTH restore.request and restore.approve
      // (identity-model.ts ROLE_CAPABILITIES), reaches the dual-control inbox from the palette as well as
      // from the same screen's header button (canCap("restore.request"), below).
    ...
    },
    {
      ...
      // Gated identically to restore.approvals above (the retention prune mirrors restore's request/approve
      // capabilities rather than inventing its own; router-retention-prune.ts gates request on restore.request and
  ...
  render(ctx: ScreenContext): HTMLElement {
    ...
    if (ctx.pattern === "/restore/break-glass" || ctx.pattern === "/restore/break-glass/:runId") {
      // The in-console break-glass restore panel: supply your break-glass key in this browser,
    ...
    const header = pageHeader(
      ...
      // The inbox entry point mirrors the palette gate above: BOTH read the
      ...
      // caller who holds either reaches the inbox by whichever entry point they find first, including a
      // role that can approve but not request (reachable via a named custom role), which is precisely
      // who the inbox is for.
