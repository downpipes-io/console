...
// engine's ability to self-test-restore. It is irreversible without a re-key, so it is confirm-to-act. It
// imports the shared leaf (./shared.ts) only, so it never forms a cycle.
...
export function renderPostureSwitch(engine: EngineClient): HTMLElement {
  ...
  // Named, and re-runnable, because the catch at the bottom needs something to offer: a failed status read
  // must not strand the owner without the break-glass-only switch for the life of the screen.
  const load = (): void => {
    ...
    void engine.status().then((s) => {
      ...
      // explicit confirmDiscardStranded. Printing the refusal alone would dead-end the owner: the engine's
      // only sanctioned path forward needs a control in the UI, on exactly the action that stands between a
      // key rotation and permanent loss of old archives.
      //
      ...
      tokenPurpose: "remove the operational key",
          ...
          confirm: {
            ...
            // This copy must not claim the switch cannot be undone without generating new keys: that is false
            // against the engine. POST /keys/add-operational installs ONLY the operational
      ...
      // This message names a remedy and leaves something to press, on the one control that tightens custody:
      // a bare failed-read message with nothing to press would leave reloading the tab as the only way back.
      body.replaceChildren(
        inlineRetry({
