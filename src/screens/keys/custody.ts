...
// never uploaded). It imports the shared leaf (./shared.ts) only, so it never imports another tab module
...
// ciphertext and each share as downloads.
...
// is cleared when the selected file is not a recognised identity.key. The read goes through
// lib/file-size-guard.ts, so a file above CONSOLE_FILE_MAX_BYTES is refused before any read starts.
function renderFileInputBlock(mountStep: (result: CustodyKeyInput) => void, stepHost: HTMLElement): HTMLElement {
  const block = h("div");
  const fileInput = h("input", { "data-dp": "keys.file.file-input-block", type: "file", accept: ".key,text/plain", "aria-label": "Select your identity.key file" }) as HTMLInputElement;
  ...
  fileInput.addEventListener("change", () => {
    ...
    if (!file) return;
    fileErr.hidden = true;
    void readTextFileBounded(file).then((text) => {
      ...
      // checks its length, so on its own it would accept material the restore path would reject, and would
      // split it. The failure would surface only at recovery, after the shares had been distributed to
      // custodians and the original file possibly destroyed, the worst possible moment. Validate with the
      // SAME audited parser the restore path uses (it decodes base64url and enforces the 96 bytes), so a
      // bad file is refused here instead. The parsed copy is only needed for the check, so it is zeroed
      // straight away rather than carried.
      try {
      ...
      fileErr.textContent = e instanceof Error ? `${e.message}.` : "Could not read that file.";
      fileErr.hidden = false;
      stepHost.replaceChildren();
    });
  });
