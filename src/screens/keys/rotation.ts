...
// old identity.key to recover old runs. It imports the
// shared leaf (./shared.ts) only, so it never imports another tab module (which would form a cycle).
//
// NO-CUSTODY invariants (current-state.md 4, Appendix A): the new break-glass private is generated entirely in
// the browser; nothing is POSTed, logged, stored, or transmitted; there is NO field, command, or upload path
// for the new or old break-glass private.
...
export function renderRotationWiring(engine: EngineClient, fresh: CeremonyResult, identityDelivered: boolean): HTMLElement {
  ...
  // control again", and this is the control it means: the one instruction given to a customer whose
  // browser refused their new private key must name a control that exists on the screen they are
  // reading. It is the only way past the block below.
