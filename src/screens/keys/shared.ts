...
export const KEYS_ACTIONS: ScreenAction[] = [
  ...
  },
  {
    ...
    // Lands on the Rotate section itself: a command named for the rotation must arrive there, not on the
    // Posture screen and leave the operator to find the tab.
...
// until it is opened. Roving tabindex + Left/Right/Home/End arrow movement (design-system 6.14/8).
//
...
export function renderKeysTabs(tabs: KeysTab[], active: KeysTabId): HTMLElement {
  ...
  // Activating a tab NAVIGATES, and navigate() is not synchronous (it schedules a view
  ...
  if (pendingTabKeyboardFocus) {
    ...
    // Declared to the shell rather than scheduled here: a queueMicrotask restore would run before the shell
    // has mounted this wrap under a view transition (so focus() would hit a detached node and do nothing),
    // and once it did land, the shell's own post-navigation focus move to <main> would overwrite it a
    // moment later. The shell reads this after the swap and honours it instead of <main>. See lib/nav.ts.
...
//
// It CONFIRMS and nothing else. Combining it with the apply call would read as "confirm, then run" but
// behave as "return whether it was confirmed, and lose the run": nothing could await the apply, and the
// apply owns the button. Keeping them split puts the busy control and the call that resolves it in one
// frame, which is the only arrangement in which either can be checked.
...
//
// A browser that REFUSES the blob tab must not fail silently: this tells the operator and reports a
