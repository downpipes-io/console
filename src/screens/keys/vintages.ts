...
// wrapped to the old key and the engine cannot re-encrypt them. The Rotate tab WARNS about that
// ("Archives sealed before this rotation can only be opened by the old identity.key. Keep it."), but a warning
// is not a check: an owner who rotates and keeps only the latest key needs an in-product view of which
// vintages exist, which runs are stranded to a key the engine no longer holds, and how many runs a further
// re-key would stop verifying. This panel reads that inventory from the engine (GET /admin/keys/vintages,
// admin/key-vintages.ts).
//
...
// VINTAGE_HEADING is the panel's accessible name. It is exported so the console's own tests and the
// harness journey that grades this surface pin the SAME string, rather than each guessing at it.
...
function inventoryBody(inv: KeyVintageInventory): HTMLElement {
  ...
  // the console verifying older receipts even when every archive is still openable. Both counts belong on the
  // same surface, because an owner reading only one of them draws the wrong conclusion.
  wrap.appendChild(
    h(
      "p",
      { class: "field__hint" },
