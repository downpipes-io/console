...
// the api validator pins the pure input builders / channel-url validators. They live in the
// ./notifications/shared.ts leaf (all pure + DOM-free); re-exporting them here keeps the public import
// surface of screens/notifications.ts stable for those callers.
export {
  severityRank,
  eventLabel,
  NOTIFY_EVENTS,
...
// The screen descriptor. It owns the three deep-linkable sub-views behind an accessible
// tablist. measure:"wide" because channels, rules and history are dense tables. The palette
// commands live in shell/registry.ts, so this descriptor contributes no duplicate actions.
