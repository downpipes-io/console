...
// The exported route constant the training walk's route union pins against (the chapters.ts ROUTE_CHECK
// pattern): a renamed route here fails `tsc` in the script rather than navigating a walk to nowhere.
export const ROUTE_DESTINATIONS = "/destinations";

export const destinationsScreen: Screen = {
  route: ROUTE_DESTINATIONS,
  title: "Destinations",
  // Wide: the multi-destination list lays its cards two-up to use the space (owner). The
  // first-run setup form self-caps via .card.measure and the single deploy-time posture card
  // is wrapped in .measure below, so neither is flung wide; values are left-aligned.
  measure: "wide",
  actions: [
  ...
  render() {
    ...
    // renders that same surface as a "Replace from the console" disclosure instead, so both
    // titles are named here and openDisclosureByTitle opens whichever one this render actually
    // produced.
    const addBtn = h(
      "button",
