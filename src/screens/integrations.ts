...
// canManageVendor above is the gate: it fails closed on an unresolved caller, which is correct, because an
// affordance must never be offered before the engine has said who is asking.
//
// The SENTENCE the surface prints is a different question. Printing a fail-closed default as the settled
// fact "Owner" (or "Configuring this is owner only") would tell an actual owner they are not the owner during
// the brief window before whoami lands, a false statement about the reader's own authority with no reason and
// no remedy attached. This is the same distinction identity-remedy.ts draws for every other gated screen: a
// role refusal and an unread role are different situations with different remedies, and telling a real owner
// to ask an owner sends them to themselves.
//
// canManageVendor is called FIRST and always, so its blind witness (noteGateComputedBlind, via canDo/canCap)
// still fires for the support pack's identity-stale-gate row on this screen.
