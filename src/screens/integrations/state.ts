...
//
// WHY IT IS NOT THE WIRE. Matching on the live format AND sink alone is not injective over this catalogue:
// Splunk and CrowdStrike Falcon Next-Gen SIEM both take splunk-hec over http, so one live Splunk push would
// render BOTH tiles Active and both panels would claim the same config as their own. Vendors that genuinely
// share a wire and are genuinely interchangeable on it (CEF over syslog-TLS backs QRadar, ArcSight, FortiSIEM,
// LogRhythm and Securonix) are a different case from two vendors whose credentials are shaped differently, and
...
// self-clearing (any Replace writes the tag), and the vendor-specific credential guidance does not ride on it
// either: that keys off the vendor's own credScheme, so a shared wire cannot put Splunk's instruction on
// CrowdStrike's form.
...
export function vendorState(v: Vendor, snap: ConfigSnapshot): TileState {
  switch (v.kind) {
    case "push": {
      ...
      // tile state and the panel body can never disagree about it.
...
// An undivided count would hide that: it would print "1 alert rule routes events to Slack" for a rule scoped
// to a downpipe that has been deleted, suppressing both the warning and the fix-it action.
