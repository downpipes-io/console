...
// unresolvedGate is the sentence for the state that must never be rendered as ownerGate: no identity report
// has arrived, so whether this reader may configure the destination is NOT YET KNOWN. Printing the owner-only
// refusal here would tell an actual owner they are not the owner, and offer them the one remedy that is
// useless to an owner, which is to ask an owner. The remedy comes from blindGateRemedy so this screen and
// every other gated screen say the same thing about the same state.
...
//
// The copy goes through the guarded clipboard primitive rather than a bare
// `navigator.clipboard?.writeText(url).then(...)`, which fails silently in two ways at once: on a browser with
// no clipboard the optional chain short-circuits and the button never changes, and a REJECTED write (a
// permissions policy, a document without focus) has no rejection handler at all. Either way the operator
// would believe they had copied an endpoint they had not, paste a stale value into the vendor, and the feed
// they configured would point nowhere. The guarded primitive says the refusal out loud and records it in the
// pack.
...
// SENTINEL_CONTENT_HUB_URL is a PLACEHOLDER: there is no live Content Hub listing yet, so this points at an
// Azure Marketplace search for the solution's name rather than a dead link, keeping the control honest and
// clickable in the meantime. Swap this one constant for the real listing URL once the listing exists; no
// other code here needs to change.
const SENTINEL_CONTENT_HUB_URL = "https://azuremarketplace.microsoft.com/en-us/marketplace/apps?search=Downpipes%20Audit%20Feed";

// sentinelContentHubLink is the from-Content-Hub alternative to the codeless-connector method line, shown only
// on the Microsoft Sentinel tile (not every pull vendor): a customer who wants the marketplace install rather
// than running the ARM template by hand. The ARM template stays the scriptable, no-marketplace path and its
// method line is unchanged and still shown alongside this.
function sentinelContentHubLink(): HTMLElement {
  return h(
    "p",
    { class: "field__hint measure", style: "margin:0" },
    h("a", { href: SENTINEL_CONTENT_HUB_URL, target: "_blank", rel: "noopener noreferrer", class: "linklike" }, "View in Microsoft Sentinel Content Hub"),
    " installs the connector, analytic rules and workbook with no ARM template to run by hand.",
  );
}
...
export function renderSetupBody(engine: EngineClient, v: Vendor, gate: ManageGate, snap: ConfigSnapshot, reload: () => void): HTMLElement {
  ...
  switch (v.kind) {
    ...
    case "pull": {
      // Sentinel-only: the Content Hub link is the primary, expected path (plan section 2A), shown before the
      // ARM-template method line, which stays as the alternative for a customer who wants the scriptable path.
      // Every other pull vendor (Exabeam, Cribl) is unaffected.
      if (v.name === "Microsoft Sentinel") body.appendChild(sentinelContentHubLink());
      body.appendChild(methodLine(v));
