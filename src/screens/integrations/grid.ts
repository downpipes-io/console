...
// only additions are the category section headers, the teal "Auto-parses" tag, and the small connected-summary
// strip.
//
...
// up. app.ts paints the screen and resolves the identity in parallel, so every gate on the first paint runs
// with caller() still null and fails closed (screens/common.ts canDo/canCap, which is the correct thing for a
// GATE to do). A boolean gate has no way to print that state honestly: it would show the fail-closed default
// as the SETTLED FACT "Owner" to an operator who is in fact the owner, a refusal asserted where the truth is
// "not read yet", with no reason and no remedy, self-clearing only once the identity lands and the screen
// re-renders. Every sibling gate in this console keeps the two apart (notifications gateReason, common.ts
// gateReason/capGateReason, the credentials and security-centre predicates all carry a null-caller branch that
// says the engine has not reported the role), so this grid does too.
//
// ManageGate therefore carries "unresolved" beside "allowed" and "refused", the chip reads "Checking" rather
// than "Owner", and the accessible action word is "checking your permissions" rather than "owner only". The
// gate itself still fails closed: nothing is enabled here that a resolved caller could not already do.
...
export function renderIntegrationGrid(
  ...
  // flag would over-promise every tile as addable; the panel gates per vendor, so this aligns the grid's
  // promise with the panel's gate. The gate is three-valued, so one that has read no identity yet reports
  // that rather than printing a role refusal it cannot back (see ManageGate above).
