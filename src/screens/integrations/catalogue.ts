...
// with no field mapping. For SIEM that is Splunk and Datadog alone; for monitoring it is the four incident
// tools with dedup/auto-resolve and the Datadog OTLP metrics push.
...
export interface Vendor {
  ...
  // disagree.
  //
  // WHY IT LIVES ON THE VENDOR AND NOT ON THE FORMAT. Keying the Splunk note off `pushFormat === "splunk-hec"`
  // would also catch CrowdStrike Falcon Next-Gen SIEM, which takes that same format over the same http sink
  // but issues a plain BEARER token: the Splunk instruction would render on the Falcon form and tell the
  // operator to prefix a Falcon token with the word Splunk. Keyed off the vendor instead, Falcon carries no
  // scheme rule and Splunk carries an enforced one.
  credScheme?: string;
...
export const CATALOGUE: Vendor[] = [
  ...
  { name: "Microsoft Sentinel", mark: "microsoft", category: "SIEM", kind: "pull", method: "Codeless connector: run the ARM template once and it polls us into a typed table.", docSlug: "microsoft-sentinel" }, // docSlug pins /integrations/microsoft-sentinel explicitly, matching the crowdstrike-style override; the Content Hub walkthrough lives on the day-2 audit-feed page and is linked separately from the pull panel below (panels.ts).
