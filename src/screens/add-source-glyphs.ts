...
// account catalogue read IDENTICALLY: both call the same rule, refusalText (components/error-view.ts),
// which uses the engine's own sentence when the throw carries one and the reviewed sentence for the
// classified kind when it does not. A private copy of this transform here would let the two attach
// surfaces disagree about the same refusal, so there is deliberately only one.
