...
// whole form to toggle the visible variant) lives here.
//
...
export function destinationForm(engine: EngineClient, fctx: FormContext, opts: DestinationFormOpts): HTMLElement {
  ...
  function setProvider(p: Provider): void {
    ...
    // deliberately: the narrow form would silently hide the whole endpoint block for any provider that is
    // neither R2 nor S3, leaving the operator with a form that has no fields in it. TypeScript cannot catch
    // that: the comparison still compiles.
    ...
    // REGION AND ADDRESSING ARE INERT ON AZURE BLOB, measured rather than assumed: azure-blob.ts,
    ...
    //
    // Google Cloud Storage is included because a GCS bucket created WITH per-object retention answers the
    // engine's Object-Lock probe 200/Enabled, honours a COMPLIANCE lock header, and then refuses a delete
    // inside the window with 403. Azure is included because its two primitives (a policy that is separately
    // unlocked or locked, plus an independent legal hold) map exactly onto the one mode and one window this
    // form collects: the two vocabularies name the same two guarantees, and the mode is a request header
    // chosen per write rather than a container property that has to be inferred. Azure Blob Storage enforces
    // this as a version-level immutability policy on a container or account configured for it: governance
    // maps to unlocked, compliance maps to locked, and the legal hold is simply never set.
    //
    // What decides the outcome is the ENGINE'S LIVE PROBE: a store that answers that this bucket cannot
    // enforce a lock gets the save refused, naming the bucket rather than the provider. R2 is the case
    // that proves the control is not merely permissive: measured, R2 over its S3 endpoint answers 501
  ...
  //
  // The key ceremony and the new-downpipe wizard both guard their work, and this form does too: it holds a
  // value the customer cannot get back. A secret access key is shown once, when it is minted, by R2 and by
  // S3 alike. Without this guard, a rail click, the command palette, or the setup strip's own Continue would
  // tear the form down with no prompt, and re-typing would not be an option: the customer would have to mint
  // a new key pair. This is also the FIRST-RUN form, reached from a setup strip whose whole job is to
  // move people between screens, so the navigation that would destroy the work is the one the product
  // suggests.
  //
