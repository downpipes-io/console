...
//
// The provider grid (./grid.ts) is the picker: the coordinator opens presetForm or samlForm directly from a
// tile, so this module exposes presetForm only and does not own the catalogue fetch or a picker.
...
function presetFields(preset: IdpPreset, wrap: HTMLElement, setGuideConnId: (id: string) => void, taken?: TakenConnIds): PresetFieldHandles {
  ...
  // The connection id is validated by connIdError (./shared.ts), shared by both add forms: it mirrors
  // the engine's CONN_ID_PATTERN (1 to 64 chars, lowercase alphanumerics with internal hyphens only, not
  // starting or ending with a hyphen) AND refuses an id this engine already holds, the same collision the
  // engine refuses at submit.
  ...
  if (preset.requiredVars.length > 0) {
    ...
    for (const v of preset.requiredVars) {
      ...
      // so requiring a value here would block the sign-in-only setup the guidance tells the operator to do.
  ...
  // PUBLIC CLIENT IS AN OIDC-ONLY CHOICE: on an OAuth2 tile the engine cannot save it. The preset create
  // path always attaches a secretRef, and validateOauth2 derives pkce_public ONLY when secretRef is absent
  // (engine/src/admin/idpconn-validators.ts:339), so a ticked OAuth2 proposal reaches validateSecretRef as a
  // CONFIDENTIAL client holding a pkce-public ref and is refused with "a confidential client secretRef.mode
  // must be 'do-plaintext'" (engine/src/admin/idpconn-validators.ts:105), a reason that never mentions public
  // clients. There is no value the console could post instead, so the checkbox is disabled here rather than
  // left to fail at submit.
  ...
  // Group-level doc link: the public-client checkbox and the write-only client-secret field below are one
  // secret-mode decision, so the link explaining confidential versus public lives on the group. It stays
  // visible when ticking the box hides the secret field, so the guidance never disappears.
...
export function presetForm(engine: EngineClient, preset: IdpPreset, reload: () => void, taken?: TakenConnIds): HTMLElement {
  ...
  const buildPayload = (): IdpPresetCreate | null => {
    ...
    if (!isPublic && secretField.value() === "") {
      // A CROSS-FIELD console rule (secret required unless the public-client box is ticked), firing on an
      // EMPTY box, so it is setError and NOT refuse. It is the required-and-empty state under another
      // name: no value was examined, so a form-rejected row would assert an inspection that never happened, and
      // idp-secret is not a member of the closed field vocabulary for exactly that reason. The operator is looking
      // straight at the message and the remedy is in it.
    ...
    return {
      ...
      // proposal.kind with no presetId lookup at all (engine/src/admin/idp-test.ts). Omitting it here would
      // fail Test connection for every OIDC/OAuth2 preset with "Unknown connection kind (none supplied)".
      kind: preset.kind,
      vars,
      id: idField.value(),
