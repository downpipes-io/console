...
// resolveId is keyFromId PLUS the drift record, and it is the RESOLUTION site, the only place the
// unknown-preset state is visible. Recording it at the render site instead would be unsatisfiable: MARKS is a
// TOTAL map over the closed ProviderKey union, with completeness enforced by tsc, so for any non-null key,
// MARKS[key] is always defined and a "no mark for this key" test at render can never fire. NULL IS THE DRIFT
// CASE: a raw SAML connection does NOT resolve to null (kind "saml" returns "saml", and any id containing
// saml/oidc/oauth maps to a real key), so null is reached ONLY for an id this build cannot resolve.
//
...
export function providerLogo(key: ProviderKey | null, size = 30): SVGSVGElement {
  ...
  // here is unsatisfiable and would record nothing while appearing to guard against drift.
