...
// remove controls (the gate is mirrored, never the control: a non-owner sees them disabled-with-reason). It
// imports the shared leaf (./shared.ts), so it never imports the forms section (which would form a cycle).
