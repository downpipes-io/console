...
//
// WHY THIS EXISTS. The engine serves POST /admin/idp/connections/cert, gated on the owner-exclusive
// keys.ceremony and routed through the `idp-conn-cert` dual-control owner action. The only other path through
// a certificate rotation is remove-and-re-add, which bumps the idpEpoch (every session signed in through the
// connection ends) and, under dual control, queues TWO approvals with a window in between where nobody can
// sign in through the provider at all. A customer never runs a terminal, so this control gives the rollover a
// portal path rather than leaving it reachable only as an HTTP endpoint.
//
...
async function runRollover(
  ...
  // THE CAP IS ON THE RESULTING PINNED SET. On Append the engine validates the EXISTING set concatenated
  // with the paste, so seven pinned plus two pasted is nine and is refused once it exceeds the limit; this
  // check adds the pasted count to the pinned count rather than counting the paste alone. The field's own
  // hint states that on Append the total counts the ones already pinned.
  //
  // WHY THIS GUARD MATTERS MORE THAN AN EXTRA ROUND TRIP. The engine's refusal for an over-capacity array is
  // the wrong-shape sentence ("idpSigningCerts must be a non-empty array of PEM X.509 certificates"), which
  // does not name the capacity, so an operator rolling a signing certificate over against their provider's
  // cut-over deadline could read a capacity refusal as their certificates being malformed. This refuses
  // before the request, with the pinned count and the remedy that actually lowers it.
  // READ DEFENSIVELY: the type says conn.idpSigningCerts is there, but the wire is what decides, and an
  // absent array must degrade to "count nothing pinned" so the engine still gets the last word. A false
  // negative here costs one round trip; a throw costs the operator the control entirely.
  ...
  // idp-connection-cert-rollover, so the row names the operation the operator is actually blocked at. Never
  // move this ahead of the engine's list.
