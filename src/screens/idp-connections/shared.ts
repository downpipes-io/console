...
// form a cycle): the list section and the forms section each import one way from here.
...
// UNDEFINED IS IN THE UNION DELIBERATELY: a caller that does not pass the set must land on "I do not know"
// rather than throwing inside the validator, which is what a non-optional parameter would do the moment a
// caller invokes the form with the set omitted.
...
// form). The engine refuses a collision at engine/src/admin/idpconn.ts:59, and it does so at SUBMIT:
// posting the same id twice returns 200 {ok:true} and then 200 {ok:false, reason: 'a connection with id
// "okta-prod" already exists'}, so without a client-side check the operator loses a filled form to a value
// the console could have refused as they typed it.
//
...
//
// The shape half mirrors the engine's CONN_ID_PATTERN. The collision half applies only when `taken` is a
// real set; when it is null the console says nothing about collisions, because it does not know, and the
// engine's own refusal still arrives at the form verbatim.
...
// the denominator splitPems itself does not track.
//
// splitPems keeps only what its regex matches end to end, and drops everything else: a block whose END line
// an editor has mangled (a wrapped line, a stray character, a truncated paste) does not match, so the block is
// silently discarded and the submission can be SMALLER THAN THE PASTE, with nobody told. At the next
// certificate rotation, weeks later, sign-in breaks against a cert the IdP is now signing with and the console
// never stored, and the SSO failure the engine records points at the KEY, the wrong end of the problem
// entirely.
//
// Counting BEGIN markers is the honest measure of intent: an operator pastes N certificates and each carries
// one. A paste with more BEGINs than blocks lost the difference. Pure; the certificate bodies are never read.
