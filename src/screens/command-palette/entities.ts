...
// ---------------------------------------------------------------------------
//
// This extends the SAME entity index the palette already owned (downpipes by name + run id)
// with four more entity providers, each reusing the existing fuzzyScore matcher, the
// BuiltItem/buildOptionRow row anatomy and navigate(), so there is no parallel search UI, and
// covers global search across runs, audit, destinations and credentials:
...
// DOM. The DOM-side fetch + render that consumes it lives in ./overlay.ts.
