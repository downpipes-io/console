...
// provider's add wizard. The whole catalogue is visible at a glance, and the detail for the chosen one
// mounts in a panel below the grid.
//
...
export const idpConnectionsScreen: Screen = defineScreen({
  ...
  render(_ctx: ScreenContext): HTMLElement {
    ...
    // ONE posture line (design-system.md 7a): the standing facts as quiet dot+phrase items. The counts fill
    ...
    function loadAll(): void {
      ...
      void Promise.allSettled([eng.idpPresets(), eng.idpConnections()]).then(([presetsRes, connsRes]) => {
        ...
        // account that genuinely has none, so this records the ambiguity rather than staying silent about it.
