...
// It imports the shared leaf (./shared.ts) only, so it never imports another panel module (which would form a
// cycle).
//
// House rules: redaction-safe one-liners; status by hue + shape + label.
...
function renderHistoryTable(rows: NotifyHistoryEntry[]): HTMLElement {
  ...
  const handle = dataTable<NotifyHistoryEntry>({
    ...
    facets: [
      ...
      // Severity and the test flag are orthogonal, so they keep narrowing (AND).
