...
// delivery without sending real alert content; fail-open delivery is the engine's concern. It imports the
// shared leaf (./shared.ts) only, so it never imports another panel module (which would form a cycle).
//
// House rules: no-custody (a channel carries the customer's own destination, never a key or value);
// disabled-with-reason, never hidden-then-403.
...
// buildChannelFormFields constructs the field handles the channel form carries.
function buildChannelFormFields(existing: NotifyChannel | null, lockedKind?: ChannelKind): ChannelFormFields {
  ...
  // `required` is DYNAMIC here: on the edit path it is literally false, and field.ts only validates a
  // required field on blur, so the explicit validator is what enforces the engine's 512-character bound
  // on both the create and edit paths.
  ...
  // on edit.
  //
  // THE 256-CHARACTER CEILING IS THE FIELD'S, not only the engine's: without a validator here, field.ts
  // has no rule to run on blur and validateForm has none to run on Save, so an over-length username
  // would be taken client-side and only refused by the engine as a generic channel error. The explicit
  // validator enforces the bound the catalogue publishes for this field, in the same idiom as the
...
// the kind / teams-mode changes, and primes the initial render. jsmGroup/servicenowGroup are the
// ADDITIONAL credential fields jsm/servicenow show alongside the (unchanged) url destination
// field, toggled here in the same refresh so a kind change never leaves a stale group visible.
...
// modal onClick boolean (true closes the modal, false keeps it open with input intact).
