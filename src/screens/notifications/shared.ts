...
// value); textContent on every server string.
...
// here at all: on the edit path the field's `required` is false, so without a validator here the
// field would carry no rule and the bound would go unenforced in the browser.
...
// accept a username the engine would refuse. Stated here for the same reason as the bound above:
// without a validator, the field's `required` alone gives field.ts no rule to run on blur, so an
// over-length username would reach the engine to be turned away there instead of at the field.
...
// listed: leaving one out would mean a customer could not build a rule targeting it individually. See
// NotifyEvent (../../api/types/notifications.ts) for the per-event severity source.
...
// beside the toolbar, replacing the per-row "Read only" hints and the bottom
