// The add/edit-rule modal's internals. Three cohesive parts: building the form controls (scope,
// severity, the event checklist, the channel checklist, digest, enabled), wiring the inline
...
//
// House rules: disabled-with-reason, never hidden-then-403; honest copy.
...
// checklist appear only when their mode selects them.
export function buildRuleFormControls(existing: NotifyRule | null, channels: NotifyChannel[], downpipes: DownpipeState[]): RuleFormControls {
  ...
  // Group-level doc link: the event checklist is a raw checkbox group, so the one link
  ...
  // Group-level doc link: the channel checklist is a raw checkbox group; the one link
