// The custom role builder (build-contract section 1, the composable-roles surface). It is the
...
// action, the catalogue list and the access gate in ./roles-builder/preview.ts. Splitting the file this way
// keeps its public surface byte-identical.
//
...
// ---------------------------------------------------------------------------
// The screen descriptor (build-contract 2.1). It owns the builder route under the Roles area and
  ...
  actions: [
    {
      ...
      // ROLE_CAPABILITIES[c.role] directly would floor a NAMED custom role to the "viewer" its role field
      // carries, which would let a custom role that genuinely holds access.policy open the builder from
      // the rail but not find it in the palette.
    ...
    const load = (): void => {
      ...
      void engine
        ...
        .catch((err) => {
          ...
          // A 404/501 means the engine's custom-role endpoint is genuinely not wired yet, and composing over
          // an empty catalogue is the honest interim. But a 500 is the engine live and broken, and a network
          // throw is unreachable: in both cases the catalogue was NOT read, and the customer must not be told
          // the feature is merely "pending the engine" while a real fault hides it. Route a genuine fault to
          // the honest block error with Retry; keep the pending-engine note only for a real not-wired build.
