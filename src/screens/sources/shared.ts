...
// the screen reuses. Kept separate from the screen module so the per-section render
// modules (the tiers, the add-source wizard, the account catalogue, the token
...
// items does not make the page a wall. A large group
...
export function selectableSourceList(opts: {
  ...
  // of one-line summaries on load. A
  ...
  // EVERY product group renders with the same collapsible chrome (R2, D1 and the rest
  // consistent with namespaces), so the page is a tidy uniform set
...
//
// Without this, the transport's raw throw shape reaches the operator around the engine's words, for
// example a status code sitting past the full stop, a message naming nothing to change, or an
// internal classifier token surfacing alone.
//
...
// the same rule the destination form and the licence and update verbs go through, so a screen never
// keeps its own near-duplicate copy of this logic.
//
