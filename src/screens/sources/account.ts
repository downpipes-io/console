...
// replace, remove) lives behind one disclosure here. It imports the shared leaf, the
// token-entry form and the deploy-token help.
...
// stays a small named unit while sharing one set of closures. Nothing here is new state:
// it is the same locals, named once.
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      void engine
        ...
        .then((res) => {
          ...
          // armed the engine answers 202 and stores nothing. Without this branch, the queued answer would be
          // cast to a DiscoveryStatus and the handler would simply refresh, repainting the OLD selection with
          // no toast and no error, so a save the engine had accepted would read as one that silently reverted.
          //
...
function renderAccountListings(ctx: AccountTierCtx, body: HTMLElement): void {
  ...
  // Two completely different faults render, three lines below, as two
