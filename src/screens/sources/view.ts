...
// built from one discovery result). Kept separate from the screen coordinator for
  ...
  const load = (): void => {
    ...
    void Promise.allSettled([eng.discoverSources(), eng.listDownpipes()])
      .then((results) => {
        ...
        // on at run time; the listing shows them FIRST, in an error state.
...
// sortable, filterable table rather than a ragged list. Kept prominent, never a bare
// collapsed count. Returns null when
...
function renderCatalogue(engine: EngineClient, ctx: CatalogueContext, refresh: () => void, polls: PollHooks, coverageIncomplete: boolean): HTMLElement {
  ...
  // load: no walls of open lists, no loose token box. ----
  if (found.tokenPresent) {
