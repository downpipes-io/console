...
// ENGINE's own account; bindings cannot cross accounts. Living here lets the add action,
// the drift re-attach and any future caller share the wizard without a cycle.
...
// attached are filtered out. Uses the shared primitives (selectableSourceList /
// changeBindings / attachTokenHelp / wranglerStanza) so behaviour and safety checks stay
// consistent, with only the wizard chrome added on top.
