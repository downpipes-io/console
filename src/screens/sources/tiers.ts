...
// tick, share a schedule, create) with its owner-level detach. Kept separate from the
// screen module for size; it imports the shared leaf, the add-source wizard (drift
// re-attach), the deploy-token help, and the bulk-summary/friendly-name helpers the
// Downpipes coordinator already exports.
...
// recovery is one step from where the problem is seen, rather than letting a broken
...
// navigation hop: ticking sources, leaving for the manual-attach page, and returning does
// not lose the selection. The draft holds only binding names + types, never a secret.
...
// whole batch writes to (first = primary); empty follows the default destination. PURE of
// the DOM (the caller drives button state + toasts off the outcome). Exported for the sources
// validator; the outcome shape is lib/bulk-create's, so reportBulkOutcome reads it the same way
...
// protect (tick, share a schedule, pick the destinations, create). This is the picker
...
export function attachedTier(engine: EngineClient, attachable: Array<{ name: string; type: ProtectType }>, found: SourceDiscovery, ctx: TierContext): HTMLElement {
  ...
  //
  // The read is awaited at the click and a failure is said out loud next to the button with a retry,
  // because a silent failure here has real teeth: if the read fails, or is still in flight when Protect
  // is pressed, the batch quietly follows the single default destination and the operator's replicas are
  // never created. Either way the operator asked for N copies of every source and got one, and would not
  // find out until the day they needed the copy that was never made. The read itself goes through the one
  // engine seam, so its failure is already carried into the support pack as a closed-class engine-call
  // fault on the sources screen; the warning here gives the operator their own chance to notice.
  ...
  if (opGate) createBtn.addEventListener("click", async () => {
    ...
    //
    // This is its own record rather than the read's engine-call row, because that row is useless for this
    // question twice over: it cannot say the operator went on to create anyway (a read that failed and was
    // then retried successfully leaves the same row), and it coalesces, tuple for tuple, with a 500 from
    // discoverSources or listDownpipes on the same screen, so an estate that silently lost its replicas and
    // an estate that had a transient discovery blip would be one row. This row exists only when the fan-out
    // was actually skipped, and an operator who deliberately chose the default never produces one.
    ...
    void engine
      ...
      .catch((e) => {
        if (isUnauthorised(e)) {
          ...
          // reported as one in this modal, which validate-source-detach.ts pins, and the re-attach handler
          // above already follows the same shape: restore the control, then leave without an inline
          // message.
