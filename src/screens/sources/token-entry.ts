...
// account tier reuse. Kept separate so the connect-first state, the account tier
...
// onboarding ticket there is ("it said verified, but nothing shows"): the token is valid but its scopes
// admit nothing, so nothing downstream (the source catalogue, the destination bucket list) can populate.
// This is not a success, so it renders as a WARN that names the cause, letting the operator fix the
// scopes instead of raising a ticket about an empty screen. Exported for the validator.
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      void engine
        ...
        .then((res) => {
          ...
          const st = res.value;
          input.value = "";
          // The engine took the token, and the number of accounts it can then see decides whether this
          // operator is about to have a good day. Zero accounts on an accepted token means the token's
          // account-read scope is not what the customer expects, and it is the single most common
          // onboarding report.
          //
          // Only whether it was zero is recorded, not the count: an account count is a coarse fact about
          // the customer's estate, and the discriminator the ticket needs is the zero, not the number.
        ...
        .catch((e) => {
          ...
          // The engine REFUSED the token set. The refusal prose is rendered at the field and is never
          // recorded: it is Cloudflare API text the console does not own, and a classifier that guessed a
          // closed fail class out of it would quietly mislabel the day the engine reworded itself. The FACT
          // of the refusal rides; its class belongs to the engine's own discovery state, which knows it
