// Overview / dashboard: the post-onboarding landing and the read-only "observe and reassure" half
// of the console: engine health, configuration readiness (PRESENCE, not "valid"), licence tier
// (fail-open, never a gate), update availability, the fleet health roll-up with freshness against
// cadence, the recovery-posture summary, and the no-custody + Access trust telemetry as REAL chips
// (reading the verdict, never hardcoded).
//
...
//
// RBAC is a client MIRROR of the engine's server-side enforcement: the one write this surface
// owns is the fleet drill (Operator+); a Viewer sees it absent-with-reason. Diagnosis lives here;
// the fix (Run now / Edit / Restore) hands off to the area that owns it (Downpipes / Runs /
// Restore), which the rows and needs-me items deep-link to.
//
// Built against the health/status/licence/updates/history + downpipes + POST /drill contract. The
// durable drill-evidence + audit store is a separate engine dependency; the recovery panel says so
// rather than faking a history.
//
...
// recovery, executive, build, view), kept there to stay under the size budget while the
// public surface here stays unchanged for external callers.
...
export const overviewScreen: Screen = {
  ...
  actions: [
    ...
    },
    {
      ...
      // POST /admin/drill and /admin/drill-all (router-pipelines.ts). callerCan resolves a named
      // custom role's real capability set rather than a role-NAME list (operator/approver/owner),
      // so a role such as restore-operator that holds drill.run without being named on the ladder
      // still sees the command, matching the button beside it.
      when: ({ caller: c }) => c !== null && callerCan(c.role, "drill.run", c.customRole ?? null),
    },
  ],
  render() {
    // The question this screen answers: "is everything healthy, and what needs me?" Eager: the
    // protection lead, the tiles, the fleet table, the attention list. Demoted: recovery posture
    // (disclosure), the topology/cost previews (a quiet link row to the owning screens).
    const engine = requireEngine();
    const root = h("div");
    if (!engine) return root; // requireEngine routed to onboarding

    // GUIDED SETUP: while setup is incomplete, this screen is the checklist and NOTHING else, the
    // same one-decision gate every other screen honours: Overview must not render a dashboard of
    // tiles, chips and tables around the setup card. The full dashboard mounts below only once the
    // facts say complete (fail-open: unknown renders it).
