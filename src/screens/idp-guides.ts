...
//
// The engine's preset registry (oidc-presets.ts) owns the TECHNICAL defaults (issuer, scopes, claim names);
// these guides own the human "what do I click" layer. They are keyed by the engine preset id, so a guide
// lines up with its form.
//
// Pure data and a lookup; no I/O, no DOM (the screen renders it).
