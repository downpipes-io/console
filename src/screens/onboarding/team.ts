// The onboarding team step (optional; honest degrade until the role store ships): the read-only
// role ladder and the Owner-gated grant-by-email form, with the copyable enrolment link the
// engine mints. It imports only the shared leaf (./shared.ts) and never a sibling deck module, so
// no cycle forms.
...
// ============================================================================
// Step 5: Invite team / assign roles (optional; honest degrade until the role store ships)
// ============================================================================
...
export function renderInviteForm(engine: EngineClient, onGranted?: () => void): HTMLElement {
  ...
  // the same capability the main Access roles table gates on). This mirrors that capability, not the owner
  // ladder: an access-admin's whole job is managing people, and gating this on owner-only alone would
  // under-grant them a control the engine allows. A caller without roles.write sees the form
  // absent-with-reason.
  ...
  // POST /admin/roles is a STEPUP_SUBS member and setRole goes through gatedFetch, so on a cookie-borne
  // session "Grant role" opens a browser passkey sheet. Like the key install, this call site has no
  // confirmation modal, so the announcement below carries the warning on its own rather than relying on the
  // confirmation-copy gate to have checked it.
  wrap.appendChild(
    h(
      "p",
