// The onboarding CAROUSEL: the rotating card deck, and the production realisation of the
// first-run flow. The whole flow renders as a single full-bleed deck of bite-size cards grouped
// into five chapters, rather than a side-railed single step. Forward movement is GATED on the
// action cards (prerequisites, connect, generate, install) so the operator cannot skip them.
// Every safety-critical operation (the in-browser key ceremony, the engine install, the
// no-custody guarantees) reuses shared helpers and step renderers; only the chassis and
// navigation live here.
...
// The chassis below mounts and navigates them.
...
export function renderCarousel(ctx: ScreenContext): HTMLElement {
  ...
  // LEAVING WITH LIVE, UNINSTALLED KEY MATERIAL IS A SILENT ONE-WAY DOOR, and this is the only place in
  ...
  //
  // Nothing else warns about that. The generate card's own replacement warning ("a brand-new set that will
  // not match the identity.key or recovery sheet you saved") is conditioned on the engine ALREADY holding
  // keys, so it does not render in exactly this case, which is the one where the mismatch is invisible until
  // a restore is attempted, possibly months later.
  //
