...
// shells). These are pure builders, kept in their own module to stay under the size budget; each
// takes the live callbacks it needs (chapter jumps) and returns the elements plus the handles the
// navigation closures drive. No behaviour lives here.
 ...
 *  lazy mount, the nav buttons, and focus, which read closure state.
 *
...
export function applyLayout(
  ...
  refs.els.forEach((el, i) => {
    ...
    // The training walk's anchor follows the card the learner is actually on rather than the deck as a
    // whole. data-tour-id="onboarding-deck" on the deck is the right subject for a rail beside a desktop
    // screen and the wrong one on a phone, where it is very nearly the whole viewport and would leave a
    // learner unable to tell what was being pointed at. Exactly one card carries this at a time, so the
    // spotlight's querySelector cannot resolve a hidden one. Inert on the genuine console, like every other
    // data-tour-id.
