// The first half of the onboarding card deck data: chapters 0 (Connect) and 1 (Your keys). This
// module holds a subset of OB_CARDS so each data module stays under the size budget;
// carousel-cards.ts concatenates this part with the second part so OB_CARDS keeps one order and
// one set of contents.
...
export const OB_CARDS_CONNECT_KEYS: CardDef[] = [
  ...
  },
  {
    ...
    mount(host, nav) {
      ...
      //
      // The second item asks whether an archive destination is available, worded around the fact
      // that downpipes writes to four kinds of store (Cloudflare R2, any S3-compatible store,
      // Google Cloud Storage, Azure Blob Storage: PROVIDERS in screens/destination-form-fields.ts,
      // providerForEndpoint in engine/src/dest/provider.ts), and the engine's R2 binding is
      // optional and commented out in its own wrangler.toml.
      //
      // The item stays required, because what it is really asking is whether there is anywhere
      // for the archives to go, which is the one prerequisite a backup product cannot proceed
      // without, and it is still something only the customer can answer. R2 stays in the how-to as
      // the worked example, because it is the in-account option and the shortest path, and the
      // other three are named beside it so nobody reads it as the only one.
  ...
  },
  {
    ...
    mount(host, nav) {
      ...
      // This runs GET /admin/health (the CONSOLE_ORIGIN diagnostic), then the authenticated status,
      // then the honest sign-in verdict as one line.
      const doConnect = async () => {
        ...
        try {
          ...
          if (!hres.ok) {
            // The engine ANSWERED and reported itself not healthy. It is reachable, so this is not a
          ...
          // The health probe itself did not come back. The engine NEVER SAW this request, so no
        ...
        try {
          ...
          void resolveAccessVerdict(client)
            ...
            .catch(() => {
              // The sign-in verdict line never resolved, so the wizard renders no verdict at all and the
              // operator cannot see whether Access is enforced on the engine they just connected.
              recordOnboardingStep("connect", "access-verdict-unresolved");
            });
          enable();
        } catch (err) {
          // THE ACCESS SIGNATURE. Health passed (healthReachable is true: the unauthenticated probe got
          ...
          // passes but it keeps kicking me to sign-in": from the engine's side the only trace is a request it
          // either never received (CORS) or refused at the edge (Access), and neither trace is in the bundle.
          //
          ...
          h("div", { class: "cx-callout cx-callout--trust" },
            ...
            h("div", {},
              ...
              h("ul", { class: "cx-callout__b", style: "margin:0;padding-left:var(--space-4)" },
                ...
                // in both. Stating "five small key files" as a flat number would be wrong for whichever
                // posture leads the fork, so the copy below states the base count and the addition instead.
  ...
  },
  {
    ...
    mount(host, nav) {
      ...
      // The docs link opens the pros-and-cons, supply-chain scenario and residual-risk page in a new tab
      // (the same page the recorded acceptance statements are quoted on, so the page IS the disclosure).
          ...
          h("div", { class: "cx-callout cx-callout--trust" },
            ...
            h("span", {},
              ...
              h("span", { class: "cx-callout__b" },
                postureLabel,
                // keys/posture.ts adds an operational key over POST /admin/keys/add-operational and
                // keys/posture-switch.ts removes one over POST /admin/keys/break-glass-only; neither runs a
                // key ceremony or touches the break-glass key. Sending a customer to a re-key they do not
                // need would leave them with a new key set that does not match the identity.key and sheet
                // they saved.
      ...
      const renderFork = () => {
        ...
        const renderAcceptance = (posture: PostureChoice) => {
          ...
          confirmBtn.addEventListener("click", async () => {
            ...
            // fault, but the failure is not swallowed: client-keys.ts acknowledgePosture never throws (it
            // resolves { ok: false } on a transport or non-2xx failure), so the result is read and acted on
            // below rather than discarded, and a failure to record leaves the operator told about it.
            //
        ...
        const fork = h("div", { class: "ob-posture-fork" },
          ...
          // the one that leads is what a customer reads as recommended. Offline-key-only leads, and the
          // asymmetry of the mistakes is why: an accidental strict posture is RECOVERABLE, since the Keys
          // screen can add an operational key later without a re-key and without touching the break-glass
          // key. An accidental operational key is NOT, because, as its own acceptance statement says,
          // removing it later does not protect archives already sealed while it was present.
          buildPanel(
            "break-glass-only",
            "Offline key only",
  ...
  },
  {
    ...
    mount(host, nav) {
      ...
      // demotes to a quiet "replace" control, so the continue path is never buried.
      ...
      // files, and advance.
      //
      // The intent marker is recorded BEFORE the ceremony runs, not after it succeeds, so a ceremony that
      // throws still leaves an audit trail distinguishable from one nobody ever started: an intent event
      // with no keys installed is a ceremony that did not complete, and no intent event at all is a
      // ceremony nobody started. It is best-effort and never blocks the ceremony.
      //
      // The file delivery is checked separately from the ceremony itself. The keys exist in this tab
      // whether or not the browser accepted the downloads, so a refusal must NOT be reported as a failed
      // ceremony. A refusal instead names the files that did not arrive and holds the operator on this card
      // with a Continue that leads to the per-file re-download controls.
      const onGenerate = async () => {
        ...
        try {
          ...
          //
          // The FALLBACK when no posture was recorded: an absent choice yields the STRICTER posture rather
          // than the weaker one. Nobody reaches here without an explicit, hash-recorded acceptance in the
          // ordinary flow, so this only governs the odd path (a restored tab, a skipped fork). Falling
          // strict is the safe direction for exactly the reason the panel order comment gives: the strict
          // posture can be relaxed later without a re-key, and the operational one cannot be undone for
          // archives already sealed under it.
          ...
          // The ceremony completed AND every file was saved. Recorded so its absence means something: the
          // failures of this step already have a more specific home (the capability-fault record, which
          // names the capability, the ceremony and whether the browser lacked it or refused it), so this row
          // is not duplicating them. What it adds is the denominator: a wizard whose generate step never
          // ended `ok`.
          ...
          // The ceremony produced NO KEYS. On a locked-down browser (no usable WebCrypto, or one that
      ...
      // keys", and surface a forward "Continue with the existing keys".
      const reframeForExistingKeys = () => {
        ...
        void statusEngine.status().then((s) => {
          ...
          // would otherwise pin the Overview checklist at "0 of N". Server-enforced + best-effort.
          ...
          // replace action rather than the prominent button.
      ...
      if (!ownerOk) {
        // refuseWithReason keeps the button focusable rather than writing `disabled` alongside
        // `aria-disabled`: `disabled` removes a control from the tab order, so an aria-disabled state on a
        // disabled button is announced to nobody. The visible hint below stays either way.
  ...
  },
  {
    ...
    mount(host, nav) {
      ...
      // signer public key is not stored in the archive. It is a PUBLIC key, so keeping it beside the
      // sheet gives an attacker nothing.
      ...
      // THE SHEET ALREADY ON DISK CANNOT KNOW ABOUT A SCHEME CHOSEN HERE. downloadCeremonyFiles delivers
      // recovery-sheet.txt back at the generate step through sheetParams(result) with NO custody argument,
      // and sheetParams omits the custody block entirely
