...
// mounts. They import only the shared leaf (./shared.ts) and never a sibling deck module, so no
// cycle forms.
...
export function renderInstallKeys(engine: EngineClient, result: CeremonyResult): { el: HTMLElement; onInstalled: () => void } {
  ...
  if (result.operational) {
    wrap.appendChild(
      h(
        "p",
        { class: "field__hint" },
        // posture-switch.ts removes the operational key later with no key ceremony at all (POST
        // /admin/keys/break-glass-only, break-glass key untouched), so the copy below points at that switch
        // rather than a re-key. It also states the part that matters most at this moment: switching later
        // does NOT retrieve archives already sealed while the operational key was present, which is the only
        // part of this decision that cannot be taken back.
  ...
  // but that gate judges the copy of a CONFIRMATION, and this call site has no confirmation modal at all, so
  // the announcement below carries the warning on its own rather than relying on the gate to have checked it.
  // The wording matches the standardised sentence used at the other gated-write sites, including the
  // dismissal half, because "nothing was installed" is the fact a customer needs before they cancel a sheet
...
// in-memory key material (the reference is dropped; the browser reclaims the bytes by garbage
...
// engineFaultOutcome is the TOTAL, PURE classifier for ANY failed engine call in the wizard. Total matters because
// two states that look like "the call never reached the engine" are in fact cases where the engine is up and
// healthy:
//
//   Cloudflare ACCESS served its login page   the operator's session lapsed. Access answers an authenticated call
//                                             with HTML, not a 401, and that is what it actually does. failResponse
//                                             shape-gates the body, recognises the Access page and throws the
//                                             dedicated marker; classifyError names the kind `access-redirect`; and
//                                             the console's own FAULT_BY_ERROR_KIND maps that kind to the
//                                             faultClass `auth`, so support is pointed at sign-in rather than at
//                                             engine reachability.
...
// flight, which is the console working as designed, not a refusal by an engine that was never asked. The ring
// makes the same exclusion at every other emit.
//
// It is exported because the wizard's OTHER swallowed reads (the resumed-configure key pre-check, the completion
// readiness read) classify by the same rule, so every engine-call failure in the wizard agrees on what happened.
export function engineFaultOutcome(err: unknown): WizardFault | null {
  ...
  switch (kind) {
    ...
    // THE CONSOLE'S OWN WORKER MANUFACTURED THE 500, so no request reached the engine. It is not
    // `engine-not-ok`, which would say the engine answered and refused and send support to the engine's logs
    // for a fault the engine never saw. It is not `engine-binding-absent` either: the binding is there, and
    // the remedy is the engine's deployment (deleted, throwing on boot, over its limits) or the console's
    // dispatch.
    ...
    //
    // stepup-required and access-reauth-required belong here too: onboarding performs two step-up-gated WRITES,
    // installKeys (POST /admin/keys/install, gatedFetch, and /keys/install is a STEPUP_SUBS member) below, and
    // the team step's role grant (team.ts, POST /admin/roles). On a cookie-borne session either can reach this
    // classifier, and the engine answered: the call needs a fresh identity check (a re-verify), an
    // engine-answered state, NEVER a lapsed session (so never "unauthorised").
    case "stepup-required":
    case "access-reauth-required":
    case "forbidden":
    ...
    // from the engine arrived and this kind does not establish that. No dedicated member is added for it,
    // because the wizard's outcomes are mirrored in a CLOSED vocabulary that the engine carries its own copy
    // of (lib/client-diag/vocab.ts CLIENT_DIAG_ONBOARDING_OUTCOMES and
    // engine/src/admin/client-diag-vocab.ts), so a new member is a cross-repo mirrored change. The gap is
    // written down here where the next reader of this switch will find it.
    case "console-fault":
      return "transport-error";
  }
}
...
async function installKeysHandler(
  ...
  try {
    ...
    if (fault === "unauthorised") {
      // The operator's session lapsing mid-ceremony is a different fault from an install the engine never
      // received, and it is recorded before the redirect takes the tab away. It arrives as a clean 401 or as
      // the Cloudflare Access login page, and the classifier names both the same way, so a lapsed session is
      // never reported as an unreachable engine.
    ...
    // AN INSTALL POST THAT NEVER REACHED THE ENGINE (transport, DNS, a CORS block, an engine that is down) is
    // invisible to the engine by construction: its own recordKeyCeremonyFailure fires only for requests it
    // actually received, so the console is the only witness and records the fact here.
    //
...
//
// Re-entrancy guard: the manual Check restarts the loop, but a prior scheduled
...
//
// Re-entrancy guard: the manual Check restarts the loop, but a prior scheduled tick chain
...
function startPollLoop(engine: EngineClient, parts: ReadinessCardParts): (cb: () => void) => void {
  ...
  // THE ENDING MUST NAME WHICH OF FOUR STATES HAPPENED. The catch leg below swallows a thrown tick and falls
  // through to the same exhaustion branch as a tick that succeeded and reported the keys absent, and those
  // four states have different remedies: (1) the engine answered forty times with the signer present and
  // break-glass ABSENT (the half-keyed engine, which router-keys.ts documents as a real reachable state); (2)
  // it answered forty times with BOTH absent (the install never took); (3) it was NEVER ONCE READ (every tick
  // threw; the engine was already gone when the card mounted); (4) it ANSWERED, AND THEN WENT QUIET (the
  // ordinary shape of an engine that disappears mid-wizard: installing the key secrets is itself a redeploy).
  //
  // THESE ARE PER-RUN, AND THE RUN IS WHAT start() RESETS. Carrying them across a Check-triggered restart
  // would let a fresh run's exhaustion message reuse a stale lastRead from the run before it, and coalesce a
  // new run's rows into the previous run's by tuple key so the pack over-reports how many times the engine
  // answered. They are reset in start(), not here.
  let everRead = false;
  let lastRead: StatusReport | null = null;
  // HOW THE MOST RECENT TICK ENDED, CLASSIFIED, and null when it read the engine cleanly.
  //
  ...
  // a lastRead that is minutes stale. A bare boolean recording only THAT the tick failed cannot separate the two
  // remaining cases either: an engine ANSWERING 500 ON EVERY TICK and an engine that was never reachable at all
  // would both read the same, asserting a reachability the console never tested when the engine's logs in fact
  // hold forty refusals.
  //
  // So the class is kept, not a boolean, and engineFaultOutcome is the classifier: the SAME total, pure mapper
  ...
  const renderStatus = (s: StatusReport) => {
    ...
    // The destination is not part of this step; it belongs on the Destinations screen, so
    // onboarding completes on the keys alone and does not list or poll it here.
    checklist.replaceChildren(...items);
    // KEYS are this step's job; the destination has its own later step (the
    // Destinations screen), so Continue must never hostage on it: locking /destinations until
    // after keys would deadlock the guided setup.
  ...
  const tick = async (myEpoch: number) => {
    ...
    try {
      ...
      if (s.signerConfigured && s.breakGlassConfigured) {
        stopped = true;
        // The poll SUCCEEDED. Recorded once (the loop stops here), so a pack whose readiness poll has no
      ...
      // A session that lapses during the two-minute wait tears the wizard down to the sign-in screen with the
      // keys possibly already installed. The row is written BEFORE the redirect takes the tab away, and it is
      // the same `unauthorised` member the connect and install steps already record: the obStep is what tells
      // the three apart. It covers the Access login page as well as the clean 401, which is the shape a fenced
      // engine's lapsed session actually takes.
      ...
      // is a false claim, and it is the sentence the operator quotes to support. It is also the sentence that
      // sends a customer to look at an engine that is fine when the thing with no binding is the console they
      // are reading it on.
    ...
    if (attempts < MAX && !stopped && myEpoch === epoch) window.setTimeout(() => void tick(myEpoch), POLL_INTERVAL_MS);
      ...
      // THE POLL RAN OUT. Forty attempts over about two minutes and the wizard sat on "Waiting". This is a
      ...
      //
      // WHICH exhaustion is the whole point: there are FIVE endings, the product of TWO questions:
      //
      ...
      //                      times, and every one of those refusals is in its own logs. A row must not assert a
      //                      reachability fault the code never tested.
      ...
      //                      it is up, and this is its own logs' problem, not a reachability one.
      ...
      if (lastTickFault === "engine-binding-absent") {
        ...
        // THE CONSOLE ANSWERED FOR THE ENGINE, and the engine never saw a single tick. Tested here, beside
  ...
  const start = () => {
    ...
    attempts = 0;
    stopped = false;
    // A RUN'S EVIDENCE IS THE RUN'S. "Check engine" re-enters this same closure, so these three must be
    // cleared here: without it, a fresh run's exhaustion branch could read a lastRead taken minutes ago from
    // an engine that has since gone, skip the leg that says "we never got an answer", and re-emit the
    // previous run's rows, which would coalesce by tuple key and merely bump a count, asserting the engine
    // answered more times than it ever did. The observation window is one run of the poll.
    everRead = false;
    lastRead = null;
    lastTickFault = null;
    epoch++;
...
function presenceItem(label: string, present: boolean, fieldName: string): HTMLElement {
  // Presence is not validity (build-contract; status.ts): this reports the env var is
...
export async function finishSetup(): Promise<void> {
  // Finishing lands the operator on Overview (the wizard-to-shell transition).
  ...
  // keys present). CRITICAL: AWAIT it before navigating: a fire-and-forget call would race the
  // Overview setup-state read, so Overview could see keysReady still false and show the
  ...
  if (engine) {
    // WAS THE COMPLETION EVER ACKNOWLEDGED? The wizard finishes and navigates either way, which is the
    // right call (an operator must never be trapped on the last card by a slow request), and it means the two
    // endings look identical to the operator and identical in the engine's own state. An ack that does not land
    // leaves the engine still believing setup is incomplete, so Overview would greet a fully configured
    // customer with the "nothing is configured" framing. The ack timing out and the ack being refused are the
    // same non-event on the engine, and neither is recorded there.
    //
