...
// ============================================================================
// Step 1: Connect (with the CONSOLE_ORIGIN diagnostic)
// ============================================================================
...
//
// TOTAL OVER THE VERDICT UNION: every verified method says which one it was (a live OIDC or SAML
// sign-in, or a recovery-code sign-in, each get their own line), and the only neutral line is for
// the states where nothing was established.
...
// resolveAccessVerdict reads the honest verdict. When whoami is available it returns verified
// (access) / token-fallback (token) from the real result; when it is not, it degrades to
// "session-present" (a 200 proves an authenticated session, but not the method) and never claims
// the green/amber distinction it cannot back. It refreshes the shared identity so the shell chip
// stays in step.
export async function resolveAccessVerdict(engine: EngineClient): Promise<AccessVerdict> {
  ...
  try {
    ...
    // ONE READ OF THE METHOD, AND THIS IS IT. accessVerdictFromCaller is the single classifier: it answers
    // honestly for all six methods (a live OIDC or SAML session and a recovery-code sign-in included) rather
    // than folding an unrecognised one into token-fallback, which would be the worst claim the console can
    // make about a customer's posture (implying the fleet runs on the shared break-glass credential when it
    // does not). It records the unknown-enum row itself for any method a newer engine adds, so the pack can
    // show that this surface was handed a value it could not read. WhoAmI carries every field Caller
    // requires, so it needs no re-shaping.
    return accessVerdictFromCaller(who, true);
  } catch (err) {
    if (isUnauthorised(err)) throw err;
    // whoami absent: prove the session another way. A 200 from status means
...
export function fileRow(name: string, dest: string, tone: StatusTone, onDownload: () => void, opts: { keepOffline?: boolean; tip?: string } = {}): HTMLElement {
  ...
  // must protect; routine to-engine files get a quiet neutral chip.
...
// recordCeremonyIntent records the key-ceremony INTENT in the audit log, best-effort and
// Owner-gated server-side. It records ONLY that a ceremony was STARTED, never a value or a fingerprint.
//
// It is called at the ATTEMPT, before the ceremony runs, not after it succeeds, so a ceremony that throws
// (no WebCrypto, a hardened browser) still leaves an audit trail distinguishable from one where the operator
// never pressed the button: an intent event with no keys installed is a ceremony that did not complete, and
// no intent event at all is a ceremony nobody started. The word "intent" is exact for a marker taken at the
// attempt.
//
...
export function recordCeremonyIntent(): void {
  ...
  void engine.recordAuditIntent("key-ceremony-intent").catch(() => {
    // No-op: the audit store may not be present, or the caller may not be Owner.
...
// recoverySheetHTML).
//
// A browser that refuses the blob tab says so and reports a closed-class capability fault into the support
// pack, instead of leaving the operator looking at a tab that never opened.
...
//
// It returns the names of the files the browser REFUSED, and it keeps going after a refusal rather than
// abandoning the rest of the burst, because a refusal is a delivery failure and not a ceremony failure: the
// keys already exist regardless of whether the browser accepted the download. The caller names the files
// that did not arrive and offers them again, and each refusal is a closed-class capability fault in the
// pack. The names returned are product constants, and they are used only to word the on-screen message;
// nothing about a file's CONTENT is read, held or recorded.
...
// discover months later, when they need it.
...
export function cardActions(opts: {
  ...
  if (opts.primary) {
    ...
    //
    // THIS CARD KEEPS `disabled` AND IT IS THE RIGHT CALL HERE, which is worth stating because most of
    // the console instead uses aria-disabled alone. `btn.disabled` is not decoration on this control, it is
    // the runtime guard the always-attached listener reads, and the gate flips mid-session through
    // enableGatedPrimary / disableGatedPrimary. Swapping it for aria-disabled would leave the listener live
    // behind an announcement, which is not a refusal.
    //
