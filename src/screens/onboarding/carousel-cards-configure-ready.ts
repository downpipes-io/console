...
// 4 (Ready), kept in its own module to keep each data module under the size budget.
// carousel-cards.ts concatenates the first part with this part so OB_CARDS keeps the exact
...
export const OB_CARDS_CONFIGURE_READY: CardDef[] = [
  // -- Chapter 2: Configure ------------------------------------------------
  {
    ...
    // naming a key that only the operational posture would install, on the card where the install happens,
    // would misname the posture for the customer who chose offline-key-only precisely so that no
    // operational key would exist. Naming a key the customer declined is the worst place to be imprecise
    // about custody.
    ...
    mount(host, nav) {
      ...
      if (result) {
        ...
        void engine
          ...
          .catch((err: unknown) => {
            // Recorded as its own step, "keys-precheck", not "install": this read installs nothing (no POST, no
            // key material, and the engine's own key-ceremony recorder has nothing to join against), so it must
            // not coalesce with the genuine install-POST row from steps.ts. It is classified by the same total
            // classifier every other site uses. A null class is an aborted read (a navigation or an unmount),
            // which is not a fault and is not recorded.
  ...
  //
  // THIS STEP MATTERS BECAUSE THE GATE IS CONDITIONAL ON A SECOND PERSON EXISTING. A restore approval must
  // come from someone other than the requester, so on a one-person estate no approval could ever be granted,
  // and requiring one unconditionally would make the apply unreachable in a configuration the product itself
  // calls valid.
  {
  ...
  // -- Chapter 4: Ready ----------------------------------------------------
  {
    ...
    mount(host, nav) {
      ...
      // named function so the failed card's Retry re-RUNS the check rather than navigating away
      // from the card.
      const load = (): void => {
        ...
        Promise.all([engine.status(), engine.preflight().catch(() => null)]).then(([s, pf]) => {
          ...
          if (whoamiAvailable() && c) {
            // Reads the ONE classifier (accessVerdictFromCaller) rather than the raw method: any method that is
            // not `access` or `passkey` -- a live OIDC session, a live SAML session, a recovery-code sign-in, and
            // any method a newer engine adds -- must not be reported as "token fallback", since that names the
            // shared break-glass posture for a method that never used it. accessVerdictFromCaller answers
            // honestly for all six methods and records the wire-anomaly row itself when it meets one it cannot
            // interpret.
          ...
          //
          // CLASSIFIED, not assumed: hard-coding `transport-error` here would assert the engine never saw the
          // request, but an engine that answered 500 to the readiness read is a live engine with a fault, and
          // its own logs hold the other half; telling support no such evidence can exist sends them away from
          // it.
