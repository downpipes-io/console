...
// The single BuilderState drives everything; a re-render of the preview is triggered on every change. It
// imports the shared leaf (./shared.ts) and the preview section (./preview.ts) one way, so it never forms a
// cycle.
//
...
export function builderBody(engine: EngineClient, existing: CustomRole[], reload: () => void): HTMLElement {
  ...
  // Restore an in-progress composed role, so browser-back / referrer-Back does not
  // discard an unsaved role; else start fresh.
...
function identityFields(state: BuilderState, existingNames: Set<string>, rerender: () => void): HTMLElement {
  ...
  const nameField = field({
    ...
    autocomplete: "off",
    onInput: (v) => {
      // state.name holds the value AS TYPED (trimmed only), never a lowercased shadow copy. The Save gate
      // composes its proposal from `state`, so a silently normalised copy would let an operator typing
      // "KV-Restorer" see the field's own red error (the pattern above is lowercase-only) while a save went
      // through under a different name than the one shown as invalid, with nothing relating the two on
      // screen. That would be worse than an ordinary display/storage mismatch, because the altered value is
      // the role's IDENTITY: the normalised name is the storage key
      // (engine/src/sched/scheduler-do-rbac-mutations.ts:515), so a create in different casing would
      // silently OVERWRITE the existing role of that name.
      //
      // Keeping the typed value in `state` means previewRole's mirror of the engine's
      // CUSTOM_ROLE_NAME_PATTERN refuses the save for the same reason the field is already showing. The
      // clash hint compares on the normalised form, because that is what a save would collide with.
      state.name = v.trim();
...
function surfaceRow(screen: { id: string; label: string; desc: string }, state: BuilderState, redrawSurface: () => void): HTMLElement {
  ...
  // keyboard/screen-reader user never sees. It names the permission in customer language
  // (capabilityPhrase), never the raw dotted id, when this fires for a missing capability. Contrast the
  // capability MATRIX above (Capabilities section), which deliberately shows the mono id alongside the
  // human label as a teaching aid; this hint is not that surface, so it gets translated the same as every
  // other gated control.
...
function segControl(
  ...
  for (const m of modes) {
    ...
    // Name the permission in customer language, never the raw dotted id, in this tooltip too. A screen
    // with no write side (writeCap null, e.g. audit/reports/posture) can still land here
