...
export function creatorCapabilities(): ReadonlySet<Capability> {
  const c = caller();
  // An EMPTY capability set is this screen's version of the viewer fallback: "the role builder claiming
  // they hold no capabilities" is a real, visible state. A null caller does not mean the operator holds
  // nothing; it means we have not been told yet, and the no-escalation guard is about to be computed against
  // an authority of zero. The witness holds the screen and records a row if the identity later lands.
  if (!c) {
