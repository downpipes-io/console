...
// existing-catalogue list with its delete control, and the honest access.policy gate card. It imports the
// shared leaf (./shared.ts) only, so it never imports the form section (which would form a cycle).
//
...
export function saveRow(
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const doSave = async (): Promise<void> => {
      // Disable before the await so a double-click cannot submit the create twice, matching every sibling
      // save that disables before its await (e.g. the downpipe editor's submit). Set here rather than in the click
...
function catalogueRow(engine: EngineClient, role: CustomRole, reload: () => void): HTMLElement {
  ...
  del.addEventListener("click", () => {
    void confirmModal({
      ...
      // to the viewer floor server-side on its holder's next request, and that consequence must ride the
      // custom-role-change delete event, or "Bob lost access to restores after we tidied up old roles" cannot
      // be linked to this delete. The count is taken here because after the delete the linkage is
      ...
      try {
        ...
        // exists and NOBODY is floored. Recording the count without the fate would assert a downgrade for a
        // proposal an approver may still reject, sending a support engineer chasing an access loss that had
        // another cause.
        //
        // isPendingResult decides the fate from the engine's REAL 202 body, so a queued delete and an applied
        // one are recorded as their own distinct row rather than coalesced together.
        //
        ...
        if (isPendingResult(res)) {
          // A queued delete must not toast "Deleted custom role": the role still exists until an approver
          // acts, so that would tell the operator a lie about their own account. Same helper
