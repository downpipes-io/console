// Restore: the whole-account BATCH restore queue (/restore/batch): "recover my whole account in one
// sitting". This batches
...
function renderQueue(engine: EngineClient, seeds: Seed[]): HTMLElement {
  ...
  // not the cumulative role rank, means a restore-operator, which sits off the rank ladder yet
  // holds restore.request, still sees the request path as the recovery-only role the engine accepts.
  // A caller without restore.request (a Viewer, an Access-admin) still sees only the summary.
  const controls = h(
    "div",
...
async function applyRowClick(row: BatchRow, ctx: QueueCtx): Promise<void> {
  ...
  try {
    ...
    // The queue applies for real, one live write per row, and records
    ...
    if (kind.kind === "restore-unapproved") {
      ...
      row.phase = "awaiting-approval";
      delete row.approval;
      // THE SAME SIX CAUSES ARRIVE HERE AS THE SAME FLAT 403. classifyRestoreApplyRefusal
      // (engine/src/admin/approvals.ts) tells expired, consumed, applying-lease, pending, self-approval and
      // missing-approver apart; router-restore answers all six with { error: "restore not approved" }. "A
      // distinct approver must sign it again" is the remedy for only one of them, and for a CONSUMED approval
      // it is an instruction to run the same restore a second time, so the toast below states the fact (the
      // apply was refused on the approval) rather than guessing which of the six caused it.
...
function statusTone(row: BatchRow): StatusTone {
  switch (row.phase) {
    ...
    case "applied": {
      ...
      // Each non-clean kind is named explicitly here, rather than folded into a single danger state, so a
      // restore that wrote every record and merely lost a TTL does not show the same alarm as one that
      // failed outright. Only a genuine failure is danger.
      //
      // "skipped" covers a restore that deliberately did not write a secrets record (there is no runtime
      // write path for one). That is not a failure and must never read as "Applied with failures" in red,
      // the exact reading that sends someone hunting for a fault that does not exist. Nothing failed; some
      // records are outstanding.
      // A SWITCH rather than an if-chain, so the compiler is what catches the next kind: a fall-through is a
      // valid program, so an if-chain lets a new kind land on the wrong branch silently. `never` makes the
  ...
  switch (row.phase) {
    ...
    case "approved": {
      ...
      if (row.phase === "planned") {
        ...
        // that capability, not the role rank. A restore-operator holds restore.request and sees the
        ...
        // rather than the approver rank means a restore-operator, which holds restore.apply,
