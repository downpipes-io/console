// The in-browser Shamir M-of-N reassembly CARD: the SAME reviewed reconstruction is reused by both the
// standalone /restore/recover-key screen (reconstruct-and-download) and the in-console break-glass restore
// panel (break-glass.ts), which reassembles a split key to decap a run's per-run master. There is no new
// cryptography and no new engine route: every primitive here already ships and is already unit-tested
// (combine + verifyWrappingKey in ../../lib/shamir.ts, parseShareFile + parseEnvelopeFile in
// ../../lib/custody-files.ts, decrypt in ../../lib/envelope.ts, parseIdentityFile in ../../lib/keydecap.ts,
// all pure and network-free).
//
...
import { readTextFileBounded } from "../../lib/file-size-guard.ts";
...
//
// WHY THIS EXISTS AS A SHARED HELPER RATHER THAN DUPLICATED INLINE. Every mount site needs this
// MutationObserver-on-disconnect block (recover-key.ts, attend.ts's setup form, and both branches of
// restore-flow.ts): a site missing it is a site where a reassembly card's secrets survive navigation. The
// pattern is the load-bearing half of the custody promise, so it belongs in the module that owns the wipe
// contract, where a new caller finds it beside the teardown it is meant to call.
//
...
export interface ReassemblyOptions {
  ...
  //
  // Handing back the PARSED private, rather than the recovered text, avoids creating a second immutable JS
  // string of the customer's break-glass key in each caller (nothing can overwrite a JS string), on top of
  // the one this card already holds: this card parses once, to decide isIdentity, and simply gives the
  // caller what it already has.
  //
...
export function renderReassemblyCard(opts: ReassemblyOptions = {}): ReassemblyCard {
  ...
  // index, so keying by index alone would let a stray share loaded FIRST lock out every correct share after
  // it and blame the correct ones in the message. Worse, the file picker fires an independent async
  // FileReader per file, so completion order is not selection order: the same four files could be accepted
  // or rejected differently on two consecutive attempts, which is not something a support engineer can
  // reproduce.
  ...
  // Linux font stack that floor runs wider than it does on macOS, so this card can escape its own
  // content box at narrow widths (guarded by route-set-containment.spec.ts and
  // shell-chrome-reflow.spec.ts), which stays invisible on macOS because its shorter glyph metrics
  // never push the floor past the card's own width. minmax(0, 1fr) pins the column's minimum to 0
  // instead, the same fix used for the identical shape at /access/roles/builder. A row whose own
  // content still cannot shrink wraps or scrolls inside its cell; it no longer forces every other
  // row in this single-column card wide along with it.
  ...
  // 1a. File-picker: multiple share files at once, each read locally with FileReader and never uploaded. Each
  // read goes through lib/file-size-guard.ts, so a file above CONSOLE_FILE_MAX_BYTES is refused unread.
  ...
  // ---- 2. the encrypted key file (identity.key.enc): file-picker only, one file, read through the same
  // size guard as the shares. -----------------------------------------------------------------------------
  ...
  ciphertextFileInput.addEventListener("change", () => {
    ...
    void readTextFileBounded(file)
      .then((text) => {
        envelope = parseEnvelopeFile(text);
      ...
      .catch((e: unknown) => {
        // A refused read (oversized, or the browser failed it) and an unparseable file land in one place.
        envelope = null;
        ...
        refreshReconstructEnabled();
        // A stale prior result must never survive an input change, even a failed one: without this, a
        // successful reconstruct followed by loading a DIFFERENT, unparseable ciphertext would leave
        // Download enabled and still serving the PREVIOUS ciphertext's bytes.
        invalidateResult();
      });
  });
  card.appendChild(
  ...
  async function doReconstruct(): Promise<void> {
    ...
    // (shamir.ts). It also interpolates over every share it is handed, so passing all of them would let ONE
    // bad share defeat an otherwise-good quorum, leaving the operator to try removing a share with no idea
    // which. That is up to C(n,t) blind attempts, performed in front of assembled custodians.
    //
  ...
  const wipe = (): void => {
    ...
    // Re-grade the controls against the state that is now EMPTY: without this, wipe() would drop every
    // byte but leave Reconstruct as the last input left it, enabled on a card holding nothing. Reset alone
    // would hide that because it refreshes straight afterwards, but teardown (the nav-away / panel path)
    // does not, so a torn-down card would still present an enabled Reconstruct. The button cannot rebuild
    // anything in that state, but a control that stays lit after the material behind it is gone is exactly
    // the thing an operator reads as "the key is still here", so it is graded rather than left to the caller.
    refreshSharesUI();
    refreshReconstructEnabled();
  };
