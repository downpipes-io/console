// Restore (screens 3 and 4) restore flow (the right column): pick the run and choose
...
// programmatic re-entry kicks off the dry-run so the operator lands on the review.
//
// Step-gating: once a dry-run plan builds successfully, the pick-run form COLLAPSES into
// a one-line summary ("Restoring run-x to original bindings") with a "Change" button; the
// plan review shows expanded below. "Change" re-expands the form (and re-arms the gate);
// if a live, unconsumed approval currently arms Apply, a confirm asks first, since
// re-expanding is the path that voids it. Nothing is ever REMOVED (only its display
// toggled via the CSSOM), so a tour anchor inside the form survives collapsed.
...
export function renderRestoreFlow(
  ...
  // reenter rebuilds this flow column in place, scoped by a prefill (a retry-subset re-entry). It
  ...
  //
  // On a COLD entry the picker is the lead affordance, not a quiet link: opening on a required "Run
  // id" field with the picker as a linklike underneath would ask a customer arriving mid-incident for
  // the one thing they are least likely to have to hand. With a run already in hand (the
  // /restore/:runId entry, or a re-enter for a subset) the field is the answer and the picker goes
  // back to being the quiet way to change your mind.
  ...
  // the snapshot belongs. Empty leaves cf-config records out of band.
  ...
  // leaves media records out of band (the existing docs-admitted API-only path).
  ...
  // LABELLED "(media)", not "Cloudflare account id": without the qualifier this field and
  // cfAccountField above would carry the byte-identical label "Cloudflare account id" while both are
  // visible on /restore at once, so nothing on screen would tell an operator which account each one
  // meant, and a locator by accessible name would resolve two, the same shape as the re-key
  // ceremony's two deploy-token fields. The qualifier follows this screen's own convention, as in the
  // sibling "Cloudflare zone id (zone surfaces)" and "Media edit token (optional)".
  ...
  // ONLY WHEN THE RUN ID IS KNOWN AT MOUNT TIME. runId0 is prefill?.runId ?? prefillRun ?? "", so on
  // the runless /restore landing SCOPE_DRAFT is null for the entire life of that mount: nothing below
  // reads a draft and, because the persistScope wiring is inside the same guard, nothing WRITES one
  // either. A scope typed on the landing is held in the fields and nowhere else.
  //
  // This matters because the navigation below (remountRebuildsRequest at the foot of this file) can
  // only assume a re-mount will rebuild the same scope when a draft actually exists to carry it. On
  // the runless landing it does not, so the navigation must be suppressed for anything but a bare run
  // id rather than relying on a draft to carry include/exclude/max/record across the re-mount.
  //
  ...
  //
  // THE BLAST-RADIUS TARGET IS DELIBERATELY NOT PERSISTED: a redirect must be re-chosen on every
  // entry and never restored from a stale draft, because it is the highest-consequence control on
  // this screen. The risk this guards against is not the reset itself: it is the navigation below
  // re-mounting mid-flow and then BUILDING AND ARMING a plan against the calm default, silently,
  // moments after the operator chose a redirect, instead of either keeping their choice or refusing
  // and asking for it again. Resetting a choice on a fresh entry and discarding one already made are
  // different acts, and only the second is a fault. The guard below prevents the second while leaving
  // the first untouched.
  ...
  // not a quiet text field (flow.md Stage 1). Original bindings is the
  ...
  const paintStepper = (step: number) => {
    // (shared.ts restoreStepper): hand the stepper the step this paint is advancing FROM,
    ...
    // Apply succeeded (the terminal step): the pick summary's "Change" affordance no
  ...
  // A re-entry note (a retry-subset re-entry): an honest line explaining the flow was re-scoped.
  if (prefill?.note) {
  ...
  const buildRequest = (confirm: boolean): RestoreRequest => {
    ...
    if (Number.isFinite(max) && max > 0) req.maxRecords = Math.round(max);
      ...
      // A Max records value the operator typed that the builder could not use (it did not parse, or it was
      // zero or negative): the cap is dropped and the run is planned UNCAPPED, which is exactly the "I set
      // Max records to 100 and the plan showed 40,000 writes" scenario. The engine received no maxRecords
      // and planned the whole run faithfully. Recorded only when something was actually TYPED: an empty box
      // is the ordinary "no cap" choice and is not a fault.
  ...
  // chooseRun is what the run picker and the date browser call once the operator settles on a run. It
  // sets the run id IN PLACE, in the field, rather than deep-linking to /restore/:runId unconditionally:
  // a deep link there re-mounts this whole flow from empty fields and, because the re-mount's prefillRun
  // branch fires its own dry-run, would paint a plan built from the run id alone, dropping any
  // include/exclude/max, redirect target or Cloudflare edit token the operator had already typed on this
  // landing.
  //
  // The rule is remountRebuildsRequest's own rule (at the foot of this file) applied at the other end:
  // hop to the bookmarkable URL only when a fresh mount there would rebuild this exact request. For
  // anything but a bare run id it cannot, so the run id is set in place instead, matching the
  // break-glass panel's own picker (it passes an onPick so the operator's already-supplied key is not
  // discarded by a re-render).
  //
  ...
  // does at remountRebuildsRequest. This landing never WRITES the per-run scope draft, but an earlier
  // visit to /restore/:runId for the CHOSEN run may have, and a deep-link re-mount would read it back and
  // ADD a scope the operator did not type here. Narrower is the less destructive direction, but it is
  // still a plan the operator did not build.
  ...
  const runDryRun = async () => {
    ...
    // A REDIRECT with an empty binding would plan against the LIVE ORIGINAL bindings (the exact
    ...
    if (maxTyped !== "") {
      ...
      // This gate must agree with the field's own rule (wholeNumberAtLeast({min:1})): if it accepted
      // anything merely finite-and-above-zero, a typed 2.7 would pass here while buildRequest's
      // Math.round turned the cap into 3, so the operator would see the field's "whole number" error,
      // press Dry run anyway, and get a plan capped at a number they never typed. Requiring the whole
      // number here keeps the gate and the field in agreement, so the builder's Math.round only ever
      // sees a value that is already whole.
      ...
      // The browser blanks an entry it could not read as a number (a comma, a separator, a stripped
    ...
    // (dry-run verification sweep): bound to THIS request only. mountDryRunSweep only starts
    ...
    try {
      // The request is held rather than passed inline because the navigation below has to ask what
      // was in it. See remountRebuildsRequest: the navigation re-mounts the flow, and a re-mount can only
      ...
      // type here: a landing request of just {"runId":"..."} could re-mount into
      // {"runId":"...","include":["session:"]}, and the narrowed plan would be the one that paints.
      // Narrower is the less destructive direction and the re-mounted fields would show it, but it is
      // still a plan the operator did not build, so the same rule applies rather than an exception to it.
  ...
  // A prefilled run (a "restore from this run" deep link) or a programmatic re-entry (a
  // retry-subset re-entry) kicks off the dry-run so the operator lands on the review panel, the
  // low-error primary path (flow.md Stage 1).
...
// remountRebuildsRequest answers the only question the navigation above is entitled to ask: would a fresh
// mount of this flow at /restore/:runId build the SAME request this one just sent? If not, the navigation
// would silently swap the operator's plan for a different one, because the re-mount's own dry-run is the
// one that paints.
//
...
//
// Each of these would otherwise be silently dropped or reset on the re-mount: a cfConfig or
// mediaRestore context would vanish and the diff it earned would collapse to "This plan writes
// nothing, so there is nothing to apply"; a chosen redirect target would fall back to the LIVE
// ORIGINAL bindings, with the redirect's type-to-confirm never firing because the re-mounted plan is
// not a redirect; and a capped, prefixed scope would plan and apply UNCAPPED over every record.
//
...
//
// Stated as a structural check rather than a list of known fields, because a list of fields to suppress
// has to be kept in step by hand with every field buildRequest ever gains. A field added there and
// forgotten here would cost a URL the operator does not get, which they can see, instead of a plan they
// did not ask for, which they cannot.
...
function targetChoice(prefillBinding?: string): TargetChoice {
  ...
  // A prefilled redirect binding (a retry-subset re-entry of a redirected restore) starts the
  ...
  // Shown when a redirect is chosen with an empty binding, so the dry-run guard can block the
