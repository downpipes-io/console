...
// delete was intended, but no distinct approver has signed off this exact plan yet. Without this panel,
// an operator on exactly the estate posture this row exists to serve would hit an unexplained 403 with no
// portal path forward, forcing them out to a CLI the portal is meant to make unnecessary. The wording of
// the refusal line is deliberately the same as the standard restore flow's own dual-control banner
// (restore-flow/confirm.ts renderArmed), because it is the same rule: a distinct second identity must
// approve before an irreversible action proceeds.
...
export function renderBreakGlassPrune(engine: EngineClient, prefillDownpipeId?: string): BreakGlassPrunePanel {
  ...
  //
  // WHY IT IS NEEDED AT ALL. Reconstruct from a quorum, then edit the share list: the staleness rule
  // correctly drops the private, but without this reset the status line would go on saying "Split key
  // reassembled in this browser. Nothing was uploaded." with no key actually held. An operator mid-incident
  // reads that line as custody state, so it must never say more than what is currently true. attend.ts:480
  // resets its own line on the same event; this panel does too.
  ...
  const teardown = (): void => {
    ...
    // the material that REBUILDS them would not be a teardown, so this call reaches the card too.
  ...
  root.appendChild(
    h(
      ...
      h(
        "span",
        "Your break-glass key is read in this browser and never uploaded. It recovers a per-run key for every run in scope, the ones being kept as well as the ones being superseded, because the plan is a subtraction and the engine refuses a batch that does not cover every candidate. Those per-run keys are sent to your own engine, held for the one request and forgotten. We never receive your break-glass key.",
      ),
    ),
  );
  ...
  // APPLY KEEPS `disabled`, DELIBERATELY, unlike most other gated controls in this console, which use
  // aria-disabled so they stay in the tab order and their reason is reachable. aria-disabled is an
  // announcement, not an enforcement, and this button's handler is attached UNCONDITIONALLY at the
  // bottom of this function because the
  ...
  //
  // THE REASON IS CARRIED IN TEXT, NOT ONLY IN THE TITLE. planHost below is role="status" aria-live="polite"
  // and states the sequence once something has happened, but it is built EMPTY and only cleared or filled
  // AFTER a preview or an apply, so at the one moment Apply is refused -- before anything has been
  // previewed -- it holds nothing, and a hover title alone would be the only carrier of the reason. A phone
  // has no hover, so the reason has to live in text a screen reader and a touch device can both reach.
  //
  ...
  //
  // WHAT IS NOT FIXED: the control cannot be FOCUSED, so a keyboard user reaches the reason by reading the
  // region rather than by tabbing onto the button. Fixing that means moving the handler behind the
  // readiness test, which is a behavioural change to a destructive path.
  ...
  // therefore carry the reason correctly right up until the customer used the button, then silently stop.
