...
// /restore/:runId exactly as run-picker.ts's runPickerRow does, so the restore framing is never lost.
...
// See lib/recovery-calendar.ts's own header for the full reasoning (also covers the UTC pin and how the
// ring floor stays consistent).
//
// FIELD CATALOGUE. Every control here is a plain <button> (or a non-interactive <span> for a day with
// nothing to pick): no <select>, no typeable input, no radio, no aria-pressed/aria-checked/role=radio/
// role=switch toggle. field-census.mjs only counts a button as a catalogue control under those last four
// conditions (field-census.mjs:191-193), so this whole surface adds zero field-catalogue rows -- it only
// grows the action catalogue. The day cells use role="gridcell" + aria-selected (the ARIA grid pattern's
// OWN selection attribute), which field-census.mjs does not treat as a widget trigger, so the grid stays
// field-free while still describing its selection state honestly to assistive technology.
...
export function openDateBrowser(engine: EngineClient, onPick?: (runId: string) => void): void {
  ...
  // Named loadFleet (not "load"): run-picker.ts's own retry closure is already named "load", and the
  // hook census derives a control's key from the identifier its click handler calls, so a same-named
  // closure here would collide with, and renumber, that already-shipped key elsewhere.
...
// listAllHistory() on a large fleet and is naturally per-downpipe.
...
export function renderCalendarView(args: CalendarViewArgs): HTMLElement {
  ...
  const nextBtn = h(
    "button",
    {
      ...
      // The click handler is attached CONDITIONALLY rather than left in this attrs object unconditionally
      // and relying on `disabled` to have the browser swallow it: gating the handler behind the same test
      // the refusal is behind makes the refusal provably inert, rather than inert only by courtesy of the
      // browser.
...
// role=switch, which field-census.mjs would count as a field-catalogue control -- see the module header).
// The callback is named onSelectDay (not "onPick"): the hook census derives a control's key from the
// identifier its click handler calls, and run-picker.ts's runPickerRow already owns "onPick".
...
// renderTimeRow mirrors run-picker.ts's runPickerRow look and behaviour exactly, reusing the same
// selection mechanism: activating it deep-links to /restore/:runId, auto-building the dry-run.
...
// onSelectRun (not "onPick"): run-picker.ts's own runPickerRow already owns "onPick" for the hook census.
