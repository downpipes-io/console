...
//
// Without this screen, an operator who supplies a break-glass key, previews a plan and clicks Apply on an
// enforce-on downpipe hits an unexplained 403 with no portal path forward, and would have to raise a
// dual-control request via a raw API call instead. This screen, plus break-glass-prune.ts's inline request
// panel, keeps every step (request, approve, reject, apply) inside the portal.
...
function renderPruneApprovalCard(engine: EngineClient, r: PruneApproval, me: string | null, canApprove: boolean, reload: () => void, downpipeName: string | undefined): HTMLElement {
  ...
  if (r.status === "requested") {
    ...
    // This is the state the engine is briefly in between a reserved apply and its commit (or a
    // release back to "approved" on a failed or errored attempt). The engine's try/finally
    // (engine/src/admin/router-retention-prune.ts) releases the reservation on every exception path, so
    // this should be seen rarely and briefly, but a bare badge with no sentence would read as broken
...
function approveConfirmBody(r: PruneApproval, downpipeName: string | undefined): HTMLElement {
  ...
  // This is the maker-not-checker's last chance to catch a mistake before archive bytes are destroyed, and
  // the run counts above are the only blast-radius figure the request route can compute
