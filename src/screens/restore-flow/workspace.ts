// Restore (IA screens 3 + 4) the two-column workspace: run context on the left (the run
...
// a run picker and the fleet restore-test recency card.
//
// Runless landing: with no prefilled run there is no context column, so the flow
// would otherwise auto-place into the grid's narrow 360px track and read as cramped; the
...
export function renderWorkspace(engine: EngineClient, prefillRun: string | undefined): HTMLElement {
  ...
  // /restore view only; on /restore/:runId it is irrelevant to the chosen run and would blow
  // the eager-section budget (design-system.md 7a), forcing scroll past the receipt area.
