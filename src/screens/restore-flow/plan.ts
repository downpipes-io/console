// Restore (screens 3 and 4) dry-run review (the hero panel): the impact summary
...
// plan-hash binding cue, and the confirm + apply gate.
...
export async function renderPlan(
  ...
  // tabindex="-1": programmatically focusable so collapsing the pick form into its one-line summary
  // can move focus here, without adding this heading to the Tab order.
    ...
    if (reason.startsWith(BREAK_GLASS_PREFIX)) {
      card.appendChild(
        inlineOutcome({
          ...
          reason,
          // The break-glass panel can apply a restore too (its own confirm+apply block, gated by the same
          // dual control), not only attended verification (proof, never a write), so an operator who
          // reached the STANDARD flow first (this screen) rather than the panel's own secondary entry point
          // is told both are on offer, not steered at the one that cannot write.
  ...
  // the approval that arms against it. Without it, narrowing a redirected plan to one record would silently
  // write that record back over its live original binding, because the re-scoped plan would not be a
  // redirect and so would carry no type-to-confirm.
  //
  // One expression, spread at every re-entry site (including confirm.ts's retry-subset re-entry), is one
  // thing to keep true rather than several.
  ...
  // redirected or large restore (flow.md Stage 2) and CALM for the routine default.
  ...
  // reads as a wall of failures. Group by what the operator should actually DO: investigate (no
  ...
  if (plan.configChanges && plan.configChanges.length > 0) {
    ...
    // Open when something will actually apply, matching the media preview below: on a config-only restore
    // a closed disclosure is the difference between the diff being present on the page and the diff
    // actually being read. The summary states the surfaces that WILL apply first, because that is the
    // number the operator is deciding on; the total examined follows it.
  ...
  // The precise integrity statement (flow.md Stage 2): verified, not "proven perfect".
  //
  ...
  // approval binds to. Computed client-side from the request (the engine recomputes and
  // gates on it server-side). Redaction-safe (names, counts, selectors only). If it cannot
  // be computed, the flow still works; the hash is a cue.
  ...
  try {
    ...
    // omitted it would produce a different hash for every cf-config restore.
  ...
  // 3. Confirm + apply. Gated by BOTH the role AND a usable dual-control approval bound to this
  // plan hash; renderConfirm is async because it asks the engine (listApprovals) whether a distinct
  // approver has already signed this plan before it offers an enabled Apply.
...
function planFigures(plan: RestorePlan): HTMLElement {
  ...
  // The caption states what the number counts, on every plan rather than only the config ones: a
  // config-only plan's honest zero would otherwise read as the whole plan doing nothing, and a figure
  // that only explains itself when it would otherwise mislead is one the operator learns to read wrongly
  // everywhere else.
...
function resolvedDestinations(
  ...
  // (drawer-section__title). restore-sample__title carries the element's own spacing.
...
// ---- where the dry-run reassurance sits -------------------------------------------------------
//
// "Nothing has been written yet" is the one sentence a frightened operator needs earliest, so it ends
// the impact banner in EVERY tone, which is the first element on the card: the reassurance arrives
// before the blast radius (the resolved-destinations table, the skipped groups, and the cross-account
// and cross-zone warnings), not after it.
//
// The integrity statement keeps its own copy of the fact deliberately: that sentence is about the
// records "above" and their read-only hash verification, which is a claim about the table it follows,
// and it is correctly placed for them.
