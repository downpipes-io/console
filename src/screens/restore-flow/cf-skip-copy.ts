...
//
// WHY THIS EXISTS. A bare skip count cannot tell "your plan caps DNS records" from "the restore token
// lacks the edit scope for this surface" from "the snapshot item is malformed", and those are three
// tickets with three different remedies. This module classifies each skip by its reason so the receipt
// can say why an item did not land, not just that one did not, and so the customer knows whether the fix
// is a wider token or nothing at all.
//
