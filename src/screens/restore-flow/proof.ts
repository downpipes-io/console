// Restore (screens 3 and 4) proof surface (one shared region for every "prove this
...
// live data and nothing is licence-gated. No-custody is never weakened.
...
// renderProofCard is the SINGLE proof surface for a run: one shared result region, one
// slow-note, no toast echo (the verdict paints in view). The PRIMARY verb is the blind restore test:
...
export function renderProofCard(engine: EngineClient, run: RunHistoryEntry, downpipeId: string): HTMLElement {
  ...
  // rank ladder, so a rank-based check would disable the drill for the recovery-only role the engine
  // accepts.
  ...
  // The refusal and the busy latch are two different things, and keeping them apart matters. A
  // refused button is refuseWithReason'd once below: focusable, announced, with its reason in text,
  // and no handler attached. The busy latch is a transient `disabled` on a button that IS live, which
  // is the only use of `disabled` here, because there is no reason to read there and the state lasts
  // one request.
  //
  // setBusy therefore touches only the live buttons: applying `disabled` unconditionally would
  // re-apply it to a refused sibling on the first request of the other button, silently disabling the
  // button for a caller who holds one capability but not both.
...
// reads as RUNNING from t=0 rather than an inert disabled button. The .ob-spinner is reduced-motion
// gated in tokens.css, and the message is replaced by the verdict (or the break-glass posture) when the
// call lands.
...
function paintBreakGlassPosture(args: {
  ...
  // Attended verification is this posture's in-platform proof path (no CLI step required): it needs
  // the break-glass key, supplied in the browser, so it leads here.
  const attendBtn = h(
    "button",
  ...
  attestBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      // this one, so without this the attestation control would stay dead after its first press on
...
// wireVerifyButton attaches the blind restore test click handler (the PRIMARY verb).
  ...
  drillBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok && (res.reason ?? "").startsWith(BREAK_GLASS_PREFIX)) {
        ...
        // disturbed by an evidence-write fault, but the fault is surfaced rather than swallowed: an
        // auditor reading the recoverability report cannot distinguish "never drilled" from "drilled,
...
function renderAttestationResult(host: HTMLElement, res: KeylessAttestationResult): void {
  ...
  //
  // Painting those placeholders as "failed" would turn a mistyped run id into three security assertions
  // about checks that never happened: that the archive's signature is invalid, that a shard is MISSING,
  // and that the archive has been ROLLED BACK, inside an assertive danger surface. Anti-rollback is a
  // tamper indicator, so a false alarm on it would be its own harm: it would send an operator to a
  // data-loss incident when the truth is a typo. Where the product cannot tell, it must say so; a
  // could-not-check outranks a pass, and here it must also outrank a FAILURE.
  //
...
// did not pass (see renderAttestationResult above); collapsing it into "fail" would misrepresent a check
