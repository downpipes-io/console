// Restore, runs and actions (IA screens 3 + 4) shared leaves: the blast-radius constants,
...
// figure/fact rows, the badges, the prefix splitter, the dated last-proven line). No-custody
// is never weakened and no confirmation / dual-control wording is touched.
...
// same-binding restore (blast-radius calibration, design-system.md 6.16, flow.md D-8).
...
// The break-glass-only drill reason the engine returns verbatim (flow.md s0): the
...
// SEPARATELY FAILABLE call, so its outcome is CARRIED, never assumed: swallowing it into an empty catch would
// leave the auditor's recoverability report showing NO drill evidence for a customer who drills every month,
// with nobody able to say why, since "never drilled" and "drilled, evidence write refused" would look
...
// "retry the failed subset" affordance re-enters scoped to the failed record names. Every field is
...
// rather than a false "proven". The engine wire carries the proving method, so an ATTENDED
...
export function lastProvenLine(state: DownpipeState | undefined): string {
  ...
  // instant it cannot parse. The claim that a recovery has actually been rehearsed is the strongest thing
  // this screen says, so this is the one sentence on the restore path where a stale or unreadable stamp
  // must decide whether an operator goes looking for a drill, rather than being asserted over a blank.
  //
  // Other dateOnly call sites (licence/detail-rows.ts:44 and :68, licence/view.ts:287, licence/shared.ts:510,
  // lib/billing.ts:181 and :182) fall back to the raw stamp on an empty return (`dateOnly(x) || x`), so the
  // operator sees SOMETHING rather than a gap; protection-statement.ts:129 handles the empty return
  // explicitly too. Falling back to the raw stamp is not right here: an unparseable instant is not a date to
...
// overwritten); it exists solely to drive the connector fill: the ONE separator that just
...
// plannedWrites 0; a bare "Write 0 verified records back to their original source bindings in this account"
// would misstate an apply that changes real config. The count is not inflated to cover it: plannedWrites is
// read by the large-write friction threshold, shown to the approver in the approvals inbox and reconciled
// against on the receipt, so folding surfaces in would make one number mean two things. The config leg is
// stated beside it instead, and leads when there are no records at all, because a sentence that opens on a
// zero reads as a plan that does nothing.
...
// The trailing "Nothing has been written yet." is deliberately not part of this string. It is appended by the
...
// impact banner and again on the integrity statement, painted at DRY-RUN time. An apply whose response never
// comes back leaves the whole plan card on screen underneath the failure, and the card could then go on
// asserting that nothing has been written while the records are already back in the account and the
// single-use approval is already spent, both recorded server-side with monotonic instants before the socket
// closed. Marking every copy of the sentence with this one class and constant means a caller that later
// learns the true outcome can find and retract every copy at once, rather than rewrite a claim that is
// welded into a longer string. On a SUCCESSFUL apply the receipt hides the whole plan card and the sentence
// correctly disappears with it.
...
export function approvalStatusBadge(status: RestoreApproval["status"]): HTMLElement {
  ...
  // "applying" is called out explicitly (rather than left to the titleCase fallback below, which produces
  // the same "Applying"/info badge) to match pruneApprovalStatusBadge's explicit branch
  // (prune-approvals.ts) for the same PruneApprovalStatus member, since ApprovalStatus mirrors it.
...
// stable, immutable SUBJECT (iss|sub), not the mutable email. This
...
// (confirm.ts startPoll), so nothing looks again for the life of the tab: a tab left open past the
// approval's own expiresAt would otherwise go on offering an enabled Apply over an approval the engine would
// by then refuse. expiresAt is REQUIRED on the wire (lib/api/types/rbac.ts RestoreApproval), so this function
// reads it directly rather than trusting the listing's snapshot status.
//
...
//
// It exists because "the approver signed the plan and Apply never lit up" is otherwise unfalsifiable: four
// completely different states can produce that identical screen and identical (absent) evidence -- the client
// could not compute the plan hash at all, the caller has no attributable identity, the approval that exists is
// bound to a DIFFERENT hash than the one this console is matching on, or the only approval is the operator's
// own. Each is a different remedy, and none of them is visible from the tab alone. The engine's approval
// records ride in the support pack and say an approval EXISTS; this class is what says why the console is not
// using it.
//
// It returns NULL for a fifth state, an approval that has simply not been given yet, and that omission is
...
export function gateBlockClassFor(
  ...
  // gate. Leaving the lapse test out of one of the two would make them disagree in precisely the case it
  // guards: the lookup would refuse to arm while the check below returned null, meaning "a usable approval
  // exists: the gate is armed, not blocked", and the screen would fall back to the standing dual-control copy
  // for a plan that HAS been approved. The lapse is surfaced as its own sentence by the caller (confirm.ts
  // approvalLapsedNote) rather than as a new member of this enum, because the enum is a CLOSED vocabulary the
  // engine must admit member-for-member: a console-only addition would be dropped whole on arrival by
  // projectRecord and read as silence in the support pack.
  ...
  // The approver really did sign; the console is matching on a different key. Without this class the state
  // would be indistinguishable from an unsigned plan: both render as "awaiting approval".
...
// GATE_BLOCK_ADVICE is the CUSTOMER half of gateBlockClassFor: the four classes each have a different
// remedy, so a single sentence ("This apply needs a second authorised identity to approve this exact plan
// first.") would be wrong for three of them.
//
// Two of the four are unliftable without knowing which one you are in. plan-hash-failed can never arm however
// many approvers sign, because the client has no key to match an approval against, so advice about raising a
// request cannot work. plan-hash-mismatch is worse to read: the operator can SEE an approved record in the
// inbox and Apply stays dark, so telling them to go and get the approval they already have is no help.
//
...
// mean "how many times we polled" rather than "how many restore plans were blocked this way", which is the sort
...
// recorder, or the queue's writes stay invisible.
//
// SILENCE ON ITS OWN MEANS TOO MANY THINGS: an apply that never ran, a restore that reported success and
// wrote nothing, and a restore whose apply came back with no failures would all leave the same no-row
// absence in the pack, and support cannot act on those three differently from one shared silence. An
// apply-outcome row is recorded on EVERY ending instead, and the class says which:
//
