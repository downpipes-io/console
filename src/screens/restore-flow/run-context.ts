...
// loads independently of the flow so a slow history fetch never blocks the dry-run. No-custody
// is never weakened (counts and dated facts only, no value).
...
export function renderRunContext(
  ...
  // is the join key; flow.md D-13 describes the all-downpipe envelope), alongside the fleet's
  ...
  void Promise.all([engine.listAllHistory(), fetchDownpipeNames(engine)])
    .then(([all, names]) => {
      ...
      // budget (design-system.md 7a) while staying find-in-page-able.
...
//
// Hovering or keyboard-focusing one cell lights it and dims every sibling cell, mirroring the
// map's own hover-focus idiom (components/live-flow-node-layer.ts setFocus / tokens.css
// .live-flow__node--lit): ONE setFocus call re-derives every cell's lit state from scratch on
// each hover/focus change (rather than each cell toggling only its own class), so a fast
// pointer/keyboard handoff between cells can never leave two cells (or none) lit at once. The
// lit cell also raises a small tooltip (run id, status, relative age) built from the SAME
// .info-tip__pop chrome the (i) tooltip primitive uses (info-tip.ts), nested inside .run-cell
// (already position:relative), so this gets the one tooltip look the console already has
...
function renderRunTimeline(ring: RunHistoryEntry[], currentRunId: string): HTMLElement {
  ...
  for (const r of ordered) {
    // The canonical tone: an abandoned run must never wear the still-running "inflight" treatment in the
    // timeline a frightened customer reads while choosing a restore point. The cell must agree with the
    // label and tooltip beside it, which state the honest status.
    ...
    const cell = h(
      ...
      { "data-dp": "restore-flow.button.cell",
        ...
        // NO INLINE OUTLINE: an inline outline cannot be overridden by `.run-cell:focus-visible`, which
        // would make the run being restored paint identically whether it held focus or not. The selected
        // look lives in `.run-cell--current` as an inset shadow instead, leaving the outline free for focus.
