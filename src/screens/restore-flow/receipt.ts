// Restore receipt (IA screen 5, the trust-closing step): the applied-result classifier plus the
// receipt card and its copyable summary.
...
// The applied results whose success tick has already drawn, keyed by OBJECT IDENTITY rather
...
// renderReceipt is the trust-closing step (flow.md Stage 5).
...
//  - "Approved by <checker>" is carried from the usable approval that armed the apply, so the
//    maker != checker pair is visible on the durable receipt.
...
//
// Each of these fields exists because a restore can write every record successfully and still lose
// something the archive held: d1SchemaObjectsFiltered names a subset restore whose dropped index or view
// could otherwise leave an app breaking with no answer why, and metadataFieldsDropped names a dropped TTL
// or similar field that would otherwise sit silent behind a receipt claiming full fidelity.
// metadataShedPhrase turns ONE member of the engine's METADATA_SHED_FIELDS (engine/src/dest/restore-fault.ts)
// into the sentence an operator reads, because a single sentence for every member would be wrong about the
// most ordinary one.
//
// The engine keeps `kv-expiration` and `kv-expiration-lapsed` as separate members: an UNUSABLE expiration is
// a defect signal (something wrote a value the format does not allow), while a LAPSED one is the expected
// consequence of restoring a backup older than the namespace's TTLs. Collapsing the two into one phrase, or
// printing the raw wire key, would name a cause that has not happened and speak in the transport's words, on
// the exact case a customer meets whenever they restore an old archive.
//
// An unrecognised member falls through to the field name with NO cause attached: the console cannot know why
// a field it has never heard of was shed, and guessing would risk naming the wrong cause.
...
export function renderReceipt(
  ...
  if (!outcome.partial) {
    // On the FIRST mount of a clean applied receipt, the success tick draws once via
  ...
  if (windowed) {
    ...
    // The honest partial: state the split plainly (flow.md Stage 5).
    card.appendChild(
  ...
  // checker) is shown here, so the receipt carries the maker != checker pair on the durable record.
  ...
  // A failure entry with NO record name cannot become an include selector, so the retry subset can cover
  // FEWER records than failed. Naming the gap, with its count, stops that shrink from going unexplained.
