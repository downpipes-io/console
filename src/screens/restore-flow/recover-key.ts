...
// local download. The reconstruction card itself lives in ./reassembly.ts (shared with the in-console
// break-glass restore panel, which reassembles a split key to decap a run master); THIS screen is a thin
// wrapper that mounts the card in DOWNLOAD-ONLY mode (showDownload:true, no onRecovered), so it stays exactly
// reconstruct-and-download, with no engine call anywhere.
//
...
//
// Gate: now that reassembly FEEDS a real restore (the break-glass panel decaps a run master from the
// reconstructed key and calls POST /admin/restore/capsule + /admin/restore), the reassembly surface gates on
// the capability the restore path actually enforces at its entry: restore.verify (the read-safe viewer floor
// the capsule route, the blind-test and the attest routes all gate on, and the SAME capability proof.ts
// already mirrors on this screen family). Mirroring the real per-route capability in BOTH directions is what
// keeps the console gate honest rather than a role rank that no engine call backs.
