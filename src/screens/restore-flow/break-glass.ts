...
// panel's apply path share the same capability gate and dual-control reservation with every other apply in
// the product, with no second code path to drift out of sync. crossZoneNeedsConfirm and LARGE_RESTORE_WRITES
...
export function renderBreakGlassRestore(engine: EngineClient, prefillRun?: string): BreakGlassPanel {
  ...
  // scopeInclude narrows a re-preview to specific record names after a partial apply failure, mirroring the
  // standard flow's retry-subset. null/empty means the whole run. Reset whenever a fresh run or key
  // invalidates the current scope.
  ...
  // installs never fires for this caller. Dropping the reference instead of stopping it would leave an
  // armed lapse timer, up to the approval's whole remaining lifetime, holding the detached block. This is
  // also the panel a customer
  ...
  const teardown = (): void => {
    ...
    // met, the reconstructed break-glass private. wipeMaster()/wipePrivate() never touch any of it, so
    // stopping there would leave the secret REBUILDABLE: the material the reconstruct runs on would still
    // be loaded and the control would still run. Wiping the two derived copies while leaving the inputs
    // that produce them is not a teardown. Best-effort-idempotent and safe when nothing was ever loaded.
  ...
  const runPreview = async (): Promise<void> => {
    ...
    // narrows to specific record names after a partial apply failure, mirroring the standard flow's
    // retry-subset; when absent or empty, the whole run is used.
    ...
    // (iv) the real apply path. This reuses the STANDARD flow's own confirm+apply block (confirm.ts), the
    // identical capability gate and dual-control reservation every other restore apply goes through, with
    // the recovered master threaded onto the apply call via opts (see confirm.ts's header comment). The
    // apply happens here, on the SAME screen the master was recovered on, rather than being sent to a
    // standard flow that has nowhere to accept a break-glass key at all. paintStepper is a no-op: this
    // panel has no step indicator of its own. reenter re-runs THIS preview scoped to a partial apply
    // failure's failed record names, reusing the already-supplied key rather than discarding it.
    const confirmFlags = {
    ...
    try {
      ...
      // (confirm.ts / plan.ts carry the same catch): the hash is a cue the dual-control gate binds to;
...
function renderPlan(plan: RestorePlan): HTMLElement {
  ...
  wrap.appendChild(
    h(
      "p",
      { class: "field__hint measure", style: "margin:0" },
      // The real apply gate (the identical dual-control confirm + Apply the standard flow uses) renders
      // directly below, on this same screen, so this line states only what THIS preview did, never a claim
