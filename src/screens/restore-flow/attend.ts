...
import { ATTEND_SESSION_KEY } from "../../lib/storage-keys.ts";
...
const SESSION_RESUME_KEY = ATTEND_SESSION_KEY;
...
// the caller's local variable for the session. A file above CONSOLE_FILE_MAX_BYTES (lib/file-size-guard.ts)
// is refused before any read starts, and a file that is not a valid identity.key rejects with a clear
...
export async function readIdentityFile(file: File): Promise<HybridRecipientPrivate> {
  const text = await readTextFileBounded(file);
  try {
    return parseIdentityFile(text);
  } catch (e) {
    throw e instanceof Error ? e : new Error("that file is not a valid identity.key");
  }
}
...
export function renderAttendRunner(engine: EngineClient): HTMLElement {
  ...
  // Restore operator, Approver and Owner). Gating by that capability, not the operator role rank, means a
  // restore-operator, which holds drill.run (a recovery rehearsal is part of the recovery role) yet
  // sits off the rank ladder, still sees this in-platform recovery proof. A caller without drill.run (a
  // Viewer, an Access-admin) still gets the note.
  if (!canCap("drill.run")) {
  ...
  // wipeIdentity ZEROES the key bytes before dropping the reference. Setting `identity = null` alone
  // drops a reference and leaves the 96 bytes in the heap for the garbage collector to reclaim whenever it
  // likes. This is the longest-lived key holder in the product (an attended session runs for as long as the
  // estate takes), so it is the one place that most needs the wipe, matching /restore/break-glass and
  // /restore/recover-key, which do this too.
  //
  ...
  const start = async (scope: string[] | undefined, sampleRate: number): Promise<void> => {
    ...
    // a test) answers a generic {ok:true,...} with no `runs` field, and `session.runs.length` below would
    // then throw on any caller, not only the demo world's fallback. Checked here too (client-attest.ts's
      ...
      try {
        ...
        for (const id of batch) {
          ...
          try {
            ...
            // both: what this removes is the long-lived typed array, which is the part we control.
            master.fill(0);
          } catch {
  ...
  const resumeSession = async (sessionId: string): Promise<void> => {
    ...
    try {
      ...
      // reads as `undefined` here, and `record.runs` below would throw on it directly (there is no field to
      // be missing; the whole record is). Folded into the existing catch below (rather than a new branch) so
...
function renderSetup(engine: EngineClient, opts: { onStart: (identity: HybridRecipientPrivate, scope: string[] | undefined, sampleRate: number) => void; onCard?: (card: ReassemblyCard) => void }): HTMLElement {
  ...
  // separation of duties rather than for any cryptographic gain, so this screen accepts both a single
  // identity.key file and a reassembled split key: without the second path, those customers could not run
  // an attended verification AT ALL, the one routine thing that proves their offline key opens a real
  // archive. The shared reassembly card is the same
  ...
  // the recovered bytes it still holds, the same wipe the single-file route gets. Without this the split-key
  // route would leak that key material on navigation away.
...
// within, so it belongs to the estate, not to whichever browser happened to set it. It reads and
// writes the engine (GET/POST /admin/config/attended-cadence), so the nudge follows the operator to another
// machine, survives private browsing, and survives cleared site data. It rides the signed control-plane
// export, and feeds the attended-verification-cadence posture check.
//
...
// split holder that resuming needs a quorum of shares again, so the file picker alone is not enough:
// without the reassembly route here too, a split holder who reopened the tab mid-session would have to
// abandon the session and start fresh, which on a large estate means a re-convened quorum plus everything
// the session had already done.
//
...
export function attendEntryCard(engine: EngineClient): HTMLElement {
  ...
  //
  // The interval comes from the engine, not this browser, so the nudge means the same thing on
