// Restore (IA screens 3 + 4) the shared mechanics behind the flow's five restrained, one-shot
// motion cues: the stepper's connector fill (shared.ts restoreStepper), the
// plan-hash seal tick (confirm.ts), the approval-state crossfade (confirm.ts) and the
// receipt seal stroke (receipt.ts) all call into the two helpers below, so confirm.ts and
// receipt.ts stay thin at the call site and neither duplicates the other's stroke-draw code. The
// Apply button's disabled -> enabled fade needs no helper: it is a plain CSS transition on
...
// change; a re-render that finds the SAME key/state is always a no-op past the first line.

import { motionOK } from "../../lib/a11y-prefs.ts";

// drawStrokeOnce animates an aria-hidden glyph's path(s) with a one-shot "draw" via
// stroke-dashoffset (the plan-hash seal and the receipt tick): each path measures its OWN real
...
//
// replay is the CALLER's own "have I already drawn this exact key" decision (the plan-hash seal compares the plan
// hash string; the receipt tick checks a WeakSet keyed on the applied result object), so this helper holds no
...
// crossfadeSwap replaces host's content with next by fading host's own opacity to 0,
