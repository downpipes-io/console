// Restore (IA screens 3 + 4) the confirm + apply gate (the safety heart). THE APPLY GATE is a
// client MIRROR of the engine control: an apply requires BOTH the Approver/Owner role AND a usable
// dual-control approval bound to THIS exact plan hash with a DISTINCT approver (maker != checker).
// Apply is NEVER enabled on role alone; until a usable approval exists the REQUIRED next step is to
// raise a request and have a distinct approver sign it. Friction is proportional to blast radius
// (type-to-confirm for redirect / non-latest / large). The receipt carries the real applier and the
// distinct approver.
...
export async function renderConfirm(
  ...
  // The capability gate, mirrored client-side: an apply requires restore.apply, the capability the
  // engine enforces at router-restore.ts (gate(caller, "restore.apply")) and which Restore operator,
  // Approver or Owner hold. Gating by CAPABILITY, not by the cumulative role RANK, matters because a
  // restore-operator sits off the rank ladder (roleRank 0) yet holds restore.apply: a rank check would
  // hide the whole apply/request panel from the recovery-only role that exists for disaster recovery,
  // even though the engine accepts its restore. A caller without restore.apply never sees an Apply
  // button; dry-run stays available to all (it wrote nothing), and a restore.request holder can still
  // RAISE a request (the step below, where the estate requires one).
  ...
  // light poll light Apply up once a distinct approver signs. This is the dual-control gate the
  ...
  // we degrade to the request step rather than a misleading enabled Apply.
...
//
// IT RETURNS THE WAY TO STOP IT, because it arms timers and nothing else can reach them. Without a way
// to clear them, the 12-second approvals-read guard below would keep running after the read it guards
// has already won its race, and watchForLapse would arm one timer per arming for the approval's whole
// remaining lifetime, with no handle kept. Both fire-time guards are correct, so nothing WRONG would be
// painted; what would accumulate is live timers, each holding the closure over `section`, `approvalHost`
// and `paintStepper`, which is the panel's whole detached subtree, for as long as an hour after the
// operator navigates away. A console left on the restore screen re-arms on every poll tick.
//
// This would surface as a hang rather than as retention, because a browser tab never has to exit and
// node does: `npm run validate` is a blocking preflight inside `npm run deploy` (scripts/deploy.sh), so
// a leaked timer here would stop a published deploy dead with no output and no timeout, which is worse
// than a failure because it cannot be told from slow.
function startApprovalGate(ctx: ApprovalGateCtx): () => void {
  ...
  // noteGateBlock is the gate's own account of why it did not arm. Without it, all four fault states below
  // would render the same "awaiting approval" panel, so an operator whose approver HAD signed, and whose
  // console was matching on a plan hash it could not compute (or a different one from the approval's), would
  // have no way to say so, and neither would the pack. The reporter latches per class, so the five-second
  // poll below cannot turn a persistent block into a count in the thousands, and the legitimate state
  // (genuinely awaiting an approver) records nothing.
  ...
  // through to the apply handler and onto the receipt).
  const renderArmed = (approval: RestoreApproval): HTMLElement => {
    ...
    armed.appendChild(
      verdictSurface({
        ...
        // an instant it cannot parse, and a sentence that keeps its verb and blanks its date reads as a
        // fabricated claim, so the clause is dropped whole rather than rendered empty. That is also the only
  ...
  //
  // THE LISTING'S OWN LAPSE CHECK ONLY COVERS HALF THE CASES. findUsableApproval refusing a lapsed record
  // closes every case where the LISTING is read after the deadline: a fresh navigation, a reload, the
  // batch queue, the operator's own Check for approval. It covers nothing for a tab that armed WHILE the
  // approval was still live and stays open past the deadline, because startPoll stops the moment the gate
  // arms, so nothing is left running in that tab to notice the deadline pass. watchForLapse is what closes
  // that remaining case.
  //
  ...
  // withhold one.
  //
  // THE HANDLE IS HELD, AND ONE AT A TIME. renderArmed runs on every re-arm, so without a held handle each
  // re-arm would add another live timer running for up to the approval's own remaining lifetime, each
  // retaining this closure's `section` and `approvalHost` even after the panel has left the screen.
  // Clearing the previous one before arming the next keeps exactly one, and `stop` (returned by this
  // function, called from the panel's teardown and from the section's own disconnect) clears that one.
  const watchForLapse = (approval: RestoreApproval): void => {
    ...
    // to the maximum, it is silently reset to 1 ms. So an approval more than 24.8 days out would retract
    // about one millisecond after it armed: the operator would watch "Approved and ready to apply" turn
    // into the window-has-closed sentence over an approval that was live, valid and signed by a distinct
    // approver, with the remedy on offer being to re-run a plan that needed no re-running.
    //
  ...
  // showUnarmed), because rebuilding it would wipe a half-typed reason-for-change; but the cause is not known at
  ...
  // downgrade. So the sentence is carried on the screen, where it is needed, and the vocabulary is left
  // alone.
  ...
  // it is not already showing the unarmed panel. Rebuilding it on every recheck (the 5s poll runs
  // recheck on each tick) would wipe a half-typed reason-for-change, destroy keyboard focus, and erase
  ...
  const recheck = async (): Promise<boolean> => {
    ...
    try {
      ...
      // does not there is no record to find and the read can only ever come back empty. Without this check
      // the screen would sit on "Needs approval" forever on a solo estate, even though the engine would
      // accept the apply: the console would still refuse something the engine does not require.
      //
      ...
      // A PROMISE .catch CANNOT DEFEND A CALL THAT FAILS BEFORE IT RETURNS A PROMISE: `engine
      // .getConfigApprovalPolicy().catch(() => null)` would defend a REJECTION and nothing else. If the
      // method is absent or throws while evaluating, the TypeError is raised before .catch is attached and
      // escapes to recheck's outer handler, taking every classification branch below with it -- collapsing
      // four distinct gate-block classes into one generic state, so an operator holding a valid approval for
      // a DIFFERENT plan would be told nobody had signed and sent to fetch one they already had. try/catch
      // covers a throw and a rejection alike, rather than only the half that returns a promise.
      ...
      // rejects. A lookup that does NEITHER reaches no catch and no continuation, so approvalHost would keep
      // the synchronous "Checking whether this plan has an approval" placeholder for the life of the screen
      // and neither .restore-confirm__unarmed nor .restore-confirm__armed would ever exist -- a frozen
      // screen with no error, which is worse than either alone: the operator cannot act and has nothing to
      // report.
      //
      ...
      //
      // THE EXPIRY IS TAGGED, NOT JUST THROWN, as its own ApprovalsReadTimeout type rather than a bare
      // Error. A bare Error would fall through to the generic showUnarmed() at the foot of the catch, which
      // paints the standing dual-control copy: a plan that WAS approved, by a distinct approver, bound to
      // the exact plan hash, would read as a plan nobody had signed, with the remedy on offer being to raise
      // a SECOND request for an approval that already exists. A read that could not be completed is not a
      // fact about the approval, and that is the one thing this branch must not say.
      //
      // THE LOSING ARM IS CANCELLED. Promise.race settles on the first arm and abandons the other, so a
      // healthy read that comes back quickly would still leave this timer armed for the rest of the
      // 12-second window. The poll below re-enters this function every APPROVAL_POLL_MS, so an unarmed gate
      // would otherwise hold several dead timers at all times for the life of the screen, each retaining
      // this closure. It is cleared in the `finally` at the foot of this try, which is the only place every
      // exit passes through: there are eight returns between here and there, across the try and the catch.
      ...
      // even compared (engine/src/admin/auth.ts, the tokenRateLimited branch) and answers 429 for it
      // (engine/src/admin/router.ts, the verdict.throttled branch), so it says nothing about the caller's
      // session. This branch is the console half of that pair, and testing it first is not an ordering
      // nicety: leaving a 429 to fall through to the generic branch below paints correctly only by
      // accident, and nothing then stops a future widening of isUnauthorised from treating a throttled
      // approvals poll as a reason to end a session in the middle of a restore.
      //
      ...
      if (isUnauthorised(err)) {
        // PAINT FIRST, THEN LEAVE. Returning here without painting would be the only path through this
        // function that leaves approvalHost holding the SYNCHRONOUS "Checking whether this plan has an
        // approval" placeholder it was seeded with: the host is only ever replaced asynchronously, so on
        // this path it would never be replaced at all, and the operator would sit on "Checking..." forever
        // with neither .restore-confirm__unarmed nor .restore-confirm__armed existing for anything (a
        // person or a test) to wait on.
        //
        // Reaching here now means a genuine 401 (a rate-limited throttle is handled above, before this
        // branch), so the sign-out is the right answer. The paint stays regardless, because a session that
        // ends while the operator is mid-flow must still leave a readable screen behind it rather than a
        // frozen one.
        showUnarmed();
        goSignedOut();
        return false;
      }
      // The approval mechanism not yet wired (or a transport fault): degrade to the request step honestly,
      // never a misleading enabled Apply. The request panel surfaces the pending dependency when the
      // request route is also absent.
      showUnarmed();
      return false;
    } finally {
  ...
  // The interval handle is held in pollTimer, so `stop` can end the poll at once rather than at its next
  // tick. The tick's own three exits are the on-screen lifecycle; stop is the off-screen one.
  const startPoll = () => {
...
// crossAccountConfirmTargets returns the DISTINCT foreign target accounts a cross-account restore would
// write into, sorted for a deterministic, reproducible match value. crossAccountWarnings carries one entry
// PER LEG (cf-config, media), and each leg's targetAccount is whatever accountId the request supplied for
// that leg, so a plan with a cf-config leg targeting one foreign account and a media leg targeting a
// DIFFERENT one has two warnings naming two accounts. Every distinct account must be typed before Apply
// arms, so a second, untyped foreign account can never be confirmed to the engine on the strength of a
// gesture the operator never made toward it. Deduplicated because two legs can legitimately share one
// foreign target, in which case the operator still only has one account to type.
...
// renders the receipt (the maker != checker pair) or the honest failure verdict. A
...
async function applyRestore(ctx: ApplyControlsCtx, applyBtn: HTMLButtonElement, oneSentence: string): Promise<void> {
  ...
  // retrySubset re-enters the FULL flow scoped to the failed record names. The names become
  ...
  // CROSS-ACCOUNT: the type-to-confirm gate above (confirmApply) already requires the operator to type
  // EVERY distinct foreign target account crossAccountConfirmTargets finds across all legs, not just the
  // first -- that is what makes it safe to stamp confirmDifferentAccountId onto each cross-account leg here
  // unconditionally, echoing that leg's own target account. A same-account leg is left untouched, so the
  // common case is unchanged. The engine still cross-checks the SIGNED origin, so this only unlocks a write
  // the operator has explicitly confirmed, never one to an account the archive did not come from without
  // confirmation.
  ...
  try {
    // opts carries the break-glass restore panel's browser-recovered per-run master (never present for the
    // standard operational-key flow, where opts is undefined). The engine threads it into the SAME
    // runRestore open the dry-run preview already used (engine/src/admin/router-restore.ts:302,
    // unconditional on body.confirm), gated by the identical capability check and dual-control reservation
    // this whole function already runs above -- there is no separate "break-glass apply" code path to keep
    // in sync, only this one, able to carry a master.
    ...
    // a race (the approval was consumed/rejected/expired between arming and apply), so we also
    // re-hide this stale Apply and prompt a re-check. Any other transport/auth fault stays the
    // inline block error with Retry.
    const kind = classifyError(err, { origin: location.origin });
    if (kind.kind === "restore-unapproved") {
      paintStepper(3);
      applyBtn.remove();
      // SIX CAUSES ARRIVE HERE AS ONE BYTE-IDENTICAL 403. engine/src/admin/approvals.ts
      // classifyRestoreApplyRefusal tells `expired`, `consumed`, `applying-lease`, `pending`,
      // `self-approval` and `missing-approver` apart as a closed enum, and engine/src/admin/router-restore.ts
      // flattens every one of them into { error: "restore not approved", planHash }. This console therefore
      // CANNOT say which, so picking one and asserting it -- "a distinct approver must sign this exact
      // plan" -- would be wrong for three of the six.
      //
      ...
      // signing revives it. So the sentence says what is known (the engine refused this apply on the
      ...
      // A TRANSPORT FAULT ON THE APPLY IS THE ONE CALL ON THIS SCREEN WHOSE OUTCOME CANNOT BE KNOWN.
      // Assuming "the gate is still armed and the approval unconsumed, so Retry genuinely re-runs the apply
      // handler" would be an assumption, not a fact: the engine is a Worker on the other side of a socket
      // and does not care whether the browser is still there. The apply can be received, its records
      // written and the approval consumed, all recorded server-side with monotonic instants, before the
      // socket is destroyed, in which case a screen that goes on reading "Nothing has been written yet" and
      // "Approved and ready to apply" is stating both falsely.
      //
      ...
      // right control once the operator has checked. The screen must not assert the two things it cannot
      // know.
      paintStepper(4);
    ...
    // Stop the slow-note timer the moment the call lands (success or failure) so it never fires
...
function renderRequestPanel(engine: EngineClient, plan: RestorePlan, req: RestoreRequest, paintStepper: (step: number) => void): HTMLElement {
  ...
  requestBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      // recordName and cfConfig bind into the plan hash, so they MUST travel on the request or the approval
      // the engine mints carries a planHash that never matches the hash this screen shows the operator:
      // dropping them would leave a granular single-record or a cf-config restore approvable in name only,
      // with the dual-control gate stuck permanently. Pass recordName verbatim, and pass cfConfig as the
      // account + optional zone ONLY: the Cloudflare token is NEVER forwarded here and NEVER enters the hash
      // (a secret must not transit or bind), matching how the requester computes the mirror (helpers.ts
      // restorePlanHash) and how the engine recomputes it server-side.
      ...
      // mediaRestore binds into the plan hash the same way (its account, never the token), so it MUST also
      // travel on the approval request or a media restore's approval carries a planHash that never matches
      // the hash this screen shows the operator, the same failure shape as recordName/cfConfig above.
      // Account only; the media edit token is NEVER forwarded here and NEVER enters the hash.
