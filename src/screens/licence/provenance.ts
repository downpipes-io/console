...
// adds services, assurance and support, not features.
...
// baked version) with the artefact traceability and the
...
export function renderProvenanceBody(data: LicenceData): HTMLElement {
  ...
  // Artefact hash: the engine SELF-STAMPS the SHA-384 of its own deployable bundle and reports it
  ...
  if (hash) {
    ...
    // placeholder below describes; without this branch a corrupted stamp would read as an ordinary
    // dev build. The malformed value itself is never rendered.
    //
    // The raw stamp rides at the top of the support pack (section 3, engine.artefactSha384), so this row
    // points at evidence that is already there. The malformed value has no field on the record and
  ...
  // "rolled-back" and "rollback-failed" belong here for the same reason "applied" and
  // "applied-unconfirmed" do: all four PROMOTED or attempted to promote a version and recorded the digest
  // they enforced, so this account's own evidence of which bytes went live exists for exactly the outcomes
  // an operator would go looking for it after.
...
// and a manual cross-check link against the published release record. This is the
...
function renderVerifiedApply(last: NonNullable<UpdateStatusRecord["last"]>, channelBase: string | null, prov: UpdateProvenance | undefined): HTMLElement {
  ...
  // Each outcome names itself, rather than collapsing every non-"applied" outcome to "rolled back",
  // which is the reassuring word for the one state where the rollback did NOT happen.
  const outcomeWord =
  ...
  // The release-record cross-check is a MANUAL comparison against a link rather than an in-browser
  // fetch. buildCsp() sets connect-src to exactly 'self' (src/worker.ts), so an in-browser fetch of
  // the channel's record would be blocked by the console's own CSP before it reached the network.
  //
  // It is also not proxied through the console Worker the way the claim-code exchange is. channelBase
...
//
// Precise claims: "tamper-evident", "post-quantum hybrid".
