// The read-only "what's in this update" release-metadata block for an AVAILABLE, verified update.
...
// nothing to render (so the caller appends nothing, keeping calm density, no empty block).
export function renderReleaseMetadata(upd: UpdateStatus): HTMLElement | null {
  ...
  // Build provenance shows before the apply: when the signed channel carries a provenance block, say so
