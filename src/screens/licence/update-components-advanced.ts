// The ADVANCED per-component disclosure for the safe-apply update card:
...
export function componentAdvancedSection(engine: EngineClient, updates: UpdateStatus, env: UpdateBodyEnv, ownConsoleVersion: string | null): HTMLElement | null {
  ...
  // standalone control uses (confirmEngineRollback, best-effort, never a blocker); the CONSOLE
  // rollback needs no such pre-flight: console rollback alone is always the safe direction, no plan read.
