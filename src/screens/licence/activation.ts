...
// no Worker secret, no command line, no redeploy. Copy, markup and the no-custody / fail-open framing hold
// across both the claim-code path and the full-token path.
...
// screen's calm-density budget (design-system.md §7a, boxes are for interactive objects, and this control
...
// Honestly absent when the engine does not report it (the common single-account deployment never sets
// CF_ACCOUNT_ID, or an older engine predates the field): the claim then sends no account id.
export function activationSection(engine: EngineClient, lic: LicenceStatus, reload: () => void | Promise<void>, focusAnchorId: string, cfAccountId?: string): HTMLElement {
  ...
  //
  // A NON-EMPTY value that is not a claim code must say so, rather than fail silently: a customer who
  // mistypes or half-pastes the code from their licence email needs to know it did not match, not have the
  // field stay silent while the submit handler quietly ignores it.
  //
  ...
  const tokenField = field({
    ...
    required: true,
    autocomplete: "off",
    // Presence alone is not a validator: the field is already `required`, so field()'s own required arm
    // catches the empty box first, and a validator that can only reject the empty string is the exact dead
    // shape field.ts warns about.
    //
    // The engine wants a SHAPE, at engine/src/admin/router-updates.ts:221, and refuses anything
    // else before it verifies anything: the trimmed token must be at most 20000 characters and must match
    // /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/, EXACTLY TWO dot-joined parts. Inputs like "x", "not a token", an
    // over-length paste, or a three-part "abc.def.ghi" all get 400 "that does not look like a licence
    // token"; a two-part shape that still fails to decode, like "abc.def", passes the shape check and gets
    // a different 400 (reasonCode "decode"). The engine trims first, so surrounding whitespace on a
