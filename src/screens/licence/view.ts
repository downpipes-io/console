...
// payload. The calm contract: three eager sections (the tile band, the Licence card with the fail-open status
// plus the folded-in activation action area, and the read-only Updates section carrying the one safe-apply
// interactive control), with the Enterprise services and Provenance bodies present collapsed. Fail-open is
// stated ONCE, in the Licence card, and pull-not-push ONCE, in the Updates section, never echoed per section.
// The section modules are imported one-way; this view never imports the coordinator.
...
export function render(engine: EngineClient, data: LicenceData, reload: () => void | Promise<void>): HTMLElement {
  ...
  // The console-component update verdict (multi-component): the engine's components map vs THIS
  ...
  //
  // THE CLASS IS `running-unstamped`, NOT `unstamped`. This is a fact about THE BUNDLE THIS TAB IS RUNNING (no
  // baked __CONSOLE_VERSION__ define). The post-apply check's `unstamped` is a fact about what THE ORIGIN just
  // served. console-version.ts calls them "two distinct questions, deliberately separated", and they must stay
  // in separate rows: pushing both under one {console-build-check, updates, unstamped} tuple key would coalesce
  // them into one row, and support could not then tell "the update verdict was computed blind, no apply ever
  // happened" from "an apply landed assets carrying no stamp". Different remedies, same row.
  //
  // LATCHED ONCE PER SESSION, so ordinary repeat visits do not inflate the count and destroy any reading of
  // how many post-apply checks actually failed. It is a fact about the build, and a build does not change
  // while the tab is open: stating it once is stating it exactly as often as it is true.
...
// consoleUpd is the SPA's own console-component verdict (multi-component), so
...
function summaryBand(data: LicenceData, consoleUpd: boolean): HTMLElement {
  ...
  // exactly the version the channel recommends, so updateAvailable is false, which without this precedence
  // would read as a green "Up to date" over an engine serving a build its own canary rejected.
...
// folded-in Activate licence action area (a status and its one action are
// one object).
function licenceCard(engine: EngineClient, data: LicenceData, reload: () => void | Promise<void>, isEnterprise: boolean): HTMLElement {
  ...
  // and the Provenance section do not repeat it); the full security-feature list
  ...
  // THE LICENCE WAS MINTED FOR ANOTHER ACCOUNT. The engine computes this verdict on every licence read.
  // Without surfacing it, a token issued against someone else's Cloudflare account would be stored, reported
  // as the tier it names, and confirmed to the operator as activated, with nothing anywhere saying it was the
  // wrong licence.
  //
  ...
  if (licenceAccountMismatch(data.licence)) {
    ...
    // THE ENGINE HAS NOT IDENTIFIED ITS OWN ACCOUNT YET, the state GET /admin/status's cfAccountId being
    // absent leaves. Saying nothing here would read exactly like "binding does not apply" or "binding
    // already happened" -- indistinguishable for a fresh deployment's operator. A quiet hint, not a banner:
  ...
  // THE EXPIRY BANNER IS GATED ON EXPIRY, not tier. The engine resolves an expired licence to tier
  // "community" (its fail-open path funnels every reason code through one community() helper), so gating
  // this banner on `tier === "enterprise"` would make it unreachable for the one customer whose licence had
  // actually expired. It reads licenceExpired instead, which asks reasonCode, the closed discriminator the
  // engine actually sends.
  //
  // The copy does not name Enterprise, because on this path the console cannot know the tier: the engine
  ...
  // A withheld line and a MALFORMED one must not render identically (both simply absent), since a mis-minted
  // licence or a corrupted estate figure would then read as an ordinary quiet card. Name the fault instead:
  // the console never renders the malformed value, only the fact that it could not be read. The malformed
  // estate figures ride in the pack raw and nothing else flags them, so neither the bot nor a support
  // engineer is prompted to look at a card that reads as an ordinary quiet one. The console already has the
  // honest verdict on screen; this puts it in the bundle.
  ...
  // so there is no renews-soon cue and no expired cue), which is the exact shape of a healthy licence. Say
  // that the date cannot be read, so nobody waits for a warning that can never fire: a disarmed warning is
  // indistinguishable from a licence that is simply not near its expiry, and that gap must not be silent.
  ...
  // --- Activate licence (the Licence card's action area) ---------------
  ...
  // sections (calm-density): a licence status and its one action belong to the same object. Owner-gated
...
// --- Updates section --------------------------------------------------------
// An unboxed section (boxes are for interactive objects; this is read-only
...
function updatesSection(engine: EngineClient, data: LicenceData, reload: () => void, consoleUpd: boolean): HTMLElement {
  ...
  // URGENT one-click rollback: when the hourly canary found a promoted-but-unsettled new version UNHEALTHY,
  // the engine records rollbackNeeded (it holds no deploy credential, so it cannot revert itself). Surface it
  // PROMINENTLY, above the normal apply control, as a first-class rollback the owner completes in one click.
  // It outranks an available update (fix the live engine first). Owner-gated; the engine enforces.
  //
  // A second incident needs its own surface: a settle that records "rollback-failed" writes pending:null and
  // carries no rollbackNeeded forward (setUpdateSettled's engine branch clears it, because settling normally
  // does resolve the flag), so without updateIncident every signal this section reads would say there was
  // nothing to show while the engine served the rejected build. updateIncident derives it from the recorded
  // outcome.
  ...
  // the read-only facts above (no empty interactive box). Owner-gated (mirrors the engine's
  ...
  // destConfigured reuses the engine's existing StatusReport.destConfigured fact (already loaded for the tile
  // band above), so a live apply can never be offered with nowhere to fly the canary flight that verifies it.
  ...
  // the engine has no known-good target and the control would always refuse, so it stays unrendered. When
