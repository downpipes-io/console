...
// SAME-ORIGIN, through the console Worker's narrow claim proxy (src/worker.ts, isControlPlaneSurface):
// buildCsp()'s connect-src 'self' blocks a direct fetch to https://control.downpipes.io from the browser,
// so the Worker holds the vendor host and the browser never names a foreign origin.
...
// tries again.
//
// UNREACHABLE and ERRORED are deliberately two different sentences: a claim service that ANSWERS with a 500
// must not read as "could not be reached. Check your connection", which would send a customer whose network
// is perfectly fine to go and inspect it, when the actual fault is a vendor outage they can do nothing
// about. The copy only says "could not be reached" when nothing answered.
...
//
// THE PACK SEES ALL TEN OUTCOMES. This exchange is a direct
// fetch to the vendor control plane, not an /admin/* call, so it does not pass the engineFetch seam, and
// each outcome is recorded here instead. A vendor 5xx,
// a CORS regression after a control-plane deploy (which hits every customer at once), the 8-second timeout,
// and a 200 whose body carries no token are four completely different incidents that would otherwise read to a
// support engineer as one. "band-full" is the tenth: a
// legitimate business refusal (the licence's estate band is already fully bound) rather than a fault,
// carried through with its own operator-facing message rather than folded into the generic error text.
//
...
// claim then sends no account id.
