...
// stage completes (never a growing list on the happy path), plus a collapsed "Details" disclosure carrying
// the full step-by-step log for support (step checklists, per-component rows, the verbose recorded-outcome
// prose) -- present but invisible by default, so the detailed log moves behind Details, never in front of
// the operator. Uses the repo's existing disclosure idiom unchanged (<details class="disclosure"><summary>
// ...<div class="disclosure__body">, the same shape update-ramp.ts / update-components-advanced.ts already
// use), so it matches every other collapsed section on this screen.

import { h } from "../../lib/dom.ts";

// ProgressRegion is the small DOM handle the orchestrated flow drives: setStage for each of the progress
// strings (with the optional "this can take a minute" subline), setTerminal for the flow's one terminal or
// stall sentence, and detail() to push the verbose log into the collapsed disclosure rather than the
// visible line.
export interface ProgressRegion {
  ...
  root: HTMLElement;
  // setStage replaces the visible line with the next stage string; an optional calm subline renders
  ...
  // setTerminal replaces the visible line with one of the terminal/stall sentences (the caller supplies
