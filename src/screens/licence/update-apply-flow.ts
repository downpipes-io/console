// The LIVE apply flow for the safe-apply update control. ONE token paste + ONE click drives the
...
// state that keeps checking by itself. NO user-facing refresh exists anywhere in this flow: every deploy in
// it swaps a worker, so a naive next read would race the swap window; update-retry.ts's
// settleWithBudget/readRecordedStatusWithRetry are what avoid that. The full step-by-step log renders into
// the collapsed Details disclosure (update-progress.ts); the visible line only ever shows the four stage
// strings, the two terminal sentences, or the stall line.
//
...
// COPY goes through the same progress/terminal machinery as every other path.
//
// TOKEN CUSTODY: the one-shot deploy token arrives as a parameter, lives only in this
// call's stack/closures for the apply -> settle -> (check/rollback) chain, and is never persisted, copied to
// an attribute, or echoed by the engine.
...
// recoverSettleConclusion is the persistence-first settle recovery: a settle's HTTP response can lose a
// race against the engine's own auto-rollback deploy, so when the settle call throws or returns a
// non-definitive result, the response is NOT the truth, the engine's persisted record is. It replaces the
// settle heading with the no-claims interim line, re-reads GET /update/status with a bounded retry (never a
// single shot), renders the RECORDED conclusion (applied / rolled-back with the engine's reason / expired-cleared /
...
export async function runLiveApplyFlow(
  ...
  // ONE progress region for the WHOLE flow, so the card stays one interactive object. It is built
  ...
  try {
    ...
    if (promote.outcome === "refused") {
      // Refused BEFORE any change: the engine is unchanged. The destination gate is the one
    ...
    // the two-phase engine settle -- ever runs for it). This branch must run BEFORE the "promoted" check
    // below: a console-only response never carries "promoted" (that string is specific to the two-phase
    // engine flow), it carries the console's OWN outcome string ("applied"/"applied-unconfirmed"/
    // "rolled-back"/"rollback-failed"), so falling through to the "promoted" check would report a
    // genuinely successful console-only apply as an unexpected outcome.
    if (!includesEngine) {
    ...
    if (includesEngine) {
      ...
      // also not the "outcome could not be read" state the unresolved branch below reports: a queued settle
      // must be recognised here rather than left to burn the whole retry budget and land on that branch,
      // which would tell the operator the engine could not be read when it had answered clearly.
      if (run.kind === "queued") {
    ...
    // destination gate can also surface here if the refusal happened before any change was even
...
// promoteSummaryDetail / settleOutcomeDetail render the verbose phase-summary prose for the Details
// disclosure only (the visible line only ever carries a stage string or a terminal sentence). Kept local
// and tiny (a handful of plain-words branches) rather than reusing promoteSummaryLine/settleOutcomeLine's
// SHAPE-typed signatures, so the Details content stays honest even from the recovered (non-SettleResult)
// path above.
...
// droppedSourcesBanner is the loud, actionable post-update source-verification prompt (the same wording +
// action the pending-verification body uses): a KEPT update whose pre/post binding diff found
