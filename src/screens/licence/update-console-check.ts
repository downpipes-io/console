// The post-apply CONSOLE build check. After a live apply that landed the
...
export async function runConsoleBuildCheck(opts: ConsoleCheckOpts): Promise<boolean> {
  ...
  // check that failed and a tab the operator then closed would leave the pack asserting a healthy console update
  // with nothing anywhere to the contrary. The class separates the remedies: `wrong-version` is an origin still
  ...
  if (confirmed) {
    // AND STASH IT ACROSS THE RELOAD WE ARE ABOUT TO PERFORM. The ring is in-memory, and this flow calls
    // location.reload() ~1.5s from now to finish the update, which destroys it. Without a stash, the
    // `confirmed` row would be written and then destroyed by every successful console apply's own reload,
    // making "no console-build-check row" indistinguishable from "the check never ran" on every healthy
    // update. The stash carries the class across the one reload this flow itself triggers; it is re-admitted
    // by set membership and drained into the fresh ring at boot.
    //
