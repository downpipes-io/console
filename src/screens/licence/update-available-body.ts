...
// custody, copy and control flow of the engine-only body are unchanged; the live apply progression
...
// the two buttons (updateBtn is OPTIONAL: the destination-gate case renders Preview with no
...
interface PreviewContext {
  ...
  // PreviewContext: the destination-gate branch renders Preview with no apply control at all, so
...
// always the structured PromoteResult. Preview is offered even with no destination configured: a
...
async function preview(ctx: PreviewContext): Promise<void> {
  ...
  try {
    ...
    // changed nothing). Optional-chained: the destination-gate branch has no apply button and no
...
export function availableUpdateBody(engine: EngineClient, updates: UpdateStatus, env: UpdateBodyEnv): HTMLElement {
  ...
  // DESTINATION GATE: a live apply can never confirm a canary flight with nowhere to fly it, so
  // with no destination configured the apply control (the token, Update now, the advanced per-component
  // controls, the ramp) is REPLACED by the destination-gate line + a link to /destinations. Preview stays available
  ...
  // The acknowledgement gate: a release step marked blocking must be acknowledged before the LIVE apply
  // enables (the "Required" badge alone is not enough to stop a click). When the
  ...
  if (canApply && blockingSteps.length > 0) {
    ...
    // Group-level doc link: the acknowledgement checkboxes are one gate, so the link
