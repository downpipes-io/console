...
// arbitrary node), an honest "field not reported" placeholder, and the Enterprise renewal sentence.
// Pure DOM leaf builders, no state, no engine calls.
...
//
// This line must never promise a grace window: there is no such thing. engine admin/licence.ts checkExpiry
// is `expiry <= Date.now()`, so a licence lapses on the instant its validity date passes, with no buffer of
// any length. What protects a customer whose renewal is late is FAIL-OPEN: backups and restores continue,
// while the paid services and assurance do lapse, so the copy must not claim nothing is interrupted. This
// is the sentence a customer reads in the thirty days BEFORE they lapse, so it is the one that decides
// whether a late renewal frightens them.
