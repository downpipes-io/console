// The OPT-IN gradual percentage ramp control for the safe-apply update section.
...
// atomic "Update now". It is collapsed by default, clearly secondary to the one-click apply, and carries
// the caveat that a ramp serves REAL production traffic: it is not an isolated sandbox (isolated previews
// would need *.workers.dev, which is banned). It collects the same one-shot deploy token (held only locally
...
export function rampSection(engine: EngineClient, out: HTMLElement, reload: () => void, restoreFocus: () => void): HTMLElement {
  ...
  inner.appendChild(
    h(
      "p",
      { class: "field__hint" },
      // Starting the ramp cannot canary the slice it is creating, so nothing is auto-verified and nothing
      // auto-reverts. The check is a second, deliberate step, and it is the operator who keeps or rolls back.
      // This control must never claim an automatic safety net it does not run.
  ...
  async function startRamp(): Promise<void> {
    ...
    try {
      ...
      // The reload is essential here: the engine's success outcome is always "ramp-pending" (phase 1 cannot
      // canary the slice it is still creating), so this is the branch that fires, and the reload is what
      // re-renders the Updates section so the pending card appears. That card, which recognises a ramp from
      // the recorded percentage, is the only surface that can settle a ramp. Without the reload the operator
      // has no route to finish the check while a live traffic split sits unverified.
      //
