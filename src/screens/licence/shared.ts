...
// section module (which would form a cycle); each section imports one way from here.
...
// The ONE version-skew decision, shared with the Overview's Updates tile (lib/update-skew.ts), so a
// payload the versions cannot compare cannot render as a green "Up to date" on one surface while this
// one calls it out. Both surfaces read one predicate and print one set of words.
...
//
// The "pending" arm carries `percentage` because the pending's SHAPE decides WHICH settle route can finish
// it: the engine records a `percentage` on a pending ONLY when it came from the opt-in gradual ramp, POST
// /update/settle refuses that shape, and POST /update/ramp/settle refuses the shape without it. Absent means
// an ATOMIC pending (the plain settle); present means a RAMPED one (the ramp settle), and it is the live
// traffic percentage the ramped version is serving.
...
// options object keeps each builder under the four-parameter limit.
export interface UpdateBodyEnv {
  ...
  canManage: boolean;
  // destConfigured: whether at least one destination is configured, reused verbatim from the engine's
  // existing StatusReport.destConfigured fact (already loaded for the tile band; no new wire field needed).
  // A live apply can never confirm a canary flight with nowhere to fly it, so when this is false the
...
export function lastUpdateSummary(updateState: UpdateStatusRecord | null): string | null {
  ...
  switch (last.outcome) {
    ...
    case "rollback-failed": {
      ...
      // no pending, rollbackNeeded already cleared by the settle) has to tell the operator, so it must not
      // fall through to the silent default arm below.
    ...
    case "expired-cleared":
      // This standing sentence is UNPREFIXED (it is not phrased as "Last update: ...", it is its own
      // first-class reassurance) -- a first load straight onto this outcome (no live flow ran) must read
      // identically to the flow's own terminal rendering of the same outcome.
...
//
// A blind string transform here (strip a verb prefix, strip a trailing ": <status>", show whatever is
// left) is correct only for a refusal the ENGINE composed and the client folded in as
// "<verb>: <reason>: <status>", and wrong for every other shape the same throw can take, because the
// transport folds INTERNAL PRODUCT TOKENS into the message by design and they would survive a blind strip
// untouched. On the licence-activation field, at the moment a customer has just paid, the paste error
// could read `engine-binding-absent`, `console-origin-fault`, `access-redirect`, `html-body-not-json`, a
// bare `503`, or the browser's own `Failed to fetch`. Every one of those is a word this console defines
// for its own diagnostics; none names anything the operator can do, and `503` in a field that otherwise
// explains typos reads as a corrupt token.
//
// The transform itself lives in components/error-view.ts (refusalTextFrom), beside errorDetail, because
// the shape it reads is the TRANSPORT'S and not this route's: every save in the console throws it. These
// two wrappers stay verb-specific: a pattern that names the verb it expects is strictly tighter than a
// generic one, so the licence and update fields keep the narrowest possible strip.
//
...
// absent (an unstamped build / older engine). It reads status.artefactSha384, trims it, and accepts ONLY a
// well-formed 96-hex-char SHA-384 (the engine's own contract) so a malformed/garbage value is treated as
// honest absence rather than rendered as if it were a real hash. Pure; the provenance card branches on null
// vs the hash.
...
// A settle's HTTP response can LOSE a race against the engine's own auto-rollback deploy: if the canary
// fails shortly after promote and the engine rolls back, the settle response can die with the deploy. The
// engine persists the settled outcome (applied / rolled-back + reason) to its DO BEFORE any rollback
// deploy, so the RECORDED status is the truth a broken response cannot carry. The console's rule: after a
// settle that throws or returns a non-definitive result, NEVER assert an outcome from the failure itself;
// re-read the update status and render what the engine recorded.

// SETTLE_UNCONFIRMED_LINE is the interim copy while the recorded outcome is re-read. It deliberately claims
// NOTHING about the update's fate: asserting "the new version is live" here would be false whenever the
// settle response has raced a rollback.
...
// recorded it rolled back (with its reason), cleared a stale pending as expired (a first-class outcome,
// never "could not be read"), still has it pending verification, or the record could not be read at all
// ("unknown").
export type RecordedSettleOutcome =
  ...
  // The engine's "rollback-failed" and "applied-unconfirmed" outcomes are DEFINITIVE conclusions, not a
  // settle that has not finished: the engine settled, wrote its record and cleared the pending. What
  // differs is that one of them is an INCIDENT, so neither may fall through to "unknown" and the stall line.
...
//
// `component` selects WHICH slot to read: "engine" (the default) reads `rec.pending`/`rec.last`; "console"
// reads `rec.lastConsole` instead and never consults `rec.pending` (a console apply is single-phase and
// never leaves an engine-shaped pending record awaiting settle -- see UpdateStatusRecord.lastConsole).
// Reading the engine's `last` for a console-only apply would read the WRONG slot (the engine's unrelated
// outcome, which a console-only apply never touches) and would report "unknown" whenever the engine had no
// last-outcome of its own yet, even though the console's own outcome is sitting in `lastConsole`.
...
// used inside the Details disclosure, which holds the step-by-step log and this kind of explanatory prose;
// the terse happy-path terminal sentence the operator sees by default is update-outcome-copy.ts's
// appliedTerminalLine/rolledBackTerminalLine instead). Honesty rules:
...
export function recordedSettleOutcomeLine(o: RecordedSettleOutcome): string {
  switch (o.kind) {
    ...
    case "rollback-failed": {
      ...
      // say the safety net held; this one cannot, and softening it would tell the operator the safety net
...
//
// The console-only branch checks the console's OWN terminal-success outcomes ("applied"/
// "applied-unconfirmed"), never "promoted": a console component is atomic and self-verifying in one
// request, so the engine answers with its own outcome vocabulary rather than the two-phase ENGINE flow's
// "promoted" (that string is specific to this function's sibling, runLiveApplyFlow, which settles the
// engine flow separately).
...
// three copy states: checking, confirmed (the reload prompt), and not-confirmed (the
...
// live in ./detail-rows.ts: this file is at the max-lines guardrail.
