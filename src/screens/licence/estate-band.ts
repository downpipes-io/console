...
// rather than added there, matching the family's pattern for a small, cohesive sub-feature
...
//
// The two sentences, and the calm-density rule they both keep (design-system.md §7a): quiet plain
...
//     and the self-serve terms page; the email's wording is canonical and this line matches it.
...
// surface uses for this figure (bandCoversPhrase, the pricing page). parseBandFeatures's
// all-or-nothing check means a feature key that does not match this name would silently fail the
// triple on every real licence and the band line would never render for any customer.
...
// bandCoversPhrase's rendering of the identical number in the licence emails byte-for-byte, which
// cross-surface agreement outranks matching the estate line's own, differently-sourced,
// continuously-measured figure.
...
// the type promises a full EstateSummary, the wire does not, so this degrades honestly instead of
// rendering a bad number).
...
// validly present: a missing piece or a malformed number means show nothing extra, never a
// partially-guessed sentence. The tier id itself is accepted as any non-empty
...
// the key control-plane's tierToFeatures actually emits (issue.ts:314); a parser reading any other
// key would withhold the band line and the over-band check on every real licence.
