...
// Kept in its own pure, DOM-free leaf (mirrors this family's existing convention, e.g.
// update-release-metadata.ts, for a cohesive slice of copy) so the validator pins each string directly and
// no renderer hand-composes its own variant.
//
// One deliberate substitution: house lint (lint:prose, scripts/writing-rules-source.mjs) bans a typographic
// em dash (U+2014) anywhere in src, including inside a string literal, so the stall-state sentence below uses
// the ASCII "--" this family already uses for the identical reason (componentRowLine's "Console -- up
// to date", consoleReloadPromptLine's "reload to finish").

// ---------------------------------------------------------------------------
// The in-place progress stages (each REPLACES the last; plain English, no engine jargon)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// The honest stall state -- the WHOLE client retry budget is exhausted; it keeps polling by itself, no
...
// ---------------------------------------------------------------------------
// The stale-pending-expired first-class outcome. Never "could not be read".
// ---------------------------------------------------------------------------
...
// appliedTerminalLine is the "Updated to <version>." terminal sentence, optionally extended with the
...
// rolledBackTerminalLine is the "Rolled back automatically: <reason>. Nothing else was changed." terminal
...
// ---------------------------------------------------------------------------
// The engine's "rollback-failed" and "applied-unconfirmed" outcomes each need their own copy: without it,
// both would render as the STALL_LINE above, which is wrong for a settle that has already concluded. Both
// sentences say what state the engine is in, and both name the remedy, because in each case the remedy is a
// control this screen already offers.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// The destination gate -- the engine's exact refusal reason (matched verbatim to detect it reactively)
...
// ---------------------------------------------------------------------------
// The paired-rollback confirm line, shown BEFORE the token is spent when the engine's rollback plan
