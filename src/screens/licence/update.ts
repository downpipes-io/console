...
// and the two body builders live in sibling modules.
...
// module keep their existing import (the implementation lives in update-release-metadata.ts).
...
// not-settled update). It is the ONE boxed interactive object in the Updates section (boxes are for
...
//
// destConfigured is the caller's already-loaded StatusReport.destConfigured fact, threaded down to the
// available-update body so it can replace the apply control with the destination-gate line when false.
// Defaults to true, so a caller that does not pass it keeps offering the apply control; this is additive.
export function updateControl(engine: EngineClient, updates: UpdateStatus, state: UpdateControlState, reload: () => void, destConfigured = true): HTMLElement {
  ...
  // single-engine wording applies when no components map is present.
  ...
  // not know whether it is true: printing it at a genuine Owner, on the screen carrying the action that
  // most often clears the fault, would be wrong. Worse, an engine that predates /admin/whoami is one of the
  // causes of that state, and an engine update is its remedy, so this is the sentence a customer would meet
  // at the exact moment it is least true and most in the way.
  //
  // The gate is not lifted: capGateReason still refuses, and it carries what to do about the specific
  // failure (lib/identity-remedy.ts). The claim is only about whether the role could be read, never an
  // assertion of the reader's role itself.
  if (!canManage) {
