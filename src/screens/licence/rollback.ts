...
// urgent (the hourly canary found a live-but-unverified bad version, the engine's rollbackNeeded flag) leads
...
// Both call POST /admin/update/rollback (the SAFE recovery direction the engine never gates behind a second
// owner) and both collect the one-shot deploy token (held only locally for the call, never stored).
...
//   urgent:true, the hourly canary found a live-but-unverified bad version (the engine's rollbackNeeded flag):
...
export function rollbackControl(engine: EngineClient, mode: RollbackControlMode, reload: () => void): HTMLElement {
  ...
  // never a blocker) and adds a warning line when the engine reports the target would strand the live
