...
// small presenters in ./security-centre/shared.ts. The split keeps the file size manageable while the public
...
// shell/registry.ts, so this descriptor contributes none here.
// ---------------------------------------------------------------------------
...
export const securityCentreScreen: Screen = {
  ...
  render(ctx: ScreenContext): HTMLElement {
    ...
    // scroll at construction time, against the short skeleton, would target a position the page then
    // scrolls past as the regions load. Track each region's first settle and fire the scroll once both are in.
...
// buildCoverageRegion builds the SECOND, independent async region:
...
function buildAccessControls(engine: EngineClient, ctx: ScreenContext): { el: HTMLElement; deepLinkTarget: HTMLElement | null } {
  ...
  // is off by default so a one-identity estate is not locked out of applying a restore. A deep link
  // (/security?open=restore-approval) opens it.
    ...
    actions: [
      { label: "Cancel", variant: "secondary", onClick: () => {} },
      {
        ...
        onClick: async () => {
          ...
          // gate armed the engine answers 202 and stores nothing. Without this branch, the queued
          // answer's missing `counts` field would make the destructure below throw a raw TypeError over a
          // request the engine had accepted, showing the operator a crash rather than "queued for approval".
