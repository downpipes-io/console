...
// audit trail out.
// Delivery rides one of three sinks: an HTTPS endpoint, an S3 bucket the SIEM reads, or a
...
//
// Precise claims only: this is at-least-once delivery, never "guaranteed".
...
// what to offer (disabled-with-reason, never hidden, design-system.md).
...
async function toggleEnabled(opts: { engine: EngineClient; view: PushDestinationView; refresh: () => void; btn: HTMLButtonElement; errorHost: HTMLElement }): Promise<void> {
  ...
  if (!input) {
    ...
    // carries no endpoint). Returning here in silence would leave the operator clicking Enable with
    // nothing happening: no error, no toast and nothing in the pack. Say what is wrong and name the
...
function openPushForm(opts: PushFormOpts): void {
  ...
  //
  // IT VALIDATES. The field enforces the vendor's credential scheme, matching the note below that tells the
  // operator to paste the token with its scheme prefix: a field that merely required non-empty text would
  // accept a value the note itself says is not enough, on a field that is never re-displayed, against an
  // endpoint whose only answer to a bare token is a 401 that looks exactly like a revoked credential.
  // pushAuthSecret enforces the vendor's scheme, plus the engine's own length and control-character screen
  // (push-secret-drift-gate.mjs differentially fuzzes it against the engine's isValidPushHeaderValue, so the
  // bound is derived rather than transcribed).
  ...
  // carries a scheme. It is gated on the vendor's credScheme, not on the format: CrowdStrike Falcon Next-Gen
  // SIEM shares splunk-hec over http with Splunk but takes a plain bearer token, so gating on format alone
  // would tell a Falcon operator to prefix their token with the word Splunk. The text is built from the same
  // credScheme the validator enforces, so the instruction and the enforcement are one decision.
