...
//
// This is at-least-once delivery, never claimed as "guaranteed".
...
// engine's retention cap. At the cap the console says "most recent N", never a total it cannot know, the
// same discipline settings/support.ts PULL_TRAIL_MAX follows.
const PUSH_TRAIL_MAX = 50;

// The bounded handful of individual attempt lines shown at once (calm-density budget):
// pushTrailSummary carries the aggregate count; these are a look, never the full notifications-history
// data table.
...
// LOCKSTEP (FORMATS-AND-TRANSPORT.md). ndjson leads and is the default for a new destination: not one SIEM
...
// the sink is "HTTP endpoint", never "HTTPS endpoint". docs/scripts/check-push-formats.mjs guards the
// FORMAT side with its own FORMAT_TOKEN map mirroring pushFormatLabel(). If a sink label changes here, the
// docs-side mirror has to change with it, and a new sink must be added to the engine's PushSink allow-list
// first (PUSH_SINKS above is in lockstep with it).
...
// checks would SILENTLY SHIP CEF, because a record builder that reads "LEEF if leef, else CEF" puts every
// other format into the CEF arm. A combination that validates and then sends a different wire than the
// operator chose is worse than one that is refused: the receiver sees plausible bytes under the wrong
// parser, and nothing anywhere records the substitution. The engine refuses the same combination at its
// router and its DO, and its sender takes a narrowed two-member type so the fallback cannot be written.
//
...
export function pushDestinationDetailLine(view: PushDestinationView): string {
  ...
  if (sink === "syslog-tls") {
    ...
    // the wrong label. A destination stored with a different format is the only way to reach this arm,
    // and the honest thing to tell its owner is that it is not delivering and what to do about it.
...
// means the console is the only place that code can become a diagnosis.
//
// A Splunk Cloud stack serves its HEC ingest port with Splunk's stock self-signed certificate while 443
// carries a proper one, so strict TLS refuses the ingest port before any HTTP is exchanged. The engine
// names that network-tls rather than network-error, so the operator reads "the certificate chain ... was
// not trusted" instead of a generic failure.
//
...
// discarding routing transform or a blocked indexer queue then drops them with the answer unchanged. Saying
// "delivered" would promise the part nobody outside the SIEM can see, so this says acceptance, matching the
// engine's own drain comment and day-2/audit-log-push.
//
...
export function pushAttemptLine(a: PushDeliveryAttempt): { tone: StatusTone; text: string } {
  ...
  // CLAIM about the size of the delivery: treating it as the else arm for every row whose counts did not
  // all arrive would mean a real batch carrying fromSeq and toSeq but no count renders "your sink accepted
  // a test event", BYTE-IDENTICAL to a genuine single test send. This trail is the only place an operator
  // can check whether their SIEM is receiving the audit log at all, so a row that turns an unknown
  // quantity into the smallest possible one would answer the exact question the screen exists for, wrongly.
  //
