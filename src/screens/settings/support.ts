...
//
// THREE STATES, NEVER TWO. The engine separates "expired" from "the stored expiry cannot be read",
// sending the second as `expiryUnreadable` beside the coarse `expired` boolean rather than instead of
// it, because collapsing the two into one flag is wrong for three reasons an operator pays for:
...
// The console never re-parses expiresAt to decide this: a second derivation here could disagree with
// the gate that refuses the pull, which is the engine's alone to keep in sync.
export function supportGrantPresentation(grant: SupportGrantView | null): {
  ...
  // At the cap the line says "most recent PULL_TRAIL_MAX", never a total it cannot know.
  const pullLine =
    n === 0
      ? "Never pulled."
      : n >= PULL_TRAIL_MAX
...
// pullFreshnessSentence turns the pull trail into one plain, relative sentence ("Last polled 4 minutes ago." /
// "Never pulled yet.") so the owner reads freshness at a glance rather than parsing supportGrantPresentation's
// absolute "last <ISO timestamp>" pullLine. Pure and DOM-free; exported for the validator. nowMs is a
// parameter rather than read from Date.now() internally, so the validator can pin a moment and assert an
// exact sentence.
export function pullFreshnessSentence(pulls: { at: string }[], nowMs: number): string {
  const last = latestPullAt(pulls);
  if (last === null) return "Never pulled yet.";
  const lastMs = Date.parse(last);
  if (!Number.isFinite(lastMs)) return "Pulled at least once; the last recorded time cannot be read.";
  const deltaMs = Math.max(0, nowMs - lastMs);
  const minutes = Math.round(deltaMs / 60_000);
  if (minutes < 1) return "Last polled less than a minute ago.";
  if (minutes < 60) return `Last polled ${minutes} ${minutes === 1 ? "minute" : "minutes"} ago.`;
  const hours = Math.round(minutes / 60);
  if (hours < 24) return `Last polled ${hours} ${hours === 1 ? "hour" : "hours"} ago.`;
  const days = Math.round(hours / 24);
  return `Last polled ${days} ${days === 1 ? "day" : "days"} ago.`;
}

// gapServedBannerMessage is the one honest, plain sentence naming a served gap (SupportGrantView.gapServed):
// what the owner sees, not the engine's counter name, and it states the hole cannot be recovered, only
// detected, matching the plan's error-handling matrix ("Gap or backfill after downtime"). Pure; exported for
// the validator.
export function gapServedBannerMessage(): string {
  return "Some audit events may not have reached your SIEM. A poll on this credential resumed after a gap in the retained log; the missing events cannot be recovered, only detected going forward.";
}
...
// upload is not working, never as the first step.
...
export function renderSupportBundle(engine: EngineClient): HTMLElement {
  ...
  downloadBtn.addEventListener("click", () => {
    ...
    // The console's own bounded ring of coarse, closed-class error records is request-scoped and rides
    // along with this one build. It is read HERE, at the deliberate Generate action,
    ...
    void engine
      ...
      .then((text) => {
        // THE RING IS CLEARED ONLY ONCE THE PACK IS IN THE CUSTOMER'S HANDS. Clearing it before the download
        // succeeds would let a browser that refuses the blob write destroy the very evidence the pack exists
        // to carry: the pack would be gone, the ring empty, and a retry would rebuild a pack with no console
        // error classes in it at all, while the line under this button still promised the customer that the
        // bundle includes them. A pack that quietly stops carrying its evidence is worse than a download that
        // plainly failed.
  ...
  // The one-line inline disclosure is a plain statement, never a checkbox. The section
...
// renderEmailDelivery: the email-delivery test exists because a missing Email Service set-up is
// otherwise a silent no-op: the bootstrap link reports "sent" even though nothing arrives. The
// verdict carries the fix when one is needed; the intro does not pre-teach the dashboard path
// before the test has even run.
...
function scopeBlock({ engine, scope, grant, accessPerimeter, revealHost, refresh, docsHref }: ScopeBlockOpts): HTMLElement {
  ...
  // Gap or backfill after downtime is named plainly here rather than left buried in the support pack.
  // Dormant until an engine sends gapServed (see SupportGrantView), which none does today; renders
  // nothing in the meantime.
  if (grant?.gapServed === true) {
    block.appendChild(banner({ tone: "warn", message: gapServedBannerMessage() }));
  }
  ...
  if (grant !== null) state.appendChild(h("span", { class: "field__hint" }, pullFreshnessSentence(grant.pulls, Date.now())));
...
// browser can decline the anchor click; every recovery artefact other than this one goes through
// lib/file-delivery.ts deliverFile, which is written for exactly that failure and is total. This function
// must be equally total on its own: a throw here must never surface as a failure to BUILD the bundle,
// which is a different fact from a browser that refused to write it, and the caller treats the two
// differently.
//
...
// a surface an older engine does not know has the row dropped on arrival, so widening the vocabulary is a
// paired console-and-engine change, not something this function does alone. The ring-clearing order in
// renderSupportBundle above is what actually protects the evidence.
