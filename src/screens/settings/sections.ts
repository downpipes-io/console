...
// demoMode). None of the engine's secrets are written here.
...
export function renderConnection(engine: EngineClient): HTMLElement {
  // An unboxed section (design-system.md 7a): heading + content on the canvas; a
...
export function renderResidency(engine: EngineClient): HTMLElement {
  ...
  // A bare failure message here would name no remedy and leave nothing to press, hiding this panel's whole
  // answer (where the archives land, and whether that moves data out of the account) for the life of the
  // screen, so the failure state offers a specific reason and a retry instead.
  const load = (): void => {
...
function residencyBody(status: StatusReport): HTMLElement {
  const wrap = h("div");
  // Each destination kind is matched explicitly rather than left to fall through to an else: an
  // unhandled kind (a configured Google Cloud or Azure destination) would otherwise land in the same
  // else arm as "no destination configured" and tell an operator with a working destination that they
  // had none. The compiler cannot flag a missing arm here, because the ternary chain always has an
  // else, so a new destination kind must be added by hand.
  ...
  const destLabel =
    status.destKind === "r2"
      ? "In-account R2 (no destination credentials on the wire; cannot enforce Object Lock)"
      : status.destKind === "s3"
