...
//
//   /access            Enforcement: the LIVE Cloudflare Zero Trust verifier (reads the
//                      REAL verdict, never a hardcoded green) + the Access setup wizard
//                      (instructional; the console configures nothing, captures no
//                      secret).
//   /access/roles      Roles and access: the in-app RBAC table (D3 grant/revoke), the
//                      last-Owner guard state (server-side; mirrored here), the role
...
//                      change-control detail, and honest boundary copy.
...
// tokens carry the light companion and the teal --trust verified accent). Precise claims
// throughout: post-quantum hybrid / tamper-evident, never tamper-proof / used a second
// factor. No custody is never weakened: the verifier reads the engine's verdict; the
// wizard SHOWS the wrangler commands and never submits a secret; every server-supplied
// string is a text node (dom.ts), so the closed audit target union and the role table can
// render only safe fields.
//
...
// buildPostureStrip renders the standing posture in ONE line (design-system.md 7a): counts computed
...
// ---------------------------------------------------------------------------
// Division of responsibility. The three honest rows.
// ---------------------------------------------------------------------------
...
// maturityCard is the persistent, calm capability-maturity disclosure, elevated to
...
function maturityBody(): HTMLElement {
  ...
  // this disclosure carries only the per-control detail list.
  const card = h("div");
...
// so the mutable boolean is not loose module-level surface.
