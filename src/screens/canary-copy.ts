...
export const CANARY_COPY = {
  ...
  states: {
    ...
    // THE LINE SAYS WHAT HAPPENED, NOT WHY. Naming corruption specifically would be the single most alarming
    // thing this screen could say, and it is true of only some deaths: a flight also dies when the RUNLOG
    // entry does not verify fresh and chained, when a record fails to decrypt, when the restore-write itself
    // fails, and when a read-back fails verification for a reason the engine could not classify (the
    // fail-closed catch, which exists precisely because the cause is unknown). The metaphor stays in the
    // label; the line states the verdict and points at deadDestNote, which renders the engine's reported
    // reason where there is one and nothing where there is not.
...
// ailingCauseLine turns the engine's closed ailing cause into ONE honest operator line.
//
// The engine classifies the fault into a specific cause (an expired credential reads differently from a
// black-holed endpoint), and this is the console half of that: each class names the thing to go and check,
// so an operator is never sent looking at the network when the answer is their key.
//
