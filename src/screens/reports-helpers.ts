// Reports pure helpers + small UI/value helpers, kept in their own module so the screen
// module stays under its size bound. reports.ts re-exports the validator-facing ones, so
// importers and the validator use a single import path.
//
...
export function humanSeconds(seconds: number): string {
  // The RTO projection can arrive as something that is not a finite, non-negative number of seconds; the
  // report honestly reads "unknown" in that case, but the silence alone is not enough: an executive reading
  // "we cannot project this" cannot tell an estate with no drill history from an engine that sent a broken
  // figure, so the anomaly is recorded as well as shown.
...
export function rtoEstimateLine(est: RtoEstimate): RtoLine {
  ...
  //
  // {known: true, basedOnDrills: 3} with the seconds field absent, or with seconds arriving as the string
  // "7200", would otherwise render the same "Unknown (no recovery drills yet)" over the basis "No
  // restore-test history yet, so there is no signal to estimate from." as a brand-new estate that has
  // genuinely never run a drill. The two need opposite actions: one operator should go and run a drill; the
  // other has already run several and has an engine sending a figure this console cannot read, so telling
  // them the first story sends them to run a further drill that will change nothing.
  //
