...
function renderDiff(diff: PendingChangeLine[]): HTMLElement {
  ...
  if (lines.length === 0) {
    ...
    // is honest about it, and recording the skew ties "asked to approve a change with no description" to the
    // engine build that sent nothing, rather than leaving it unattributed.
...
function renderChangeActions(engine: EngineClient, change: ConfigChange, me: string | null, reload: () => void): HTMLElement {
  ...
  // and defers to the engine. That is the correct fail-open, but a live-looking button the engine will
  // always refuse is a poor outcome to produce silently, so recording the skew flags the unrecognised
  // kind rather than saying nothing.
