// First load, onboarding wizard, and the key ceremony.
...
// the placeholder onboarding screen (screens/onboarding.ts); it is a
...
//
// The five steps, each deep-linkable at /onboarding/:step and resumable:
//   1. connect        GET /admin/health (+ the CONSOLE_ORIGIN diagnostic), probed on
//                     entry; the honest sign-in/Access verdict renders as one status line
//                     here (the same verdict also lives in the readiness checklist, the
//                     /access verifier and the shell chip)
...
//   4. invite         in-app roles by email, enforced, plus the copyable enrolment link
//   5. readiness      a reviewable posture checklist + the first run (POST /trigger)
//
// No-custody invariants honoured, never weakened: the
...
// in ./onboarding/preflight.ts, and the rotating card deck in ./onboarding/carousel.ts.
...
export const onboardingCeremonyScreen: Screen = {
  // Owns the same route the placeholder screen did, so this is a true drop-in replacement.
