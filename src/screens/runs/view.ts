...
export class RunsView {
  ...
  private async load(): Promise<void> {
    ...
    if (wasRefresh) {
      // isUnsuccessful, not `=== "failed"`: this live region echoes the summary tile that already reads
      // "Failed or abandoned", so counting only the failures would let an estate whose runs are ALL
      // abandoned overnight announce "no failures" to a screen-reader user, on the one screen they opened
      // to find out what went wrong. The wording follows the tile rather than inventing a third vocabulary
      // for the same set.
  ...
  private renderContent(runs: FleetRun[]): void {
    ...
    // disclosed inside buildSummary, matching the restore date-picker and the
