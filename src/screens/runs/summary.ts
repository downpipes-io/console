...
export function buildSummary(runs: FleetRun[]): HTMLElement {
  ...
  // The set the tile below is really asking about: counting only `failed` would let an estate whose runs are
  // all being reclaimed by the lease read a green "none" with the word "needs attention" nowhere on screen,
  // because abandoned runs land in `total` and in none of ok, failed or in-flight. Both must count for the
  // band to add up and for the one tile a customer scans for trouble to actually show it.
  ...
  // run is one of them. The count across the fleet is the secondary, so a backup short of a full copy is loud here rather than
  ...
  // history (notifications/history.ts) disclose for the identically-shaped ring. Without the
  // disclosure below, "Failed or abandoned: 0" would read as "nothing has ever failed" rather than
  // "nothing failed within the last 50 runs of each downpipe" -- a materially different claim for
  // anything run more than about once a day.
  return h(
    "div",
    grid,
    h(
