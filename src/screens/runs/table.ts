  ...
  const columns: Array<DataColumn<FleetRun>> = [
    ...
    },
    {
      ...
      render: (r) => {
        ...
        // The recorded reason, for BOTH terminal unsuccessful statuses, so the one string an abandoned run
        // carries -- the engine's "abandoned (run lease expired)" -- reaches the row rather than leaving it
        // showing abandoned and explaining nothing.
        ...
        // the backup is short of a full copy rather than reading a plain ok, whichever of the three drove it.
        // Absent or 0 on all three shows nothing.
    ...
    },
    {
      ...
      // runBytes is the guard, not `typeof === "number"`: typeof NaN is "number", so a bare typeof test would
      // let a CORRUPT byte figure through, rendering the same "-" as an absent one and recording nothing. The
...
//
// Abandoned sits below failed because a failed run carries a diagnosed cause and an abandoned one carries
// a lost verdict, and above in-flight because it is terminal.
...
// a filtered view is bookmarkable and survives a refresh, WITHOUT adding a history entry.
// It writes the canonical shape (?q=, ?status=ok,failed,
...
// members, because a hand-written copy could silently omit a member (e.g. ?status=abandoned): the guard
// would reject the value and stateFromQuery would discard a link an operator had been given.
