...
export function openRunDetail(engine: EngineClient, run: FleetRun): void {
  ...
  openDetailDrawer({
    ...
    // key on its own; sources-downpipes/detail.ts uses the same key shape for the same reason.
...
function buildRunDetailBody(run: FleetRun): HTMLElement {
  ...
  // The rich per-run detail the wire shape carries (counts and sizes only;
  ...
  // hasThroughputFigure must ask the same question the rows answer: testing `typeof run.bytes === "number"`
  // alone is not enough, because a FAILED run's byte figure can arrive as NaN, which still passes that guard
  // and would suppress the honest "the run failed before writing" line in favour of four dash rows.
  ...
  if (isUnsuccessful(run.status) && !hasThroughputFigure) {
    ...
    const throughput = drawerSection(
      ...
      // corruption source; guarding only the archive total would leave the plaintext-read figure drawing the
      // identical dash for a corrupt value and an absent one, with no row anywhere. runBytes() also gates the
  ...
  if (seal) {
    ...
    // instant is checked before it is converted. When it will not parse the drawer renders the seal verdict
  ...
  // The ABANDONED verdict is a real recoverability signal (the engine
  ...
  if (run.status === "failed" && run.error) {
    ...
    // The doc affordance sits on the one surface where a first-timer meets a failure: the screen where the
...
function buildDrillSection(run: FleetRun, drillResult: HTMLElement): HTMLElement {
  ...
  if (!opGate) {
    ...
    // the serialisation issue it is. A run without an id cannot be restored AT ALL, which makes it the most
    // consequential of these anomalies, so it is recorded as a wire anomaly here.
  ...
  return [
    {
      ...
      onClick: async () => {
        ...
        try {
          ...
          // drill is not evidence of recoverability, so only a pass is recorded. A refused write does
          // not shadow the drill verdict, and it is not silent either: it is stated beside
    ...
    {
      // THE WAY OUT OF A FAILED RUN. Drill and Build a restore plan are
      // both things to do with an archive that EXISTS; the commonest day-two action on a run that failed
      // is to fix the cause and run it again, and Run lives on the downpipe
      // (sources-downpipes/detail-actions.ts). This action lets a customer get there without already
      // knowing where to go.
      label: "Open the downpipe",
      icon: svgIcon(ICON_DOWNPIPES, { size: 14 }),
      // The tour's fix-and-rerun step pins its spotlight and "Your task is here" chip to this button,
      // not to the failure verdict's title text, which is prose that does nothing when clicked.
