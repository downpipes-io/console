...
//
// This array must include every member of RunStatus (the alias above derives from the wire type, so it
// always contains all four); it is hand-written because it also carries each status's label and tone, and it
// is the single vocabulary the table's facets, its status sort and its URL guard all share. Omitting one
// status here makes it unfilterable, hidden whenever any status facet is active, dropped from its ?status=
// link, and sorted into the wrong bucket.
//
...
// reports one. That is "we lost track of it", not "it failed for THIS reason", and the difference is exactly
...
// keeping them distinct in storage. The console follows the same rule: counted together where the question is
