...
export function sealVerifyDetail(seal: NonNullable<RunHistoryEntry["sealVerification"]>): string {
  ...
  // misconfigured one; every remaining cause is a setting or a size they can act on, except break-glass,
  // which can only appear on a run sealed before the seal path gained its own per-run key, so it is worded
  // as history rather than as a current posture.
...
//
// The coercion above reads correctly, but it must not read silently. A recordsIncomplete that arrives non-numeric or
// negative would otherwise read as "fully captured", which is the most reassuring answer this function can give,
// precisely when the wire is wrong. The customer would then see a clean run beside a restore that comes up short.
...
//
// A bare `typeof r.archiveBytesWritten === "number"` guard lets a CORRUPT figure walk straight
// through: typeof NaN and typeof Infinity are both "number". A run whose byte figure arrives non-finite would pass
// the guard, reach humanBytes, and render "-" -- the identical glyph to a run that legitimately reports no
// figure at all. Two states, one dash, and no row anywhere. The summary band has the same exposure: it would
// be short by the same amount and say nothing, because sumDefined skips a non-finite entry in silence.
//
// This reuses the `bytes` field anomaly class also used on the topology surface. The VALUE never rides: the class and the anomaly do.
//
...
// a clean complete ok, whichever of the three drove it. Presentation only.
