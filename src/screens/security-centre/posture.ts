...
// determination. It imports the shared leaf (./shared.ts) only, so it never imports another section
// module, which would form a cycle.
//
// House rules: no-custody (a check carries names, statuses, a control reference, an observed detail, a
// method line and a remediation only); disabled-with-reason, never hidden-then-403.
...
// (needs-attention eager, the rest collapsed).
function buildChecksSection(engine: EngineClient, report: PostureReport, reload: () => void): HTMLElement {
  ...
  if (report.checks.length === 0) {
    // A posture report can arrive well-formed and carrying no checks at all. This is not a fresh account
    // with nothing to check (a fresh account still evaluates every check and passes or warns on it); it is
    // an engine that computed a report and put nothing in it, and the screen's own copy says so. Recording
    // the skew here is the only trace in a support pack that the console was ever handed an empty report,
    // since the screen's own honest copy leaves no other record of it.
  ...
  // line each with their counts, because a long all-green scroll buries the group that matters; their
  // bodies still render, so find-in-page and one click reach every check.
...
function checkCard(engine: EngineClient, check: PostureCheck, writeGate: boolean, reload: () => void): HTMLElement {
  ...
  // offline-key-only in-platform proof path), needed because a break-glass-only posture leaves the
  // engine unable to run a scheduled test itself.
  //
  ...
  // fix is one click away must not be left as a dead end.
  //
...
async function withdrawOverride(engine: EngineClient, check: PostureCheck, reload: () => void): Promise<void> {
  ...
  try {
    ...
    // stepUpAwareText, not errText: /posture/unaccept is step-up gated, and a cancelled ceremony would
    // otherwise throw "unaccept posture risk: stepup-required: 401" straight into this toast.
...
// handleOverrideSubmit is the override modal's primary-action handler, kept separate from
// openOverrideForm so the form body construction and the openModal call stay readable. It
