...
// save-confirm is ticked. This module holds these controls separately from the security-centre coordinator to
// keep file size manageable; it imports the shared leaf (./shared.ts) only, so it never imports another
// section module (which would form a cycle).
//
...
function reflectApprovalPolicy(view: ApprovalView, on: boolean, opts: { busy?: boolean; available?: boolean } = {}): void {
  ...
  // below removes and the early return never re-appends, which would leave a dangling aria-describedby of
  // the same shape signin-context.ts also guards against. An absent feature is
...
function loadApprovalPolicy(engine: EngineClient, view: ApprovalView): void {
  void engine
    ...
    .catch((err) => {
      ...
      // The control is BUILT ONCE at screen render, so a load fault here needs its own retry path rather
      // than a dead switch with no remedy short of reloading the tab. All three switches on this screen
      // read the same policy, so one engine fault would otherwise dead-end three controls at once.
  ...
  const load = (): void => {
    ...
    void engine
      ...
      .catch((err) => {
        ...
        // A failed read must not take the WHOLE panel with it: behind this one line sits the operator's
        // remaining recovery-code count, the Regenerate control and the Owner-only break-glass retire.
        // `load` is right here in scope and re-runs the read, so the retry below gives a way back rather
        // than a dead end.
...
export function recoveryCountTone(remaining: number): StatusTone {
  ...
  // false for NaN, so an unreadable count would otherwise earn the SAME ok green a healthy eight does. The caller
...
//
// THE FIELD IS TYPED `number | undefined` AND THE WIRE DOES NOT PROMISE EITHER. Without this guard: a count
// of NaN would render "NaN unused codes remaining" beside an OK GREEN dot, the same hue a healthy eight
// earns, because every `<=` comparison is false for NaN and the function falls through to its healthy arm; a
// count of null would render a DANGER dot beside the words "You are running low", two different verdicts
// inside one card, because `null <= 0` is true (null coerces to 0) while `null === 0` is false. A string "8"
// reads as ok by coercion, which happens to be right, and is right by accident.
//
...
async function regenerateRecoveryCodesFlow(engine: EngineClient, reload: () => void): Promise<void> {
  ...
  // not the catch block above) but the answer did not carry a set of codes to show -- the tour's
  // demo world can fall through its unmodelled /admin/auth/* fallback to a generic {ok:true} shape here.
  // recoveryCodesPanel guards against a missing array too (hasUsableRecoveryCodes backs it), but failing
  // HERE, before a non-dismissable modal opens, gives the
...
function retireBreakGlassControl(engine: EngineClient, status: StatusReport, isOwner: boolean, reload: () => void): HTMLElement {
  ...
  // (not one call with a computed data-dp), so the static hook census reads both `#1`/`#2`
  // hooks.
  if (isOwner) {
...
async function retireBreakGlassFlow(engine: EngineClient, reload: () => void): Promise<void> {
  ...
  try {
    ...
    // 202 and the latch is NOT set: the static token still signs in until a second owner approves. Without
    // this branch the screen would claim the token could no longer sign in while it still could, which is
    // the one failure mode this control must never have: an operator told a way in is closed must never
    // stop treating the live bootstrap token as live.
    if (isOwnerActionQueuedResult(res)) {
    ...
    // group is the internal marker constant, so without the check below the toast would read "Could not
    // retire the break-glass token: stepup-required" -- a protocol token dressed as the engine's
    // explanation. The engine refused nothing there; the operator dismissed their own passkey prompt and
