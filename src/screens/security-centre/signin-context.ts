// The OWNER-OPT-IN "Notify on a sign-in from a new location" control of the Security centre. It mirrors
// the change-number toggle exactly: a clearly-labelled switch bound to the signin-context policy, ENABLED
// only for an Owner (the engine's POST is owner-only; everyone else sees the state read-only), loaded
// from the same approval-policy view (which carries notifyNewSignInContext), and degrading honestly on an
// engine that does not back it. When ON, a successful sign-in whose COARSE network context (an IPv4 /24
// or IPv6 /48 prefix) is not in that operator's recent baseline sends a warning notification through the
// configured channels. The check runs entirely in the customer's own account: the raw IP is never stored,
// only the coarse prefix in a small rolling per-operator set.
//
// House rules: no-custody (no value or key transits); precise claims.
...
export function signInContextControl(engine: EngineClient): HTMLElement {
  ...
  // Group-level doc link for this owner-opt-in switch: the alert-events section
...
function reflect(view: SignInContextView, on: boolean, opts: { busy?: boolean; available?: boolean } = {}): void {
  ...
  // UNAVAILABLE clears the standing refusal instead of stating it: setting aria-disabled +
  // aria-describedby pointing at "signin-context-disabled-reason" and then having
  // stateLine.replaceChildren() below remove that node would leave the switch pointing at an id
  // getElementById cannot resolve, since the early return skips the !isOwner branch that re-appends it.
  // A screen reader would then read a disabled switch with NO reason, on a control the native `disabled`
  // has already dropped out of the tab order. An absent feature is not a refusal about who this caller is
  // (this file's own split, above), so null is the accurate state.
...
function loadPolicy(engine: EngineClient, view: SignInContextView): void {
  void engine
    ...
    .then((p) => {
      // "Why did the sign-in notification option vanish?" The approval-policy payload can come back without
      // the field, and the console honestly hides the control. That is right, and it is also indistinguishable
      // from an engine that never had the feature. This row says the console SAW a policy payload with the
      // field missing, which the engine version alone cannot say.
    ...
    .catch((err) => {
      ...
      // Mirrors security-centre/access.ts loadApprovalPolicy: this branch disables the switch for the
      // life of the screen and shows POLICY_READ_FAILED with a retry control, rather than a
      // parenthesised engine verb with nothing to press.
