// Coverage / gap detection for the Security centre: which resources are
...
// unprotected rows). This module holds coverage separately from the security-centre coordinator to keep file
// size manageable; it imports the shared leaf (./shared.ts) only, so it never imports another section module
...
// It is the denominator parseInventoryGroup has never had.
//
...
// place. The cap keeps the calm default BOUNDED: without it a large inventory would render one card per
// resource with no ceiling, growing the page without bound. Matches the slice(0, LIMIT) idiom used
...
function coverageCardActions(finding: CoverageFinding): HTMLElement {
  ...
  if (finding.action === "back-up") {
    ...
    // posture, where a plain "run a restore test" hint would leave nothing to act on.
