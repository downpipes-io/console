...
// sections share, so they live in this leaf and no section module imports another section module, which
// would form a cycle.
//
...
export function scoreStatus(score: number): { tone: StatusTone; label: string } {
  // A SCORE THAT IS NOT A READABLE SCORE MAY NOT REACH A BAND AT ALL. This is the same failure mode
  // recoveryCountTone guards against in a different number, guarded here for the same reason: every `>=`
  // below is FALSE for NaN and TRUE BY COERCION for null and for a numeric string, so an unguarded band
  // would answer confidently about a value it never read.
  //
  // On the real badge: a null score would coerce to 0 in both the comparison and Math.round and print
  // "0 / 100 (needs work)", BYTE-IDENTICAL to a genuine zero; a score arriving as the string "88" would
  // print "88 / 100 (fair)", byte-identical to a real 88, right by accident and wrong in principle; NaN
  // and an absent score would both print "NaN / 100 (needs work)"; and a score of 150 would wear the OK
  // GREEN a strong posture wears. This is the Security Centre's headline figure and it rides into the
  // signed posture report, so a number nobody can read must not be given a band.
  //
...
// beside the checks toolbar, keeping the read-only reason visible without a separate gate card. The
// disabled accept/withdraw buttons keep their title reasons.
...
//
// This avoids interpolating the engine client's own "<verb>: <status>" throw (for example "get approval
// policy: 500"): that would hand the operator an internal verb name rather than a remedy, and leave the
// switch under it disabled for the life of the screen with nothing to press. What the operator needs to know
...
// errText lives in lib/errors.ts; re-exported here so the security-centre screens keep a local import.
