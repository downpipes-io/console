...
function renderVerifyBanner(verify: ConfigChainVerdict): HTMLElement {
  ...
  // The DELETED TAIL check comes first: the recompute only checks the versions still held, so removing the newest ones
...
async function loadVersionDetail(engine: EngineClient, v: ConfigVersionHeader, parent: ConfigVersionHeader | null, body: HTMLElement): Promise<void> {
  try {
    ...
    // A generic message like "one of the versions is no longer retained" would name neither version and
    // send the operator (and support) looking in both. The version HEADERS are listed, so both
...
// unloadableDiffReason is the PURE sentence for a diff that could not be built. A generic message like
// "one of the versions is no longer retained" would name neither version, so an operator asking "what
// changed in version 30?" could not tell support which snapshot body the engine had lost, and a diff that
// failed for some OTHER reason would be reported as a retention gap that may not exist. Each branch here
// is a precise, honest claim.
...
function renderSnapshotSummary(version: ConfigVersion): HTMLElement {
  ...
  const counts: Array<{ label: string; n: number }> = [
    ...
    // role grants get their OWN field family: a snapshot that reports "0 role grants" when there were several
    // is the sharpest case of this going wrong, since an auditor reads a zero as evidence nobody held a role,
    // and it is the count that most needs to be right.
...
//
// The degrade is right, and its SILENCE would be the bug if left unrecorded. A field that arrived as
// something other than an array is counted as 0 and printed as a fact ("0 role grants"), so a signed,
// hash-chained snapshot would present a count that is not a measurement at all. ABSENT is not recorded
// (an older snapshot legitimately lacks a family, and a row there would fire on every historical version);
// PRESENT-AND-NOT-AN-ARRAY is engine drift and is recorded.
// EXPORTED for the validator, which drives THIS function over the payload a skewed engine really sends,
// rather than hand-posting a contract-skew row into the recorder and asserting it comes back.
