...
// hidden, so the screen owns its single h1, mirroring the signed-out screen.
//
...
// coordinator that wires the engine and shared status host into those pieces, keeping this file short.
...
// PASSKEY_UNSUPPORTED_MESSAGE is the exact banner copy shown when this browser has no WebAuthn API. It is
// exported as a named constant, rather than left as an inline literal, so the browser-requirements doc-vs-
// code pin (test/validate-security-headers.ts) reads the same string this screen renders: one source, so
// the two sides cannot drift apart the way SECURITY.md and buildCsp once did over connect-src.
...
// authenticated role here yet), so actions is [].
  ...
  render(match: RouteMatch) {
    ...
    if (!supported) {
      card.appendChild(
        banner({
          tone: "warn",
          message: PASSKEY_UNSUPPORTED_MESSAGE,
        }),
      );
      inner.appendChild(card);
      page.appendChild(inner);
