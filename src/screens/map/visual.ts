...
// abstraction the controller's lifecycle drives. buildVisual feature-detects internally and
// falls back to the SVG topology when WebGL is unavailable, and honours prefers-reduced-motion
// by painting a static frame; the canonical accessible table stays present in every branch. It
// owns no view state and no `this`: the controller passes the interaction handlers (open a flow,
// filter by a node, clear the node filter) as parameters.
