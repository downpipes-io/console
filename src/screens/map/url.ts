...
// and the in-place filter application over the cached flow set. These own no view lifecycle and
// no `this`: they take the present filter state (the FilterState the filter bar already reads)
// plus the flows as parameters and return a value (a filtered flow set, a query string, or
// perform the best-effort history write).
