...
// These are free functions over a small LoadHost interface the controller implements: the
// controller still owns the lifecycle state (in-flight guard, first-load latch, the failure count
// and the "as of" timestamp) and the render / poll hooks; the orchestration sequence lives here so
// the controller stays a thin coordinator.
