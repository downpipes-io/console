...
// the screen's honest error state); the others degrade to unknown / relaxed freshness. The
// read orchestration belongs with the data leaf.
...
export function mapDownpipesToFlows(data: MapData, nowMs: number = Date.now()): FlowRecord[] {
  ...
  // A FAILED heartbeat read collapses replByDp to {}, which is byte-identical to "this downpipe has no
  ...
  return downpipes.flatMap((state) => {
    ...
    // The anchor rides off the downpipe's OWN engine state (GET /admin/downpipes forwards sealedRuns +
    ...
    // where the anchor was never recorded, and an absent anchor keeps the milder lane. It is also WITHHELD when
...
interface FlowInputs {
  ...
  // The replication ANCHOR for this downpipe (its monotone successful-run count + the per-destination
...
// flag (plus its down REASON) are per-destination for a fan-out downpipe (a single-destination
...
function buildFlowRecord(f: FlowInputs, endpoint: FlowEndpoint, edgeId: string, destId?: string): FlowRecord {
  ...
  // The SAME per-destination reason the drawer's Copies row names (replication.ts summariseReplication
...
export function classifyFreshness(input: FreshnessInput): CoreFreshness {
  ...
  // in-flight run over an older good run is judged on that good run's age, keeping the dashboard
  // and the map in agreement: an in-flight run does not reset staleness.
  if (lastGoodAt !== null) {
    // THE STAMP IS TESTED FIRST, AND NOT INSIDE THE CADENCE BRANCH. The last GOOD run's instant is what every
    // green claim on this downpipe rests on. If it will not parse, this build cannot date the backup, and it
    // must not then call it fresh. Testing it only inside `cadenceSeconds > 0` would let a corrupt stamp
    // arriving with an unusable cadence (a string, null, 0, or a cadence this build could not read) skip the
    // test entirely, fall through to "healthy", and render green on the map and "Ok" in the table with an EMPTY
    // PACK: byte-identical to a genuinely fresh downpipe. The cadence decides only whether the stamp is OLD; it
    // has no bearing on whether the stamp is READABLE.
    ...
    // read failed while history kept serving the same old run.
  ...
  // A run status this console build does not know renders "unknown" on the map, which is the same thing an
  // unreadable history renders, so a fleet that is running perfectly on a newer engine can read as a fleet
  // nobody can see. The coarsening is right (no green claim we cannot back); silent misclassification is not.
  // The VALUE never rides: a status is engine-supplied and an unknown one is precisely the value not to trust.
  // "abandoned" is a KNOWN terminal status the engine mints (an in-flight run the SRE path retires); it is
  // included here rather than falling into the unknown-enum branch below, so a legitimate abandoned run does
  // not get coalesced with genuine future-skew and stamp a working engine as a skewed one.
...
// cadenceSecondsOf is the ONE seam that reads a downpipe config's cadence for the freshness rule, and the
// only place the console is allowed to coarsen it. Every surface that judges staleness (the map, the downpipes
// table, the Overview fleet roll-up) calls it: duplicating this test invites silent coarsening to drift
// between copies, and a null cadence DISABLES THE ENTIRE STALENESS TEST. A downpipe whose last good run was
// two months ago would then render healthy on the map and "Ok" in the table with an empty pack, because a
// version-skewed or corrupt engine sent a cadence in a shape this build cannot read.
//
...
function sourceKind(type: SourceSpec["type"]): NodeKind {
  // This switch is exhaustive over the union this console was COMPILED against, which means a source type
...
// bucket name, so the node can name the actual archive rather than only its kind, "in-account
// R2 archive"). The label is HONEST about what is known:
//   - destination read ok with a bucket: the REAL bucket name (the kind tag carries R2/S3);
//   - destKind "r2": the in-account R2 archive (no-custody, and unable to enforce Object Lock);
