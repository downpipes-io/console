...
// local facts only and shows them to the operator.
...
export function freshnessLine(flow: FlowRecord, ring: RunHistoryEntry[] | null): HTMLElement {
  ...
  const detail =
    ...
    // keeps the original sentence. The same partial/no-copy distinction extends to "failed".
    flow.status === "failed"
      ? (flow.laneFailed
    ...
    // This sentence says only what the console can PROVE. The lane only fires once at least two backups have
...
export function recentRunsList(entries: RunHistoryEntry[]): HTMLElement {
  ...
  for (const e of entries.slice(0, RECENT_RUNS_DISPLAY_LIMIT)) {
    // runStatusTone is the canonical mapping (components/status.ts): deriving status locally as
    // ok / failed / else would send an ABANDONED run down the else arm and print the visible word
    // "in-flight" for a run the engine has already given up on, which would be wrong in the text,
    // not only in the hue.
    ...
    // isUnsuccessful, not `=== "failed"`: using strict equality would withhold the engine's reason string
    // for an abandoned run ("abandoned (run lease expired)"), so the one line explaining what happened
    // would never reach this drawer. runs/table.ts uses the same check for the same reason.
