...
// setFlows (a 2px indeterminate bar marks the refetch) rather than blanking the canvas. It
// imports the pure data leaf (./data.ts), the presentation helpers (./panels.ts,
// ./view-chrome.ts, ./filter-bar.ts, ./drawer.ts), the URL helpers (./url.ts), common.ts (one
// level up) and the design-system components.
...
// screen. Nothing phones home; the freshness is recomputed in-account from the engine's own data. The poll pauses
...
// the canvas (never blank on a poll). The lifecycle is bound to DOM
...
class MapView implements DrawerHost, LoadHost {
  ...
  // BEFORE the element is attached. Without the latch, reading that as "navigated away" would
  // silently dispose the view and leave the loading skeleton on screen forever, which shows up
  // whenever every fetch returns in microseconds (a local fixture engine, for example).
  ...
  renderLoading(): void {
    // The two-column skeleton (never a blank canvas) is the stateless leaf
  ...
  // copies the block to the clipboard. One paste of this block names the culprit (no WebGL,
  // an emulated/forced reduced-motion state, a frozen rAF, an extension freezing animations)
  // without another remote round trip. Local reads only; the operator chooses where the text goes.
  ...
  private async recordRenderer(): Promise<void> {
    ...
    // does mount in background tabs, and without this watch every one of those sessions would file a false
    // renderer fault. A tab hidden HALFWAY THROUGH gives the same false reading, which is why the watch spans
    // the window.
...
export function renderMap(ctx: { query: URLSearchParams }): HTMLElement {
  ...
  // The screen-level doc link: this explains what the map renders, how freshness and coverage
  // stay honest, and what the drawer's two gated clean-up actions do.
  root.appendChild(
    h(
      "a",
