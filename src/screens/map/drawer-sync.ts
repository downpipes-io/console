// The deep-linked read drawer reconciliation for the topology map. These are free functions over a small
// DrawerHost interface the controller implements: the controller still owns the mutable selection state
// (openId, the open handle, the suppress-navigation guard) and exposes it here, so the drawer open / sync
// logic is one cohesive unit without the controller carrying it inline.
...
export function openMapDrawer(host: DrawerHost, id: string, flows: FlowRecord[]): void {
  ...
  // matching the deep-linkable-drawer contract (detail-drawer.ts).
