...
// loop, an accessibility setting the operator themselves chose), and the answer is browser advice. The only
// other evidence is the Copy-view-diagnostics block, which reaches support ONLY if the customer manually
...
// session, so it is left out; the causes below are the ones that actually degrade what the operator sees.
...
//
// Sampling only a "running" animation matters because getAnimations() also returns animations that legitimately
// cannot advance: a FINISHED animation with a fill mode stays in getAnimations() with its currentTime pinned at
// the end -- the console ships one (tokens.css, the restore-step fill) -- as do paused and idle-timeline
// animations. Any of those would read as css-anim-frozen on a perfectly healthy page if sampled directly by
// position. Only an animation whose playState is "running" is capable of advancing, so only a running one can
// be evidence that nothing is advancing.
...
// watchVisibility is the guard that stops an ORDINARY BACKGROUNDED TAB from manufacturing a false frozen
// reading. A hidden document SUSPENDS requestAnimationFrame while setTimeout keeps ticking, so probeRaf's
...
export function classifyRenderer(f: RendererFacts): ClientDiagDegradeCause {
  ...
  // unobserved sits ABOVE the two motion readings and below the two facts that do not need an observer. A
