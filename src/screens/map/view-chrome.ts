...
// view controller stays the lifecycle owner while these stay testable leaves.
//
...
// a paste-able block; nothing is transmitted.
...
// renderLoadingSkeleton builds a skeleton of the two columns, never a blank canvas. Built from
// the same tokens as the map; the async-region bar handles the poll refetch. Two stacked
// skeleton groups read as the source column and the destination column; the table skeleton sits
// below.
...
// unknownNote: when SOME flows could not be resolved to a real status (history unavailable, or
// the whole status read failed), say so plainly so the "unknown" edges are explained rather than
// read as a fault. Returns null when there is nothing to say (no unknown flows). historyOk
// distinguishes "history could not be read" from "no readable run yet".
...
// screenshot. Without it, a silently degraded state - static frame via a motion preference, the
// SVG fallback after a GL failure - has nothing on the screen admitting to it. onCopyDiagnostics
// wires the escape hatch from guessing: one click captures the LOCAL environment facts as a
// paste-able block (read in this browser, copied to the operator's own clipboard; nothing is
// transmitted, no-custody).
export function viewStatusLine(visual: VisualStatus, onCopyDiagnostics: () => void): HTMLElement {
  ...
  if (mode === "canvas2d") {
    ...
    // usually brings it back, since a browser with graphics acceleration off would otherwise show
    // a still diagram with nothing admitting why.
    viewText =
...
// affordance so data never seems to have vanished. onClearAll clears every filter.
...
// paste-able block. One paste of this block names the culprit (no WebGL, an emulated/forced
// reduced-motion state, a frozen rAF, an extension freezing animations) without a remote-support
// round trip of guessing about a frozen map. Local reads only; the caller chooses where the text
// goes (clipboard or a prompt fallback).
