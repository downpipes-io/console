...
// consequences before the save, and POSTs it.
//
...
export interface SubmitContext {
  ...
  // value captured once here could go stale and stop following what the operator actually chose.
  r2Block: R2Block;
...
// carry a rate the operator did not choose", stated independently of the field rule enforced at submit, so the
// suite can hold the two against each other. If they ever disagree -- a value the field waves through that this
// reader still swaps -- the silent substitution is possible again, and a test that drove only one of them would
// not see it. A submit gate looser than the field creates exactly that risk.
//
// It is EMPTY on every save that lands: that is a property of the product, not a hope. The four rate controls
// carry nonNegativeRate, submitDestination runs it before anything is built or sent, and the field rule refuses
// a strict superset of what this reader swaps (it also refuses "1e3" and "0x10", which Number() would take).
// Nothing downstream may treat a non-empty list as an ordinary state.
export function readPricing(ctx: SubmitContext, provider: Provider): { pricing: DestinationPricing; coerced: ClientDiagFormField[] } {
  ...
  // A rate that does not parse (a typed currency symbol, a thousands separator, a comma decimal) must never be
  // silently replaced with the vendor PRESET while the save still asserts `source: "operator"`: that would tell
  // the engine, and every cost estimate downstream, that the preset is a rate the customer chose.
  //
  // The remedy lives at the call site, not here: submitDestination runs the console's own nonNegativeRate rule
  // on these four controls before anything is built or sent, so a malformed rate cannot reach a save. What
  // survives here is only the fallback for a box the operator deliberately LEFT EMPTY, which is the ordinary
  // "use the vendor's published rate" choice. The typed value is customer commercial data and is never
  // recorded; the catalogue field id is.
  //
  // THE FALLBACK CANNOT TELL A MALFORMED RATE FROM AN EMPTY ONE, AND THE REASON IS THE HTML SPEC. The rate
  // controls are <input type="number">, and the value-sanitisation algorithm makes .value return the EMPTY
  // STRING for any content that is not a valid floating-point number. So "$0.015", "0,015" and "1,234.5" all
  // reach this reader as "" and take the empty-box path, one line below, which returns the preset in silence
  // because an empty box IS the ordinary "use the preset" choice. Number(t) on a NON-empty value from a number
  // input is always a finite float, so the !Number.isFinite branch cannot fire either; the only reachable
  // coercion here is a negative rate.
  //
...
export function buildDestinationInput(
  ...
  return provider === "r2"
    ? {
        ...
        // discovered engine account. It is asked LIVE, at submit, so a mid-form change is honoured; reading
        // `fctx.engineAccountId ?? accountIdField.value()` instead would always answer the engine account
        // once discovery had named it, regardless of what the operator typed.
    ...
    : {
        ...
        // to a destination with no Entra fields set. Clearing both boxes clears a stored principal, which is
...
export async function submitDestination(ctx: SubmitContext): Promise<void> {
  ...
  // RUN THE CONSOLE'S OWN RULE ON THE RATES AT SUBMIT, NOT ONLY ON BLUR. The four rate controls carry
  // nonNegativeRate, but validating only on blur is not enough, because a blur is not a save: an operator can
  // reach a save without ever blurring a rate field. Running the rule here as well means a rate that fails to
  // parse is refused rather than silently saved under `source: "operator"`.
  //
  ...
  // is never sent.
  //
  // NO PROVIDER IS EXCLUDED HERE. A GCS bucket created with per-object retention answers the engine's
  // Object-Lock probe Enabled and honours a compliance lock, and an Azure container with version-level
  // immutability enforces a policy where governance maps to unlocked and compliance to locked, so Google
  // Cloud Storage and Azure Blob Storage are offered on the form and submitted like any other provider.
  //
  // The policy is sent for R2 as well, and it is the ENGINE that refuses it: measured, R2 over its S3
  ...
  // not do is second-guess the probe, which is the only authority on what a bucket actually enforces.
  ...
  if (wormMode === "governance" || wormMode === "compliance") {
    ...
    if (!Number.isInteger(days) || days <= 0) {
      // "I am blocked configuring WORM" is a real, recurring complaint, and the block happens HERE, in the
      ...
      //
      // The empty box also takes this branch: Number("") is 0, which fails `days <= 0`, so an operator who
      // picks a mode and presses Save before typing a number would otherwise be recorded as a refusal over a
      // value that never existed, indistinguishable from the real refusal this row is for ("the day-count rule
      // is tighter than Object-Lock's"). The raw value goes to the recorder, which writes nothing when it is
  ...
  if (provider === "s3") {
    ...
    if (roleArn !== "") {
      ...
      if (dur !== undefined && (!Number.isInteger(dur) || dur < 900 || dur > 43200)) {
        recordFormRefused("dest-sts-duration", durRaw); // Refused in the browser, invisible to the engine.
      ...
      if (!/^[a-z]{2}(-[a-z]+)+-\d{1,2}$/.test(stsRegion)) {
        ...
        // support-pack ingest recognises, and adding it there needs an engine-side update. The inline refusal
        // below covers it locally; the pack telemetry for this specific refusal waits on that vocabulary
        // update.)
  ...
  //
  // The message avoids naming Amazon's "account root" (its word for the most privileged principal), because
  // this form offers immutability for Google Cloud Storage and Azure Blob Storage too, where the equivalent
  // principal is a project owner, an organisation administrator or a subscription owner and no "account
  // root" exists to look for. The sentence names the property rather than one vendor's name for it: a
  // compliance lock is above every principal the store has, whichever provider it is.
  ...
  try {
    ...
    // NOTHING IS RECORDED HERE, BECAUSE THERE IS NOTHING LEFT TO RECORD. `coerced` is empty on every save that
    // reaches this line: the rate rule ran at the top of this function and refused the save outright, so a rate
    // the number input could not parse never reaches a stored value at all, let alone one silently swapped for
    // the vendor preset.
    //
    // The refusal itself is recorded by the field() funnel, as `rejected` on the same catalogue control id, and
    // that only works because recordFormRefused reads validity.badInput: on a control the browser emptied, the
    // raw value is "" and an emptiness rule alone would drop the one refusal it needs to keep.
    //
...
//
// err.message is the transport's throw, not a sentence written for an operator: an engine refusal can carry
// the wire verb on the front and the status glued past the full stop (for example "add destination: the
// store rejected the credentials (SignatureDoesNotMatch)...: 400"), a refusal the engine did not explain
// names nothing to change ("add destination: 400"), and a 403 the transport classifies as not-an-engine-
// refusal carries only the raw internal token ("forbidden-class=not-engine-body: 403").
//
...
// did not, including the cancelled step-up case, which is not a reason-bearing kind.
