...
export function buildAttachPanel(idsField: HTMLElement, ownerGate: boolean): AttachPanel {
  ...
  // instead. The binding flow inside is merely parented here so it can be hidden as a unit.
  ...
  // The no-token fallback sits inside a disclosure: no customer is expected to run a terminal,
