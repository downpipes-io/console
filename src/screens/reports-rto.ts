// The recovery-time (RTO) sub-section of the SLA report card, kept in its own module so
// the screen module stays under its size bound.
//
  ...
  const load = (): void => {
    ...
    void engine
      ...
      .catch((err) => {
        ...
        // failed RTO read must not break the card. The message names no internal status code (an operator
        // cannot act on one) and offers a retry rather than a dead end.
