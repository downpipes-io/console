...
// state (the live provider, setProvider, the pricing-edited flag).
//
...
// form accepts is not thrown away at submit. A prefix check alone (startsWith("https://")) is looser than
...
//   - "https://[" and "https://%zz" pass that pattern and then throw inside new URL() at
...
export interface FormContext {
  ...
  // list that could not be read are different facts: rendering them identically would show the select
  // replaced by a free-text account id with no indication why the bucket dropdown is gone. The form states
  // which one it is instead (buildR2Block below).
...
// naming it "the provider's" pricing would be a false statement about the operator's own destination
// whenever the destination is not Amazon S3, and the rate term is most of what the cost estimate
// computes. The costs screen labels the identical preset "Amazon S3 Standard" (screens/costs/pricing-
// section.ts), so the label here matches it: one honest label for the same fifteen numbers.
//
// Naming more vendors here would not fix it. No rate in PRESETS carries a source, a checked-on date,
// or a gate, and the non-Cloudflare storage market moves independently of this codebase, so a longer
// preset list is more unwitnessed numbers going stale, not more accuracy. The honest move is to say
// whose price this is and to tell the operator to replace it.
...
export function pricingNote(p: Provider): string {
  ...
  // source or a checked-on date, and the non-Cloudflare storage market moves independently of this
  // codebase, so adding an unwitnessed GCS preset would be a number that reads authoritative and
...
// the block has two live states (see buildR2Block): which bucket control is mounted, whether the
// Account ID is answerable, and which account the endpoint derives from all change while the form is open.
// Returning the handles by value would be wrong by construction: a caller that captured `accountIdField`
// once, at build time, would have no way for it to become non-null later.
...
//
// THE DEFAULT KEEPS THE SIMPLIFICATION, and deliberately so: when discovery names the engine account there
// is no Account ID to type, the bucket is chosen from that account's real buckets, and the block states the
...
//
// THE OVERRIDE EXISTS BECAUSE THE SIMPLIFICATION ALONE WOULD BE A CEILING. Deriving the endpoint as
// `r2Endpoint(fctx.engineAccountId ?? accountId)` would make the engine's own account win whenever
// discovery names it, with no field for another account's id: an R2 API token minted in a different
// account would have nowhere to go, and a destination could only ever be written into the engine's own
// account. A second copy in the SAME Cloudflare account is not a second blast radius, and the 3-2-1
// posture the product sells needs the second account reachable from the console.
//
...
export function buildR2Block(fctx: FormContext, updateCircWarn: () => void): R2Block {
  ...
  // replaceChildren would tear the field down and re-attach it after every character.
  //
  // Without this guard, ticking the override, clicking Account ID and typing a 32-character id would leave
  // the field holding only the first character typed, with document.activeElement back on BODY and every
  // later keystroke going to the body. A field that cannot be filled is worse than no field at all.
  ...
  const paintState = (): void => {
    ...
    if (!explicitAccount() && fctx.engineAccountId !== null) {
      ...
      // account here instead would be true and would answer a question the operator had not asked, which
      // is exactly what would make a mismatch invisible.
      endpointNote.replaceChildren(
        document.createTextNode("Writes go to "),
...
export const AZURE_CRED_COPY: Readonly<{ key: FieldCopy; secret: FieldCopy }> = {
  ...
  // THE SECRET BOX TAKES ANY OF THE THREE KINDS THE ENGINE ACCEPTS. It tells them apart from the value
  ...
  // be mistaken for one; and a filled Entra block makes the same box the client secret. A label naming only
  // one of the three would leave an operator holding a SAS or a client secret with no way to know it is
  // accepted, which is a capability the product has and the interface would withhold.
  secret: {
 ...
 *
 * It exists because the refusal arrives after the work. On three of the four providers Object Lock is a
 * property a bucket is CREATED with and can never gain afterwards, so an operator who chooses compliance
 * mode, types a retention window and presses Verify and save would learn only from the engine's refusal
 * that their bucket was never a candidate, and that the remedy is a new bucket rather than a setting.
 * Stating Amazon's vocabulary ("the bucket must have been created with Object-Lock enabled") for every
 * provider would be the wrong sentence for three of the four. Azure is the fourth, and it is the one where
 * the prerequisite CAN still be met on an existing container, which is the difference the operator most
 * needs told.
 *
 ...
 *          be created with it. R2 over its S3 endpoint answers 501 NotImplemented to x-amz-object-lock-mode,
 *          which wormCannotBeEnforced reads as a definite cannot-enforce and turns into a refused save
 *          (engine/src/admin/router-destinations.ts). R2's own bucket lock is a different mechanism that
 *          neither this policy nor the probe reads.
 *   s3     S3 Object Lock proper: enabled at bucket creation, never afterwards.
 *   gcs    Google Cloud Storage implements S3 Object Lock through its interop API, but only for a bucket
 *          created with per-object retention. Such a bucket answers the probe Enabled and honours a
 *          compliance lock; one without it answers 404 and the save is refused. Per-object retention
 *          cannot be turned on after the bucket exists.
...
export function buildPricingBlock(opts: DestinationFormOpts, pricingState: { edited: boolean }): { section: HTMLDetailsElement; note: HTMLElement; storageField: FieldHandle; classAField: FieldHandle; classBField: FieldHandle; egressField: FieldHandle; currencyField: FieldHandle } {
  const startPricing: Pricing = opts.initialPricing
    ...
    : { ...PRESETS.r2 };
  // Each rate validates at the field, AND THAT RULE RUNS AT SUBMIT TOO. Validating only on blur is not
  // enough: submitDestination's validateForm must include these four fields alongside the credential and
  // endpoint fields, or readPricing would swap an unreadable rate for the vendor preset in silence, and an
  // operator who typed a contracted rate with a currency symbol in it would be told the destination was
  // verified and saved while reading cost estimates of a rate they had not entered. The four cost-screen
  // rate boxes refuse this at the field the same way; these four are the same value on a different screen.
...
export function buildWormBlock(opts: DestinationFormOpts): { section: HTMLElement; wormModeField: FieldHandle; wormDaysField: FieldHandle; wormPrereq: HTMLElement } {
  const wormModeField = field({
    ...
    // The bucket prerequisite lives in the provider line above (WORM_PREREQ_COPY) rather than here, so
    // this hint carries only what holds everywhere.
 ...
 *
 * WHY THE BLOCK EXISTS AT ALL. The engine accepts three Azure credential kinds. Without this block the
 * console could express only one of them, and a capability reachable over the API and absent from the
 * only interface a customer has is not a supported capability.
 */
export function buildEntraBlock(opts: DestinationFormOpts): { section: HTMLElement; tenantIdField: FieldHandle; clientIdField: FieldHandle } {
  ...
  const clientIdField = field({
    ...
    hint: "The application the service principal is, from the same overview page: a GUID. Its client secret goes in the credential box above, which takes an account key, a SAS token or a client secret and is labelled for all three.",
    doc: { href: "https://docs.downpipes.io/backing-up/destination-providers", anchor: "the-four-providers-axis-by-axis" },
    validate: (v) => {
      const t = v.trim();
      if (t === "") return null;
