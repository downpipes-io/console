...
// manual-by-default framing, stated once in the Updates section). Built
// against the existing contract (licence, updates, status.engineVersion).
// Calm contract (design-system.md 7a): three eager sections, the tile band, the Licence card (its
...
// commercial-model validator) depend on. The screen body is assembled by ./licence/view.ts; the pure
// DOM-free decision logic, the release-metadata helpers, the summary-band tile
...
// Enterprise bodies in ./licence/provenance.ts. licenceScreen is the public export, and every validated
// symbol is re-exported from here unchanged.
...
// re-exporting them here keeps the public import surface of screens/licence.ts byte-identical.
export {
  artefactHashState,
  changelogTypeLabel,
  compatBlockedReason,
  ...
  UPDATE_MANAGE_CAP,
  // Multi-component updates (P1 + P5): the pure component logic + copy the validator pins.
  UPDATES_HEADING_ID,
  updateControlState,
  updateExplainerText,
  updateRefusalText,
...
export const licenceScreen: Screen = {
  ...
  actions: [
    ...
    // probe to run, so an "action" kind would promise more than happens. The
    // Provenance section carries id="provenance" for a real deep-link.
    {
      id: "check-updates",
