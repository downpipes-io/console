...
function protectedAnswer(data: OverviewData): ExecAnswer {
  ...
  // The list can settle ok and still carry no array: an engine-console contract break, not an
  // unread list. The honest "Unknown" verdict below reads identically to a genuine unread list
  // (data.downpipes.ok === false), which is a different failure mode, so this row records the
  // contract break separately. It fires only on the contract break: an unread list records
  // nothing here.
...
function compliantAnswer(data: OverviewData): ExecAnswer {
  ...
  // The floor is present. The durable, dated audit + drill trail is an engine addition when not yet
  // wired; be honest about that rather than claiming a complete evidence chain, and do not describe a
  // real read fault as an unbuilt feature.
...
// accessAnswer reads the SAME Access verdict the trust chip reads: the AccessVerdict
// accessVerdictFromCaller produces. It never hardcodes a green: the token fallback is an honest
// watch (actions are not attributable to a person), and an unknown verdict reads unknown.
//
// This must derive from the shared verdict rather than re-implement the method read. A private
// three-state read that collapses everything but "access" and "passkey" into "token" would mark a
// live OIDC session, a live SAML session, a recovery-code sign-in and any method a newer engine
// adds as a shared-token session, with a remedy ("put the engine behind Cloudflare Access") that
// fixes nothing and contradicts the trust chip on the same screen. Deriving from the verdict is
// what keeps the card and the chip in agreement.
...
// execAccessVerdict reads the live caller through the trust-chip classifier, rather than through a
// private copy of it. accessVerdictFromCaller is the one place that maps a whoami method to a
// posture claim: it knows all six methods the engine can report, it answers `unknown` for a method
// this console build cannot interpret rather than asserting the break-glass posture, and it records
// the wire-anomaly row from the surface that actually saw the value. A private copy of this logic
// risks disagreeing with the trust chip on any method it does not model correctly.
