...
// fleetProtection generator the downpipes screen subtitles with.
...
export function buildProtectionLead(data: OverviewData): HTMLElement {
  ...
  // configured" off an absent list; it shows the honest unknown instead.
...
function licenceTile(licence: Settled<LicenceStatus>): HTMLElement {
  ...
  // When the read SETTLES ok and the payload carries no tier, a PAYING customer's tile reads "Community /
  // Fail-open" off a defensive default. The defaulting is right (a thrown tile blanks the whole Overview),
  // but an engine-console contract break is a genuine defect signal that would otherwise be byte-for-byte
  // identical to an ordinary community licence, so it is recorded. Recorded only when the read SUCCEEDED
  // and the field is absent: an unread licence is the ok:false branch above and is not a contract break.
...
function recoveryTile(data: OverviewData, fleet: FleetSummary): HTMLElement {
  // The recovery-point tile is an observability read. The durable
  // drill-evidence store is an engine addition; until it ships the console has no
  ...
  // surface, so the two cannot disagree about the same fact: reading fleet.newestGoodAt instead
  // would report the fleet's BEST case under a label an operator reads as their exposure.
