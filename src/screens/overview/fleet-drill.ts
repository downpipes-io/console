...
// One drill failure: the downpipe id plus the REASON it did not pass. Both the in-flow
...
export interface DrillOutcomes {
  ...
  // How many of the failures gave up only after EXHAUSTING the capped 429 retries. A large-fleet drill
  // that dies to a rate-limit storm and one that dies to N genuinely broken pipes look identical in the
  // drill evidence (N pipes did not pass) and identical in the toast, but they call for opposite
  // remedies: one is the engine's limiter pacing a fleet that is too big for one sweep, the other is a
  // restorability problem.
...
export function collectDrillTargets(byDownpipe: Record<string, RunHistoryEntry[]>): DrillTargets {
  ...
  for (const id of Object.keys(byDownpipe)) {
    ...
    // A downpipe whose latest history row carries no runId is DROPPED from the fleet drill, silently. It
    // is not that the drill failed for that pipe: the drill was never attempted, so no evidence is
    // written and the pipe reads exactly like one that has never been drilled. Absence here would
    // otherwise look identical to absence for any other reason, so the skip is counted.
...
// The target list AND what it silently left out. The skip count is part of the result rather than a side
...
// opens and the durable, dated drill-evidence log. It points at "the details below" so the
...
// per-item failures modal the bulk run/disable/delete path uses. Deferred downpipes are
