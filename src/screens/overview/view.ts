...
// wires it all together) in ./mount.ts.
...
// LoadKind distinguishes the two load modes the lifecycle branches on: "initial" is the first load
// (show the skeleton, never settle tiles); "poll" is an in-place refresh (keep the rendered content,
// show the 2px bar, settle changed tiles).
...
export class OverviewView {
  ...
  // settle ONLY the tiles whose value actually changed.
  ...
  // "navigated away" would leave the loading skeleton on screen forever.
  ...
  private async load(kind: LoadKind, forceDiscovery = false): Promise<void> {
    ...
    // THE LAST-RESORT CATCH. buildOverview guards each section, so a section fault fails as a section;
    // this is the floor under a throw that reaches neither. Every other exit from this method clears
    // inFlight and reschedules the poll by hand; an uncaught throw would skip all of them, so the
    // rejection would escape `void this.load(...)`, `inFlight` would stay true for the life of the tab,
    // no further load would ever be scheduled, and the first-load SKELETON would stay on screen for
    // ever. That is worse than a blank page: a skeleton reads as "still loading", so the operator waits
    // instead of acting, and the poll that would have recovered the screen on the next tick is disarmed.
    //
  ...
  // write this surface owns; Operator+). It is gated client-side as a MIRROR
  ...
  private async drillFleetInner(): Promise<void> {
    ...
    try {
      ...
      // A failed fleet read means NOT ONE drill was attempted. From the engine's side this is
      // indistinguishable from a drill that was never started: no drill arrives, no evidence is written, and
    ...
    for (const t of targets) {
      ...
      if ((await this.drillOne(t, done, targets.length, o)) === "signed-out") {
        // THE ABORT. The remaining targets are never attempted, so the engine's drill evidence would otherwise
        // show a partial sweep that is byte-identical to a deliberate partial drill: 8 of 30 pipes drilled on
        // one date, with no way to tell "the operator drilled 8" from "the drill died at pipe 9". `targeted`
        // stays the FULL list (what the loop set out to do) and the abort cause says why the rest is missing.
    ...
    // Recorded on EVERY ending, not only the bad ones, when the session runs to completion: `passed` is the
    // denominator that makes `failed` mean something, and a clean session that produced no row at all would
    // make an aborted one impossible to recognise.
    ...
    // log, written best-effort above, not this toast (the toast is never the only
    ...
    // Every failure, with its REASON, goes in the shared per-item failures modal (the same surface
    ...
    // last-drilled stamp once the evidence just recorded is wired through.
    void this.load("poll");
  }
  ...
  private async drillOne(t: DrillTarget, done: number, total: number, o: DrillOutcomes): Promise<"signed-out" | "done"> {
    ...
    // A 429 from the engine's limiter is honoured (Retry-After, capped) and the drill of this
    ...
    for (let attempt = 0; attempt <= MAX_RATE_LIMIT_RETRIES; attempt++) {
      try {
        ...
        if (res.ok) {
          o.passed++;
          // After a drill SUCCEEDS, record the durable, dated drill evidence, so a privileged action
          // leaves a trail, not just a toast. It is BEST-EFFORT: a missing or not-yet-wired
          // drill-evidence route (404/501) must not shadow the drill result, so the error is swallowed
          // (a 401 is the one exception, routed to signed-out, since the whole session has expired).
          await this.recordDrillEvidenceBestEffort(t.runId);
        } else {
          // Capture the REAL ok:false reason (the in-flow outcome), never a bare "did not pass".
        ...
        // This drill gave up while STILL rate-limited, having spent every retry it is allowed. A
    ...
    // The thrown-error path records a REASON (a 429 reads as "rate limited", every other
  ...
  // not-yet-wired route or any non-auth error so it never shadows the drill result. A 401
  // propagates so the caller routes to signed-out; everything else is silent (the evidence
  // trail is a fast-follow, not a gate).
  private async recordDrillEvidenceBestEffort(runId: string): Promise<void> {
    try {
      ...
      // A not-yet-wired route (404/501) or a transient failure: do not surface; the drill result
