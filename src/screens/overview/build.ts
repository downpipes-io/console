// Overview technical builder: buildOverview assembles the full technical dashboard body from the
// independently-resolved data (the protection lead, the fleet banner, the status tiles, the
// two-column fleet + side region, and a quiet link to the cost projection). Pure given the data +
// callbacks, so a poll re-runs it on fresh data and swaps the result atomically.
...
export function buildOverview(
  ...
  // The fleet roll-up feeds four of the sections below, so it is resolved inside the boundary too, rather
  // than as a bare call before any section exists: a throw there would take the whole page down before a
  // single section could render. An empty summary is not a fabrication, it is what a fleet nobody could
  // summarise contains, and every section that reads it already renders an honest empty state; the section
  // that would have shown the fleet says plainly that it could not be built.
  ...
  //    across the whole page and read as a second posture row, matching the status row at the top,
  //    rather than trapped in the narrow side column.
  ...
  //    .ov-grid class (tokens.css section 13) collapses it to one column under 1023px, so the fleet
  //    table and the attention list stack on medium/compact.
  ...
  // 4. A quiet link to the cost projection. The full, interactive topology lives on /map; the overview
  //    does not embed a second, lower-fidelity copy of it, so there is exactly one map to keep accurate.
  if (fleet.total > 0) {
    wrap.appendChild(
...
// ---------------------------------------------------------------------------
//
// WHY THIS BOUNDARY EXISTS. buildOverview is a sequence of appendChild calls with no boundary of its
// own, called from an async load with no catch. A TypeError anywhere in any one section would take the
// whole function down, the rejection would escape OverviewView.load, and the first-load skeleton would
// stay on screen forever: one malformed roster row beside otherwise-healthy downpipes can take the
// Overview from thousands of characters of body text and nine stat tiles to nothing. That failure mode
// is worse than any single tile lying, because the surface that exists to tell you something is wrong is
// the surface that disappears when something is wrong, and it takes the other nine tiles with it,
...
// say something useful (lib/downpipe-readable.ts is one example). A section that reaches this card has
// lost the chance to explain itself, which is why the card names the section and points at the support
// pack rather than pretending to diagnose.
...
// recordSectionFault banks the fault so the boundary cannot make the support pack QUIETER than an
// unhandled crash would be: an uncaught throw here would otherwise reach the window handler and produce
// an `unhandled` row, so the catch records the same kind and the same derived classes on the
// `window-error` channel, which is what it literally is: a synchronous throw out of a synchronous
// builder. The error's message and stack are never read; only the closed fault class and the platform
// error class travel, exactly as the window handler records them.
...
// moreLink: one quiet pointer row entry (icon + label + a chevron) to a screen that owns the
// detail this screen keeps out of its own body. The chevron, not the external-link glyph: this is
// in-app navigation, not a new tab. The label matches the rail's name for the screen ("Costs"), so
// the pointer and its destination read as one thing.
