...
export interface FleetSummary {
  ...
  // actual worst-case recovery point: a fleet with one downpipe backed up five minutes ago and nine
  // untouched for a month has a recovery point of a month, not five minutes. A recovery point is set
  // by the stalest protected system, never the freshest, so reading the newest good backup instead
  // would be reassuring in exactly the direction that hides risk. Disabled downpipes are excluded
  // because they are not expected to back up; a disabled downpipe is a decision, not a gap.
...
export interface OverviewData {
  ...
  // history), without blanking the screen or routing to signed-out.
...
export function newestDrill(entries: DrillEvidenceEntry[]): DrillEvidenceEntry | null {
  ...
  // A drill-evidence stamp the console cannot parse is skipped by the newest-wins pick above, so
  ...
  // posture claim the customer is relying on, so it is recorded as its own field class: the remedy is
