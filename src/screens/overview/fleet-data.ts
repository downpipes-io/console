...
// history (the SAME seeding the /costs screen uses).
...
export function observedCostSeed(byDownpipe: Record<string, RunHistoryEntry[]>): { inputs: Inputs; runs: number } | null {
  ...
  // a partial settlement can hand us an undefined map, so coalesce before Object.keys.
  ...
  // When successful runs exist and not one of them carries the throughput fields the cost card
  // needs, the card never projects and reads "not projected yet" as if the estate were simply young.
  // This row exists to tell that apart from a genuine estate that has never run anything.
  //
  // The check is scoped to runs with status "ok". The engine stamps archiveBytesWritten and
  // segmentsWritten at completion and nowhere else: an in-flight row is appended at trigger with no
  // byte fields, a failed completion posts none, and an abandoned run never completes. Testing `all`
  // instead would flag three ordinary, legitimate states -- a first backup still running, an estate
  // whose every backup is failing, an abandoned run -- identically to a genuine missing-field
  // contract break.
  //
  // What the row asserts, precisely: every SUCCEEDED run in this ring lacks the throughput fields. It
  // does not assert a contract break on its own, because the browser cannot establish one: an ok run
  // recorded before the engine version that added the fields also, legitimately, lacks them (engine
  // types.ts says so outright). The pack carries engine.version beside this row, which is what tells
  // those two apart; the row's job is to point at the question rather than to answer it.
...
// cohort (the signal must follow bytes, not the run count). Clamped to [0, 1]; all-zero
// history reads as 0 churn (a NaN-safe guard).
...
export function summariseFleet(data: OverviewData, nowMs: number = Date.now()): FleetSummary {
  ...
  // table; the fleet then degrades to an honest empty/unknown reading.
...
function buildFleetRows(data: OverviewData, history: Record<string, RunHistoryEntry[]>, nowMs: number): FleetRow[] {
  ...
  // list, so coalesce to [] before the for-of below.
  ...
  // Index the config by id. This is a row-level guard distinct from the list-level coalesce above:
  // the list guard defends the CONTAINER, and a roster record with no config object at all still
  // needs a safe read of `d.config.id` per ROW, or a single bad row among many good ones can throw
  // and blank the whole fleet table.
  //
  ...
  for (const id of ids) {
    ...
    // The cadence goes through the one seam (map.ts cadenceSecondsOf), which records a cadence that
    // arrives in a shape this build cannot read instead of nulling it silently. Passing the raw value
    // straight into the freshness rule would let a non-number simply fail `cadenceSeconds > 0` and
    // disable the staleness test, so a two-month-old backup would count as fresh. A row with no
    // backing config records nothing here: there is no cadence to read, which is not an anomaly.
...
//
// This reads the oldest good backup, not the newest: a recovery point is how much you could lose
// right now, and that is set by the stalest protected system. A fleet with one downpipe backed up
// five minutes ago and nine untouched for a month has a recovery point of a month, not five minutes.
//
...
// print for it. Folding it in would print a number smaller than the truth.
