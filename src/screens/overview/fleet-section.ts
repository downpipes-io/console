...
// The true-empty teaching state and the per-row triage deep-links are kept.
...
function buildAttentionTable(
  ...
  const handle = dataTable<FleetRow>({
    ...
    facets: [
      ...
      // rows, never AND to an impossible empty.
...
// is a <div role="img"> with the aria-label stating the whole sequence (role="img" on a <ul>
// conflicts with list semantics and suppresses the children; a <div> carries the image role
// cleanly). Each cell is a decorative <span> (aria-hidden), and the failed/in-flight glyph
// marks are aria-hidden too, so the strip reads as one labelled image, never a stale green
// and never colour alone (failed = hatch + a cross glyph; in-flight = dashed hollow + an
// ellipsis glyph; the same shape redundancy as sources-downpipes).
...
export function runTimeline(recent: RunHistoryEntry[]): HTMLElement {
  ...
  for (const e of cells) {
    ...
    // for a monochrome / colour-blind reader. All three marks are aria-hidden: the
