...
//
// WHY notoken IS A SEPARATE STATE. Folding it into unavailable would tell an estate that has simply
// never stored a discovery token "Not on this engine" for four surfaces the engine supports
// unconditionally (the adapters are compiled in; router-discovery.ts:247 advertises them the moment a token
// exists). Folded in, the tile would also be non-actionable, so the one control that fixes it would not be
// offered, and the remedy the copy would imply (roll the engine forward) would never work.
//
// This is the same class of mistake lib/token-source.ts guards against under its tokenPresent gate: "testing
// the flags alone therefore called a healthy, current, correctly-configured engine version skew for every
// customer who had simply never stored a read-only Cloudflare token". That module gates on tokenPresent and
// declines to report skew, expressly because "the screen tells the operator so". This screen reads
// tokenPresent the same way, including the distinction that matters: tokenPresent ABSENT
...
// The target screen enforces the role; this surface only navigates. No <style> is injected (rules
// live in tokens.css).
...
function coverageSummary(cov: SurfaceCoverage[]): HTMLElement {
  ...
  // COUNTED, NOT TYPED: this reads `cov.length` rather than a "9 surfaces" literal beside a grid
  // derived from SURFACES, so a tenth surface renders ten tiles under a chip that says ten, not nine.
  // See editor-wizard-source-rows.ts for the same rule applied to a source-type registry.
  ...
  const parts: Array<HTMLElement | string> = [
    totalChip(),
    " · ",
    // This chip must not say "protected": a surface merely having a downpipe is coverage, not health
    // (a paused downpipe, or one whose every run has failed for two months, counts here too). The tile
    // below says "In a downpipe", so the chip matches it (see security-centre/coverage.ts:131: reusing
    // the word "protected" here would overclaim).
  ...
  // The unknown count needs its own part too. Naming it only when EVERY surface is unknown would let a
  // mix of three read and six unread render "3 in a downpipe · 0 added · 0 not added": six surfaces
  // nobody checked, presented as two zeros, where "0 not added" reads as no gaps. Every other minority
  // state here gets its own count, so this one does too.
  if (unknown > 0) {
  ...
  // opt-in) and the actionable one. Counting it as "not on this engine" instead would make a healthy
  // current estate report four surfaces its engine supports as missing from that engine.
  if (notoken > 0) {
  ...
  // silent even on an up-to-date build.
  if (unavailable > 0) {
