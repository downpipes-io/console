...
//
// Index 0 of destinationIds is the PRIMARY the run seals to (engine src/sched/destinations.ts
// primaryDestinationId takes the first non-blank entry), and a restore reads its own bucket. Both
// pickers report the ticked ids in TICK order rather than the order the checkboxes are listed in, so
// the primary is whichever destination the operator ticked first, matching every other statement of
// the contract: the on-screen summary ("the first ticked is the primary"), the field catalogue row for
// the control, and the published docs page (backing-up/multiple-destinations, "Tick order is the
// selection order: the first ticked is index 0, the primary").
