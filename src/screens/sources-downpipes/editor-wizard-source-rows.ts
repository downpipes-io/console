// Source-step row construction for the guided create wizard. See ./editor.ts for the barrel.
//
...
// the two paths run identical logic with no synthetic DOM event.
...
// share one reference. Two selection families coexist, MUTUALLY EXCLUSIVE (ticking one clears the other):
...
export function buildSourceList(ctx: WizardSourceCtx, found: SourceDiscovery): void {
  ...
  if (!ctx.tokenPrefillApplied && tokenPrefill?.type === "cf-config") {
    ...
    // Nothing is recorded here, deliberately: a type-only hand-off (no zone/account named) is the DESIGNED
    // Add-a-source journey (add-source.ts builds the link with the type alone and lets the wizard collect
    // the account/zone), and the whole point of the fallback above is to leave the section for the
    // operator to disambiguate when more than one zone or account is discovered. Recording a diagnostic
    // here would fire on that ordinary click, on an ordinary estate, indistinguishable from any genuine
    // fault.
    //
    // The genuine fault such a class would exist to name -- a deep link that NAMES a zone discovery no
    // longer returns -- has no producer in this console. tokenSourceCreatePath can build a ?zone=/?account=
    // link, and the editor's prefill reader parses one, but no screen ever emits one: every in-app route to
    // /downpipes/new carries the type alone. A class no build can put in the ring is dead vocabulary that
    // reads like coverage, and a pack section that can never be populated is worse than an honest absence:
    // it tells a support engineer the evidence was looked for and not found. If a per-zone "Protect this
    // zone" control is ever built, it can emit the named link, and a class with a real producer behind it
    // can come back then.
  }
  ...
  // cf-config offers a SCOPE PRESET (what a ticked zone's downpipe captures) plus tick-many
  // checkboxes over the discovered zones and accounts,
  ...
  // Counted off the live discovery catalogue, never hardcoded into the copy: the surface count changes as
  // Cloudflare's registry changes, and a fixed number in this string would go stale silently. An empty
  // catalogue (an older engine, or discovery not yet answered) renders the sentence without
        ...
        { "data-dp": "sources-downpipes.button.open-editor",
          ...
          on: {
            click: () => {
              ...
              // chosenAccountId. cf-config is a tick-many selection (chosenCfZones /
              // chosenCfAccounts): a SINGLE ticked zone or account (nothing else ticked) carries
              // its identity across just as a single pick would; a bulk (2+)
              // selection carries nothing cf-config-specific, the same as a multi kv/r2/d1 tick
              // already carries nothing (there is no one identity to hand off to a
              ...
              // default, per the compact-wire-form invariant).
  ...
  if (!ctx.tokenPrefillApplied && tokenPrefill !== undefined && tokenPrefill.type !== "cf-config") {
    ...
    // Nothing is recorded here either, for the same reason as the cf-config hand-off above: the type-only
    // fallback only auto-selects when there is exactly ONE candidate row, so recording anything here on a
    // multi-account estate would stamp a class documented as "the deep link named an ACCOUNT and no
    // discovered row carries it" when no account had been named at all -- a row asserting a fact the code
    // never established, on a legitimate state, which costs support time rather than saving it. No console
    // screen emits an ?account= link, so this honest state has no producer, and no class exists for it.
  }
}
