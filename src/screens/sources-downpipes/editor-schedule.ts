// Advanced-schedule disclosure section (engine DownpipeConfig.schedule). Exported so editor-upsert.ts
// mounts it; see ./editor.ts for the barrel.
//
// The section's static fields, blackout-window repeater, and live preview are factored into sibling
// modules (editor-schedule-fields.ts / editor-schedule-windows.ts / editor-schedule-preview.ts); this
// file orchestrates them and owns the assemble / showError closures.
...
export function buildScheduleSection(existing: DownpipeSchedule | undefined): {
  ...
  // validateFields() runs the cron and timezone validators at SUBMIT (design-system.md 8: validate on blur, then
  ...
  const body = h(
    ...
    h(
      ...
      windowsHost,
      addWindowBtn,
      // Group-level doc link: the per-row start/end time fields are one maintenance-window
