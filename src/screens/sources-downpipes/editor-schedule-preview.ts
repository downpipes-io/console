// Live next-runs preview for the Advanced-schedule section. The section wires this onto every
// relevant input.
//
// The preview does not own the cron and timezone refusals: both controls carry their own validator
// (editor-schedule-fields.ts) and refuse through the field funnel on blur and on submit. The preview
// reads the same two functions only to decide what it can show; it renders no error of its own, and it
// never records, since it runs on every keystroke and a half-typed cron is not a refusal.
