...
// wizard's name step and the Sources bulk-protect tier. It is a LEAF module (imports lib/dom only)
// so the Sources screen can reuse it without pulling in the wizard's module graph (which would
// cycle through the editor). With more than one destination it is a MULTI-select: tick the
// destinations a downpipe is copied to, the first ticked is the primary, the rest are replicas;
// leaving all unticked follows the default (a single copy). With one destination or none it
// degrades to the caller's one-line summary.
