// Create / edit upsert editor for the Sources + downpipes screen (flow.md D). The big cohesive
// sections (secrets repeater, cf-config surfaces, retention, destination picker) and the submit-path
// SourceSpec assembly live in sibling section modules. See ./editor.ts for the barrel.
...
export function openEditor(
  ...
  //
  // This editor is the path most exposed to a lost update: the drawer's switch and the row toggle each
  // move one boolean, but this moves the name, the cadence, the source filter, the destination list, the
  // retention policy and the schedule at once, whole-object. Without the precondition, two operators who
  // open this drawer on the same downpipe could both be told they saved, with whoever saves first losing
  // everything they changed and nothing said about it. Absent (a create, or an engine older than the
  // precondition) states nothing.
  existingRev?: number,
): void {
  ...
  const typeField = h(
    ...
    typeSeg,
    typeHint,
    // Group-level doc link: the segmented radios are one control, so the link explaining
  ...
  // Secrets repeater: { name, binding } rows.
  ...
  // to weekly on every upsert, which would silently reset a tuned or opted-off drill
  // if this field ever omitted it (0 = off must be re-sent explicitly).
  ...
  function setType(t: SourceType): void {
    ...
    // for all three media types regardless of what is typed here. These fields hide for media too, so an
    // operator cannot type a prefix that saves with no error and no effect. Hidden the same way cf-config/
    // workers are; a record-count cost projection is meaningless for media too (it has no per-run KV read cost).
  ...
  function assembleDownpipe(): { dp: Downpipe; schedule: Downpipe["schedule"] } | null {
    // Runs the active override's own pattern (and, for secrets, each started row's pattern) at submit, not
    // only at blur: the engine's own checks are LOOSER than all five patterns here (resId is
    // /^[A-Za-z0-9_-]{1,128}$/, engine/src/sched/config-validate.ts:365), so a namespace id with no hex in
    // it, a bucket name starting with a hyphen or a binding starting with a digit would otherwise reach the
    // engine and be stored exactly as typed, under a field the console shows as invalid.
    ...
    // Reject workers.dev hosts up front before the engine even sees the request. The console
    // custom-domains-only rule means a binding that looks like a workers.dev URL is almost certainly a
    // misconfigured paste (the field should hold a binding name like KV_uploads, not a URL). A clear
    // inline error is better than a late engine-side 400 with a cryptic message.
    const rawName = nameField.value().trim();
    if (isWorkersDevHost(rawName)) {
      // A CONSOLE rule, not the engine's, so it records (refuse) rather than merely showing (setError).
    ...
    if (
      ...
      // The advanced editor refuses the create locally here, because an account-scoped source reached it with no
      // account. The console is right to refuse (an engine 400 the operator cannot recover from in this editor is
      // worse), and no request is made, which is the fact this class carries: there is nothing to find in any
    ...
    if ("error" in built) {
      if (built.error === "secrets-empty") {
        ...
        // The binding box is EMPTY, so this is setError and not refuse: the console examined no value,
    ...
    // Runs the cron / timezone validators at submit, through the field funnel, so a malformed cron that the
  ...
  async function submit(): Promise<void> {
    ...
    try {
      ...
      // A 403 is not a configuration problem, so it must not imply the configuration was at fault. It is
      // not automatically a ROLE denial either: only the route gate's own refusal (forbiddenClass
      // engine-capability) establishes that. The CSRF check, the DO's authz funnel and an edge block page
      // do not, and errorDetail says what each of them means, so the capability sentence is added ONLY
      // where it is true. A 400 from the DO is an inline, expected outcome; the drawer stays open and
      // populated, input never lost (flow.md D step 7, current-state.md C4).
