// Secrets repeater section for the upsert editor (the { name, binding } rows). See ./editor.ts for
// the barrel.
...
// buildSecretsSection builds the secrets repeater: a list of
...
export function buildSecretsSection(
  ...
  const addSecretRow = (preset?: { name: string; binding: string; storeId?: string }): void => {
    const i = ++idSeq;
    // Both boxes state their shape in the hint and validate it: a row whose name or binding the engine
    // will not take is DROPPED at submit only when it is entirely EMPTY (getSecrets below); a row with a
    // malformed name would otherwise reach the engine and come back a 400 against the whole downpipe,
    // naming no row. Blank stays valid on both, because a blank row is the repeater's own "unused row"
    // state, not an unfinished entry.
    ...
    //
    // Without it, every row's two fields carry the labels "Secret name" and "Binding", so a repeater
    // holding five secrets announces five controls with one name each to assistive technology, with
    // nothing to say which row is being edited.
    //
    ...
    const delBtn = h(
      "button",
      // Named per row for the same reason as the two fields above. This is invisible to the
      // duplicate-name oracle, which collects input, select and textarea and not buttons, so five
      // identical "Remove this secret" buttons would not be caught by that check; it is the same defect
      // for assistive technology regardless, so each is named individually here.
  ...
  // Pre-populate from the existing config when editing, so an operator editing a secrets downpipe can
  // add or remove secrets without a full recreate. A fresh editor reached from the add-source flow
  // seeds the first row's binding from the prefill
  ...
  const el = h(
    ...
    secretsRepeater,
    addSecretBtn,
    // Group-level doc link: the repeated name + binding rows share one concept (the secret
  ...
  return {
    ...
    // building the request: the engine's name check is length-only (1 to 256, engine/src/sched/config-
    // validate.ts:377) and its binding check permits a LEADING DIGIT, both looser than the patterns here,
    // so a row that never triggers a blur validation would otherwise reach the engine and store intact
    // under a field the console marks bad. A row with neither box filled is the repeater's idle state and
    // is skipped, matching getSecrets, which drops it.
