...
// reuses. Living here lets the per-section render modules share them without a cycle.
...
// cost library's DAYS_PER_MONTH (30.44), imported rather than redeclared, so this per-run
// ceiling and the screen's cost line always agree on one month length.
...
export function freshnessFor(state: DownpipeState, facts: RunFacts | undefined, now: number = Date.now()): Freshness {
  ...
  // THE SHARED RULE CAN REFUSE TO DATE A DOWNPIPE. classifyFreshness returns "unknown" when it was
  // handed run data this build cannot read (a lastGoodAt that will not parse, a run status it does
  // not know) or a history it could not read at all, and it returns it precisely so that NO GREEN
  // CLAIM is made on a backup nobody can date. Checking only `core === "stale"` would let "unknown"
  // fall through to a green "Ok" here, so the same downpipe would read green in the list the customer
  // actually looks at while the map says nobody could date it; checking core === "unknown" explicitly
  // keeps the two surfaces in agreement. A run row exists (an Idle pipe has already returned above),
  // so this is not the never-run state: it is a run whose freshness cannot be judged.
  if (core === "unknown") {
...
export function restoreTestRecency(
  ...
  // collapsing them into one loud "Last test failed" would read a freshly created or just-edited
  // downpipe with nothing to drill yet as a recoverability failure. The three: a DEFERRAL (could not
  // test: no completed backup yet, or a break-glass-only posture), a real FAILURE (the drill ran and
  // the archive failed it; the persisted coarse cause is named), and a legacy not-passed with no
  // recorded cause (recorded before deferral tracking).
...
// The route to the surface that actually sets it: Destinations sets it from the console.
...
// OUT_OF_ACCOUNT_DEST_NAME names the store in the block's title. A generic "S3 destination" label would
// misname a store the console can otherwise name precisely; "an S3 destination" over an Azure Blob
// container names the wrong protocol as well as the wrong company, because Azure is not reached over
// the S3 API at all.
...
function outOfAccountDestBlock(kind: "s3" | "gcs" | "azure"): HTMLElement {
  // Neutral, not a warning: an off-account store is the destination that Object Lock can actually
  // protect (Cloudflare R2 cannot enforce it on any bucket), and out-of-region latency or egress cost
  // is a cost note the operator can weigh, not a risk finding.
  const block = h("div", { class: "dest-block" });
  block.appendChild(
    h(
      "div",
      ...
      h(
        "div",
        h("div", { class: "dest-block__title" }, OUT_OF_ACCOUNT_DEST_NAME[kind].name, badge("default", "Out of account")),
  ...
  block.appendChild(
    h(
      ...
      `${OUT_OF_ACCOUNT_DEST_NAME[kind].article} ${OUT_OF_ACCOUNT_DEST_NAME[kind].name} is off-account: the kind of destination an Object-Lock policy can protect. It may add latency and egress cost outside your Cloudflare account or region, so confirm the region suits you.`,
    ),
  );
  return block;
}
...
// The in-account R2 destination: convenient (nothing to provision in another cloud), and unable to
// enforce Object Lock on any bucket, so it shares this account's own credential and blast radius with
// the sources it protects. Valid on its own and a good secondary leg, not badged as preferred.
function r2DestBlock(): HTMLElement {
  ...
  block.appendChild(
    h(
      ...
      h(
        "div",
        h("div", { class: "dest-block__title" }, "In-account R2", badge("default", "No Object Lock")),
  ...
  block.appendChild(
    h(
      ...
      { class: "dest-block__note" },
      svgIcon(ICON_INFO, { size: 14 }),
      "In-account, convenient, no immutability: Cloudflare R2 cannot enforce Object Lock, so this destination shares this account's blast radius.",
    ),
  );
  return block;
}
...
export function destinationSummary(status: StatusReport | null): string {
  ...
  // Named, for the same reason the residency block above is: leaving these two to fall through to the
  // residual would read "Not set (set it in Destinations)" over a configured, working destination.
...
// The console only ever uses custom domains; a workers.dev value
...
//
// Returning err.message untouched would let the transport's throw stand as the sentence: a wire verb
// on the front, the status glued past the full stop, and on a 403 the console's own classifier token
// ("forbidden-class=not-engine-body: 403") shown as though it explained something. It would also have
// no reading at all for a cancelled step-up ceremony, whose throw is an internal marker and a 401.
//
