// Wire SourceSpec assembly for the guided create wizard's "Create downpipe" step. See ./editor.ts for
// the barrel.
//
// buildWizardSource maps the wizard's chosen-* selection to the engine SourceSpec, branching on the
// chosen type. It is a pure function of the shared WizardState plus the media content opt-in; the
// caller still owns the Downpipe envelope (id, name, cadence, destinationIds).
...
//
// THERE IS NO BINDING ARM, and there is no reachable one. The binding-backed kinds (kv / r2 / d1,
...
// (assembleCfConfigBulk). So nothing in the console ever assigns chosenType = "kv" | "r2" | "d1", and a
// binding cannot go missing on this path even in principle: on the path that HAS bindings, the binding is the
// map key. WizType carries exactly the four radio types (editor-types.ts), so a fallback arm handling a
// binding-backed chosenType would be unrepresentable, not merely unreached.
//
...
export function buildWizardSource(state: WizardState, chosenType: WizType, includeContent: boolean): SourceSpec {
  // Each branch below carries the account only when it is non-null (`...(x !== null ? {accountId: x} :
  // {})`). If chosenType reaches here with no account picked, the assembled source spec is missing a field
  // the operator supplied two steps ago, and the engine answers a generic 400 that names a shape problem
  // rather than the missing account, leaving support unable to tell a broken product journey from operator
  // error. noteWizardLoss records that this state was reached (see its own comment for the one path that
  // can actually cause it); the account id itself is the customer's own and never rides in the diagnostic.
...
// noteWizardLoss records the accountless spec buildWizardSource is about to bake and SEND. It reads the
...
// console screen emits one, so this state reflects that no account was ever picked, not a pick lost mid-flow.
//
