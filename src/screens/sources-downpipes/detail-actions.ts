...
// operations and the per-item failures summary.

// openBulkSummary lives as a shared component, reused by the fleet drill; re-exported here so
// the detail.ts -> sources-downpipes.ts -> tiers.ts import chain stays stable for existing callers.
...
// A naive toggle spreading `state.config`, THE CONFIGURATION THIS SCREEN LOADED, over
// `POST /admin/downpipes` (a WHOLE-OBJECT upsert) would carry this page's stale copy of the cadence,
// the schedule, the source filter, the destination list and the retention policy back over whatever the
// account is actually holding: an operator who clicks one switch would silently revert a colleague's
// edit to a field they never touched, with both operators told they succeeded.
//
// THE WINDOW IS THE LIFETIME OF AN OPEN DRAWER, not one round trip, so nobody has to be unlucky for
// this to bite: any edit a second operator makes while this drawer sits open is at risk until the
// switch is flipped.
//
// Re-reading and then writing would only NARROW that window, from the lifetime of a screen to one round
// trip, and a repair that narrows a window while still overwriting silently is still a silent overwrite.
// So the read is used to DECIDE rather than to refresh: if anything but `enabled` has moved since this
// screen loaded, the flip is refused and the operator is told what happened, which is the same thing the
// engine's change-control path already says when an approved change finds its base moved. Only the
// `enabled` field is then flipped, on the LIVE record.
//
// THE REMAINING ROUND-TRIP WINDOW, between this read and the write below, is closed by the engine's own
// precondition: it takes a stated base on the upsert and the delete and refuses a moved one with 409
// (engine/src/sched/downpipe-precondition.ts), and the write below states that base. The read-decide-write
// closure happens inside the engine's storage boundary, not on this screen.
export async function toggleEnabled(engine: EngineClient, state: DownpipeState, reload: () => void, closeDrawer: () => void): Promise<void> {
  ...
  try {
    ...
    // THE BASE IS STATED on the write, closing the remaining round-trip window: the check above compares
    // this screen's config to the live one and refuses a flip that would revert an edit, but it is a
    // read-then-write, so an edit landing between the read on the line above and this write is still
    // inside one round trip. `live.configRev` is the revision that read returned, so if anything moved in
    // between, the engine refuses with 409 and names the field, rather than both operators being told
    // they succeeded.
...
// states one, from the other side of the same collision: without it, a delete racing another operator's
// save could answer {"deleted":true} while that save recreates the downpipe, enabled and on its cadence,
// telling this operator a backup had stopped while the product goes on running it. The stated base lets
// the engine refuse the race instead.
export async function deleteDownpipe(engine: EngineClient, dp: Downpipe, reload: () => void, closeDrawer: () => void, base?: DownpipeBase): Promise<void> {
  ...
  try {
    ...
    if (res.status === "pending") {
      // The change-control gate queued the delete for a second approver (HTTP 202). Falling into
      // the "nothing was deleted" warning below for this case would read as an undeletable
      // downpipe, so the queued state is said honestly instead.
      closeDrawer();
...
export async function bulkDisable(engine: EngineClient, rows: DownpipeState[], reload: () => void): Promise<void> {
  ...
  const ok = await confirmModal({
    ...
    // The body states both things a pause does, not just one: it pauses scheduled backups AND stands
    // down the retention prune, so nothing in the archive is deleted while the downpipe is paused. It
    // also names the one activity a pause does NOT stop, the canary, at the control rather than only
    // on a page.
...
// `action` is the CLOSED bulk-action id, threaded from the three entry points above: it is what
// distinguishes a half-failed DELETE from a half-failed RUN in the ring, rather than the two
// coalescing into one indistinguishable row. `verb` stays the operator-facing prose the toast and the
// modal render.
...
export async function runBatch<T>(
  ...
  for (const row of rows) {
    ...
    if (outcome === "signed-out") {
      // A session expiry cut the batch short, so the rest was never attempted. Counts only (how many
      // items did not complete); the downpipe names never enter the ring.
      halted = true;
...
// limiter storm reads as one labelled failure rather than N unexplained ones, and the loop yields
