...
// upsert editor, the advanced-schedule section, the wizard and the list importer live in sibling
// modules by section; this module keeps the query prefill and re-exports the moved entry points, so
...
export function prefillFromQuery(query: URLSearchParams): EditorPrefill {
  ...
  if (type === "kv" || type === "r2" || type === "d1" || type === "secrets") out.type = type;
    ...
    // The deep link named a source type this console build does not know, so the type is dropped on
    // the floor and the editor opens with nothing selected. This is indistinguishable from an operator
    // who opened the wizard themselves: the prefill is consumed browser-side, no request is made, and
    // the engine sees nothing at all.
    //
    // NOISE: a bare /downpipes/new with no `type` at all is the ordinary way the wizard is opened and
    // records nothing. Only a link that named a type the console could not honour does. The type string
    // never rides further: it is a query parameter, an arbitrary string from outside the console.
