// Live KV cost-projection line for the upsert editor (KV only). See ./editor.ts for the barrel.
//
...
export function buildCostLine(onUseDaily: () => void): {
  ...
  const recalc = (currentType: string, cadence: number, count: number): void => {
    ...
    // Shares the SAME average-month constant (DAYS_PER_MONTH, 30.44) that projectMonthlyKVReads divides
    // by, so this run count and every runs-per-month figure on the costs screen agree with each other.
