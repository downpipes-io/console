// Blackout-window repeater for the Advanced-schedule section. Owns the maintenance-window row DOM and
// the read into a BlackoutDraft; the schedule section composes it and supplies the change callback for
// the live preview.
...
export function createWindowsRepeater(WEEKDAYS: string[], onChange: () => void): WindowsRepeater {
  ...
  function addWindowRow(seed?: { days: number[]; startMinute: number; endMinute: number }): void {
    ...
    // doc: the maintenance-blackout-windows section states this field's HH:MM 24-hour format,
