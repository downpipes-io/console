// Source-step section assembly for the guided create wizard. See ./editor.ts for the barrel.
//
...
// Stream, Images, Artifact Registry), each gated on what the engine advertises. The per-row
// apply()/builder closures are passed in; they live in ./editor-wizard-source-rows.ts and own the
...
// cfConfigWithheldReason names WHY the Cloudflare configuration section is not on offer, distinguishing
// the reasons rather than letting the section simply not render with no explanation. Each reason names
...
// cfWithheldCatalogueClass is cfConfigWithheldReason's CLOSED-CLASS TWIN: the same branches, in the same
...
//
// cf-catalogue-empty IS GATED ON `tokenPresent === true`, WITHOUT WHICH IT WOULD BE THE COMMONEST FALSE ROW
...
// a healthy, current, bound-sources-only engine would write a row telling support to update it, the same fault
// the token-source module gates against (src/lib/token-source.ts: tokenSourceSkewFamilies returns []
// unless tier === true), inverted.
//
...
// added (which the caller uses to pick the list vs the empty state).
export function appendSourceSections(
  ...
  // so without this gate they would be offered here even when nobody added them. An ABSENT field (an older
  // engine, or the env-token IaC fallback) means UNGATED: keep the legacy behaviour and offer every supported
  // type. A PRESENT array gates: a type is offered only when it was added, so this wizard mirrors the Sources
  // screen, never a source nobody added.
  ...
  // own. (Gating on engineAccountId would silently drop this whole section for a multi-account or
  // under-scoped token, since the engine leaves engineAccountId null until one account is
  // designated, so config would vanish with no explanation.) Offer the scope preset, then
  ...
  // the wizard N times. When no zones list, say how to broaden the token rather than dropping
  // config.
  ...
  if (cfSupported && cfAccts.length > 0) {
    ...
    if (!cfAccts.some((a) => a.zones.length > 0)) {
      // The three WITHHELD classes below only fire when the whole Cloudflare section is dropped; a token that
      // can read the account but lists no zones keeps the section on offer, so none of them fires, the
      // account-wide surfaces work, and the zone half is simply absent. The hint beside it says so to whoever
      // is looking at the screen, but that dies with the render, so this branch also records the observation
      // below.
      //
      // THE ROW IS EMITTED ONLY OFF ACCOUNTS THE TOKEN ACTUALLY READ (`found.accounts`), never off cfAccts. The
      // block above synthesises a placeholder account from engineAccountId when discovery returned NONE, and that
    ...
    // Cloudflare configuration is not on offer. Simply vanishing here would read on screen as "this product
    // does not exist", giving an operator whose discovery token had been rescoped no way to tell an
    // under-scoped token from an old engine from a source nobody added, so name which of the three it is.
    // These are the only three reasons the section can be withheld, and each has a different fix, so each
    // gets its own sentence rather than one hedged catch-all.
    //
    // This also records which of the three confusable reasons applied. The hint below says it to whoever is
    // looking at the screen and dies with the render; the row says it to support: an under-scoped token, an
    // engine that predates the feature, and a source that was never added are three different remedies. No
    // token, account id or surface name rides: the class is the whole row.
