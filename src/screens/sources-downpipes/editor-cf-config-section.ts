// Cloudflare configuration (cf-config) source controls for the upsert editor. See ./editor.ts for the barrel.
//
...
export function buildCfConfigSection(
  ...
  // inBand is carried through from the engine catalogue so the picker distinguishes the surfaces a
  // restore actually re-applies from the ones the customer has to re-apply themselves; without that
  // distinction every surface looks identical, which is the single most over-claimable number in the
  // product, and the picker is where a customer forms their expectation of it.
  //
  // `available` is the other half of that split: a surface whose write path exists but has never been
  // proven against a real account is not the same as a surface with no write path at all, even though
  // both are off by default. To an operator deciding whether to name one they matter differently: naming
  // an available surface puts it in the restore scope and binds it into the approval hash, and naming a
  // surface with no writer does nothing.
  ...
  // every account one, so this downpipe stops duplicating the account configuration in every zone);
  // "Account + this zone" ticks every visible surface (mirrors the existing "All"
  ...
  const el = h(
    ...
    cfSummary,
    cfPanel,
    // Group-level doc link: the capture-mode select and the per-surface tickboxes are one
    ...
    void engine
      ...
      .then((found) => {
        ...
        if (cfCatalogue.length === 0) {
          // The editor re-read the catalogue and got nothing back, on a downpipe that is ALREADY backing up
          // cf-config. The saved selection is preserved (correctly), so the screen looks calm, and nothing says why
          // the list is not there. Two states arrive here and they are not the same fault, so only one of them is
          // recorded, on the same gate the wizard's twin applies (cfWithheldCatalogueClass,
          // editor-wizard-source-sections.ts):
          //
      ...
      .catch((err) => {
        // The surface list did not load, so the operator is re-picking surfaces against a list that is not
        // there. The throw is classified, matching the Rediscover button next door (detail-config-section.ts),
        // so a swallowed read does not read as no fault at all.
        //
        // A lapsed Cloudflare Access session is the commonest editor open there is (a tab left open overnight,
        // and this console is Access-fenced), and it must not write a fault row against a healthy engine and a
        // healthy token on the very screen whose remedy is "go and look at your discovery token": it records
        // nothing here, and neither does a 401, which signs the operator out exactly as it does everywhere
        // else. The remaining classes are five different phone calls: the address, the role, an edge refusal,
        // a rate limit, a dropped call.
