// Bulk list-import drawer for the Sources + downpipes screen. See ./editor.ts for the barrel.
...
// Exported for the validator. Both this and runImportLoop below decide what a bulk import actually
...
export function validateImportInput(raw: string, known: ReadonlySet<string>): { ok: true; unique: string[]; dupCount: number } | { ok: false; error: string } {
  ...
  // Reject workers.dev hosts in the import list up front, per the custom-domains-only rule. An
  // operator pasting a URL instead of a binding name gets a clear error.
...
// Exported for the validator, as above.
  ...
  for (const binding of unique) {
    ...
    try {
      ...
      if (isUnauthorised(err)) {
        // A session expiry cut the import short. Counts only: how many of the bindings never completed,
        // and the class of the ending. The binding names are operator labels and never enter the ring.
        halted = true;
...
export function openImport(engine: EngineClient, status: StatusReport | null, known: ReadonlySet<string>, onDone: () => void): void {
  ...
  const listField = field({
    ...
    // The shape of each line is checked at the field, and the offending line is named, so a paste of
    // forty bindings with one typo is not refused as a whole at submit, leaving the operator to
  ...
  importBtn.addEventListener("click", async () => {
    ...
    // Runs the field's own per-line binding-pattern rule first, through validate(). validateImportInput
    // below re-checks emptiness, workers.dev hosts and already-known bindings, but not the per-line
    // binding pattern, and the engine's own binding check permits a leading digit the console's does not.
    // Running the field's rule first means a malformed line is reported by line number, not as a row of
    // engine 400s.
