...
// render cells live in detail-cells.ts.
...
function loadRunHistory(
  ...
  void engine
    ...
    .catch((err) => {
      ...
      // A bare "Could not load recent runs." would name no remedy and leave nothing to press. This read
      // also feeds the freshness banner and the throughput summary, so a failure here quietly takes
      // three things off the drawer at once; the Retry brings all three back.
...
function openDownpipeDrawer(
  ...
  const handle = openDetailDrawer({
    ...
    // back on the row the user activated, not on <main>.
    focusReturn: () => findRowByKey(dp.id),
    // onClose fires a SECOND, real navigation (back to the plain list). Without the declaration below,
    // that second navigation would quietly undo the focusReturn above: teardown() restores focus to the
    // row synchronously, then this navigate() call resolves the /downpipes route, whose screen declares
    // no post-navigation focus intent of its own -- an ORDINARY route change, which is correctly supposed
    // to move focus to <main> (lib/nav.ts's PostNavigationFocus "none" case). The shell's setMain applies
    // that move on a LATER tick (the View Transition's deferred update callback), after teardown() has
    // already returned, so it would silently overwrite the just-restored row a moment later, landing
    // focus on <main> instead of the row even though the row was briefly focused first.
...
export function openDetail(
  ...
  // per-action gates internally. Neither takes a shared operator-rank flag; each reads its own
  // capability directly.
