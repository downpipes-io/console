...
export function cfConfigDiscoverySection(engine: EngineClient, dp: Downpipe, state: DownpipeState, opGate: boolean, reload: () => void): HTMLElement {
  ...
  modeSel.addEventListener("change", async () => {
    ...
    try {
      ...
      // the engine answers 202 and applies nothing. Without this branch, the queued answer (no `ok` field)
      // would fall into the else below, telling the operator the change could not be made and reverting the
      // select for a request the engine had actually accepted. The select is reverted here TOO, but for the
      // true reason (nothing has changed YET), and the toast names the queue rather than a failure.
  ...
  rediscoverBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (res.ok && res.discovery) {
        ...
        // the owner took out of discovery scope and a rate limit are three different remedies, so they are
        // shown as three distinct rows rather than one coalesced count. The engine's sentence selects the
        // member and is never copied.
  ...
  // the discovery token), so collapsing them into one "N unavailable" figure would hide which remedy applies.
...
// neverDiscoveredHint is the "not yet discovered" line, with the row that makes it visible remotely.
