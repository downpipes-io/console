// Guided create wizard (flow.md J) for the Sources + downpipes screen. See ./editor.ts for the barrel.
...
// openCreateWizard is the guided "New downpipe" flow: nothing hand-typed, one decision at a
// time, one step visible. Two steps when a destination exists: pick the source from what is
...
export function openCreateWizard(
  ...
  // this function share one reference rather than each keeping separate closed-over state.
  ...
  //     becomes its own downpipe, so an N-zone customer bulk-creates in one pass. All five are
  //     token-authenticated (the read-only discovery token), so none need an inline attach.
  ...
  // ./editor-wizard-source-rows.ts) is the wizard's one coarse, guided cf-config decision,
  ...
  // there (./editor-cf-config-section.ts).
  ...
  const destProbe = Promise.all([
    ...
    .then(([d, list]) => {
      ...
      if (d.present) {
        ...
        destStep = "configured";
        // The DEPLOY-TIME destination is the case a first-time customer is most likely to meet, so it
        // needs to name the provider and explain why no bucket appears rather than leaving both
        // unsaid. The engine DOES report the kind here
  ...
  // the neighbour at the exact moment the operator is watching. Measured both ways, and reserving at the
  // transition moved Cancel 4.4px where doing nothing moved it 4.1px.
  queueMicrotask(() => {
  ...
  // Restore a name/cadence draft: survives Back/Continue, and also an interrupted session that
  // returns to "New downpipe" later, rather than retyping. Setting
  ...
  // make); done steps are revisitable via onJump.
  ...
  const renderSourceStep = (): void => {
    ...
    void engine
      ...
      .catch((err) => {
        ...
        // THE SIBLING READ. This is the SAME call the editor makes (GET /sources/discover), and when it
        // throws here the wizard shows a block error and the Cloudflare-configuration offer is withheld with it:
        // the operator cannot add a cf-config downpipe at all. This throw is classified through the same
        // classifier as the editor's twin call, which records NOTHING for a lapsed Access session (the
  ...
  // selectionSummary is the name step's collapsed one-line source summary: the
  ...
  const renderNameStep = (): void => {
    ...
    seedName();
    stepHost.replaceChildren(
      // The collapsed one-line summaries of the earlier steps:
  ...
  const advanceFromSource = (): void => {
    ...
    if (state.chosenInput !== null) {
      ...
      //
      // ONE continuation, not two linked by a flag: splitting the queued-vs-advancing decision across a
      // mutable flag would put the button-release logic in a different closure from the busy-set, so a
      // reader would need both closures together to see whether the button is ever released. Keeping it
      // in one continuation keeps that reading a single, local one.
      void engine
  ...
  // Downpipe; a CHECKBOX selection assembles the batch through the shared
  ...
  const submitCreate = (): void => {
    ...
    // The RADIO path (a Cloudflare-wide token source): the single-create path, per-type wire
