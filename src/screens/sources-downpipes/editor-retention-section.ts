// Retention policy section for the upsert editor. See ./editor.ts for the barrel.
//
...
export function buildRetentionSection(existing: Downpipe | null): {
  ...
  // that scheduled pass defers and records passSkipCode "break-glass-only" rather than guessing. The house
  // no-customer-CLI rule means the hint cannot send the operator to a terminal: a break-glass-only
  // operator instead runs this same policy from their browser, on demand, with their own key (see
