...
// thin orchestrator (guardrail: file size + function size). The MULTI-DESTINATION list
...
import { banner, noteQuiet, skeletonRows } from "../components/feedback.ts";
...
export function renderList(engine: EngineClient, list: DestinationList, reload: () => void): HTMLElement {
  ...
  const finding = offAccountLockFinding(list.destinations);
  if (finding) wrap.appendChild(finding);
  // Add a destination, ABOVE the table (owner), mounted on first open so it stays cheap and
...
//
// ALL THREE MATTER, not only the commercial cloud: an Azure destination the engine routes to its Azure
// client and its Shared Key signer must be badged by the vendor it actually uses, and for a US Government
// or China-cloud account, matching by suffix rather than by the commercial host alone is what makes that
// possible. The badge is the sentence that tells the operator where their archives go, so naming the wrong
// vendor is not a cosmetic miss. The list is written out here rather than matched loosely for the same
// reason the engine writes it out: a nearly-right suffix is a host that never resolves, and the closed
// clouds absent from the engine's list (Microsoft Cloud Germany and the US air-gapped clouds) are absent
// from this one too, so the two agree by construction.
...
// PROVIDER_BADGE gives each provider its label and tone. Every provider reads neutral: R2 is
// in-account, convenient and cannot enforce Object-Lock on any bucket, and an in-account store
// shares this account's own blast radius with the sources it protects, so it does not wear the
// trust tone. The out-of-account stores are neutral too, because any of them can move data out
// of the Cloudflare account or region and the drawer carries the honest residency note. The label is
// always present (never colour alone), and naming Google or Azure rather than showing a generic "S3"
// is the point of the whole provider derivation: an operator reading their destination list should
// see where the archives actually go.
const PROVIDER_BADGE: Readonly<Record<Provider, { tone: "trust" | "default"; label: string }>> = {
  r2: { tone: "default", label: "R2" },
...
// hasOffAccountLock is true once at least one destination is both off-account (not R2, so it does
// not share this Cloudflare account's credential and blast radius) and Object-Lock enforced on the
// live probe (objectLock === "enforced": a configured-but-refused policy protects nothing, and an
// unknown read is not a proof). "deploy" source rows fall through providerKind to its "s3" guess
// (see providerBadge above), which is the safe direction for this specific check: a wrong guess here
// can only WITHHOLD the reassurance, never wrongly clear the finding, because a genuine deploy-time
// R2 binding cannot report objectLock "enforced" (the engine refuses a lock policy on R2 outright).
function hasOffAccountLock(destinations: readonly DestinationStatus[]): boolean {
  return destinations.some((st) => providerKind(st) !== "r2" && st.objectLock === "enforced");
}

// offAccountLockFinding is the destination screen's own posture note: a calm, non-dismissible warning
// when no destination leg is off-account on an Object-Lock-capable store. It is a finding, not a
// failure -- an operator who only holds R2 has a working, backed-up estate, so nothing here turns
// their otherwise-green state red; it names the one gap a same-account compromise would reach every
// copy through. Returns null when the finding does not apply (an empty list has nothing to warn
// about; the empty/unconfigured states carry their own copy) or when it is already satisfied.
function offAccountLockFinding(destinations: readonly DestinationStatus[]): HTMLElement | null {
  if (destinations.length === 0 || hasOffAccountLock(destinations)) return null;
  return banner({
    tone: "warn",
    message: "No destination is outside this account with immutability. A compromise of this account can reach every copy. Add an off-account destination with Object Lock as the primary leg.",
    dismissible: false,
  });
}
...
//
// The not-enforced cell reads "writes refused" and is a DANGER, not a warning. Reading it as merely "not
// enforced" would be true of the probe verdict but would understate the consequence to the point of
// misleading: a reader would take it as a working destination with a weaker guarantee. Both R2 (501
// NotImplemented) and AWS S3 (ObjectLockConfigurationNotFoundError) REFUSE a write carrying the lock
...
function openDestinationDrawer(engine: EngineClient, st: DestinationStatus, reload: () => void, count: number): void {
  ...
  // receives nothing at all. Saying the headers are merely ignored and archives are simply unprotected
  // would tell the reader they have a working destination with a weaker guarantee, when they have no
  // working destination. Object-Lock must be enabled when the bucket is created; it cannot be turned on
  ...
  // Once a second destination joins a deploy-bound one, THIS row is the deploy-time binding itself, a
  // real, permanent member of the collection (engine ensureDeployDestSeeded). It carries no console-stored
  // credential, so there is nothing for Replace to act on -- said here, beside the row it describes, rather
  // than leaving the lever to answer a submit with a raw refusal.
  if (st.source === "deploy") {
    body.appendChild(
  ...
  const handle = openDetailDrawer({
    ...
    // than only a single instance of it.
...
export function renderConfigured(engine: EngineClient, st: DestinationStatus, reload: () => void, count = 0): HTMLElement {
  const ownerGate = canDo("owner");
  const wrap = h("div", { style: "display:grid;gap:var(--space-5)" });
  const finding = offAccountLockFinding([st]);
  if (finding) wrap.appendChild(finding);
...
function renderPostureRows(engine: EngineClient, st: DestinationStatus, reload: () => void, ownerGate: boolean): HTMLElement {
  ...
  //
  // Checking only authMode === "sts" here would leave an Entra destination with no authentication row at
  // all, so a customer could not tell from the console that the destination signs in as a service
  // principal against Microsoft's identity platform rather than with a storage account key.
...
// warn line.
//
// BINDING INVARIANT: a destination whose delete probe was refused ("denied") never renders a reclaimed
// count. A count on that posture would claim deletions on a bucket that refuses them; the deletes-refused
// line cross-references the object-lock note below instead, so the two reads name one posture rather than
