...
// (build-contract 2.1). Each IA screen is a self-owned module that exports one
...
//              dashboards; design-system.md 2). Drives shell.setMain's width cap.
...
// the trust chip reads the REAL verdict, never a hardcoded green.
...
// Screen is the explicit descriptor every screen module exports (build-contract 2.1).
...
// is, from the header, the same "add" intent. The Destinations screen needs this: its
// deploy-bound state (a destination bound at deploy time, no console record yet) renders
// the add surface as a "Replace from the console" disclosure rather than an "Add a
// destination" one, so a caller that only asks for one state's title finds nothing there
// and falls through to the scope-scroll branch, leaving the button looking inert.
...
//
// The null branch WITNESSES (noteGateComputedBlind). canDo and canCap are the console's PRIMARY
// gates, so recording this case here lets the support pack's identity-stale-gate row name any screen
// that gates through here, restore included, rather than a raced Owner and a genuine viewer producing
// byte-identical evidence on exactly the screens that matter most. currentScreenGatedBlind() reads
// this witness to set app.ts's forceForBlindGate, so a same-identity re-render can fire for those
// screens once a late blind gate resolves.
...
// (design-system.md, "disabled-with-reason, not hidden-then-403"). It states the
// required role plainly so a gate reads as governed, never a mystery failure.
// It names the caller's CUSTOM role when they hold one, for the same reason capGateReason does: a
// caller on a named custom role carries role="viewer" as the engine's ladder floor (see canCap), so
// reading c.role would tell a customer on a "Recovery lead" role that they are a Viewer, a false
// statement about their own identity in the sentence meant to explain why they are stuck. The two
// generators agree on this.
...
// GRANT_REMEDY supplies the third sentence an authorisation refusal needs beyond WHAT is required and
// WHY it is refused: how to stop being stuck. Naming WHAT and WHY alone leaves a customer who reads
// "Requires the Owner role; you are Viewer" told nothing about how to act on it. A refusal a customer
// cannot act on is a defect even when the refusal itself is correct.
//
...
// established: on the Integrations grid, that reads as "Owner" (meaning "you are not the owner") to the
// owner themself, on every tile, whenever the caller has not resolved yet. Every other gated screen already
// keeps the two apart in its COPY (gateReason and capGateReason below both branch on a null caller and say
// the engine has not reported the role); this is the same distinction for a surface that renders a CONTROL
// STATE rather than a sentence, so the two cannot drift.
//
...
// bar. So "An Owner can change this on the Access screen" would name an action no Owner can take, on
// every control gated by them: the four Identity providers controls, the licence update apply and
// rollback, and the bulk source re-attach. The console says the true thing elsewhere ("Owner-reserved;
// cannot be delegated", roles-builder/form.ts), and this sentence must agree with it: under-naming who
// can help is survivable, naming somebody who will then be refused is not.
//
// The remedy here says what a customer can actually do, which is ask an Owner to perform the action
...
export function capGateReason(capability: Capability): string {
  ...
  // Name the caller's custom role when they hold one: canCap resolves authority from the custom
...
// refuseWithReason: the shared refusal primitive, which lives in lib/dom.ts beside h() so
// components/ can use the SAME implementation without forming a circular dependency (a components/
// file importing this module directly would create one; lint:circular). It is re-exported here so
// every screen that imports it from ./common.ts keeps working. The rationale, and the browser
// measurements behind the aria-describedby form, live with the implementation.
...
// back, so the copy is accurate and never fakes a capability. The
