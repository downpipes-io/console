...
function renderActionCard(ctx: RenderCtx, action: OwnerAction): HTMLElement {
  ...
  // An owner-action kind this console build has no label for renders its RAW id in the card heading, and
  // that heading is asking an owner to authorise it. The console says nowhere else that it does not
...
function renderSummary(summary: string): HTMLElement {
  ...
  if (text === "") {
    // The same shape as the unapprovable change on the config-change inbox, on the owner-action surface:
    // the operator is asked to authorise something the engine declined to describe.
...
// refusalCode classifies a REFUSED owner-action call from what the code can actually establish: the ONE
...
//
// THE AUTHORITY AXIS IS TESTED BEFORE THE LIFECYCLE, AND THE FATE IS READ ONLY FOR AN OWNER. A non-owner's
// listing is CALLER-SCOPED: canRequesterSeeOwnerAction (engine src/admin/owner-action.ts) shows an OWNER every
// record and shows everybody else only their OWN proposals. So a non-owner's re-read can never contain the
// action they just tried to approve; testing the fate off that listing first would read every such absence as
// `already-decided` ("a second owner rejected or ran it, go and read the audit") for an action that is still
// pending and that nobody has decided. The fate is therefore consulted only where absence is a FACT (an
// owner's listing is complete), and the engine's own 403 decides the authority axis first.
//
...
// at mount and threads them unchanged into every card. Classifying a refusal from those captured values would ask
...
// answer false, so self-approval would have no producer on the approve path and a genuine maker-checker
// refusal would be filed as a generic engine-refused. whoami resolves asynchronously, so by the time the operator
// clicks, the identity that was null at mount usually IS resolved, and it is the one the engine just judged; that
// is why the fresh read is used here instead.
//
...
//
// IT TAKES THE LISTING, NOT A FATE: the fate is derived HERE, from the caller the console holds and the listing
// the engine served, so a test must supply the same listing shape the engine would rather than a hand-made fate
// ("still pending", for a viewer looking at somebody else's action) that the real pipeline could never return
// for that caller.
...
// OwnerActionFate is what the inbox says became of the action, re-read AFTER the refusal. `unknown` is the
...
// ARMED for the proposer's one-shot re-submit: a decision, not a pending action, so the status is checked before
// presence and expiry rather than after (checking only presence and expiry would file that state in the
// residual). An unknown status from a newer engine answers `unknown` rather than being coerced into one of ours.
//
...
function approveButton(ctx: RenderCtx, action: OwnerAction): HTMLButtonElement {
  ...
  // calls (mirrors the overview drillInFlight guard).
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      // The refusal rides in the pack as a closed code BEFORE the toast that dies with it. The engine
      ...
      // the pack agrees with. This is not recorded for the sign-out case: noteRefusal drops a 401 itself.
...
// for both so the message matches the engine-recorded status and the status badge. Only the two shipped
// variants are valid.
type RejectVariant =
...
function rejectButton(ctx: RenderCtx, action: OwnerAction, variant: RejectVariant): HTMLButtonElement {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      // A reject that the engine turns down for a TERMINAL state is the "it vanished" ticket seen from the
      // other end: the operator goes to withdraw their own proposal and it has already expired out from
      // under them. Nothing anywhere records that.
