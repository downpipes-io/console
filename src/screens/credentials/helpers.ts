...
// only); status by hue + shape + label, never colour alone; honest copy.
...
// via aria-disabled rather than the native `disabled` attribute: the control stays in the
// tab order and the reason reaches a screen reader, rather than both silently dropping out
// together, which a disabled control paired with aria-readonly has no defined meaning for.
...
export function toDateInputValue(rfc3339: string): string {
  ...
  if (!Number.isFinite(ms)) {
    ...
    // value back. The empty string stays (a fabricated date would be worse), and the anomaly is recorded so an
