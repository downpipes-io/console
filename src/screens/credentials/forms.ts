...
// shape; a refusal is shown inline. It imports the shared leaf (./helpers.ts) only, so it never imports the
// list section (which would form a cycle).
//
...
function buildItemFields(existing: ExpiryStatus | null): ItemFormParts {
  ...
  // (engine/src/admin/expiry.ts:394-395, "expiresAt is required for a certificate or licence").
  // Refuse the COMBINATION at entry so the client never builds a request the engine would answer 400
  // to, losing the operator's form work at submit: with Certificate or Licence chosen the box is
  // announced as disabled with the reason, cannot be ticked by mouse or keyboard, and the date field
