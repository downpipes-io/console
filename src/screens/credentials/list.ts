...
// reader). It imports the shared leaf (./helpers.ts) and the write flows (./forms.ts) one way, so it
// never forms a cycle.
//
// House rules: status by hue + shape + label, never colour alone (an expired item never reads green, a
// no-expiry item reads NEUTRAL); honest copy.
...
// WriteMode avoids a boolean flag on the attention renderers (calm §6: no boolean
...
// Exported for the validator: the read-only gate here is what stands between a viewer and an
...
export function openItemDrawer(engine: EngineClient, row: ExpiryStatus, reload: () => void): void {
  ...
  const handle = openDetailDrawer({
    ...
    // over the first, the same guard sources-downpipes/detail.ts relies on.
