...
function renderGuide(engine: EngineClient): HTMLElement {
  ...
  wrap.appendChild(h(
    ...
    typeHint,
    discoveryNote,
    // Group-level doc link: the source-type radios are one control, so the link that explains
  ...
  function renderTokenHandoff(t: TokenSourceType): void {
    ...
    continueBtn.addEventListener("click", () => {
      ...
      void engine
        ...
        .catch((e) => {
          // Re-enable Continue and say why on failure (an expired session, or a refused add), rather
          // than leaving the button dead with nothing said.
          if (isUnauthorised(e)) {
  ...
  function getValidatedInput(): SourceInput | null {
    ...
    // The console's OWN validator turns the operator away here, so NO request is made and the engine sees
    // NOTHING: a console validator that drifts TIGHTER than the field catalogue is otherwise undetectable in
    // the field. The catalogue control id rides; the TYPED VALUE (a namespace id, a bucket name, a store id)
    // never does. A field id the closed vocabulary does not know is dropped rather than coerced into a
    // neighbouring row.
    //
    // AN EMPTY BOX IS NOT A REFUSAL: recordFormRefused takes the control's CURRENT raw value and writes
    // nothing when it is empty, because validateHexId / validateResourceName / validateDatabaseId each return
    // "The ... is required." for "", and pressing Attach on a fresh, untouched form would otherwise coalesce
    // with the refusal a real invalid value produces (for example a UUID the hex-only regex turns away),
    // burying the genuine case inside a count of half-filled forms. The value is read only for that emptiness
    // test; the typed value (a namespace id, a bucket name, a store id) never leaves the browser. A field with
    // no control to read (an error on a key fieldFor does not map) records NOTHING rather than asserting a
    // value existed.
  ...
  //
  // A rejected discovery is never silent: a failed read and an engine that genuinely supports nothing
  // more would otherwise look identical, leaving the picker showing four source types with no explanation.
  // The read outcome is stated, with a retry, so the screen never implies a catalogue it did not read.
  function offerTokenSources(): void {
    ...
    void engine
      ...
      .then((found) => {
        ...
        // The discovery read can succeed, the account can hold a discovery token, and the engine can still not
        // advertise a capability the console knows how to use. That token source then vanishes from Add a source
        // and the picker silently degrades to what is left. It is the compatibility branch of an OLDER engine
        // (the console asked for a capability the engine does not advertise), which is why it is
        // `legacy-path-taken` and not a fault: it is version skew, and after an engine ROLLBACK it is the reason
        // a customer's Workers source "disappeared".
        //
        // ONE ROW PER MISSING CAPABILITY, which is the whole point. The four flags landed on different dates, so
        // any engine build in between advertises a strict SUBSET, and a subset is not zero: an all-or-nothing
        // predicate would record NOTHING for an engine that offers cf-config but not Workers, which is exactly
        // what a rollback lands on. fieldFamily is in the ring's tuple key, so the SET of rows is the evidence:
        // cf-config-only is three rows, none-at-all is four, and a healthy engine is silence. The predicate is
        // pure and exported (token-source.ts), so the validator drives the REAL decision rather than the recorder.
        //
        ...
        // discovery observation while the screen says so to the operator. Firing here would call a current,
        // correctly-configured engine "version skew" on every mount, with the opposite remedy.
