...
// A break-glass-only flight reads its own cell back with that flight's per-run master, so it
// decrypts, restores and verifies like any other. Ailing therefore means only that a check could
// not complete: every ailing flight is a genuine check problem and reads as one.
...
// ailingDestNote names WHY an ailing destination is ailing. It mirrors deadDestNote, which names the dead
// ones: an ailing flight proves nothing either way, and the engine classifies the fault so this screen has
// something specific to say rather than nothing at all. An expired credential and a black-holed endpoint
// would otherwise produce identical copy and send the operator looking in the wrong place.
//
...
function deadDestNote(view: CanaryView, flying: boolean): HTMLElement | null {
  ...
  // AN ABSENT REASON IS NOT A CORRUPTION FINDING: deadReason is nullable on the wire, so the fallback here
  // states plainly that the engine did not report why, rather than asserting a byte-level corruption claim
  // against a named destination from no evidence at all. This follows the same rule ailingDestNote's comment
  // above states for a missing ailing cause.
    ...
    void engine
      ...
      .then((v) => {
        ...
        if (elapsed > FLIGHT_TIMEOUT_MS) {
          // The CLIENT ceiling can expire while the engine still has the run in flight. The animation ends,
          // the screen reloads, and the operator sees "flights end abruptly" against a run that is in fact
...
function renderDestinationPicker(view: CanaryView, owner: boolean, saveConfig: SaveCanaryConfig): HTMLElement {
  ...
  // Dangling pins: destinations the operator PINNED that no longer exist. They render no checkbox
...
export function renderSettings(engine: EngineClient, view: CanaryView, reload: () => void): HTMLElement {
  ...
  // Group-level doc link: the enable toggle and the destination picker below are one
      ...
      f.status === "alive"
        ...
        : dead.length > 0
          ...
          : f.status === "ailing"
            ...
            : // A FLIGHT WHOSE AGGREGATE IS DEAD BUT WHOSE per-destination ROWS CARRY NO DEAD ENTRY must not
              // fall through to "in progress" here, or a FAILED verdict would render in the history as a benign
...
// ---- WHY THE CANARY PICKER DOES NOT USE sources-downpipes/tick-order.ts --------
