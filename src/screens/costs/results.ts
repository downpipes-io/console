...
// reimplemented. It imports only the shared leaf (./helpers.ts), the cost-model library and the
...
interface PlatformLedger {
  ...
  // exact is true when the platform figure came from the run history's measured op tally.
  exact: boolean;
}
...
// engine's EXACT per-resource op tally AND the screen is in Observed mode, that is priced
...
//
// Gating the measured op-count seed on mode, not only on its presence, keeps the arithmetic honest with
// the label it produces: without this gate, switching to Manual and typing a hypothetical cadence would
// still price the platform ledger off the real historical mean, while the card's own label claims the
// whole monthly figure is "measured from your run history." A Manual-mode scenario is priced the same
// estimated way as every other hand-entered figure on this screen, and "measured" is true exactly when
// it is shown.
