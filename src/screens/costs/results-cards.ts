...
// Every figure is consumed from lib/cost-model.ts, never reimplemented.
...
// history's measured op tally; otherwise it is the over-estimate from object counts.
...
export function buildSourceTypeBreakdown(rows: SourceTypeCostRow[]): HTMLElement {
  ...
  // A source-type id with no label in this console build becomes a cryptic raw id in the cost table.
  // It is an engine that backs up a source type this console has never heard of, which is version skew,
  // not a rendering bug.
  for (const r of rows) {
