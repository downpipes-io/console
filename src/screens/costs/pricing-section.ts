...
function buildDestinationBlock(host: CostPricingHost, state: PricingState, index: number, rerender: () => void): HTMLElement {
  ...
  // The four rate fields, captured here so applyPreset can refresh their displayed values; scoped to
  // this block only.
  ...
  // drops the refusal, silencing the diagnostic event for a rejected rate entry.
  const priceField = (
    id: string,
  ...
  // does not: a screen reader would announce "Storage" three times with nothing to say which destination
  // it priced, and a locator by accessible name would resolve three controls rather than one.
  //
