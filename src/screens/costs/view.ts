...
// branch ends in a fully usable calculator, and a 401 routes to signed-out. It imports the shared
// leaf (./helpers.ts), the observed-seed derivation (./seed.ts), the results builder (./results.ts),
// the loading skeleton (./loading.ts), the standing assumptions copy (./assumptions.ts), the
// cost-model library and the design-system components.
...
export class CostView {
  ...
  // observedOpCountsPerRun is the EXACT mean per-run Cloudflare op tally from run history,
  ...
  constructor(engine: EngineClient | null) {
    ...
    // A polite live region announces a recomputation without stealing focus.
  ...
  // a fast engine can resolve the first fetch BEFORE attachment; without this latch, checking
  // connection state directly would treat that timing as "navigated away" and leave the
  ...
  private applyInitialMode(result: HistoryLoad): void {
    if (result.kind === "seedable") {
      ...
      // error (non-401): fall back to manual with an inline note, never block.
      this.mode = "manual";
  ...
  private buildMarginControl(): HTMLElement {
    ...
    // Group-level doc link: the range slider takes no placeholder, so the link that explains
