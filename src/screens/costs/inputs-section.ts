...
// per-month ops fields. Each builder takes the CostInputsHost so it can read the active inputs
// and route every edit through the single patchActive mutation path (which clamps and
// recomputes).
...
// CostInputsHost is the slice of the CostView the inputs builders read and mutate, kept narrow
// so each builder depends only on what it needs rather than the whole view.
...
function sourceSizeField(host: CostInputsHost, inputs: Inputs): HTMLElement {
  ...
  // Group-level doc link: the number and its unit select are one control, so the link that
...
// engine's behaviour, the honest default) or cross-run dedup (not current behaviour; the
// projection applies only once it ships). The whole
...
function churnField(host: CostInputsHost, inputs: Inputs): HTMLElement {
  ...
  // Low / expected / high quick-set. These set the churn field to a
