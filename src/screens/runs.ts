// Runs and history (IA screen 3): the fleet-wide activity view, the per-downpipe
...
//     replaceState so a filtered view is bookmarkable and survives a refresh.
//     The ?status= / ?downpipe= deep links the Overview uses (e.g. /runs?status=failed)
//     still work and seed the table's initial state.
...
//     shape carries: records, plaintext bytes, archive bytes written, segments
...
//     so a recoverability trail exists; a missing evidence route never shadows the
//     drill result.
//
...
// logs or stores any secret; the rich detail is counts and sizes only.
//
...
export const runsScreen: Screen = {
  ...
  title: "Runs",
  measure: "wide",
  // The screen contributes its own palette commands: a navigation to
...
// runsActions are the screen-owned palette commands. Deduplicated by id
// against the shell baseline (common.ts allScreenActions, first wins). There is
// deliberately NO "Refresh runs" command: as a navigation, one would wipe the operator's
// ?status/?q/?sort mid-triage; the header Refresh button reloads in place instead.
// Adding one back would need a defaultDispatch case that reaches the live view's
