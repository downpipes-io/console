// Shared pacing for the console's bulk loops: the bulk run/disable/delete path and the fleet-drill loop
// fire one engine request per item. Without these helpers, a 429 storm from the engine's rate limiter
// would read as N unexplained failures: a generic "engine returned an error (...: 429)" with no
// Retry-After, no backoff and no pacing. These helpers let each loop (1) recognise a 429 distinctly, (2)
// back off by the engine's Retry-After (capped) before retrying, and (3) label a 429 failure honestly in
// the summary instead of as a bare "did not pass".
//
// They are deliberately PURE (no DOM, no network) except `sleep`, so the backoff maths and the
// labelling are unit-testable without a real timer.
...
//
// Returning the raw thrown error text would be a defect, not a shortcut. The transport's message is a
// log line, not a sentence: it carries a wire verb on the front, the HTTP status glued past the end, and
// on a 403 a `forbidden-class=` token this console defines for its OWN classifier -- exactly the kind of
// console-internal token that must never surface to a customer, while the engine's own words may. That
// is the rule refusalText enforces, applied here the same as on the destination form, the downpipe editor
// and the source token field. A restore batch row is a harder case than any of those, not an easier one.
