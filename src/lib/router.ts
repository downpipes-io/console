// A small History-API router with the IA's flat route table
// (information-architecture.md 7, build-contract 2.5). Zero dependency. It maps a
...
// the console Worker serves an SPA fallback (any 404 -> index.html,
// current-state.md 1); a hash fallback is kept for the file:// / no-server case.
//
// What the router does NOT do: it never enforces auth or roles. The edge + engine
// enforce auth; the screen and the action registry gate by Caller.role and engine
// state (build-contract 2.5). The router only resolves which screen renders.
...
// There is deliberately no hand-maintained route table here. A route's single source of truth is its
// owning screen's `route:` field (screens/common.ts; app.ts binds via screenRoutes(SCREENS, ...), reading
// each screen module's own field), and, where a caller outside that screen needs the literal, the screen's
// own exported ROUTE_* constant (e.g. keys/shared.ts ROUTE_KEYS). A hand-kept second list would drift from
// what the screens actually declare, so the FULL derived list (for anything that genuinely needs to
// enumerate every bound pattern, e.g. a coverage gate) lives at lib/app-registry.ts's ROUTES export,
// computed FROM the screens rather than typed out beside them. scripts/route-table-gate.mjs asserts the
// two stay in step.
