...
//
// WITHOUT THIS GUARD. Driven on the SERVED bundle in a real Chromium: three well-formed downpipes plus
// ONE malformed row, and the Overview never leaves its first-load skeleton. Not a degraded tile: 2,632
// characters of body text and nine stat tiles become 559 characters and none, because the throw escapes
// an async load with no catch. The screen the operator lands on to find out something is wrong is the
// screen that disappears when something is wrong, and it takes the other nine tiles with it, including
// the ones that would have named the fault.
//
...
// takes the true arm keeps the full type it would have with no guard at all, and NOTHING on the healthy
// path changes shape.
