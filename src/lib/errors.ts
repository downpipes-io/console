// The two-channel error model. This module owns the CLASSIFICATION: it turns a thrown
// transport/auth error or a non-2xx response into a discriminated kind the screens render
// consistently. The rendering itself (inline block error with Retry, the signed-out route, the
// capability-gate copy) lives with the toast / field / banner components and the screens; this is
// the shared brain so no screen conflates the channels.
//
// The two channels:
...
export type ErrorKind =
  ...
  | { kind: "stepup-required" }              // 401 { stepUpRequired }: a step-up ceremony was needed and not satisfied -> re-verify, NOT signed-out
  | { kind: "access-reauth-required" }       // 401 { stepUpRequired, reauth: "access" }: the Access assertion is older than the engine's freshness window -> sign in to Access again, no passkey prompt
  ...
  | { kind: "console-origin"; origin: string } // CORS / CONSOLE_ORIGIN misconfigured
  ...
  | { kind: "engine-binding-absent" }        // THIS console's deploy has no ENGINE service binding: a config fault in the console, not an engine outage
  | { kind: "console-origin-fault" }         // THIS console's own worker manufactured the 500: its dispatch threw, or the ENGINE binding's fetch REJECTED. No request reached any engine
...
export function classifyError(err: unknown, _opts?: unknown): ErrorKind {
  ...
  // THE CONSOLE'S OWN WORKER SAID IT HAS NO ENGINE SERVICE BINDING. Recognised FIRST, because it is the one
  ...
  // redeploy that drops that binding would make those paths fall through to the SPA, so the console would parse its
  // own index.html as engine data and every other classifier below would reach the wrong conclusion (`html-body`, whose
  // remedy is "correct the engine URL", on a URL that is entirely correct). The worker instead answers a 503 carrying
  ...
  // THE CONSOLE'S OWN WORKER MANUFACTURED THE 500. Recognised beside the binding-absent token above and for
  // the same reason: the request never reached an engine, so every classifier below (all of which read the status)
  // would reach a conclusion about an engine that never saw it. Left as a bare 500 it would read as `server`, which
  // says in as many words that the ENGINE ANSWERED AND REFUSED, sending support to read refusals in the logs of a
  ...
  // A 401 the transport tagged with STEPUP_REQUIRED_MARKER is a step-up-verification gap (the
  ...
  if (message.includes(ACCESS_REAUTH_MARKER)) return { kind: "access-reauth-required" };
  ...
  // A 2xx THAT DID NOT PARSE IS NOT "COULD NOT REACH THE ENGINE" (as with a malformed-json body on GET
  // /admin/history). extractStatus deliberately narrows to 400-599 (a 2xx is not an HTTP error status), so
  // client-transport.ts parseJson's throw for a malformed 2xx body -- "<verb>: <status>" with the SUCCESS
  // status still on the end -- would fall through every branch above and reach the network default below,
  // whose copy ("Could not reach the engine", "Check the engine is reachable on its custom domain") is
  // written for a fetch that got no response at all. That reading is backwards here: the request plainly
  // completed and the engine plainly answered, only the body could not be read (most often a
  // version-skewed deploy, or a genuine bug on that one route), so sending the operator to check
  // reachability is misdirection, not merely imprecision. answeredStatus recognises the same shape
  // ownerActionCodeForError already names "answer-unreadable" (client-diag/ring.ts) for the owner-action
  // refusal classifier; the same name is reused here so the two classifiers cannot drift apart on what
  // "the engine answered" means.
  ...
  // not probe health would otherwise be mis-diagnosed as "CONSOLE_ORIGIN is not set".
  //
  ...
  //
  // THE PREMISE BEHIND THIS BRANCH IS TESTED RATHER THAN ASSUMED. "No status in the message means fetch
  // itself threw" is not simply asserted: without the check below, every throw carrying no status would
  // land here, including a TypeError raised by the console's OWN render code long after a perfectly
  // successful response. With every /admin read answering 200 with a well-formed body of the WRONG SHAPE,
  // six routes (/credentials, /security/owner-actions, /access/roles, /access/roles/builder,
  // /config/changes, /runs) would render "Could not reach the engine" with a heading AND a remedy
  // byte-identical to a genuinely aborted fetch, so an operator could not tell an engine that is down from
  ...
  // On this path health necessarily answers, because the engine is up and serving 200s, so a false positive
  // here banks as `origin-rejected`: the pack would tell support to set CONSOLE_ORIGIN on an account whose
...
// STEPUP_REQUIRED_MARKER is the token the transport folds into a thrown message when a 401
...
// ACCESS_REAUTH_MARKER is the step-up variant an Access caller receives: the engine judges an Access
// caller on the age of its Access assertion, and one older than the freshness window is refused with
// { stepUpRequired: true, reauth: "access" }. There is no passkey to assert with on an Access session, so the
// transport must NOT run the passkey ceremony on this body; it folds this marker instead and the error view tells
// the operator to sign in to Access again. Checked before the plain step-up marker, since the body carries both.
export const ACCESS_REAUTH_MARKER = "access-reauth-required";

// detectAccessReauthBody recognises the Access form of the step-up 401: stepUpRequired:true AND reauth:"access".
// A shape gate like detectStepUpRequiredBody; nothing from the body rides.
export function detectAccessReauthBody(bodyText: string): boolean {
  try {
    const b = JSON.parse(bodyText) as { stepUpRequired?: unknown; reauth?: unknown } | null;
    return typeof b === "object" && b !== null && b.stepUpRequired === true && b.reauth === "access";
  } catch {
    return false;
  }
}
...
// ENGINE_BINDING_ABSENT is ONE frozen product token doing two jobs, deliberately: it is the `error` code
...
// ENGINE_BINDING_ABSENT_HEADER is the SAME fact as the 503 body above, carried where a seam that must not
...
// CONSOLE_ORIGIN_FAULT_HEADER carries the SAME discipline one status code over: the 503 guard above covers
// a missing binding, and this covers the console's other self-manufactured failure.
//
...
// sees only the numeric status, so without this header it would write {kind: engine-call, httpClass: "5xx",
// faultClass: "server"}, and a classifier reading that would map it to `engine-not-ok`, whose meaning is:
//
...
//
// That would tell a support engineer the engine is up and refusing when it is in fact down or absent, sending
// them to logs that hold no trace of the request: a row asserting a fact the code never established, which does
// not merely fail to save time but actively costs it.
//
...
// ---- the 403 SHAPE GATE ---------------------------------------------------------------------------
//
// A BARE 403 IS NOT AN AUTHORITY FACT. At least four different things answer 403 to the same console call,
// and the transport throws the identical "<verb>: 403" for all of them, so every screen and every
// diagnostics row that keys on the status alone would assert a fact nothing established:
//
...
// ceremony surfaced. A gated screen checks this BEFORE isUnauthorised so a step-up gap does not
...
// (client-transport.ts). Nothing in the error carries which of them happened, so a single merged message
// cannot honestly cover every case: telling an operator whose device has no passkey enrolled to "approve
// the passkey prompt when it appears" is wrong when no prompt will ever appear.
//
...
// (a token or Access session never runs a ceremony at all) and keeps the general advice. browser-refused
// and check-unavailable are kept distinct from no-assertion because a bare catch merging them into it
// would make the copy below say something untrue about each: see stepUpFailureMessage for the sentence
// each state gets and why the distinction matters.
export type StepUpFailure = "begin-refused" | "no-passkey" | "no-assertion" | "finish-refused" | "browser-refused" | "check-unavailable";
...
// nothing was written.
//
// A MERGED SENTENCE HERE WOULD DO MORE THAN MISDESCRIBE THE STATE, IT WOULD PRESCRIBE THE ACTION THAT
// DEEPENS THE HARM. Chromium refuses an assertion whose allowCredentials list exceeds 64, throwing a
// DOMException named `RangeError` before it opens any prompt, and the engine can emit an account's entire
// credential list into that field. So for an account over the ceiling no prompt appears; if that were read
// as `no-assertion` ("this device has no passkey enrolled, enrol one from Security"), following the advice
// would take the account from 65 credentials to 66. The count only rises, and the way back down is a
// credential delete that is itself step-up gated: the advice would be the trap.
//
// The engine bounds that list, which keeps new accounts off the ceiling. This state is the other half: when
// a browser refuses a ceremony, for this reason or one nobody has met yet, the console must not attribute
// it to a device with no passkey.
export function stepUpFailureMessage(reason: StepUpFailure): string {
  switch (reason) {
    ...
    case "no-passkey":
      // An OIDC or SAML session asserts a passkey for every gated action, and this account holds none.
      // A retry cannot succeed; enrolment can. The sign-in screen is where a passkey is enrolled.
      return "This action needs a passkey assertion and no passkey is enrolled for your account, so nothing was changed. Your session is still active. Enrol a passkey from the sign-in screen (sign in with a recovery code, or ask an owner for an invite), then try again.";
    case "finish-refused":
...
// rejection to a closed faultClass: the message is read to find a 3-digit status and is never
...
// THE CALL SUCCEEDED, and only the body could not be read. The transport composes exactly that throw: on a
