// The guided first-run's brain (the setup-first IA): steps must be done in order, and each one done
// right before the next unlocks.
//
...
// the owner asked for. Once the destination is verified, Sources unlocks, then Downpipes.
//
...
export function deriveSetup(s: SetupState): SetupView {
  ...
  const steps: SetupStepView[] = [
    ...
    },
    {
      ...
      // The mainline is the no-terminal attach on the same screen: tick, paste a one-shot token, press
      // Attach now (screens/sources/account.ts buildAttachPanel). Deploying it yourself still exists and is
      // still supported, one disclosure down, for a self-hoster who would rather not hand over a token. This
      // line sits on the Overview hero, so it must not tell a new customer the product needs a terminal that
      // it does not.
...
export async function fetchSetup(engine: EngineClient): Promise<SetupView | null> {
  try {
    ...
    // This fail-open is correct, but leaving it silent would be a bug of its own. "Unknown" means no gating,
    // so a consolidated-facts read that is quietly 5xx-ing leaves the guided-setup strip absent and lets the
    // Overview say nothing is configured on an engine that is fully configured, reading to the operator as
    // "the console has forgotten their setup" while a support pack built from a healthy read shows nothing
    // wrong.
    //
...
const ALWAYS_ALLOWED = new Set<string>([
  ...
  // Settings and Keys. Without this exemption, an estate with zero downpipes could
  // never reach it, deep link or rail click, for the estate's entire life.
  "/licence",
...
// current step. Everything else waits (the forcing the owner asked for).
...
export function setupCelebrated(): boolean {
  try {
    ...
    // seen the celebration, the honest choice, and recording it here is what keeps this failure visible.
