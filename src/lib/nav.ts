...
// IS the navigation: a tablist whose sections each own a route (the Keys sections, the Notifications
// areas) re-renders the whole screen on an arrow key, and the shell's focus move then lands on <main>
// instead of the newly selected tab. The tablist's own restore cannot win that race, because a view
// transition runs the shell's swap-and-focus in a LATER task than the render that scheduled it, and a
// focus() call on a node that is not mounted yet does nothing at all.
//
// Without this: one ArrowRight moves aria-selected and the roving tabindex correctly and leaves focus on
// MAIN, so every further arrow does nothing and a keyboard user gets exactly one tab move per manual
// re-focus.
//
...
// PostNavigationFocus is the reader's RESULT, and the reason it is a discriminated triple rather than a
// bare nullable element is the support pack: a bare element cannot tell `none` (no screen declared
// anything, the ordinary route change, the shell focuses <main> and that is correct) apart from `detached`
// (a screen DID declare where focus belongs and the element was not mounted when the shell read it, so
// focus falls to <main> and every further arrow key does nothing). Both would answer null, and the caller
// could not tell them apart, so neither could anything downstream of it. `state` is that discrimination.
...
// reload, a tab close, back/forward through browser chrome) would otherwise bypass every one of the
...
// PRESERVED rather than reset to "/". This matters because routeToSignIn can run more than once in quick
// succession -- every screen's own load-catch calls goSignedOut() independently on its own 401, and a
// session that has genuinely died answers several concurrent admin reads with 401 at once, which a reload
// or a detour through a screen with several settled reads both produce -- so whichever caller's read
// settles LAST must not clobber the FIRST caller's correct capture of the real destination with the bare
// "/" default. Because the redirect is a REPLACE navigation, the later call would otherwise win and reset
// to "/" the instant the address bar had already become /passkey, discarding the very next= the first call
// had just, correctly, written. A genuine first landing on /passkey or /signed-out with no next= (or an
// unsafe one) still defaults to "/", unchanged: the same-origin guard (a bare in-app path, never "//", a
// protocol-relative off-site redirect) mirrors the one app.ts's onAuthenticated already applies when
// CONSUMING next= after a successful sign-in.
