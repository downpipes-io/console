// In-progress form/selection drafts that survive a navigation hop or a refresh: the console's
// navigation model requires that it never silently discard the operator's work. A thin typed
// wrapper over sessionStorage, scoped per engine host so two engines in two tabs never cross-fill.
//
...
import { DRAFT_PREFIX } from "./storage-keys.ts";
...
export function saveDraft<T>(id: string, value: T): void {
  try {
    ...
    // sessionStorage unavailable or full: the draft simply does not survive the hop. This swallow can present
    // as "the console keeps losing my half-filled wizard", and the quota arm and the policy arm of it are
