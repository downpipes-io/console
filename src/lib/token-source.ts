...
// TOKEN_SOURCE_META carries the label + one-line summary per token source. The summaries state the
// honest claims plainly: cf-config is zone & account settings, token-based with no binding; workers is
// Worker scripts, token-based with no binding, and restore is reprovision (you re-deploy the script),
// never a one-click in-console write, so the copy can never overclaim.
...
// the picker falls back to the four bindings rather than erroring.
...
//
// WHY PER-CAPABILITY AND NOT ALL-OR-NOTHING. An engine build can advertise a strict SUBSET of the four
// capability flags rather than all four together, and a subset is not zero: an engine that offers cf-config
// but not Workers is byte-identical in the pack to a healthy, current engine unless the missing capability is
// checked on its own. A rollback to an older build can land on exactly this state. The missing capability is
// the discriminator, and because fieldFamily is part of the ring's tuple key, "cf-config only", "cf-config +
// workers" and "none at all" are three distinguishable pack readings.
//
// THE tokenPresent GATE. Account discovery is OPT-IN, so tokenPresent false is the DEFAULT, and the engine's
// no-token branch returns none of the capability flags either. Reading the capability flags alone, without
// checking tokenPresent first, would call a healthy, current, correctly-configured engine "version skew" for
// every customer who has simply never stored a read-only Cloudflare token. The no-token state needs no row from
// the browser: the engine records it itself as a no-token discovery observation, and the screen tells the
// operator so.
//
...
// sends it; a response that omits it altogether is an engine that predates the account tier entirely, the
// OLDEST and most skewed engine there is. A gate that merely tests `!== true` would swallow the undefined and
// read it exactly like the healthy no-token default, so this field gets its own explicit check. The family
// asserts only what the code established, that the response carried no account-tier field, and never that a
// token exists (on this engine the console cannot know). The wire is read defensively because the wire is
// untrusted: the response is a bare JSON cast, so the declared `boolean` is a promise about a current engine,
