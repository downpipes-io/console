...
// actually finds in their bucket after total account loss is `.sealed.json`, not `.json`. Nothing else in the
// console can open it: `control-plane-recovery.ts`'s `looksLikeControlPlaneExport` gates on five
// plaintext-only fields, so pasting the default artefact would refuse with DP-R03, leaving only the offline
// `downpipe unseal-export` CLI -- a terminal, against the standing rule that customers never run one.
//
...
// key: a confirmation oracle that this field itself introduces. Accepted, not closed (closing it
...
// narrow, stated residual risk, never a zero-knowledge claim. Full accounting is in the field's own doc
// comment, engine/src/admin/control-plane-seal.ts's SealedControlPlaneExport.bodyHash.
...
//
// THE OP IS A PARAMETER, not a constant. Both the estate-import flow and the sealed control-plane reconcile
// run the IDENTICAL browser-side unseal for a different disaster. If the op were hardcoded, every
// client-side refusal recorded here would be filed against the flow the operator was NOT using, and a
// support pack would say an estate import refused when no estate import happened. It defaults to
// "estate-import-sealed" so the existing caller needs no change.
