...
export const CLIENT_DIAG_KINDS = [
  ...
  // operator who simply chose the default never produces this row, so the two states stay distinguishable
  ...
  // recovery ceremony, and did not get. A kind of its OWN, and the reason is the whole point of the gap:
  // recording the refusal as `unhandled` would run it through the faultClass mapper that answers `other` for a
  // synthetic throw, so a REFUSED identity.key download and an unrelated null-deref on the same screen would
  // produce the SAME row, byte for byte, and on /access a refused recovery-codes download, a dead clipboard copy
  // and a console bug would all coalesce into one row. `unhandled` is defined to mean a console
  ...
  "capability-fault",
  // console-build-check -- WHAT BUILD THE BROWSER IS ACTUALLY BEING SERVED. The engine cannot probe a
  ...
  "console-build-check",
  // console-rollback -- the operator pressed the post-apply console rollback. Its EXISTENCE is the
  ...
  "console-rollback",
  // identity-unresolved -- the caller's identity report (/who) did NOT resolve, so caller() is null and
  ...
  "identity-unresolved",
  // restore-gate-blocked -- the console did NOT offer Apply on a restore plan, and this says WHY. Without
  // gateBlockClass the four preconditions of the client apply gate are indistinguishable, in the tab and in the
  // pack: no approval yet, no attributable caller and the client could not compute the hash it matches on all
  // render the same unarmed panel and record nothing at all. gateBlockClass separates them. The LEGITIMATE
  ...
  // wire-anomaly -- an engine-supplied wire value that the console could not use: it did not parse, it
  ...
  "wire-anomaly",
  // transport-fault -- HOW the transport failed, in the vocabulary the console's OWN
  ...
  "transport-fault",
  // read-degraded -- a FIRE-AND-FORGET engine READ failed and its failure was swallowed into a
  ...
  "read-degraded",
  // onboarding-step -- a step of the setup wizard ended, and this says which step and how. The wizard is
  ...
  "onboarding-step",
  // discovery-connect -- what the account-discovery token actually SAW. This closes the console half of
  ...
  "discovery-connect",
  // claim-exchange -- the licence claim-code exchange with the vendor control plane. This one is not a
  ...
  // no token at all.
  "claim-exchange",
  // admin-write -- a PRIVILEGED CONSOLE WRITE and how it ended. Its shape is always
  ...
  "admin-write",
  // recovery-refusal -- a DISASTER-RECOVERY refusal: a control-plane reconcile, an estate import
  ...
  "recovery-refusal",
  // identity-stale-gate -- A CAPABILITY-GATED SCREEN RENDERED WHILE THE CALLER IDENTITY WAS UNRESOLVED.
  ...
  "identity-stale-gate",
  // update-channel-unverified -- THE ENGINE CONSULTED THE UPDATE CHANNEL AND THE VERDICT WAS NOT VERIFIED.
  ...
  "contract-skew",
  // fleet-drill -- ONE FLEET-DRILL SESSION, as the CONSOLE assembled it. The engine records each drill it
  ...
  "fleet-drill",
  // owner-action-refusal -- a DUAL-CONTROL owner action the approve/reject did not go through on. The
  ...
  // role-delete-impact -- a custom role was DELETED, and this is how many grants still referenced it at
  ...
  "role-delete-impact",
  // csp-violation -- the browser BLOCKED something the page asked for, under the console's own strict
  ...
  "csp-violation",
  // input-dropped -- the console SILENTLY DISCARDED part of what the operator pasted, before any request
  ...
  "input-dropped",
  // handoff-dropped -- a WIZARD OR DEEP-LINK HAND-OFF lost the operator's earlier pick between steps. The
  ...
  "handoff-dropped",
  // ceremony-step -- a BROWSER-SIDE KEY-CEREMONY OR CREDENTIAL-ENROLMENT STEP and how it ended. This is
...
export const CLIENT_DIAG_SCREENS = [
  ...
  "keys",
  "security",
  // config-changes / owner-actions -- THE TWO APPROVAL INBOXES, kept apart from the `security` bucket that
  // PATTERN_PREFIXES would otherwise map them into (it maps "/config" and "/security" alike). Without their own
  // screen ids, a render throw inside a .then with no .catch -- nearly always a TypeError -- would give the two
  // screens the byte-identical tuple {unhandled, security, other, unhandled-rejection, TypeError} and COALESCE
  // them into one row with a bumped count. They are two different screens with two different remedies, and a
  // shared tuple cannot tell support which one the customer was looking at. A screen id is a route TEMPLATE and
  // a product constant, so this costs no custody.
...
// driftClass -- class ONLY, never the offending value or the field name (4). The console records
...
//
// selection-dropped is not a failure of the batch at all, it is a failure of the batch's INPUT: rows the
...
// bulkAction -- bulk-outcome only. WHICH bulk operation the row is about. Without it every bulk loop
...
// differently on each: without this a clean apply records nothing, and "recorded nothing" is also what a
// restore that never ran looks like:
//
...
//                         without its own member it would be indistinguishable from a clean full apply and from
//                         no apply at all.
...
//                    at all, so there is nothing to save and no future backup could ever be recovered.
...
//   map-diagnostics      the topology map's Copy-view-diagnostics block. The block is the console's own
...
//               browser as a broken one, and folding it into `unhandled` would inflate the console-DEFECT
//               counter on every Copy press on a plain-http host.
...
// bootClass -- boot-fault only. WHICH bring-up step failed. A boot-fault carrying only a faultClass
...
// buildCheckClass -- console-build-check only. What the ORIGIN was serving when the console asked it
...
//                 FACT ABOUT A DIFFERENT ARTEFACT from `unstamped`, kept as its own member so the two never
//                 coalesce into one row. console-version.ts's own header calls them "two distinct questions,
//                 deliberately separated" (what THIS RUNNING CODE is, versus what THE ORIGIN SERVES RIGHT NOW):
//                 folded together, {console-build-check, updates, unstamped} would be emitted both by the
//                 licence screen for an unstamped RUNNING bundle and by the post-apply check for an unstamped
//                 ORIGIN read, and support could not tell "the update verdict was computed blind
...
// rollbackClass -- console-rollback only. How the console rollback ended, in the engine's OWN
...
// gateBlockClass -- restore-gate-blocked only. WHY the console kept Apply disabled on a restore plan
// whose approval the operator believes is in place. Each member is a DIFFERENT ticket and a different remedy;
// without this class all four render the same indistinguishable "not approved yet" panel:
//
...
// fieldClass -- wire-anomaly only. WHICH CLASS of engine-supplied field was unusable. It is a class,
...
//
// Four of the classes below name a value the console COARSENS rather than crashes on. Each is a value the
// console cannot interpret that then renders as a plausible-looking verdict, which is worse than an error: an
// unrecognised whoami method reads as the break-glass TOKEN FALLBACK posture (a false claim that the
// customer's security posture is degraded), an unknown downpipe status becomes "unknown" on the map, an
// unparseable lastRunAt becomes a "-" beside runs that plainly exist, and a non-finite byte figure becomes a 0.
export const CLIENT_DIAG_FIELD_CLASSES = [
  ...
  "b64url-id",
  "run-id-missing",
  "auth-method", // trust-chips: the whoami method this console build does not know
  "status-enum", // topology/tiles: a downpipe status enum this console build does not know
  "source-kind", // topology: a source kind this console build cannot draw
  "bytes", // topology/tiles: a byte figure that was not a finite number
  "duration", // reports: an RTO/duration in seconds that was not a finite, non-negative number
  // cadence: the downpipe config's cadenceSeconds, read by the ONE freshness seam (map.ts
  ...
  "cadence",
  // The THREE PROTECTION-STATEMENT recency stamps. A generic `timestamp` row cannot close this gap and
  ...
  "restore-proven-at",
  // blackout-minute: the stored blackout WINDOW bound, in minutes since midnight. minutesToTime CLAMPS an
  ...
  "blackout-minute",
  // The LICENCE and ESTATE malformations. Each one silently DISARMS a cue the customer is relying on, and
  ...
  "estate-figure",
  "artefact-sha",
  // The DRILL-EVIDENCE stamp. The Overview picks the newest drill evidence by parsing recordedAt, and an
...
// anomaly -- wire-anomaly only. HOW the value was wrong: it did not parse, it was not a finite
// number, it was negative where only a count is meaningful, or it was absent where the screen needs it.
//
// A fifth member, `render-threw`, would name the seal instant that takes the whole run drawer down when it
// will not convert to a date (one run row opens to nothing while every other row is fine). That site is
// GUARDED rather than crashing: the drawer renders the rest of the run and states the check honestly as
...
// out-of-range: the value PARSED, is finite and is not negative, and still lies outside the range the
...
// errorClass -- unhandled / boot-fault. The JS ERROR CLASS of a fault nobody caught, chosen by SET
...
// faultSource -- unhandled only. WHICH CHANNEL the uncaught fault arrived on, which is what tells the
...
// transportClass -- transport-fault only. WHAT the transport failure actually WAS, in the
...
//   engine-binding-absent THIS CONSOLE'S OWN DEPLOY HAS NO ENGINE SERVICE BINDING. On the default
//                       single-hostname topology the console worker proxies /admin, /support and /metrics to the
//                       engine over a service binding; without that binding those paths would fall through to
//                       the static assets, so the SPA's own index.html would be served at HTTP 200 where engine
//                       JSON was expected, telling three different observers something false: the console would
//                       classify its own shell as `html-not-engine`, whose remedy is "correct the engine URL"
//                       when the URL is correct; a Prometheus scrape of /metrics would get HTML with a 200, so
//                       the customer's monitoring would report health while nothing was being backed up; and
//                       the engine, which never received one of these requests, would appear
...
//                       worker instead answers its own 503 with a frozen code, and this class
...
//                       THE BOUND ON IT: the ring rides to the engine ONLY on the customer's
...
export const CLIENT_DIAG_TRANSPORT_CLASSES = [
  ...
  // console-origin-fault: THE CONSOLE'S OWN WORKER MANUFACTURED THE 500. Its last-resort handler answers a
...
// callClass -- read-degraded only. WHICH swallowed read went quiet. Each one drives a specific
...
//
// The gap proposed a sixth member, `downpipe-count`, for the count the command palette gates its
...
// obStep -- onboarding-step only. WHICH step of the setup wizard. They share one route, so `screen`
...
// and no key material exists. Folding its failure into {install, transport-error} would misrepresent it: an
// engine that ANSWERED 500 or 401 to the probe would write the row whose own definition below says the engine
// "never saw it, so no engine-side evidence of this can ever exist", and it would COALESCE
// with the genuine install-POST row from steps.ts, so the install row could not be trusted to mean an
// install had been attempted. Its own step earns its place: when this read fails the wizard tells
...
// obOutcome -- onboarding-step only. HOW the step ended. `ok` is a member and is recorded, so the
...
//                             of the call can be joined to it. It must not be read as meaning "an outage": a
//                             lapsed Cloudflare Access session and a console with no ENGINE binding are NOT
//                             outages either, and each has its own member (`unauthorised` and
//                             `engine-binding-absent`), because in both of them the engine is up and healthy and
//                             a `transport-error` row here would send support to go and revive it.
...
//
//                             ITS REACH IN A PACK IS BOUNDED, AND THE BOUND IS HONEST. The only vehicle for
...
export const CLIENT_DIAG_ONBOARDING_OUTCOMES = [
  ...
  "poll-refused",
  "engine-binding-absent",
  // console-origin-fault: the poll's ticks were answered by THIS CONSOLE'S OWN WORKER with a 500 it
...
// obSecret -- onboarding-step only. WHICH KEY THE ENGINE KEPT REPORTING ABSENT when the readiness
...
// channelReasonClass -- update-channel-unverified only. WHY the engine's update-channel verdict came
...
//
// The two members below name the MISCONFIGURED channel. Gating the emit on the engine's `configured` flag
// would conflate the engine's VERDICT with the operator's INTENT: the engine answers configured:false for an
// unparseable UPDATE_CHANNEL_URL, a non-https one and an UPDATE_SIGNER_PUBLIC that will not parse -- three
// states in which both env vars are set and the operator plainly meant to have updates. So a truncated or
// whitespace-mangled paste of the signer key (a base64url blob the operator pastes by hand, and one of the
// likeliest ways a channel silently freezes) would produce ZERO rows and a pack byte-identical to a healthy
// verified channel. The emit is keyed on channelIntended instead.
//
...
// discoveryOutcome -- discovery-connect only. WHAT THE TOKEN SAW. The most common onboarding ticket
...
// claimResult -- claim-exchange only. HOW the licence claim-code exchange with the vendor control
...
// adminOp -- admin-write only. WHICH privileged write the console attempted. Every
...
//
//   update-*        the safe-apply lifecycle. A PRE-FLOW refusal never reaches the engine's update
...
//   role-* / group-role-* / custom-role-* / passkey-revoke / terminate-*  the access-control levers. The
...
//   recovery-codes-regenerate  the worst single case in the audit: regenerate errored, and now neither
//                   the old codes nor the new ones work. Its success is recorded for the same reason.
//   recovery-codes-confirm  the save-confirm panel's own
...
//   approval-policy-set / change-number-policy-set / signin-context-set / posture-*  the governance
//                   toggles. "I turned dual control on last week" and a pack that says it is off.
//   notify-* / expiry-* / idp-connection-* / credential-*  the remaining unaudited admin writes.
//   source-attach / source-detach / source-reattach  the binding safety harness. The owner's stated
...
export const CLIENT_DIAG_ADMIN_OPS = [
  ...
  // rollover, not the connection edit they never attempted. This list is a
  ...
  // diagnostic row rather than the one field, so this list and the engine's must never drift apart.
  ...
  // The DUAL-CONTROL INBOX ops. They name which owner-action route a refusal fell on, and the two are not
  ...
  "owner-action-reject",
  // The OFFBOARDING IdP-CLEANUP ATTESTATION. The operator ticks "I have removed this person in our IdP",
...
// ownerActionCode -- owner-action-refusal only. WHY a dual-control approve or reject did not go
...
//
// THE AUTHORITY AXIS IS TESTED BEFORE THE LIFECYCLE, AND THAT ORDER MATTERS. Reading the fate off the inbox
// FIRST would be wrong, because the inbox listing is CALLER-SCOPED: the engine's canRequesterSeeOwnerAction
// shows an owner every record and shows everyone else only their OWN proposals. A non-owner's re-read
// therefore CANNOT contain somebody else's action, so "gone from the listing" is true BY CONSTRUCTION for
// them, and reading the fate first would file a plain permissions refusal as `already-decided` -- "a second
// owner rejected or ran it, go and read the audit" -- for an action that is still pending and that nobody has
// decided. The demoted owner is the live case: the inbox repaints only when the (id, status) signature
// changes, so their card stays on screen and their Approve still fires. The fate is therefore consulted ONLY
// for a caller whose listing is COMPLETE (an owner).
//
// AND A BARE 403 IS NOT AN AUTHORITY FACT: THE BODY SHAPE DECIDES. A 403 on these routes does not by itself
// mean "you may not approve": FOUR things can answer 403 to this one call, three of them are not about the
// caller's authority at all, and the transport discards the body, so treating all four as one throw would
// record all four as not-owner: the capability gate ({error:forbidden, required, have}), the DO's
// anti-enumeration AuthError funnel (a BARE {error:forbidden}, whose live producer here is the PROPOSER having
// lost owner), the CSRF-origin check (which 403s EVERY cookie-borne save in the console when a deploy drops
// CONSOLE_ORIGIN), and a Cloudflare WAF block page the engine never saw. Three opposite remedies would wear
// one label. The shape gate instead returns a closed ForbiddenClass, and the four are four separate members.
export const CLIENT_DIAG_OWNER_ACTION_CODES = [
  ...
  // 400, so the status distinguishes nothing here.
  "expired",
  ...
  // one belongs here rather than in the residual, because the fate reads the status the engine handed back, not
  ...
  // email); recording it as the residual instead would make it byte-identical to an owner whose approve the
  // engine turned down for a reason nobody can see. The remedy is specific and it is not "look at the engine":
  // sign in with an attributable identity (Access or a passkey) and approve as that person.
  "bare-token",
  ...
  // re-resolves when it replays the proposer's action (scheduler-do-dual-control), and the operator
...
// cspDirective -- csp-violation only. WHICH CSP directive blocked the resource, read off the browser's
...
// cspBlocked -- csp-violation only. WHAT CLASS of resource was blocked, derived from the report's
...
// cspInlineOrigin -- csp-violation only. WHICH inline script was blocked: the console's OWN sanctioned
...
// deleteFate -- role-delete-impact only. WHETHER the delete the count belongs to actually HAPPENED.
...
// dropSurface -- input-dropped only. WHICH paste lost part of itself.
...
// dropFact -- input-dropped only. WHICH of the paste's two numbers this row carries; `count` carries
...
// handoffClass -- handoff-dropped only. WHICH hand-off lost its intent between the click and the form,
// and, for the two members below that end in a broken create, WHETHER THE ENGINE EVER HEARD ABOUT IT.
//
// Only a hand-off with a live producer belongs here: a member no console call site can ever emit is dead
// vocabulary, and a pack section that can never be honestly populated is worse than an honest absence, since
// it tells a support engineer the evidence was looked for and not found. The wizard's own account and zone
// pickers cannot lose a pick between steps (each discovered-row apply() writes the account id in the same
// block as the binding, and Continue stays disabled until a row is picked), and no in-app route builds a link
// naming a zone or an account the discovery catalogue does not carry, so no member exists for either.
//
// `wizard-spec-account-absent` and `editor-refused-account-absent` are two separate members, not one, because
// they are opposite tickets: the wizard SENDS an accountless spec to the engine (there is a real 400 in the
// customer's engine logs to find), while the advanced editor REFUSES THE SAVE LOCALLY and makes no request at
// all (there is nothing in any log, anywhere). One class for both would bury whichever is rarer under the
// commoner one, and the remedies differ: validate the inbound link, versus give the cf-config create path an
// account picker. Naming WHICH tells a support engineer whether the engine ever heard about it.
...
// ceremonyStep -- ceremony-step only. WHICH browser-side ceremony step ran. Each is a step whose
...
// ceremonyOutcome -- ceremony-step only. `ok` is recorded as well as `failed`, and it must be: the
...
// ceremonyFault -- ceremony-step only. The COARSE class of what went wrong, chosen from the thrown
...
export const ADMIN_OPS_RECORDING_SUCCESS: ReadonlySet<ClientDiagAdminOp> = new Set<ClientDiagAdminOp>([
  // "Sign out everyone errored; are the sessions actually dead?" is the single hardest case among the
  // access-control levers to reconstruct, and it is answerable only if the applied ending is on the record too.
  "terminate-user-sessions",
  "terminate-other-sessions",
  "terminate-session",
  "terminate-all-sessions",
  ...
  "update-rollback",
  // "The console said IdP removal was recorded for a departed employee, but the audit export has no entry."
  // This is the one op whose whole purpose is to PROVE a control was in force, so an absent `applied` row is the
  // proof's absence, and it is the answer the compliance review is actually asking for. The toast tells the
  // truth about whether the attestation write landed, and the ring carries both endings.
  "idp-cleanup-attest",
]);

// recoveryOp -- recovery-refusal only. WHICH disaster-recovery flow was refused. The four share
...
//
// cp-acknowledge is the ACKNOWLEDGE-ONLY latch clear, and it is its own op rather than a re-use of
...
// recoveryCode -- recovery-refusal only. The frozen DP-R refusal codes, MIRRORED from
...
// reads, which must survive an engine too dead to build a pack at all; this is the
...
//
// DP-R15/R16/R17 are three specific refusals split out of DP-R12's residual: without them, "wrong token
// vs signature vs shape vs no-custody vs not-Owner" collapses three of its five states
// into ONE row that coalesces on the tuple key. The console cannot subdivide a 400 by reading the engine's prose
// (the prose can name a bucket or a key, and a classifier over a sentence the console does not own mislabels the
// moment the engine rewords itself), so the ENGINE returns a CLOSED `refusalClass` member in its 400 body
...
//
// DP-R18/R19 then SPLIT DP-R15, because a "signature did not verify" is not one state and the remedies
...
//   DP-R15  means what it says: NEITHER half verified. The wrong key, or an altered export. Tamper lives
...
export const CLIENT_DIAG_RECOVERY_CODES = [
  ...
  // 20/21 above are ALSO reachable straight from the browser (the sealed wrapper's signature is verified
  // locally, byte-identical to the engine's own check, before anything is decrypted); these seven are
  ...
  "DP-R29",
  "DP-R30",
  // The acknowledge-only latch clear's three refusals. Mirrored from
...
// The record shape. Every string field is one of the closed unions above; every numeric
...
// intentClass -- intent-dropped only. WHICH half-filled restore option the request builder threw
...
// probeSurface -- probe-outcome only. WHICH operator-initiated test ran. The screen cannot stand in
...
// probeOutcome -- probe-outcome only. HOW the test ended, in ONE union whose members are surface-
...
//                           test that ran and failed, and the two must not share one red toast.
...
// formField -- form-rejected only. The FIELD CATALOGUE control id the console's own validator acted
...
export const CLIENT_DIAG_FORM_FIELDS = [
  ...
  "secretName",
  "dest-endpoint",
  // NOT IN THIS LIST: dest-r2-bucket, dest-account-id, dest-s3-bucket, dest-region,
  ...
  //
  // dp-bucket is answered from add-source and the destination form, and also from the downpipe editor, which
  // is where a downpipe's bucket override is actually edited. dp-sched-cron and dp-sched-tz are on the console-
  // wide funnel too (editor-schedule-fields.ts passes validateCronExpr as dp-sched-cron's `validate`, and the
  // timezone refusal comes from a direct setError() on the live preview).
  //
  ...
  // so field()'s validate() hits the required-and-empty branch first and returns before the validator ever
  // runs. Their sole possible refusal is therefore the one refusal the funnel deliberately does not record (an
  ...
  // engineer the evidence was looked for and not found.
  //
  // THE STRUCTURAL RULE: a field earns a member here only if it has a validator capable of refusing a
  // NON-EMPTY value. components/field.ts records a refusal only when there was a value to refuse, so a
  // validator whose every branch is an emptiness test (`v.length >= 1 ? null : "..."`, including a cross-field
  // emptiness test such as `scopeKindField.value() === "downpipe" && v.length === 0`) can never actually
  // produce this row, whatever member it is given: `rejectOutcome: "rejected"` would then assert that the
  // console's validator turned the operator away from THE VALUE THEY TYPED, when the code examined no value at
  // all, sending a support engineer to investigate validation on an attempt that never happened. A refuse()
  // call carries the same rule: it records only when there is a value to refuse, and a console rule that fires
  // on an empty box (a confidential-client secret, a binding a binding-backed source type needs) uses setError
  // instead, which records nothing and claims nothing -- it is the required-and-empty state under another name,
  // with the operator already looking straight at the message.
  //
  // The dead-vocabulary gate that enforces this decides by MEANING, not by the spelling of the validator: a
  // validator whose every use of its argument is an emptiness test cannot refuse the value the operator typed,
  // whatever else it inspects, and the gate confirms this by probing the validator over a set of non-empty
  // strings. Give a control a real client-side rule (a shape, a length bound, a range) and its member belongs
  // in this list; until then it is left out.
  "channel-addresses",
  "channel-url",
  "cost-cadence-custom",
  "cost-churn",
...
// rejectOutcome -- form-rejected only. The two are NOT the same ticket and must never be one row.
//
...
// catalogueClass -- catalogue-degraded only. WHY the Cloudflare-configuration offer was withheld or
...
//                             Without the members below, every one of these (a lapsed Access session, a 401,
//                             an HTML page at the engine's address, the console's own 500, a 403, a 429 and a
//                             dropped connection) would collapse into this one row, and the first two of those
...
//                             Access answers a lapsed session with its LOGIN PAGE, so recording it here would
//                             write a fault against a healthy engine and a healthy token, on the very
...
//                             THE STATE THAT MOST NEEDS RECORDING, and a bare success check records nothing at
//                             all. Rediscover
...
//                             address. Folding it into cf-rediscover-denied would send support to fix
...
//                             token and not the network. Filing it as cf-rediscover-transport would say
//                             the call got NO answer, when it got one, from the wrong system.
...
//                             and Access answers a lapsed session with its LOGIN PAGE, not a 401, so recording it
//                             here would write "the network dropped it" against a perfectly healthy
...
// featureClass -- feature-probe only. WHICH route family the console could not read and then made a
...
// A member with no console call site to record it is dead vocabulary: a pack section that can never be
// populated is worse than an honest absence, because it tells a support engineer the evidence was looked for
// and not found.
...
// featureOutcome -- feature-probe only. THE VERDICT THE CONSOLE REACHED, which is the field the whole
...
//                           CONSOLE'S OWN 500 (below): folding that in on the bare status would invert the
//                           member's meaning, reporting an engine that is absent or down as up and refusing.
...
//                           IT HAS A SECOND PRODUCER: the console's own worker is not
...
//                           split matters: carrying both under one member would tell support the remedy is
//                           never the engine's address, which is false in exactly the case where the
...
//                           are what make that claim true; without them this member would absorb both of them.
...
//                           is a first-class transport throw (HTML_BODY_MARKER); filing it as `network` instead
//                           would send support to the wrong system. It reaches this ring on a
//                           NON-2xx as well as a 200 (client-transport failResponse): the commonest wrong-address
//                           shape of all is a static host 404ing an HTML page, and reading that as a bare
//                           404 would make it byte-identical to `route-absent` -- a REAL engine declining a route
//                           it does not serve. Those two have opposite remedies, so they must not share a row.
...
//                           service binding and answered the call itself. It is a console deploy fault.
...
// govGate -- gov-gate only. WHICH client-side governance gate fired. Both are invisible to the engine
...
// skewClass -- console-skew only. THE RUNNING BUNDLE AGAINST WHAT THE ORIGIN SERVES RIGHT NOW.
//
// THIS AXIS IS DELIBERATELY NOT A COMPARISON OF THE CONSOLE'S VERSION AGAINST THE ENGINE'S. Comparing the
// console's baked version against the ENGINE's `currentVersion` would compare two INDEPENDENTLY INCREMENTED
// numbers from two separate repos -- the engine says so itself ("the engine knows its own running version, but
// it CANNOT know the console's") -- so on a perfectly healthy, correctly-built, matched pair (console 0.1.10,
// engine 0.1.9) that comparison would return "console-ahead", which a vocabulary built on it would have to
// define as meaning "every route the console calls 404s". EVERY PACK FROM EVERY HEALTHY CUSTOMER would then
// carry a fabricated fault, and a genuinely STALE console (behind its own release) would be reported as AHEAD
// -- sending support to hunt the engine when the remedy is a reload. A row that asserts a fact the code never
// established costs support time rather than saving it.
//
...
// materialClass -- material-rejected only. WHY the console refused a piece of key or ceremony
...
// contractClass -- contract-skew only. HOW the engine's payload broke the contract the console
...
// fieldFamily -- contract-skew only. WHICH family of data broke. A closed PRODUCT vocabulary of
...
export const CLIENT_DIAG_FIELD_FAMILIES = [
  ...
  // A family must be fed by a value that arrives on the wire. integrations/marks.ts's integrationMark, for
  // example, is guarded on a CONSOLE-SIDE CONSTANT (every caller passes a mark key off CATALOGUE, a constant in
  // this repo, and all 40 catalogue keys resolve), so no engine of any version could ever fail that guard, and a
  // family for it would be coverage-shaped nothing: it would promise support a contract-drift signal about a
  // value THE ENGINE NEVER SENDS, and its absence from a pack would read as "the integrations vocabulary is in
  // sync" when nothing could ever have said otherwise. Every family below is fed by a value that arrived on the
  // wire.
  ...
  // The TOKEN-SOURCE CAPABILITY families, one per capability, rather than a single combined member: a single
  // member would fire only when the engine advertised NOT ONE capability, blind to the commoner state, a
  // strict SUBSET (a subset is not zero). An engine offering cf-config but not Workers produces no row at all
  // and reads in the pack exactly like a healthy, current engine -- and a subset build is precisely what an
  // engine ROLLBACK lands on, so the state that most needs recording ("our Workers source disappeared after
  // the update") is exactly the one a combined member cannot see. Per-capability families make the SET of rows
  // the evidence, and because fieldFamily is part of the ring's tuple key they do not coalesce.
  //
...
// drillAbort -- fleet-drill only. HOW the fleet-drill session ended. `none` is a member on purpose: a
...
// drillFact -- fleet-drill only. WHICH of the session's counts this row carries; `count` carries the
...
export interface ClientDiagnosticRecord {
  ...
  cspInlineOrigin?: ClientDiagCspInlineOrigin; // csp-violation only, and only when cspBlocked is "inline"
...
export interface ClientDiagnosticsPayload {
  ...
  // consoleBuild: WHICH CONSOLE BUILD the customer is actually running. The pack has always carried
