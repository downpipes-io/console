...
// clock, the route-template -> screen mapper, and the fault mappers, split out of ring.ts as its own module.
//
...
//
// The no-custody guarantee lives HERE, at its sharpest. These mappers are the only code in the console
...
const PATTERN_PREFIXES: ReadonlyArray<readonly [string, ClientDiagScreen]> = [
  // THE TWO APPROVAL INBOXES GET THEIR OWN SCREEN, AND THEY GO FIRST. Falling into the shared `security`
  // bucket ("/config" and "/security" both map there) would coalesce them: a render throw inside a .then
  // with no .catch is nearly always a TypeError, so the config-approvals screen frozen on skeleton rows and
  // the owner-approvals screen frozen on skeleton rows would emit the byte-identical tuple {unhandled,
  // security, other, unhandled-rejection, TypeError} and collapse into one row that cannot tell support
  // which screen it came from. Listed BEFORE their families, because the first prefix that matches wins.
...
// misclassify the single most common benign rejection in the console as a network fault.
...
const FAULT_BY_ERROR_KIND: Readonly<Record<ErrorKind["kind"], ClientDiagFaultClass>> = {
  ...
  "access-reauth-required": "auth", // the Access form of that gap (a stale Access assertion), the same auth matter
...
// into a field (review B5). It reads the message for exactly one thing: the trailing 3-digit HTTP STATUS the
...
// So the text is lowercased, TESTED against frozen substrings, and DISCARDED; the RETURN is a member of the
...
//
// The order matters: a signature failure is checked first, so a reason naming both a signature and a fetch
// cannot be mislabelled as merely unreachable.
...
//
// WHY THE ENUM COMES FIRST. Reconstructing the class by substring-matching a sentence written for a human would
// be a silent, ungated coupling: reword "could not fetch the update channel" and every row would degrade to
// `none` while every test stays green. The sentence is the fallback, never the source.
//
