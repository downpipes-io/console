...
//
// WHY THIS DOES NOT GO THROUGH THE WINDOW SEAM. Reporting the refusal by throwing a synthetic Error at
// window.reportError, so the existing unhandled-fault listener records it, is tempting but the evidence
// would be worthless: the ring's mapper answers "other" for a throw with no HTTP status, so a REFUSED
// identity.key download would produce {kind:"unhandled", screen:"keys", faultClass:"other"} -- byte for
// byte the same row as an unrelated null dereference on the same screen. Worse, the tuple those rows
// coalesce on holds none of the discriminating fields, so on the access screen a refused recovery-codes
// download, a dead clipboard copy and a console bug would collapse into ONE row with a count of three. A
// support engineer holding only the pack could not say a download was refused, could not say which
// ceremony, and could not separate any of it from a defect.
//
...
// `unhandled` console fault, which would inflate, on every Copy press on a plain-http host, the counter that
// is defined to mean "a console DEFECT that reached no catch site").
...
// a break-glass rotation) and takes NO error, so nothing of the throw can travel. Without it, keys/posture.ts
// and keys/rotation.ts would catch the ceremony's failure, render err.message into a field__error the customer
// alone ever saw, and report nothing anywhere: a locked-down browser that cannot generate a key pair would
// produce ZERO pack evidence.
