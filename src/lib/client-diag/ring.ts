...
// customer then chooses to share (request-scoped, no persistence anywhere).
//
// THE NO-CUSTODY GUARANTEE IS STRUCTURAL, NOT A MATTER OF DISCIPLINE. A record has ZERO free-string
...
//
// Storage: in-memory only. There is deliberately no sessionStorage, so a shared workstation cannot carry
// one operator's classes into another's pack.
...
// The coalescing key is the FULL closed tuple, so a repeat of the same closed tuple increments a count
...
// ending. A row that looks specific and is not is a recurring failure mode this key guards against.
function tupleKey(r: ClientDiagnosticRecord): string {
  return [
    ...
    r.reasonClass ?? "",
    r.applyClass ?? "",
    // capability / surface / capabilityOutcome. Left out of this key, they would discriminate NOTHING:
    ...
    r.surface ?? "",
    r.capabilityOutcome ?? "",
    // Every one of these is a discriminator this key exists to preserve, so every one of them is in the key.
    // Left out, each would be silently destroyed by the coalescer: a build check that came back with the wrong
    ...
    r.errorClass ?? "",
    r.faultSource ?? "",
    // These discriminators are in the key for the same reason as every field above it, and each one would be
    // destroyed by the coalescer if it were not: a CORS block and a genuine engine outage would be one
    // `transport-fault` row wearing whichever class was written first, which is the exact pair the row exists
    // to separate; the control-plane recovery read going quiet and the credentials-count read going quiet
    // would be one `read-degraded` row on whatever screen the operator happened to be on; a wizard that could
    // not reach the engine at the connect step and one whose readiness poll timed out would be one
    // `onboarding-step` row; an account that is empty and an account whose listings were refused for want of
    // scope would be one `discovery-connect` row, which is the confusion the fields exist to end; and a
    // vendor 5xx and the customer's own flaky wifi would be one `claim-exchange` row. A field outside this
    // key tells apart nothing.
    r.transportClass ?? "",
    r.callClass ?? "",
    r.obStep ?? "",
    r.obOutcome ?? "",
    // obSecret and channelReasonClass are in the key for the same reason as everything above. Outside the
    // key, a poll that exhausted with the break-glass secret absent and one that exhausted with the signer AND
    // break-glass absent would be one `poll-exhausted` row wearing whichever secret was written first, so a
    // half-keyed engine and an install that never took would be the same evidence. And an update channel
    // whose signature has been failing for six weeks and one the engine simply could not reach would be one
    // `update-channel-unverified` row, which is the pair the kind exists to separate.
    r.obSecret ?? "",
    r.channelReasonClass ?? "",
    r.discoveryOutcome ?? "",
    r.claimResult ?? "",
    // These discriminators follow the same rule, same reason: outside this key they discriminate nothing. A
    // refused role-delete and a refused approval-policy save would be one `admin-write` row wearing whichever
    // op was written first; a rollback the engine turned down and a settle it never saw would be one row; a
    // "sign out everyone" that APPLIED and one that 500'd would be one row, which is the exact question a
    // compromise ticket asks. On the recovery side a pasted export that would not parse in the browser and a
    // break-glass token the engine rejected would be one row, and they are not even the same half of the
    // system.
    r.adminOp ?? "",
    r.writeOutcome ?? "",
    r.recoveryOp ?? "",
    r.recoveryCode ?? "",
    // The rule that governs every field above governs these too, and the coalescer would destroy each of them
    // just as thoroughly if they were left out: a restore whose Max records was discarded and one whose media
    // token had no account beside it would be ONE `intent-dropped` row wearing whichever class was written
    // first; a destination verify that could not reach the bucket and one whose delete probe was denied would
    // be one `probe-outcome` row, and a destination verify and an IdP test would be the SAME row without
    // probeSurface, so the surface and the outcome are BOTH in the key; a members table that 404s (the engine
    // has not built it) and one that 500s (the engine is broken and the console said "pending") would be one
    // `feature-probe` row, which is the exact conflation the kind exists to end; a greyed-out Delete and a
    // skipped change-number prompt would be one `gov-gate` row. And adminOp is already in this key ABOVE,
    // which is what lets a gov-gate row say WHICH control was greyed out without a second op field.
    r.intentClass ?? "",
    r.probeSurface ?? "",
    r.probeOutcome ?? "",
    r.formField ?? "",
    ...
    r.govGate ?? "",
    r.skewClass ?? "",
    // Same rule, same reason. Outside this key a bulk DELETE that half-failed and a bulk RUN that half-failed
    // are ONE row wearing whichever action was written first, which is the exact pair bulkAction exists to
    // separate; a pasted share refused for base64 PADDING and one refused for a SMART QUOTE would be one
    // `material-rejected` row, and they are different answers down the phone; an unknown IdP preset and an
    // unknown source-type id would be one `contract-skew` row, as would a `connections` array that arrived
    // MALFORMED and one that was simply ABSENT; and every count of a fleet-drill session (targeted, passed,
    // failed, skipped, retry-exhausted) would collapse into ONE number, with an ABORTED session's counts
    // summing silently into a clean session's. Every one of these fields is here.
    r.bulkAction ?? "",
    r.materialClass ?? "",
    r.contractClass ?? "",
    r.fieldFamily ?? "",
    r.drillAbort ?? "",
    r.drillFact ?? "",
    // Same rule, same reason, and each of these would be destroyed by the coalescer exactly as thoroughly as
    // the ones above it. Outside this key a self-approval refusal and a not-owner refusal on the same inbox
    // are ONE row wearing whichever code was written first, and they are the maker-checker rule working
    // versus a role that can never approve; the stale hashed chunk the console's own policy blocked and a
    // third-party injection attempt are one `csp-violation` row, which is a broken pre-paint and a security
    // incident told apart by nothing; a dropped SAML certificate and a dropped coverage-inventory line are one
    // row, as are the accepted count and the dropped count of the same paste, so "one of two" and "one of
    // nine" become the same evidence; a wizard that lost the ACCOUNT and one that lost the BINDING are one
    // row, and they fail on different source types; and a Shamir split that died in WebCrypto and a
    // recovery-codes copy the browser refused would be one `ceremony-step` row on the keys screen, which is
    // the exact conflation the kind exists to end. Every one of them is here.
    r.ownerActionCode ?? "",
    r.cspDirective ?? "",
    r.cspBlocked ?? "",
    // cspInlineOrigin and deleteFate exist to separate exactly these pairs. Outside this key the two states
    // each was added to separate would coalesce straight back into the one row they came from: a stale
    // pre-paint hash (a broken deploy) and an injected inline script (an attack) are both {script-src,
    // inline}, and a queued custom-role delete (nobody floored) and an applied one (N people floored) are
    // both role-delete-impact with the same count.
    r.cspInlineOrigin ?? "",
    r.deleteFate ?? "",
    r.dropSurface ?? "",
    r.dropFact ?? "",
    ...
    r.ceremonyFault ?? "",
    // Same rule, same reason, and each of these would be destroyed by the coalescer exactly as thoroughly as
    // the ones above it. Outside this key a blocked localStorage and a FULL sessionStorage are ONE
    // `storage-blocked` row wearing whichever class was written first -- a browser policy and a full disk,
    // which are not even the same conversation -- and a lost wizard draft, a forgotten engine URL and an
    // auto-refresh that will not stay paused are one row, which is the exact three-symptom conflation these
    // fields exist to separate. On the map, a canvas the browser REFUSED and a static frame the operator ASKED
    // FOR would be one `renderer-degraded` row: one is a locked-down browser to unlock, the other is an
    // accessibility setting working as designed, and calling the second a fault is how a signal starts crying
    // wolf. Every one of them is here.
    r.storageArea ?? "",
    r.storageOp ?? "",
    r.storageClass ?? "",
    r.storageSurface ?? "",
...
// rows is the ring: a keyed Map, so a push is an O(1) keyed increment and NOT a scan (a 60fps redraw loop
// must not do O(ring) work in the hot path it is already thrashing). Map preserves insertion order, which
// gives the newest-wins eviction its "oldest first" order for free.
const rows = new Map<string, ClientDiagnosticRecord>();
// trueCountByKind is the per-kind UNCAPPED true count of coalesced events, kept even for rows the caps
...
// actually reasons over, so a cap loss degrades the detail, never the ratio.
...
// failure. A plain integer; it names nothing and identifies nothing.
...
export function push(candidate: ClientDiagnosticRecord, by = 1): void {
  ...
  const row: ClientDiagnosticRecord = {
    ...
    // A field missing here, while still present in the vocabulary, the tuple key and its recorder, means the
    // two classes it exists to separate reach the pack as BYTE-IDENTICAL rows that do not even coalesce. Every
    // closed field on the record must appear here, in the tuple key, and in snapshot(). All three, or the
    // evidence is dead.
...
// consoleBuild is the build this tab is running, seated ONCE at boot by app.ts from the build-time
...
// recordEngineCall records that an engine call failed. The screen is the active compile-time literal; the
// classes come from the total pure mappers. Nothing else is passed, and nothing else could be.
...
// recordContractDrift records that the console received a 2xx the engine's contract does not describe, and
// coerced it into something plausible (an unrecognised member of a closed field, a body that would not
// parse, an expected field absent, a route this console knows and this engine does not). ONLY THE CLASS
// TRAVELS: the offending value, the field name and any label are never read, so there is no field for them
// to occupy. Nothing here says WHICH field or WHICH value, deliberately.
...
// recordBulkOutcome records that a bulk action finished with per-item failures that live ONLY in a
// dismissable dialog, so the evidence would otherwise die with the dialog. COUNTS ONLY, never an item
// identity: the frozen record has a single numeric, so `count` carries the number of items that DID NOT
// COMPLETE (the magnitude support needs to size the fault), and `reasonClass` carries how the batch ended,
// which is what the attempted-versus-failed relation reduces to in a closed class: `all-failed` when every
// attempted item failed, `partial` when some succeeded, `auth` when a session expiry cut the batch short and
// the remainder was never attempted. The item names are the operator's own labels and never enter the ring.
...
// bulkReasonForFailures separates two states with OPPOSITE remedies that a plain success/failure count
// cannot tell apart on its own.
//
// Every bulk loop funnels its per-item failures through openBulkSummary, which classifies the batch from the
// COUNTS ALONE: some succeeded (partial) or none did (all-failed). On its own that would make a batch the
// engine REFUSED because the input was malformed, and a batch that died because the engine was DOWN, produce
// the identical row. One says "fix your input"; the other says "the engine is unavailable, retry". This
// function supplies the `validation` and `server` classes that tell them apart.
//
...
// bulkAction is REQUIRED: it is the discriminator that tells bulk loops apart. Without it every bulk loop in
// the console would produce the SAME tuple (bulk-outcome, downpipes, partial): a bulk delete that half-failed
// and a bulk run that half-failed would coalesce into one row with a count of two, and "my bulk delete
// half-failed last Tuesday" would be unanswerable from a pack that plainly contains a bulk-outcome row. It is
// passed by the CALL SITE (each loop knows which operation it is), never parsed out of the operator-facing
// verb the modal renders.
//
// It rides BESIDE the cause classification above, not instead of it: the action says WHICH loop, the reason
// class says WHY it failed, and a bulk delete refused for malformed input and a bulk delete that died on a
// 500 are two tickets with opposite remedies.
...
// recordSelectionDropped records the bulk-outcome half that otherwise carries no evidence at all. The data
// table drops the selection entries for rows that have VANISHED on a refresh, so an operator who ticked 20
// downpipes and then watched a background poll land can click Delete and have 17 deleted. Every count the
// loop reports is then self-consistent (17 attempted, 17 done, 0 failed), the pack agrees with them, and the
// three that were never touched are nowhere unless this recorder runs.
//
...
// all, and moved no data. Without this class, that ending is indistinguishable from a clean full apply and
// from an apply that never ran, so the pack could not tell an operator's three retries against a restore that
// wrote nothing apart from a restore that worked first time.
...
// recordApplyOutcome records ONE row per live-apply ending, on EVERY ending. `count` is how many applies
// ended this way (a plain repeat tally, never a sequence number and never an attempt id), so an operator who
// pressed Apply three times against the same silent ending reads as a count of 3, and the client attempt
// count can be compared with the engine's own restore-apply audit events.
...
// recordFanoutDegraded records that a bulk protect went ahead while the destination list was UNREADABLE, so
// every downpipe in the batch took the default destination and the operator's intended replicas were never
// created. The row's EXISTENCE carries both facts that matter (the destination read failed, and the batch
// fell back to the default); an operator who deliberately chose the default produces no row, so the two
// states are not the same evidence. `count` is how many downpipes the batch created without their
...
// recordCapabilityFault records that a browser capability the console asked for at a named point in the key
// or recovery ceremony did not get granted. The three closed classes ARE the record: WHAT the browser would
...
// null dereference, which is why capability faults get their own kind. `count` is how many times the same
...
// recordBootFault records the app-root boot path failing: the console failed to come up at all, which is the
// one fault the operator can least describe and support can least see (there is no screen to read an error
// off). The screen is the frozen `boot` sentinel, never a location read: the router has not resolved a route
// yet, so there is nothing to read even if we wanted to.
...
// recordUnhandled records a window unhandledrejection / onerror: a fault NOBODY caught, which is by
// definition a console defect rather than an expected outcome. The caller applies the noise filters first
// (window-faults.ts); this only records the frozen classes it is handed.
//
// faultSource and errorClass are the discriminators that keep these rows apart. Without them every uncaught
// defect in the console would reduce to {unhandled, <screen>, other}: an approvals screen frozen on skeleton
// rows because a render throw inside a .then had no .catch, and an unrelated synchronous fault on the same
// screen, would be the same row and coalesce into it. faultSource says which of the two symptoms the customer
// is describing (a screen that never finishes loading, or a painted screen whose control died), and
// errorClass says what class of defect it was.
...
// recordBootClass records that a named BRING-UP step failed, rather than the app root throwing. The two
...
// recordDeepLinkLost is the producer for the `deep-link-lost` kind.
//
...
// recordConsoleBuildCheck records WHAT BUILD THE BROWSER IS BEING SERVED, which no engine-side field can
// answer (the engine cannot probe a console behind Access). Recorded on EVERY outcome including `confirmed`,
// so the absence of a row after an applied console update is itself a fact rather than an ambiguity. The
// screen is the frozen `updates` literal: the check runs from the licence screen, and it is the update the
// row is about. No version string travels: a version is not a member of any closed union, and the CLASS is
// what the ticket turns on ("the origin is serving a different build" / "the bundle carries no stamp at
// all").
...
// recordConsoleRollback records the recovery half of a console build check: the operator pressed the
// post-apply console rollback, and this is how it ended. The row's EXISTENCE is "a rollback was attempted";
// `not-sent` is the member the engine can never record, because a rollback POST that never arrived leaves no
// engine-side rollback record to find.
...
// recordIdentityUnresolved records that the caller's identity report did not resolve, so every client-side
// capability gate fell back to `viewer` and an actual Owner is looking at a console where nothing is enabled.
// A 401 never reaches here (the caller filters it): a lapsed session is the ordinary state the signed-out
// route exists for, and recording it would fabricate a fault every time one expires.
//
// httpClass separates an engine that REFUSED the read (4xx/5xx) from one that could not be REACHED
// (network/timeout). faultClass IS THE SECOND HALF AND IT IS NOT OPTIONAL: httpClassForStatus collapses 401,
// 403 and 404 alike to "4xx", so without faultClass a whoami 404 -- an OLDER ENGINE THAT DOES NOT SERVE THE
// ROUTE AT ALL, which is the cause of the "identity pending" chip -- and a whoami 403 would produce the
// byte-identical row {identity-unresolved, screen, 4xx} and coalesce into it. faultClassForStatus maps 404 to
// "not-found" and 401/403 to "auth", so route-absent (upgrade the engine) and refused (fix the caller's
// access) are two rows. Both fields are in the tuple key.
...
// recordIdentityStaleGate records the race that recordIdentityUnresolved cannot catch, because that recorder
// only fires when whoami THROWS. The commoner state is a RACE in which nothing throws at all: router.start()
// paints the first screen before the boot-time whoami round trip returns, every capability gate on that render
// reads caller()?.role ?? "viewer", and when the identity later lands nothing re-renders the screen. An actual
// Owner deep-linking to /security then sees every control greyed "owner only", the whoami read SUCCEEDED, and
// without this recorder the pack would carry no row of any kind, making it BYTE-IDENTICAL to the pack of a
// genuine viewer. No httpClass can express this because there is no HTTP failure to classify.
//
...
// produces no row, so the genuine viewer and the raced Owner are not the same evidence.
//
...
// recordUpdateChannelUnverified records that the engine consulted the update channel and the verdict came back
// NOT VERIFIED. app-refresh.ts reads `if (!upd.configured || !upd.verified) return null;` and silently hides
// the chip, so a channel whose signature has been failing for six weeks and a healthy channel with nothing new
// to offer both show no chip, and without this recorder the pack would be identical for both:
// status.updateChannelConfigured is an env-var presence check that stays true throughout, so it is not a
// verdict. Without this row, an operator two releases behind has nothing anywhere saying why.
//
...
// recordRestoreGateBlocked records that the console kept Apply disabled, and WHY. The class is the whole row.
// The caller records only the four FAULT states; a plan that is simply still awaiting its second approver
...
// recordWireAnomaly records an engine-supplied value the console could not use. fieldClass says which
...
// recordTransportFault records the console's OWN transport classification. It is called from
// components/error-view.ts blockError, the ONE rendering seam every screen routes a channel-two
// (transport/auth) error through, so a screen cannot forget to record it the way an individual catch site
// could.
//
...
// recordReadDegraded records that a fire-and-forget engine read failed and was swallowed into a degraded chip
// or a fail-open gate. callClass names WHICH read went quiet (the screen cannot: these fire from
...
// recordOnboardingStep records one row per step ENDING, including the good ones, so a wizard the operator
// never got through is legible by what is MISSING as well as by what is there. The screen is pinned to
...
//
// obSecret is the discriminator on the readiness poll, and it is optional because it is meaningful on exactly
// one ending. At `poll-exhausted` the caller emits ONE ROW PER SECRET the engine kept reporting absent, so a
// HALF-KEYED engine (the ceremony wrote the signer and then failed) writes one row and an install that never
// took at all writes two or three. Without it both would be {onboarding-step, boot, readiness-poll,
// poll-exhausted}, one row, one count, and the operator's "it sat on Waiting for two minutes" would be the
...
// recordDiscoveryConnect records what the account-discovery token actually SAW. It takes a CLASS. The
// Cloudflare listing-error prose the screen renders is never read here, because it can name an account, a
// bucket or a namespace; only the fact that such errors EXIST rides, which is the discriminator needed (a
// scope gap and an empty account are otherwise the same silence).
...
// recordClaimExchange records how the licence claim-code exchange with the vendor control plane ended. It is
// pinned to the `settings` screen, which is where the licence activation form lives; the class is the whole
// discriminator anyway. The claim code, the response body and the control-plane URL have no field to travel
// in.
...
// recordAdminWrite records that a PRIVILEGED WRITE ended. adminOp comes from the CALL SITE (which knows
// exactly which write it is making) and writeOutcome from the numeric status, so no URL, no body and no
// refusal text is read on this path.
//
// An `applied` outcome is recorded ONLY for the ops in ADMIN_OPS_RECORDING_SUCCESS, where "did it actually
// take" is the question that matters. Everything else records only its failures, so a healthy console
// produces NO admin-write row at all: a row in the pack always means a privileged write did not go through,
// and the per-kind cap is never spent on good news that would evict the refusals underneath it.
...
// recordRecoveryRefusal records that a disaster-recovery flow was refused. The code is the frozen DP-R token
// the console has already stamped into the sentence the operator is reading, handed in as a typed member by
// the site that chose it; it is never parsed back out of the message (the message is prose, and on the engine
// branches it carries the engine's own reason, which can name a bucket or a key).
...
export function snapshot(): ClientDiagnosticsPayload {
  ...
  // consoleBuild rides on the ENVELOPE, not on a record: it is a property of the SESSION (which build
...
// the engine's pre-parse body cap (the engine caps Content-Length BEFORE it parses and answers a larger body
// with a 400, so the console must never send one). The ring's own caps already hold the section to about 13
// KB, so this bound is a backstop, not the working limit; when it does bite it drops the OLDEST rows first,
// keeping the most recent evidence. engineAttempts always survives, so the bot keeps its denominator even in
...
// rollupByKind is the per-kind uncapped true count. Exported for the validator, which proves a kind that
// lost rows to the cap still registers its true total.
...
// ---- recorders for console-decided refusals and probe outcomes -------------------------------------------

// recordIntentDropped records that the restore request builder DISCARDED something the operator typed. The
// engine never sees the dropped intent, so this row is the only possible witness. The class is chosen by the
// builder, which is the one place that knows what it dropped; the typed value (a Cloudflare edit token, an
// account id, a database name) has no field on the record and cannot ride.
...
// recordProbeOutcome records one operator-initiated test and how it ended. `ok` is recorded as well as the
// failures, and must be: a common complaint is "it fails every morning and works on retry", which is a claim
// about a ratio, and a ring of failures alone cannot confirm or deny it.
...
// the compiler enforces the discrimination the classifier depends on. A FLATTENED shape here (`{ ok: boolean;
// reason?: string; deleteProbe?: "ok"|"denied"; ... }`) would hide defects from tsc: it would make
// `deleteProbe` look reachable on the failure arm (it is not) and make `ok:true` look like it could not carry
// a denied delete (it can, and that is the WORM case). Keeping the union means a future re-ordering of
// destProbeOutcome's arms is a type error, not a silent unreachable branch.
...
// all. Testing `res.ok` first would make dest-delete-denied unreachable on every response the engine can
// produce, folding the WORM/retention bucket (writes accepted, deletes refused, so the customer's retention
// CANNOT BE MANAGED) into the same coalesced row as a perfectly healthy verify. A bucket whose lifecycle is
// stuck and a bucket that is fine are opposite tickets and must never share a row.
...
// recordFormRefused: the console's OWN validator REFUSED A VALUE THE OPERATOR TYPED. No request is made, so
// the engine has no view of it whatsoever. formField is the FIELD CATALOGUE control id (a closed product
// vocabulary); the typed value is never read into the row.
//
// IT TAKES THE RAW VALUE, AND THE EMPTINESS RULE IS ENFORCED HERE, WHICH IS THE ONLY PLACE IT CAN BE. A
// `rejected` row asserts the console examined a value and turned it away, so there has to have been one: a
// required box the operator has not filled in yet is an operator part-way through a form, the commonest event
// in the console, and recording it would bury every real refusal under it. Enforcing the rule at each
// individual call site risks it being broken at any one of them: the setup screen could record an untouched
// KV form as two rejections (validateHexId returns "The ... is required." for ""), and the destination form
// could record an empty retention box as dest-worm-days/rejected (Number("") is 0, which fails `days <= 0`),
// BYTE-IDENTICAL to the real refusal each member exists for. So the rule lives in the RECORDER instead: the
// value is a required argument, and a `rejected` row cannot be written without one. THE VALUE IS READ ONLY TO
// TEST WHETHER IT IS EMPTY and is never stored, never copied into the row and never leaves this function; the
// row carries the catalogue field id and nothing else. There is no way to call this with the outcome and not
// the value, which is the regression gate: the type checker is.
//
// AND THE EMPTINESS RULE NEEDS ONE MORE FACT, WHICH IS THE FACT THE RULE IS ABOUT. The rule exempts an
// operator PART-WAY THROUGH A FORM: nothing was examined, so there is no refusal to claim. An <input
// type="number"> whose content is not a valid floating-point number reads back as the empty string too -- the
// HTML value-sanitisation algorithm throws the operator's text away -- and that is the OPPOSITE state. The
// operator typed a contracted rate, the control could not convert it, and the console refused them over it. On
// the value alone the two are indistinguishable, which is exactly why validity.badInput exists: without it
// this rule would silently drop the one refusal it is not meant to drop: the box reads empty, no row is
// written, and "I cannot enter my contracted rate" has no evidence anywhere.
//
// So the rule is "empty AND the control did not report a conversion failure". This is strictly narrower than a
// plain emptiness check on the state it exempts (an untouched required box has badInput false and still
// records nothing) and it stays alert to the state it must never exempt. The typed value is gone by the time
// this runs, which is precisely why this row can carry a catalogue field id, a count, and no customer data
// whatsoever.
...
// recordFormCoerced records that the console SILENTLY REPLACED a value the operator typed (a number input
// eating a rate). It is the other rejectOutcome, and it is split from recordFormRefused because it makes the
// OPPOSITE claim: nothing was refused, the operator was never told, and the value that got through is not the
// one they entered. It has no emptiness question to answer (there is no coercion of a value nobody typed), so
// it takes no value and can carry none.
...
// stable member here. It would be easy to do: a prefix check, one member, and the list would look more
// complete. But those controls are `required: true` with a validator that only rejects the empty string, so
// the only refusal they can ever produce is required-and-empty -- the one refusal the funnel does not record.
// The member would have no producer, and a member with no producer is a section that can never be populated,
...
// recordCatalogueDegraded records that the Cloudflare-configuration offer was withheld, or made against a
// catalogue the console could not trust. It records a WITHHELD AFFORDANCE, never a fault on a healthy path:
// the operator opened the screen and did not get the option.
...
//
// Without this classifier every refusal would compose ONE row, cf-rediscover-failed, on the same screen, with
// the same class, coalescing on the tuple key into a single count. "The discovery token was cleared" (paste a
// token), "the account fell out of the owner's discovery scope" (only the OWNER can fix it) and "Cloudflare
// rate-limited us" (do nothing, retry) are three different phone calls and this keeps them as three rows.
...
// be recorded at all. It classifies by the throw's KIND FIRST and only then by its numeric status, because
// keying on errorStatus() alone would put THREE distinct states in one row.
//
// The console transport has first-class throws that carry NO status. Without classifying by kind first, they
// would all fall through together to cf-rediscover-transport, whose own vocabulary asserts "the call never got
// an answer: the engine or the network dropped it":
//
...
//   is 401-only, so the caller's sign-out guard never catches it either. The engine is fine, the operator left a
//   tab open overnight, and without this check the pack would say the network dropped the call. It is the
//   ordinary overnight state and it records NOTHING: null, no row. There is deliberately no member for it,
//   because no path would write one.
//
...
//
//   THE CONSOLE'S OWN WORKER, with no ENGINE service binding. No request reached any engine, so a row blaming
//   the engine or the network would assert a fact nothing established. It is the residual, not transport.
export function cfRediscoverThrowClass(err: unknown): ClientDiagCatalogueClass | null {
  ...
  // The console's own worker answered the rediscover with its own 500. Like the bindingless case above, no
  ...
  const st = errorStatus(err);
  // A bare 403 does not establish that the ENGINE refused anything. The transport already folds the refusal
  // body's SHAPE into the throw, so cf-rediscover-denied -- "the engine refused this caller's role" -- is
  // reserved for that shape; a WAF block page or a static host at the engine's address is neither a role
  // problem nor necessarily backed by an engine at all.
...
// at all. It is the twin of cfRediscoverThrowClass above.
//
// WITHOUT THIS CLASSIFIER, SEVEN STATES WOULD WRITE THE SAME ROW, cf-surface-list-unreadable, and two of them
// are not faults:
//
...
//   a naive classifier would gain a fault row against a healthy engine and a healthy token, on the very screen
//   whose remedy is "go and look at your discovery token". It records NOTHING: null, no row.
...
// "Discovered 0 surfaces in use" with no fault row anywhere else to say otherwise. A healthy token over an
// account that simply has nothing configured fills `empty`, not `unavailable`, so this cannot fire on a
// legitimate account: it needs zero present, zero empty, and at least one unavailable.
...
// recordFeatureProbe records WHICH DIAGNOSIS THE CONSOLE REACHED about a route it could not read. The whole
...
// and only then by its numeric status, because the status alone is not enough to place every throw correctly.
//
...
// first-class throws that carry NO status at all, and without classifying by kind first both would fall
// through to `network` -- the row whose own vocabulary entry asserts "the engine is unreachable":
//
...
//      ACCESS_REDIRECT_MARKER. The 401 guard below never sees it, so without this check every operator who left
//      a tab open overnight and reloaded the security screen would write "the engine is unreachable" against
//      the roles table, the audit events, the group roles and whoami at once. It is the ordinary overnight
//      state and it records NOTHING, on exactly the reasoning the 401 rule is built on: a fault row for a
//      legitimate state cries wolf. There is deliberately no `access-session-lapsed` MEMBER: no path would ever
//      write it, and a vocabulary member with no producer is dead vocabulary that reads like coverage.
...
export function featureOutcomeForError(err: unknown): ClientDiagFeatureOutcome | null {
  ...
  // The console's OWN worker refused the call for want of an ENGINE service binding. It carries no status, so
  // without this check it would fall through to `network` -- the member that asserts the engine is
  // unreachable, a fact nothing here establishes, because no request was sent to any engine at all.
  if (kind === "engine-binding-absent") return "engine-binding-absent";
  // The console's OWN worker manufactured the 500 (its dispatch threw, or the ENGINE binding's fetch rejected
  // because the engine worker is deleted, throwing or over its limits). It must be read BEFORE the status rules
  // below: on the bare status this would land on `server-error`, whose meaning is that the ENGINE saw the call
  // and refused it, sending support to read refusals that cannot exist. Nothing here establishes anything
  // about the engine except that the console answered in its place.
  ...
  // A BARE 403 DECIDES NOTHING ABOUT WHO REFUSED. `forbidden` tells support the remedy is "a permission or an
  // edge rule, never the engine's address", so it must be reserved for exactly that: the state where the
  // address IS the fault -- a static host or a bucket at the engine's address answering 403 with a web page, no
  // engine deployed there at all -- must not read as `forbidden`. THE DISCRIMINATOR IS ALREADY IN THE THROW: the
  // transport classifies the refusal BODY BY SHAPE (classifyForbiddenBody) and folds the closed member into the
  // message, so forbiddenClass() reads back whether the refusal spoke the ENGINE'S own vocabulary
...
// recordGovGate records a governance gate the CONSOLE applied. Neither member can leave an engine-side trace
// by construction, and they fail in opposite directions (one blocks an action that should have run, the other
// lets through an action that should have been stopped for a change number).
...
// recordConsoleSkew records the RUNNING console bundle, as its relation to what the origin serves right now.
...
// versioned repos, so comparing them would fabricate a skew on every healthy customer and could invert the
// direction the comparison is meant to show. See the vocabulary note beside CLIENT_DIAG_SKEW_CLASSES. WHICH
// console build is running rides in the pack directly, as `consoleBuild` on the payload envelope, next to the
// engine's version: support compares those two themselves rather than being handed a verdict the browser
...
// ---- recorders for material, contract and fleet-drill evidence ---------------------------------------------

// recordMaterialRejected records that the console REFUSED a piece of operator key or ceremony material. Every
// one of these refusals is decided in the browser, so no request is made and the engine cannot hold a record
// of it: the browser is the only witness there can ever be. The class is chosen at the REJECT BRANCH, which
// knows exactly why it refused, and NOTHING about the material rides with it. Not the text, not the offending
// character, not its index, and deliberately not a length: a length is a fingerprint of the secret, and the
// class already says the length was the problem.
...
// recordContractSkew records that the console was handed data it could not interpret and rendered something
// plausible anyway. contractClass says HOW the contract broke and fieldFamily WHICH family broke it; both are
// in the coalescing tuple, so an unknown IdP preset and an unknown source-type id are two rows, not one, and
// an absent field and a malformed one are two rows, not one. The unrecognised VALUE never rides: a new enum
// member could be an operator-named preset id or the customer's own label for a source.
...
// The counts of ONE fleet-drill session, as the console assembled them.
...
// recordFleetDrill records ONE fleet-drill session's SHAPE. The engine already records every drill it handles
// per downpipe, so this is deliberately not about which pipe failed: it is about the session the engine cannot
// see. Five rows, one per fact, each carrying its number in `count`, all sharing the session's abort cause so
// an aborted session's counts can never sum into a clean one's.
//
...
// ---- recorders for owner-action, role-delete, CSP, input and ceremony evidence -------------------------------

// recordOwnerActionRefusal records that a dual-control approve or reject did not go through. adminOp names the
// route (owner-action-approve / owner-action-reject) and ownerActionCode why, and both are in the tuple. The
// three console-decided codes never reach the engine at all (the console refuses before it calls), so the
// browser is the only witness there can ever be; the proposal summary, the proposer's email and the action id
// have no field here and cannot ride.
...
// self-approval and for a non-owner alike, so a classifier over that text would guess, and guess wrong the
// moment the engine rewords itself), and NOR IS ITS STATUS taken to mean more than it does.
//
// THE STATUS CANNOT SAY WHICH REFUSAL IT WAS. `terminal-state` is reserved and never returned from a bare
// 409/404/410 status, because the engine cannot produce any of those on these routes: every refusal
...
// AuthError becomes a 403). Returning `terminal-state` from a 400 would give it no real producer, and would
// mislabel the one 404 that can reach here -- a route-not-found from an engine with no owner-action routes -- as
// "already executed or expired", a fact nothing established. The discrimination is made from the CONSOLE'S OWN
// re-read of the inbox instead (ownerActionFateFromListing, screens/owner-actions.ts), never from a status the
...
// lapsed session with its LOGIN PAGE rather than a 401, so without this check an overnight tab would write a
// dual-control refusal into the pack: a governance fault fabricated out of the ordinary end of a working day.
//
// A 2xx THAT DID NOT PARSE IS NOT "UNREACHABLE". parseJson throws "<verb>: <status>" with the SUCCESS status
// still on it when a 2xx body is not the JSON the contract describes and is not an Access page or an HTML page
// either (a truncating proxy, a middlebox rewrite). errorStatus narrows to 400-599, so that throw carries no
// status, and treating it as `unreachable` -- whose stated meaning is that no engine saw the call and no
// engine-side record of it can exist -- would say the opposite of what happened: on an approve, the engine took
// the approval and, for a DO-executed kind, ALREADY REPOINTED THE DESTINATION. It has its own member,
// `answer-unreadable`, for exactly this.
//
// AND `engine-refused` HERE IS COARSE ON PURPOSE, INCLUDING ON THE 403. It is a status-only reading and it
// claims only "an engine-shaped answer came back with a refusal status": a 400, a 429, a 500 and a 403 all
// reach it. The 403 is then SPLIT BY THE BODY SHAPE in refusalCode (screens/owner-actions.ts), because four
// different things answer 403 to one call and only one of them is the caller's authority; a status-only
...
// recordRoleDeleteImpact records that a custom role was deleted, and `count` is how many grants still
// referenced it. Every one of those members falls to the viewer floor server-side, silently, and the pack's
// custom-role-change delete event says nothing at all about them. The ZERO case is recorded too, deliberately:
// a tidy-up that broke nobody and one that downgraded four people must not both be an absence, and push()
// writes the row even when the increment is zero. Counts only; the affected members' emails and subjects have
...
// recordCspViolation records that the browser blocked a resource under the console's own
// Content-Security-Policy. Both classes come from the total, pure mappers below; the report body itself is
// read only by them, and they copy nothing out of it.
...
//
//   the stale pre-paint hash after a deploy  -> a BROKEN DEPLOY
//   an injected inline script                -> a SECURITY INCIDENT
//
// Without cspInlineOrigin they would coalesce into one row: a broken deploy and an attack, told apart by
// nothing.
//
...
//
// `self` is the one worth naming: it is the console's own origin, blocked by the console's own policy. This
// happens when an asset the page legitimately asked for no longer matches the hash allowlist (a stale cached
// chunk after a deploy). It is a broken deploy, not an attack, and it must not read as one.
...
// recordInputDropped records that the console silently discarded part of a paste before submitting it. TWO
// rows, one per fact, so the accepted count and the dropped count are both on the record and neither can be
// inferred from the other: "one of two certificates was dropped" and "one of nine" are the same droppedCount
// and different tickets.
//
...
// recordHandoffDropped records that a wizard or deep-link hand-off lost the operator's earlier pick. The class
// is the whole row and it is chosen at the site that DROPPED the value, which is the one place that knows what
// it lost. The query string, the zone id, the account id and the binding name are customer values and there is
...
// recordCeremonyStep records that a browser-side key-ceremony or credential-enrolment step ended. `ok` is
// recorded as well as `failed`, so an artefact that was never produced is legible by the row that is MISSING
// as well as by the one that is there.
//
// The screen is pinned to `keys`: these panels are mounted from the key ceremony and the enrolment flow, and a
// step outcome filed under whatever route happened to be active would not be findable. NOTHING about the
// material rides: no key or share bytes, no ciphertext, no payload fragment, no decode offset, and not N or
// the threshold.
...
// recordStorageBlocked is the emit for a browser store that refused. It is called from the storage leaves
// themselves (draft.ts, store.ts, the preference modules), which swallow the throw by design: a preference
// that will not persist must never break a flow, and that design is exactly why a locked-down profile would
// otherwise be invisible to support. The row says WHICH store, WHICH operation, WHY, and WHAT the customer
// lost. The KEY is never recorded (a draft id can carry a run id) and the VALUE never leaves the browser at
// all.
...
// recordRendererState is the emit for WHICH RENDERER WAS LIVE on the map, recorded on every map mount
...
// recordFocusLanding is the emit for an accessibility fact this vocabulary can carry: without it, no member
// of any closed union here matches keyboard, a11y, tablist, roving or focus, so a keyboard user who cannot
// arrow past the first tab produces a support pack byte-identical to a healthy one.
//
...
// customer never touched would carry identical evidence, which is how a surface like this stays invisible.
//
