// transport-probe.ts -- the CORS-versus-outage discriminator, made reachable at the one seam that needs it.
//
// THE STATE THE PACK MUST DESCRIBE. A customer reports "every screen fails". Two causes, one remedy each:
//
...
//
// A CORS-blocked fetch and a fetch to a dead host are THE SAME TypeError in the browser, so telling them apart
// needs a second observation: the `origin-rejected` transport class is promoted from a network throw only when
// the unauthenticated health probe is ALSO confirmed reachable.
//
// THAT FINGERPRINT NEEDS TWO THINGS TO HOLD, AND BOTH MATTER HERE:
//
//   1. Every screen's transport error must pass through the same discriminating check. Relying on each
//      individual catch site to remember to run the health probe is fragile: a screen that omits it records a
//      CORS block as engine-unreachable BY CONSTRUCTION -- the same row a genuine outage writes, coalescing
//      into it on the tuple key.
//   2. /admin/health must NOT sit behind the same origin-equality CORS gate as the data routes, or in the
//      CONSOLE_ORIGIN scenario the browser blocks the health probe too. If that happens healthReachable reads
//      false, the class resolves to engine-unreachable, and `origin-rejected` becomes reachable ONLY by a state
//      (health passes, authenticated calls do not) where "set CONSOLE_ORIGIN" would be the WRONG advice.
//
// BOTH HALVES OF THE FIX ARE REQUIRED. The engine serves /admin/health with an unconditional
...
// itself, at the ONE render seam every screen's transport error passes through, so the discriminator does not
// depend on every call site remembering to pass a flag.
//
