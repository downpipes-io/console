...
// is right, and it has one consequence worth naming. The post-apply console build check records `confirmed` and
// then, ~1.5 seconds later, calls location.reload() to finish the update. The reload destroys the ring. So on
// the HAPPY PATH the confirmed row is written and then annihilated, ALWAYS: without this handoff, the absence
// of a console-build-check row after an applied console update would be the GUARANTEED state after a
// successful apply, byte-identical to "the check never ran" and to "the pack came from a different tab" --
// never the fact it should be.
//
