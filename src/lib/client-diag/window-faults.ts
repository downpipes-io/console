// The window-level fault handlers.
// A rejection nobody caught, or an uncaught throw, is by definition a console DEFECT rather than an expected
// outcome: it never reaches a catch site, so it never reaches a toast, so it dies in a devtools console
...
//
// NOTHING OF THE ERROR TRAVELS. The handlers never read error.message / error.name / error.code /
...
//
// THE NOISE FILTERS ARE THE POINT. Without them these signals are worthless, because the browser
...
// storage-blocked and env-capability are deliberately NOT collected at all: they are dominated by
// private-browsing, and an ordinary browser would fire the second.
//
// CSP violations are collected despite extension noise, because dropping the whole channel would remove the
// only evidence for two things the product has no other witness to: the console's own asset being silently
// blocked by its own policy after an update, leaving the page broken with no diagnosis; and any script
// injection attempt, which a security team will eventually ask about. The extension noise is filtered instead
// of dropping the whole signal: noteCspViolation drops a violation whose blocked URI carries a
// BROWSER-EXTENSION scheme before anything is classified, exactly as an opaque cross-origin "Script error." is
// dropped above.
...
//
// A screen that renders inside a `.then`
...
// sanctioned pre-paint script being refused by the console's own policy. A stale hash would silently block
// the pre-paint for every visitor, and the page would come up with no theme and no accessibility attributes
// before first paint.
//
