// identity-gate.ts -- the witness for one REAL state: A CAPABILITY GATE THAT WAS COMPUTED BEFORE THE IDENTITY
// REPORT ARRIVED.
//
...
//
// canDo and canCap decide almost every gate in the console, and both fail closed on a null caller in
// silence; the four screen-local predicates above are hand-copied duplicates of canCap that exist so a
// gate outside canDo/canCap's own reach still gets witnessed. scripts/identity-gate-witness-gate.mjs
// DERIVES the required set from source, so a new fail-closed gate cannot be added silently and this list
