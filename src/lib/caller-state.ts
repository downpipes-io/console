...
//
// Why it is separate from store.ts. store.ts imports EngineClient, so if these facts lived in store.ts's
// state object, anything that needed the caller would have to reach through the whole API client to get
// it. That would create a genuine runtime import cycle that the gating madge job flags:
//
