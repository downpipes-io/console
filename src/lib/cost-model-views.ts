...
// ---------------------------------------------------------------------------
// Marginal and one-off views (spec section 2.2 derived views)
// ---------------------------------------------------------------------------
...
// spec: restoreCostOnce = (objectsReadPerRestore / 1e6 x P_classB) + (restoreGB x P_egress if downloaded).
...
// no regime or month, so A0 is the spec-aligned, finite, full-archive basis; the operator can
...
// only (Class B), no egress. spec: drillCostOnce = (objectsReadPerDrill / 1e6 x P_classB).
...
// delta from the baseline (the operator's current inputs), implementing the spec's
// sensitivity view (churn, retention depth, cadence). The baseline and every swept point are
