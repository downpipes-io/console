...
export async function controlPlaneExportDownload(t: Transport): Promise<{ export: unknown; signature: string }> {
  // A FAILED signed-export download is recorded as a refusal here, where the status is still a number, so
  // it is visible to whatever reads recorded refusals; parseJson then throws exactly as it otherwise would
  // (it owns the Access-redirect / malformed-body naming, unchanged).
...
//
// IT MUST BE GIVEN THE WHOLE BODY, not the truncated display string. `refusalClass` is the LAST key the
// engine serialises, so reading it off a string clamped to 300 characters for the operator's screen would
// sever the JSON mid-string on any refusal whose prose runs past the cut (the partial-signer-rotation
// sentence is a 504-character body), the parse would throw, and the class would silently degrade to
// DP-R12: the catch-all, byte-identical to a plane that was not empty. A discriminator that collapses into
// the bucket it was built to escape is worse than none, because it looks closed. The display clamp is
// applied AFTER the class is read, and only for the screen.
//
...
// -- which is what an OLDER ENGINE (one that does not send the field) produces, so the console keeps
// working against it.
...
export async function controlPlaneRestore(
  ...
  if (!r.ok) {
    // The engine's own reason is CARRIED on every branch, including 401, since dropping it there in favour
    // of a fixed sentence would throw away the one thing that distinguishes a wrong token from a token the
    // engine could not check at all. Every branch also carries a stable refusal code the operator can read
    // out: this flow runs on a wiped engine, where the audit ring is empty and a support pack may not
    // build, so the message is the only witness.
    const body = await r.text().catch(() => "");
    // The engine answers 400 for a bad signature, a failed shape check, a no-custody refusal and a plane
    // that was not empty alike, so a plain status code alone cannot tell these apart. The engine returns a
    // CLOSED refusalClass member in the 400 body; engineRefusalClass reads it BY SET MEMBERSHIP off the
    // WHOLE body (not the clamped display string, which severs the trailing key and drops the class back to
    // DP-R12) and refusalClassToCode maps it to a distinct code. `txt` is the engine's own prose, clamped
    // for the screen only: never classified, never carried.
...
export async function controlPlaneImport(
  ...
  if (!r.ok) {
    ...
    // refusal code.
    const body = await r.text().catch(() => "");
    // Same split as the reconcile above, and the same rule: the class is read off the WHOLE body and the
    // prose is clamped only for the screen. Same code, different operation: a signature refusal on an
    // import (a wrong signer.pub) and one on a reconcile (this engine's own signer key) are different
    // situations, and recoveryOp is in the ring's tuple key, so they cannot coalesce.
    const code = r.status === 400 ? refusalClassToCode(engineRefusalClass(body)) : refusalCodeForStatus(r.status);
    const txt = body.slice(0, 300);
...
// controlPlaneAcknowledgeRecovery clears the recovery-required latch and does NOTHING ELSE. It is the one
// exit an ESTABLISHED account has: the other two affordances the banner offers both rebuild a wiped plane
// and so refuse a non-empty role table, and the last-Owner guard makes a non-empty role table the norm, so
// those affordances are the ones the engine has to refuse for an account whose role table is already intact.
//
...
export async function controlPlaneAcknowledgeRecovery(t: Transport): Promise<ControlPlaneAcknowledgeResult> {
  ...
  if (!r.ok) {
    ...
    // through capabilityPhrase and the role by its DISPLAY LABEL: a raw capability id or a raw built-in
    // role id in customer copy is a string only this repo can read.
