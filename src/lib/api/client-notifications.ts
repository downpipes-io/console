...
export async function upsertNotifyChannel(t: Transport, channel: NotifyChannelInput): Promise<MutationResult<NotifyChannel>> {
  ...
  // ceremony and retries once; plain engineFetch would let that 401 reach the screen instead, so the
  // save would fail closed with a visible error and no ceremony run. These four govern the availability
  // of evidence: silencing an account by deleting or narrowing the rule that carries a critical alert is
  // a stale-ambient-cookie action with no visible effect and no undo, which is why the engine gates both
  // directions, set as well as delete.
...
export async function testNotifyChannel(t: Transport, channelId: string): Promise<{ ok: boolean }> {
  ...
  // delivery that failed, or a probe that never ran. That is enough to tell an operator whose channel test
  // has never once passed from one whose test passes and whose real alerts are being dropped downstream
  // (which the notify history answers). The channel id is never recorded: it is the operator's own label
  // for a webhook.
  try {
