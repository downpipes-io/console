// Downpipe + run + licence + restore-apply domain functions for the EngineClient: free functions over the
// shared Transport. Each takes the same arguments the corresponding EngineClient method takes, building the
// request URL, method, body and headers and parsing or throwing on the response; the EngineClient method
// itself is a thin delegator to it.
...
export async function health(t: Transport): Promise<{ ok: boolean; service?: string }> {
  ...
  // redirect HTML body) is classified rather than throwing an unclassified parse error.
...
// route is a WHOLE-OBJECT upsert: without this precondition, two operators who load the same downpipe and
// save different edits would both be answered 200, one edit discarded with nothing said, and neither
// operator able to tell (for example, a schedule change made inside one operator's open drawer silently
// reverted by an unrelated switch flip in another operator's).
//
...
// of an open screen to one round trip and would still overwrite silently, which only looks like a fix.
...
// (HTTP 202). Reading a queued 202 body with a plain parse would yield `{ deleted: undefined }`, telling
// the operator "the engine reported nothing was deleted" about a delete that is actually QUEUED FOR
// APPROVAL, with the row (honestly) still present - reading as an undeletable downpipe. parseJsonOrPending
// avoids that by surfacing the pending state explicitly.
...
export async function restore(t: Transport, req: RestoreRequest, change?: ChangeRef, opts?: { restoreMasterB64?: string }): Promise<RestorePlan | RestoreResult> {
  ...
  // The OPTIONAL browser-recovered per-run master (base64url of 32 raw bytes) rides the distinct
  ...
  // gatedFetch, NOT a bare engineFetch. This route is a sensitive write and the engine can
  // answer it 401 { stepUpRequired: true }; only gatedFetch runs the re-auth ceremony and retries with the
  // x-downpipes-stepup header. On a plain engineFetch, a step-up 401 would be final, with no retry.
  //
  // Without gatedFetch, a legitimate maker-checker sequence can dead-end: a maker plans and requests, a
  // distinct approver approves, and the review itself can easily outlast the maker's session freshness
  // window. The approval is present and the session is otherwise valid, so the apply gate arms, but the
  // apply then meets a 401 step-up-required with no way to raise the passkey prompt on a bare engineFetch.
  // The only escape would be outside the flow entirely, which is worse for the in-console break-glass
  // restore specifically, because the per-run master is decapsulated in the tab and would be lost with it.
  //
  ...
  if (!r.ok) {
    ...
    // an Access-redirect non-2xx body before degrading to the plain "restore: <status>".
...
// The engine returns the envelope { entries: RunHistoryEntry[] }, an OBJECT, not a bare array, so this
// unwraps .entries here so the Runs UI binds to a real array; treating the envelope itself as the array
// would leave every consumer calling .length / .map on an object that has neither.
...
// byDownpipe, so a view built from the rings alone would print the same sentence over both: "Nothing needs
// your attention yet." One of those estates has lost every record of its backups and one has nothing to lose.
//
// The fact that separates them is runsRecordedTotal, derived from the account-global monotonic
// runlogCounter, which is already populated on every estate now running, so it answers for history that
// is already gone rather than only for loss observed from here. runsRolledOverCount is recordedTotal -
// retainedCount clamped at zero, and runlogCounterReset says the figure is a FLOOR rather than a
// measurement (a counter wound back below a retained index under-reports the loss, and saying so is the
// honest answer).
//
// ALL FOUR ARE OPTIONAL, and absent is not zero. An older engine omits them entirely, and a console that
// read an absent counter as 0 would tell every estate on an older engine that nothing had rolled over,
// which is the defect wearing the costume of its fix.
