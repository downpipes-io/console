...
export async function testEmailDelivery(t: Transport): Promise<{ ok: boolean; reason?: string; code?: string }> {
  ...
  // and complete. This row does NOT copy it: it records its PRESENCE as a class, so a refusal the platform
  // NAMED (a sending domain that was never onboarded, a sender that was never verified: a configuration
  // the customer can fix) is told apart from one it did not, and the operator-initiated tests gain a
  // history that a one-off preflight probe cannot have, since preflight runs once, at pack-build time,
  // and says nothing about the fortnight before it.
  try {
