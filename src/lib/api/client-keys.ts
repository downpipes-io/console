...
// over the shared Transport, each building the request URL, method, body and headers and parsing or
// throwing on the response.
...
// SURFACING half the rotation warning needed: the engine has computed the inventory since G-P0-098, and
// with no console caller an owner who rotated and kept only the latest key had no way to see the old
...
// the step-up ceremony and retries automatically, matching installKeys, rotateBreakGlass and
// setBreakGlassOnly, so every key route below runs the same ceremony. The engine REFUSES (a non-2xx
// { error }), never silently reissuing, when an operational key is already present; the console
// surfaces that reason via readErrorReason.
...
export async function rotateBreakGlass(t: Transport, token: string, breakGlassPublic: string): Promise<{ ok: true }> {
  ...
  // Read through parseJsonOrOwnerAction so a 202 can NEVER be coerced into a false "applied" and a false
  // "break-glass key present" while the posture never moved. The decoder's non-2xx { error }-reason fold
  // and its 2xx parse are byte-identical to a hand-rolled read, so only an illegitimate 202 changes
...
export async function setBreakGlassOnly(t: Transport, token: string, confirmDiscardStranded = false): Promise<{ ok: true }> {
  ...
  if (!r.ok) {
    ...
    // checked, and whether the scan was complete. Flattening it to a plain message string would leave the
    // owner with a dead end: a sentence and a re-enabled button, with no sanctioned path forward in the UI.
...
//
// Retiring is the GATED owner action `break-glass-retire` (engine scheduler-do-routing-config.ts, the POST
// /policy/break-glass-retired case, which wraps the retire:true direction in gatedOwnerAction and answers
// through ownerActionJson; the un-retire direction is never gated). With dual control armed the engine
// answers HTTP 202 + { ownerActionQueued:true, id, status } and the latch is NOT set. Reading a queued
// body with a plain parseJson (which throws only on a non-2xx) would cast it to { retired } and let the
// screen say "Break-glass token retired. The static token can no longer sign in." while the static token
// still signs in: the operator is told a way in is closed and stops treating the bootstrap token as live,
// while it stays live until a second owner approves, which is the worst way this class of bug can fail.
// Reading through parseJsonOrOwnerAction instead gives the caller a discriminated { status:"queued" }. The
// non-2xx path is unchanged: parseJsonOrOwnerAction runs the SAME readErrorReason fold (throwing
// "<verb>: <reason>: <status>", which retireRefuseText parses) and falls through to failResponse when the
// body carries no usable reason.
