// Source-discovery + cf-config + binding-attach domain functions for the EngineClient. Free functions
...
//
// Changing the browsed accounts is the gated owner action `discovery-accounts-set` (engine
// scheduler-do-routing-config.ts, the POST /sources/discovery-accounts case, which wraps the write in
// gatedOwnerAction and answers through ownerActionJson). With dual control armed the engine answers HTTP
// 202 + { ownerActionQueued:true, id, status } and writes nothing. Reading through plain parseJson would
// cast that queued body to a DiscoveryStatus and resolve it as an applied result: the account chooser
// would simply refresh, showing the old account selection with no toast and no error, leaving the operator
// to conclude their save had silently reverted. Reading through parseJsonOrOwnerAction instead, the same
// decoder its sibling setDiscoveryToken uses, gives the caller a discriminated { status:"queued" } to
// surface honestly. The gate-off path returns { status:"result", value } and is unchanged.
...
//
// The capture mode is the gated config mutation `cf-config-mode-set` (engine scheduler-do-routing.ts, the
// POST /cf-config/mode case, which dispatches through gatedConfigMutation), so with the change-control gate
// armed the engine answers HTTP 202 + { queued:true, id, status, contentHash } and applies nothing. Reading
// through plain parseJson would let that 202 sail through (it throws only on a non-2xx), casting the body
// to { ok, error } with `ok` undefined: the capture-mode select would then take its else branch and tell
// the operator "Could not change capture mode (failed)" for a request the engine had accepted and queued,
// reverting the select to the old mode. A false failure is the worse half of this class: it would drive the
// operator to retry, queuing a second identical change for the approver, or to believe the mode is
// unchanged when a second approver is about to change it. Reading through parseJsonOrPending, the decoder
// every other change-controlled mutation in this console uses, gives the caller a discriminated
// { status:"pending" } to surface honestly. The gate-off path returns { status:"applied", value } and is
// unchanged.
...
export async function changeBindings(t: Transport, token: string, add: AttachSourceInput[], remove: string[]): Promise<OwnerActionResult<{ attached: string[]; detached: string[] }>> {
  ...
  // sources-detached event, so unlike a successful change it is invisible in the pack, and this is the
  // owner's stated number-one fear domain. The op is decided HERE because it cannot be decided anywhere else: an attach
