...
// lastRestoreTestAt to an ISO string, so every consumer reads the one normalised contract. Without this
// mapping the engine's restoreProven would be silently dropped and "offline restorability last proven"
// would always render "never"; the integrity stamp is mapped the same way so "integrity-checked" can
// render true once the engine emits it (on a clean run or a passed attestation). The bare-token prover
...
export function mapEngineDownpipeState(d: EngineDownpipeState): DownpipeState {
  // Contract drift: the deferral kind is a closed engine enum, and the spread below narrows it
  ...
  // data", and it is invisible without this guard. The class is recorded, never the value: the value is the
  // engine text the class stands in for. An absent deferral is the normal case and is not drift.
  ...
  return {
    ...
    // restoreProven.method is kept here (narrowed to the known union) so the proven / coverage surfaces
    // name the method faithfully. An unknown wire value drops out rather than rendering raw.
...
//
// It uses the cost library's average month, not its own: every other runs-per-month figure in the
// console comes from cadenceSecondsToRunsPerMonth (DAYS_PER_MONTH = 30.44, the 365.25/12 averaging
// constant the spec pins). A projection that steers an operator off a cadence has to agree with the
// screen that prices it, and the shared constant errs slightly high, which is the direction the cost
// engine's over-estimate stance already takes.
...
//                    allow-list the engine reports on the plan). The surface set is bound because it is
//                    part of what an approval authorises: widening it must invalidate an approval that was
//                    granted for a narrower one. `surfaces` is taken from RestorePlan.cfConfigSurfaces
...
export async function restorePlanHash(req: RestorePlanHashRequest): Promise<string> {
  const binding: PlanBinding = {
    ...
    // re-arms the gate. Included only when set, so a whole-run plan hashes the same way a request with no
    // recordName always has (matching the engine's spread-when-present construction).
