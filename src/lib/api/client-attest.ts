...
// classifier and the copy layer both read, and folding a bare `error` over it loses the reading: without
// this, an attested verification refused by the capability gate would render as a role denial the console
// never established.
...
export async function createAttestSession(t: Transport, input: { scope?: string[]; sampleRate?: number } = {}): Promise<CreateAttestSessionResult> {
  ...
  // blockError with a Retry) instead of returning a session whose unguarded `session.runs.length` would
...
export async function attestStatus(t: Transport, sessionId: string): Promise<{ session: AttestSessionRecord }> {
  ...
  // resumeSession then reads `.runs` off directly.
