...
// functions over the shared Transport, each building the request URL, method, body and headers and parsing
// the response.
//
...
// parseJsonOrOwnerAction is the correct read for exactly this reason: a plain parseJson cannot tell a
// queued 202 from an applied one, so it would cast the queued body {ownerActionQueued,id,status} straight
// to ConfigApprovalPolicy, and requireConfigApproval would read back as undefined (falsy) -- an owner who
// queued a disarm would be told the gate had already turned off, while it is, correctly, still on.
...
// setSignInContextPolicy turns the OWNER-OPT-IN unusual-location sign-in notify (R6, V6.3.5) on or off.
