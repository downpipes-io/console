...
// emit site for the console-diagnostics ring, which ships the engine-call kind on its own.
//
...
// honest attempt denominator (the bot reasons over a failure ratio, never a raw count, and the denominator
// must stay honest precisely when every call is failing), and adds no per-screen ceremony.
//
...
// EngineFetchOpts.adminOp names the privileged write this call is making, and it is supplied by the call
// site, which is the only place that knows. It is not inferred here, and the reason is the paragraph above:
// this function is contractually forbidden from reading the URL, and it stays that way. A route would have
// to be parsed to infer the op, a path can carry a customer id, and one route serves two ops (an attach and
// a detach are the same POST, told apart only by the arguments the console itself built).
//
...
//
// The console's own 503 is not an engine fault. When the console worker has no ENGINE service binding it
// answers the engine surface itself, with a 503 (src/worker.ts). Classified on the numeric status alone,
// this would read as {engine-call, 5xx, server}: a fault class whose meaning is "the engine answered and
// failed", for a request the engine never received. That would point the reader at engine logs that hold
// no trace of it and feed the bot's console-engine-calls-failing ratio with a fabricated engine 5xx. The
// truth is the one the transport classifier already reaches (classify.ts maps the engine-binding-absent
// kind to `transport`, not `server`): the call did not reach the engine, so no engine-side evidence of it
// can exist. The engine-call row is still written, and must be: every call from this console is failing,
// the attempt denominator counts them all, and a seam that dropped the fault would report a healthy call
// ratio on a console where nothing works. It is written with the class the code established.
//
...
export function noteEngineResponse(r: Response, adminOp?: ClientDiagAdminOp): void {
  ...
  //
  // Classified on the bare status alone, this would read as {httpClass: "5xx", faultClass: "server"}, and
  // `server` says in as many words that the engine saw the call and refused it. The wizard would then file it
  // as `engine-not-ok` and the bot would tell the support engineer "the evidence is in the engine's own logs,
  // go and read the refusals there. Do not chase reachability." There are no refusals to read: the engine is
  // down or absent, and that reading would say it was up, asserting a fact the code never established.
  if (isConsoleOriginFaultResponse(r)) {
    if (adminOp !== undefined) recordAdminWrite(adminOp, "unreachable");
    recordEngineCall("network", "transport");
    return;
  }
  // The same rule again, for the 5xx the console worker did not live to stamp. The header gate above only
  // catches a 500 the worker answered. When the worker never runs (over its CPU or memory limits, script
  // gone, an edge fault), Cloudflare answers for it with its own HTML error page, which carries no header of
  // ours; without this check the seam would file it as {5xx, server}: the engine saw the call and failed. In
  // the proxied topology the engine is a service binding and cannot answer HTML at all, so it did not.
  //
  // This seam may not read the body (a body can be read once, and the transport must have it), so the test is
  // the content type, which is a header: an engine 5xx is jsonError JSON. It is a substring test against a
  // constant this repo ships and it copies nothing out of the response.
...
export async function engineFetch(url: string, init?: RequestInit, opts: EngineFetchOpts = {}): Promise<Response> {
  ...
  try {
    ...
    // fault, so it is excluded from the failing signal.
    //
