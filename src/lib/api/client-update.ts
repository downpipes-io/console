...
// Transport.
//
// The console drives the engine's own brick-safe self-update: read status, preview (dry-run) or apply
// (promote), then settle (canary-gate -> keep or auto-rollback). The deploy token is one-shot and held
// only in browser memory for the apply -> settle pair; api.ts never persists it (no storage write here)
// and never carries it to the vendor, it rides in the request body to the in-account engine and is gone.
...
export async function applyUpdate(
  ...
  // branch inline. Keeping the decode in one place is what carries the malformed-202 honest-throw here too: a
  // 202 whose body is not the OwnerActionQueued shape throws the honest answer-unreadable error instead of
  // falling through to the structured parse and coercing a malformed-but-valid body (even a bare `{}`) into a
  // false "deployed", at the highest stakes, a live apply. The only shape difference is the queued
  // discriminant (parseJsonOrOwnerAction carries the full OwnerActionQueued body, applyUpdate surfaces just
  // its id), so map queued -> { id } and pass the structured result through unchanged. Every throw (the error
  // fold, failResponse, the malformed-202) propagates the same way.
...
export async function rampUpdate(t: Transport, opts: { token: string; percentage: number; allowDowngrade?: boolean }): Promise<UpdateApplyResult<RampResult>> {
  ...
  // Same as applyUpdate: route the 202-queued decode, the non-2xx { error } fold and the structured parse
  // through the shared parseJsonOrOwnerAction decoder so the malformed-202 honest-throw lives once, never
  // reimplemented inline. A ramp is a live production traffic shift, so a malformed 202 read as a false
...
// rollbackUpdate is the STANDALONE one-click revert (POST /admin/update/rollback): revert to the recorded
...
export async function rollbackUpdate(t: Transport, token: string, components?: UpdateComponentId[]): Promise<StandaloneRollbackResult> {
  // The rollback exists for a known-bad version that stays live while retries keep failing. Its applied
...
// rollbackPlan reads what a STANDALONE ENGINE rollback would do WITHOUT
// spending the one-shot deploy token: the console's paired-rollback confirm needs to know, BEFORE
...
// older engine degrades to the plain rollback experience, never a blocker and never a surfaced error.
...
// echoed back.
//
// The keep direction of a settle is the gated owner action `update-settle` (engine router-updates.ts: when
// the canary's verdict is keep and the pending release is migration/breaking-class, the handler calls
// ownerActionGate and answers ownerActionQueuedResponse, HTTP 202 + { ownerActionQueued:true, id, status },
// without settling; a rollback verdict is the safe direction and is never gated). Reading through plain
// parseJson would cast that queued body to a SettleResult with no `outcome` field: settleWithBudget's ladder
// treats a non-definitive outcome as "not yet" and retries, so a queued settle would burn the whole retry
// budget re-submitting, then fall through to the persisted-record recovery and tell the operator the settle
// outcome could not be read, when the engine had in fact accepted the settle and was waiting on a second
// owner. Reading through parseJsonOrOwnerAction (the decoder applyUpdate and rampUpdate already use for the
// same gate on the same surface) gives a discriminated { status:"queued" } the flow can surface instead of
// retrying it. The queued discriminant is mapped to its id, matching applyUpdate/rampUpdate's
// UpdateApplyResult shape; the gate-off path is { status:"result", value } and is unchanged. Every throw (the
// error fold, failResponse, the malformed-202 honest throw) propagates the same way, because
// parseJsonOrOwnerAction runs the same readErrorReason fold on a non-2xx that a hand-rolled block would.
...
// settleRampUpdate is the RAMP's phase 2 (POST /admin/update/ramp/settle), and it is the control a ramped
// update needs to reach. The engine's plain settle REFUSES a ramp-shaped pending ("this pending verification
// is a gradual ramp; use the ramp settle call instead"), so a customer who started a gradual ramp needs a
// control that reaches this route, and customers never run a terminal. The pending card routes on the
// pending's OWN shape (a recorded `percentage` means a ramp), so a ramped pending settles here and a plain
// one settles at settleUpdate above.
//
