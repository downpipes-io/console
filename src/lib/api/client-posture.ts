// Security-centre posture + coverage/gap-detection domain functions for the EngineClient. Free functions
...
//
// Storing the inventory is the gated config mutation `coverage-inventory` (engine
// scheduler-do-routing-signals.ts, the POST /coverage/inventory case, dispatched through
// gatedConfigMutation), so with the change-control gate armed the engine answers HTTP 202 +
// { queued:true, id, status, contentHash } and stores nothing. Reading through plain parseJson would cast
// that queued body to a CoverageInventoryStored: the security centre's populate action destructures
// `result.counts`, which is absent on a queued body, so the save would throw a raw TypeError instead of
// reporting that the inventory is queued for approval. Reading through parseJsonOrPending instead gives
// the caller a discriminated { status:"pending" } and, on the gate-off path, the same
// { status:"applied", value } counts.
//
// parseJsonOrPending folds a non-2xx reason only for the statuses that carry a plain { error } (a 400/422
// validation refusal), leaving 401/403/429/5xx to failResponse's own markers so a 403 keeps its
// FORBIDDEN_CLASS_MARKER and classifyError can read the refusal class, matching every sibling mutation in
// this client.
