...
//
// The Clear control runs requireChange (screens/settings/push.ts), so under Require Change Number the
// operator is asked for a change number, and this optional parameter carries that reference through to
// the write.
//
// The engine half is open: /push/delete is owner-exclusive but is not an owner action, so it never reaches
// gatedOwnerAction and therefore never reaches enforceChangeControl, which is the only thing that writes
// the change-recorded audit event. Until that route enforces, the reference arrives on the caller and
// nothing records it, so a cleared push destination still has no change request against it in the
...
export async function testPush(t: Transport): Promise<PushTestResult> {
  // The engine returns httpStatus as a number, so the 4xx/5xx split needs no text at all, and the
  // egress-guard refusal (the endpoint was never called: an allowlist decision, not a vendor fault) is the
  // one class an operator most often misreads as their SIEM being down. The endpoint URL and the vendor's
  // body never ride.
  try {
