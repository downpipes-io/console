// Role-based access control domain functions for the EngineClient: the in-account role table, the
...
// console mirror is UX only). The last-Owner guard and maker-side checks live in the engine.
...
// second approver. Plain parseJson only tests `!r.ok`, and a 202 is ok, so it would parse the queued body
// and resolve as a successful delete, letting a screen toast "Removed <member>" and drop the row while the
// member's full access persists. parseJsonOrPending is what tells applied from queued: a 202 with the
// pending shape resolves to { status:"pending" } the caller surfaces as "queued for approval", and a
// malformed 202 throws the honest answer-unreadable error, never a false removal.
...
// owner): the capability model lets a group claim confer viewer/operator/restore-operator/approver/
// access-admin, but never owner (owner stays explicit per-email). The engine enforces this at the
// authority boundary with a 400, and the type prevents the caller from even constructing an owner
// request. exactOptionalPropertyTypes: no optional fields here.
...
// deleteGroupRole removes a group->role mapping. group-role-delete is a change-control gated mutation
// (engine maps POST /group-roles/delete -> gatedConfigMutation("group-role-delete")): with the gate armed
// the engine answers 202 { queued: true, id } and writes nothing, so the mapping still stands, queued for a
// second approver. Plain parseJson would coerce that queued 202 into a false "mapping removed", the same
// risk as deleteRole. parseJsonOrPending, the decoder its sibling setGroupRole uses, resolves the queued
// 202 to { status:"pending" } honestly and throws answer-unreadable on a malformed one, never a false
// removal.
...
//
// custom-role-delete is a change-control gated mutation (engine change-control.ts). With the gate armed
// the engine answers 202 { pending: true, id } and writes nothing: the role still exists, queued for a
// second approver. Plain parseJson only tests `!r.ok`, and a 202 is ok, so it would parse the queued body
// and resolve as a successful delete, letting a screen toast "Deleted custom role", reload a catalogue
// that still held the role, and record a downgrade of members that had not happened. parseJsonOrPending,
// the same decoder its sibling createCustomRole uses, is what tells the two apart.
