// The EngineClient: the typed transport to the in-account engine admin API.
// Re-exported verbatim from ./api.ts so the existing callers are unchanged.
//
// STRUCTURE: the behaviour is factored into a leaf transport (client-transport.ts: the shared headers /
// gatedFetch / parseJson* / readErrorReason plumbing) plus one module per domain group (client-<domain>.ts)
// of free functions over that transport. EngineClient exposes one method per engine capability, and each
// method is a thin delegator to its domain function. The leaf transport is imported by every domain module
// and the domain modules are imported here, so there is no import cycle (the transport never imports a
// domain module or the client).
...
// Its surface is the constructor, the public onStepUpRequired hook, two static start-path builders, and
// one method per engine capability, each a one-line delegation to its domain module over a shared
// Transport.
export class EngineClient {
  ...
  // ---- Session management --------------------------------------------------------------------------
  bootstrapSession(): Promise<{ ok: boolean; method: string }> { return session.bootstrapSession(this.t); }
  listSessions(): Promise<{ sessions: SessionSummary[] }> { return session.listSessions(this.t); }
  terminateSession(sid: string): Promise<{ ok: boolean }> { return session.terminateSession(this.t, sid); }
  ...
  // The engine's discriminated union, not a flattened re-declaration. A flattened shape would hide a real
  // bug from tsc: it would make deleteProbe look reachable on the failure arm (it is not) and make ok:true
  // look unable to carry a denied delete (it can, and that IS the WORM case).
  ...
  // clientDiagnostics (optional) rides the pack build as a request-scoped section: supplying it
