...
// builders), each building the request URL, method, body and headers and parsing the response.
//
...
export async function testIdpConnection(t: Transport, proposal: IdpPresetCreate | IdpSamlProposal): Promise<IdpTestResult> {
  // The IdP probe is the product's deepest vendor diagnostic (OIDC discovery, JWKS reachability,
  // SAML metadata and certificate validity) and it is computed, drawn once and kept nowhere, so "the IdP
  // test showed a red cert check yesterday, what did it say?" has no answer. The engine answers 200 on every
  // outcome (the checks carry the verdict), so the fail class must come from the check STATUSES, not the
  // status code. The check `detail` lines carry the certificate subject and the discovery URL and are never read.
  try {
...
export async function rolloverIdpSigningCerts(
  ...
  // Sensitive owner mutation: routed through gatedFetch so an engine 401 { stepUpRequired } runs the
  // step-up ceremony and retries once. The OPTIONAL change reference rides as X-Downpipes-Change.
  // adminOp names the ROLLOVER as its own admin operation, distinct from the connection upsert. This is
  // safe to be precise about only because the engine admits the member: it checks adminOp by set membership and fails
