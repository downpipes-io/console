// The shared transport plumbing for the EngineClient: the single god class is split into a leaf transport
// plus per-domain modules of free functions, each delegated to by a thin EngineClient method. This module
// is the LEAF: it imports only the error helpers and the wire types, never a domain module or the client,
// so the domain modules can import it and the client can import them without forming a cycle (madge stays
// clean).
...
import { ACCESS_REAUTH_MARKER, ACCESS_REDIRECT_MARKER, CONSOLE_ORIGIN_FAULT, ENGINE_BINDING_ABSENT, FORBIDDEN_CLASS_MARKER, HTML_BODY_MARKER, STEPUP_REQUIRED_MARKER, classifyForbiddenBody, detectAccessReauthBody, detectAccessRedirectBody, detectHtmlBody, detectStepUpRequiredBody, isConsoleBindingAbsentResponse, isConsoleOriginFaultResponse, RESTORE_UNAPPROVED_REASON, RATE_LIMIT_MARKER } from "../errors.ts";
...
export class Transport {
  ...
  // token is the bare ADMIN_TOKEN for the moments between the operator pasting it and the engine exchanging it
  // for a session; discardToken() clears it so the secret never rides a later request.
  ...
  // discardToken forgets the bare ADMIN_TOKEN once it has been exchanged for a minted session: from here on
  // every request rides the __Host- session cookie (credentials: "include"), and the static secret is
  // neither held nor sent again.
  discardToken(): void {
  ...
  // gatedFetch wraps a sensitive-action fetch: on a 401 { stepUpRequired:true } it runs the injected
  // step-up re-auth ceremony (a fresh passkey assertion -> a single-use token) and retries once with the
  // x-downpipes-stepup header. With no ceremony wired, or a cancel/failure (token null), the original 401 is
  // returned so the screen surfaces it. A non-401, or a 401 that is not a step-up prompt, passes through.
  //
  // Console-diagnostics note: the ceremony opens with a 401 { stepUpRequired: true }, which is a normal
  // protocol handshake, not a fault. So the diagnostics response record is deferred here and taken only on
  // the paths where the non-2xx is genuinely surfaced to the operator (no handler wired, a body that is not
  // a step-up prompt, or a cancelled/failed ceremony). The successful-ceremony path records the retry's own
  // outcome instead, so a working step-up leaves no phantom `auth` fault in the ring and cannot drive a
  // false console-engine-calls-failing signal. The attempt is counted on both legs either way. `adminOp`
  // names the privileged write this gated call is making, and rides the same deferral as the engine-call
  // record: the ceremony's opening 401 must not be recorded as a `denied-role` refusal of the write, or
  // every successful step-up would put a phantom permissions denial in the pack for the very save it went
  // on to let through. The write's outcome is taken on the paths where the non-2xx is genuinely surfaced,
  // and on the retry, which is where the real answer is.
  async gatedFetch(path: string, init: RequestInit, opts: { adminOp?: ClientDiagAdminOp } = {}): Promise<Response> {
    ...
    let accessReauth = false;
    try {
      const body = (await r.clone().json()) as { stepUpRequired?: boolean; reauth?: unknown };
      stepUpRequired = body.stepUpRequired === true;
      accessReauth = stepUpRequired && body.reauth === "access";
    } catch {
      noteEngineResponse(r, adminOp);
      return r;
    }
    // The Access form of the step-up 401 asks for a fresh Access sign-in, not a passkey. An Access session
    // has no passkey to assert with, so running the ceremony here would open a prompt that can never satisfy
    // the engine; the raw 401 is surfaced and failResponse folds it as ACCESS_REAUTH_MARKER for the error view.
    if (!stepUpRequired || accessReauth) {
      noteEngineResponse(r, adminOp);
      return r;
    }
  ...
  // own login HTML, often with a non-2xx, which would otherwise read as "engine down". When detected
  // it throws a message carrying ACCESS_REDIRECT_MARKER so classifyError maps it to
  // { kind: "access-redirect" } ("your Cloudflare Access session needs refreshing"), not a
  // server/network fault. No-custody: the body text is inspected for the redirect and then discarded;
  // it is never logged, stored, or transmitted, and only the marker (no body content) rides in the
  // thrown message.
  async failResponse(r: Response, verb: string): Promise<never> {
    ...
    // because this response did not come from the engine and must not be classified as if it had: a 503 read
    // as a server fault sends support to the engine's logs, where there is nothing to find (the engine never
    // received the request), and falling through to the HTML-body check below would read it as
    // `html-not-engine`, which sends the operator to correct an engine URL that is correct.
    //
    ...
    //
    // Two independent admissions of the same frozen token exist as a pair on purpose: the header (which the
    ...
    //
    // Without this admission the seam would ignore the header and throw the bare "<verb>: 500", and every
    // reader downstream would classify it on the status alone: the feature probe would file `server-error`
    // (whose meaning is "the engine is broken and it saw the call, so read its refusals"), the wizard would
    // file `engine-not-ok`, and the block error would tell the operator the engine had returned an error.
    // There are no refusals to read. Worse, the response seam already reads the header and correctly records
    // the engine-call row as {network, transport}, so the pack would carry two rows that contradict each
    // other, and the throw-derived one is the row support reads for "is it broken or not built".
    //
    ...
    // 500 by a header this console stamps, so it can only catch a 500 the console worker lived to answer. When
    // the worker never runs (over its CPU or memory limits, script gone, an edge fault) Cloudflare answers for
    // it with its own HTML error page (1101, 1102, 1027), which carries no header of ours and would otherwise
    // arrive as a bare 5xx: `server-error`, the row whose meaning is that the engine saw the call and refused
    // it.
    //
    ...
    //
    // In the split topology the identical page is most often Cloudflare's 1101 in front of a real, broken
    // engine, which is what the HTML rule below says. The topology flag is false there, and false whenever the
    ...
    // HTML on a non-2xx is not the engine, and only a 200 says so elsewhere: parseJson throws HTML_BODY_MARKER
    // when a web page answers where engine JSON was expected, but only on the success path. Without this
    // branch the ordinary wrong-address shape (an engine URL pointing at a static host or a proxy, which 404s
    // an HTML page) would take the plain status throw and become a bare 404, byte-identical to a real engine
    // answering a route it genuinely does not serve. The console's whole pending-the-engine tile rests on
    // telling those apart: in the first there is no engine at that address and the tile would be a lie the
    ...
    //        still HTML on a 401). A 401 records nothing, deliberately, and an HTML body must not be the thing
    //        that turns the ordinary lapsed-session state into "your engine address is wrong".
    ...
    // classifyError can pace the bulk loops by it. The trailing status stays the last token, so a non-429
    // throw is byte-unchanged and extractStatus still reads the status. A missing/non-numeric Retry-After
    // degrades to the plain status throw (the loop then uses its default backoff).
    if (r.status === 429) {
    ...
    // capability gate, the DO's anti-enumeration AuthError funnel, the CSRF-origin check that pre-empts the
    // whole route dispatch for every cookie-borne mutation, and a WAF block page the engine never saw), and a
    // throw that is byte-identical for all four would let the dual-control inbox file "a deploy dropped
    // CONSOLE_ORIGIN on the engine and every save in this console is 403ing" as a governance fault against an
    // owner. The class is decided by a shape gate over the body (classifyForbiddenBody: parse, equality
    // against tokens this console defines, a typeof test on `required`) and folded in as a closed member on
    // the RATE_LIMIT_MARKER pattern, so the trailing status stays the last token and extractStatus is
    // unchanged. No byte of the body rides: only the enum member.
    if (r.status === 403) {
      throw new Error(`${verb}: ${FORBIDDEN_CLASS_MARKER}=${classifyForbiddenBody(body)}: ${r.status}`);
    }
    // A 401 that still carries stepUpRequired reaches this generic thrower only because a step-up
    ...
    if (r.status === 401 && detectAccessReauthBody(body)) {
      throw new Error(`${verb}: ${ACCESS_REAUTH_MARKER}: ${r.status}`);
    }
  ...
  async parseJson<T>(r: Response, verb: string): Promise<T> {
    ...
    try {
      ...
      // copy prescribes checking the URL/deployment, never a re-authentication.
      if (detectAccessRedirectBody(text)) throw new Error(`${verb}: ${ACCESS_REDIRECT_MARKER}`);
      // Contract drift: a 2xx whose body is not the JSON the contract describes. The Access-redirect case
      // above is deliberately not recorded: a lapsed Access session is a normal condition of the perimeter,
      // not engine drift, and recording it would flood the ring with the same false positive the step-up 401
      // would have been. Only the class is recorded; the body is never read into a field (it is inspected
      // locally for the two shapes and discarded).
  ...
  // parseJsonOrReason is parseJson plus a reason-fold, for a mutation that returns a plain record (never a
  // 202-pending) but whose refusals carry an actionable reason the operator must read. A 400/422 whose body
  // carries the engine's coarse `error` is thrown with that reason (superseded, decided-by-another,
  // maker-is-checker, self-approval, unknown-downpipe, which-artefact-is-wrong), while 401/403/429 and 5xx
  // keep their own failResponse markers (stepup / forbidden-class / rate-limit / server), the same status
  // gate parseJsonOrPending uses, so a role denial still reads "Not permitted", never a 400 sentence. Used
  // by the dual-control approve/reject family, the run-trigger and the DR break-glass reconcile, each of
  // which needs that reason rather than a bare "<verb>: <status>".
  ...
  // being silently coerced into a false "applied"; a non-2xx still routes through failResponse so an
  // Access-redirect body is named honestly. The 202 is the only status that means "pending"; a 200 always
  // means applied, so a gate that is off behaves the same way (the applied record flows straight through).
  // No-custody: the pending body carries an id, a kind and a plain-English diff only (no value, no key); we
  // read its id and pass the rest through untouched.
  async parseJsonOrPending<T>(r: Response, verb: string): Promise<MutationResult<T>> {
    if (!r.ok) {
      // A plain validation refusal (a 400/422 that is not one of the special-marker statuses failResponse
      // folds a closed class for) carries the engine's coarse reason in body.error, so fold it in (status
      // trailing) exactly as parseJsonOrOwnerAction already does for owner mutations. Without this the
      // operator would see only a bare "<verb>: 400" and lose the reason (e.g. the SSRF default-deny
      // sentence on a notify channel save). 401/403/429/5xx keep their own failResponse markers (stepup /
      // forbidden-class / rate-limit / server), so the reason fold is gated out for them.
    ...
    try {
      ...
      if (detectAccessRedirectBody(text)) throw new Error(`${verb}: ${ACCESS_REDIRECT_MARKER}`);
      recordContractDrift("malformed-body"); // same discipline as parseJson: class only, Access excluded
      if (detectHtmlBody(text)) throw new Error(`${verb}: ${HTML_BODY_MARKER}`);
      throw new Error(`${verb}: ${r.status}`);
    }
    ...
    if (r.status === 202) {
      ...
      // A 202 says "queued for a second approver", but this body does not carry the pending shape the
      // contract promises. Falling through and reading a shape mismatch as applied (a bare `{}` body
      // resolving to { status:"applied", value:{} }) would let toggleEnabled and every other caller show a
      // success toast and close the drawer while the engine's own state never moved. A 202 whose body cannot
      // be trusted must never resolve to a value: throw the same honest "answer-unreadable" shape parseJson
      // already throws for an illegible 2xx body. classifyError maps "<verb>: 202" to
      // { kind: "answer-unreadable", status: 202 }, "the engine answered, only the response could not be
      // understood", never a false success and never a bare reachability fault either. The class alone is
      // recorded (never the missing field's name, which could itself be engine- or customer-derived).
  ...
  //
  // A 401 that carries stepUpRequired:true must return null here, before the generic `error`-string
  // read below, never a plain reason string. The engine's step-up 401 (requireStepUp,
  // engine/src/admin/router-core.ts) answers { error: "step-up required", stepUpRequired: true }: a
  // well-formed, non-empty `error`, so returning it here would bypass the step-up handling. Every
  // caller of this method shares the same shape: read a reason; if present, throw
  // "<verb>: <reason>: <status>"; otherwise fall through to failResponse(), and failResponse already
  // folds a stepUpRequired 401 into STEPUP_REQUIRED_MARKER via its own shape gate
  // (detectStepUpRequiredBody). Returning the reason here instead would shadow that gate: the thrown
  // message would read "<verb>: step-up required: <status>", which does not contain the marker token
  // ("stepup-required", no hyphen after "step"), so classifyError would never match it, would fall
  // through to the bare 401 branch, and isUnauthorised(err) would read true. A step-up gap (the
  // operator is authenticated and needs to prove presence for a sensitive action) would then be
  // misread as a lapsed session and sign the operator out, discarding a valid session at the exact
  // moment they were doing something that mattered. The shape is shared by every caller of this
  // method (client-attest.ts, client-keys.ts, client-session.ts, client-posture.ts, and
  // parseJsonOrOwnerAction's own inline read above), so this one guard covers all of them.
  //
  ...
  // 401 body that carries an `error` but not stepUpRequired:true still returns its reason.
  ...
  // so an Access-redirect body is named honestly. The 202 is the only status that means "queued"; a 200
  // always means applied, so the gate-off path (the default) behaves the same way. The 202 body is read
  // defensively (isOwnerActionQueued): a 202 that is not the OwnerActionQueued shape throws the honest
  // "answer-unreadable" error rather than being silently coerced into a false "result": it never fabricates a
  // queued id no inbox could resolve, and it never fabricates an applied value the engine never sent.
  // No-custody: the queued body carries an owner-action record id only (no value, no key).
  async parseJsonOrOwnerAction<T>(r: Response, verb: string): Promise<OwnerActionResult<T>> {
    if (r.status === 202) {
      ...
      // The engine deferred to a second owner (202) but this body is not the OwnerActionQueued shape, neither
      // well-formed JSON matching the contract nor parseable at all (readOwnerActionQueued returns null for
      // both). Falling through to the ordinary 2xx path below would parse a malformed-but-valid body (even
      // `{}`) and resolve it as an applied value, telling the operator a destination/connection/token change
      // had landed when the engine's own state never moved. A 202 whose body cannot be trusted must never
      // resolve to a value: throw the same honest "answer-unreadable" shape parseJson already throws for an
      // illegible 2xx body (classifyError maps "<verb>: 202" to { kind: "answer-unreadable", status: 202 }),
      // never fall through to a false "result". The class alone is recorded (never the missing field's name,
      // never the body).
      recordContractDrift("missing-field");
      throw new Error(`${verb}: ${r.status}`);
    }
...
// isEngineBindingAbsentBody is the shape gate on the console worker's own 503 body. It parses the text as
