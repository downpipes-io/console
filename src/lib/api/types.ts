...
//
// The definitions live in sibling modules under ./types/<domain>.ts and are re-exported here, so
// every importer (api.ts does `export *` from this file) sees the identical surface. The only
// cross-module type references are the engine identity mirror (Role / AuthMethod / Capability /
// CustomRole, imported from ../identity.ts where needed) and CanaryLiveness (canary -> updates).
