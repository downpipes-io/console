// Restore dual-control + restorability-assurance domain functions for the EngineClient. Free functions
...
// with maker != checker enforced server-side. requestRestore raises a request (Operator+);
...
// types/restore-types.ts, where its contract (the engine's cross-check, the plan-hash binding of
// recordName / cfConfig, and the no-custody guarantee) is documented.
...
// rejectReason is the checker's closed verdict and it is required, so a rejected restore always carries an
// actionable reason for the requester and for support alike. It is a closed member, never a typed
// sentence: the approver's prose would name the customer's own data and it rides into the sealed pack.
