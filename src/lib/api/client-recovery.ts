// Recovery-code domain functions for the EngineClient. Free functions over the shared Transport.
//
...
// confirmRecoveryCodes promotes a STAGED recovery-code set to live: it is what the save-confirm panel's
// "Continue" must call, for an enrolment whose finish answered recoveryCodesPending:true, before it hands
// control back to the caller. Until this call the old set the operator held before the enrolment keeps
// signing them in; a page that closes or a request that fails before this fires simply leaves the old set
// live, which is the safe outcome. A confirm for an email with nothing staged answers promoted:false and is
// not an error (the operator is already signed in and their live set is already correct either way).
//
