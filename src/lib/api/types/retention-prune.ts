...
// non-secret capsule wraps only, never a value or a recovered master.
...
// second-authority approval, mirroring RestoreApproval's request -> approve -> apply structure (never
// reusing RestoreApproval itself, which carries restore-specific blast-radius cues -- target binding,
// isLatest, bytes -- this action has none of). Bound to a KEYLESS planHash over the downpipe id + its
// CURRENT retained/superseded split, so a request can be raised and reviewed before anyone recovers a
// master.
// "applying" mirrors the engine's own live status (engine/src/admin/prune-approvals.ts:33): the atomic,
// lease-guarded reserve between approved and consumed/rejected-back-to-approved. Every status needs a
// case in renderPruneApprovalCard: a status with no case falls through to a bare badge with no
// explanatory sentence, unlike every other status.
...
// isPruneApproval / isPruneApprovalList mirror the same narrowing discipline used throughout this file: a
// route the engine (or a demo/stub standing in for it) has not modelled is indistinguishable from a real
// success by status code alone, so the response shape is checked before any field is indexed.
