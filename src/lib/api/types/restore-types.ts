// Restore request / plan / result / blind-test mirror types. See ../types.ts for the barrel.
...
// and RunHistoryEntry live in status.ts.)
...
export interface RestoreRequest {
  ...
  exclude?: string[];
  maxRecords?: number;
  // recordName (GRANULAR single-record restore, E4/C1) mirrors the engine's RestoreRequest.recordName
...
// RECOMPUTES server-side from the plan, rejecting on any mismatch. They are typed here
...
export interface RestorePlan {
  ...
  // at the type boundary, which is exactly what test/validate-engine-field-coverage.ts guards against.
  //
  ...
  // cfConfigSurfaces mirrors the engine: the RESOLVED cf-config surface allow-list the plan was
  ...
  // operator matches the one an approval binds to across account, zone and surfaces.
  ...
  // silently discards, which would throw away evidence about a reduced-fidelity restore on the receipt
  // screen. Absent must read as "nothing to warn about", never as "checked and fine": an older engine does
  // not send it.
...
export interface RestoreResult {
  ...
  // skipReasonCounts is the CLOSED {class: count} map BEHIND that bare `skipped` integer, so the console
  // can show a surface skipped and show WHY. The integer alone cannot tell "your plan caps DNS records"
  // from "the restore token lacks the edit scope" from "the snapshot item is malformed", and those are
  // three tickets with three different remedies. Every key is a CfConfigSkipClass member and every value a
  // count, so it is safe to render and safe to pack; the raw Cloudflare message stays in the operator's
  // live response and is never recorded.
  ...
  // FIDELITY SHORTFALLS. Each is the engine recording something a restore did NOT fully carry across:
  // d1SchemaObjectsFiltered exists "so an app that breaks after a complete subset restore has an answer",
  // metadataFieldsDropped because dropped TTLs would otherwise be "SILENT behind a receipt claiming full
  // fidelity". Declaring them here keeps that evidence from being discarded at this type boundary, so the
  // receipt does not call such a restore clean with a success tick. Optional, because an older engine
  // sends none of them.
  ...
  // that does not yet emit them leaves them absent, which the console reads as a complete apply and
  // falls back to the derived count. Redaction-safe (counts and a boolean only).
  outOfWindow?: number;
  windowed?: boolean;
  complete?: boolean;
  reason?: string;
