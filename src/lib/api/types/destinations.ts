// Archive-destination mirror types. See ../types.ts for the barrel.
...
export interface DestinationInput {
  ...
  //
  // DECLARED HERE RATHER THAN LEFT IMPLICIT. A field that rides on the wire but is absent from the
  // contract lets a capability be silently dropped by the next person who reads the type and believes it:
  // a spread skips excess-property checks, so an undeclared field can transit without the type system
  // ever seeing it.
  azureEntra?: { tenantId: string; clientId: string };
}
