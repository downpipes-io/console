...
// console tone (a migration/breaking release is rendered with more care) and the dual-control rule
...
// to the running version means "up to date"); the rest is the per-component slice of the rich metadata.
...
// section. configured/verified/updateAvailable drive the tiles; the rich-metadata fields (changelog,
...
// unverified channel consult. It travels as this typed field rather than as free-text prose, so the console
// never has to re-derive the class by substring-matching a sentence that could be reworded and silently
// downgrade every recorded row to "unstated" with no gate catching it.
...
export interface UpdateStatus {
  ...
  // versionSkew (ADDITIVE, presence-safe: an older engine omits it and the console falls back to its
  // baseline behaviour) is the engine's three-way answer to where it sits relative to what the channel
  // recommends. updateAvailable is a boolean and cannot carry this nuance, so relying on it alone risks
  // advertising an update the engine's own apply path would refuse. "uncomparable" is the member that
  // matters here: the engine could not
  ...
  // ---- Rich metadata for the recommended version (present only when verified + the field is set) ----
  changelog?: ChangelogEntry[];
  impact?: string[];
...
//                final word, never a signal to call settle. Every reader's outcome switch must recognise
//                "applied" here as this terminal state (see update-apply-flow.ts's own outcome switch and
//                consoleWasApplied, shared.ts), not fall through to an "unexpected outcome" branch.
...
export interface SettleResult {
  ...
  steps: UpdateStep[];
  reason?: string;
  // confirmationPending (ADDITIVE): present + true ONLY on an "applied" outcome where the engine's relaxed
  // judgment (selfCheck gates on CRITICAL classes only) kept the update before the hourly canary had a
  // chance to sing on the new version in the background. The console renders this as its own calm
  // first-class outcome ("verified in the background, no action needed"), never as a plain "applied"
  // (which would overclaim) and never as "pending" (which would ask for a token that is not needed, a
  // keep needs none). Absent/false on every older engine and on a normal confirmed keep.
...
// StandaloneRollbackResult is POST /admin/update/rollback: a first-class, always-available revert to the
...
export interface StandaloneRollbackResult {
  ...
  // rollback would (toVersion, paired, pairedConsole) so the console can show the paired-rollback line
...
// RampResult is POST /admin/update/ramp (OPT-IN; TWO-PHASE, exactly like apply's promote/settle split):
// phase 1 verifies + uploads + shifts `percentage`% of LIVE traffic to the new version and returns
// immediately. It NEVER flies the canary: the request that shifted traffic is still executing the pre-ramp
// code, so an inline flight would necessarily re-test the OLD version and falsely report the ramped one
// healthy. A genuinely separate POST /admin/update/ramp/settle call (RampSettleResult below) verifies the
// ramped slice and keeps or rolls back. Outcomes (mirrors engine update-ramp.ts RampOutcome):
...
// RampSettleResult is POST /admin/update/ramp/settle (the ramp's phase 2): a genuinely SEPARATE
...
export interface UpdateSettledOutcome {
  ...
  //
  // "rollback-failed" and "applied-unconfirmed" are the engine's two worst update-path states: rollback-failed
  // means the canary rejected the new version, the automatic rollback deploy ITSELF failed, and the engine is
  // STILL SERVING the rejected build; applied-unconfirmed means the promote was accepted and the confirming
  // read-back could not be performed, so the live version is not proven. The engine writes both into this
  // field and clears `pending` and `rollbackNeeded` in the same DO write, so nothing else is left to tell the
  // operator: a union missing either member leaves every reader falling through its default arm, rendering
  // a green "Up to date" tile with no last-update line and no rollback control. The parity between this
  // union and the engine's own enum is CHECKED, not asserted, by test/validate-update-outcome-vocabulary.ts.
  ...
  // applied-confirmation-pending first-class outcome rather than a plain "applied". Present only
...
export interface UpdateStatusRecord {
  pending: null | {
    ...
    // percentage is THE DISCRIMINATOR BETWEEN THE TWO SETTLE ROUTES. The engine writes it onto a pending
    // record ONLY from the opt-in gradual ramp (never from the atomic apply, see engine
    // scheduler-do-records.ts), and its presence is what makes POST /update/settle and POST
    // /update/ramp/settle mutually exclusive: each refuses the other's shape. The pending card must read
    // this field to route to the matching settle call, or a ramped pending is met with the engine's "this
    // pending verification is a gradual ramp; use the ramp settle call instead" refusal with no way to send
    // the other call. It is the live traffic percentage the ramped version is serving. Absent on every
    // atomic pending.
  ...
  // resolve a console version id as its target). Without this field, a console-only apply's own outcome
  // has nowhere to be read from, so both the live apply flow and a re-read of this record after a reload
  // have no way to recognise it as settled (see recordedSettleOutcome's `component` parameter). Absent =
  // no console component has ever settled here.
  lastConsole?: null | UpdateSettledOutcome;
  // rollbackNeeded is set when the HOURLY CANARY found a promoted-but-unsettled new version
