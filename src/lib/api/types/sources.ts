...
//
// gated vs unavailable (engine cf-config-discovery.ts) are BOTH surfaces the probe could not capture, but
// distinguish why: `gated` is a DEFINITIVE Cloudflare account-plan/entitlement refusal (the account's plan
// does not carry the product, benign, nothing to capture, no remedy available to the operator); `unavailable`
// is everything else (a token-scope gap, a transient fault), which IS actionable -- typically a permission
// group missing from the discovery token (docs operations/cloudflare-config-backup-restore.mdx, "Permission
// groups the 'Read all resources' template does not reliably include"). Distinguishing the two lets the
// drawer tell a plan limitation apart from a token to fix. Both are non-optional: the engine's probeCfConfig
// always returns them (an empty array, never absent).
...
export interface SourceDiscovery {
  ...
  // missing inBand must read as false, never as "assume it restores": the field exists so the picker
  // never treats every surface as equal without evidence.
  ...
  // engine binding (the discovery token is the credential), so the engine records which were added so
  // the wizard offers only added types, the same add-then-protect discipline a bound source has.
  // BACKWARD-COMPATIBLE: an OLDER engine (or the env-token IaC fallback) omits this field, and the
  // console falls back to the legacy "all supported" wizard behaviour; a PRESENT array (even empty)
  // means the engine gates, so the wizard shows only its members.
  addedSources?: string[];
}
