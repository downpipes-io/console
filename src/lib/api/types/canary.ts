// Canary backup mirror types. See ../types.ts for the barrel.
...
// "other" are genuine check problems. Optional: a record with no such field, or a dead/alive result, reads
// as absent. A break-glass-only flight never ails by posture, so the enum carries no "no-read-back-key"
// cause.
