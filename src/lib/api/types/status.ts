...
export interface LicenceStatus {
  // The seven tier ids the platform mints: Community (free) and Enterprise (bespoke services
  // subscription) either side of the five self-serve, estate-banded tiers, business-1/business-3/
  // business-10/business-25 (buy directly) plus msp (operator-mint only). No other ids are valid.
  // Display names are NOT a bare title-case of the id ("business-1" would read "Business-1"; "msp"
  // reads "MSP / MSSP"); screens/licence/../lib/billing.ts's tierDisplayName() is the one place that
  // maps every id (this map is display metadata only -- it never gates a feature).
  ...
  // src/admin/licence.ts LicenceReasonCode). `tier` is not a signal for this: the engine's fail-open
  // path resolves EVERY one of these causes to "community", so an expired licence is indistinguishable
  // from one that was never bought unless this field is read. `reason` is the same fact as human prose
  // and must not be parsed. This is the field to read.
  reasonCode?:
    | "no-token"
    | "no-pin"
    | "pin-invalid"
...
export interface RunHistoryEntry {
  ...
  // leaving in flight forever). It is a current, legitimate engine state, distinct from the future-skew
  // this union's enum guard exists to catch, so it needs its own member here rather than falling into the
  // guard's UNKNOWN bucket.
  ...
  // prevRunId / prevRunIdStatus let the console reconstruct a downpipe's per-run predecessor
  ...
  //   "unknown"  -- the row predates this field, so no predecessor pointer was recorded.
  ...
  // opCounts is the engine's EXACT per-resource metered-operation tally for the run: KV/R2/D1
...
export interface StatusReport {
  ...
  // build-id.ts, written at build): the real hash, never a placeholder string.
  ...
  // cfAccountId is the engine's own Cloudflare account id (env.CF_ACCOUNT_ID), mirrored so the Licence
  // screen's claim exchange can send it with a claim code (screens/licence/claim-code.ts fetchClaimToken)
  // and the control plane can bind the self-serve licence to it. This is the customer's OWN account id,
  // not a secret. Honestly absent (the common single-account deployment never sets CF_ACCOUNT_ID, and an
  // older engine predates this field entirely): the claim then sends no account id.
  cfAccountId?: string;
}
