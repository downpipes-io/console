...
// comparison the engine avoids, and a second derivation could disagree with the gate.
// gapServed mirrors the engine's ingest-pull-gap-served counter (support-ingest.ts): true once a poll on
// this grant has been served with gapBefore=true (the caller's checkpoint was older than the retained log's
// earliestSeq, so a permanent hole exists between the last-delivered event and the first new one; the engine
// resumes from the checkpoint it has, it does not refuse the poll). NOT YET SENT BY ANY ENGINE: redactGrant()
// does not populate this today, so it is always absent here and the banner it drives never renders until a
// companion engine-side change adds it. Optional and safe-absent, matching accessPerimeter/metrics above: an
// engine that never sends it must render as "no gap known", never as a false all-clear.
