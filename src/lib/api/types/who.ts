...
// in-memory view the shell holds, resolved from whoami or a degraded default until that call resolves;
// Caller is not a wire type, the engine resolves its own server-side):
...
//     mutable email a recycled account could inherit. The console keeps
