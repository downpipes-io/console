// Hash-chained audit + drill-evidence mirror types. See ../types.ts for the barrel.
...
export type AuditAction =
  ...
  | "restore-approve"
  | "restore-reject"
  // Dual control: a retention-prune APPLY deletes archive bytes outright, so it needs the same
  ...
  | "run-trigger"
  // run-failed: engine-observed (actorMethod "engine"), mirrored from the SRE alerting mixin's own
  // transition detector. Appended exactly once per genuine healthy-to-failed transition, never per
  // reconciliation tick and never on a healthy-to-stale transition. Target is the closed `run` kind
  // (runId only), the same target run-trigger already uses; no new target kind was needed.
  | "run-failed"
  | "role-change"
  | "group-role-change"
  | "custom-role-change"
  ...
  // removes the operational secrets, and the two FAILURE rows, which name the step the ceremony died on
  // and the closed cause. An install that died at PUT #2 leaves a HALF-KEYED engine, and these two rows are
  // what records why.
  | "keys-installed"
  | "break-glass-rotated"
  ...
  // verifies until the operator confirms. Mirrored from the engine; recovery-codes-generated above is the
  // row that says the old set actually died.
  ...
  // engine-account-verified: the engine proved its own Cloudflare account id for the first time, from a
  // successful attach or update-apply. Recorded once (never overwritten); who + when only, never the
  // account id itself.
  ...
  // Session and credential lifecycle, mirrored from the engine's audit.ts. session-terminate records
  // a self / admin / all-users session termination;
  ...
  // signin-factor-revoke: ONE act, an operator removing every way one
  ...
  // (ADMIN_TOKEN, SCIM_BEARER_TOKEN, DISCOVERY_API_TOKEN or a destination access key) named in the rotation
  // schedule was rotated, moving its tracked cadence baseline to now. A signer or recipient key's rotation is
  // engine-observed from its public fingerprint instead and never appears here. Target: secret-rotation.
  ...
  | "test-fault-disarmed"
  | "test-fault-fired"
  // the control-plane fault hook's clear (engine sched/scheduler-do-test-fault.ts): the harness cleared a
  ...
  // events are all written by the CURRENT engine. Each is mirrored below so the audit filter can select it
  // and render it by name rather than its raw internal noun.
...
export type AuditTarget =
  ...
  // authn-attempt is the target of a FAILED authentication ceremony (the authn-failure action). The connection
  // is named only when it resolved from storage; connId is the same operator-chosen slug the idpconnection
  // target carries, and attempts is how many coalesced failures this one row stands for. No assertion, id_token,
  // authorisation code, credential id or presented token is representable here.
  | { readonly kind: "authn-attempt"; readonly ceremony: "passkey-login" | "sso-callback" | "step-up"; readonly connKind?: "oidc" | "oauth2" | "saml"; readonly connId?: string; readonly attempts: number }
  ...
  // secret-rotation carries the closed cadence id of the bearer or destination credential an owner attested
  // was rotated. Never the credential value: the id is one of a fixed, non-secret vocabulary ("admin_token",
  // "scim_bearer_token" and its siblings), unrepresentable as anything else.
  | { readonly kind: "secret-rotation"; readonly secretId: string }
...
//
// headTruncated is the verdict `intact` CANNOT reach. The engine's recompute walks the entries it still
...
// rolledOver / rolledOverCount / earliestSeq are the RETENTION side, and the engine sends all three on
// this same response. Without them, an estate whose log had rolled would show "Chain intact through entry
// 12345" over a chain that begins at 2346, with no mention that the 2,345 entries before it are gone. The
// engine's own note beside them says they exist "so the console can show 'chain begins at entry N, M
// earlier entries rolled over' rather than a spurious break", which is a module asserting in writing what
// a screen it cannot see does not do.
//
...
// ---- Drill-evidence mirror (optional) ---------------------------------------
// DrillEvidenceKind / DrillEvidenceEntry mirror the engine's optional drill-evidence log.
// "in-account" is an engine-run drill outcome; "offline-rehearsal" is an
