...
// refuses an EMPTY one. Without it the console cannot tell the two apart: on an established account
// (which is all of them, since the last-Owner guard keeps the table non-empty) every affordance the
// banner offers is one the engine would have to refuse. OPTIONAL, and absent means UNKNOWN rather than
// false: an engine that predates the field must not be read as an empty role table, so the banner
// degrades to the state it can prove instead of asserting one.
