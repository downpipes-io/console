// Attended-verification (attest) session mirror types. Attended verification is the offline-key-only
// posture's IN-PLATFORM proof path: the CLIENT drives the loop, recovering each run's master in the
// browser from the operator's break-glass identity.key (which is NEVER uploaded) and handing only the
// per-run masters to its OWN engine for a keyed sample verify to a discard sink. These shapes are
// redaction-safe by construction: a session carries run ids, friendly names, counts and a per-run
// pass/fail only, never a value or a key. The engine is the authority; the console mirrors the wire shape
// so the runner can type its calls. Where a field can be absent at runtime the mirror marks it optional so
// the runner degrades honestly rather than assuming a field.
...
// modelled degrades to a generic {ok:true,...} shape with no such field, and reading `session.runs.length`
// on that shape throws. Checked at the wire boundary (client-attest.ts) and at the screen that drives the
// loop (restore-flow/attend.ts), so every caller is protected, not only one route. Deliberately loose
// beyond `runs` (an array is enough to unblock the `.length` read; the per-run shape is not re-validated
// here) so a harmless engine-side field addition never trips it.
...
//
// The engine's AttestSessionRunState.state (engine/src/sched/types.ts) accepts "skipped" (the DO's
// record-verify guard, scheduler-do-attest.ts:176, writes it verbatim), even though no current caller
// produces it -- POST /attest/session/verify (router-attest.ts) only ever sends "verified" | "failed". It
// is carried here for type completeness, so a future engine change (or a hand-edited/replayed session
// record) is not silently mishandled.
...
// `session` is unmodelled the same way: a route the engine has not modelled degrades to a 200 with no
// `session` field at all, so `(await engine.attestStatus(id)).session` reads as `undefined`, and a caller
// reading `record.runs` on it unguarded would throw. Only `sessionId` is required by the wire type, so
