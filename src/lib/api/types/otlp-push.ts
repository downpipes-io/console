// Outbound OTLP/HTTP metrics push mirror types (see ./push.ts for the SIEM audit-log push sibling
// this mirrors). The engine pushes the canonical backup-health metric snapshot
// (downpipe_backup_last_success_timestamp_seconds, _success, _recent_attempts/successes/failures,
// _duration_seconds, _size_bytes, destination_healthy) to a customer OTLP/HTTP collector on the cron
// tick, zero-agent. OTLP-JSON is confirmed for Datadog only; Dynatrace, Elastic and Splunk
