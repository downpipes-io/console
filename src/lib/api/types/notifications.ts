// Notifications mirror types. See ../types.ts for the barrel.

// Notifications, mirrored byte-for-byte from the engine's notify types so the two sides cannot
// drift. A channel/rule/history entry carries the customer's own config and a redaction-safe
// outcome detail only; never a secret, key or value.
...
// engine's own order, so a targeted rule (e.g. "route dual-control-disabled to the security team's
// PagerDuty, everything else to Slack") can be built for any event, several of them security-relevant
// (dual-control-disabled, recovery-code-abuse, sign-in-new-context, canary-dead). An "all events" rule
// receives every event at whatever minSeverity it set (events:"all" on the wire is not enumerated).
// Severity is fixed per event (the engine's severityOf, notify-routing.ts) and mirrored
