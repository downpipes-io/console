// Downpipe / schedule / retention / state mirror types. See ../types.ts for the barrel.
...
export interface Downpipe {
  ...
  // retention is the per-downpipe archive RETENTION policy, mirrored byte-for-byte
...
export interface DownpipeState {
  ...
  // last). The boundary mapping keeps it, so the "restorability last proven" and coverage surfaces can
  // name the method faithfully ("attended verification", "blind restore test", "keyless attestation")
  // rather than a generic claim. Absent until restore has been proven at least once.
  ...
  // would paint the escalate-flavoured "no copy" lane on a healthy configuration the moment an operator adds
  // a destination to a running downpipe, unable to tell it apart from a leg that has been dark for months.
  //
...
//   undefined  the caller states nothing, so the engine applies the write with no base check -- what a
//              screen that genuinely does not hold a revision must send, rather than guessing one.
