// Passkey (WebAuthn) sign-in wire mirror types. See ../types.ts for the barrel.
...
// SessionSummary is one row of the caller's OWN live sessions (GET /admin/sessions): coarse
// provenance and instants only; the engine never returns a token. `current` marks the session that made the
// request, so the screen can label it and withhold its own Sign out control.
...
//
// recoveryCodesPending: true when recoveryCodes were minted as a STAGED set, not yet live -- an enrolment
// that ran while the email already held a working set (a self-add; the forced re-enrolment after a
// recovery-code sign-in is the common case). The OLD set keeps signing the operator in until the console
// calls confirmRecoveryCodes(): that is what the save-confirm panel's "Continue" must do before it hands
// control back. Without that call, an abandoned or crashed enrolment leaves the old codes live (harmless)
// rather than an immediate mint killing them before the operator has had a chance to save the new ones.
// Absent or false means the mint already went live (bootstrap, invite, or an older engine that predates
// this field): nothing further to confirm.
...
// ConfirmStagedRecoveryCodes is the POST /admin/auth/recovery-codes/confirm body: promoted:true means
// a staged set just went live (the old one is now dead);
