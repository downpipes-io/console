// Envelope encryption for the offline break-glass custody experience. The break-glass private
// (identity.key, 96 bytes: x25519 scalar(32) || ML-KEM seed(64), src/keygen.ts:17) is too large to
// Shamir-split directly, so the custody pattern is: AES-256-GCM encrypt the key file with a fresh
// random 256-bit WRAPPING key, store the small ciphertext (Tier 1 vault / Tier 2 cold storage), and
// then either Shamir-split the wrapping key M-of-N (src/shamir.ts) or derive it from a security key
