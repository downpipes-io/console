// The console identity mirror core: the capability and role model. This is
...
//
// It is mirrored byte-for-byte from the engine's src/admin/identity-roles.ts. Any drift between the two unions
// matters immediately: a method this console does not recognise falls through to the amber break-glass claim,
// telling an OIDC or SAML customer their fleet runs on the shared token when it does not. Drift in this union
// is not a cosmetic bug, it is a false claim about the customer's security posture.
...
//
// It is the difference between a signal and a phantom. The engine refuses "sign out my other sessions" with a
// 400 for a caller who holds no first-party session, and for an access/token caller THAT REFUSAL IS CORRECT AND
// EXPECTED (the screen says so). Treating it as a refused privileged write would put a guaranteed phantom row
// into every token-auth console's pack, in the very row a genuine refusal coalesces into. A null caller
// (identity not yet resolved) reports false: we do not claim a write we cannot attribute.
...
// the engine's identity.ts table exactly. The four original roles keep their
