// Typed, textContent-first element creation. This is the SAFE default path: text
// content goes in as text nodes, never parsed as markup, so a server-supplied
// string (a downpipe name, a run id, an email, a coarse reason, an enum) can
// never inject markup into the in-account console. No framework, no dependency;
// just the DOM.
...
// currentColor stroke so the icon themes for free. An icon paired with a
// visible label is decorative (aria-hidden); a standalone icon button must
// carry its own aria-label on the button, not here.
...
// circular dependency. screens/common.ts re-exports it, so every existing screen import site is
// unchanged.
//
...
//
// THE REASON IS A DESCRIPTION, NOT PART OF THE NAME. A plain visually-hidden child reading
// `: <reason>` would be a CONTRIBUTION to the control's accessible name: the browser's
// name-from-content algorithm joins sibling contributions with an inserted separator space
// regardless of the next one's own leading punctuation, so the control would announce as "Remove
// token : Requires the Owner role", with the colon read out as a separate token. In Chromium, the
// same markup computes "Remove token : ..." with a plain span, and "Remove token" with the two
// attributes below, while the reason moves to the description where it belongs.
//
// Each of the three mechanisms is load-bearing:
...
// The name a screen-reader user hears is exactly the label a sighted user reads, and it FOLLOWS a
// label the screen rewrites at runtime, which an explicit aria-label of label-plus-reason does not
// (that form keeps announcing the old label after the visible text has changed).
//
...
//
// The title is kept, not replaced: a mouse user loses nothing, and a tooltip must never be the ONLY
// place essential information lives. It is written as an ATTRIBUTE rather than through the `title`
// property because the test DOM shim deliberately does not mirror `title` in either direction, so a
// reader going through getAttribute would see nothing.
//
