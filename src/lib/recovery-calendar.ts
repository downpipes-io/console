...
// plaintext and no key material (engine/src/sched/types.ts), so this module is unit-tested directly
...
//
// HONESTY. A day before the ring's floor must never read as "outside the retained window" or be treated
// as unrecoverable: the ring is a bounded, recent-run-history view (RING_CAP = 50,
// engine/src/sched/scheduler-do-limits.ts), a DIFFERENT mechanism from the archive's actual retention
...
// that has rolled off the ring remains restorable BY ID: engine/src/admin/restore-open.ts's openVerifiedRun
// reads the destination object store by runId and never consults the ring at all. The console states this
// plainly elsewhere -- console/src/screens/restore-flow/run-context.ts's honest note reads "This
...
// calendar can show, never a recoverability verdict. Never rename it to "outside the retained window",
// "expired", or any word implying the archive itself is gone: that would mislead an operator at the worst
// possible moment.
//
...
// harness oracle reading the same ring independently, and every operator's browser, one identical
// calendar-day boundary. Grouping by the OPERATOR'S LOCAL time would let a run near local midnight land
// on a different day for the subject than for an independent reader, a boundary-day flake; UTC removes
// the hazard rather than papering over it with a shared-but-arbitrary convention.
//
...
// the successful runs the ring currently holds". Grouping by startedAt while classifying against a
// completion-instant floor would use two different instants, which could misclassify a run that started
// before the floor's day but completed within it (or the reverse) as on the wrong side of the boundary.
// Using one instant throughout removes that hazard.
...
// environment (the console, the harness, any Node or browser runtime), matching this
