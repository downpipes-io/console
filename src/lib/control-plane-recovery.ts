...
// RecoveryBannerModel is the banner's resolved presentation: show or not, plus the copy. On an estate whose
// configuration came back, a standing danger banner would assert that backups have stopped when they are
// running, which is both untrue on the first screen an operator sees and a mask over the next real latch, so
// tone, title and body are all chosen by the same `exit` discriminant below instead, and the sentence and the
// button can never disagree. reason is the engine's redaction-safe one-line explanation (or a stable fallback
// when the engine sent none). staged carries the auto-heal's staged-recovery summary when one exists, so the
// banner can offer the break-glass CONFIRM (no pasted export needed) instead of the manual reconcile form,
// which a sealed estate's artefact can never satisfy (see ControlPlaneStagedSummary).
...
// every estate.
//
// Two of the five members offer NO ACTION, deliberately: a button wired to a route that must refuse is worse
// than no button. It costs an operator mid-incident a round trip and a refusal to learn what the console
// already knew. Each of the two says what does open the way instead, and both of those paths are reachable
...
//                     EXIT AN ESTABLISHED ACCOUNT HAS.
...
// It also resolves WHICH EXIT the estate has, from the engine's two emptiness facts. The table below is
// written out rather than nested, because the guards it mirrors are exact complements and a reader has to be
// able to check it against them line by line:
//
...
//   true             ABSENT        confirm/reconcile  the empty role table alone forces the wipe path
...
//   ABSENT           true          confirm/reconcile  a known-empty config drives the wipe path when the role
//                                                     table's own state is unknown
//   ABSENT           ABSENT        confirm/reconcile  an unread emptiness is not a recovered plane
//
...
// configEmpty at all, which is why validate-console-gap-fixes.ts checks that a confirmed latch still raises
// the real danger banner in that case. Every unknown row falls back to the confirm/reconcile wipe-exit
// behaviour.
export function recoveryBannerModel(status: ControlPlaneStatus | null): RecoveryBannerModel {
  ...
  // emptiness was reported. Everything left is the unknown or the genuine wipe, and both use the
  // confirm/reconcile wipe-exit default.
...
// exits use the standing danger copy, because on a genuinely wiped plane it remains true.
...
export function parseReconcileInput(raw: { exportText: string; signature: string; token: string }): ParseReconcileResult {
  ...
  // A browser-side rejection the engine can never see (and on a wiped engine there may be no engine left to
  // see anything). The op is `cp-restore`, not `estate-import`: the two flows share this code vocabulary but
  // are different flows.
