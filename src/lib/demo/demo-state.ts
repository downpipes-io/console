// The demo-state fixture knob. This is a TEST-ONLY variant selector over the tour/demo world
...
// real deployment, exactly like the ?tour=1 flag itself (see tour-mode.ts's own header comment for
// that guarantee, which this file inherits unchanged): it adds no new gate, only a second query param
// read behind the SAME isTourMode() boundary, parsed no differently to that flag.
//
// This is a URL query flag interpreted once at boot, not a field({...}) call, a
// raw input/select/checkbox/radio, a screen's control set, or a validator -- the same reasoning that
// already exempts the pre-existing ?tour= flag from the field catalogue. It
// adds no customer-facing control, moves no field, and changes no validator, so the field-catalogue
// cross-check protocol does not apply to this change.
//
// The five variants:
...
//
// KNOWN LIMITS (honest, not silently perfect -- named so the harness's screen x state matrix can
 ...
 *  hasInventory to false (the honest no-inventory-configured render), matching the true-empty
...
function trainingStartOverrides(world: DemoWorld): Pick<DemoWorld, "setupState" | "configApprovalPolicy" | "status"> {
  return {
    ...
    // (the generate card reframes to "your keys are already set" on breakGlassConfigured), so a training
    // world whose status still carried the seeded keys would let the deck skip the very ceremony the walk
    // teaches. The demo key/destination/downpipe writes flip these back as the learner earns them.
    status: {
      ...world.status,
      signerConfigured: false,
      breakGlassConfigured: false,
