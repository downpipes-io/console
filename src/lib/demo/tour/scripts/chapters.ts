...
// The welcome card offers a one-tap fork, and each choice runs a DIFFERENT walk over the same director:
...
// entry is `satisfies TourRoute`, so a typo on either side fails `tsc`. Every entry pins to the owning
// screen's own constant, the same pattern the breadth routes (idp / notifyRules / integrations) use,
// rather than a central ROUTES map the router does not itself bind from and so could silently fall behind
// the screens. Every TourRoute literal, "/reports" included, has a pin here.
const ROUTE_CHECK = {
...
// stays in the DOM, so it resolves, and it measures 0x0 with a null offsetParent: without the
// re-expand below, the spotlight would paint at opacity 0 with nothing on the screen to click, and
// the director's own retry ladder would exhaust with the anchor never visible.
//
...
// still being boxless, so a future flow that leaves the form open is untouched, and the three earlier
// beats of this chapter (restore-no-data, restore-verify, restore-drill) lay out unaffected at every
// viewport width.
...
// PRICING_URL is the one commercial page every conversion path channels to: the pricing page carries the
// enterprise-support offer, so "see pricing" is the honest ask everywhere. The
...
// FUNNEL_CTAS are the two deliberate exits the closing funnel chapter presents: the primary
