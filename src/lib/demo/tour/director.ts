...
// tour. It keeps the REAL console screen fully visible (no scrim, no spotlight, no bubble) so the product is
// never hidden, and the guide sits ON it: a bottom-centre nav-bar (tour/nav-bar.ts) plus "?" info-points
...
//
// Controls: MANUAL by default (the visitor drives with Back / Next; the tour never advances on its own). Play
// turns it into a GUIDED autoplay: it reveals the current chapter's "?" explanations one at a time, each held
// a dwell scaled to its prose (a visible per-step dwell bar in the nav shows how long until the next), and
// only after the last "?" advances to the next chapter; on the last chapter it stops back to manual. Pause
...
// A BEAT: the guided walk's reading unit (kept under the "?" info-point name below). Each
...
export interface InfoPoint {
  ...
  title: string;
  body: string;
  // Retained for script compatibility with older chapter data; the beat model does not read it.
...
// ctaHrefValid checks that an ending CTA's href would actually resolve. An href that does not parse
...
export interface TourChapter {
  ...
  // what they are doing or why they are doing it. Absent on the tour, whose chapters are shown rather than
  // worked.
  goal?: string;
...
// event before the browser's own navigation proceeds. Kept on the chapter type so the funnel chapter carries
// its exits as data like any other chapter.
...
// Guided autoplay dwell scaling: Play reveals the "?"s one at a time, each held long enough to read.
...
export function createTourDirector(script: ReadonlyArray<TourChapter>, deps: DirectorDeps = {}): TourDirector {
  ...
  let idx = -1;
  // currentChapterId names the chapter a degradation event belongs to. The chapter's ROUTE is its stable
  ...
  // Hand the drift recorder THIS script's vocabulary, so a degradation event can carry a chapter
  ...
  // into the chapter after it, skipping every beat AND every task gate between. Measured on the training
  // walk: a Next landing during the connect chapter's render skipped its token task entirely.
  ...
  function awaitScreen(): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      const finish = (): void => {
        ...
        observer?.disconnect();
        if (pollTimer !== null) { clearTimer(pollTimer); pollTimer = null; }
        // The tour proceeds either way (a hung tour is worse than a mistimed one), so a screen that never
  ...
  function armTask(beat: InfoPoint): void {
    ...
    // the card reads as complete and NOTHING advances: without the arrival check, walking back onto a
    // completed step would re-arm the grader, see it pass at once, and throw the learner forward again,
    // making every finished stage impossible to return to.
    const evaluate = (arrival: boolean): void => {
      ...
      // the voice off mid-word. closeStep then advances on its own, since the task's gate is already clear.
  ...
  function renderChapter(moveFocus: boolean): void {
    ...
    void awaitScreen().then(async () => {
      ...
      // the learner just unlocked can be bounced by the stale view while the refresh admits the retry: a
      // chapter reached moments after an unlock (e.g. once keysReady flips) can otherwise redirect the
      // director's single navTo back to the deck. A pathname the environment cannot report (the headless
      // shim) reads as unverifiable and is left alone.
      ...
      if (chapter.preAction) {
        // The pre-action opens the drawer / disclosure the chapter is about to narrate. When it throws, the
      ...
      // The finale CTA is the conversion, and a CTA whose href does not resolve navigates the demo tab to a
  ...
  function closeStep(): void {
    ...
    if (b?.interact) {
      ...
      if (el && typeof el.click === "function") {
        ...
        // The control the autoplay was to demonstrate is not there. The walk advances anyway (correct: a
  ...
  // wireUnload installs a pagehide listener that counts a drop_step if the visitor closes or navigates away
  // while the guide is still showing, so the funnel's abandonment point is measured. pagehide is the reliable
  // page-teardown signal (close, reload, bfcache), and a beacon survives it (analytics.emit prefers
  // navigator.sendBeacon). Wired while the guide is up; removed on Exit/destroy. Re-entrant-safe. A view with
  // no window skips it.
  ...
  // the guide back at the chapter they left. It is a real button, keyboard reachable, tokenised, and removed
  // on resume.
