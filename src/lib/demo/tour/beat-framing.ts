...
//
// This module is split out from director.ts: the director is a control-flow state machine over shared
// closure state (the chapter index, the beat index, the render token, autoplay), while this is geometry over
// the document, reading only four facts from the director, injected below. Keeping the geometry separate
...
export function createBeatFramer(deps: BeatFramerDeps): BeatFramer {
  ...
  // to read or work, and the highlight visibly wanders.
  //
...
//
// The exhaustion branch below distinguishes three different states rather than folding them all into one
// noteTourDegraded("anchor-missing") call, because only two of them are actual faults:
...
//     anchor low on a short page can never be seated in the band however many times it is scrolled to.
// The exhaustion branch records only what the DOM ESTABLISHED, and records nothing at all for the third
// state, so a legitimate short-page layout is never reported as a broken anchor.
...
// seatAnchorAcross scrolls the anchor's own sideways-scrolling region so the subject is inside it: the one
// axis vertical scrolling never reaches. A subject in a horizontally scrolling region can be laid out far
// outside that region's client box and so be CLIPPED ENTIRELY while still reporting a rect, so without this a
// beat can frame a rectangle over the guide panel for a subject nobody can see.
...
function beatIntoView(beat: { anchor: string }, token: number, attempt: number): void {
  ...
  // Clearance under the console's own sticky top bar, MEASURED rather than assumed: tokens.css lays the
  // shell out as `minmax(var(--header-h), auto)` below the wide breakpoint, so the bar is 89px at 768 and
  // 141px at 390, not the fixed 76px (--header-h's 56px plus a 20px margin) that only holds at desktop
  // widths. A fixed constant would let "already comfortably in view" accept an anchor seated BEHIND the bar
  // wherever it wraps, and would seat the top of a tall section behind it too.
  ...
  //
  // Both "already framed" early returns below are gated on !boxless. A 0x0 anchor can sit at coordinates
