...
// morphs between anchors as the walk advances (the rail narrates; this points). It is the guided
// walk's attention anchor: a single fixed "hole" element whose enormous box-shadow paints the dim, so
// the cutout needs no SVG mask, the dim can never intercept a click (a shadow is paint, not a hit
// target, and the hole itself is pointer-events:none), and the morph is a plain CSS transition on the
// hole's box metrics. Under reduced motion the morph is instant (information is never motion-gated).
//
...
//
// Styling is per-property CSSOM (strict CSP).
...
// found by a grep or by the harness's dom-id pin gate, and an assertion over an id nothing can produce
...
//
// THE HEIGHT IS MEASURED, NOT ASSUMED, because tokens.css lays the shell out as
// `minmax(var(--header-h), auto)` below the wide breakpoint, so the sticky .context-bar's actual height
// varies with viewport width rather than staying at the bare --header-h (56px): it measures 89px at 768
// and 141px at 390. A fixed clearance would be right on a desktop and wrong on a phone, clamping less
// than the bar actually covers.
//
// AND THE OVERLAP IS PUT TO THE BROWSER RATHER THAN INFERRED FROM TWO RECTANGLES: the run-detail drawer
// is an overlay with its own stacking context and paints ABOVE the bar, so an anchor inside it can
// overlap the bar's box while being entirely visible. At 390, run-status sits at y=126 under a 141px bar,
// and elementFromPoint still answers the anchor; clamping on the rectangles alone would cut that anchor's
// hole down to 28% of it. elementFromPoint answers what the reader would actually touch, so that is what
// decides.
...
export function createSpotlight(doc: Document = document): Spotlight {
  ...
  //
  // THE TOP EDGE IS MEASURED, NOT ASSUMED: tokens.css lays the shell out as `minmax(var(--header-h), auto)`
  // below the wide breakpoint, so the sticky .context-bar's actual height varies with viewport width rather
  // than staying at the bare --header-h (56px) -- it measures 89px at 768 and 141px at 390. A fixed clearance
  // would be right at desktop widths and wrong at every width where the bar wraps.
  //
  // THE RIGHT EDGE NEEDS THE SAME TREATMENT. Without clamping it too, at 1024, where the guide docks as a
  // 344px right rail, a hole tracking an anchor in a wide table can run under the rail and off the right of
  // the viewport: a hole wider than the screen paints no dim at that edge, so there is no spotlight. The rail
  // is detected as the not-bottom-bar variant of the same element.
  ...
  // deck does not have one, so using it here would seat the chip at y=0 across the product logo on a phone.
  ...
  // CENTRES on a wide one: left-aligning a whole-screen subject (the onboarding deck) would put the chip
  // in the far top-left corner over the console's own logo, where it would read as chrome rather than as
  // a pointer. The vertical seat is clamped into the stage for the same reason: a lit area whose top is
  // off-screen must not push the chip up behind the fixed bar.
  function placeCallout(top: number, left: number, width: number, stage: { top: number; bottom: number }): void {
    ...
    // Never under the console's own sticky bar. At 390 with a full-screen subject, the chip would otherwise
    // sit at y=0 across the product logo, which reads as broken chrome rather than as a pointer at anything.
    // The bar's height is asked for directly here (rather than taken from the stage, which is computed for the
  ...
  //
  // This distinguishes a dialog that opens OVER an anchor (the training course's downpipe step anchors the
  // empty-state table, and pressing New downpipe opens the create dialog over it: the anchor is still there,
  // still measurable, but completely hidden, so lighting it would point at something nobody can see) from an
  // anchor INSIDE a drawer, which is the OPPOSITE case and stays lit: there the dialog contains the anchor,
  // which is how the tour's run-detail beats work.
  ...
  function tick(): void {
    ...
    if (anchorId !== null) {
      ...
      if (rc && rc.width > 0 && rc.height > 0) {
        ...
        // the docked rail) has no honest hole to cut: without this guard, a near-zero overlap would paint a
        // sliver at full opacity, which reads as a fully dimmed console pointing at nothing. So hide instead,
        // exactly as an absent anchor does; the loop keeps watching, so the spotlight returns the moment the
        // anchor is back on the stage.
