...
//
// The whole reason the task card exists is the link between the two halves of the screen: without it the
// rail reads as a text box on the side, unconnected to the console beside it, saying things while the console
// does things with nothing joining them. The card is the join. It states the task as a task, says where on
// the screen to do it (the spotlight is aimed there), and flips to DONE the moment the grader sees the
// learner's action land, before the walk moves.
//
...
//
// Below the rail breakpoint the guide is a panel sitting OVER the console. Unmanaged, the panel can grow tall
// enough on a narrow viewport to cover the screen the learner is meant to work on, while the goal and the
// prose each hit their own scroll cap and clip mid-sentence. Three states prevent that, and each has one job:
//
...
// republish. Publishing only on the events this module knows about lets the correction land after the
// learner has already scrolled, so the page visibly shifts under them. A ResizeObserver fires on every box
// change, including the ones no call site predicts: a task card painting, a mode change settling, a font
// arriving. Returns its own disconnect, and is a no-op where ResizeObserver does not exist (the headless
// test DOM), where the explicit publishes still run.
