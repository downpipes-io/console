// The tour's lightweight, self-hosted funnel analytics. For a solo operator, knowing which proof-point
// converts WITHOUT a single sales call is the
...
//   - demo_drift:     the faked route table outgrew the console: an unmodelled route, an unknown id, a
//                     fabricated success. Carries the closed drift kind + the normalised /admin route pattern.
//   - tour_degraded:  the guided session itself degraded: the honesty chrome did not install, a chapter
//                     narrated over a blank screen, a spotlight anchored to nothing. Carries the closed stage,
//                     the chapter id and the anchor id.
//
// The two events above are DIAGNOSTIC, not funnel: they exist because a demo that lies (a delete that
