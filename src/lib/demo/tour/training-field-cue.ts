...
export function mountTrainingFieldCues(doc: Document = document): TrainingFieldCues {
  ...
  // observer would never fire when the card holding the key install appears: a password field would show
  // with no cue beside it, the exact moment a learner would go looking for a real Cloudflare token.
