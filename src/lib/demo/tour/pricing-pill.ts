// The free-roam pricing affordance: visitors who skip or exit the tour and click around the live demo
// find no other call to action anywhere. This mounts one quiet "See pricing" pill stacked above the
// bottom-right tour pill slot, appearing only after genuine exploration time (delayed-then-stable reads
// as earned; immediate reads as clutter) and hiding itself whenever the guided tour's chrome is up (the
// rail carries its own asks there). It is a real anchor to the pricing page; the ?src=pricing-roam
// parameter is counted server-side on downpipes.io, so the tour itself still sends nothing anywhere.
// Styling is per-property CSSOM (strict CSP); the emit is the local, flag-gated funnel event the other
// CTAs share.
