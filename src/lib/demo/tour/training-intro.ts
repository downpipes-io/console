...
//
// WHY IT EXISTS. A course that starts mid-task, on the onboarding deck with a rail beside it, teaches nothing
// in its first minute: a learner cannot tell what they are doing or why. This card answers four questions
// before anything moves: what this page is, how the course works, what the learner will have built at the
// end, and how long it takes.
//
...
export function mountTrainingIntro(deps: TrainingIntroDeps): TrainingIntro {
  ...
  const card = h(
    ...
    h(
      ...
      startBtn,
      // The Escape hint is for a device that HAS an Escape key. On a phone it would be advice about a key
      // the reader does not have, which is the kind of line that quietly tells a learner the thing was not
