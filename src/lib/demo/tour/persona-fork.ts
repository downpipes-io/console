...
// a true modal: it does not trap Tab, does not make the app behind inert, and it keeps the real screen the star
// by carrying NO dim backdrop at all, so the booted Overview behind it stays fully visible and reachable to
// every input mode alike - mouse, keyboard and screen reader. The always-visible "Free Explore" control (a
// real, keyboard-reachable button styled linklike so it reads quieter than the two walk primaries) and Escape
// are the explicit exits for all input modes, leaving the visitor in the genuine, faked console, with a
// persistent "Take the tour" relaunch control so skipping is reversible (it re-mounts the welcome), mirroring
// the in-tour Exit, which offers a Resume affordance. Both exits count as a welcome engagement: destroy() emits
// persona_chosen with the coarse label "explore" (once per mount).
...
// returns) and a test can find + dismiss it deterministically. One welcome card per page. The id is
// "tour-persona-fork", so the boot path and a test's stable selector can rely on it.
...
export function mountPersonaFork(deps: PersonaForkDeps = {}): PersonaFork {
  ...
  if (existing) {
    ...
    // double-driven tour spotlights two things at once and narrates over itself. It is not visible in the
    // funnel (both sessions emit the same step_reached events), so it is recorded here.
  ...
  // fork is not a one-way door, matching the in-tour Exit, which offers a way back.
  ...
  const panel = h(
    ...
    // describedby target, so it must stay populated. The pitch paragraphs (source-type counts, the pricing
    // model) belong to the marketing site, not here; the welcome earns the walk in two lines.
    h(
      "div",
      ...
      // the "readable teal foreground", and this eyebrow is TEXT: --trust on the light panel is 4.14:1 against
      // the 4.5:1 this size and weight require, while --trust-fg is 6.14:1 there. In dark BOTH resolve to
      // --t-300d at 7.83:1, so this changes the light theme only.
      ...
      // THE LENGTH IS THE TOUR'S OWN ARITHMETIC, NOT A ROUND NUMBER. The walks are ten chapters of 24 and 30
      // beats, and director.ts already holds the product's own judgement of how long each beat needs to be
      // read: dwellForBody scales the hold to the prose length (MS_PER_CHAR, clamped between MIN_DWELL_MS
      // and MAX_DWELL_MS) plus STEP_GAP_MS between steps. Summed over the shipped scripts that is 5.6
      // minutes for the governance walk and 6.9 for the engineering one at 1x, so a round time estimate
      // would understate what the tour itself budgets. "Ten chapters" is the honest measure and it is the
      // one a visitor can check against the progress bar in front of them; the pace stays theirs, because
      // the walk is manual by default and Play is opt-in.
  ...
  // The hosting layer + the centred card. The booted Overview stays visible behind the welcome, never hidden,
  // so this layer carries NO dim backdrop at all (it is fully transparent), only positioning. It is
  // pointer-TRANSPARENT, so the Overview behind it stays reachable to
...
// bottom-left corner belongs to the rail's pinned "Help and shortcuts" row. A view with no body is a no-op
// (the relaunch is an affordance, never a gate).
