// The bottom-centre navigation bar for the public page-walkthrough tour. It keeps the REAL console screen fully
// visible and the guide sits ON it, never over it. This bar is the guide's one persistent control surface: a
// big, obvious, premium strip pinned to the bottom centre of the viewport, on the console's own design tokens.
//
...
//
// Accessibility: the bar is a labelled region; every control is a real <button> reachable and operable by
// keyboard with the console's own visible focus ring; the director's keyboard layer maps ArrowLeft/Right to
// Back/Next, Space to Play/Pause and Escape to Exit (this module renders the operable buttons + exposes their
// handlers); a polite ARIA live region announces each chapter; status is conveyed by label + shape, not hue
// alone; and prefers-reduced-motion (OS or the in-app setting, via motionOK) drops the bar's entrance animation.
// The bar is pointer-events:auto over a pointer-transparent host, so it never blocks the page while staying
// clickable itself.
...
// The current BEAT's narration (the guided walk's reading unit): the rail (or
...
// The rail's variant options. All absent on the tour, so every default below matches the tour's own
// behaviour: `eyebrow` names the surface (a course says which course it is), `progressUnit` names
...
export function mountNavBar(doc: Document = document, opts: NavBarOptions = {}): NavBar {
  ...
  // sits exactly where Next sits (last, on the right).
  //
  // Every control in the cluster is flex:none by design (the foot must not squash), so nothing here can shrink
  // to absorb overflow: once Back appears (from the second step of the course onward), the four labelled
  // controls (Back, Sound, Leave, Next) measure 324px plus their gaps against a 344px rail with 20px of padding
  // each side, and without a dedicated slot Next would be pushed outside the panel.
  ...
  function layoutForWidth(): void {
    ...
    if (isRail()) {
      ...
      // SHRINK-UNDER-PRESSURE, not fixed: "0 1 auto" with min-height 0 lets this column give way and scroll
      // internally when a beat runs long, rather than push the foot controls out of a box that is pinned top
      // and bottom and is overflow-y:hidden, where they would be clipped and unreachable.
      //
      ...
      // the controls leaving: of the narration and the foot controls, only one of the two can be recovered by
      // scrolling.
  ...
  return {
    ...
    setDwellActive(durationMs: number | null): void {
      ...
      // "restart a CSS transition" idiom: without the synchronous flush, the 0% and 100% writes for an element
      // that has already animated collapse into a single frame, so the fill reads as staying full and the
      // visitor cannot see how long is left to read. Reading offsetWidth is the reflow trigger; it is a
      // harmless no-op under the test DOM shim (which never lays out, so offsetWidth is undefined and the void
      // simply discards it).
      dwellFill.style.setProperty("transition", "none");
      dwellFill.style.setProperty("width", "0%");
