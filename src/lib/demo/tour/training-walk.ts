...
// director's own awaitScreen reads). The tour does not meet this race because its director starts on a
// human click long after boot; the walk starts itself at boot, and a director that navigates before the
// router's initial dispatch has its navigation overwritten by the boot landing on Overview: without the
// deferral the rail would narrate chapter one over the wrong screen.
