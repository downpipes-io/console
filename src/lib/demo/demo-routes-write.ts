// The faked engine's write handlers (see demo-seed.ts for the file layout).
...
// RESTORE_XACCT_DEMO_RUN_ID is a dedicated, non-narrative run id that exists ONLY to give the harness's
// visual-baselines pass a one-click path to a cross-account restore plan with an ALREADY-ARMED
// dual-control approval, so it can capture the type-to-confirm copy (confirm.ts confirmApply) without
// inventing a click sequence that also has to raise and sign a
...
// real key material. Leaving POST /admin/auth/recovery-codes/regenerate to fall through to benignWrite's
// generic {ok:true} shape would crash recoveryCodesPanel's regenerate-context caller, so this route is
// modelled properly instead: it makes Regenerate actually demonstrate the real feature to a visitor
// instead of ending in a "could not regenerate" toast.
...
// writeCreateAttestSession models POST /admin/attest/session/create: this route and its /prove,
// /capsules and /verify siblings must be modelled rather than left to benignWrite's generic
// {ok:true,deleted:true,applied:true,status:"result",value:{ok:true}} fallback, which carries none of
// the fields restore-flow/attend.ts's runner reads (`session.runs.length` would be undefined and crash
// it). It pins every downpipe with a completed ("ok") run, honouring an explicit scope, so the runner
// shows the estate's REAL runs and record counts instead of either a crash or a false "Nothing to verify
// yet".
//
...
// readBody (and the currentWritePath route marker its drift note names) lives in demo-world.ts because
// both write-route modules parse bodies from there; see its header there for the body-parse drift
// contract.
...
export function routeWrite(pathname: string, init?: RequestInit): Response | Promise<Response> {
  ...
  if (chgMatch) {
    ...
    // An id the world does not hold is answered HONESTLY: queued writes land in world.configChanges
    // (writeAddDownpipe unshifts the real pending change), so approving or rejecting an unknown id
    // returns a 404 rather than silently flipping a DIFFERENT object than the one acted on. The drift
    // row still records the id as unknown and enumerable.
    if (change === undefined) {
  ...
  switch (pathname) {
    ...
    // benignWrite this would answer ok:true, and every Overview render on a gated world would fire a
    // redundant acknowledge-refetch cycle against a world that had not changed.
    ...
    default:
      ...
      // world mutation; a reload re-seeds. The pathname rides so the unmodelled write can be NAMED
...
// queuedChange (the ONE deferred-mutation 202 body minter) lives in demo-world.ts because two route
// modules defer config mutations; see its header there for why the body must be the engine's own.
...
//
// AN IMMUTABILITY POLICY ON AN R2 DESTINATION IS REFUSED HERE. The real engine refuses that save with a
// 400: measured, R2 over its S3 endpoint answers 501 NotImplemented to x-amz-object-lock-mode, the probe
// reads that as a definite cannot-enforce (wormCannotBeEnforced, engine/src/admin/router-destinations.ts)
// and no R2 bucket can be created with Object Lock either. Teaching a success the product will not give is
// worse than teaching nothing: a learner who saves a compliance lock on an R2 destination and sees it
// read back as enforced would leave believing their R2 archives are immutable when they are not.
//
...
function writeAddDestination(init?: RequestInit): Response {
  ...
  const status: DestinationStatus = {
    ...
    // MIRRORS engine/src/sched/scheduler-do-dest-config.ts, including its precedence: a Microsoft Entra
    // service principal resolves to "entra" rather than falling through to "sts" or "keys", so the card
    // shows the correct Authentication row rather than contradicting the product.
  ...
  world.status = {
    ...world.status,
    destConfigured: true,
    // FOUR-WAY, mirroring the engine (demoProviderForEndpoint in demo-world.ts), so a Google Cloud or
    // Azure destination reads as its own kind on the residency panel, the topology map and the downpipe
    // drawer, rather than defaulting to S3.
...
function writeDrill(init?: RequestInit): Response {
  ...
  // The drill PASSES for a run the seeded world does not hold. It reports recordsVerified:1 and
...
function writeVerifyRestore(init?: RequestInit): Response {
  ...
  const dp = downpipeForRun(id);
  // A CLEAN blind restore test (ok:true, failures:[]) over a run that does not exist. recordsVerified
...
function writeAttestRestore(init?: RequestInit): Response {
  ...
  const id = typeof runId === "string" ? runId : "";
  const dp = downpipeForRun(id);
  // signatureValid / complete / notRolledBack, all true, for a run the world does not hold.
...
// light Apply for the owner driving the apply). Seeding the owner here would leave Apply permanently gated
// in free-explore (caller == approver). The body carries the planHash directly. Async only to share the table's
...
async function writeRestore(init?: RequestInit): Promise<Response> {
  ...
  // The restore plan for an unknown run borrows the SEEDED plan's counts below (recordsVerified,
  ...
  if (req.confirm !== true) {
    ...
    const plan: RestorePlan = {
      ...
      skipped: [],
      planHash,
      // crossAccountWarnings: the ONLY thing that makes renderConfirm's flags.isCrossAccount true, which
      // is what the visual baseline exists to render.
  ...
  // and the receipt screen offers it as a download: the one artefact a customer would hand an auditor, so
  // it must be present here too.
  //
