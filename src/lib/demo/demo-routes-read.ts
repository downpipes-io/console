// The faked engine's GET route table (see demo-seed.ts for the file layout).
...
// derives them from observation. This is what lets the guided first run be TAUGHT over the demo: on the
// training-start variant the collections begin empty, and each step's fact flips exactly when the
// learner's own write lands it (a verified destination appears in world.destinations, the discovery
// token flips sourceDiscovery.tokenPresent, an attach grows bound, a created downpipe grows
// world.downpipes, a settled run puts an ok entry in a ring). The stored flags the world cannot observe
// (keysReady and its signer/break-glass/email siblings) stay on world.setupState, flipped by the matching
// key writes. boundSourceCount counts the four seeded BINDINGS rather than the downpipe count; its only
// consumer (lib/setup-state.ts deriveSetup) reads it solely as greater-than-zero, so the guided view
// holds regardless.
  ...
  const destination: import("../api/types.ts").SetupState["destination"] =
    first === undefined
      ...
      : {
          ...
          // FOUR-WAY, mirroring the engine (demoProviderForEndpoint in demo-world.ts), so a Google Cloud
          // or Azure destination reads as its own kind rather than defaulting to S3.
...
// rather than holding a second copy of it. One fact, so every write that moves the collection or its
// default moves this view with it: a learner who verifies and saves their first destination sees it
// immediately in the New downpipe wizard, which asks this route and not the collection. The seeded
...
export function route(path: string, init?: RequestInit): Response | Promise<Response> {
  ...
  if (method === "GET") {
    switch (pathname) {
      ...
      case "/admin/retention-prune/approvals":
        return json([]);
      // The legacy singular owner-action inbox path, kept answering an empty owner-action list (the
      // dual-control restore inbox is /admin/restore/approvals above).
      ...
      case "/admin/history": {
        const id = query.get("id");
        if (id !== null) {
          // The same rule: an UNKNOWN downpipe id is answered with a fabricated empty ring, which reads on the
      ...
      // plain object fallback would error-card this disclosure on the public tour, which demo-world.ts's own
      // header forbids for an unmodelled screen load. It is the same class of defect as the retention-prune
      // inbox above.
      //
      ...
      case "/admin/signin-factors": {
        ...
        const factors = grants.map((r, i) => ({
          ...
          // `invites` is NOT optional and the panel indexes it directly: omitting it throws
          // "Cannot read properties of undefined (reading 'live')", which would error-card the disclosure
          // with a misleading message about the engine being unreachable. Zero live invites is the honest
          // demo posture: every seeded member has already enrolled.
  ...
  // screen issuing this verb means that screen's demonstration is broken.
