...
export function startDemo(): void {
  installDemoFetch();
  // The demo-state fixture knob (the harness's vision-judge-every-screen; demo-state.ts): a `?demo-state=`
  ...
  //
  // The honesty chrome installs one component at a time, each in its OWN guard: a banner mount that throws must
  // never leave the tour running with NO DEMO LABELLING AT ALL -- no banner, no corner marker, no site link, no
  // favicon swap -- on a public surface whose entire integrity rests on being visibly a demo. Each guard means a
  // component that fails costs only itself, and each failure is recorded with the component that failed, so the
  // labelling that is missing can be named rather than guessed at.
  //
  ...
  if (typeof setTimeout === "function") {
    setTimeout(() => {
      try {
        ...
        // funnel that is indistinguishable from one who chose not to take it.
