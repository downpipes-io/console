...
//
// The complete finance-flavoured Northwind Trading Co seed covers all NINE Cloudflare source types the
// engine backs up (a D1 ledger, a KV payments store, an R2 statements bucket, an account cf-config
// surface, a Secrets Store gateway-secrets pipe, an account-scoped Workers pipe, a Stream videos pipe and
// an Images pipe both capturing bytes, and an Artifact Registry pipe; runs per pipe with one FAILED and
// one in-flight; a primary plus a replica S3 destination in ap-southeast-2 + us-east-1 with WORM; SAML /
// OIDC / OAuth2 IdP connections incl. an Okta SAML signing cert expiring in 21 days; an enterprise
// licence; an eight-to-ten-entry tamper-evident audit chain; drill evidence; one restore plan; a pending
// dual-control restore approval; the posture, coverage, expiry, RTO and signed-report reads). It answers
// every /admin/* read the proof-of-moat screens (Overview, Reports, Reports evidence packs, Security
// centre, Access + identity, Runs, Verify restorability, Destinations, Restore) issue. The write handlers
// and the honesty banner build on this same shape (see demo-routes-write.ts).
//
...
// one; buildSeed() is the single constructor so a future "Back" replay can re-seed deterministically.
// ---------------------------------------------------------------------------------------------------
...
// applies the dual-signed plan. Seeding the owner as the approver would leave the apply permanently gated
// in free-explore (caller == approver), so the headline dual-control moment could be narrated but never driven.
...
// returns. Writes mutate this object in place; reads serialise the relevant slice.
export interface DemoWorld {
  ...
  // plus a seeded dry-run restore PLAN for a recent run (the restore write returns it).
  ...
  // push is the SIEM audit-log push destination, the outbound sibling of the audit-feed PULL credential
  // above (support.auditFeed). Seeded configured, enabled and caught up, in deliberate contrast with
  // support.accessPerimeter (true): the pull credential above shows an Access warning, and this one shows
  // a clean delivery trail, so the demo itself teaches the dials-out advantage rather than just asserting
  // it in copy.
  ...
  changeSeq: number;
  // attestSessions holds the pinned runs + effective sample rate for each attended-verification session
  // POST /admin/attest/session/create has minted, keyed by session id, so /prove, /capsules and /verify
  // can answer with the SAME runs the create response pinned rather than falling through to the generic
  // benignWrite fallback. Session-scoped like every other write-mutated field on this object: a reload
  // re-seeds it empty.
...
export function buildSeed(now: number): DemoWorld {
  ...
  const downpipes: DownpipeState[] = [
    ...
    },
    {
      config: {
        ...
        // "Account + this zone" shortcuts, render ONLY when a zoneId is present, so a second, zone-scoped
        // downpipe is needed alongside the account-scoped one above for those controls to be reachable by
        // any journey.
        //
  ...
  const paymentsRuns: RunHistoryEntry[] = [
    ...
    // prevRunId/prevRunIdStatus: 0040 is genuinely 0041's immediate predecessor and stays in this same
    // ring, so "retained" is the honest verdict, not a guess -- the run-detail drawer's Predecessor row
    // renders the real chained id.
    ...
    // grid on the tour and demo rather than a one-column strip; every entry follows the same hourly
    // cadence as the rest of the ring.
  ...
  const primaryDest: DestinationStatus = {
    ...
    source: "console",
    // The honest prune telemetry for a compliance-WORM bucket whose delete probe is refused: the last
    // pass ran and object lock refused its deletes (an error outcome carrying wormBlocked, exactly as the
    // engine records a WORM refusal), and there is deliberately NO lastApplied. A fabricated reclaimed
    // count here would claim deletions this bucket cannot have performed and would break the invariant
    // that a "denied" deleteProbe never renders a count. This seed feeds the public tour and the offline
    // test bed, so the fixture is the truthful posture, not a prettier one.
  ...
  //
  // downpipes writes to four kinds of store. A third destination that is not S3 makes that concrete on
  // the tour and in the training course: without it, both would show only Amazon's provider badge and
  // copy, with no immutability prerequisite an operator of a different store would recognise.
  //
  ...
  // "Reset demo" affordance must not appear. The distinct tour-mode banner is the public-demo signal.
  ...
  // A seeded dry-run restore PLAN for a recent run (the restore write returns it, so the assurance world
  // is complete). It verifies a sample of the payments run's records WITHOUT exposing
  ...
  const notifyChannels: NotifyChannel[] = [
    ...
    // Monitoring integrations: jsm and servicenow, showcasing the two incident channels.
    // apiKeyPresent:true mirrors the engine's redacted read (the credential itself is write-only and
    // never returned); no secret value appears here or anywhere else.
  ...
  const support: SupportStatus = {
    ...
    // The metrics scrape credential: active and recently pulled at a scrape-like cadence (minutes
    // apart), in deliberate contrast with the audit feed's daily cadence above, so the demo shows a
    // Prometheus/Grafana agent actively polling.
    metrics: {
  ...
  // leads with ndjson over http on purpose, so the demo reads generic-first (the split-friendly default),
  // not tied to any one vendor. This is a deliberate contrast with
  ...
  const sourceDiscovery: SourceDiscovery = {
    ...
    cfConfigSurfaces: [
      ...
      // Two account-scoped surfaces that ARE in band, so the account-scoped picker (which is what the
      // seeded cf-config downpipe renders, it has an accountId and no zone) exercises both sides of the
      // split rather than only ever reporting "0 of the 1 re-apply": a console that marked nothing at all
      // could otherwise pass unnoticed. scope, tier and inBand here all match the engine.
