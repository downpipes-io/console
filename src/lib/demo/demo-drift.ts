...
// TOUR_DEGRADE_STAGES is WHERE the guided session broke. Each member names a different repair.
// chrome-install-failed exists because each honesty-chrome component installs under its own guard: one
// failure never silently drops the others, so the tour is never left with no demo labelling at all.
//
// anchor-missing and anchor-not-visible are two separate members, not one, because they call for different
// repairs and the DOM tells them apart: querySelector finding no element at all (a renamed data-tour-id) is
// anchor-missing, and an element present with a 0x0 box for the whole retry window (a reveal that never ran)
// is anchor-not-visible. An anchor that IS in the DOM with a real box, where scrolling simply could not seat
// it in the band on a short screen against a fixed bottom bar, is deliberately NOT a member: there is nothing
// to repair there, so a row for it would be a false alarm on a legitimate state and would teach a reader to
// ignore the other two. The director classifies only what the DOM actually established, and each member below
// names only that fact:
//
...
//                       NOT A MEMBER, ON PURPOSE. There is nothing to repair, so a row here would be a false
//                       alarm on a legitimate state, and the noise would teach a reader to ignore the other two.
...
//
// This function is the entire no-custody argument, so it is written to be read: there is no branch
