...
// rebuilds it for a deterministic "Back"/Restart replay without a page reload.
export let world: DemoWorld = buildSeed(Date.now());

// resetWorld rebuilds the pristine seed. Exported so the tour director and the tests can return
...
// route without threading a pathname through every handler signature. The demo router is
...
// reader cannot silently drop the drift note.
...
export function readBody<T>(init?: RequestInit, pathname = currentWritePath): Partial<T> {
  ...
  try {
    ...
    // The write body could not be parsed, so the faked engine applied ITS DEFAULTS and answered success.
...
// queuedChange mints the deferred-mutation 202 body, matching THE BODY THE ENGINE ACTUALLY SENDS:
// { queued: true, id, status, contentHash } (sched/scheduler-do.ts, the one gated-config 202 there is).
//
// This shape matters because the console's own queued-for-approval guard is written against it: a demo that
// answered a different shape would let the pending toast and its link into the approval inbox render here
// while the guard silently misses against a real engine, so every 202 would degrade to "applied" and the
// operator would be told a queued change had been saved when it had not. A simulator that answers a body the
// real thing never sends does not de-risk the real path, it hides it. One helper, shared by both write-route
// modules, keeps the demo from drifting off the wire.
...
//
// WHY THE DEMO NEEDS IT AT ALL. The product supports four destination kinds, so deriving the kind with a
// bare `includes("r2.cloudflarestorage.com") ? "r2" : "s3"` check would misreport a Google Cloud or Azure
// destination as "s3". That answer is read by the residency panel (the screen that says whether data leaves
// the Cloudflare account), by the topology map's destination phrase, and by the downpipe drawer, so a wrong
// answer there means the course would teach the wrong vendor for the learner's own destination, on the
// screens whose whole job is to say where the archives went.
//
...
export function notImplemented(method: string, path: string, kind: DemoDriftKind = "fallback-substitution"): Response {
  // An honest 501 is still DRIFT. The console asked the faked engine for something it does not model, and
...
export function benignGet(pathname = "/admin"): Response {
  // THE fabricated-success funnel on the read side. This 200 makes an unmodelled screen render blank and
  // healthy, which is the right thing to do to a prospect mid-tour, but it also means console-vs-demo drift
  // can go unnoticed for a long stretch without this event firing. The route PATTERN (product words only, ids
  // replaced) is emitted so the routes the table outgrew can be enumerated instead of discovered by a human
  // replaying the tour.
...
export function benignWrite(pathname = "/admin"): Response {
  // The same funnel on the WRITE side, and the more serious of the two. This 200 answers "deleted: true,
