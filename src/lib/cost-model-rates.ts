...
// DAYS_PER_MONTH ROUNDS the spec's averaging constant 365.25 / 12 = 30.4375 to 30.44, so any figure
...
// (engine/src/dest/types.ts: MAX_STREAM_SEGMENT_BYTES = 1 GiB, the engine's single-put
...
// DEFAULT_OVERHEAD_BYTES is the spec's modest per-run fixed overhead O (root and shard
...
// R2's zero egress is presented as a fact, not a sales claim. (Source: the spec's section 4
// figures; verify against current published pricing before relying on them.)
...
// against that list.
//
// WHY THIS EXISTS. A preset rate can look fine when nothing here has ever verified it, because it can be
// correct by a stable vendor rather than by any control: a less stable vendor's list can reprice while the
// table sits unnoticed. A number that is right by luck reads identically to a number that is right by
// design, and only one of them stays right.
//
...
// against the observed reprice interval rather than as a round number: the non-Cloudflare presets have
// repriced twice within about two months, so a window materially longer than that would let a whole
// reprice cycle pass unnoticed, and a much shorter one would fire on a market that had not moved.
...
//                 addressing key is a recorded SPEC-level decision, not current
...
// ---------------------------------------------------------------------------
// Cadence and retention-window conversions (spec section 2)
// ---------------------------------------------------------------------------
...
// DAYS_PER_MONTH (30.44). These match the spec's stated approximations: every 15 min ~= 2880,
...
// window or f yields 0 (keep everything). spec section 2.
