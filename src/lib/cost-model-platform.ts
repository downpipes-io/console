...
export const SOURCE_COST_PROFILES: Readonly<Record<SourceType, Readonly<SourceCostProfile>>> = {
  ...
  // until the engine meter reports real D1 rows read.
...
// the Worker invocations. A labelled ESTIMATE (the engine meter will report exact counts);
// deliberately generous, per the over-estimate stance.
...
// the bridge until the engine meter persists exact per-resource counts.
