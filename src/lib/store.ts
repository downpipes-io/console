// The in-memory app state (build-contract 2.1). A single module-scoped store
...
// is what makes "clear sensitive state on navigation / sign-out" (current-state.md
// S2) a single call. Nothing here is persisted except the connection target and
...
import { ATTEND_SESSION_KEY, DRAFT_PREFIX } from "./storage-keys.ts";
...
export function connect(url: string, token?: string): EngineClient {
  ...
  try {
    ...
    // Best-effort; a blocked localStorage just means the URL is not remembered, which surfaces later as
    // "we have to re-point the console at our engine after every refresh" -- recorded here so that is
...
export function restoreConnection(): EngineClient | null {
  ...
  try {
    ...
    // A blocked read silently falls back to this origin, so a customer whose engine is elsewhere would land
    // on a console pointed at the wrong place with no explanation at all if this were not recorded.
    url = null;
  ...
  try {
    ...
    // The remembered engine URL can be POISONED. When the client cannot be constructed, no client is
    // constructed at all, so NO ENGINE CALL IS EVER ATTEMPTED, and every other diagnostic in the ring is
    // therefore silent by construction: the engine-call seam never fires, the transport classifier never
    // runs, and the pack that comes out the other end says nothing is wrong with a console on which nothing
    // works. The operator's report is "every screen says Not connected"; the cause is a string in
    // localStorage that this build's stricter validation rejects.
    //
...
// These four delegate to lib/caller-state.ts, a leaf that imports nothing but its own type, so the store
// keeps its public API (no caller changes) while the caller state lives outside the EngineClient import
// path, avoiding an import cycle between the store and the screens that read it. See caller-state.ts's
// header.
...
// ---- clearing sensitive state (current-state.md S2) -------------------------
...
// clearSessionData purges every piece of AUTHENTICATED-SESSION data the browser holds, in memory and in
// storage (ASVS V14.3.1): the caller identity, the entity and search caches, the ceremony material and the
// posture choice; every wizard draft in sessionStorage (the DRAFT_PREFIX family) and the attended-session
// resume record in localStorage. It runs on the explicit sign-out AND on a server-initiated end of session
// (an idle or absolute expiry, an admin's terminate: the 401 the next request meets), and it needs no
// server round trip, so it clears when the engine is unreachable too. Preference keys (theme, view mode,
// motion, the remembered engine URL) are not session data and are left alone. Idempotent.
export function clearSessionData(): void {
  ...
  // Collect first, then remove: removing while iterating sessionStorage.key(i) skips entries.
  try {
    const doomed: string[] = [];
  ...
  try {
    localStorage.removeItem(ATTEND_SESSION_KEY);
  } catch {
    // Best-effort, as the attend screen's own clear is: an unremovable resume record holds a session id
    // and a sample rate, never a key, and the engine refuses a resume the session no longer backs.
  }
}

// signOut clears the in-memory engine client (and thus any in-memory bearer
// token), the caller, the caches and the session's browser storage. Auth is at the edge, so this is a local
