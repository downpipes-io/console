...
// amber "Renews soon" cue. Every licence renews as a FRESH MINT on invoice, with the vendor issuing a
// new token each term and emailing it. The operator pastes the renewed token into Activate licence on
...
// licenceExpired answers whether this customer's licence has passed its validity date.
//
...
// of an expired Enterprise licence is "community", exactly like a customer who never bought one: the
// screen cannot tell the two apart by tier alone.
//
...
// so an invalid licence carrying a notAfter already in the past is expired whatever else is true of it.
// Both routes matter: either one failing silently would leave an expired licence unreported.
...
//
// A self-serve licence binds to Cloudflare accounts at claim, up to its band's estate count
// (control-plane's licence/claim.ts
...
// licenceAccountUnknown is true, so a customer with a fresh deployment can tell "account binding does
// not apply yet" apart from "it already happened" rather than seeing nothing at all here. It states the
// plain cause and the plain remedy (nothing to do, it resolves itself), and it must never read as a
// fault: fail-open holds regardless, and claiming with the licence's own claim code still works, it
// simply cannot bind THIS account until the engine has proven it.
...
//
// `previous` is the status the screen was rendered from, before this activation. It is optional: a caller
// with nothing to compare against sees no shortening clause, and when it is supplied the shortening
// clause is APPENDED rather than replacing anything: a shortened licence and a
...
// vocabulary (control-plane's src/licence/issue.ts) is estate-banded: business-1/business-3/
// business-10/business-25 name the four fixed Business checkout tiers. The four business-N strings are
// copied VERBATIM from control-plane's own tierDisplayName() (the same function the self-serve
// activation email's tier_name reads), so a customer sees the exact same words in the email and in the
// console. "msp" does not read fine title-cased either ("Msp" is wrong), so every call site uses
// tierDisplayName() rather than titleCase() directly. "MSP / MSSP" (spaced slash) is the website pricing
// page's spelling, made canonical here; control-plane is outside this file's reach and still says
// "MSP/MSSP" with no spaces. The two non-self-serve ids (Community, Enterprise) are included too so the
// whole tier badge/tile/toast family can share the one function instead of special-casing five ids and
// title-casing the rest. Display metadata ONLY: it never gates a feature (the same framing rule as
// ENTERPRISE_SERVICES above -- a tier name is not a capability list).
