...
export async function refreshExpiryCount(shell: Shell): Promise<void> {
  ...
  try {
    ...
    //
    // Keeping the chip is right, keeping QUIET about it is not: a status read that is persistently failing
    // leaves the credentials count frozen at whatever it last was, so a credential that expired while the
    // read was broken is never flagged, and the chip looks like a healthy zero rather than an unknown.
...
export function updateChipView(upd: UpdateStatus, ownConsoleVersion: string | null): UpdateChipView | null {
  // The engine consults the channel host and RETURNS the verdict here; the console records it before hiding
  // the chip, rather than hiding the chip and throwing the verdict away. A channel whose SIGNATURE is failing
  // and a healthy verified channel with nothing new to offer would otherwise both produce no chip and an
  // identical pack, because status.updateChannelConfigured is Boolean(env.UPDATE_CHANNEL_URL &&
  // env.UPDATE_SIGNER_PUBLIC), an env-var presence check that stays true while the signature fails. Presence
  // is not a verdict; this row is.
  //
  // Only INTENDED-BUT-UNVERIFIED is recorded. `configured` is the ENGINE'S VERDICT, not the operator's
  // intent: the engine answers configured:false for an unparseable UPDATE_CHANNEL_URL, a non-https one, and
  // an UPDATE_SIGNER_PUBLIC that will not parse. In all three BOTH env vars are set and the operator plainly
  // meant to have updates, so a truncated or whitespace-mangled paste of the signer key -- a base64url blob
  // the operator pastes by hand, and one of the likeliest ways an update channel silently freezes -- is still
  // recorded as an intended-but-broken channel, never silently dropped into a pack byte-identical to a
  // healthy verified channel with nothing new to offer.
  //
  // An engine with NEITHER channel env var set is channelIntended:false and records nothing. That is a
  // customer who does not want update notifications, it is the product working as chosen, and a fault row on
  // it would cry wolf on every healthy estate that never wired the channel. A VERIFIED channel records
  // nothing either.
  //
  // The reason is CLASSIFIED, never carried: channelReasonClassFor prefers the engine's own CLOSED
  // channelFault and falls back to the prose classifier, which reads upd.reason only to SELECT a member. The
  // text -- which can embed the channel URL and the signer key id -- is discarded; clamping it into the pack
  // would be a leak with a length bound on it, and a clamp is not a redaction.
...
export async function refreshUpdateChip(
  ...
  try {
    ...
    // This deliberately does not compare ownConsoleVersion against upd.currentVersion: upd.currentVersion is
    // ENGINE_VERSION, the ENGINE's own running version, and the console's is package.json's. They are two
    // independently incremented numbers from two separate repos, so comparing them would fabricate a skew on
    // every healthy customer.
    //
    ...
    //
    // "hidden stays hidden" is indistinguishable, on screen and in the pack, from "there is no update": an
    // operator stuck two releases behind, never told by the console, is this read failing silently, and
    // nothing anywhere records that it did.
