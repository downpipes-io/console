...
// the call site, so there is no markup surface here.
...
//
// The DEFAULT arm is not defensive padding. The switch is exhaustive over the declared union but carries no
// safe floor without it: an engine that adds a source type this console build does not know must still
// render something, not "uploads (undefined)" on the Overview lead line, the downpipes table and the map.
// That is a readable row with an unfamiliar word, not a corrupt one, so the honest read is to name what
// arrived rather than to refuse the row: the type IS the label when there is no better one. The value is a
// closed engine enum (never operator text), so echoing it carries no customer data, and it reaches the DOM
// through the same h() textContent path as every other string here.
...
// unparseable stamp is handled by its clause as UNREADABLE, never routed through here, so a corrupt
// stamp can never misread as healthy (see stampMs).
...
// DO field, a non-string the type promised was a string) must never resolve through Date.parse to
// NaN, which isOverdue would judge NOT overdue, making the clause claim the check had passed
// recently and the downpipe read "covered" off a value nobody can read. That is the worst kind of
// false state: it is indistinguishable from health. An unreadable stamp gets its own honest read
// (see unreadableClause), never a pass.
...
//
// It also RECORDS the corruption, and records WHICH stamp: a support engineer working from a diagnostics
// bundle needs to know which assurance failed to parse without eyeballing every stamp on every downpipe.
// `fieldClass` is the discriminator, and a generic `timestamp` row would not be one: these three stamps
// make three DIFFERENT assurances on the Overview (a restore has been tested, integrity has been
// checked, offline restorability is proven), and a corrupt one is the customer believing a specific
// thing that is not true. As one row they would coalesce and support could not say WHICH assurance was
// lost. The malformed value itself has no field here.
...
export function protectionStatement(
  ...
  // THE GUARD EVERY SIBLING IN THIS FAMILY CARRIES.
  //
  // Every read of engine-supplied state must guard both the CONTAINER (the downpipe LIST that settles
  // ok:true carrying no array, a history MAP that comes back undefined, a licence OBJECT with no tier) and
  // each ROW INSIDE a container that arrived well-formed: an unguarded `cfg.source` or `cfg.source.type`
  // inside an already-passed list guard can still throw and blank the whole Overview from a single
  // malformed row, freezing the first-load skeleton for ever because the TypeError escapes an async load
  // with no catch.
  //
  ...
  // destinationFromStatus returns configured: null when the engine status could not be read at all, distinct
  // from a confirmed `false`. Both `tone` and `sentence` must carry this distinction: every consumer reads
  // `tone`, so a caveat that lived only in `sentence` would never reach a screen that renders tone alone.
  //
  // The absent verdict is neither a match nor a mismatch: the sentence still names what could not be
...
export function fleetProtection(
  ...
  // as the four. A zero adds nothing, exactly like every other arm here, so a healthy fleet's summary never
  // carries this text at all.
