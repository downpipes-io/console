...
// The self-owned screen descriptors. The router wiring registers each screen's route(s)
// FROM this one list via screenRoutes(), and the command palette reads their actions via
// allScreenActions(), so a screen is the single owner of the routes it serves and the
// actions it contributes. These are the elevated area modules: sources-downpipes (screen
// 2), restore-flow (screens 3+4 restore family), access-security (the unified
// Access/Roles/Audit/Fallback governance area, screens 6+7), onboarding-ceremony (the
// first-run wizard + key ceremony, screen 10), and the command palette's own
// deep-linkable landing; runs, keys, settings and licence keep their own descriptors.
// Overview is handled separately in app.ts
...
function reconcileRegistry(...lists: (typeof COMMANDS)[]): typeof COMMANDS {
  ...
  for (const list of lists) {
    for (const cmd of list) {
      ...
      // A REPEATED CHORD IS A DUPLICATE CHORD, NOT A DUPLICATE COMMAND: only the chord is dropped, the
      // command itself is kept, even when its shortcut collides with an earlier command's (e.g. "Restore
      // from a run", group Actions, keywords recover / roll back / dry-run, would otherwise vanish from
      // the palette entirely for declaring the same shortcut "g s" as the static go-restore row).
...
//     Settings and Licence) stay the canonical chord map the IA names; a screen
...
// own `route` fields, never typed out by hand, so it cannot silently fall behind them the way a
// hand-copied list could. Nothing here should ever need to add a route by
 ...
 *
 * This export can read as unused: its only consumer imports it DYNAMICALLY, which a static analyser
 * cannot follow. The gate resolves this module at run time (`await
 * import(pathToFileURL(resolve(CONSOLE_ROOT, "src/lib/app-registry.ts")))`,
 * scripts/route-table-gate.mjs:381) precisely so it can be pointed at a DIFFERENT console checkout than
 * the one it lives in, which a static import cannot do. Do not delete the export to satisfy an unused-export
 * check: that removes the very thing the gate compares against, and the gate would then fail on a missing
 * symbol rather than on a real route drift.
 */
...
// The routes that run full-bleed without the shell chrome (the one exception). The passkey
...
// operator leaves the ceremony context (no-custody hygiene).
