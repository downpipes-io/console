  ...
  while (index < total) {
    ...
    try {
      ...
      if (isUnauthorised(err)) {
        // C3 bulk-outcome: a session expiry CUT THE BATCH SHORT. The items past this point were never
      ...
      if (isRouteMissing(err)) {
        // C2 contract-drift: the engine's router does not know POST /admin/downpipes/bulk, so this console
...
async function singleFallback(
  ...
  for (let i = from; i < items.length; i++) {
    ...
    try {
      ...
      if (isUnauthorised(err)) {
        // C3: the same halt, on the per-item fallback leg. `total - settled` is the remainder that was
        // never attempted.
        out.halted = true;
        recordBulkOutcome("create", "auth", total - settled);
        return out;
