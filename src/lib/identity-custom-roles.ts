...
//
// The strip is correct and stays; it must not be silent. A custom-role holder whose engine names
// a capability this console has never heard of loses the buttons that capability gates, and that reads as a
// console defect unless it is recorded. The count is all that travels. The unrecognised id NEVER does, and on this field that is not a
