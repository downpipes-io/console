...
// (ENTERPRISE-UX-BLUEPRINT section 7.3), kept apart from the DOM-facing custody component so the
// round-trip can be asserted directly: a share file parses back to the exact bytes that combine() to the
// wrapping key; the envelope file decrypts; and the (PUBLIC) credentialId rides with the ciphertext so a
// future restore can re-derive a security-key wrapping key.
//
...
// wrong kind and a future format can be detected. These strings are stable, so a file a
// user already downloaded keeps parsing.
