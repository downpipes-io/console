...
// dash-form literal. Values are taken from the engine's own CHANGE_WRITE_CAPABILITY (admin/change-control.ts),
...
// (not roles.write), a notify channel/rule set or delete needs notify.config, an expiry item set or delete
// needs expiry.config, a posture accept/unaccept needs posture.riskaccept, a coverage-inventory write needs
// access.policy and a cf-config capture-mode set needs downpipe.write. All 18 engine kinds are covered; a
// webhook channel is a notify-channel-set like any other channel kind, with no separate kind of its own.
...
// Pure; the caller renders text as a text node (no markup).
