...
//
// WHY A WATCH, NOT A SINGLE READ AT BOOT. The recovery latch is a STANDING condition that the engine raises
// OUT OF BAND: the */15 cron health pass detects amnesia (the control plane is empty but the destination
// bucket still holds the account-global _RECOVERY/RUNLOG marker) and latches recoveryRequired, at a moment no
// console tab is party to. A tab that reads the latch only once, at boot, keeps that answer for the rest of
// its life: if the plane was healthy at boot, the banner host stays empty and the operator can sit on an
// Overview headed "Your console is set up" while scheduled backups have actually stopped. That is the exact
// false all-clear the banner exists to prevent.
//
// A single boot read is also unrecoverable within the tab: neither an in-app navigation nor a same-URL
// revisit re-reads the status, so a console that read the latch before it was raised stays wrong, silently,
// for the state it is most important to get right.
//
...
//     to tell.
//
// THE AUTHORITY LEG, and why the three legs above are not enough on their own. This read is not
// identity-independent in the way the boot leg is identity-independent: the check may RUN without identity,
// but GET /admin/control-plane/status answers 401 to a caller with no credential, and 401 is mapped below to
// the deliberately SILENT class. Now consider the one state this whole feature exists for. A SchedulerDO wipe
...
// onAuthenticated: navigate plus re-resolve identity), never a page load, so nothing re-arms the watch on its
// own. The interval leg can be up to two minutes away and the focus leg is floored at thirty seconds from the
// boot read, so a console that has just learned who the operator is would not re-ask the one question whose
// answer is "your backups have stopped" until one of those legs comes due, which can leave a healthy-looking
// Overview on screen well after the console's own same-origin read would report recoveryRequired=true.
//
...
// sign-in call site is deliberate: a future entry point that resolves identity gets the re-read for free. The
// leg is NOT floored -- authority changes are operator actions, not a poll, and a floor would swallow the
// moment that matters. It is an ADDITION: the boot leg still runs before and independently of identity, so a
// whoami that never resolves still cannot suppress the banner.
//
...
//                 ordinary signed-out path, which this identity-independent check always reaches. Silent,
//                 and NOT recorded (it fires on the healthy signed-out path).
...
export async function checkControlPlaneRecoveryOnce(deps: RecoveryWatchDeps): Promise<void> {
  ...
  try {
    ...
    updateRecoveryBanner(status, {
      ...
      // The ACKNOWLEDGE-ONLY latch clear, the one exit an ESTABLISHED account has and the route
