...
export const RECOVERY_REFUSAL_CODES = [
  ...
  "DP-R03", // the JSON parsed, but it is neither a v:1 plaintext control-plane export nor a v:1 SEALED one (the estate-import paste admits both shapes)
  ...
  // DP-R12 alone cannot distinguish signature, shape-check and no-custody failures, so the engine returns a
  // CLOSED refusalClass in its 400 body (refusalClassToCode below admits it by set membership) and these
  // three each get their own code.
  ...
  // The engine verifies BOTH halves of the hybrid signature over the SAME export bytes, so a failure that
  // leaves either half verifying PROVES the export intact and puts the damage in the operator's own kit.
  // Reading that out as a tamper code (DP-R15) would tell a customer their disaster-recovery artefact had
  // been altered when the remedy is "take another copy of your recovery kit", on the one path where the
  // artefact in front of them may be the only one left. Each verify outcome below gets its own code:
  ...
  // seven also have an on-the-wire twin (DP-R29/DP-R30 below); see there for why the twin is a separate code
  ...
  // a local refusal and a wire refusal one indistinguishable row.
  ...
  // The ACKNOWLEDGE-ONLY latch clear (POST /admin/control-plane/acknowledge-recovery), the one exit
...
// ---- the engine's 400 sub-cause ------------------------------------------------------------------------------

// ENGINE_REFUSAL_CLASSES mirrors the engine's own frozen RECOVERY_REFUSAL_CLASSES (engine
// src/admin/diag-records.ts). refusalCodeForStatus alone maps a status to a code, and the engine answers 400
// for a bad signature, a failed shape check, a no-custody refusal and a plane that was not empty alike, so a
// bare status code cannot tell "wrong token vs signature vs shape vs no-custody vs not-Owner" apart: without
// this set, three of those five states fuse into one row that coalesces on the ring's tuple key, and support
// cannot tell a tampered export from a hand-edited one from a wrong kit.
//
...
// ENGINE returns a CLOSED `refusalClass` member in its 400 body, and the console admits it ONLY by SET
...
const ENGINE_REFUSAL_CLASS_SET: ReadonlySet<string> = new Set([
  ...
  // The two classes the ACKNOWLEDGE route answers by name
...
const REFUSAL_CLASS_TO_CODE: Readonly<Record<string, RecoveryRefusalCode>> = {
  ...
  // Two branches, two codes, opposite next steps: DP-R31 needs an owner role granted with the
