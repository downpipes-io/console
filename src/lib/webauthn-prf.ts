...
// experience. A single YubiKey 5 CANNOT store
...
export async function enrolSecurityKey(opts: {
  ...
  // verification, the browser blocks the ceremony outside a secure context), so the rejection is caught here
  // and turned back into the null the contract promises, rather than left to surface as an unhandled rejection
  // with the ceremony half-finished on screen. Nothing about the error is read: not its message, not its name.
...
export async function deriveWrappingKeyFromAssertion(opts: {
  ...
  // The same contract applies here: a cancelled tap, a key that is never presented, or a failed
  // user-verification step all REJECT here, and this function promises a null.
