...
//
// THE NAMED DESTINATION IS SETTINGS, NOT A SUPPORT SCREEN, and both the destination and its reachability
// are checkable. There is no Support screen: the route-table gate binds 53 patterns and none of them is
// /support, there is no Support item on the rail, and the pack lives on SETTINGS, in a disclosure titled
// "Support bundle" (screens/settings.ts). The reachability claim holds on both axes: /settings is in
// setup-state.ts ALWAYS_ALLOWED, so it is reachable mid-setup as well as from the viewer floor, which is
// where a first-hour customer meeting this actually stands. The remedy's own validator asserts the words
// "support pack" and "every signed-in role can reach", not that the named destination exists, so a future
// rename here would not be caught automatically.
