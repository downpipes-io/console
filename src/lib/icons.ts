// The bespoke inline-SVG icon set. Each value is the inner markup of a 24px-grid icon, drawn with
// currentColor and a 1.5px stroke by the svgIcon() wrapper in dom.ts, so icons theme for free and add
// no network request. No icon font, no icon library (a supply chain risk). Only the glyphs the
// console needs; add one when a real surface needs it.
//
// House rule: the BRAND MARK is locked and used everywhere. It lives here as the single source so it
// cannot drift.
