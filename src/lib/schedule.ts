...
// The cron grammar, the timezone validation, and the next-fire preview live in sibling modules and are
// RE-EXPORTED here so importers of schedule.ts keep their import paths unchanged; the public API of
...
//
// THE CLAMP IS A SILENT EDIT OF THE CUSTOMER'S SCHEDULE, so it is recorded. An engine-supplied
// blackout bound outside 0..1440 (or a non-finite one) is coerced to the nearest legal value and renders as a
// perfectly plausible "24:00" or "00:00". The operator sees a normal window, has no way to know it is not the
// one stored, and their next save writes the CLAMPED value back over the real one: the masking and the
// rewrite would otherwise both be invisible.
//
// The coercion is KEPT (a form cannot render an illegal time), and it is flagged. The VALUE never rides:
...
export function minutesToTime(total: number): string {
  ...
  // A NEGATIVE bound is recorded as `negative`, not `out-of-range`. Both are true of it in plain English,
  ...
  // later than stored). `negative` already fits exactly.
...
function validateBlackoutWindows(windows: unknown): string | null {
  ...
  for (const w of windows) {
    ...
    // Mirrors the engine's equal-start-end reject (config-validate.ts), so a fat-finger of the SAME time
