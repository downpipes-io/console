// The console entry point: it is app.ts because that is the esbuild entry and the
// <script> in index.html. It runs the pre-paint theme hook, restores the engine
// connection, mounts the app shell once, registers every IA route against the small
// router, wires the shell handlers, resolves the verified identity (degrading
// honestly when whoami is absent), and starts the router.
//
...
// The window-level fault handlers are installed at MODULE SCOPE, before boot() is called below, so a
// rejection thrown during boot itself is still seen. They record a coarse closed CLASS and nothing else,
// and they filter out the browser's own noise (an aborted fetch, a cross-origin extension script); see
// lib/client-diag/window-faults.ts.
...
async function boot(): Promise<void> {
  ...
  // envelope of the pack the customer generates, beside the engine's version, so "your audit table says
  // unrecognised target (newer engine?)" can become "deploy console X": the pack identifies the engine, and
  // this is what also identifies the console the browser is running.
  setConsoleBuild(consoleVersion());
  // Ask, once, whether this tab is running the bundle its own ORIGIN is serving. That is the stale-asset
  // diagnosis: it is the one version comparison the browser can make honestly, because both numbers come
  // from the same repo and the same release, and it needs no engine call at all. Fire-and-forget: it never
  // throws and boot never waits on it.
  void recordServedBundleSkew(recordConsoleSkew);
  try {
    // The post-apply console build check writes its outcome and then, ~1.5s later, reloads the tab to finish
    // the update, and the reload destroys the in-memory ring: without this drain, `confirmed` is lost on every
    // successful console apply, making "no console-build-check row" indistinguishable from a healthy update.
    // Drain the ONE closed class the check stashed across the ONE reload it itself triggered, before anything
    // else can record. Set-membership admitted, deleted on read.
    drainBuildCheckHandoff();
    await bootInner();
  } catch (err) {
    // A boot-fault: the console failed to come up. This is the fault an operator can least describe (there
    ...
    // as it does (the re-raised rejection also reaches the window handler, so a boot crash registers as both
    // a boot-fault and an unhandled; both are true of it, and the kinds are distinct).
...
async function bootInner(): Promise<void> {
  ...
  // 0. Public faked tour: a single guarded branch that does NOTHING unless the
  ...
  // Wire the step-up re-auth ceremony onto every engine client: a sensitive action that
  ...
  // External-link interstitial: warn before navigating outside the app.
  ...
  // 6b. START THE RECOVERY WATCH. The engine latches recoveryRequired out of band (its */15 cron health
  //     pass detects control-plane amnesia), so this is a standing condition that can turn true while a
  //     tab is open. It is deliberately started HERE, beside the identity resolve rather than inside it:
  //     nesting it inside resolveIdentity's try would make the loudest banner in the product conditional
  //     on an unrelated read succeeding, and would sample it only once per page load, so a tab that booted
  //     healthy could never learn that backups had since stopped. The watch re-polls on an interval and on
  //     focus, and raises the banner only on a CONFIRMED recoveryRequired=true (lib/recovery-watch.ts).
  startRecoveryWatch({
    engine: () => getEngine(),
    onReconciled: () => {
      void resolveIdentity(shell, router);
...
function installAfterEach(router: Router, shell: Shell): void {
  router.afterEach((match) => {
    ...
    // Attribute any subsequent console-diagnostics record to this screen. What is handed over is the
    // resolved route TEMPLATE (for example "/downpipes/:id"), never the concrete path, and screenFromPattern
    // maps it through a table whose values are frozen screen LITERALS, so the value stored is always a
    // member of the closed vocabulary and a customer id could not survive the mapper even if one reached it.
    // Nothing is read from location/history/router params at emit time.
...
function performSignOut(shell: Shell, router: Router): void {
  ...
  // This catch is the one whose silence has a SECURITY consequence, not merely a cosmetic one. The
...
// preserved as ?next= so a successful sign-in returns the operator where they were going. The console must
// work with passkey OR Access OR token: the passkey screen names Access and the token as the other ways in
// alongside the passkey option, so this is the right default. The /signed-out screen stays registered for a
// direct deep link, but the 401 default is the live sign-in.
//
...
function routeToSignIn(router: Router): void {
  // The session ended on the server's say-so (a 401 met by a screen or the identity resolve), so the
  // authenticated data the browser holds is cleared here, before the sign-in screen, without a round trip.
  // The engine client and remembered URL stay: the sign-in screen needs the engine address.
  clearSessionData();
...
function registerRoutes(router: Router, shell: Shell): void {
  const bind = (pattern: string, screen: Screen) =>
    router.add(pattern, (match: RouteMatch) => {
      // The guided-setup gate (the forcing the owner asked for): while setup is
  ...
  //
  // Without a record, the redirect is a silent one: it is right for the operator, but it leaves support
  // blind, since a customer who clicked a deep link in an alert e-mail lands on the dashboard and nothing
  // says a link was lost. The state that matters is not a typo, it is a console version that RENAMED or
  // REMOVED a route, after which every alert link already in flight dead-ends for everyone, silently.
  // recordDeepLinkLost is the producer for the `deep-link-lost` kind; it records the FACT and never the
  // path (an unroutable path is customer-influenced and routinely carries a downpipe id).
  router.notFound(() => {
    recordDeepLinkLost();
...
// signed-out route through the router, so chrome and history stay consistent. It builds the screen's
...
// landed, closing the identity-stale-gate race (lib/client-diag/identity-gate.ts): a hard reload or deep
// link onto an owner-gated screen (/security, /security/owner-actions, /access/roles/builder) paints it
// while whoami is in flight, so its client gates fall back to the fail-closed `viewer` default. Without this
// re-render the owner controls would stay disabled (populate absent, retire "owner only") until a manual
// re-nav. It is deliberately narrow:
...
function reRenderCurrentScreenForResolvedIdentity(shell: Shell, router: Router, forceForBlindGate: boolean): void {
  ...
  // with, OR when the on-screen screen has a gate that ran blind: a same-identity re-resolve during which
...
// never shows a green verified chip it cannot back.
//
...
async function resolveIdentity(shell: Shell, router: Router): Promise<void> {
  ...
  try {
    ...
    // THE IDENTITY HAS LANDED. Any capability gate that already ran, on any screen, ran against the `viewer`
    // default because this round trip had not returned; nothing has yet re-rendered those screens. The
    ...
    // Capture whether the ON-SCREEN screen has a blind gate BEFORE noteIdentityLanded() clears the
    ...
    if (!firstIdentityApplied) {
      ...
      // The CONSOLE/ENGINE VOCABULARY SKEW, recorded at the one seam that sees the caller's resolved
    ...
    // `viewer` default, and without this call nothing re-renders it when the report arrives -- so a genuine
    // Owner would sit looking at disabled owner controls (populate absent, retire "owner only") until a
    // manual re-nav. Now that the VERIFIED caller is stored, re-render the on-screen screen so its owner/role controls
    ...
    if (isUnauthorised(err)) {
      ...
      clearSessionData(); // the server ended the session; purge what the browser held for it
      shell.setCaller(null, false);
      return;
    }
    // whoami absent (D1 pending) or transient: honest degrade, never a faked role.
    //
    // RECORD THAT THE IDENTITY REPORT WAS ABSENT, NOT LOW. From here every capability gate in the console
    ...
    // two states, "your role is viewer" and "we never learned your role", are otherwise indistinguishable in the
    // tab and absent from the pack; this row is what tells them apart, and httpClass separates an engine that
    // REFUSED the read (4xx/5xx) from one that could not be REACHED (network/timeout), which is a different
    // remedy.
    // httpClass ALONE COALESCES 404 WITH 403. httpClassForStatus collapses 401, 403 and 404 alike to "4xx", so
    // a whoami 404 -- an OLDER ENGINE THAT DOES NOT SERVE THE ROUTE (the cause of a "whoami absent (D1 pending)"
    // identity-pending chip) -- and a whoami 403 would produce the byte-identical row and coalesce into it.
    // Route-absent is a state worth telling apart from a refusal, so the recorder carries faultClass too, which
    // maps 404 to "not-found" and 401/403 to "auth": upgrade the engine, or fix the caller's access. Two rows,
    // two remedies.
    const httpClass = httpClassForThrown(err);
    if (httpClass !== "aborted") recordIdentityUnresolved(httpClass, err);
    // AND PUT THE SAME DISTINCTION WHERE THE CUSTOMER IS. faultClassForError separates not-found (the engine
    // predates the route, so update it) from auth (the read was refused, so fix access) from transport (it
    // could not be reached), and puts all three only in the support pack. Every gate in the console otherwise
    // refuses because the role could not be read and stops there, which is a dead end the console already has
    // the answer to. This adds no gate and lifts none; see lib/identity-remedy.ts.
