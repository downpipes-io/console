...
// leaves it: a strict script-src is what stops injected script from exfiltrating it.
//
// The recovery sheet is opened as a blob: URL via <a target=_blank rel=noopener> (see
// onboarding-ceremony.ts openRecoverySheet and keys.ts), navigated into a NEW top-level browsing
// context. A blob: URL opened this way is served with no HTTP headers of its own, so the console's
// header-delivered CSP does not propagate to it. The blob document runs with a null origin and no
// inherited policy, so the print button's addEventListener script (recovery-sheet.ts:128) requires
// no CSP exemption here, and script-src carries no 'unsafe-hashes'.

import { type LogFields, log } from "./log.ts";
// ENGINE_BINDING_ABSENT is the frozen code this worker answers with when an engine-surface path arrives and
...
import { connectionSpecificHeaderRefusal } from "./lib/connection-specific-guard.ts";
...
//
// The recovery sheet's print button needs no second hash here ('unsafe-hashes' + the sha256 of
// 'window.print()'): it is opened via <a target=_blank rel=noopener href="blob:..."> into a new
// top-level browsing context, and header-delivered CSPs do not propagate to new top-level blob:
// navigations, so the blob document receives no policy from this header and its addEventListener
// script runs without any CSP exemption on the console side.
//
// A stale hash here silently blocks the pre-paint script for every visitor (no theme/a11y
// attributes before first paint, a CSP violation in every console), so validate-csp-hash.ts pins
// this constant to the real bytes rather than trusting it to stay current by hand.
// TOUR_EVENT_NAMES / TOUR_VALUE_RE are the /tour/event sink's redaction boundary. The route is a
...
// tourValueOk gates a blob by STRUCTURE, not merely by character class: a character-class regex like
// /^[a-z0-9:/._-]{0,64}$/ cannot exclude a URL with a scheme ("https://evil.example/x" is composed
// entirely of characters in that class), so a passing value could still carry one. This checks that
// every slash-separated segment is a product word, so a scheme, an "@", a space, a "<" and a query
// string are all unrepresentable rather than merely unlikely. The sink is the owner's own Analytics
// Engine and there is no customer pack behind the tour, so nothing has ever leaked through this; the
// stricter check keeps the gate's guarantee matching what it actually enforces.
...
const CSP_SCRIPT_THEME_HASH = "sha256-jrMZu3AmUP+cA+hqiZKAS/IgoRKmQasXgzzGGxzihPY=";
...
//                                  own opaque origin). The policy carries no unsafe-* anywhere.
...
// stay exactly as tight. The redirect_uri / ACS the IdP calls back to is the engine's host, not
...
export function buildCsp(): string {
  ...
  return [
    ...
    // browser ("contains an invalid source ... It will be ignored"). Omitting the quotes silently
    // blocks the sanctioned inline pre-paint script under EVERY hash value, so validate-csp-hash.ts
...
function withSecurityHeaders(res: Response, pathname?: string): Response {
  ...
  // ASVS V4.1.1: a text-family body declares its charset. The Workers static-assets binding emits text/html and
  // text/css with no charset parameter, and this wrapper is the one place every console-originated response
  // passes through, so the parameter is added here rather than in every asset. Binary and JSON types are left
  // alone (no charset parameter is defined for them); a type that already carries one is not touched.
  const ct = withCharset(headers.get("content-type"));
...
// withCharset appends "; charset=utf-8" to a text-family media type that lacks a charset parameter: text/*,
// application/xml and any +xml structured suffix. Everything else (application/json and its +json suffixes,
// application/octet-stream, application/pdf, image/*, font/*) is returned unchanged, as is null and any type
// that already declares a charset. Exported so the vitest can drive the boundary cases directly.
export function withCharset(contentType: string | null): string | null {
  if (contentType === null) return null;
  if (/;\s*charset=/i.test(contentType)) return contentType;
  const mediaType = (contentType.split(";")[0] ?? "").trim().toLowerCase();
  const textFamily = mediaType.startsWith("text/") || mediaType === "application/xml" || mediaType.endsWith("+xml");
  return textFamily ? `${contentType.trim()}; charset=utf-8` : contentType;
}
...
//
// /metrics (the Prometheus text-exposition scrape endpoint, monitoring integrations) is the
// REACHABILITY path for the metrics scrape credential Settings mints
...
//
// Why this route exists. A direct browser fetch to https://control.downpipes.io/licence/claim would be
// blocked before it reaches the network, because buildCsp() sets connect-src to exactly 'self' and
// Chromium refuses a cross-origin fetch under that policy: every activation attempt would land in the
// catch. The in-repo tests cannot see this because validate-licence.ts replaces globalThis.fetch, so
// the policy is never in the path there. Routing the exchange through this same-origin route is what
// the CSP requires.
//
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    try {
      // V4.2.3: a hop-by-hop header that survived the edge is refused before routing (lib/connection-specific-guard.ts).
      if (connectionSpecificHeaderRefusal(req.headers) !== undefined) {
        return withSecurityHeaders(
          new Response("malformed request: connection-specific header\n", {
            status: 400,
      ...
      // Public-tour funnel sink. The tour's browser half beacons small,
      ...
      if (isEngineSurface(pathname, req.method)) {
        // AWAITED, and that is load-bearing. `return env.ENGINE.fetch(req)` (without await) would return the
        // PROMISE itself, so a rejection from the service binding (the engine worker deleted, throwing, or
        // over its resource limits) would escape the last-resort try/catch above entirely and the runtime
        // would answer a bare 500, making the handler's own ASVS V16.5.4 claim ("a throw from ... the ENGINE
        // service binding must not escape unhandled") false for the one case it names by name.
        if (env.ENGINE !== undefined) return await env.ENGINE.fetch(req);
        // A MISSING ENGINE SERVICE BINDING WOULD OTHERWISE MASQUERADE AS A HEALTHY ENGINE, and this line is
        // what stops that. Without it, an engine-surface request with no ENGINE binding falls through to the
        // assets fetcher, and the SPA fallback below answers /admin/status, /support and /metrics alike with
        // index.html at HTTP 200. The consequences run in three directions and none of them names the cause:
        //
        //   the console       every screen would show "engine unreachable" or a JSON parse error, because it
        //                     is parsing an HTML shell as engine data. The classifier calls that `html-not-engine`,
        //                     which is the WRONG ENGINE URL diagnosis: it sends the operator to correct an
        //                     address that is perfectly correct.
        //   the scraper       a Prometheus scrape of /metrics would receive HTML with a 200, so the customer's
        //                     own monitoring reports everything as fine while nothing is being backed up.
        //   the pack          the engine never receives a single request, so its own status booleans and every
        //                     section read green. The pack shows a healthy engine and an idle one at once.
        //
        ...
        // its body, and a body can be read only once, so with the status alone it would record this as an engine
        // 5xx: an engine fault, fabricated, for a request the engine never received. The header lets that seam
        // tell the console's own refusal from the engine's, without consuming the body failResponse still needs.
      ...
      // Coarse, fixed line only: no raw message/stack/header/body. The path is derived
      // defensively because the URL parse can itself be the throwing step; a bad URL
      // falls back to an omitted path rather than re-throwing here.
