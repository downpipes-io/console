// The deep-linkable DETAIL DRAWER (design-system.md 6.11 / 6.14). The richer
...
//
// THERE IS NO TABSET HERE, and its absence is a decision rather than an omission. Each of
// the five callers (the destination, downpipe, map-node, run and credential drawers)
// carries one to four short sections an operator opens the drawer to read at once, and
// putting half of any of them a click away would hide what the drawer is for. Design-system
...
// real URL each and build their own tablist on the shared `.drawer-tab*` CSS.
//
// It is built ON the shared overlay engine (dialog.ts: focus-trap, inert background,
// restore-focus, Esc, click-out) and the shared drawer geometry, NOT a re-implementation
// of any of it: openDetailDrawer composes dialogSurface({variant:"drawer"}) + openOverlay,
...
//
// Deep-linkable: the caller reflects the selected entity in the URL before opening
...
export interface DetailDrawerOptions {
  ...
  //
  // REQUIRED rather than optional. An optional key is a hazard a call site can silently omit,
  // and omitting it lets a screen re-render (app.ts's identity-resolved quiet re-render, a
  // fresh screen.render() outside the router, so closeAllOverlays never fires for it) re-fire a
  // deep link's "open this drawer" side effect and stack a second .overlay--drawer over the
  // first, covering its Edit/Remove buttons. Making the key required moves the guard from
  // "remember to opt in" to "cannot compile without one" -- the compiler enforces it, not a
  // reviewer's memory. There is no reasonable default to derive it FOR the caller (this
  // component has no notion of the caller's entity id), so the key stays caller-supplied; a
  // stable value derived from whatever uniquely identifies the row (its id, or a composite like
  // `${downpipeId}:${runIndex}` when a bare id can repeat) is normally already at hand at the
  // call site (see the rowKey a caller's own table likely already computes).
  key: string;
...
export interface DrawerAction {
  ...
  // as its title, never hidden-then-403 (design-system.md, the gate reads as governed).
  disabled?: boolean;
  disabledReason?: string;
  // gateOp names the privileged write this action WOULD have made, in the same closed op vocabulary the
...
function buildActionButton(action: DrawerAction): HTMLButtonElement {
  ...
  // Disabled-with-reason, FOCUSABLE (the Primer/Atlassian pattern): aria-disabled
  ...
  // The CLIENT-SIDE ROLE REFUSAL is recorded where it is actually shown. This is the only place the fact
...
// drawer.ts is the single source of truth and this file carries no separate copy.
