...
// under the size budget; each function reads the live table state and callbacks through
// the shared ctx object rather than a closure.
...
// true totals (design-system.md 6.9). A scroll handler swaps the slice.
export function renderWindowed<Row>(ctx: DataTableCtx<Row>, rows: Row[]): HTMLElement {
  ...
  // which would drift the spacer maths; see WCAG 1.4.12).
  ...
  const paint = () => {
    ...
    // rendered data row of the slice if none is present (design-system.md 8).
    if (opts.onRowActivate && !tbody.querySelector('tr[data-key][tabindex="0"]')) {
