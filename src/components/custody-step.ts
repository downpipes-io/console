// The optional "protect your break-glass key" step. It presents the tiered offline-storage menu
// and, for the M-of-N tier, performs the envelope encryption and Shamir split IN THIS BROWSER,
// offering the ciphertext and each custodian share as separate downloads. The chosen scheme and
// custodian sign-off flow back to the recovery sheet via the onChange callback (public metadata
// only).
//
...
// NEVER POSTed, so captured shares stay useless, and a share below the threshold reveals nothing.
...
// reduced-motion honoured (no animated affordances are introduced; the only transient is the existing
// copy "Copied" flash which the design tokens already gate); accessible labels on every control.
...
export interface CustodyStepOptions {
  ...
  // Returns whether the browser actually accepted the delivery. Typing this `=> void` would discard the
  // one honest signal the host's downloadText already computes: a browser that blocks the second and
  // subsequent automatic downloads does not throw, so the split panel would report "Downloaded." while a
  // share never reached disk, and the wrapping key exists only in that closure.
