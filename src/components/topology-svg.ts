...
// validator asserts that model, not this projection.
//
// This file stays the figure/layer ORCHESTRATOR. The per-part SVG builders live in sibling
// modules to keep this file under the structural threshold: topology-svg-edges.ts (the edge
// group + the shared arrowhead defs), topology-svg-nodes.ts (the node columns + group labels)
// and topology-svg-util.ts (the SVG namespace constant, the textContent-only text/glyph
// helpers, the roving-tabindex wiring and the reduced-motion read). The only public export is
// buildSvgFigure, which topology.ts imports; the per-part builders are imported from those
// siblings here rather than declared in this file.
//
