...
// No-custody is never weakened: every server-supplied string reaches the DOM through svgText
// (textContent), so there is no markup-injection surface.
...
export function buildGroupLabel(group: LayoutGroup): SVGGElement {
  ...
  // destination labels sit in the SAME place relative to their nodes; left-aligning both at the column's
  // left edge would read inconsistently (beside the wide source column, beneath the far destination
  // column). The column's left edge is padX (source) or padX+colWidth+channel (destination); its centre is
  // that plus half a node box.
...
function buildNode(node: LayoutNode, onActivate?: (id: string) => void): SVGGElement {
  ...
  // hugs its OUTER (left) edge, the destination column hugs its OUTER (right) edge; anchoring both sides
  // to the LEFT of the box would drift the destination column's placement toward the centre channel
  // instead of mirroring the source. The glyph is 16px (svgGlyph), inset 14px from the
  ...
  // danger-toned line naming WHY, so the outage is stated in WORDS (not colour alone) right on the
  ...
  // plain "down - KIND" text, so this never renders a dangling "down - " with nothing after it.
...
function wireNodeInteractivity(g: SVGGElement, node: LayoutNode, onActivate?: (id: string) => void): void {
  ...
  // The accessible name GAINS the down reason (the same destDownReasonLabel mapping the visible node
  // line and the drawer use) alongside the existing "currently unreachable" wording; a generic/absent
  // reason leaves the phrase as plain "currently unreachable".
  const downReasonLabel = node.down ? destDownReasonLabel(node.downReason) : null;
