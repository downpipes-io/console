...
//
// WHY IT IS A MODULE AND NOT A SECOND COPY. This panel is shared verbatim by every flow that needs to collect
// a break-glass identity in the browser: the "recover a lost estate into a fresh account" flow
// (components/estate-import-modal.ts) and the sibling disaster where the account survives and only the
// scheduler was wiped both reuse this same collection, unchanged. Two hand-maintained copies of a surface
// that holds a reconstructed break-glass private key is precisely the arrangement where one of them quietly
// stops wiping, so the logic lives here once rather than being written twice.
//
// THE WIPE CONTRACT IS THE LOAD-BEARING PART. There are TWO independent secret holders behind this panel,
// not one:
//
...
// deliberately no way to zero one without the other, because a caller that can zero just one is a caller
// that eventually will.
//
// A CALLER STILL OWES ONE THING: navigating away is an exit too, and wiring only the dismiss and cancel paths
// leaves it unwiped for that exit. dialog.ts tears every overlay down WITHOUT invoking onClose callbacks,
// deliberately, so an onClose that navigates cannot fight a navigation already under way. Mount this panel
// behind `wipeOnDisconnect(body, panel.wipeAll)` as well as the dismiss and cancel paths. `wipeAll` is
// idempotent, so a dismiss firing it and the observer firing it again costs nothing.
...
// The read goes through lib/file-size-guard.ts, so a file above CONSOLE_FILE_MAX_BYTES is refused before a
// byte is read. A small, deliberate duplicate of screens/restore-flow/attend.ts's readIdentityFile (a
// components/ module does not reach into a screens/ one for a short wrapper; renderReassemblyCard, which this
// module DOES import, is the one genuinely shared, non-trivial piece, the M-of-N reconstruction).
async function readIdentityFileLocal(file: File): Promise<HybridRecipientPrivate> {
...
// would make a CATALOGUED control invisible to the census: the field-catalogue gate would then report it
// uncatalogued at the caller and orphaned at the panel. A single stable id is also correct on its own terms,
// because both flows that mount this panel are modals and the overlay stack holds one at a time, so two panels
// are never on the page together.
//
// It is written out at BOTH sites rather than hoisted to a named constant, because a constant reference is not
// a literal either: the census reads the source text, and `id: IDENTITY_INPUT_ID` would be as invisible to it
// as `id: opts.inputId` would be. The duplication is the price of being catalogued, and the field-catalogue
// gate is what notices if the two ever disagree.
