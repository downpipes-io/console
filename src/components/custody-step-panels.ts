...
// is NEVER POSTed, so a captured share stays useless (ciphertext separation), and a single share
...
//
// Coding rules for this file: CSSOM via node.style / the h() style helper (never setAttribute("style"));
// every server/operator string via textContent or escape; status by shape + label, not colour alone;
...
export function renderTier12(ctx: CustodyPanelContext, s: "password-manager" | "encrypted-usb-paper"): HTMLElement {
  ...
  // (There is deliberately no optional security-key (YubiKey) layer in this UI: the per-credential PRF
  // secret would make a single recorded credential unrecoverable by a backup key. The encryption +
  // custody options above protect the key file on their own without that risk. The enrol/derive plumbing
  // in webauthn-prf.ts is kept for a future multi-wrap design.)
...
async function doTier12Encrypt(
  ...
  try {
    ...
    // The in-browser key-file encryption failed. Without this record the only trace would be the sentence
    // rendered on screen, leaving support to debug it from screenshots. The coarse fault class is chosen from
    // the thrown value's CONSTRUCTOR NAME and nothing else: the message is rendered to the operator (where
    // they need it) and is never read into the ring, because a WebCrypto message can carry a byte length and
    // a length is a fingerprint of the key.
...
export function renderSplit(ctx: CustodyPanelContext): HTMLElement {
  ...
  // The split-parameter refusal is recorded on BLUR, not on every keystroke. That is the whole difference
  // between evidence and noise here. On input, "12" passes through "1", which the ceremony correctly refuses,
  // so an operator who types a perfectly good pair would produce a refusal row every time; on blur, the row
  // means the operator LEFT the picker with a pair the ceremony will not take, which is what "the split
  // button stays disabled" looks like from here. A row here is not on its own proof the ceremony was lost: a
  // later `ceremony-step` shamir-split row says the split then ran, and the two together tell an operator who
  // fixed it apart from one who is still stuck. Neither N nor the threshold ever rides.
  //
  // AND AN EMPTY PICKER IS NOT A REFUSED PAIR. Number("") is 0, so without the guard below, clearing the
  // share-count box and tabbing away would run the ceremony's rule over a pair the operator never chose and
  // write custody-split-n/rejected -- indistinguishable from the operator who really did ask for a 1-of-1
  // split, which is the row the member exists for. A number control also reports "" for text it could not
  // convert, so a mid-edit box is empty here in both senses. Both boxes must hold a value before the pair can be judged at all:
  ...
  // A CONTENT-WIDTH ROW, not two half-panel columns. A generic two-column `1fr 1fr` grid would give each
  // picker a 292px track for a single digit and, worse, squeeze the threshold label below the 277.5px it
  // needs to hold one line; once that label wraps, the two number boxes sit 13.4px apart and the two Learn
  // more links 6.7px apart, and the onboarding first-setup card meets the same constraint at 1440 and 1280
  // because its interior column is narrower still. `.custody-split-pickers` sizes each field to its own
  // content and wraps the pair onto separate rows rather than shrinking them, so a label that cannot fit is
  // alone on its row with no peer left to fall out of line with.
...
function renderShareSignoffs(ctx: CustodyPanelContext, n: number, threshold: number): HTMLElement {
  ...
  for (const lab of labels) {
    ...
    const dated = field({
      ...
      // NAMED PER SHARE, like the holder field above it: a constant "Signed / dated" label would have
      // assistive technology announce identical controls for what are five distinct shares, in a ceremony
      // whose entire purpose is recording WHICH custodian held WHICH share and signed when.
...
function renderSplitOutput(
  ...
  // blocks the second and subsequent ones does NOT throw, so without this tracking the panel would report
  // "Downloaded." while a share never arrived. The wrapping key exists only in this closure, so navigating
  // away would then make the ciphertext permanently unopenable. Per-share Re-download buttons mitigate this,
  // though they only help an operator who noticed.
...
// label go to the operator's OWN engine, which emails it. The ciphertext never goes with it, so a
...
function renderShareEmailForm(ctx: CustodyPanelContext, lab: { index: number; title: string }, shareBytes: Uint8Array, n: number, threshold: number): HTMLElement {
  ...
  // THE FIELD'S RULE IS ENFORCED HERE, not merely displayed. atMostChars above runs on blur, and without
  // this check the Send handler would read the raw value and post it regardless, so an over-long name
  // would show a red error AND still be sent, with the engine's cleanLabel truncating it to 120 in silence:
  // the custodian would receive a cut name while both the console and the engine reported success. Running
  // the field's own validator before the request is built is the only point at which the operator can
  // still fix it.
...
async function doSendShare(
  ...
  // sendShare is a HOST-INJECTED prop. Today's three injectors all route to sendCustodyShare, which is
  // documented fail-soft and catches everything, but nothing at THIS seam enforces that, and the failure
  // mode if it ever stopped holding is the worst one there is: the button stays disabled reading "Sending",
  // the spinner line stays up, and the custodian never receives a share the operator believes was sent.
  // That is the frozen-placeholder shape, in a one-shot custody ceremony. The panel owns its own rejection
  // here rather than borrowing another module's discipline.
...
async function doSplit(ctx: CustodyPanelContext, n: number, threshold: number, out: HTMLElement, btn: HTMLButtonElement): Promise<void> {
  ...
  try {
    ...
    // 2b. Compute the PUBLIC wrapping-key checksum: it travels in every share file and
