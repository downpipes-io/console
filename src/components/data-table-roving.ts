...
// tab stop, design-system.md 8) and focusRow (which moves the tab stop and focus to a row,
// scrolling a windowed off-screen row into view first). Split out as a dependency-light
// sibling so the component module stays under the size budget; each function reads the
// live table state and callbacks through the shared ctx object rather than a closure.
...
// move the active row (one tab stop, design-system.md 8). For the plain path it is
