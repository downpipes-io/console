// Inline field validation, channel one of the error model (design-system.md 6.4 /
// 8, build-contract 2.3/2.4). A field-level error is rendered AT the field, never
...
export interface Field {
  ...
  value(): string;
  // badInput: did the CONTROL ITSELF fail to convert what the operator typed? On a type="number" input
  ...
  // TYPED. It shows the same inline error AND records the refusal, so a console rule that lives outside the
...
export interface FieldOptions {
  ...
  // An optional documentation link rendered under the field: the page that
...
export function field(opts: FieldOptions): Field {
  ...
  // The optional doc link: a real external anchor to the canonical docs page for this field,
  ...
  // Set again as a PROPERTY (not just the `hidden: true` attribute above): the lightweight test shim
  // (test/cov/components-field.ts's SHIM NOTE) mirrors `.hidden` onto the DOM only via the property
  // setter, not from the attribute an element is built with, so a slot never touched by a property
  // assignment reads not-hidden under the shim even though the attribute is set. A real browser reads
  // this identically either way (the assignment is a same-value no-op); the shim does not, and
  // applyDescribedBy below depends on disabledSlot.hidden reading true before setDisabled is ever
  // called.
  ...
  //
  // RECORDING ONLY ON BLUR AND SUBMIT IS WHAT KEEPS THE COUNT MEANINGFUL ("which fields rejected, HOW OFTEN"
  // is the whole question the pack exists to answer). validate() is wired to blur AND, once a field has
  // errored, to every `input` event. Correcting a refused field is the WORKING PATH: an operator who pastes an
  // SSO URL without its scheme, sees the error, clears the box and types the correct
  // "https://idp.example/app/sso/saml" one character at a time passes through eight invalid prefixes on the
  // way. Recording every one of those keystrokes would write nine refusal rows for a connection that SAVED,
  // while an operator who is genuinely stuck, pastes three URLs the console will not take and rings support,
  // would show three -- the stuck operator reading as a third of the noise of the one who succeeded, even
  // though the two are byte-identical on every other field of the row.
  //
  // custody-step-panels.ts records its own split-parameter refusal on BLUR and not on input for the same
  // reason: on input, "12" passes through "1", which the ceremony correctly refuses, so an operator typing a
  // perfectly good pair would produce a refusal row every time. field() carries roughly fifty of the
  // vocabulary's members to the custody panel's four, so the same rule applies at scale here.
  //
  ...
  // take it, with a value it will not take -- record. That is the row's meaning, and that is what produces it.
  function runValidate(record: boolean): boolean {
    const v = readValue();
    if (opts.required && v === "") {
      // NOT recorded. A required field that is still empty is an operator part-way through a form, and
    ...
    if (opts.validate) {
      ...
      if (msg) {
        // The console's OWN validator refused a value the operator actually typed. No request is made, so
        // the engine has no view of this at all: "the form will not accept my bucket name / cron / endpoint" has
        // no evidence anywhere else. THIS is the one funnel every field() control validates through,
        ...
        // is what a wolf cry costs. A caller can express the SAME emptiness rule through its VALIDATOR arm
        // instead (`validate: (v) => (v.length >= 1 ? null : "Paste the deploy token.")`), which would bring the
        // exempted state straight back in through this branch and write a row asserting a fact the code never
        // established: rejectOutcome "rejected" means the validator turned the operator away from THE VALUE THEY
        // TYPED, and on an empty field no value was examined. A support engineer reading a token-field rejection
        // on a 02:00 rollback pack would go and investigate token validation on a rollback that was never
        // attempted.
        //
        // So the funnel records a refusal only when there was a value to refuse. A validator that can only
        ...
        //
        // The emptiness rule is enforced INSIDE recordFormRefused, which takes the raw value and cannot write a
        // row without one, so this funnel simply hands it over: the rule is the recorder's, not each caller's, so
  ...
  control.addEventListener("input", () => {
    ...
    // SILENT: the error clears the moment the value becomes good, and a still-invalid prefix of a value
    // the operator is halfway through typing records nothing. See runValidate above.
    if (errored) runValidate(false);
  });

  // refuse(): the console's own rule refused this control from OUTSIDE the validate() funnel. It records the
  ...
  //
  // THE EMPTY VALUE IS NOT RECORDED HERE EITHER, guarded HERE rather than trusted to each caller. The funnel's
  // rule (a `rejected` row asserts the console refused THE VALUE THE OPERATOR TYPED, so there has to have been
  // one) is structural in validate(); this guard closes the same gap on this arm, so a rule that refuses an
  // EMPTY box (the required-and-empty state under another name, shown to an operator who is looking straight at
  // the message) cannot write a row claiming the validator examined a value it never saw. A rule that refuses a
  // value the operator DID type (a workers.dev URL in a binding box, a PEM paste with no complete block in it)
  // records exactly as it should.
...
//
// For example: with a Splunk HEC token typed without its "Splunk " scheme, one press on the Enabled
// checkbox can put mousedown on INPUT#push-enabled and mouseup on a div 45px lower once the secret
// field's error slot appears, so the click lands on the FORM instead of the checkbox and the checkbox
// does not toggle. The same press with an accepted secret, or with the error already on screen (in
// either case nothing to paint), toggles the checkbox normally: nothing about the checkbox itself is
// wrong, the row it sits in moves under it.
//
// The reading that shape invites is worse than the fault itself. It looks exactly like a checkbox that
// will not take a click, which reads as "an operator cannot untick Enabled, or tick anything in the
// notify rule form", because those checklists are built by the same factory. They are fine.
//
// The checkbox is not the worst of it either. The SAVE button sits below every field on the form, so the
// FIRST press of Configure after typing a refused secret can be swallowed the same way and by the same
// 45px: mousedown lands on the button, mouseup lands on the error slot that has just appeared, and the
// button's own click handler runs zero times. The operator does see the refusal, so it is not silent, but
// the press they made on Save did nothing and nothing on screen attributes that.
//
...
// returns true only if all are valid (design-system.md 8: focus the first invalid
