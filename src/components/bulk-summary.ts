// The shared per-item failures summary modal for a bulk loop (the honest partial-success surface). It lives
// in its own module so EVERY bulk loop reuses the SAME surface (the bulk run/disable/delete path, the
// Sources bulk-protect, and the fleet drill) rather than re-implementing it or truncating its failures to
// "3 named + N more". Each failure is a real, focusable list item (not a vanishing toast), so a 30-failure
// drill explains all 30.
//
// No-custody: every server-supplied string (a name, a reason) enters the DOM via the typed h() builder
// (textContent under the hood), never innerHTML.
...
//
// `action` is the CLOSED bulk-action id and is required. `verb` is operator-facing prose ("created",
...
export function openBulkSummary(action: ClientDiagBulkAction, verb: string, done: number, failures: Array<{ name: string; reason: string }>): void {
  // THIS is the seam: every bulk loop in the console (bulk protect, the import, the bulk run/disable/delete,
  // the fleet drill) funnels its per-item failures HERE. Without a record at this seam they would exist only
  // inside this dialog: the operator dismisses it and the evidence is gone, so a "some of my downpipes would
  // not create" ticket would arrive with nothing behind it. Recording at the one shared surface gives complete
  // coverage of the kind, exactly as engine-fetch.ts does for engine calls, without asking six call sites to
  // remember.
  //
  ...
  // `action` is WHICH bulk loop this was, without which a half-failed delete and a half-failed run would be
  ...
  // leaves the browser.
  //
  // CLASSIFIED BY CAUSE, not by counts. Counts alone would fold a batch the engine REFUSED (malformed input)
  // and a batch that died because the engine was DOWN into the same row, and those have opposite remedies.
  // The failure reasons are read ONLY through errorStatus (a number or null; it copies nothing) to select a
  // closed class.
