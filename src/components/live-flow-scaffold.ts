// The static DOM scaffold + the topology-options projection for the live-flow canvas view. These
// live apart from live-flow.ts and renderCanvasView to keep both under the structural threshold,
// and build only the elements/options that depend on opts (never on the per-poll
// controller/model state), so the renderer's wiring stays focused.
...
// topology default). The same projection is needed by the initial canvas-unavailable branch and by
...
export function buildCanvasScaffold(opts: LiveFlowOptions): CanvasScaffold {
  ...
  // any resize with no JS). The canvas draws pipes, streams and the hub ring; the LAYER is what
  // makes the endpoints legible, since a canvas-only marker with no name would read as bare
  // curves. DOM text stays crisp at any DPR, follows the theme tokens, and keeps every server
  // string on the textContent path. When the caller wires onActivateNode the labels are real
  // buttons (activate to filter, Escape to clear), the same affordance the SVG topology's nodes
  // carry; otherwise (the read-only embeds) the layer is decorative and hidden from assistive
  // tech, whose canonical surface is the table.
