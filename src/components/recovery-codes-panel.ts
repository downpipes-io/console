...
// RecoveryCodesResult (which must decide whether to open this panel at all). An unmodelled or stale endpoint
// can fall through to a generic 200 whose body carries no `recoveryCodes` field, leaving
// `result.recoveryCodes` `undefined`; without this check, recoveryCodesText's own `.map` would throw straight
// through the click handler. A malformed response is not specific to any one deploy: any engine could answer
// with a stale or truncated shape, so the check lives here (PURE, no DOM) rather than only at one call site.
...
export interface RecoveryCodesPanelOptions {
  ...
  // It returns whether the browser accepted the delivery: a refused or impossible download must not be a
  // silent no-op on the one artefact the operator cannot ever get back, so the panel needs the answer.
...
// is a SINGLE real button, so a click on the visible "Copy" label always hits the button itself.
//
// BOTH controls report a refusal, to the operator and to the pack. A browser that declines the
// clipboard write or the download must not leave the operator looking at a panel that has not changed, on the
// one artefact that is shown once and never again, with no trace of the refusal afterwards. Copy goes
...
function buildActionRow(codesText: string, downloadText: (name: string, content: string) => boolean): HTMLElement {
  ...
  copyBtn.addEventListener("click", async () => {
    if (!(await copyToClipboard(codesText, "recovery-codes"))) {
      // The copy was refused on the one artefact that is shown once and never again. The capability-fault row
      // already records that the browser declined the clipboard; this records which enrolment step was lost by
      // it, which no capability row can, and it is the row that answers whether the recovery codes were ever saved.
...
// buildDegradedPanel is the honest fallback for a `codes` value that fails hasUsableRecoveryCodes. It states
// plainly that the codes could not be shown (never a blank screen, never a generic engine-error toast the
// operator cannot act on), is explicit that nothing here confirms whether a save-worthy set exists, and gives
// the one thing the operator can actually do next: try again. The single "Close" action calls onConfirm so the
// caller's existing dismiss/reload plumbing still runs. This file's only two callers treat onConfirm as a plain
// dismissal rather than "codes were saved": access.ts validates before opening this panel via
// hasUsableRecoveryCodes, and passkey/flows.ts already guards codes.length > 0 before ever calling
// showRecoveryCodes. A caller for whom onConfirm means "codes were saved, tell the operator so" would need its
// own guard too; every caller is protected at the panel boundary even if it forgets to check first.
...
//
// `opts.codes` crosses a JSON wire boundary (an engine response, real or faked), so its runtime shape is never
// guaranteed by the TypeScript type alone. A missing or malformed set would otherwise reach recoveryCodesText's
// `codes.map` and throw past the click handler that opened this panel. hasUsableRecoveryCodes is the ONE check
// (shared with the callers that can pre-empt this) that decides whether there is anything safe to render; a
// "no" degrades honestly instead of throwing.
