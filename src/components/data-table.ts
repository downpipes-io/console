// The configurable DATA TABLE (design-system.md 6.9). One typed table machine the
...
//
// A11y (6.9 / 8): a real <table> with <th scope="col">; aria-sort on the sorted header
...
// (data-table-render.ts), and the toolbar (data-table-toolbar.ts). The pure helpers and
// the type surface stay in their existing siblings.
...
export function dataTable<Row>(opts: DataTableOptions<Row>): DataTableHandle<Row> {
  ...
  return {
    ...
    setRows(rows: Row[]): void {
      ...
      //
      // The drop is counted. This is the site where "I selected 20 downpipes but only 17 were acted on"
      // happens: a background refresh lands between the operator ticking the rows and clicking the action, the
