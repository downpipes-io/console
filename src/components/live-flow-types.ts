...
// public options/handle, and the colour + table-spec shapes. This lives apart as a
// dependency-light sibling so the pure model and the canvas renderer share one type source and a
// screen can import the option/handle shapes without pulling in the canvas controller.
// live-flow.ts re-exports every externally-referenced declaration here by name, so the public
...
// down). The 2D-canvas controller is the only implementation (see chooseRenderMode).
