...
// focusable trigger (design-system.md 7a, calm-density budget: essentials visible,
...
export function infoTip(text: string, opts: InfoTipOptions = {}): HTMLElement {
  ...
  // moved into the shared body-level layer while portalled (see positionPop below). Checking
  // only `wrap.contains(target)` would miss a portalled popover living outside wrap's own
  // subtree, reopening the outside-dismiss hole this check exists to close.
  ...
  // ---- Clip-aware positioning: the popover's own CSS (bottom/left/transform, above and
  // centred on the trigger) is the default and stays the default everywhere it already works.
  // This only steps in when a clipping ancestor (an overflow:hidden ancestor a nearby carousel
  // stage genuinely needs for its card transition, say) or the viewport itself would cut the
  // popover off, and it moves the popover to a shared, unclipped layer rather than widening or
  // un-hiding the clipping ancestor.
  ...
  function positionPop(): void {
    ...
    // layout engine), where the default CSS position stands untouched and the validator grades
    // the same click/focus/escape/dismiss contract regardless.
