...
// both dependency-light siblings split out so this leaf stays under the size budget.
// topology.ts re-exports from this module rather than keeping its own copy, so there is no
// second copy to keep in sync; this file also re-exports the sibling surface by name so
// topology.ts and the validators keep importing everything from here. The whole layer touches
// no DOM, no clock (beyond an explicit `now`) and no randomness, so it runs in the bare-Node
// validator and is deterministic.
...
// Re-export the public surface by name so topology.ts, topology-table.ts, live-flow.ts
// and the validators can import every symbol from this module. The encoding and the geometry
// live in dependency-light siblings; the public API here stays stable.
export {
  presentStatus,
  throughputWeight,
  throughputText,
...
// A "no-copy" lane is DELIBERATELY absent from both allow-lists here (and from the flowing-dash
...
function buildEdges(drawnFlows: FlowRecord[], nodeById: Map<string, LayoutNode>, now: number): LayoutEdge[] {
  ...
  for (const flow of drawnFlows) {
    ...
    // case does not collapse every edge onto one point.
    const x1 = rightEdgeX;
  ...
  for (const f of flows) {
    ...
    if (!existing) {
      seen.set(id, {
        ...
        ...(ep.down ? { down: true } : {}),
        // The first down reason seen for this node wins (mirrors "secondary" above), so a node
