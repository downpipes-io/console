...
// the clear control. The bar is a polite live region so its appearance is announced. Split
// out as a dependency-light sibling so the component module stays under the size budget.
// The body is moved verbatim from the dataTable() factory; the only change is reading the
