...
//
// The whole layer touches no DOM, no clock (beyond an explicit `now`) and no randomness, so it
// is deterministic.
...
export function presentStatus(status: FlowStatus): StatusPresentation {
  switch (status) {
    ...
    case "no-copy":
      ...
      // Reading it as "partial" would tell operators to wait indefinitely for a copy nobody is making.
...
export function throughputWeight(bytesPerRun: number | null | undefined): EdgeWeight {
  // A byte figure that ARRIVED as a number and is not finite (a NaN out of a division, an Infinity out of
...
export function tally(flows: FlowRecord[]): StatusCounts {
  ...
  for (const f of flows) {
    ...
    // Counted in its OWN bucket. The else-fallthrough below lands on `unknown`, so without this branch a
    // no-copy lane would be tallied as "status could not be fetched" -- the aria summary would say the console
    // does not know, when it knows exactly: that copy has never been made.
...
export function freshnessPhrase(flow: FlowRecord, now: number): string {
  ...
  // Never fall through to "fresh". The RUN is fresh; this destination's copy of it does not exist.
...
export function toMsLocal(input: string | number | null | undefined): number | null {
  ...
  // there would fire on every fresh account. A value that ARRIVED and would not parse is a wire fault: it is
  // the case where freshness would otherwise show a dash beside a run that exists. The value never rides.
