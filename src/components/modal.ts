...
//
// A destructive primary is NEVER the default-focused control (design-system.md
// 6.11): a modal with a danger action focuses Cancel. The footer renders the
// primary on the right and Cancel to its left.
//
// Error contract: when an action's onClick rejects, the modal stays open, the
// control state is restored (busy path: spinner cleared + buttons re-enabled), and
// the error is surfaced to the operator via a warn toast. Both the busy and the
 ...
 *
 * MEASURED RATHER THAN ESTIMATED, and the estimate would have been wrong. Character count predicted
...
export function reserveBusyWidth(btn: HTMLElement, busyLabel: string): void {
  ...
  // reserving nothing it THROWS instead of silently doing nothing.
...
export function openModal(opts: ModalOptions): OverlayHandle {
  ...
  async function runAction(action: ModalAction, btn: HTMLButtonElement): Promise<void> {
    ...
    // surface the error and keep the modal open.
    try {
      const result = await action.onClick();
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    if (btn !== undefined && busy !== undefined && busy !== "") {
      ...
      // THE CONTROL DECLARES ITS IN-FLIGHT STATE WHILE IT IS STILL AT REST, which is what lets anything
      // measure that state without ACTING on the product. This asks whether a control going busy moves
      // what is beside it, and it could not be driven at all: a click on this console navigates, submits
      // or deletes, so a sweep may not press these, and `data-busy` appears only once the press has
      // already happened. The remedy is either a console that declares its in-flight controls or a walk
      // permitted to act on the product, and this is the first half.
      //
...
// confirm() (current-state.md A2). It resolves true on confirm, false on
