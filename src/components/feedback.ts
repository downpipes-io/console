// Empty, skeleton (loading), and banner primitives (design-system.md 6.21, 6.22).
...
//
// The strip carries NO trailing affordance of its own, deliberately: each of its three callers already
// puts the full detail behind its own affordance elsewhere on the same screen (/access renders a
// lazyDisclosure ("Governance posture and who enforces what") immediately below the strip,
// /identity-providers puts the detail in the provider grid the strip sits above, and /credentials puts
// it in the expiry table directly beneath), which is what design-system 7a asks for. A trailing link on
// the strip itself would be a second affordance for detail already on the page.
