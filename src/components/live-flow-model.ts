...
//
// The model touches no DOM, no clock (beyond an explicit `now`) and no randomness, so it runs
// in the bare-Node validator and is deterministic; the small colour-parsing pair is pure string
// maths. live-flow.ts re-exports every externally-referenced symbol here by name, so the model
// and the canvas renderer share one source without duplicating the public surface.
...
// accessible table). Canvas2D is the only raster path: one path that runs everywhere beats a
// faster path that silently does not. Pure and deterministic given the probe result.
...
// right (blueprint section 8.2). Every flow is drawn as TWO legs through the engine hub: a
...
// It mutates the shared node list and index directly.
...
//     is horizontal and centred rather than kinking below centre. Only the 1x1 case gets the
//     shallow offset; a lone node facing a spread column stays centred.
...
function columnY(index: number, count: number, column: Column, otherCount: number): number {
  ...
  if (count <= 1) {
    ...
    // the graph does not collapse to one flat streak but shallow enough to avoid reading as a
    // steep diagonal.
