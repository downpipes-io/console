...
// Sourced from the dependency-light type sibling (not the topology.ts barrel): topology-svg-nodes.ts
// imports destDownReasonLabel from this module, and the barrel pulls in the SVG renderer itself. Importing
// the leaf type here keeps this pure, DOM-free module out of that import cycle.
...
export interface ReplicationSummary {
  ...
  // instead of a flat "unreachable" that sends triage to the wrong side. A down destination the engine sent
  // no reason for is simply absent from this map.
  downReasons: Record<string, string>;
  // `pending` covers two DIFFERENT states, and the operator cannot act on either without telling them apart:
  //
  ...
  //
  // Folding them together would tell the operator to wait for a copy that is never being made.
  //
  // THE ANCHOR SPLITS "never reported" AGAIN, and it has to. Row absence alone is the state of a destination
  // added five minutes ago AND of one dark since March, so "escalate" would be said to both. A never-reported
...
export function summariseReplication(
  ...
  for (const id of destinationIds) {
    ...
    if (st && !st.lastOk) {
      ...
      if (typeof st.reason === "string" && st.reason.length > 0) downReasons[id] = st.reason; // WHY it is down
    }
    // A destination the engine has never reported on, whose silence the ANCHOR proves has outlasted at
    // least two successful backups, is not "catching up". Nothing is catching up: there is no replication row
    // for it anywhere, which means no copy is being made. Folding it in with the reachable-but-behind
    // destinations would leave it amber indefinitely, so the operator would wait instead of escalating.
    //
...
// reason needs a different action) rather than a flat "unreachable". It returns null for the generic
...
export function destinationLaneStatus(
  ...
  // The lane on the map edge is the signal an operator reads for a single destination's health.
  //
  // Row absence ALONE is not evidence of a fault: a destination an operator adds to a running downpipe has no
  // replication row until the next backup completes, which is up to a full cadence away, and the engine is
  // behaving correctly for every minute of it. Painting "no copy" there would put the second-worst status in
  // the whole fleet (panels.ts STATUS_RANK: worse than stale, worse than partial) on a perfectly healthy
  // configuration, and a map that cries wolf devalues every true warning it shows.
  //
  // The ANCHOR is the evidence that tells a genuinely new destination apart from one proven to hold no copy,
  // and it is the SAME anchor the pack and the bot gate on: how many backups have SUCCEEDED since this
  // destination entered the fan-out and still produced no copy. Under two, the lane stays "partial" (catching
  // up), which for a new destination is the correct advice, not a hedge. At two or more, the silence has
  // outlasted backups that did complete, and "no copy" is the honest word. A destination whose anchor the
  // engine does not hold (a legacy record) keeps the milder lane: its age cannot be established, so no claim
  // is made about it.
