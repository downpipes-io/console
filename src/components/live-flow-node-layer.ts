// The DOM node layer over the live-flow canvas: a labelled anchor per source/destination node
// plus the "Engine" caption under the hub ring, positioned by the model's normalised coordinates
// as left/top percentages (so they ride the canvas box through any resize with no JS). This
// lives apart from live-flow.ts to keep that file under its structural threshold. All names
// reach the DOM via textContent (h()); nothing here weakens no-custody.
...
// drops its labels for ONE calm summary chip; the canvas anchor mark still marks the bank and
...
function buildNodeEl(
  ...
  if (node.column === "engine") {
    // The hub's caption: the ring drawn on the canvas is the mark; this names it. Decorative (the canvas
...
// the full list). Counts only, honest; the canvas anchor mark still marks the dense bank.
