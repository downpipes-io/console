...
// target binding name) must match before the Apply button enables; the impact summary is real text; focus
// lands on Cancel, never the destructive button. Used by restore-apply and any redirect or large restore.
// A proper focus-managed modal stating the exact effect in one sentence, for the highest-stakes action,
// rather than the browser's native confirm().
//
// The friction is calibrated: a small same-binding restore uses confirmModal (a single button + Cancel);
// type-to-confirm is reserved for genuinely high blast radius. Both live here so the restore flow picks the
// right one from the plan.
...
export function typeToConfirm(opts: TypeToConfirmOptions): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    // This disabled -> enabled flip is a plain native :disabled toggle, so the restrained opacity/border
    // fade lives entirely in tokens.css's .btn--danger transition; nothing here needs to trigger or gate
    // an animation.
