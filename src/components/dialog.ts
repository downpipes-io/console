// The shared overlay engine behind the modal and the drawer (design-system.md
// 6.11). It owns the three things every dialog must get right, which a native
// confirm/alert cannot, having no focus management at all (current-state.md A2):
//
...
export function openOverlay(opts: {
  ...
  // the Cancel button so focus never lands on the dangerous control.
  ...
  // create/edit/import wizards), which stack normally: this parameter changes NOTHING
  ...
  // and stacking a second, visually identical surface on top of it. This is what stops a
  // console re-render from stacking a duplicate deep-linked drawer: app.ts's
  ...
  // it) -- a route-driven "open this deep-linked drawer" side effect can then fire a second
  // time while the first drawer is still mounted, and the second .overlay--drawer would cover
  // the first one's Edit button. Keying the drawer by the entity id makes the second open a
  // safe no-op instead of a stack. A confirm or any other unkeyed overlay raised over a keyed
  // drawer is unaffected: it never collides on a key.
  key?: string;
  ...
  // (e.g. by its data-key) so a keyboard user returns to the row they activated.
...
function teardown(entry: OverlayEntry): void {
  ...
  // Restore focus to the invoking control. The resolver is evaluated NOW, at close, so a caller
  // whose invoking element was rebuilt by a reload can re-find the current one; a resolver that
  // finds nothing (or a detached node) leaves focus be.
...
// whatever had focus at open, restored if still in the document. A function is kept as-is and
// re-evaluated at close, so a caller whose invoking element is rebuilt by a reload re-finds the
// current one instead of restoring a detached node.
...
// trapTab keeps Tab/Shift-Tab within the surface (focus trap, design-system.md
// 6.11 / 8). When focus is at an edge it wraps; if nothing is focusable it holds
