// The control-plane recovery banner -- the console face of the engine's silence-killer. A SchedulerDO
...
// Sealed-shape detection reuses the same module built for the estate-import modal (no second port of the
// shape gate).
//
// This file imports unsealControlPlaneExport for the sub-case where an estate's bucket holds only a
// `.sealed.json` and its engine has no CONFIG_RECIPIENT_PRIVATE, so nothing server-side can stage it and no
// plaintext pair was ever written. There, the browser holding the break-glass key is the only thing that can
// open the artefact, exactly as on the estate-import path. This differs from the CONFIRM path, which restores
// an already-staged recovery and needs no key.
...
export interface RecoveryBannerDeps {
  ...
  // acknowledge clears the recovery latch and nothing else, over the operator's OWN session: no
...
// BANNER_HOST_ID is the singleton host the banner lives in. It is inserted as the FIRST child of <body> via
// insertBefore(host, body.firstChild), not appended last, so the banner sits above the shell chrome (in
// normal document flow, no CSS position needed) and is never replaced by a screen navigation (which only
// swaps the main region).
//
// Inserting last would place the host after the whole shell (#app: the sticky header, the rail, the main
// region), whose own root is `min-height: 100vh` (tokens.css .shell). On an ordinary viewport that pushes the
// standing danger banner off the BOTTOM of the page, ~800px down on a stock 1280x720 view: present in the
// DOM, danger-toned, correctly wired, and invisible without scrolling past the entire Overview screen first.
// That is a distinct failure mode from the banner not rendering at all, and a simple DOM-presence or
// isVisible() check cannot catch it because both are blind to being scrolled off-screen. Inserting first
// avoids the whole class of failure: normal document flow puts the host above the shell on every load, no
// z-index/position/fixed-height layout change needed, and the shell's own min-height (not a hard height)
// means nothing below it clips.
...
//
// The latch exists because deriving the banner from a single read alone is unsafe: a later status read that
// fails, is route-absent, or races against the raising read could otherwise take a legitimately-shown danger
// banner back down and leave the demoted operator on a clean-looking Overview -- a false all-clear over a
// wiped plane whose backups had stopped, the worst class of silence this banner exists to kill. The banner
// stands until the engine itself, on a read that SUCCEEDED, reports the plane healthy again.
...
export function updateRecoveryBanner(status: ControlPlaneStatus | null, deps: RecoveryBannerDeps, read: RecoveryStatusRead = "ok"): void {
  ...
  // A standing recovery-required state always renders the danger banner. The banner is derived from the
  // latch, not from a single (possibly failed, route-absent or racing) read, so it cannot be silenced by one.
  if (recoveryLatch) {
...
function recoveryDangerBanner(model: RecoveryBannerModel, deps: RecoveryBannerDeps): HTMLElement {
  ...
  // The action follows model.exit, which follows the engine's own two emptiness facts, so the console never
  // offers a route the engine must refuse. TWO EXITS OFFER NO BUTTON AT ALL and that is deliberate: their
  // bodies name the path that is open instead. A banner with no action is legal (`banner` takes an optional
  // action) and says the true thing rather than a reachable-looking untrue one.
...
// sealedPastedIntoReconcileMessage names the TRUE cause of a sealed paste into the manual reconcile form: not
// a shape problem, a format one this form cannot open, plus the honest next step -- check for the auto-heal's
// own confirm action first (it needs no pasted export at all), since that is what the vast majority of
// estates on the default sealed posture will find once the next scheduled check runs.
//
// If no confirm action is offered, auto-heal has already tried and refused with "sealed-no-op-key": the
// latest export is sealed and this engine holds no operational key to open it. That refusal is TERMINAL, not
// transient. `engine/src/cron/control-plane-pass.ts:598` says so in its own words, "re-run the key ceremony,
// or recover offline", so no later run of the same check can succeed while the key is missing. Telling an
// operator to wait, on the one screen they reach mid-disaster, spends their time on something that will
// never happen, so the message names the cause and the real remedies instead.
...
// openApplyStagedModal confirms an auto-heal-staged recovery: unlike openReconcileModal, it asks for ONLY
// the break-glass token, never the export. WHY NO BROWSER UNSEAL HERE, unlike the estate-import modal: this
// is Case 1 (the scheduler was wiped, the account -- and its env/secret bindings -- survives), so THIS SAME
// ENGINE still holds env.CONFIG_RECIPIENT_PRIVATE, the key the sealed export was ALSO addressed to
// (control-plane-pass.ts loadRecipients(BREAK_GLASS_PUBLIC, CONFIG_RECIPIENT_PUBLIC, "config")), and its cron
// auto-heal has already used it to unseal + re-sign + stage the export server-side (unsealAndResignForAutoHeal)
// before this banner action is ever offered (model.staged is set only once that has happened). Asking the
// operator to also paste their SEPARATE offline break-glass identity.key here would duplicate a decrypt the
// engine can already do with a key it never lost, for no security benefit. Compare the estate-import modal:
// Case 2 there is a FRESH engine that was never a recipient of the old export at all, so only the operator's
// own offline key can open it -- a genuinely different custody shape, not an oversight.
...
export async function _testRunReconcile(
  ...
  setFormError("");
  // A SEALED export pasted here would otherwise fall straight into parseReconcileInput's DP-R03, which names
  // a SHAPE problem ("does not look like a control-plane export") when the real cause is format: the pasted
  ...
  if (trimmedExport !== "") {
    ...
    if (parsedForShape !== undefined && !looksLikeControlPlaneExport(parsedForShape) && looksLikeSealedControlPlaneExport(parsedForShape)) {
      // A sealed paste is not a dead end when the caller wired restoreSealed: the browser opens the artefact
      // with the operator's own break-glass key and relays the recovered export to the engine, which verifies
      // the wrapper, the signature and the export again against its own pinned signer. Without that wiring
      // the explanatory refusal stands, which is a degradation rather than a broken button.
