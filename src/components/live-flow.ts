// The OPTIONAL canvas "live-flow" visualisation (ENTERPRISE-UX-BLUEPRINT section 8.2): a
// CPU-rasterised, opt-in PROGRESSIVE ENHANCEMENT over the existing SVG topology. It draws
...
//     renderLiveFlow always renders that table; the canvas is a pure visual sitting
...
//
//  2. FEATURE-DETECT AND FALL BACK. When a 2D canvas context is unavailable (no context, an
//     SSR/test environment), the visual DEGRADES to the existing SVG topology component
//     (renderTopology), which itself carries the same accessible table. So the ladder is:
//     canvas -> SVG -> table, and the table is the floor.
//
...
//
// Like topology.ts, the module is split into a PURE MODEL LAYER and a DOM/canvas RENDER LAYER.
// The model (buildLiveFlowModel + the decision helpers) touches no DOM, no canvas, no clock
// (beyond an explicit `now`) and no randomness, so it runs in the bare-Node validator and is
// deterministic. Only the render* path touches the canvas and matchMedia. Because the canvas
// cannot run headless, the validator asserts the pure parts: the data->geometry/layout mapping,
// the feature-detect/fallback decision, the reduced-motion static-frame decision, and that the
// table fallback is always produced (via the shared table-spec the renderer and the validator
// both read).
...
// name so a screen, view.ts and the validator import them straight from this module.
...
// renderLiveFlow is the public entry. It feature-detects a 2D canvas context and, when
// unavailable, returns the SVG topology component (which carries the same accessible table) so
// the visual always degrades cleanly. When a 2D canvas is available it mounts the canvas visual
// PLUS the canonical accessible table (the table is never dropped). It honours
// prefers-reduced-motion by painting
...
export function renderLiveFlow(opts: LiveFlowOptions): LiveFlowHandle {
  ...
  // topology when a 2D canvas is unavailable" contract; the table stays canonical because
...
// renderCanvasView mounts the canvas + the canonical table and drives the raster controller.
// The canvas is a PURE VISUAL (role="img" + an aria-label summary built from counts only,
// exactly like the SVG figure), so assistive tech is steered to the
...
interface CanvasState {
  ...
  // and the raster controller (so the two projections of one flow set can never disagree).
...
function renderCanvasView(opts: LiveFlowOptions, animation: AnimationMode): LiveFlowHandle {
  ...
  const state: CanvasState = {
    ...
    scaffold,
    current: opts.flows,
    // The raster controller is created AFTER the canvas is connected/laid out (see
    // ensureController), not at construction time: the canvas has no size until it is in the
    // document, so building it here would size it to 1x1 and, under reduced motion (a single
    // static frame, no loop, no re-measure), leave a permanently blank canvas. The controller
    // owns the rAF loop.
    controller: null,
    resizeObserver: null,
    initStarted: false,
  ...
  // immediately and the empty state is honest), but DEFER the raster controller until the canvas
  // is connected and laid out. buildCanvasFrame seeds the table; ensureController() (invoked
  // below once the canvas is connected) creates the canvas context against a real size.
  buildCanvasFrame(state);
...
// ensureController creates and seeds the raster controller exactly once, AFTER the canvas is
// connected (so it measures a real size). It is a no-op for the empty (zero-flow) state: nothing
// to draw, so no context is acquired; the calm empty note + the table carry the empty case. If
// controller init fails (a context that fails to create between the probe and creation, or an
// unrecoverable failure later), it falls back in place to the SVG topology; the table is already
// present either way.
function ensureController(state: CanvasState): void {
  if (state.disposed || state.initStarted) return;
  if (state.current.length === 0) return; // no context for the empty state
...
// (re)feeds the node layer and the raster controller from one shared model per data change.
function buildCanvasFrame(state: CanvasState): void {
  ...
  // One model per data change, shared by the node layer and the raster controller.
  ...
  if (empty) {
    // The empty state never holds a canvas context. If a previous non-empty poll created one,
    // tear it down so an emptied account does not keep a loop running on a hidden canvas.
    if (state.controller) {
...
// replaceWithSvgFigure tears the raster controller down (so its theme listeners are removed and
// no rAF loop survives the swap) and replaces the whole root with the SVG topology handle, which
// carries its own table. Called for a failed initial init and for an unrecoverable later failure,
// so the old controller must not be left attached to the detached canvas or to documentElement.
...
// disposeCanvas stops the resize observer and tears down the raster controller (its rAF loop + context).
...
// wireDeferredInit installs the three triggers that create the raster controller AFTER the canvas is
...
// so this file stays under the structural threshold; imported above and called by
// buildCanvasFrame().
//
