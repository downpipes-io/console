...
//
// No-custody: the reference is the operator's own non-secret ticket text.
...
//
// The fail-open is recorded, deliberately, alongside being kept: the engine is the authority, it refuses the
// action with a 400 if a reference was genuinely required, and blocking every change-controlled action in the
// console on a transient policy read would turn one flaky GET into a total outage of the product's write path.
// Leaving the fail-open silent would make it unfalsifiable from either side: the engine counts its own 400s
// (configIntegrity.changeControlRefusals) but has no way to know the console never asked, and the console
// would know and tell nobody. The row is the missing half, and the two JOIN in the pack: a change-control
// refusal with a change-prompt-skipped row beside it, on the same op, shows a console that never prompted, not
// an operator who ignored the prompt. gateOp names WHICH action it fell on, because a skipped prompt on a
// restore apply and one on a destination repoint are not the same incident.
