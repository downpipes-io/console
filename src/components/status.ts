// Status dots and badges (design-system.md 6.8). The rule the whole product obeys:
// status is conveyed by colour AND shape AND label, never colour alone (WCAG 1.4.1).
// The dot encodes state with hue + shape (filled circle ok, hollow neutral, triangle
// warn, square danger); a visually-hidden text label rides alongside for assistive
// tech. "unknown" is a first-class state (never a stale green) so an unreachable
// engine reads honestly (information-architecture.md 1, 6.1).
...
export function runStatusTone(status: "in-flight" | "ok" | "failed" | "abandoned"): { tone: StatusTone; label: string } {
  ...
  //
  // A status string this build does not know, a row whose status is absent, one that arrives null and one
  // that arrives empty must not all read as {info, "in-flight"}, which is byte-identical to a run that is
  // genuinely still going. That is the answer beside every run in the RESTORE RUN PICKER
  ...
  // choosing what to recover from must never be told a run is under way when the console cannot say anything
  // about it.
  //
  // Every caller is routed through this one function, so the fall-through is decided in exactly one place
  // rather than re-derived per surface. The wire union can grow ("abandoned" is one addition to
  // lib/api/types/status.ts among others), and the engine deploys separately from the console, so a status
