// The 2D-CANVAS controller for the live-flow view: rasterises the living map on the CPU. It is
// the sole raster path (see chooseRenderMode): requestAnimationFrame + 2D raster need no GPU and
// no CSS animation support, so it animates in essentially every environment, including where
// WebGL would not (graphics acceleration off, a blocklisted GPU, a privacy shield). It draws
// from the shared model, the shared geometry constants and the shared honesty rules (stream only
// when flowing; sheen per hasAmbientSheen; failed/disabled inert; a single static frame under
// reduced motion). Architecture: the STATIC pipes render once into an offscreen layer (rebuilt
// on data/size/theme change only); each frame blits it and draws the dynamic marks (anchors,
// hub, ripple, sheen, particles).
...
// animation feel can be adjusted in one place.
...
export function create2dController(canvas: HTMLCanvasElement, animation: AnimationMode): RasterController | null {
  ...
  // Theme colours, cached because the probe + getComputedStyle are far too costly for a frame
  // path; invalidated on theme change.
  ...
  // only place layout is read; the caller's ResizeObserver drives this event-driven contract).
  const resize = (): void => {
  ...
  // buildStatic paints the halo + core pipes once into the offscreen layer, at fixed widths,
  // alphas and a device-pixel floor so the pipes stay visible at any DPR.
  ...
  // drawAnchors paints the source/destination node markers (density-adaptive; the engine hub is
  // drawn separately).
  ...
  // drawHub paints the engine hub: breath + the watch ripple.
  ...
  // drawSheen paints the idle ambient sheen (hasAmbientSheen; animated mode only). Skipped past
  // the edge budget, the per-frame sheen over every idle edge is the O(edges) cost a large fleet
  // drops.
  ...
  // drawParticles paints the particle streams (flowing edges only). The LAST pass in the frame,
  // so a single early return past the edge budget skips all per-frame particle work (the static
  // pipes, anchors and hub are already painted).
  ...
  // renderFrame draws one frame: blit the static pipes, then anchors, hub (breath + watch
  ...
  // Theme following: recolour on data-theme or the prefers-color-scheme media change (cache
  // invalidated, static layer rebuilt). The wiring + teardown live in a helper so this factory
  // stays under the structural threshold.
