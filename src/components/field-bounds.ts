...
//
// WHY THIS MODULE EXISTS. components/field.ts validates on blur only when the field carries a rule
// (`required` or `validate`). A control whose catalogued accepted_values states a bound but carries
// neither of those would let the console take a value it has just told the operator it will not
// take: a hint reading "1 to 64 chars" means nothing if a 65-character name is still accepted, and a
// hint reading "A whole number greater than zero" means nothing if 0 is still accepted. Every field
// that states a bound in its hint validates that exact bound at blur, so the hint is never a
// contradicted promise, and the refusal happens at the field rather than later at submit or nowhere
// at all.
//
// WHY A SHARED MODULE RATHER THAN A HAND-WRITTEN CHECK AT EACH CALL SITE. The catalogue's standing
// invariant is that client and server validation must not diverge in the client-looser direction, and
// a hand-written range check at every call site is a chance to drift from the bound the catalogue
// cites. Each factory here states the rule once, and each call site names its own bound, so a changed
// bound is a one-line change at the call site and the message follows it automatically.
//
...
// is exactly the class of defect this workspace keeps paying for: nothing detects the day the engine's rule
...
//
// WHY THE SCHEME IS CHECKED HERE. For a vendor that requires one, the hint directly beneath this
// field tells the operator, for a Splunk HEC destination, to "paste the token as Splunk <token>,
// including the literal word Splunk and a space. The bare token alone is not enough." The field is
// write-only and never re-displayed, the engine seals the value on arrival, and Splunk answers a bare
// HEC token with a 401 that is indistinguishable from a revoked token, so the scheme has to be
// checked at the one place that states the rule.
//
