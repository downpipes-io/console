// The RENDERING half of the two-channel error model (build-contract 2.4; the
// CLASSIFICATION lives in lib/errors.ts). Channel two (transport/auth) is rendered
// here as an inline BLOCK error with Retry that preserves operator input; channel
// one (in-flow ok:false) is rendered by inlineOutcome() as an EXPECTED inline
// result, never an error toast. A 401 is routed to the signed-out screen by the
// caller (it is not a block error). This keeps every screen consistent and stops
// the channels being conflated (current-state.md C3/C4).
...
// kinds that must NOT be recorded.
//
...
export function transportClassFor(kind: ErrorKind): ClientDiagTransportClass | null {
  switch (kind.kind) {
    ...
    case "engine-binding-absent":
      // The console's OWN worker reports it has no ENGINE service binding. This is NOT `html-not-engine`, and
      // the difference is the whole gap: the engine URL is correct, the engine may be perfectly healthy, and the
      // fault is in the console's deploy. Filing it under html-not-engine would send the operator to correct an
      // address that is already right and say nothing about the missing binding.
      return "engine-binding-absent";
    case "console-origin-fault":
      // The console's OWN worker manufactured the 500. It is NOT `server-error` (the engine never saw the
      ...
      // is a SIBLING PRODUCER of the same false row a feature probe could write: blockError renders on every
      // screen's failed read, so falling through to `server-error` here would keep a fabricated engine fault in
...
// accounts of the same failure. Splicing errText(err)/err.message straight into a toast instead
// would show the engine client's raw "<verb>: <status>" throw, e.g. "add operational key: 500" --
// which names neither what failed nor why, and on a 403 can read as a bare internal token
// ("forbidden-class=engine-authz: 403"). Every kind here is already reviewed customer copy; this
// only skips the heading and the Retry button.
...
// (build-contract 2.4, design-system.md 6.21). onRetry re-runs the failed load.
export function blockError(err: unknown, onRetry: () => void, opts: { origin?: string; healthReachable?: boolean } = {}): HTMLElement {
  // classifyError does not auto-promote a fetch failure to console-origin from opts.origin alone:
  // the two look identical at the transport layer and are only distinguishable by an
  ...
  // The classification is recorded HERE, at this seam, rather than at each screen's own catch site: every
  // screen renders a channel-two error through blockError, so coverage is complete and no screen can miss it
  // (the same argument engine-fetch.ts makes for the fetch seam).
  //
  // A NETWORK THROW IS THE ONE CLASS THIS SEAM CANNOT RESOLVE ON ITS OWN, so it does not pretend to: a CORS
  // block and a dead engine are the SAME TypeError, and only an independent probe can tell them apart.
  //
  // Every other kind carries its own answer in its tag and is recorded here and now. A `network` kind instead
  // goes to recordNetworkBlock, which asks the engine's /admin/health directly (served with a wildcard origin
  // and no credentials, the one unauthenticated route) and records origin-rejected or engine-unreachable once
  // it knows. Fire-and-forget: the card below paints immediately.
...
function describe(kind: ErrorKind): { heading: string; detail: string; showOrigin: boolean } {
  switch (kind.kind) {
    ...
    case "stepup-required": {
      // NOT a lapsed session. A step-up identity check for this sensitive action was needed and
    ...
    case "access-reauth-required":
      // The engine judges an Access caller on the age of its Access assertion. Older than five minutes
      // and a sensitive action is refused until the operator signs in to Access again. Not a passkey prompt
      // (an Access session holds none) and not a sign-out: the session is fine for everything ungated.
      return {
        heading: "Sign in to Cloudflare Access again for this action",
        detail: "This action needs a Cloudflare Access sign-in within the last five minutes, and yours is older. Nothing was changed. Sign out of Access (open /cdn-cgi/access/logout on this console's address), sign back in, then retry.",
        showOrigin: false,
      };
    case "access-redirect":
      return {
    ...
    case "engine-binding-absent":
      // The console worker serves this surface and forwards it to the engine over a service binding; the
    ...
    case "console-origin-fault":
      // The console's own worker answered this call with a 500 it manufactured: its dispatch threw, or the
    ...
    case "rate-limited":
      ...
      // operator how long to wait when the engine sent an honour-able Retry-After.
      return {
    ...
    case "answer-unreadable":
      // A 2xx whose body could not be read must not fall through to the "network" copy below, which would
      // assert the OPPOSITE of what happened -- the request completed and the engine was reached; only the
      // response could not be understood. Sending the operator to check reachability cannot fix a
...
//                       fails its one read stays failed for the life of the screen, and the only way
//                       back is reloading the tab (or, on a control built once at screen render,
//                       navigating away and returning) -- a dead end.
//   a human sentence    the caller passes prose. Interpolating errText(err) instead would show the
//                       engine client's own "<verb>: <status>" throw, so an operator would read
...
// (design-system.md 7a) forbids. role="status" for the same reason, where blockError uses role="alert":
...
// an expected result, not a fault (build-contract 2.4).
...
//
// AN ENGINE 403 CLASSIFIED AS `not-engine-body` (a refusal that did not arrive in the engine's own
// refusal shape at all, so a block page or a proxy made it) must never read as a role denial: the
// caller's role can perfectly well hold the permission the message names, the same screen can already
// prove that by rendering the action live, and the real cause -- something in front of the engine
// refusing the call -- would go unnamed while the customer is sent to a screen where there is nothing
// to change. The same is true of the other two non-capability classes: the DO's authz funnel
// deliberately names nothing and is not always about the caller at all, and the CSRF check refuses
// every cookie-borne save in the whole console with a remedy that is an engine variable, not a role.
//
// lib/errors.ts tells the four apart by a SHAPE GATE over the body and folds the class into the
// throw, and forbiddenClass() reads it back. Only ONE of the four establishes anything about the
// caller: `engine-capability`, where the engine's route gate named the capability it wanted. Saying
// "your role" for the other three would be an assertion this code never established, sending a
// customer to change something that is already right.
//
// A NULL class keeps the original sentence, deliberately: an unclassified 403 is one this gate
// cannot read (a hand-built error with no recognised shape), and the role reading is the right
// default there.
