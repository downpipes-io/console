// Trust telemetry chips (design-system.md 6.23, access-security-model.md 1.3/1.4).
// These are unique to the product and they must be HONEST:
//
//  - The Access verifier chip reads the REAL verdict; it is NEVER a hardcoded green
//    (a hardcoded or stale verified state would misstate the customer's security
//    posture, so this is treated as disqualifying if shipped). It distinguishes verified (green,
...
//    from the indigo action accent and the green ok-status in BOTH themes (the
//    owner's locked decision). Status is hue + shape (shield-check glyph) + label.
...
export function accessVerdictFromCaller(caller: Caller | null, whoamiAvailable: boolean): AccessVerdict {
  if (whoamiAvailable && caller) {
    ...
    // one, distinct from the shared-token fallback below: collapsing it into that state would tell an OIDC
    // or SAML customer, on every render, that their fleet runs on the shared break-glass token, which is false.
    ...
    // The break-glass posture (token-fallback) is claimed ONLY for the method that IS the break-glass: an
    // engine that adds a new sign-in method must never fall through to token-fallback by default, because
    // claiming the shared break-glass token is in use when it is not misstates the customer's security
    // posture and sends them chasing a remedy ("rotate the token, we are exposed") that fixes nothing. Any
    // method this chip does not recognise resolves to `unknown` instead: no claim is made about it in
    // either direction. The unrecognised method string never rides (it is engine-supplied and could be
    // corrupt); the row records only that the class was not one this build knows.
...
// _testChipTone is exported for the test suite so the mirror
...
// the unknown state is neutral. NEVER a bare permanent green.
...
// label says "never sent to us", not "never left the browser", because the key IS downloaded onto
// the operator's own disk. Trust family + the lock glyph; earned by construction (keygen.ts is
// local; there is no upload path).
//
// The title is kept alongside the qualifying info-tip: it costs a mouse user nothing, and the tip
// carries the same qualification for keyboard, touch and screen-reader users who cannot reach a
// hover title.
