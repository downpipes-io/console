...
// flow shapes without pulling in the DOM renderer; topology.ts re-exports every
// externally-referenced declaration by name.
//
// These are pure type declarations: no DOM, no runtime, no imports beyond the FlowRecord
// presentation the model produces.
...
export interface FlowEndpoint {
  ...
  down?: boolean;
  // The engine's coarse per-destination down REASON (replication.ts DestReplState.reason - auth /
...
// behind. It is NOT "partial": folding the two together would tell the operator to wait for a copy that is
...
export interface LayoutNode {
  ...
  down?: boolean;
  // WHY the node is down, the same per-destination reason FlowEndpoint.downReason carries. The node
...
export interface StatusCounts {
  ...
  stale: number;
  partial: number;
  // Destinations the engine has never reported a copy for. Its own bucket, so the aria summary can say
