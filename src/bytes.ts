...
// EVERY reject branch below records its own closed class into the console-diagnostics ring before it throws.
// This decoder is the gate every piece of pasted key, share and recovery material passes through, and it is
// the ONLY place that knows WHY the material was refused: the screens above it coarsen the throw into one
// "that does not look right" hint, and no request is made, so the engine has no record of the refusal and
// never can have one. Without a record here, a customer who cannot get their recovery share accepted, and a
// support engineer holding their pack, are looking at the same nothing.
//
...
//
// The THROWN MESSAGE is held to the same bar: it names the FAULT CLASS and nothing else, the same closed class
// as the ring, never the offending character, its offset into the material, or the material's length. `s` here
// is a pasted wrapping key, share or recovery code, and the screens above this decoder render caught errors
// straight into the page, so a rendered message is one screenshot away from a support ticket; a message
// carrying the character, offset or length would put that fingerprint of the secret into the ticket too.
export function b64urlDecode(s: string): Uint8Array {
  ...
  // Decode STRAIGHT INTO a Uint8Array rather than accumulating a number[] and copying at the end. Accumulating
  // into a plain array would put every decoded byte of a key, share or wrapping key into a JS array that
  // nothing can zero and that the caller never sees, so wiping the returned buffer would leave a full second
  // copy behind for the collector to reclaim whenever it likes. This is the one decode on the break-glass
  // path, so that copy would be the material itself. The output length is exact: base64url-no-pad yields floor(len * 6 / 8) bytes, and the
