...
// goes green again.
//
// WHY THIS EXISTS. A poll that asks "is it red now" answers the same way on the first red run of a streak
// and on the thirtieth, so it only reports what a human already suspects long after the streak began. The
// question worth paying for is "when did it STOP being green", which only a transition can answer. This
// file is copied into every repo it protects, so a claim in it must stay true in whichever repo it lands
// in rather than naming one repo specifically.
//
...
// chain shares the chain's fate, so the moment the chain is red the gate grading the chain is silent too.
// A poll would also survive that, but a poll fires on a red deep into a long streak as readily as the
// first and cannot tell them apart, and it costs a run every N minutes forever rather than one short job
// per CI run.
//
// TWO FAILURE MODES THIS IS DESIGNED AGAINST, because either makes it useless in the situation it exists
// for.
//
//   NOISE. It must not open a fresh issue on every red run of an ongoing streak. One issue per red run in
//   a long streak is a notification stream nobody can read, which is indistinguishable from silence. So
//   an open tripwire issue suppresses opening another. The streak is not lost: each further
//   red run EDITS the open issue, which GitHub does not notify on, so the streak is visible in one place
//   at no cost to the reader.
//
//   SILENCE ON AN ABSENT PREDECESSOR. "The previous run was green" is not the only reason to fire. A
//   batched push is graded only at its tip, so earlier commits in that push get no CI run of their own.
//   A run can also be cancelled, skipped or fail at startup, and the very first run of a workflow has no
...
// every red it sees has a red predecessor, and the README states that cost plainly. A repo already sitting
// in a standing red streak when this ships would get nothing from the transition path, ever, until main
// went green once of its own accord.
//
...
// A cron that asks "is main red" answers the same way on the first red and deep into a long streak, which
// is exactly how a scheduled sweep can find a long streak only very late rather than at the start.
// Adoption is a one-shot that converts an existing streak into the same durable issue the transition path
// would have opened, after which the transition path carries it, including closing it on the first green.
 ...
 *
 * WHY THIS EXISTS. This tripwire grades ONE workflow, but its issue can read as though it graded the whole
 * of main. At a commit where the CI workflow concludes success while Security Audit concludes failure,
 * main is red while this tripwire stays silent, which is correct for the workflow it watches and
 * misleading about main. A reader cannot be expected to hold that difference in their head, so the issue
 * names the gap.
 *
 ...
 * listed. Comments are stripped before any key is matched, so a commented-out `push:` is never read as a
 * live trigger.
 */
...
// on it. That is not hypothetical; it is a real shape a commit's checks can take.
...
const ORDER_FIXTURES = [
  ...
  // false`, so it concludes `skipped` on every run while a repo is private, and the resulting skip streaks
  // run long enough that a naive "latest run was not success" reader would call each of them a red main.
  // A repo that carries no `scorecard.yml` at all has no such streak to speak of. A repo whose workflows
  // API lists only this tripwire and its own CI workflow has no Scorecard run history to read down either,
  // by the same rule.
  //
  // This file is copied across several repos, so a skip-streak count measured in one repo is never a fact
  // about another, and any such count is stale as soon as it is written because it grows by one on every
  // push while the repo stays private. The durable fact is the `if` condition above, not any particular
  // count.
...
const PARSE_FIXTURES = [
  ...
  // The comment trap. A scanner that reads commented-out YAML as live YAML gives a false positive, so both
  // the whole-line and the trailing forms are graded here.
 ...
 *
 * THE `created` BOUND IS LOAD-BEARING. The runs endpoint returns the workflow's most recent runs, newest
 * first, with no notion of "before this one". Asked for one page and then filtered down to run numbers
 * below the run being graded, it answers correctly for the newest run and answers "there is no previous
 * run" for anything older than a page, because the page it fetched contains nothing old enough. That is
 * not a harmless imprecision: on the one run where the comparison is the whole finding, the first red of a
 * streak, a wrongly "absent" previousState produces the same ACTION here, but the issue body would tell a
 * reader there was nothing to compare against and would skip the ungraded-commit range that comparison
...
/**
 * The commits between the previously graded run and this one. A batched push grades only its tip, so
 * earlier commits in that push go ungraded, and the reader needs to know which commits are in the blast
 * radius rather than only which sha the runner happened to see.
 */
...
// through a symlinked directory, and main() then never runs while the process still exits 0 - a fail-open
// guard that would report nothing rather than failing loud.
