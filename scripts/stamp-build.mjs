...
// Building from the repo root, from /, from the repo's parent, or with node_modules reached through a
// symlink each names the shared chunk differently while producing byte-identical output, and the operating
// system is NOT an input either: a macOS build reproduces CI's Linux bytes exactly. Same mechanism as the
// engine's path-dependent artefact digest.
//
...
// elsewhere while node_modules itself is a real directory. Both are confirmed by mutation testing.
//
...
// removed and cwd one level up, the build emits a differently named chunk whose bytes are still
// byte-identical, and the escape check raises nothing. This is the direct check for the thing that
// actually matters, so it does not share that blind spot.
...
// path-escape filter) call process.exit(1), which skips finally entirely, so without this a staging
// directory holding a full copy of the bundle would survive every refusal and accumulate in TMPDIR. An
// exit handler runs on process.exit() as well as on a normal return, and rmSync with force is idempotent,
// so the finally below stays as the ordinary path and this covers the rest.
...
try {
  ...
  // a copy-only build would leave public/ holding a MODIFIED app.js, the NEW hashed chunks (untracked) and
  // the OLD hashed chunks (still tracked and now orphaned) at the same time. A staging area that looks
  // complete in `git status` is not, and the bundle drift guard would then refuse with a stale FILE SET
  // rather than stale bytes. That guard still compares the whole set: pruning here is what stops the state
  // it exists to catch from being produced in the first place.
  //
