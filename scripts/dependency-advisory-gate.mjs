...
//
// WHY NOT FOUR HAND-WRITTEN GATES, and why not a shared package either. Four independent, hand-written
...
//
// THE EMPTY LIST IS THE GOAL AND IT IS REACHABLE. Every repository that carries this gate can be taken to
// zero acknowledged advisories, by npm `overrides` that move a transitive package inside its own major and
// leave wrangler, miniflare, workerd and @cloudflare/vitest-pool-workers untouched. An acknowledgement is
// therefore a live claim that a fix was looked for and not found, not a resting state.
//
...
// had armed anything.
//
// BOTH SIDES ARE CANONICALISED. `resolve()` normalises a path and does not follow a symlink, so reached
// through a symlinked path a check using it alone would decide this file is not the entry point, print
// nothing, run nothing and exit 0. A gate that silently passes when it is invoked through a link is the
// exact failure this whole file exists to stop, and engine's verdict-guard rules name that shape as a real
// risk across scripts/ gates and validators generally.
