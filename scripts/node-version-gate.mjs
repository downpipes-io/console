...
//
// WHAT THIS IS FOR, and it is not tidiness. Workflows can each declare their own NODE_VERSION, and
// nothing tells a developer working locally when a workflow's declared version has drifted from theirs.
// `engines` says `>=20`, which a machine running Node 25 satisfies, so the local runtime can be several
// majors ahead of the one that gates while every declared constraint is met.
//
// That gap has a real cost: a coverage figure is a property of the runtime, so the same file can measure
// comfortably above its floor on one Node major and below it on another. A per-file coverage gate can
// therefore pass locally and fail in CI on the same code, which reads as a rule violation when it is
...
// the file only helps while it agrees with the workflows. Independent copies of a version string drift,
// and the drift is silent until a job behaves differently from the one beside it.
//
