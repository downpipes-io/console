...
// status, raise/clear the banner (including the applyStaged leg); on a read fault classify a
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // appended after it. A bare document.body.appendChild(host) places the host LAST in document order --
    // after #app (the sticky header, rail and main region the real shell mounts).
    ...
    console.log("\n-- R-81: a staged recovery offers the break-glass CONFIRM, not the manual reconcile --");
    freshHost();
