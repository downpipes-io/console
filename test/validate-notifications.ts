...
// (src/screens/notifications.ts), build-contract section 2 + section 9.
...
if (engineEvents === null) {
  ...
  if (process.env.REQUIRE_ENGINE === "1") {
    verdictCannotCheck(
      "validate-notifications: REFUSED, REQUIRE_ENGINE=1 and no engine checkout is reachable.\n" +
...
// ---------------------------------------------------------------------------
// buildChannelInputFrom: jsm/servicenow (monitoring integrations). Both carry
...
// Mirrors the engine's severityOf (notify-routing.ts) exactly: every one of the engine's events maps to a
// severity here, so each can be individually targeted by a rule.
...
// ---------------------------------------------------------------------------
// The two replication events (engine L1a/L1b) surface end to end in
...
// ---------------------------------------------------------------------------
console.log("\n-- replication events render end to end --");
