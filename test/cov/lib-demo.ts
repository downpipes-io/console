...
// with/without a limit, the WRITE handlers, create downpipe deferred to a 202 pending, trigger /
...
// Runtime polyfills for two browser-standard Node members the RAIL nav-bar variant's beat model uses:
// layoutForWidth MOVES existing nodes with insertBefore (moving preserves their listeners) and steers
...
import { verdictReached } from "../lib/verdict-guard.ts"; // ARMS the completion guard
import type {
  AuditPage,
  ChainVerdict,
...
await (async (): Promise<void> => {
  ...
  // The guard admits the training walk alongside the tour (training-mode.ts, the same dev-host-scoped
  ...
  }
  {
    ...
    // EIGHT downpipes across seven source types. The eighth is the ZONE-scoped cf-config pipe: it exists so
    // a browser journey can reach the surface picker's two scope shortcuts, which render only when a
    // zoneId is present and would otherwise be unreachable by any test.
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    // lib/api/types/destinations.ts). The fact worth grading is that the singular view answers with the
    // DEFAULT destination, so it is graded against the collection's own defaultId instead of against a
    // field the contract strips.
    ...
    // an Azure Blob container, present so a provider badge other than "S3" renders somewhere in the tour
  ...
  // ---------------------------------------------------------------------------------------------
  // The WRITE handlers (the demonstrated mutations): each mutates the world and returns the
  ...
  // The screen-load reads across these families are modelled, each in the wire shape its screen parses, so
  // every nav item renders populated. A spot-check across the families (canary, config change-control + history,
  ...
  // These WRITES return the specific shape their screen parses (a click resolves cleanly).
  {
  ...
  // (parseJsonOrPending -> pending). No engine route emits `pending`, so the simulator is pinned to the
  // engine's actual wire shape rather than a fabricated one.
  {
  ...
  //
  // The faked engine derives the destination kind from its endpoint host, distinguishing all four kinds the
  // product supports (s3, r2, gcs, azure) rather than collapsing anything non-R2 to "s3". Getting this wrong
  // would report a saved Google Cloud or Azure destination back as S3, and that misreported kind is what the
  // residency panel, the topology map's phrase and the downpipe drawer all read: the screens whose whole job
  // is to say where the archives went would then name the wrong vendor.
  //
  // The refusal is the other half. A submitted immutability policy must be refused for an R2 destination,
  // never silently stamped objectLock "enforced", because the real engine refuses it with a 400: R2 over
  // its S3 endpoint answers 501 NotImplemented to x-amz-object-lock-mode, and no R2 bucket can be created
  // with Object Lock either. Teaching a success the product will not give would leave a learner believing
  // archives are immutable when they are not.
  {
  ...
  // resetWorld restores the pristine seed deterministically (the Back/Restart hook).
  {
    resetWorld(Date.UTC(2026, 5, 27, 0, 0, 0));
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    // (the router re-resolves; here a navigate spy observes it), and shows a "Sample data reset" toast.
    {
      ...
      // so the reset's cache-clear (an in-place reset must leave no stale palette snapshot) is observable.
      resetWorld(Date.UTC(2026, 5, 27, 0, 0, 0));
      ...
      // The reset also clears the store entity + palette-search caches, so an in-place reset leaves no
    ...
    // The SCREENSHOT-ROBUST corner marker: a fixed top-right "DEMO" badge at a z-index ABOVE
    ...
    // The identity chip qualifier in tour mode: the verified-Owner account chip carries a "Sample
    ...
    // The tour-distinct favicon: swapTourFavicon points the <link rel=icon> at a tour data-URI so the
  ...
  // ---------------------------------------------------------------------------------------------
  // demo/tour/overlay: the shared focus-ring helpers. This module owns only the imperative focus-ring pair,
  // shared by the nav-bar, the info-points and the welcome card. Drive both helpers headless: applyFocusRing
  // sets the --ring outline, clearFocusRing removes it.
  // ---------------------------------------------------------------------------------------------
  {
  ...
  // Two variants off ONE control set (the beat model): the historical bottom-centre bar below 1024,
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    // brings it back. Showing the full prose at phone width (390 as 577px of an 844px viewport) would cover
    // the console the learner is meant to be looking at, so it stays collapsed until asked for.
    ...
    // window observes the resize wiring and fires the listener.
    {
      ...
      // The rail's foot is TWO rows because four labelled controls on one row measure 324px plus their gaps
      // against a 344px panel: from the step where Back appears, Next would hang outside the rail at every
      // desktop width. The walking pair (Back, Next) has its own row; the chrome (sound, pause, leave) sits
      // above it.
    ...
    // ---- the rAF-driven tracking loop (the production browser path) ----
    {
      ...
      // A MODAL OVER THE ANCHOR HIDES THE STAGE, so a dialog opening over the anchored element (for example
      // the create dialog opening over the empty-state table the New downpipe step is anchored to) never
      // leaves the spotlight lighting a rectangle nobody can see, with its top behind the sticky bar. An
      // anchor INSIDE a dialog is the opposite case and stays lit, which is how the run-detail beats work.
      {
  ...
  // chapter's BEATS (the beat model). Drive it headless with an injected navigate spy + a controllable
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    // The funnel exits are authored as data (FUNNEL_CTAS), the same the closing chapter renders. The
    // secondary CTA channels to the pricing page; its label promises a page.
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    // The pitch paragraphs are gone; the card keeps exactly two short body lines (what the tour is + the
    // single free-edition line) before the expectation line. It makes no duration promise: the tour's own
    // autoplay budget runs 5.6 to 6.9 minutes over the two walks, so any fixed "<number>-minute" claim would
    // be false for at least one walk. The copy names the chapter count and the visitor's own pace instead.
    ...
    // A duration promise the product cannot keep is the defect, not the wording, so this checks the shape
    // (any digit or spelled-out number followed by "-minute(s)") rather than one exact sentence: a
    // digit-only pattern would miss a promise written out in words, and checking for one sentence would
    // miss a differently worded promise making the same claim.
    ...
    // Free Explore is visually demoted: it keeps its exact accessible name and stays a real
    ...
    // Exploring freely is reversible: without a relaunch affordance, Exit mid-tour would show Resume while
    // the welcome left nothing, an inconsistent asymmetry. After skipping, the persistent "Take the tour"
    // relaunch affordance is mounted, and a click on it re-mounts the welcome (carrying the same deps, so
    // the spy director is honoured) and removes itself, so a visitor who skips to look around first can
    // still enter the guided narrative.
  ...
  // ---------------------------------------------------------------------------------------------
  // demo/tour/analytics: the tour's self-hosted funnel analytics. emit() is a no-op until enabled
  ...
  // two deliberate exits are authored as data (FUNNEL_CTAS). The welcome card emits tour_started (mount);
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    // The funnel exits: the website's deploy-steps page (https, primary) and the pricing page (secondary).
    // The analytics kind is the coarse, fixed conversion descriptor, never free text.
    {
      ok("the funnel offers exactly two deliberate exits (deploy + see pricing)", FUNNEL_CTAS.length === 2);
      const deploy = FUNNEL_CTAS.find((c) => c.kind === "deploy-final");
      const pricing = FUNNEL_CTAS.find((c) => c.kind === "pricing-final");
    ...
    // director: the nav-bar renders the two exits, and clicking each emits cta_clicked with the coarse kind.
    {
      ...
      // The primary funnel exit is the WEBSITE's deploy-steps page (https://downpipes.io/deploy), not a docs
      // page. The lookup below matches on that href so the click assertions actually exercise real anchors.
