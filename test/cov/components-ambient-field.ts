...
// in the console (the rain canvas). It is Canvas2d + rAF only, so the shared dom-shim is
...
// Marks this file a MODULE, and arms the completion guard. Unlike its siblings this file imports its
// SUBJECT dynamically, because it installs its own document, rAF and localStorage globals at top level
// and only then loads the component: a static import of the subject would be hoisted above that setup
// and the component would load into a bare environment. The guard is different and is imported
// statically on purpose. It touches no DOM global, only process.stdout, process.stderr and an exit
// listener, so hoisting it is harmless, and arming as early as possible is the point of it. Some
// module-level import is needed regardless: without one the compiler reads the file as a script, so
// `failures` and `ok` collide with every other test file's globals and top-level await is rejected.
