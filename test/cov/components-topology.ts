...
function sampleFlows(): FlowRecord[] {
  return [
    ...
    },
    {
      ...
      destination: { name: "vault-mirror", kind: "r2", down: true, downReason: "auth" }, // a DOWN destination node with a reason for why it is down
...
function nodeAndHandlerTests(): void {
  ...
  ok("node: B30 the down line names WHY, not just 'down - KIND'", downLine!.textContent === "down - credential rejected");
  ...
  ok("node: B30 the title's reason matches the same label as the visible line", (downTitle!.textContent).includes("currently unreachable (credential rejected)"));
...
// (4b) Fallback when a down destination carries NO reason (an absent DestReplState.reason,
// same as "other" or an unrecognised newer-engine class): the line must NOT grow a dangling
// "down - " or an empty "(...)" parenthetical. It keeps the bare "down - KIND" line and the
// plain "currently unreachable" title. A separate one-flow render (not sampleFlows(), whose
// vault-mirror node carries a reason) keeps this isolated from the counts every other test in
// this file pins against the shared 5-flow sample.
