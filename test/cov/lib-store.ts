...
async function main(): Promise<void> {
  ...
  // ========================================================================
  console.log("\n-- clearSessionData / signOut purge the session's browser storage and leave preferences (V14.3.1) --");
  // ========================================================================
  {
    // sessionStorage is not part of the shared DOM shim; a Map-backed stand-in with the Storage surface the
    // store reads (length, key(i), removeItem) is installed for this section only.
    ...
    g.sessionStorage = sessionShim;
    try {
      sessionShim.setItem("dp-draft:engine.test:sources-protect-selection", "[\"KV_a\"]");
      sessionShim.setItem("dp-draft:other:restore-scope", "{}");
      sessionShim.setItem("dp-unrelated", "keep");
      localStorage.setItem("downpipes.attend.session", "{\"sessionId\":\"s1\",\"sampleRate\":3}");
      localStorage.setItem("dp-theme", "dark");
      localStorage.setItem("dp-engine-url", "https://engine.test");
      setCaller({ id: "u-1", role: "owner" } as unknown as Caller);
      setDownpipesCache([{ id: "dp-a" }] as unknown as DownpipeState[]);
      // NEGATIVE CONTROL first: before any clear, every seeded item is present, so an absent item below is the clear.
      ok("control: both drafts and the attend record are present before the clear", sessionShim.getItem("dp-draft:engine.test:sources-protect-selection") !== null && sessionShim.getItem("dp-draft:other:restore-scope") !== null && localStorage.getItem("downpipes.attend.session") !== null);
      clearSessionData();
      ok("clearSessionData removes every dp-draft: key", sessionShim.getItem("dp-draft:engine.test:sources-protect-selection") === null && sessionShim.getItem("dp-draft:other:restore-scope") === null);
      ok("clearSessionData leaves a sessionStorage key outside the draft family", sessionShim.getItem("dp-unrelated") === "keep");
      ok("clearSessionData removes the attended-session resume record", localStorage.getItem("downpipes.attend.session") === null);
      ok("clearSessionData leaves the theme and the remembered engine URL", localStorage.getItem("dp-theme") === "dark" && localStorage.getItem("dp-engine-url") === "https://engine.test");
      ok("clearSessionData nulls the caller and the entity cache", getCaller() === null && getDownpipesCache() === null);
      clearSessionData();
      ok("clearSessionData is idempotent", getCaller() === null);
      // signOut goes through the same clear.
      sessionShim.setItem("dp-draft:engine.test:x", "1");
      localStorage.setItem("downpipes.attend.session", "{}");
      signOut();
      ok("signOut purges the storage too", sessionShim.getItem("dp-draft:engine.test:x") === null && localStorage.getItem("downpipes.attend.session") === null);
      ok("signOut still leaves the remembered engine URL", localStorage.getItem("dp-engine-url") === "https://engine.test");
      // A blocked sessionStorage does not throw out of the clear.
      g.sessionStorage = { get length(): number { throw new Error("blocked"); } };
      let threw = false;
      try { clearSessionData(); } catch { threw = true; }
      ok("a blocked sessionStorage is recorded, not thrown", threw === false);
      // Structural: the server-initiated endings in app.ts (the 401 a screen meets, and the identity resolve's
      // 401) both run this clear before the sign-in screen, so a session the server ended is purged too.
      const appSrc = readFileSync(new URL("../../src/app.ts", import.meta.url), "utf8");
      const routeBody = appSrc.slice(appSrc.indexOf("function routeToSignIn("), appSrc.indexOf("function registerRoutes("));
      ok("app.ts routeToSignIn calls clearSessionData before navigating (V14.3.1)", /clearSessionData\(\)/.test(routeBody) && routeBody.indexOf("clearSessionData()") < routeBody.indexOf("router.navigate("));
      const resolveBody = appSrc.slice(appSrc.indexOf("if (isUnauthorised(err)) {", appSrc.indexOf("reRenderCurrentScreenForResolvedIdentity(shell")));
      ok("app.ts identity resolve clears session data on a 401 (V14.3.1)", resolveBody.slice(0, 900).includes("clearSessionData()"));
    } finally {
      if (hadSession) g.sessionStorage = priorSession; else delete g.sessionStorage;
      localStorage.removeItem("dp-theme");
      localStorage.removeItem("dp-engine-url");
    }
  }
