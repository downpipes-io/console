// Coverage for src/components/trust-chips.ts: the honest trust telemetry chips (design-system.md
// 6.23, access-security-model.md 1.3/1.4). The sibling validator
...
//
// The crux these chips must honour is this rule: only the verified and passkey-verified states
...

{
  ...
  ok("verified chip is not amber or neutral (only verified is teal)", !classesOf(chip).includes("trust-chip--warn") && !classesOf(chip).includes("trust-chip--neutral"));
...

{
  ...
  ok("token-fallback chip is not teal", !classesOf(chip).includes("trust-chip--trust"));
...

{
  ...
  // Trust-family guard via the render path: feeding the derived passkey verdict back through accessChip yields
...
{
  // The chip's label is the claim ("your key is never sent to us") and NO_CUSTODY_SCOPE is the
  // qualification that bounds it. A title attribute alone is revealed by hover only: the chip is a
  // plain span with no tabindex, so no keystroke reaches it; a touch device has no hover at all; and
  // a generic span's title is not announced as a description. So a reader who does not use a mouse
  // must not be left with the UNQUALIFIED label, which reads as a stronger claim than the product
  // makes. design-system.md 6.15 requires the qualification to be triggered on focus as well as
  // hover, never hover-only, and never the only place essential information lives. The sibling
  // Access verdict chip satisfies this because components/verdict.ts repeats its explanations as
  // visible prose on the same screens; this chip carries no such panel, so the Overview's info-tip
  // is the reachable carrier that keeps the qualification available without a mouse.
  //
  ...
  // focusable info-tip, so removing the reachable copy fails here instead of silently leaving the
  // console hover-only.
