...
//
// The one raster renderer is the 2D canvas controller (there is no WebGL path), so a hand rolled 2D
// context + a fireable ResizeObserver/MutationObserver + a controllable rAF are enough to run the
// whole render path headless. The stub is a richer sibling of the one in validate-live-flow.ts (this
// one's getComputedStyle returns a real parseable colour, its observers genuinely fire, and its
// rAF/setInterval are driven by the test), kept local so the shared suite's careful stub is untouched.
// It reads no real GPU and transmits nothing. Run with `node test/cov/components-live-flow.ts`
// (auto-run by test/cov/run.mjs).
