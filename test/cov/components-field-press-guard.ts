...
//
// On the live push configure form this shows up concretely: a Splunk HEC token typed without its
// "Splunk " scheme, then one press on the Enabled checkbox, puts mousedown on INPUT#push-enabled and
// mouseup on a div 45px lower, so the click lands on the FORM and the checkbox does not toggle. The same
// press with an accepted secret, and the same press with the error already on screen, both toggle it: the
// checkbox is never the fault. The same shape also swallows the first press of the SAVE button after a
// refused secret: mousedown on the button, mouseup on the error slot that has just appeared, and the
// button's own click handler runs zero times.
//
// Layout is not what this file tests: this tests the RULE that removes the defect, which is testable
// exactly. Every assertion here fails against field.ts without the guard (the error paints during the
// press) or against a guard that never releases (the error never paints at all), so neither the defect
// nor an over-correction can pass.
//
