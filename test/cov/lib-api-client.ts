...
async function main(): Promise<void> {
  ...
  // ========================================================================
  console.log("\n-- parseJsonOrPending: applied (200), pending (202), malformed-202 throws, errors --");
  // ========================================================================
  {
    ...
    // result models the id and nothing more.
    ...
    // A malformed 202 (not the pending shape) THROWS an honest answer-unreadable error instead of
    // being coerced into a false applied value. The message carries the 202 status, exactly like
  ...
  // ========================================================================
  console.log("\n-- parseJsonOrOwnerAction: queued (202), malformed-202 throws, error fold, no-error --");
  // ========================================================================
  {
    ...
    // A malformed 202 (not the owner-action shape) THROWS an honest answer-unreadable error instead of
    // being coerced into a false result, the same class of guard parseJsonOrPending carries.
    ...
    // A non-2xx with NO usable error degrades to the shared failResponse throw. On a 403 that throw NAMES THE
    ...
    // token, so extractStatus and every classifyError branch read it.
    ...
    // parseJsonOrPending routes (notify channel/rule save) fold the engine's 400 reason too, exactly
    ...
    ok("a notify channel-save 400 folds the engine reason (not a bare status)", msg === "save notify channel: url points at a private/loopback address: 400");
    restore();
    ...
    ok("a notify 403 keeps the forbidden-class marker, not a folded reason", msg.startsWith("save notify channel: forbidden-class=") && msg.endsWith(": 403"));
    restore();
    ...
    ok("a notify rule-save 400 folds the engine reason", msg === "save notify rule: channel not found: 400");
    restore();

    // The dual-control approve/reject family and the run-trigger fold the engine's 400 reason via
    // parseJsonOrReason, so a refusal names why instead of surfacing a bare status.
    ...
    ok("an approve-config-change 400 folds the engine reason (superseded/decided)", msg === "approve config change: the base config moved; re-propose: 400");
    restore();
    // A 403 on the same route keeps the forbidden-class marker (the fold is gated out for 401/403/429/5xx).
    restore = stub({ status: 403, text: JSON.stringify({ error: "forbidden" }) });
    msg = await expectThrow(() => engine.approveConfigChange("chg1"));
    ok("an approve-config-change 403 keeps the forbidden-class marker, not a folded reason", msg.startsWith("approve config change: forbidden-class=") && msg.endsWith(": 403"));
    restore();
    // The restore self-approval refusal (a 400) names itself instead of reading a bare status.
    ...
    ok("a restore self-approval 400 names the reason", msg === "restore approve: cannot approve your own request: 400");
    restore();
    // Run-now on a stale downpipe id folds the engine's "unknown downpipe" reason (the list is stale).
    restore = stub({ status: 400, text: JSON.stringify({ error: "unknown downpipe abc" }) });
    msg = await expectThrow(() => engine.trigger("abc"));
    ok("a trigger 400 folds the unknown-downpipe reason (not a bare status)", msg === "trigger: unknown downpipe abc: 400");
    restore();
  }
  ...
  // ========================================================================
  {
    ...
    // A cancelled/refused step-up ceremony surfaces the stepUpRequired 401 tagged as a step-up
    ...
    ok("a cancelled ceremony classifies stepup-required, never unauthorised", classifyError(msg).kind === "stepup-required");
    sf.restore();
    ...
    // Even with no ceremony wired, a stepUpRequired 401 means the session is valid but the action
    ...
    ok("no ceremony wired: it classifies stepup-required, not unauthorised", classifyError(noCeremony).kind === "stepup-required");
    sf.restore();
  ...
  // ========================================================================
  {
    ...
    // ----------------------------------------------------------------------
    // applyUpdate and rampUpdate decode their 202 through the shared parseJsonOrOwnerAction, which throws the
    // honest answer-unreadable error for any 202 body that is not the OwnerActionQueued shape. This matters at
    // LIVE apply/ramp stakes: a 202 body coerced into a false { status:"result" } would tell the owner an
    // engine deploy or traffic ramp had landed while the engine had only queued it (or moved nothing). The
    // sharpest case is a 202 body that LOOKS like a successful promote/ramp: it must throw and never resolve.
    // ----------------------------------------------------------------------
    {
      ...
      restoreB8b();
      ok("applyUpdate promote-shaped 202 never resolves (no false deploy)", applyResolved === false);
      const applyOutcome = classifyError(caught);
      ok("applyUpdate promote-shaped 202 throws answer-unreadable, status 202", applyOutcome.kind === "answer-unreadable" && applyOutcome.status === 202);
      ...
      ok("applyUpdate bare-{} 202 throws verb + status, never a false result", msgB8b.includes("apply update: 202"));
      restoreB8b();

      // applyUpdate: an UNPARSEABLE 202 (truncated JSON) throws honestly too -- the other half of malformed
      // OR unparseable, either of which a naive decode could read as a false success.
      restoreB8b = stub({ status: 202, text: '{"ownerActionQueued":true,' });
      msgB8b = await expectThrow(() => engine.applyUpdate({ dryRun: false, token: "tok" }));
      ok("applyUpdate unparseable 202 also throws verb + status", msgB8b.includes("apply update: 202"));
      restoreB8b();
      ...
      restoreB8b();
      ok("rampUpdate ramp-shaped 202 never resolves (no false ramp)", rampResolved === false);
      const rampOutcome = classifyError(caught);
      ok("rampUpdate ramp-shaped 202 throws answer-unreadable, status 202", rampOutcome.kind === "answer-unreadable" && rampOutcome.status === 202);

      // A well-formed owner-action 202 STILL resolves to queued: the guard is precise, not a blanket
      // "throw on every 202".
      ...
      ok("a WELL-FORMED owner-action 202 still resolves to queued (no over-throw)", stillQueued.status === "queued" && stillQueued.id === "oa_b8b");
      restoreB8b();
    }
  }

  // ========================================================================
  console.log("\n-- the four routes that read a 202 with plain parseJson (deleteRole / deleteGroupRole / terminateUserSessions / rotateBreakGlass) --");
    // These four routes read a 202 with plain parseJson (whose only test is `!r.ok`, and a 202 IS ok), so a
    // malformed or queued 202 must not be allowed to coerce to a FALSE success. The sharpest case is
    // deleteRole: with requireConfigApproval ON the engine answers a real 202 { queued: true }, and reading
    // that as ok would toast "Removed <member>" and drop the row from view while the member's grant is still
    // present.
  ...
  // ========================================================================
  {
    ...
    const cases: Case[] = [
      ...
      // mintSupportCredential is an OWNER-ACTION the dual-control gate defers (parseJsonOrOwnerAction): a
      // 200 resolves to { status:"result", value } and a 202 to { status:"queued" } (mints NO secret). A 202
      // must never be read as a revealable credential, because parseJson only tests `!r.ok` and a 202 is ok.
      ...
      // deleteRole is change-control GATED (parseJsonOrPending), so a 200 resolves to { status:"applied", value }
      // and a 202 to { status:"pending" } (the queued path + the malformed-202 throw are characterised above).
      ...
      // deleteGroupRole is change-control GATED (parseJsonOrPending): same applied/pending shape as deleteRole.
      ...
      // bare body: a 202 means the delete was QUEUED and nothing was written, and must never read as a success,
      // because parseJson only tests `!r.ok` and a 202 is ok. A 200 is still an applied delete.
      ...
      // rejectReason is a REQUIRED closed verdict, because a rejected restore with no reason is a dead end
      // for the requester and for support alike. Omitting it here passed only because
      ...
      // deleteNotifyChannel is change-control GATED (parseJsonOrPending): a 200 resolves to
      ...
      // deleteNotifyRule is change-control GATED (parseJsonOrPending): same applied/pending shape.
      ...
      // deleteExpiryItem is change-control GATED (parseJsonOrPending): same applied/pending shape.
  ...
  // ========================================================================
  {
    ...
    // must never be read as an outcome: an absent `outcome` would otherwise make the retry ladder re-submit it.
