...
// router.ts is a generic, screen-agnostic module, so it holds no hand-maintained ROUTES list of its
// own -- a second copy would drift from what the screens actually declare with nothing here to catch it.
// The full, derived list lives at lib/app-registry.ts (ROUTES, computed from the screens), and
// scripts/route-table-gate.mjs is the non-vacuous gate that checks it agrees with what is actually on
// disk.
