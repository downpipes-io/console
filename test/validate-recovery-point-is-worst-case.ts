...
// WHY THIS EXISTS
// ---------------
// R-100. Both recovery-point tiles read `fleet.newestGoodAt`, the freshest good backup
...
// The load-bearing case: this is the exact scenario R-100 describes, and the old implementation returned
// FIVE_MIN_AGO for it.
{
  const rows: Row[] = [
...
for (const [name, path] of [["tiles.ts", TILES], ["recovery.ts", RECOVERY]] as const) {
  ...
  if (best.length > 0) fail(`${name} sets the recovery-point value from fleet.newestGoodAt, which is the fleet's BEST case (R-100)`);
