// Validate the SHARED freshness rule (CON-H4) and the corrected observed-churn seed (CON-H14).
// Run with: node test/validate-freshness.ts
//
// Coverage:
//   CON-H4 -- Overview and Map must agree on freshness, on ONE rule that matches the engine's
...
//
//   CON-H14 -- the observed-churn seed (overview.ts observedCostSeed) must derive from actual
