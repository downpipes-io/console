...
// the byte-driven (not run-count) churn signal for the labelled future model, and the CON-L27
...
export function run(h: Harness): void {
  ...
  // Churn is still derived from the byte shape (CON-H14: bytes, not run count) because it
  ...
  // -------------------------------------------------------------------------
  // CON-L27: overview.ts observedCostSeed churnFraction clamp must be NaN-safe.
  ...
  ok("CON-L27: observedCostSeed returns non-null for >= COST_MIN_OBSERVED_RUNS runs", ovResult !== null);
  if (ovResult) {
    ok("CON-L27: overview observedCostSeed churnFraction is finite", Number.isFinite(ovResult.inputs.churnFraction));
    ok("CON-L27: overview observedCostSeed churnFraction is in [0, 1]", ovResult.inputs.churnFraction >= 0 && ovResult.inputs.churnFraction <= 1);
  }
  ...
  if (ovZero) {
    ok("CON-L27: zero-byte history churnFraction is finite (not NaN from 0/0)", Number.isFinite(ovZero.inputs.churnFraction));
    approx("CON-L27: zero-byte history churnFraction = 0", ovZero.inputs.churnFraction, 0);
  }
}
