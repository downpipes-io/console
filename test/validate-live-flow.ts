// Validate the OPTIONAL WebGL "live-flow" view's PURE parts (ENTERPRISE-UX-BLUEPRINT section
// 8.2). WebGL cannot run headless, so this targets exactly the four pure concerns the brief
...
function testFlowingAndSheen(flows: LiveFlows, model: LiveModel): void {
  ...
  // before the first readable run, and a young account's map must not look dead (the
  // walkthrough finding). The hue claims nothing the pipe does not already claim.
...
function testFeatureDetect(): void {
  ...
  ok("render mode: 2d available -> canvas2d (the only raster renderer)", chooseRenderMode({ canvas2dAvailable: true }) === "canvas2d");
...
function testTableFallbackAndModelCases(flows: LiveFlows, model: LiveModel): void {
  ...
  // (the "not centered" walkthrough finding).
...
// ---------------------------------------------------------------------------------------------
// A minimal, self-contained DOM + WebGL stub for the render-layer tests (R8). It is NOT a general
...
// renderLayerTests runs the three render-layer cases against the DOM stub, awaiting each so the deferred GL
...
// (GPU context loss/restore does not apply: a 2D context cannot be lost. The animated-loop
// liveliness is covered by the real-browser checks in the deploy workflow; the no-rAF invariant
// is pinned by case (b).)
...
// persisted opt-in) animates even when the OS prefers reduced motion (the walkthrough finding:
// a map frozen by an OS setting with no way to say "animate this one"), "reduced" forces the
