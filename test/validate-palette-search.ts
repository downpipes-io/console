// Validate the global command-palette SEARCH providers, covering runs / audit / destinations /
// credentials alongside downpipes. Run with:
//   node test/validate-palette-search.ts
//
// The palette's entity index carries four capability-gated result PROVIDERS alongside the
// downpipes-by-name + run-id index, each reusing the SAME fuzzy matcher and navigate() route (no
// parallel search UI). This validator proves, for EACH provider:
