...
// The caller being non-null is not enough; whoamiAvailable drives the outer branch.
{
  ...
  // Negative control: the verified path requires BOTH conditions; a caller alone
...
// non-verified state. This is the disqualifying-if-shipped rule in runtime form.
...
ok(
  "negative control: 'unknown' is neutral in chip, not trust or warn",
  _testChipTone({ state: "unknown" }) === "neutral",
);
ok(
  "negative control: 'unknown' is neutral in panel, not trust or warn",
