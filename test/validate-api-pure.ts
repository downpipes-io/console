...
// a GUARDED dynamic import rather than a static one. This file is reached from validate-api.ts,
// which sits in the repo-local validate chain, and a static cross-repo import made that whole chain
// unrunnable on a single-repo checkout. The two siblings that already cross-check the engine
// (validate-plan-hash.ts:355-375 and validate-identity.ts:243-251) use exactly this pattern, down to
// the ENGINE_WORKTREE override, so a multi-repo pipeline can point at the engine root while a
// console-only runner skips the parity block with a visible note instead of dying at module load.
// Engine resolution lives in test/engine-path.ts, which explains why: the relative hop this used to do
// lands on console/.worktrees/engine from a worktree, and in this workspace that is a real engine checkout
// on an unrelated branch, so this parity block passed against whichever engine sat in that slot.
...
async function runPlanHashParity(h: Harness): Promise<void> {
  ...
  if (engineRestorePlanHash === null) {
    console.log("  note engine worktree approvals.ts not importable; skipping the plan-hash parity cross-check");
    console.log("  note set ENGINE_WORKTREE, or run npm run validate:workspace with the engine checked out, to exercise it");
    return;
  }
  ...
  //
  // THE SURFACE SET IS PART OF THE BINDING, and that is what this block is really pinning. Widening
  ...
  //
  // Both directions are asserted, because a mirror that ignores the surface set passes any
  // test that only ever supplies one value for it, and drop the very case that leaves an operator
  // viewing a hash the approval did not bind to.
  {
    ...
    // EXPLICIT-UNDEFINED PARITY, which is what this actually tests. There is no token in cfConfig at all: the
    // media edit token lives on mediaRestore and helpers.ts already documents that only its accountId is
    // bound. What is being pinned here is the
