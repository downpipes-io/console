// Validate the three engine admin surfaces the surface/cell census found the console did not reach
// (harness scripts/surface-cell-gap.mjs, findings G-P3-001..003). Each is a different resolution, so each
// gets a different kind of assertion here.
//
//   G-P3-001  GET /admin/keys/vintages had no console caller at all: `grep -r vintages console/src` was
//             empty while the engine served the keyless vintage inventory built as the SURFACING half of
//             G-P0-098 / G-P0-099. The console now reads it on the Keys screen's Rotate tab. Asserted here:
//             the client hits the right route and unwraps { inventory }; the verdict is computed by the
//             honesty rules rather than by counting alone (an unread history outranks a zero count); and the
//             panel renders the stranded vintages, never a fabricated all-clear when the read failed.
//
//   G-P3-002  POST /admin/policy/require-access was built expressly for the console to consult before it
//             advises disabling the shared admin token, and the console never asked: it stated instead that
//             the posture was one it "cannot read or set", and printed `wrangler secret delete ADMIN_TOKEN`
//             unconditionally. That is the exact lock-out the engine's AUTH-1 pre-flight exists to refuse.
//             Asserted here: the closing command appears ONLY when the engine says safeToDisableToken, the
//             engine's own lockoutWarning is surfaced verbatim when it does not, and a pre-flight that
//             cannot be read withholds the command rather than falling through to it.
//
//   G-P3-003  the legacy single SRE-alert webhook policy had console client methods and no screen. Alert
//             delivery moved to notify channels and rules, so those were client methods for a screen that
//             had been deliberately replaced; they are removed rather than left as a shim. Asserted here
//             from SOURCE: no console module targets /admin/policy/webhook and EngineClient exposes no
//             webhook-policy method, so the dead half cannot come back unnoticed. The ENGINE half went in
//             P3.2b, so the three routes no longer exist; the assertions below now also hold the mirror
...
// ONLY by the identity.key the owner must still hold. This is the state G-P0-098 said had no in-product view.
...
// ---- G-P3-001: the client reaches GET /admin/keys/vintages ------------------------------------------------

async function clientReadsVintages(): Promise<void> {
  console.log("\nG-P3-001 client: GET /admin/keys/vintages");
...
// ---- G-P3-001: the verdict rules -------------------------------------------------------------------------

function vintageVerdictRules(): void {
  console.log("\nG-P3-001 verdict: the honesty rules outrank the counts");
...
async function vintagePanelRenders(): Promise<void> {
  console.log("\nG-P3-001 panel: the Rotate tab can answer 'which key opens which vintage'");
  const inv = rotatedInventory();
  let restore = stubFetch(() => jsonResponse({ inventory: inv }));
  const engine = connect("https://engine.example.com") as EngineClient;
  ...
  ok("it reports signer continuity alongside recipient stranding (G-P0-099)", /Signature check/.test(text));
  panel.remove();
  restore();
...
// ---- G-P3-002: the token-disable advice is gated on the engine's verdict ---------------------------------

const CLOSE_COMMAND = "wrangler secret delete ADMIN_TOKEN";

function requireAccessGating(): void {
  console.log("\nG-P3-002 advice: the closing command is gated on the engine's pre-flight");
...
async function requireAccessFailsSafe(): Promise<void> {
  console.log("\nG-P3-002 fail-safe: an unread verdict is not a permission");
...
// ---- G-P3-003: the removed half stays removed -------------------------------------------------------------
...
function webhookPolicyHalfStaysRemoved(): void {
  console.log("\nG-P3-003 removal: no console path targets the legacy webhook policy");
  ...
  // P3.2b: the MIRROR types that shadowed the removed engine surface must go with it, or the console keeps
