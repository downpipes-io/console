// Validate the console's bulk-loop resilience layer:
//
//   src/lib/bulk-pacing.ts   - 429 recognition, Retry-After -> capped backoff, honest 429 labelling, sleep.
//   src/screens/overview/fleet-drill.ts - the drill outcomes carry a REASON per failure: the
//                                          summary names every failed id and the per-item modal shape
//                                          carries each reason (no more "3 named + N more" with no cause).
//   src/components/slow-note.ts - scheduleSlowNote paints a reassurance line after a threshold and cancels
//                                  cleanly so a fast call never flashes it.
//
// The slow-note section creates DOM, so the DOM shim is installed first (the same pattern the
// run-incomplete / map-panels validators use). The bulk-pacing maths and the fleet-drill helpers are
// pure and need no DOM. Every section has a negative control that would fail vacuously if wrong.
...
// ---- isRateLimited: the hot-path predicate the loops branch on ---------
...
// ---- bulkFailureReason: a 429 is labelled DISTINCTLY, not "did not pass" ---
...
const manyFailures: DrillOutcomes = {
  // This counts targets that gave up STILL rate limited. Not what this case is about, so none did.
  passed: 5,
...
ok(
  "drillSummary never reads as 'N of N passed' when some were only deferred, not verified (negative control)",
  !summaryDeferred.includes("of 5 passed") && !summaryDeferred.includes("of 3 passed"),
);

// A fully break-glass-only fleet (0 passed, all deferred) must
