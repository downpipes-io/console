...
function main(): void {
  ...
  console.log("-- destinationLaneStatus: per-edge status under failover --");
  {
    ...
    // THE LANE THE TICKET IS ABOUT, AND THE LANE THAT CRIED WOLF.
    //
    ...
    // Painting them the same lane is a bug either way round. Reading DARK as "partial" told operators to wait
    // for a copy nobody was making; reading NEW as "no-copy" put the second-worst status in the fleet
    // (panels.ts STATUS_RANK ranks it 1 of 7, worse than stale AND partial) on a healthy configuration.
    //
    // The ANCHOR is the discriminator, and it is the SAME one the support pack and the bot gate on: sealedRuns
    // (the downpipe's monotone count of successful runs) minus the destination's anchor (that count when it
    // entered the fan-out) = how many backups have succeeded and still produced no copy. Under two: catching up.
    ...
    ok("THE CRY-WOLF: NEW (added minutes ago to a running downpipe) -> partial (catching up), NOT no-copy",
      destinationLaneStatus("healthy", "d", repl, "r4", NEW) === "partial");
    ok("THE TWO STATES DO NOT PAINT THE SAME LANE (the whole fix)",
      destinationLaneStatus("healthy", "d", repl, "r4", DARK) !== destinationLaneStatus("healthy", "d", repl, "r4", NEW));
    ok("no-copy and partial are DIFFERENT lanes on the same downpipe",
    ...
    ok("HONEST ABSENCE: an UNANCHORED destination (a legacy record) keeps the milder lane, never a guessed fault",
      destinationLaneStatus("healthy", "d", repl, "r4", { sealedRuns: 90, anchors: { other: 0 } }) === "partial");
    ok("HONEST ABSENCE: NO anchor at all (an engine that does not send one) keeps the milder lane",
