...
//
// CON-H1: the printed restore command must include --apply in both the plaintext
//         rendering (recoverySheet) and the HTML rendering (recoverySheetHTML).
//         Without --apply the command is a dry run and writes nothing, so an operator
//         following the sheet verbatim would silently fail to recover their data.
//
// CON-H3: a record name containing HTML metacharacters (<, >, &, ") must be escaped
//         in the HTML rendering. The no-custody + HTML-escape guarantee documented in
//         the recoverySheetHTML comment is verified here so a careless edit cannot
...
// ---------------------------------------------------------------------------
console.log("\n-- CON-H1: --apply flag in restore command --");

// The COMMAND line, not any prose that mentions the command. 0aabdef added a warning sentence
...
// rendered through headless Chrome the sheet measured a 30.5pt top margin, about 10.5mm, on a 612x792
...
// ---------------------------------------------------------------------------
console.log("\n-- CON-H3: HTML-escape of metacharacters in HTML rendering --");
