...
//       NEVER a bare string) to clean non-empty lines, without throwing (an earlier version assumed each
//       element was a string and called .replace on it, which threw the moment a real engine's
//       object-shaped diff reached the checker's inbox, so it rendered no card at all)
...
async function testPureHelpers(): Promise<void> {
  ...
  // ------------------------------------------------------------------------
  {
    // The engine's real PendingChangeLine[] is an ARRAY OF OBJECTS { kind, area, text }, never a bare
    // string. An earlier diffLines assumed each element was a string and called String.prototype.replace
    // on it directly, which threw "x.replace is not a function" the moment a real engine's diff reached
    // it, so the checker's config-approvals inbox rendered no card.
    ...
    // These are the engine's own dash-form wire values (admin/change-control.ts's ConfigChangeKind),
    // confirmed live against a real harness-crud engine -- a proposed notify-channel-set queues as
    // kind:"notify-channel-set" verbatim, and the console's dot-form map must have an entry that string can
    // match, or every real pending change renders its raw engine kind string on the approvals card. All 18
    // kinds are asserted here (not just the live-confirmed one), so a future rename on either side is caught
    // by this test rather than by a live approval card again.
...
async function test202Handling(): Promise<void> {
  ...
  // ------------------------------------------------------------------------
  {
    // THE FIXTURE IS THE ENGINE'S OWN 202, not one this test invented. The engine has exactly one gated-config
    // 202 (src/sched/scheduler-do.ts):
    //
    //     return this.jsonStatus({ queued: true, id: p.id, status: p.status, contentHash: p.contentHash }, 202);
    //
    // Asserting `isPendingChangeBody({ pending: true, id })` would pass against a body NO ENGINE ROUTE EMITS,
    // because that shape is not one the engine can produce. The console would then read every REAL 202 as an
    // applied change: the operator sees "Saved" for a mutation that is queued and has not taken effect. A
    // contract fixture may only assert a row the product can actually produce.
    ...
    // A 202 whose body does NOT carry the pending shape (governance-lie, confirmed live by the write-fault
    // capability's fulfilMalformed202 probe on addDownpipe) must throw an honest error, never resolve to a
    // fabricated "applied" value. `{}` is the exact body the live probe uses; resolving it to
    // { status:"applied", value:{} } would show a success toast while the engine's own state never moves.
    {
    ...
    // The other half of "malformed OR unparseable": a 202 body that is not even valid JSON (truncated)
    // must ALSO throw honestly, never resolve. addDownpipe shares parseJsonOrPending with every other
    // config mutation (setRole, upsertNotifyChannel, acceptPostureRisk, ...).
    {
      const g = globalThis as unknown as Record<string, unknown>;
    ...
    // ARM path, and a DISARM with no second owner to queue for) resolves to status:"result", as an
    // OwnerActionResult rather than a bare ConfigApprovalPolicy, so a queued disarm is distinguishable
    // from an applied one -- see the "disarm queues" case below.
    {
    ...
    // undefined (which the caller would read as false -- "the gate turned off" -- while it is, correctly,
    // still on).
    {
...
async function testInboxScreen(): Promise<void> {
...

  {
    ...
    // engine's real PendingChangeLine[] object shape: rendering it end to end through the actual screen
    // (not just the pure diffLines helper) proves the real-screen render path does not crash on it.
    ...
    // The own-proposal cue is a one-line hint rather than a banner, so multiple own proposals do not
    // stack a banner per card; the assertion matches the copy's stable lead.
...

  {
    ...
    // The own-proposal cue is a single hint line ("You proposed this; awaiting a different approver
...
async function testSecurityCentreToggle(): Promise<void> {
...

  {
    ...
    // Disabled-with-reason, not the native `disabled` attribute: aria-disabled, so the switch stays
    // reachable in the tab order and announces why.
