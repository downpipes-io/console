// TC-07: validate the pure helpers in src/api.ts and the client/server plan-hash parity.
// Run with: node test/validate-api.ts
//
// This file was split (finding console-test-002-06) from a single 1700+ line module into
...
//                                 screen flows (CON-H7 blast-radius cues, restorability blind
//                                 test, CON-L5 onboarding writes) under the shared DOM shim
...
async function main(): Promise<void> {
  ...
  verdictReached(h.failures); // the verdict is now DECLARED, so a silent exit 0 cannot pass as green
