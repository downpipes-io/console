// R-28: the same unguarded-response crash shape R-26 fixed on the recovery-codes panel, on the
...
//      benignWrite fallback, the same reason R-26 hardened recoveryCodesPanel rather than only the one
...
async function testDemoRouteModelsAttestSession(): Promise<void> {
  ...
  ok("R-28 FIX: the body is a USABLE CreateAttestSessionResult (the reported gap was the benign\n" +
...
// =====================================================================================
// SECTION 2: the type guards -- the exact predicate the R-28 fix checks, proven against the exact
...
function testGuardsRejectTheBenignShape(): void {
  ...
  ok("R-28: isCreateAttestSessionResult(benignWrite shape) is false (no `runs` field)", !isCreateAttestSessionResult(BENIGN_WRITE_SHAPE));
  ...
  ok("R-28: isAttestSessionRecord(undefined) is false (benignGet has no `session` field at all)", !isAttestSessionRecord((BENIGN_GET_SHAPE as { session?: unknown }).session));
  ok("isAttestSessionRecord(a real record) is true", isAttestSessionRecord({ sessionId: "s1" } satisfies AttestSessionRecord));

  console.log("\n-- the exact R-28 reproduction: session.runs.length on the benignWrite shape throws (was: the live crash) --");
...
// Start button -- the exact R-28 reproduction end-to-end, against a stub engine answering the malformed
// shape directly (bypassing client-attest.ts entirely, so this proves the SCREEN does not trust its
// caller, the same reason R-26 did not trust recoveryCodesPanel's caller).
// =====================================================================================
async function testStartDegradesHonestly(): Promise<void> {
  console.log("\n-- restore-flow/attend.ts start(): a malformed engine response degrades to an honest\n" +
    "   outcome, never an unhandled rejection or session.runs.length throwing (R-28 end-to-end) --");
  ...
  ok("R-28 FIX: no unhandled rejection from Start (was: TypeError from session.runs.length)", unhandled === null);
...
async function testResumeDegradesHonestly(): Promise<void> {
  console.log("\n-- restore-flow/attend.ts resumeSession(): the same shape on the STATUS read (R-28, the\n" +
  ...
  ok("R-28 FIX: no unhandled rejection from resuming (was: TypeError from record.runs on an undefined record)", unhandled === null);
  const bodyText = textOf(root);
...
// =====================================================================================
// SECTION 5: client-attest.ts's OWN wire-boundary guards (R-28 layer 2), driven through the REAL
...
async function testClientAttestOwnWireBoundaryGuards(): Promise<void> {
  console.log("\n-- client-attest.ts: createAttestSession/attestStatus validate the SAME shape client-attest.ts\n" +
    "   itself now guards (R-28 layer 2), through the REAL Transport with a stubbed fetch --");
  ...
  try {
    ...
    ok("createAttestSession: R-28 -- the benignWrite 200 throws AT THE WIRE BOUNDARY", createThrew instanceof Error);
    ...
    ok("attestStatus: R-28 -- a session-less 200 throws AT THE WIRE BOUNDARY", statusThrew instanceof Error);
