...
import {
  ...
  metricsEndpointNote,
  downloadJsonText,
  pullFreshnessSentence,
  gapServedBannerMessage,
...
// ---------------------------------------------------------------------------
// pullFreshnessSentence: the plain relative-time line beside supportGrantPresentation's pullLine.
// ---------------------------------------------------------------------------
console.log("\n-- pullFreshnessSentence: plain relative freshness --");

const NOW = Date.parse("2026-06-03T00:10:00.000Z");
eq("no pulls -> Never pulled yet.", pullFreshnessSentence([], NOW), "Never pulled yet.");
eq("a pull seconds ago reads 'less than a minute ago'", pullFreshnessSentence([{ at: "2026-06-03T00:09:50.000Z" }], NOW), "Last polled less than a minute ago.");
eq("a pull 4 minutes ago", pullFreshnessSentence([{ at: "2026-06-03T00:06:00.000Z" }], NOW), "Last polled 4 minutes ago.");
eq("a pull exactly 1 minute ago reads singular", pullFreshnessSentence([{ at: "2026-06-03T00:09:00.000Z" }], NOW), "Last polled 1 minute ago.");
eq("a pull 2 hours ago", pullFreshnessSentence([{ at: "2026-06-02T22:10:00.000Z" }], NOW), "Last polled 2 hours ago.");
eq("a pull exactly 1 hour ago reads singular", pullFreshnessSentence([{ at: "2026-06-02T23:10:00.000Z" }], NOW), "Last polled 1 hour ago.");
eq("a pull 3 days ago", pullFreshnessSentence([{ at: "2026-05-31T00:10:00.000Z" }], NOW), "Last polled 3 days ago.");
eq("the most recent of several pulls wins, order-independent", pullFreshnessSentence([{ at: "2026-06-01T00:10:00.000Z" }, { at: "2026-06-03T00:06:00.000Z" }], NOW), "Last polled 4 minutes ago.");
ok("an unreadable-but-present pull names the anomaly, never 'Never pulled yet'", pullFreshnessSentence([{ at: "junk" }], NOW) === "Pulled at least once; the last recorded time cannot be read.");

// ---------------------------------------------------------------------------
// gapServedBannerMessage: the plain, honest gap sentence (plan section 5, "Gap or backfill after
// downtime"). Static and closed; asserted so the wording cannot silently drift.
// ---------------------------------------------------------------------------
console.log("\n-- gapServedBannerMessage --");

ok("names the SIEM impact plainly", gapServedBannerMessage().includes("may not have reached your SIEM"));
ok("states the gap cannot be recovered, only detected", gapServedBannerMessage().includes("cannot be recovered"));
...

{
  ...
  // metrics (monitoring integrations): the third scope's note follows the same
...
// Prometheus scrape endpoint URL (monitoring integrations).
// ---------------------------------------------------------------------------
console.log("\n-- metricsEndpointNote --");
...
// coarse error classes is read into the pack and then CLEARED. If clearing happened before the file was
// written, through a delivery that could throw, a browser that declined the blob write would leave the
// customer with no pack and an emptied ring, and a retry would rebuild a pack carrying no console error
// classes at all, while the line under the button went on telling them the bundle includes them.
//
// This returns a boolean and never throws, and the caller clears the ring only on true.
