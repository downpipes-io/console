...
// skeleton: nine stat tiles to none, measured in a real Chromium on the served bundle. The repair's own
...
//
// THE PROOF THIS FILE IS THE UNIT HALF OF. The browser half lives in the harness
// and drives the committed public/ bundle in a real
// Chromium against a same-origin fixture engine. Three defects were planted back through that driver,
// each rebuilt into the served bundle: restoring the unguarded dereference, hiding the could-not-render
// card, and dropping the unreadable row. Each turned its arm red; an inert edit of the same shape at the
// same site kept every arm green; the tree was restored byte-identical each time. This file is what
// keeps the same properties from regressing without a browser in the loop.
//
...
// The wall clock, NOT a pinned date, and the difference broke this file once: it pinned
// , the healthy rows' proven/verified stamps sit hours before NOW, and buildOverview
// reads the REAL Date.now(), so six days after landing the "healthy" fixture read as stale, the
// coverage line qualified itself, and the unqualified-all-clear check went red with nothing wrong.
// The same fixture-clock fault class validate-tour.ts documents: a pinned world the screen's own
// clock has moved past is a fixture no estate ever resembles, and re-pinning forward only defers it.
