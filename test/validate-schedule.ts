...
console.log("\n-- schedule: validateSchedule mirrors the engine's tz + blackout bounds --");
{
  ...
  // B47: an equal start and end covers no time (a fat-finger of the same time in both boxes), which the engine
  // rejects; the console now mirrors that reject inline instead of drawing a late engine 400.
  ok("B47: an equal-start-end blackout window is rejected (mirrors the engine's equal-start-end reject)", validateSchedule({ blackoutWindows: [{ startMinute: 120, endMinute: 120 }] }) !== null);
  ok("B47: a start<end window is still accepted (the reject is only for equal times)", validateSchedule({ blackoutWindows: [{ startMinute: 120, endMinute: 180 }] }) === null);
