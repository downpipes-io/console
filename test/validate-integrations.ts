...
// block below drives the REAL form, so it needs a real Owner caller rather than a stub of the gate.
const { setCaller } = await import("../src/lib/caller-state.ts");
// The focus-return block below drives the WHOLE Integrations screen (the coordinator that owns open/close and therefore
...
async function main(): Promise<void> {
  ...
  // The pair the credential-scheme rule exists for: two NAMED vendors on one wire, with different credential shapes.
  ...
  // Splunk and CrowdStrike Falcon Next-Gen SIEM both take splunk-hec over http, so tile identity cannot be
  // `format === v.pushFormat && sink === v.pushSink`: that would let a single live Splunk push render BOTH tiles
  // Active and BOTH panels claim the same config as their own. The config records WHICH vendor it is instead.
  ...
  console.log("-- WCAG 2.4.6: no two tiles announce themselves identically --");
  ...
  // The pair this is measured on, named rather than left to the count: both Datadogs are "available,
  ...
  // "Add" and aria "add" even on owner-only tiles the setup panel then refuses. Model the operator gate
  ...
  // Negative control: with the all-true predicate, the SAME owner-only tile reads add-able,
  ...
  ok("negative control: an all-true grid still prints the owner-only tile as add-able", (qradarAllTrue?.getAttribute("aria-label") ?? "").endsWith("add"));

  console.log("-- REFUSAL HONESTY: an UNREAD role is not a role refusal --");
  // canDo/canCap fail closed on an unresolved caller, which is right for a gate and wrong for a sentence.
  // app.ts paints the screen and resolves the identity in parallel, so on every fresh load of /integrations
  // the gate can run with caller() null. Without care the grid would print that default as the settled fact
  // "Owner" on all 40 tiles at once, with the accessible action "owner only", even to an operator who is in
  // fact the owner, until whoami resolves and the tiles turn to "Add".
  //
  ...
  console.log("-- Sentinel Content Hub tile: docSlug, panel link scoping, freshness line, gap banner --");
  const sentinelVendor = find("Microsoft Sentinel");
  ok("Sentinel row carries an explicit docSlug", sentinelVendor.docSlug === "microsoft-sentinel");
  ok("Sentinel docsUrl resolves under /integrations/microsoft-sentinel", docsUrl(sentinelVendor) === "https://docs.downpipes.io/integrations/microsoft-sentinel");
  ok("the pull panel shows the Content Hub link on the Sentinel tile", (pullBody.textContent ?? "").includes("View in Microsoft Sentinel Content Hub"));
  ok("the ARM-template method line stays as the alternative, unchanged", (pullBody.textContent ?? "").includes("Codeless connector: run the ARM template once"));
  const exabeamBody = renderSetupBody(stubEngine, find("Exabeam"), "refused", EMPTY, () => {});
  const criblBody = renderSetupBody(stubEngine, find("Cribl"), "refused", EMPTY, () => {});
  ok("SCOPING: Exabeam (another pull vendor) does NOT show the Content Hub link", !(exabeamBody.textContent ?? "").includes("Microsoft Sentinel Content Hub"));
  ok("SCOPING: Cribl (another pull vendor) does NOT show the Content Hub link", !(criblBody.textContent ?? "").includes("Microsoft Sentinel Content Hub"));
  ok("SCOPING: every other pull vendor still shows its own method line unchanged", (exabeamBody.textContent ?? "").includes("Exabeam pulls the audit feed") && (criblBody.textContent ?? "").includes("Cribl pulls the audit feed"));

  // REACHED: the freshness line and the gap banner, through the real render path (renderSetupBody ->
  // renderPullCredentials -> scopeBlock), not only asserted of a pure return value.
  const gapGrant = { clientId: "dpc_sentinel", scope: "audit-feed" as const, grantedAt: "2026-06-01T00:00:00.000Z", grantedBy: "owner@example.com", expiresAt: "2026-12-01T00:00:00.000Z", expired: false, pulls: [{ at: new Date(Date.now() - 4 * 60_000).toISOString() }], gapServed: true };
  const gapEngine = {
    ...stubEngine,
    getSupport: async () => ({ vendorSealConfigured: false, signerConfigured: false, diagnostics: null, auditFeed: gapGrant, metrics: null }),
  } as unknown as Parameters<typeof renderSetupBody>[0];
  const gapBody = renderSetupBody(gapEngine, sentinelVendor, "allowed", EMPTY, () => {});
  document.body.appendChild(gapBody as never);
  await new Promise((r) => setTimeout(r, 0));
  const gapText = gapBody.textContent ?? "";
  ok("REACHED: the gap-served banner renders on a live gapServed grant", gapText.includes("may not have reached your SIEM"));
  ok("REACHED: the freshness line renders alongside the existing pull-count line", /Last polled \d+ minutes? ago\./.test(gapText));
  gapBody.remove();

  const noGapGrant = { ...gapGrant, gapServed: false, pulls: [] };
  const noGapEngine = {
    ...stubEngine,
    getSupport: async () => ({ vendorSealConfigured: false, signerConfigured: false, diagnostics: null, auditFeed: noGapGrant, metrics: null }),
  } as unknown as Parameters<typeof renderSetupBody>[0];
  const noGapBody = renderSetupBody(noGapEngine, sentinelVendor, "allowed", EMPTY, () => {});
  document.body.appendChild(noGapBody as never);
  await new Promise((r) => setTimeout(r, 0));
  const noGapText = noGapBody.textContent ?? "";
  ok("DISCRIMINATOR: no gap-served banner when gapServed is false", !noGapText.includes("may not have reached your SIEM"));
  ok("DISCRIMINATOR: 'Never pulled yet.' when the trail is empty", noGapText.includes("Never pulled yet."));
  noGapBody.remove();
  ...
  // The enforcement, at the field, on blur: a bare HEC token is REFUSED on the Splunk form, beyond the field
  // carrying only `required: true`.
  ...
  // Deleting a downpipe leaves the notify rules scoped to it in place, matching nothing forever.
  // Notifications says so at the rule's grain;
  ...
  console.log("-- WCAG 2.4.3: closing a detail panel returns focus to the tile it was opened from --");
  ...
  // The OTHER half of the pair, asserted so this cell also fails if someone deletes the open half's focus
  // move. Focus must be IN the panel after opening.
  ok("opening moves focus into the detail panel (the half that already exists)", activeElement() === (detailPanelEl as unknown));
  ...
  // The symptom this refuses by name: focus dropped on <body> loses the operator's place on the page.
