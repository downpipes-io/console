...
// the same way validate-break-glass-restore.ts does. Setting .value on a file input does nothing, which
// is what the first version of this test got wrong: the reconstruct control stayed disabled and the
// failure looked like the card refusing a good quorum rather than the test never delivering the envelope.
class ShimFile {
  _text: string;
  name: string;
  type = "text/plain";
