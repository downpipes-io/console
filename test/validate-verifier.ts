// The enforcement verifier is the trust-closing step, so its
// verdict mapping and error-path branching are correctness-critical for the security display. Three
// pieces carry that logic and nothing else in the suite exercises them:
...
// mapping function the screen does not use: the screen must not hold a PRIVATE copy of the mapping that no
// test drives, one that could call a live OIDC, SAML or recovery session the shared break-glass token.
// verifierErrorTile and roleBasisNote render DOM, so they run under the shared DOM shim with a
// fake engine; the test never re-implements them.
...
function main(): void {
  ...
  // The four states distinct from the shared-token claim, on the one screen whose job is the posture.
  ...
  // DISCRIMINATION: the seven methods yield six distinct verdicts (oidc and saml share idp-verified, and the
  // panel separates them by protocol), rather than collapsing four of them onto the worst one.
  ok(
