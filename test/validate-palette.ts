// Validate the command-palette actionRoute routing contract (CON-H9).
// Run with: node test/validate-palette.ts
//
// Coverage:
//   actionRoute correctness (CON-H9 regression + full contract):
...
// ---------------------------------------------------------------------------
console.log("\n-- CON-H9 regression: misrouted palette actions --");
...
const nonOverviewTargets: Array<{ target: string; wantRoute: string }> = [
  ...
  { target: "settings.alert-webhook",  wantRoute: "/settings" }, // CON-H9
  { target: "licence.view-provenance", wantRoute: "/licence"  }, // CON-H9
];
...
// ---------------------------------------------------------------------------
// CON-L13 regression: no duplicate "Open recovery sheet" commands.
//
...
// ---------------------------------------------------------------------------
console.log("\n-- CON-L13 regression: no duplicate recovery-sheet commands --");
...
// ---------------------------------------------------------------------------
// CON-L11 regression: a resolved RUN entity routes to /runs/:downpipeId/:index
// with no preselection query string.
//
...
// ---------------------------------------------------------------------------
console.log("\n-- CON-L11 regression: resolved run route is /runs/:downpipeId/:index --");
