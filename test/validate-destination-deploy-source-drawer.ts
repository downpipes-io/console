// Once a second destination joins a deploy-time-bound one, the engine registers the deploy-time
// binding as a real, permanent destination (source:"deploy", engine's
// ensureDeployDestSeeded/DEPLOY_DEST_ID) rather than silently reassigning its runs to whichever
// destination becomes the new default. That row appears in the console's multi-destination LIST
// like any other, and this drives what the drawer must get right about it:
//
...
//     3-2-1 removal guard as any other destination: it is added, never replaced.
//
