...
function buildCtx(
  ...
  const ctx: SubmitContext = {
    engine,
    // discoveryRead/downpipesRead say whether each read SUCCEEDED; an absent value reads as failed and
    // drives the DEGRADED path instead (the form renders "could not be read" notes and the circular-backup
    // safety check silently does not run). This fixture means a read that worked and found no buckets, a
    // different fact, so both flags are set true explicitly.
    ...
    // The R2 field group is held whole (SubmitContext.r2Block) because it has live state: which
...
async function main(): Promise<void> {
  ...
  // ------------------------------------------------------------------------
  console.log("\n-- Retention line: render states over planted statuses (real renderConfigured) --");
  // ------------------------------------------------------------------------
  ...
  // so the levers render disabled.
  {
    ...
    // THE AUTHENTICATION ROW: renderPostureRows renders it only for a recognised authMode, so an Azure
    // destination carrying a Microsoft Entra service principal must answer authMode "entra", not "keys",
    // or the card says nothing at all: a customer could not tell from the console that the destination
    // signs in against Microsoft's identity platform rather than with a stored account key. This is the
    // cell that keeps the row from going missing for Entra.
