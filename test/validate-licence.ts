...
import {
  ...
  // Multi-component updates: the pure component logic + copy, and the persistence-first settle
...
// The console's own version identity + the lazy-chunk preloader: lib modules, imported directly.
...
// The update-UX copy, the bridge from a recorded outcome to that copy, and the injectable-timer retry
// engine are all imported DIRECTLY from their own leaf modules (the established mixed-import style this
// file already uses for update-apply-flow.ts / update-components-advanced.ts, rather than routing
// everything through the screens/licence.ts barrel).
import {
  STAGE_CHECKING,
...
// ZERO_HOOKS is EVERY 0.1.5 timing knob zeroed (the update flow's own invariant: all timers are injectable
// so validators can zero them), spread into every DOM-flow test below so the grace period + retry ladder +
// status backoff + stall-poll interval all resolve on the SAME tick instead of the real ~60-90s budget. A settle
...
// Every LIVE tier, not a sample: a list that names a retired tier vocabulary (starter/growth/a bare
// "business") is stale, since the pricing rename deleted those ids outright. The current live set is
// community/business-1/business-3/business-10/business-25/
...
// website's contact CTA, sales@downpipes.io). It must NOT point at the retired provii.au domain.
...
ok("ENTERPRISE_CONTACT does not point at the retired provii.au domain", !ENTERPRISE_CONTACT.includes("provii.au"));
...
if (routerSrc === null || licenceSrc === null) {
  ...
  if (process.env.REQUIRE_ENGINE === "1") {
    // A REFUSAL, NOT A FINDING. Exiting 1 here would print a FAIL line asserting a divergence this run
    // never established: the refusal bodies this block composes are pulled out of the engine's own
    // router-updates.ts and licence.ts, so with no engine there is no expected body and nothing was
    // compared. verdictCannotCheck classifies that correctly, matching the other could-not-check in this
    // file below.
    verdictCannotCheck(
      "validate-licence: REFUSED, REQUIRE_ENGINE=1 and no engine checkout is reachable.\n" +
...
// 5a/5a'. THE SKIP ABOVE MUST ACTUALLY REACH THIS FAR: the bodies below are
...
// cannot end in ")." and the two parenthetical-hint assertions would FAIL on any engine-less runner if the
// skip above did not actually stop this section from running. The Validators CI job checks out console
// alone, so if this section ran there it would read red on two assertions about the engine's wording that
// no console change could ever satisfy.
//
...
// ===========================================================================
// 11. RICH RELEASE METADATA -- risk class, compat verdict, changelog grouping
// ===========================================================================
...
console.log("\n-- rich release metadata (risk class / compat / changelog) --");
...
// ===========================================================================
// The ramp is TWO-PHASE (mirroring apply's promote/settle): phase 1 (POST /update/ramp) shifts
...
}
{
  ...
  // THE FOURTH OUTCOME. This engine emits "rollback-failed-still-split" and the outcome line must handle
...
// ===========================================================================
// 17. CONSOLE VERSION IDENTITY -- consoleVersion / fetchServedConsoleVersion / pollForServedVersion
// ===========================================================================
...
// ===========================================================================
// 19. RELEASE APPLY SET + CONTROL STATE -- releaseComponentsToApply / consoleUpdateAvailable
// ===========================================================================
...
// consoleWasApplied: evidence-driven, never assumed.
{
  ...
  // branch was unreachable against real engine output (live-confirmed against a real console-only apply).
...
// EXACT GET /admin/update/status shape recorded live for a console-only apply: the engine's own `last` (the
...
// token, the section reloads, and neither the preloader nor the served-version check ever fires.
{
  ...
  eq("the exact terminal sentence rendered, byte-for-byte", progressLine(env.out), appliedTerminalLine("0.2.0", { confirmationPending: false }));
}
...
// the section does NOT reload underneath it.
{
  ...
  eq("the exact terminal sentence rendered, WITH the reloading suffix", progressLine(env.out), appliedTerminalLine("0.2.0", { confirmationPending: false, consoleApplied: true }));
...
// (live-confirmed against a real console-only apply). Fixed on both
...
// reload prompt renders -- replaying, end to end through the real runLiveApplyFlow, the exact console-only
// apply recorded live (engine
// 0.3.0 unchanged, console 0.2.0 -> 0.2.1, apply outcome "applied"). THE DEFECT THIS PINS: the harness's
// status-region poller (and the real console under it) never recognised this exact response as terminal --
// the region stuck on "An unexpected outcome was returned; the engine state is unclear" for the whole poll
// window while GET /admin/update/status already showed `lastConsole` applied. Section 22a above pins the
// re-read (recordedSettleOutcome's `component` parameter reading lastConsole); this pins the LIVE response
// path (runLiveApplyFlow's own !includesEngine branch, consoleOnlyApplyOutcome), which is the one that
// actually rendered the defect during the apply itself, before any re-read ever happened.
{
  const calls = newCalls();
  const engine = flowEngine(calls, {
...
// "still live / hourly canary" claim; the section then reloads.
{
  ...
  eq(
    "the exact terminal sentence rendered (the SHORT rolled-back form, not the verbose Details-only line)",
...
// renders and the section reloads (the required second addendum case).
{
  ...
  // in this flow's output: the terminal progress line, the interim "Deployed 0.2.0. Verifying it now"
  ...
  eq("the section reloaded", counters.reloads, 1);
  eq("the exact terminal sentence rendered", progressLine(env.out), appliedTerminalLine("0.2.0", { confirmationPending: false }));
}
...
// never mapped onto a guess.
{
  ...
  eq("the exact terminal sentence rendered", progressLine(env.out), rolledBackTerminalLine("gate could not confirm"));
}

// 24h. RETRY-BUDGET EXHAUSTION (the stall state): settle AND every status re-read fail throughout
// the WHOLE budget -- the honest STALL line renders (byte-for-byte, not the old bare "unknown" prose), no
// outcome is claimed, and the section is NOT reloaded (nothing concluded yet: no user action
...
// background poll correctly does not fire at all -- proven separately (never a false positive) in 24k below.
{
  ...
  eq("the exact STALL sentence rendered, byte-for-byte", progressLine(env.out), STALL_LINE);
...
// 24j. SETTLE TRANSPORT FAILURE x2 THEN SUCCESS (the fix for root cause #1: "every deploy in
...
// refresh, and the exact terminal sentence -- never the transport error, never a "could not be read".
{
  const calls = newCalls();
...
// 24k. THE STALL STATE KEEPS POLLING BY ITSELF (the update flow's own promise: "and it DOES keep polling; no user
...
// 24l. THE THIRD FIRST-CLASS OUTCOME: applied WITH CONFIRMATION PENDING -- the engine kept the
...
// 24m. THE FOURTH FIRST-CLASS OUTCOME: expired-cleared -- a stale pending the engine itself
...
// (the pending card must stop offering to resume something the engine already cleared).
{
  ...
  const { env, counters } = flowEnv(engine);
  await runLiveApplyFlow(env, "tok", null, null, ZERO_HOOKS);
  eq("the exact expired-cleared sentence rendered, byte-for-byte", progressLine(env.out), EXPIRED_CLEARED_LINE);
...
// ===========================================================================
// 26. LAZY-CHUNK PRELOAD -- preloadOwnLazyChunks
// ===========================================================================
console.log("\n-- lazy-chunk preload --");
...
// ===========================================================================
// 28. 0.1.5 UPDATE-UX -- the exact copy, pinned byte-for-byte
// ===========================================================================
// Every string in this section is normative and byte-for-byte in the UI. One transliteration: the source
// prose for the stall line uses a typographic em dash; house lint (lint:prose) bans that character anywhere
// in src, so the shipped copy substitutes the ASCII "--" this family already uses for the identical reason
// elsewhere (componentRowLine's "Console -- up to date"). The words are pinned unchanged.
console.log("\n-- 0.1.5 update-UX: exact copy --");

eq("stage 1", STAGE_CHECKING, "Checking the release…");
eq("stage 2", STAGE_DEPLOYING_ENGINE, "Deploying the engine…");
eq("stage 3", STAGE_PROVING, "Proving the new version…");
eq("stage 3 subline", STAGE_PROVING_SUBLINE, "this can take a minute");
eq("stage 4", STAGE_UPDATING_CONSOLE, "Updating the console…");

// terminal sentence 1: "Updated to <version>." (+ "Reloading to finish" when the console applied too).
eq("terminal: applied", appliedTerminalLine("0.1.5", {}), "Updated to 0.1.5.");
eq("terminal: applied + reload suffix", appliedTerminalLine("0.1.5", { consoleApplied: true }), "Updated to 0.1.5. Reloading to finish.");
eq("terminal: applied, no version known (honest degrade)", appliedTerminalLine(null, {}), "Updated.");
ok("terminal never claims a reload when the console did NOT apply", !appliedTerminalLine("0.1.5", { consoleApplied: false }).includes("Reload"));

// terminal sentence 2: "Rolled back automatically: <one-line reason>. Nothing else was changed."
eq("terminal: rolled-back with a reason", rolledBackTerminalLine("the canary did not sing"), "Rolled back automatically: the canary did not sing. Nothing else was changed.");
eq(
  "terminal: rolled-back with NO recorded reason (honest fallback, matches the family's existing voice)",
...
eq("terminal: rolled-back with an empty-string reason ALSO falls back (never a dangling colon)", rolledBackTerminalLine(""), "Rolled back automatically: the canary did not confirm the new version. Nothing else was changed.");

// The ONE honest stall state.
eq("stall line, byte-for-byte", STALL_LINE, "Still verifying -- this screen keeps checking by itself.");

// The stale-pending-expired first-class outcome.
eq("expired-cleared line, byte-for-byte", EXPIRED_CLEARED_LINE, "A leftover verification from an earlier attempt was cleared; nothing was changed.");

// The destination gate: the engine's exact refusal reason (matched verbatim, reactively) and the
// console's own proactive/reactive line + link.
eq(
  "engine refusal reason, byte-for-byte",
  DESTINATION_GATE_ENGINE_REASON,
  "updates verify themselves with a canary flight to your destination; add a destination first, then apply this update",
);
eq("destination-gate console line, byte-for-byte", DESTINATION_GATE_CONSOLE_LINE, "Updates verify themselves with a canary flight to your destination. Add a destination first.");

// The paired-rollback confirm line.
eq("paired-rollback line, byte-for-byte", PAIRED_ROLLBACK_LINE, "Rolling the engine back past what this console requires; the console will be rolled back with it.");

// terminalLineForOutcome (the shared bridge both renderers use): every RecordedSettleOutcome kind maps to
...
eq(
  "bridge: applied + confirmationPending (the calm background-confirmation line)",
...
eq(
  "bridge: applied + confirmationPending + consoleApplied (the reload suffix, engine-then-console order)",
...
// this codebase's established "a link" idiom for in-SPA navigation, e.g. the dropped-sources prompt).
{
  ...
  ok("the destination-gate node carries the exact destination-gate line", textOf(node).includes(DESTINATION_GATE_CONSOLE_LINE));
...
// ===========================================================================
// 29. THE PENDING CARD IS A RESUME SURFACE, ALWAYS WITH ITS OWN CONTROLS
// ===========================================================================
...
// strand the operator with prose about controls that are not there. The button copy is also plain language,
// with no "verify"/"settle" jargon in the resting label either.
console.log("\n-- pending card: always a full resume surface with controls --");
...
// 29a. The resting label is plain language, not the old jargon-adjacent "Verify now / roll back".
{
  const out = document.createElement("div");
...
// 29b-29f: drive the REAL settle click through every first-class outcome and prove the card's
...

{
  ...
  eq("29b applied: the exact terminal sentence rendered", progressLine(out), appliedTerminalLine("v1.1.0", { confirmationPending: false }));
...
}
{
  ...
  eq("29d rolled-back: the exact terminal sentence rendered", progressLine(out), rolledBackTerminalLine("the canary did not sing"));
}
{
  const calls = newCalls();
  ...
  eq("29e expired-cleared: the exact sentence rendered", progressLine(out), EXPIRED_CLEARED_LINE);
  eq("29e expired-cleared: reloaded (stop offering to resume something already cleared)", counters.reloads, 1);
}
{
  // THE KEYSTONE CASE (the 0.1.4 bug, reproduced and proven fixed): settle AND every status re-read fail
  ...
  ok("29f STALL (the 0.1.4 keystone case): the card's OWN controls are STILL present (same nodes)", cardControlsPresent(body, btn, tokenInput));
  eq("29f STALL: the exact stall sentence rendered (not a dangling reference to a missing control)", progressLine(out), STALL_LINE);
...
//
// The engine has had POST /admin/update/ramp/settle for a while, and nothing in the console called it.
...
// 29h-4. ROLLBACK-FAILED-STILL-SPLIT: the loudest state in the surface. Live traffic is STILL
...
// ===========================================================================
// 30. DESTINATION GATE -- proactive + reactive
// ===========================================================================
...
// REPLACED by the exact destination-gate line + a link to /destinations; Preview stays fully wired and available.
{
  ...
  ok("the exact destination-gate console line rendered", textOf(body).includes(DESTINATION_GATE_CONSOLE_LINE));
...
// 30b. REACTIVE: a live apply refuses with the engine's EXACT destination-gate reason (destConfigured looked true from
...
// 30c. REACTIVE via a THROWN refusal (the apply call itself throws, e.g. a 400 with the exact destination-gate reason
...
// ===========================================================================
// 31. PAIRED-ROLLBACK CONFIRM
// ===========================================================================
// Before a token is spent on an ENGINE rollback, confirmEngineRollback reads the rollback plan (best-effort,
// NEVER a blocker); when it reports paired:true the confirm dialog shows the exact paired line and the operator
// must proceed knowingly. Console-only rollback never calls this at all (always the safe
...
// 31a. paired:true -> the confirm dialog shows the exact paired line; Cancel resolves false, nothing spent.
{
  ...
  ok("the exact paired line is shown BEFORE the token is spent", surface !== null && textOf(surface).includes(PAIRED_ROLLBACK_LINE));
...
// 31b. paired:false -> the plain confirm, WITHOUT the paired line; Confirm resolves true.
{
...
// shows the paired line before the actual rollbackUpdate() call ever fires.
{
  ...
  rollBtn?.click();
  const surface = await openConfirmSurface();
  ok("the standalone control's OWN click shows the paired line before any token is spent", surface !== null && textOf(surface).includes(PAIRED_ROLLBACK_LINE));
...
// A release step marked blocking must be acknowledged before Update-now enables. The gate lives in
...
console.log("\n-- blocking-step acknowledgement gate (availableUpdateBody, DOM) --");
{
  const engineStub = {} as unknown as EngineClient; // handlers only fire on click; render never calls it
  const out = document.createElement("div");
  const env = { out, reload: () => {}, restoreFocus: () => {}, canManage: true, destConfigured: true };
...
// REGRESSION: the gate must not be reopened by the two OTHER writers of Update-now's disabled state.
// refreshApplyEnabled is the single writer; preview()'s finally and updateNow()'s tail both route through it.
console.log("\n-- blocking-step gate is not reopened by Preview or a no-reload apply (DOM) --");
{
...
// gets every business-N band wrong too ("business-1" title-cases to "Business-1", not the
// "Business (1 estate)" control-plane's OWN tierDisplayName (src/licence/issue.ts) puts in the
// self-serve activation email's tier_name). The map must carry all four business-N ids, not only the
// older ids they replaced (starter/growth/a bare "business"), or a real business-1..25 licence falls
// through to the ugly titleCase fallback on every screen that reads it (this Licence card AND the
// Overview tile, screens/overview/tiles.ts). The four cases below pin that and match the email word
// for word.
...
// The three retired ids (starter, growth, a bare "business") are DELETED outright, not aliased
...
eq("a retired tier id (no alias) falls through to titleCase like any unknown id", tierDisplayName("starter"), "Starter");
...
// contract's display names -- no alias for the three retired ids (no-legacy).
eq(
...
// Malformed estate (the type promises a full EstateSummary; the wire does not):
...

{
  ...
  try {
    ...
    // The claim POST goes SAME-ORIGIN, through the console Worker's narrow claim proxy: the console's own
    // CSP sets connect-src to exactly 'self', so an absolute cross-origin URL would be blocked by the
    // browser before the network, and activation would be dead in the shipped product. This test cannot see
    // a CSP violation because it replaces globalThis.fetch, which is why scripts/connect-src-gate.mjs checks
    // the policy statically. The two properties worth holding are asserted directly: the request is
    // same-origin, and it never goes to admin.downpipes.io (which sits behind Access and would redirect a
    // customer's first activation into a login page).
    ...
    // 36c. any other non-2xx -> the ERRORED copy, NOT the unreachable one. The service ANSWERED. Telling
    // a customer whose connection is demonstrably working to "check your connection" sends them to debug a
    // network that is fine while a vendor outage runs, and it would be the single most misleading string on the
    // screen. This assertion pins the correct behaviour, and the negative control
    ...
    // 36d-2. Claiming binds the licence to this Cloudflare account: a 409 band-full refusal reads the control plane's own
...

{
  ...
  try {
    ...
    //
    // THE FIXTURE MOVED FROM "pastedtoken.value" TO A TWO-PART VALUE, and the reason is the defect the
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// A MALFORMED engine/vendor value must not render as an ABSENT one.
//
// Three cues in this screen are computed from values the engine or the licence token supplies. Each is
// silently disarmed by a malformed input if "malformed" and "absent" are allowed to collapse onto the
// same null:
...
// ---------------------------------------------------------------------------
console.log("\n-- malformed engine values are named, not silently swallowed --");
...
console.log("\n-- a licence minted for another Cloudflare account --");
{
  ...
  //
  // Claiming with your own code binds the account automatically, so this mismatch mainly arises from
  // a token pasted from somewhere else: the PRIMARY remedy is claiming with this account's own code,
  // and support is the fallback, not the first line of defence.
