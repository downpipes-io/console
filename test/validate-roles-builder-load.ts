// B37: the custom-role builder screen's catalogue-load fault must tell "not wired" apart from "broken".
//
// rolesBuilderScreen loads GET /admin/custom-roles to seed the existing-roles catalogue. Its catch used to
// render the "pending the engine" note for ANY failure, so a genuine 500 (the engine live and broken) or a
// network abort (unreachable) read as an unbuilt feature, with no block error and no Retry: the exact
// B23/B24 pending-vs-broken collapse, on a third screen. The fix routes a real fault to the honest block
// error with Retry and keeps the pending-engine note only for a genuine not-wired build (404/501).
//
...
async function main(): Promise<void> {
  console.log("-- B37: a genuine fault renders the honest block error, not the pending note --");
  {
  ...
  console.log("-- B37: a genuinely not-wired endpoint keeps the honest pending note (unchanged) --");
  {
  ...
  console.log("-- B55: the custom-role Save button disables before the create await (no double-fire) --");
  {
    // A valid FRESH-CREATE composition (the kv-restorer recipe validateCustomRole accepts) with a name NOT in
    // the existing catalogue: the branch that used to run void doSave() with the button never disabled, so an
    // ordinary double-click submitted the create twice. A createCustomRole we hold pending lets us inspect the
    // button WHILE the create is in flight: the guard is the disabled state set before the await.
    ...
    ok("B55: the save button renders", saveBtn !== undefined);
    saveBtn?.click();
    await flushAsync();
    ok("B55: one click fires exactly one createCustomRole", calls === 1);
    ok("B55: the save button is DISABLED while the create is in flight (a second click cannot double-fire)", saveBtn?.disabled === true);
