// Refuter for G-P1-145 (confirmed live geometry bug, proven on the harness's visual probe): a
// maximal-length downpipe config.name (the field is 1 to 256 chars, editor-upsert.ts:50 validate
// v.length <= 256) drove the /downpipes Name cell to ~1600px, ran the name past the viewport, and
// shoved the page-header primary controls ("Pause auto-refresh", "New downpipe") off-screen at 1366px.
//
// The console DOM shim has no layout engine, so the pixel-level in-bounds proof is a headless-Chromium
// geometry measure run alongside the fix (the harness measureNameCell oracle: the widest .dp-name right
// edge went from 2388 to 1145 at a 1366 viewport, spillsViewport true -> false). THIS suite is the
// durable, default-FAIL guard on the fix's MECHANISM, the thing the geometry depends on:
...
console.log("\n-- G-P1-145: the /downpipes Name cell caps a maximal-length name --");
