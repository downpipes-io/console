// RG15: the credentials cleanup attestation, driven through the REAL modal and the REAL row.
//
...
//   3. A READER must not be able to attest at all. The row renders a REFUSED button for a reader (case 6)
...
// helper an earlier case's button (learned in RG3).
...
async function main(): Promise<void> {
  ...
  {
    // Measured as a DELTA. Toasts outlive the modal that raised them, so case (4) has already left a
    // "Could not record the attestation" toast in the document; asserting its absence would fail for a
    // reason unrelated to this branch. What must hold is that THIS click adds no such toast. (The same
    // trap caught me in RG3, which is why it is measured this way from the start here.)
