...
// DOM shim and drives its interactive contract end to end, closing that gap.
//
...
export function runDataTable(h: Harness, ctx: Ctx): void {
  ...
  // ---- facets in the same group OR; different groups (and ungrouped) AND ----
  {
    ...
    // Two mutually-exclusive values of ONE dimension: the union, never AND to empty. No row is both kind a
    // AND kind b, yet all four rows render (kind a OR kind b).
