// TC-destination-azure-provider: the Azure Blob provider asks for what Azure actually issues, hides what it
...
//
// IMMUTABILITY WAS THE THIRD, AND THIS FILE ASSERTED THE REFUSAL AS CORRECT BEHAVIOUR UNTIL.
...
console.log("TC-destination-azure-provider: Azure Blob asks for what Azure issues and never submits what it hid");
...
console.log("\n-- the rendered form --");
{
  ...
  // REGION AND ADDRESSING, hidden because they are inert on this provider. Measured in the
  ...
  // THE LABEL ITSELF IS LOAD-BEARING AND WAS UNGRADED UNTIL, and the way it was ungraded is the
...
console.log("\n-- the immutability policy, through submitDestination --");
{
  ...
  // ---------------------------------------------------------------------------------------------------
  {
    ...
    // WHAT AZ-13 ACTUALLY PROVES, measured by planting rather than assumed. Removing the half-set guard
...
console.log("\n-- the destKind display sites --");
{
  ...
  // EVERY AZURE CLOUD, NOT JUST THE COMMERCIAL ONE. The console matched `.blob.core.windows.net` alone
  // until, while the engine has always routed all three suffixes in AZURE_STORAGE_SUFFIXES to
  ...
  // refused by the leading boundary and a matcher with no "$" would pass this test unchanged: measured, a
  // build with the "$" deleted survived exactly that vector. The account-prefixed form is the domain an
