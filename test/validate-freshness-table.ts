...
// in place via setRows; buildTable memoises the per-row freshness verdict (finding
// console-src-055-11), and that memo used to outlive the row set. The run facts load
...
// B58: the Schedule column must show the schedule the engine ACTUALLY applies. A downpipe with a cron schedule
...
// label ("Daily"). A plain-interval downpipe still reads its cadence. The Schedule column is td[2].
{
  ...
  ok("B58: a cron downpipe's Schedule cell shows the cron, not the cadence label", scheduleCell(t2.el, "weekly").includes("0 0 * * 0") && !scheduleCell(t2.el, "weekly").toLowerCase().includes("daily"));
  ok("B58: a plain-interval downpipe's Schedule cell still shows its cadence", scheduleCell(t2.el, "plain").toLowerCase().includes("daily"));
}
