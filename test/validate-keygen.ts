...
async function main(): Promise<void> {
  ...
  //
  // WHERE THIS SECTION IS MANDATORY, AND WHY IT IS NOT MANDATORY HERE. Until this change
  // the null branch recorded a hard FAIL and exited 1, so on
  // any checkout without a reader sibling this validator could not pass. `npm run validate` is a chain of
  // roughly 150 members joined by &&, this file is the sixth, and the CI runner for the Validators job
  // checks out console ALONE. The result was not a strict gate: it was three CI jobs (Validators and both
  // Coverage jobs, which run the same chain under c8) permanently red, and the other ~144 validators in
  // the chain never executed in CI at all from 4a0ad57 onward. A per-commit chain that cannot run checks
  // nothing, which is worse than the skip it was written to avoid.
  //
  // So the resolution follows test/downpipe-root.ts's own two-tier contract, which already existed for
  // exactly this: requireDownpipeRoot returns null on a reader-less checkout and THROWS under
  // REQUIRE_DOWNPIPE=1. The cross-repo assertions below are now skipped loudly when there is no reader,
  // and the Cross-repo gates job in .github/workflows/ci.yml checks the reader out and runs this file
  // again with REQUIRE_DOWNPIPE=1, where the skip is impossible and a missing sibling fails the job. The
  // check did not get weaker; it moved to the one job that can actually perform it, and stayed mandatory
  // there. Everything above this line is console-against-console and still runs everywhere.
