...
//
// WHY THIS EXISTS. console f35c4b0 moved the interval from localStorage to the engine, which turned a
...
// Disabled-with-reason (UIA-33), not the native `disabled` attribute: the control stays reachable
...
// { ok: true, present: false, ... } that carries no interval, and its header calls that "THE fabricated-
// success funnel". Without a shape guard the control would render "a undefined-day interval" to a visitor on
// the public tour. A response with no number is an unreadable answer, not an exotic interval.
