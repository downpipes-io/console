...
//
// THE CLASS, AND WHY IT IS THE ONE THIS CAMPAIGN KEEPS FINDING. A rollover is documented as APPEND (so
// assertions signed by either key verify through the overlap) then REPLACE (so the retired certificate is
// pruned). Append only ever RAISES the pinned count, which the engine caps at eight, and the cap is on the
// RESULT: the engine validates the existing pinned set concatenated with the paste. This screen's guard
// tested only the paste, so seven pinned plus two pasted saw two and let it through.
//
// WHAT THE OPERATOR WAS THEN TOLD. The engine's refusal for an over-capacity array was the wrong-shape
// sentence, "idpSigningCerts must be a non-empty array of PEM X.509 certificates", because strArray returns
// one null for "not an array", "an entry is not a bounded string" and "too many entries" alike. So an
// operator rolling a signing certificate over against their provider's cut-over deadline was told their
// perfectly good certificates were not certificates, and the remedy that sentence implies, re-export from
// the provider, cannot work. The remedy that does work, Replace, is the other control on the same screen
// and was named nowhere. That half is fixed in the engine (validate-samlcert-cap-honesty); this is the
// half that refuses before the request is ever made.
//
// IT WAS NOT AN UNKNOWN. This file's own hint has always read "on Append that total counts the ones already
// pinned", its header says the mirrored cap exists "so an over-long append is refused at the field rather
// than as a bare engine reason", and the field catalogue row for saml-rollover-certs records in as many
// words that the client caps the paste while the server caps the result. What was missing was the
// arithmetic rather than the knowledge, which is why an assertion is worth more here than a comment.
//
...
async function main(): Promise<void> {
  ...
  verdictReached(base.failures, checksRun);
