...
//
// Engine 43b78012 rewrote a refusal so it names the count, the population, the bound, why splitting
// cannot work, and the step that lowers it (POST /keys/add-operational). That is the fix three previous
// passes' findings all turned on. The question this file settles is whether such a sentence REACHES THE
// OPERATOR at all on a JSON route, and the answer is NO, by design and on purpose.
//
...
// The engine's real sentence at 43b78012, quoted so the fixture cannot drift from what the engine sends.
const ENGINE_REASON =
