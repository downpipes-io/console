// RG9: the bulk import drawer's two decision functions, driven directly.
//
...
// a trailing ": 401"), not from a property on the error. Getting that wrong is a real trap: my first
// version of this helper set err.status = 401, which classifyError ignores, so the loop took the
// per-binding-failure branch instead of the halt branch. Every assertion in section (9) failed, which is
// the test doing its job: a fake of the wrong shape exercises the wrong path and would otherwise have
// "passed" while covering nothing. Same form as validate-sources-bulk-protect.ts's helper.
