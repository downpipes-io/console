// Validate that the standalone "Add a source" screen offers the TWO TOKEN sources (Cloudflare
...
// failed read is stated explicitly rather than swallowed by an empty catch: an unstated failure would look
// identical to an account that genuinely offers nothing more, which reads to the operator as "where did
// Cloudflare configuration go?".
{
...
// the scope preset control appears.
{
  const found = disc({
...
// C.4: the multi-zone BULK tick -- ticking several zone checkboxes queues one downpipe each
// (mirroring kv/r2/d1's multi-select exactly), and switching the scope preset to "Account
// configuration only" clears any ticked zones rather than leaving an invisible tick that would
// still create a downpipe: the preset's own clear must reach the zone checkboxes too, or nothing
// actually unticks.
{
  const found = disc({
    cfConfigSurfaces: SURFACES,
...
// ---------------------------------------------------------------------------
// From the wizard's source step, "Use the advanced editor" hands the picked TOKEN/MEDIA source off
// to the full editor. The hand-off must seed accountId (and the cf-config zone/surfaces), so the
// editor's Create build carries them; otherwise the engine rejects every save of a token/media
// source with no UI recovery. Each of the five token/media types is driven end to end: render the
// wizard with the hand-off, click into the advanced editor, save, and assert the captured
// addDownpipe payload carries the account (and, for cf-config, the zone).
...
// ---------------------------------------------------------------------------
// h("input", {type: "checkbox", checked:true}) -- the pattern editor-cf-config-section.ts's
// renderCfSurfaces and the wizard's zone/account rows all use to pre-tick a restored selection --
// only writes the CONTENT attribute in the shim, leaving the live .checked property false; a test
// asserting .checked on such a checkbox would see it unticked even though it rendered ticked, hiding
// real product bugs behind a shim gap. Pinned here, narrowly, in the shim itself, so a future shim
...
// ---------------------------------------------------------------------------
// The binding-attach flow collects a one-shot deploy token (type=password), calls
// engine.changeBindings once, clears the token on success, and routes an isUnauthorised error to
// the signed-out flow. Drive the REAL screen with a stubbed changeBindings and assert:
