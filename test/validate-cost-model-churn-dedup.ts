...
export function run(h: Harness): void {
  ...
  // Cross-check: the egress basis is strictly less than an end-of-month basis (accumulate
  ...
  ok("accumulate egress midpoint basis < end-of-month basis (no inflation)", mcS3Acc.egress < oldEndOfMonthEgressAcc);
  // Also confirm the storage and egress bases are consistent: both use the midpoint size.
  ...
  // no-op (monthlyCost is invariant in retentionRuns in the accumulate regime). Passing
  // regime:"retained" explicitly here would mask that dead path, so this drives the screen's
  // own call shape (no regime) and asserts the sweep genuinely moves.
  ...
  // The deltas must be distinct and not all zero: an accumulate-regime retention sweep produces
  // delta 0 at every depth (isNonDecreasing passes on an all-equal series), which would hide the
  // contradiction the live screen renders if the retention axis stopped forcing the retained regime.
  ...
  // The retention axis must be evaluated in the retained regime
