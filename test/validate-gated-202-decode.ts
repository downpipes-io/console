...
// queued-toast "View" action or an unauthorised branch from throwing for a harness reason.
...
// /admin/idp/connections/cert). This is the console's only caller for the route: without it, rotating a
// certificate means remove-and-re-add, which ends every session signed in through the connection. The
// decoder is pinned at this call site, so a queued response can never be misread as applied.
{
  let restore = stubFetch(202, QUEUED_OWNER);
...
// engine has served POST /admin/idp/connections/cert since that route shipped and no console code called it, so the only
...
// The verdict is DECLARED, so a silent exit 0 cannot pass as green. The real check count goes with it
// rather than being inferred from the output: an early return anywhere above would land here with a
// smaller number, and a run that reached no assertion at all fails on the count instead of passing quietly.
//
// THE EXIT READS process.exitCode, and that is not a flourish. verdictReached refuses a zero-check run by
// SETTING process.exitCode, and a bare `process.exit(failures === 0 ? 0 : 1)` immediately after overwrites
// it with an explicit 0, so the guard prints its refusal and the run still reports green. Proven by
// mutation: passing 0 as the check count makes this file print "declared a verdict after running 0 checks"
// and, with the bare exit, still exit 0. Honouring whatever the guard set is what makes the enrolment
// load-bearing rather than decorative.
verdictReached(failures, checks);
const guarded = typeof process.exitCode === "number" ? process.exitCode : 0;
process.exit(failures === 0 && guarded === 0 ? 0 : 1);
