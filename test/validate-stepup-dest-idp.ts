// The destination + IdP management screens (plus the session-termination
...
// runAccessReauth drives a mutation against an engine answering the ACCESS form of the step-up 401
// ({ stepUpRequired:true, reauth:"access" }, ASVS V7.5.1): the ceremony must NOT run (an Access session has no
// passkey to assert with), exactly one call is made, and the thrown error classifies as access-reauth-required.
async function runAccessReauth(name: string, invoke: (e: EngineClient) => Promise<unknown>): Promise<void> {
  const engine = new EngineClient("https://engine.test");
  let ceremonyRuns = 0;
  engine.onStepUpRequired = async () => { ceremonyRuns++; return STEPUP_TOKEN; };
  const { calls, restore } = scriptFetch({ status: 401, body: { stepUpRequired: true, reauth: "access" } }, { status: 200, body: { ok: true } });
  let thrown: unknown = null;
  await invoke(engine).catch((e: unknown) => { thrown = e; }).finally(restore);
  ok(`${name}: the passkey ceremony does NOT run on reauth:"access" (V7.5.1)`, ceremonyRuns === 0);
  ok(`${name}: exactly one call, no retry`, calls.length === 1);
  ok(`${name}: the error classifies as access-reauth-required, not stepup-required`, classifyError(thrown).kind === "access-reauth-required");
}
...
async function main(): Promise<void> {
  ...
  const mutations: Array<{ name: string; invoke: (e: EngineClient) => Promise<unknown>; pathEnds: string }> = [
    ...
    // The engine added these four to STEPUP_SUBS while the
    // console still called them through plain engineFetch, so the engine's 401 { stepUpRequired } reached the
    // SCREEN and a notify save failed closed with a visible error, no ceremony ever running. They govern the
    // availability of EVIDENCE (deleting or narrowing the rule that carries a critical alert silences an
    // account with no visible effect and no undo), which is why both directions are gated, set as well as
    // delete. Asserted here rather than in the notifications validator so the whole step-up family is read
    // in one place, and so a future route joining STEPUP_SUBS has an obvious home.
    ...
    // The one that mattered most and was missing longest. restore() used
    // a plain engineFetch while 22 other sensitive writes used gatedFetch, so a maker-checker restore that
    // was fully approved and perfectly valid could not be applied at all: a real review takes longer than
    // the 300-second freshness window, the engine answers 401 { stepUpRequired }, and on a bare engineFetch
    // that 401 is final. The console then told the operator to approve a passkey prompt that this call path
    // could not raise, measured as the credential API being called zero times across the whole apply. The
    // only escape was outside the flow, and for the in-console break-glass restore the per-run master is
    // decapsulated in the tab and lost with it.
    { name: "restore", invoke: (e) => e.restore({ runId: "01JBRESTORE00000000000000", confirm: true } as never), pathEnds: "/admin/restore" },
    // The largest instance of this class so far: FIFTEEN POST call
    // sites reached a step-up-gated route through plain engineFetch, so on a cookie-borne session the
    // engine's 401 { stepUpRequired } was the final answer and the action FAILED rather than prompting.
    // /roles had been gated in the engine, six weeks earlier.
    //
    // WHY IT SURVIVED, and the reason a list like this one could never have been enough: requireStepUp
    // opens with `if (method === "token" || method === "access") return null`. Every dev, uat and internal
    // estate is Access fenced and the harness drives with a bearer token, so every environment the product
    // is exercised in is exempt by construction and reports these calls working. The list below had sixteen
    // entries while the engine gated forty, and the fifteen sat in the gap where nothing was looking. It is
    // kept because behaviour is worth asserting directly, and scripts/stepup-call-site-gate.mjs now derives
    // the same question from the engine's own set so a route joining it cannot wait to be added here.
    ...
    // STEPUP_SUBS members and the engine gates their PARSED action instead (router.ts). That is
    // exactly why they were missed: a reader checking the Set finds nothing to check. Approve applies the
    // change and is gated; reject only discards a pending record and stays exempt in the engine, so it is
  ...
  // ACCESS REAUTH (V7.5.1): the same mutations, answered with the Access form of the step-up 401, must NOT
  // open the passkey ceremony; the plain form above is the control that proves the ceremony wiring works.
  for (const m of mutations) await runAccessReauth(m.name, m.invoke);
  ...
  {
    // The intentional case: a windowed apply (complete:false, no failures). Distinct from a failure.
