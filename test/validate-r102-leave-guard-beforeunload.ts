// TC-r102-leave-guard-beforeunload: a registered leave guard must also stop a REAL navigation (a
// reload, a tab close, back/forward through browser chrome), not only an in-SPA navigate() call.
...
console.log("TC-r102-leave-guard-beforeunload: a real navigation is caught by the same registration as the SPA guard");
...
// R-102 exists for: the value the customer cannot get back a second time.
{
  const form = destinationForm(engine, fctx, { confirmReplace: false, onSaved: () => {} } as never);
  document.body.replaceChildren(form);
  await flushAsync();
...
// ---- PART 4: the wizard's name/cadence draft (the second half of R-102 -- no draft persistence). --
