// R-20: the credential-expiry detail drawer's Edit and Remove buttons were unreachable behind a
...
async function main(): Promise<void> {
  ...
  console.log("\n-- credentials/list.ts: openItemDrawer --");
  {
    ...
    openItemDrawer(engine, row, () => {});
    await flushAsync();
    ok("(1c) R-20 FIX: the re-render's second open does NOT stack a duplicate drawer", qsa(document.body, ".overlay--drawer").length === 1);
    ok("(1d) R-20 FIX: the original (first) drawer surface is still mounted, not replaced", document.body.contains(firstSurface as unknown as Node));
    ok("(1e) R-20 FIX: the surviving drawer's surface is NOT inert -- Edit/Remove stay reachable", attr(qs(document.body, ".dialog--drawer"), "inert") === null);
  ...
  console.log("\n-- runs/detail.ts: openRunDetail --");
  {
    ...
    await flushAsync();
    ok("(2b) R-20 FIX: a second RunsView instance's re-fired deep link does NOT stack a duplicate", qsa(document.body, ".overlay--drawer").length === 1);
    ok("(2c) R-20 FIX: the original drawer surface is still mounted", document.body.contains(firstSurface as unknown as Node));
    ok("(2d) R-20 FIX: the surviving surface is NOT inert", attr(qs(document.body, ".dialog--drawer"), "inert") === null);
  ...
  console.log("\n-- map/drawer.ts: buildMapDrawer --");
  {
    ...
    buildMapDrawer(flow, deps);
    await flushAsync();
    ok("(3b) R-20 FIX: a fresh controller's re-synced drawer does NOT stack a duplicate", qsa(document.body, ".overlay--drawer").length === 1);
    ok("(3c) R-20 FIX: the original drawer surface is still mounted", document.body.contains(firstSurface as unknown as Node));
    ok("(3d) R-20 FIX: the surviving surface is NOT inert", attr(qs(document.body, ".dialog--drawer"), "inert") === null);
    closeAllOverlays();
  }
  ...
  console.log("\n-- destination-cards.ts: the destination detail drawer (via row activation) --");
  {
    ...
    (row as unknown as { click(): void }).click();
    await flushAsync();
    ok("(4b) R-20 FIX: a duplicate activation does NOT stack a second drawer", qsa(document.body, ".overlay--drawer").length === 1);
    ok("(4c) R-20 FIX: the original drawer surface is still mounted", document.body.contains(firstSurface as unknown as Node));
    ok("(4d) R-20 FIX: the surviving surface is NOT inert", attr(qs(document.body, ".dialog--drawer"), "inert") === null);
    closeAllOverlays();
  }
