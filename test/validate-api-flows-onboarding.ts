...
//   7: CON-L5 -- recordCeremonyIntent() -> recordAuditIntent and renderInviteForm's
...
export async function runOnboardingFlows(
  ...
  // Store ordering (finding console-test-002-08): `store` is the live singleton shared with
  ...
  // ==========================================================================
  // SECTION 7: CON-L5 -- onboarding wizard's two engine WRITES (REAL CODE)
  //
  // The onboarding wizard performs exactly two engine writes, and CON-L5 (gap audit
  // downpipes-AUDIT.md) flagged both as untested:
  ...
  // ==========================================================================
  console.log("\n-- CON-L5: onboarding audit-intent + invite/setRole writes (real code) --");
  ...
  async function driveInvite(
    ...
    // Catch a handler that throws instead of degrading (the exact regression CON-L5
  ...
  // B40: the onboarding invite form POSTs /admin/roles, which the engine gates on roles.write (Owner AND
  // access-admin), the same capability the main Access roles table uses. It must gate on roles.write, not the
  // owner ladder: pre-fix (canDo("owner")) an access-admin was under-granted a control the engine allows.
  {
    ...
    if (eng) {
      ...
      ok(
        "B40: a viewer sees the PERMISSION reason in customer language, not the raw id, and not the form (R-19)",
