...
// (src/screens/security-centre.ts), build-contract section 7 + section 9.
...
//
//   COVERAGE / gap detection (build-contract section 7 + section 9):
...

{
  ...
  // disclosure, never one uncapped card per resource (an uncapped render grows the page without bound). The
...
// the policy stays ON. setConfigApprovalPolicy must not read a queued body's absent requireConfigApproval
// field as false: doing so would paint the toggle "Off" for a gate that is, correctly, still armed. This
// renders the REAL
...
// THERE IS DELIBERATELY NO `if (failures > 0) process.exit(1)` BELOW THIS LINE. Almost every assertion
// in this file runs SYNCHRONOUSLY at module level (lines 104 to 431); only the config-approval toggle
// lives inside finish(). finish() is async, so it yields at its first await, and a module-level bail
// placed here would run while finish() is still in flight, pre-empting the verdictReached above: a
// failing assertion would then exit 1 having printed assertion-shaped lines but ZERO verdict lines on
// either stream, with the completion guard reporting a process exit without a declared verdict, and the
// async half never running at all. Without such a bail, a failing assertion instead prints
// "N FAILURE(S)" and "VERDICT: FAIL failures=N", which is what `grep -F "VERDICT: FAIL"` needs to find.
// A bail here would also be redundant: finish() tallies the same `failures` counter after awaiting, so
