// Validates CONSOLE-HONESTY behaviour for support-pack diagnostic coverage. Every case here guards against the
// same class of defect: the console showing a state the engine never reported. A read that FAILS must not
// render exactly what a healthy read renders, a safety check that could not RUN must not render exactly what a
// check that passed renders, and a write that is REFUSED must not render nothing at all. Otherwise support
// cannot tell an outage from a working system, because the console cannot either.
//
// The invariant every assertion below defends: the console never fabricates a state, and never collapses an
// UNKNOWN into an ALL-CLEAR. A failed check reports itself as a failed check.
//
...
//
// Coverage:
//   a rejected source discovery is stated, not silently swallowed (add-source picker + destination form)
//   audit-log capacity: read-failed and field-absent are UNKNOWN, never a silent all-clear
//   a refused drill-evidence write is surfaced beside the PASSING drill (runs detail), and on the restore-flow
//     proof card
//   an unloadable config diff names WHICH snapshot body the engine no longer retains
//   a discovery token the engine stored but which sees ZERO accounts is a WARN, not a success
//   a 403 refusal carries the ENGINE'S reason; it is never replaced by generic capability copy
//   the restore receipt breaks failures down by reason, and states the nameless ones the retry misses
//   the disaster-recovery client keeps the engine's reason on 401/403 and stamps a stable refusal code
//   every browser-side recovery rejection carries a stable code; a failed status read is not an all-clear
...
// ===========================================================================
// an unloadable config diff names WHICH version's snapshot body is gone.
// ===========================================================================
console.log("\n-- unloadableDiffReason names the missing snapshot, never 'one of the versions' --");
{
...
// ===========================================================================
// a stored token that sees ZERO accounts is not a success.
// ===========================================================================
console.log("\n-- a discovery token that sees no accounts is a WARN, not 'Token verified' --");
{
...
// ===========================================================================
// the restore receipt breaks the failures down, and owns the nameless ones.
// ===========================================================================
console.log("\n-- restore failures carry a class breakdown and the nameless count --");
{
  const failuresIn = [
...
// ===========================================================================
// a refused drill-evidence write is surfaced, never swallowed.
// ===========================================================================
console.log("\n-- a refused drill-evidence write is stated beside the PASSING drill --");
{
  let missed = 0;
  ...
  eq(missed, 1, "a refused evidence write calls onMissed, not an empty catch");
...
// ===========================================================================
// a rejected source discovery is stated, never silently degraded.
// ===========================================================================
console.log("\n-- a failed discovery read is stated, not rendered as an empty account --");
{
  const bothFail = {
...
// ===========================================================================
// the disaster-recovery refusal vocabulary.
// ===========================================================================
console.log("\n-- every recovery refusal carries a stable, value-free code --");
{
...
// ===========================================================================
// a failed recovery-status read is not an all-clear (and 404 does not cry wolf).
// ===========================================================================
console.log("\n-- a failed recovery-status read renders UNKNOWN, never silence --");
{
...
// ===========================================================================
// audit-log capacity. An UNKNOWN is never an all-clear.
// ===========================================================================
console.log("\n-- audit capacity: a failed or unreported check is UNKNOWN, never silence --");
...
// ===========================================================================
// a 403 carries the ENGINE'S reason, never generic capability copy alone.
// ===========================================================================
console.log("\n-- an engine refusal keeps the engine's reason --");
{
...
// The overview recovery tiles must tell a genuinely not-wired route (404/501, an honest engine addition)
// apart from a real fault (a 500 the engine served while broken, or a network throw). A real fault must NOT
// read as an unbuilt "Pending engine" feature, while a real 404/501 keeps the honest pending copy.
{
  ...
  ok("a 404 (route not wired) still reads 'Pending engine'", wired404.includes("Pending engine") && wired404.includes("Engine addition"));
  const wired501 = textOf(recoveryReadFailedTile("Last audited", new Error("audit: 501"), PENDING));
  ok("a 501 (route not implemented) still reads 'Pending engine'", wired501.includes("Pending engine"));
  const broken500 = textOf(recoveryReadFailedTile("Last proven recoverable", new Error("drill-evidence: 500"), PENDING));
  ok("a 500 (engine live and broken) reads an engine error, NOT 'Pending engine'", broken500.includes("Could not read") && broken500.includes("Engine error") && !broken500.includes("Pending engine"));
  ok("the 500 tile does not frame the fault as an unbuilt feature", !broken500.includes("ships with the engine") && broken500.includes("not an unbuilt feature"));
  const network = textOf(recoveryReadFailedTile("Last audited", new TypeError("Failed to fetch"), PENDING));
  ok("a network throw (unreachable) reads an engine error, not 'Pending engine'", network.includes("Could not read") && !network.includes("Pending engine"));
}

// The recovery-evidence roll-up copy (buildBoundaryNote + the executive "evidence in place" answer) reads
// trailReadState. Either read live = "live"; both a genuine 404/501 = "not-wired" (an unbuilt engine addition,
// honest); both failed with at least one real fault (500/network) = "faulted", so a broken engine is NOT
// described as an unbuilt feature.
{
  ...
  eq(trailReadState(live, ok500), "live", "either read live => live");
  eq(trailReadState(ok404, live), "live", "either read live (audit) => live");
  eq(trailReadState(ok404, { ok: false, error: new Error("audit: 501") }), "not-wired", "both a genuine 404/501 => not-wired (honest unbuilt)");
  eq(trailReadState(ok404, ok500), "faulted", "both failed, one a real 500 => faulted (not unbuilt)");
  eq(trailReadState({ ok: false, error: new TypeError("Failed to fetch") }, ok500), "faulted", "both failed with real faults => faulted");
}

// The change-control-gated deletes (notify channel, notify rule, expiry item) must report the QUEUED 202
// honestly: the client must not plain-parse the 202 into a handler that toasts "Deleted"/"Removed" and reloads
// a list still holding the item, which would be a false success for a deletion a second approver might still
// reject. removeItem (the expiry delete) is the exported representative of the three structurally identical
// handlers; the wire shape for all three is proven in cov/lib-api-client + validate-api-client. Here the REAL
// confirm modal is driven and the queued branch is asserted to surface "queued for approval" and NOT reload.
{
  ...
  ok("a QUEUED expiry removal says 'queued for approval', never 'Removed'", pendingToast.toLowerCase().includes("queued for approval") && !pendingToast.includes('Removed "prod destination key"'));
  ok("a QUEUED expiry removal does NOT reload the list (the item is still tracked)", reloadedOnPending === false);
  ...
  ok("an APPLIED expiry removal says 'Removed' and reloads the list", appliedToast.includes('Removed "prod destination key"') && rec.reloadedOnApplied === true);
}
