...
//
// WHY THIS EXISTS, and it is a pair of behaviours that pull against each other, which is exactly why one
// file has to hold both.
//
// THE FIRST HALF is defect 54's second site, closed by watchForLapse (confirm.ts). findUsableApproval
// refusing a lapsed record closes every case where the LISTING is read after the deadline; it closes
// nothing about a tab that armed while the approval was still live, because startPoll stops the moment the
// gate arms, so nothing is left running to notice the deadline pass. Another pass drove that: dry run,
// gate armed, twenty-five hours in the tab's own clock, and the screen still read "Approved and ready to
// apply" over an enabled Apply one hour past expiresAt. That fix was proved by a harness rig against a
// built bundle and had NO coverage in this repo at all, which is how the second half below happened.
//
// THE SECOND HALF is what that timer cost. It was armed for the approval's whole remaining lifetime and no
// handle was kept, so nothing could cancel it: `clearTimeout` did not appear once in confirm.ts. Each arming
// added another live timer holding the closure over the panel's detached subtree, and in node, where a
// process does have to exit, test/validate-break-glass-restore.ts printed a green verdict and then held the
// event loop for just under an hour. That member sits inside `npm run validate`, which scripts/deploy.sh
// runs as a blocking preflight under `set -e`, so the published `npm run deploy` a customer follows stopped
// with no output and no timeout.
//
// SO THE TWO HALVES ARE GRADED TOGETHER, DELIBERATELY. Either one alone can be satisfied by breaking the
// other: deleting watchForLapse ends the hang and un-fixes defect 54, and arming the timer without a way to
// cancel it retracts correctly and hangs the deploy. Nothing in this repo held both at once before this
// file, and a repair that trades one for the other is the failure mode it exists to catch.
//
...
// passes through it. This is the only way to assert the SECOND half directly: "the panel left nothing
// armed" is a statement about handles, and no assertion over rendered DOM can reach it. The wrappers are
...
// and counting those would make the assertions below about the harness rather than about the gate. The
...
async function main(): Promise<void> {
  ...
  // ---- 2. defect 54's second site: the deadline passes while the tab is already armed -------------------
  {
  ...
  verdictReached(failures); // the verdict is DECLARED, so a silent exit 0 cannot pass as green
