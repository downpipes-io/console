// The canary's ailing CAUSE reaches the operator, and says something different per cause (B30 / G-P1-127).
//
// WHY THIS EXISTS. The engine used to report one flat "unreachable" for every ailing flight, so an expired
// credential, a WORM retention refusal, a throttling store and a black-holed endpoint were the same string.
// This screen had nothing specific to render and rendered nothing at all: the operator was told the flight did
// not finish and left to guess which of those it was, which sends triage to the wrong side of the problem.
//
// The engine now classifies the fault (engine src/canary/cycle.ts derives it through destDownReason). This
// file guards the console half: that each class produces its own honest line, that an unknown or absent cause
// produces NOTHING rather than a fabricated reason, and, load-bearing, that the mapper is actually CALLED by
// the screen.
//
// THAT LAST CHECK IS NOT DECORATION. On the engine side of this same fix my first test asserted through the
// type rather than the call site, so reverting the call site to the old flat string left it green. A pure
