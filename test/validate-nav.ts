...
console.log("-- post-navigation focus survives a REAL onClose-driven navigation (the KA-RESTORE-DRAWER mechanism) --");
{
  ...
  // RESOLVER (re-found by data-key, B25) and then IMMEDIATELY fires openDownpipeDrawer's onClose -- a
  ...
  // own comment on setMain, confirmed against a real browser): before the fix, onClose declared no
  // intent of its own, so that deferred focus move silently overwrote the row focus teardown() had JUST
  // restored, a moment earlier, in the same synchronous close. The harness (KA-RESTORE-DRAWER) measured
  // exactly this: focus lands on <main>, not the row, even though the row was briefly focused first.
  //
