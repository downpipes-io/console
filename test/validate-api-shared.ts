...
export class Harness {
  ...
  // Three CI reds on the restore approval gate reported nothing but the label, so each one said that an
  // expected button was absent and none of them said what the panel held instead. A thunk rather than a
...
//
// : added after CI came back on and validate-restore-approvals.ts proved FLAKY on the
// runner while passing every time locally. Two consecutive runs failed DIFFERENT subsets, 7 then 6,
// which is the signature of a race rather than a defect. The cause is the fixed-round drain above:
// the margin it reasons about is a margin in ROUNDS, and a loaded shared runner can interleave a
// promise chain across more turns than a quiet laptop does, so the assertion reads a DOM that has
...
export function populateForm(
  ...
  // travel on the request (console-src-046-01), so the tests set the real form fields and assert the
