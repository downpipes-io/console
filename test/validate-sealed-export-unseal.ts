...
// engine/test/validate-cov-admin-router-identity.ts).
//
...
// blankComments is the repo's existing comment blanker (scripts/source-text.mjs, self-tested by
// npm run lint:sibling-read). The name bans below identify code by matching source text, and prose is
// where that shape appears most; see the block above them for what it cost here.
import { blankComments } from "../scripts/source-text.mjs";
...
// drifted.
//
// MATCHED AS WHOLE BINDINGS, NOT AS SUBSTRINGS. A per-line regex such as
// /EngineClient|Transport|api\/client|engineFetch/ has two faults that hide each other: it cannot see a
// multi-line import at all (see the ban site below), so a scan limited to whole lines never reaches the
// substring looseness either. Fixing the line-scan surfaces it immediately: client-diag/ring.ts imports
// CLIENT_DIAG_TRANSPORT_CLASS_SET and ClientDiagTransportClass from ./vocab.ts, and both contain "Transport".
// Neither is the api/client Transport, and vocab.ts is a closed-class word list with no network anything. A
// ban that cannot tell those apart is not usable, so the bindings are compared by exact name and the
// specifier by path.
...
async function main(): Promise<void> {
  ...
  //
  // A single-file scan proves nothing about a helper it calls: this
  ...
  // below. A per-line scan such as `text.split("\n").filter((l) => /^import\b/.test(l.trim()))` sees a
  // multi-line import as exactly one matching line, the bare `import {`; every named binding sits on a
  // continuation line that does not start with `import` and goes unscanned. So
  //
  //     import {
  //       EngineClient,
  //       engineFetch,
  //     } from "./api/client.ts";
  //
  // would pass a per-line ban. This is not hypothetical: src/lib/client-diag/ring.ts:20 is inside this very
  // walk and already imports across lines. The recursive walk itself is unaffected, because its own importRe
  // spans newlines; only a per-line ban would miss it. importStatements instead reads whole statements, so
  // the guarantee holds for a multi-line import too.
  ...
  // THE FOUR NAME BANS READ CODE, NOT PROSE: a raw-file scan cannot tell the two apart. A comment such as
  // "what the browsers themselves say when fetch() rejects with no response" in src/lib/errors.ts, which is
  // inside this walk, matches `\bfetch\s*\(` and would turn this file red, and `npm run validate` with it,
  // over a sentence describing behaviour rather than any code at all. That is the mirror image of a defect
  // this repo has already paid for: scripts/source-text.mjs's own header records key-material-wipe-gate.mjs
  // going GREEN because the only `master.fill(0);` had been commented out. A raw-text scanner is wrong both
  // ways round, and blankComments is the repair built for it.
  //
