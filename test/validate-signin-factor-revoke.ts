...
//
//   1. THE STEP-UP PROMPT, BY COUNTING IT. A sibling pass proved a comparable path never raised one by
//      wrapping the credential API and counting zero calls, so a claim that a prompt appears is worthless
//      unless somebody counted. This counts on both sides. The console side: the revoke really does run the
...
// whole surface exists for: on one measured estate 71 of 149 identities could sign in with no role row.
...
// comment-scanner mistake this workspace has hit before, so the reader here does not make it.
...
async function main(): Promise<void> {
  ...
  // ------------------------------------------------------------------------
  {
    ...
    if (subs === null) {
      ...
      // THE REQUIRE ARM DID NOT EXIST HERE AT ALL. Driven at console 7bf0734b with no sibling engine, this
      // file exited 0 with REQUIRE_ENGINE=1 exactly as it did without it, printing one lower-case skip line
      // among its ok lines. What goes unchecked is whether the panel's copy about the step-up prompt agrees
      // with the routes the engine actually gates, which is a claim made to an operator mid-incident.
      if (process.env.REQUIRE_ENGINE === "1") {
        verdictCannotCheck(
          `validate-signin-factor-revoke: REFUSED, REQUIRE_ENGINE=1 and the engine's STEPUP_SUBS could not be read (${rooted ? "an engine is reachable but the Set would not parse" : "no engine checkout is reachable"}).\n` +
      ...
      // half was right, and it caught the failure a sibling pass found by counting zero credential calls
      // behind copy that promised a prompt. What it did not do was survive the engine changing its mind.
      // Its own note said "if the engine later admits the route, this assertion goes quiet", and that is
      // exactly what happened: engine a83c143 added /signin-factors/revoke to STEPUP_SUBS thirty minutes
      // after console cd9ca1d wrote the copy that leans on its absence, `gated` went true, and the
      // assertion passed on copy that had become false. A one-directional consistency check is a check that
      // stops checking the moment the thing it watches moves.
      //
      ...
      // wordings), which is the comment-as-code trap this repo has paid for elsewhere.
      //
      // WHAT THIS BLOCK IS NOW, having been the only thing of its kind for a while: the SPECIMEN, not the
      // coverage. It is hard-coded to this file and this route, so measured against the fifteen call sites
      // e0ca490 rewired it covered none of them, and nine confirmations enumerated their consequences in
      // detail with no mention of the prompt. scripts/stepup-announce-gate.mjs generalises the predicate
      // below over every confirmation that sits in front of a gated write, deriving that set from the
      // engine's own STEPUP_SUBS, its inline-gated and reviewed-exempt sets, and the two dynamic approve
      // gates. This block stays because it drives the REAL panel and the real ceremony either side of the
      // copy claim, which a source-text gate cannot; openRevokeModal is that gate's positive control.
