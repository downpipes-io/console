...
// build does not know", and that row rides into the support pack. The audit-method family, the engine's
// secret-VANISHED event (target field "secret-absent"), and six target KINDS the current engine writes sit
// beside each other in the same label table and union, so this suite checks them together: missing any one of
// them files a false skew row against an engine and a console in lockstep.
//
// The secret-VANISHED event is the one that says a deploy dropped SIGNER_PRIVATE, the break-glass key or the
// destination binding: it is WHY the backups stopped and WHEN. Without it, support holding the pack of a
// customer whose backups stopped could tell them to upgrade a console that is already current.
//
...
// here, because a count literal standing in for it can go stale while staying green.
// ---------------------------------------------------------------------------------------------------------
...
if (vocab === null) {
  ...
  //
  // With REQUIRE_ENGINE set and no engine checkout reachable, exiting 1 would say the console is wrong; the
  // truth is that the comparison did not happen, which this reports as a distinct CANNOT-CHECK outcome rather
  // than a false pass or a false failure.
  ...
  if (process.env.REQUIRE_ENGINE === "1") {
    verdictCannotCheck(
      "validate-audit-mirror: REFUSED, REQUIRE_ENGINE=1 and no engine checkout is reachable.\n" +
...
// ---------------------------------------------------------------------------------------------------------
function g027(): void {
  console.log("\nthe secret that VANISHED:");

  const vanished = event("engine-secret-absent", { kind: "engine-state", field: "secret-absent", detail: "signerConfigured" });
  eq("a CURRENT engine's secret-VANISHED event records NOTHING, not an 'upgrade your console' skew", render(vanished).length, 0);
  ...
  ok("the ACTION is labelled, not hyphen-stripped to 'engine secret absent'", actionLabel("engine-secret-absent") !== "engine secret absent");
...
// ---------------------------------------------------------------------------------------------------------
// THE SIBLING SITE, WHICH IS WHERE FIVE ROUNDS DIED. The engine writes NINETEEN target kinds. The console knew
// thirteen. The six it did not know fall to describeTarget's default arm, which renders "unrecognised target
// (newer engine?)" AND records the skew row -- on the engine's most ordinary events: every IdP sign-in, every
// applied restore's receipt, every archive-destination change, every dual-control owner action, every push
// destination mutation, every spent-credential attestation.
// ---------------------------------------------------------------------------------------------------------
function everyTarget(): void {
  ...
  const targets: Array<[string, AuditTarget, AuditAction]> = [
    ...
    ["authn-attempt", { kind: "authn-attempt", ceremony: "sso-callback", connKind: "saml", connId: "okta-1", attempts: 3 }, "authn-failure"],
    ["credential-cleanup", { kind: "credential-cleanup", itemId: "it-1", tokenRef: "tok-desc" }, "expiry-cleanup-attested"],
    ["secret-rotation", { kind: "secret-rotation", secretId: "admin_token" }, "secret-rotation-confirmed"],
  ...
  // THE COVERAGE CLAIM COMES FROM THE ENGINE. An assertion like
  //   eq("nineteen target kinds driven (the engine's whole union)", targets.length, 19)
  // counts an array against a literal set to its own length: it agrees with itself and can never fail, and
  // gives no evidence the array is actually complete. The engine's AuditTarget union holds twenty-four kinds,
  // so counting only the local array would miss prune-approval, custody-share, posture-ack, attest-session and
  // test-fault while still asserting full coverage. Reading the expected count from the engine's own union
  // means this cannot be satisfied by counting the local array alone.
  if (vocab !== null) {
  ...
  // These sit on the engine's most ordinary paths, so name them as such.
  ok("an IdP sign-in does not file skew, despite being one of the engine's most common events", render(event("idp-sign-in", { kind: "idpconnection", connId: "okta-1", connKind: "oidc", op: "signin" })).length === 0);
  ok("a forced destination removal does not file skew, despite changing where backups go", render(event("dest-config-cleared", { kind: "dest-change", op: "remove", id: "d2", force: true })).length === 0);
}
...
function everyAction(): void {
  ...
  // THE COUNT COMES FROM THE ENGINE. A literal count such as `AUDIT_ACTION_OPTIONS.length === 86` looks like
  // a second opinion on the union's size, but it is not one: a count derived from the console's own union,
  // compared against a literal also chosen to match that union, agrees with it however wrong both are, and
  // stays green as the engine's real set moves past it. A literal count is a pattern that hides drift rather
  // than catching it.
  //
