// RG17: the dual-control posture card, driven through the REAL dualControlCard.
//
...
// the not-wired branch keys on. (A status PROPERTY is ignored, learned in RG9.)
...
async function render(opts: {
  ...
  // My first attempt invented { roles, caps } and the capability gate threw on a missing Set.
...
//
// Reading the region as one string and asking whether it contains "2" is not good enough, and I had that
// wrong first: with two requested restores and two pending config changes, a regression that counted all
// four approvals as pending still left a "2" in the region (the config tile's), so the assertion passed
// while the number an auditor reads was wrong. Each count must be read from its own tile.
