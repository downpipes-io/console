...
//
// THE DEFECT (measured live, nineteen controls). components/field.ts validates on blur
...

{
  ...
  // into field()'s validate arm at console 9f54c706. They came off REQUIRED_ONLY_BURNDOWN on
  // (Australian time), and the gate's own remediation line says an entry that leaves it
  // owes its message here, so here it is. Driven against the served bundle at console a3f64ae4 and
  // at the pre-fix 3b960100: both values below left the control SILENT on blur before the fix and
  // produced the messages named here after it, with the refusal arriving only at Attach in between.
  ...
  // The remaining five identifier controls, which came off REQUIRED_ONLY_BURNDOWN
  // (Australian time; UTC) and emptied it. Every one was driven against the served bundle at
  // console 0a36b60c before the repair and read CLEAN on blur with a precise error at Attach, so the
  ...
  // ===========================================================================================
  //
  // WHAT WAS RULED, Australian time ( UTC), at console b3286c06. All seven
  ...
  //
  // THE SPLIT IS NOT ONE SHAPE, and the table records the three it really is, measured by driving the
  // served bundle at b3286c06 (28 cases, one process each, 0 could-not-check):
...
{
  // push-secret, the SIEM audit-log push credential. It came off the REQUIRED_ONLY_BURNDOWN on
  // , so its message belongs here, and the control it belongs to is the most awkward one in
...
const EXPECTED: Array<{ id: string; file: string; factory: string; mustContain: string[] }> = [
  ...
  // channel-servicenow-username came off REQUIRED_ONLY_BURNDOWN (Australian time). It
  // is asserted HERE and not in Section A because its screen needs a live engine and an opened
  // overlay, which is the population this section exists for. Its sibling password field above it
  // already carried atMostChars for the same reason and this one had been left out: driven against
  // the served bundle at the pre-fix console 3b960100, a 257-character username passed blur AND
  // Save, and the only refusal came from the engine as a generic channel error.
  ...
  // The last three entries off REQUIRED_ONLY_BURNDOWN, which emptied it (Australian
  // time; UTC). All three are asserted HERE and not in Section A because their screens need
  // a live engine and an opened overlay. All three were ACCEPTING rather than merely refusing late,
  // driven against the served bundle at console 0a36b60c: a 65-character change number and a
  // 501-character justification were both taken on blur AND at Confirm, with the gate then proceeding
  // carrying the over-length value, and the OTLP auth secret took an 8,193-character value and a value
  // carrying CR and LF and POSTED both to the engine.
