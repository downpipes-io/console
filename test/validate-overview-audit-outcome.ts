// Validate the CONSOLE half of the NINETEENTH defect: the Overview "Last audited" tile composed a
...
// refused rows does NOT hold: another pass refuted it for the audit table, which renders the
// outcome in its own column and in the detail modal, and which is the screen an operator audits
// from. The claim survives on this tile alone, and that is what is graded here.
//
// THE SECOND HALF OUTLIVES THE ENGINE REPAIR, WHICH IS WHY IT IS A SEPARATE SECTION BELOW.
// `POST /admin/auth/recovery` is unauthenticated by design, and until engine 988b3fe8
