...
//
// validate-worst-day-refusals.ts covers the restore apply path; this file covers the nine sibling call sites
// with the same shape, establishing what each of them puts in front of a customer.
//
...
//
// WHAT EACH SITE DOES, and the verdicts are not uniform:
//
//   installKeys / addOperationalKey / setBreakGlassOnly (client-keys.ts)
//       Surface: components/keys/shared.ts renderTokenApply -> errorDetail. Each of the four 403s reads its
//       own class, rather than every one reading the role sentence.
//   deletePasskeyCredential (client-session.ts)
//       Surface: the revoke row's toast, which reads through refusalText rather than splicing the raw throw.
//       Splicing the raw throw would put the console's own classifier token in front of a customer (the same
//       trap validate-worst-day-refusals.ts guards against on the batch row) instead of the role sentence.
//   reattachMissing (client-sources.ts)
//       Surface: "Could not re-attach. " + errMsg -> refusalText, so a firewall block page reads as more than
//       the single word "forbidden".
//   setLicence (client-downpipes.ts)
//       Same, via engineRefusalText; the 400 verify-before-store reasons this fold exists for still ride
//       verbatim.
//   rollbackUpdate (client-update.ts)
//       Same, via updateRefusalText, on the recovery control for a live-but-bad version.
...
//       runSettleLadder's retry catch). Driven below rather than assumed, so the family reads one way even
//       though neither throw reaches a customer.
//
// A RELATED SITE, covered here though it needs no client change.
// screens/access-security/sessions-passkeys.ts refusedMessage is called on exactly one path (the `forbidden`
// branch of the two admin session levers) and reads through refusalText rather than splicing errText raw, so
// it never renders the transport's classifier token. The engine's owner-escalation guard reason answers 400,
// not 403 (engine src/admin/client-diag-vocab.ts states it), so on the one path this function runs there is
// never an engine reason to preserve.
//
...
// THE ROLE SENTENCE, quoted once. It is the exact text an unclassified 403 falls back to, which would
// wrongly reach operators whose role is fine, on nine controls.
...
async function main(): Promise<void> {
  ...
  // =================================================================================================
  console.log("\n6. refusedMessage: the admin session levers never print a classifier token");
  // =================================================================================================
  // Called on exactly one path (the `forbidden` branch of terminate-user / terminate-all), where the
  // transport folds a class and never the body's words. Splicing the raw throw instead would put
  // "terminate member sessions: forbidden-class=engine-capability: 403" in the parenthesis.
  {
    ...
    // The 400 guards land on the callers' OTHER branch, and their reason must survive there too.
    ok(
  ...
  // inline template in the row's catch block, so asserting a re-typed copy of that template would pass even
  // with the screen reverted to splicing the raw throw. So the REAL section is rendered, the REAL Revoke
  // button is clicked, the REAL confirm modal is answered, and the toast the operator would read is taken out
  // of the live toast region.
  {
