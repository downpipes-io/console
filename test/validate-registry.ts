...
// checks; this file fills the role-based-filtering gap in that coverage.
//
...
console.log("\n-- registry: requireCap honours a named custom role --");
{
  ...
  // create-downpipe command gates on requireCap("downpipe.write"), which resolves against the holder's
  // effective capability set rather than the base role alone, so the command is visible to it, WITHOUT
  // the holder gaining anything its capability set does not list (apply-restore, on restore.apply, stays
  // hidden).
