// validate-owner-floor: the console MIRROR of the single-owner dual-control deadlock guard in the UI.
// The engine is the authority; this proves the console stops the operator UPFRONT so they never hit a
// cryptic 400. Run with: node test/validate-owner-floor.ts
//
// Coverage:
...
// lock with aria-disabled rather than `disabled`, so it stays in the tab order and its reason is
