...
//
// WHY IT EXISTS, and it is not a coverage chore dressed up. src/screens/idp-connections/list.ts is a 315-line
// screen that NO TEST IMPORTED. It sat on the coverage-per-file baseline at 16.43 per cent, and on
// two commits added eleven lines to it: the sentences that tell an operator a passkey prompt is
// coming BEFORE they agree to enabling, disabling or removing a connection. The percentage fell to 15.87 and
// reddened the gate, and the reason it fell is the reason this file matters. Nothing stopped reaching the
// screen. Nothing had ever reached it, so the new copy went in with no assertion that it renders at all.
//
// The copy is the point rather than the percentage. Dismissing the passkey sheet writes nothing, and that is
// the fact that turns an unannounced browser prompt in the middle of an irreversible action from a fault into
// a checkpoint. scripts/stepup-announce-gate.mjs proves the sentence is present in the SOURCE. This proves it
// reaches the DOM an operator actually sees, which is a different claim: a sentence can be in the file and
...
async function main(): Promise<void> {
  ...
  verdictReached(c.failures, checksRun);
