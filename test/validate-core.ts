...
// Negative control: a second signOut is safe AND changes nothing. A literal `true` here could not fail:
// a second signOut that throws crashes the file (which the verdict guard catches), but one that quietly
// cleared engineUrl, or restored any of the six fields above, would go unnoticed. The state is captured
...
// A hand-maintained `ROUTES` export spot-checked by a handful of hardcoded keys is a list, not a rule:
// it can pass even after the export falls behind the routes the screens actually declare, since only the
// checked keys need to still exist. The full, derived route list lives at lib/app-registry.ts (ROUTES,
// computed from the screens' own descriptors), and scripts/route-table-gate.mjs is the non-vacuous gate:
// it independently re-parses every screen's `route:` field off disk and asserts the two agree.
