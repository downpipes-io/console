// Validate the CONSOLE half of the third expiry state a support pull credential can be in, and
...
// NaN is false, so an unparseable stored expiry reads as NOT EXPIRED at four engine readers at once. The
// engine avoids that by DISTINGUISHING rather than flipping: grantExpiryState reports `readable` and
// `expired` apart, the unauthenticated pull/scrape gate fails CLOSED with its own outcome
...
//
// The console must never throw that distinction away: rendering `grant.expired` alone would announce a
// credential whose stored expiry cannot be read as having "expired <the unparseable string>", with the
// value that does not parse printed as the lapse date -- a correct decision discarded before anyone can
// read it.
//
// WHAT THIS FILE GRADES, checked apart rather than as one pass:
...
// unreadable state at all: it silently becomes a different past date and fails closed. Every dose below
// therefore carries its own assertion about what Date.parse actually answers for it, so a dose that stops
// being unreadable is a red rather than a check that quietly grades the wrong state.
//
// NO SECOND MECHANISM. The console reads the engine's own vocabulary and renders it. It does not
// re-derive expiry from the raw string: that comparison is the NaN-blind check the engine avoids making,
// and a second derivation could disagree with the gate that refuses the pull.
//
...
// The wire payload the engine produces for an unreadable grant: redactGrant answers
