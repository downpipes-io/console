// Route-to-caller CONTRACT GUARD (stops the recurring "console-wiring-lag" defect).
...
// this tree satisfies the guard from PROSE: deleting the only real caller while its type comment
// survives left this check green. That is the same comment-as-code misread that has produced false
// negatives elsewhere in this workspace, in the one guard whose whole job is to notice a missing caller.
...
const REQUIRED_ROUTES: RouteCaller[] = [
  ...
  { path: "/admin/otlp-push", why: "the OTLP/HTTP metrics push destination (monitoring integrations, PLAN-2026-07-05.md): without a caller the zero-agent metrics push is built-but-unsurfaced, the same console-wiring-lag defect this guard exists to catch" },
  { path: "/admin/keys/vintages", why: "the keyless key-vintage inventory (G-P0-098/099 surfacing, census G-P3-001): the engine can say which runs are stranded to a key it no longer holds, and with no caller an owner who rotated and kept only the latest key could not see it" },
  { path: "/admin/policy/require-access", why: "the AUTH-1 lock-out pre-flight (census G-P3-002): the engine's authoritative verdict on whether closing the shared-token path would lock the operator out, which the console must consult BEFORE it advises the out-of-band act" },
];
...
for (const r of REQUIRED_ROUTES) {
  ...
  ok(
    present
      ...
      : `${r.path} has NO caller in src/api.ts: wire one (${r.why}). This is the console-wiring-lag defect: the engine route exists but no console caller does.`,
    present,
  );
}
