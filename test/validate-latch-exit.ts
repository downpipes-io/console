// DEFECT 23 REGRESSION COVER: an established account in recovery must be offered an exit its own engine will
// ACCEPT. Run with: node test/validate-latch-exit.ts
//
// THE DEFECT, as proven live against the `crud` estate. The recovery latch has three exits and
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // R-19: the sentence must carry the capability PHRASE, never the raw id, and the role by its display
