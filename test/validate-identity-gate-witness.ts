...
// WHY THIS EXISTS
// ---------------
// The restore apply/request panel must not gate on canDo("approver"), a cumulative role RANK: the engine
// gates on the restore.apply CAPABILITY, which restore-operator holds while sitting off that ladder at rank
// 0. Gating on rank alone would offer nothing to a role the engine accepts with a 200 on the dry-run, so
// screens/restore-flow/confirm.ts gates on canCap, and the screen-owned palette commands gate the same way
// one layer up (validate-screen-action-gates.ts).
//
// This file pins the layer below both. canCap and canDo fail closed on a null caller, correctly, but the
// witness that exists precisely to record a gate computed before the identity report arrived
// (lib/client-diag/identity-gate.ts) has to be wired into canCap and canDo themselves, not only into
// screen-local copies, or a silent blind gate has two consequences, and neither is cosmetic:
//
//   NO EVIDENCE. The identity-stale-gate row could not name screen "restore" whatever happened, so a raced
//   Owner and a genuine viewer would produce byte-identical support packs on the restore screen. That
//   equivalence is the exact confusion the row exists to end, on the screen that matters most for disaster
//   recovery.
//
...
//   screen a late blind gate would stay stuck instead of self-correcting.
//
...
console.log("TC-identity-gate-witness: a blind gate on the restore screen is recorded and recoverable");
...
// be measuring something other than the capability itself. The capability-drift gate proves the mirror equals the engine;
...
// Not the helper in isolation: renderConfirm is the real restore confirm screen, and its blind render must
// witness the same gate the helper does.
{
  onRestoreWithIdentityPending();
