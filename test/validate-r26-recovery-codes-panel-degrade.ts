// R-26: the public, no-login tour crashed on free-roam /security. Regenerate threw inside
...
async function testDemoRouteModelsRegenerate(): Promise<void> {
  ...
  ok("R-26 FIX: the body carries a USABLE recoveryCodes array (the reported gap was the benign\n" +
...
function testPanelDegradesHonestly(): void {
  ...
  console.log("\n-- recoveryCodesPanel: the EXACT R-26 reproduction -- codes:undefined, as regenerateRecoveryCodesFlow\n" +
    "   built it from result.recoveryCodes when the demo's benignWrite fallback answered --");
  {
    ...
    ok("R-26 FIX: recoveryCodesPanel({codes: undefined, ...}) does NOT throw (this is the live crash)", threw === null);
...
async function testRegenerateFlowEndToEnd(): Promise<void> {
  console.log("\n-- regenerateRecoveryCodesFlow: a malformed engine response degrades to an actionable\n" +
    "   toast, never an unhandled rejection or a false success (R-26 end-to-end) --");
  ...
  ok("R-26 FIX: no unhandled rejection from the click (was: TypeError from recoveryCodesText's .map)", unhandled === null);
