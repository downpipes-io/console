// TC-07: prove the console's client-side restore plan-hash mirror (api.ts restorePlanHash) matches
...
// hash (the re-arm-on-change UX, build-contract 1.3.5). If the two derivations drifted, the console
...
//      the SPEC and is fully hermetic (no engine worktree needed). A handful of golden literals guard
//      the reference itself against an accidental rewrite.
//   2. PARITY: when the engine worktree is reachable, the engine's own restorePlanHash is imported and
//      asserted equal to the console's over the same corpus, so the two implementations cannot drift.
//      The import is guarded: if the engine worktree is absent, the parity block is skipped with a
//      note (the reference block still pins the contract); it never fails the console suite.
//
...
// value is pinned even without the engine worktree. canonicalJSON here is the engine's canonjson.ts
...
async function main(): Promise<void> {
  ...
  // ----------------------------------------------------------------------------------------------
  // REGRESSION console-src-046-01: the requester-side hash (the mirror the restore screen shows the
  ...
  // ----------------------------------------------------------------------------------------------
  console.log("\n-- console-src-046-01: requester hash == approver hash (recordName + cfConfig + mediaRestore forwarded) --");
  ...
  const forwardNew = (full: FullReq): PlanReq => {
    ...
    // (confirm.ts r.mediaRestore = { accountId: req.mediaRestore.accountId }) -- the SAME
    // console-src-046-01 failure class, for the media-restore feature added alongside this fix.
  ...
  // Guarded import: skipped with a note if the engine worktree is not present.
  // ----------------------------------------------------------------------------------------------
...
async function checkEnginePlanHashParity(): Promise<void> {
  ...
  // Resolution lives in test/engine-path.ts. ENGINE_WORKTREE still overrides. The sibling hop this used to do lands on
  // console/.worktrees/engine from a worktree, which in this workspace is a real engine checkout on an
  // unrelated branch, so the parity ran against whichever engine was parked there.
  const here = new URL(".", import.meta.url).pathname; // .../console/test/ or .../console/.worktrees/<n>/test/
  ...
  if (loaded !== null) {
    ...
    console.log("  note engine worktree approvals.ts not importable; skipping the engine-parity cross-check");
    console.log("       (the reference assertions above already pin the contract). Run the engine worktree");
    console.log("       alongside the console to exercise this block.");
    return;
  }
  // Capability probe: if the imported engine checkout PREDATES a field this console binds (its hash is
  // unchanged with vs without the field), it is on an older branch than this console feature, so parity for
  // vectors carrying that field cannot hold. Skip those vectors (the hermetic reference above still pins the
  // console) rather than fail a shared gate on an unmerged-engine artifact; point ENGINE_WORKTREE at the
  // matching engine worktree to exercise them now, and they run automatically once the engine feature lands
  // on the imported checkout.
