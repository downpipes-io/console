...
// WHY THIS MATTERS
// ----------------
// A role-NAME gate can hide a control the engine would allow: the restore apply/request panel gates on
// canCap("restore.apply") rather than canDo("approver"), a cumulative rank ladder, because the engine
// grants the restore.apply CAPABILITY to restore-operator, a role that sits off that ladder at rank 0.
// A role-name check on the panel would refuse a role the engine actually authorises. This exact surface
// (screens/restore-flow/confirm.ts) is covered by validate-api-flows-restore.ts sections 6R/6S.
//
// The same failure mode can appear one layer up, in the screen DESCRIPTORS' `when` gates, since nothing
// else drives them: validate-registry.ts covers shell/registry.ts COMMANDS only, and the screen-owned
// actions assembled by allScreenActions() (the ones the palette, the go-to chords and the "?" cheat-sheet
// all read) need their own role-based test. Two of the restore surfaces -- "Review pending restore
// approvals" and the Overview approvals reads -- must not exclude restore-operator, the recovery-only
// role that holds restore.approve and restore.request.
//
...
// declares a chord the static COMMANDS already own must lose only the chord, never be dropped whole by
// the reconciler, or "Restore from a run" (restore-flow.ts, group Actions, keywords recover / roll back /
// dry-run) disappears from the entire palette and the cheat-sheet: an operator searching for "roll back"
// or "dry-run" finds nothing.
//
