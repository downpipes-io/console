// The harness's programme 8 (concurrency and interleaving) regression cover for defect 43: the downpipe
// drawer's enable switch used to post back THE WHOLE CONFIGURATION THIS SCREEN LOADED, so an operator who
// flipped one switch reverted a colleague's edit to a field they never touched, and both were told they
// succeeded.
//
// WHAT THIS DRIVES. The REAL toggleEnabled from src/screens/sources-downpipes/detail-actions.ts against a
// fake EngineClient that reproduces the interleave exactly as it was measured live on estate `crud` on
// : the screen holds a configuration at cadence 86400, and by the time the switch is clicked the
// engine holds 3600 because a second operator changed it. The grading is THE SENTENCE THE OPERATOR READS,
// taken out of the rendered toast, and THE BODY THE ENGINE WOULD RECEIVE, taken off the fake client.
//
