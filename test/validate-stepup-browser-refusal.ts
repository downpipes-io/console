...
//
// Measured: Chromium refuses a `create` whose excludeCredentials list exceeds
// 64, and engine cf77c541 bounded that list. This validator is about the OTHER list.
//
// MEASURED HERE,, Chromium 149.0.7827.55 via a CDP virtual authenticator on a
// local origin, dose-response over allowCredentials sizes 0, 1, 32, 63, 64, 65, 66, 100, 128, 256:
//
//   sizes 1..64  -> the assertion completed
//   size 65+     -> DOMException named `RangeError`,
//                   "The `allowCredentials` attribute exceeds the maximum allowed size (64)."
//
// The same rig reproduced that excludeCredentials result exactly (first throw at 65) as its own
// control, and the refusal did not move when the genuine credential was placed last rather than first: it
// is a verdict on LIST SIZE, returned before any authenticator is consulted.
//
...
//
// AND THE DESCRIPTION IS THE DEFECT, exactly as it was for the register path. runStepUpOutcome caught every
// throw bare and answered `no-assertion`, whose copy is:
//
...
//
// No prompt appears, because the browser threw before opening one. So the operator is told to enrol another
// passkey, which takes the account from 65 to 66 -- further above the ceiling, in the only direction that
// cannot be undone. That is not merely a misdescription; it prescribes the action that deepens the harm.
//
// The register path already tells a browser refusal from a transport fault (passkeyErrorMessage, ace89505).
// This file drives the SAME DOMException down BOTH paths, so the register path is a differential control:
// it proves the misdescription belongs to the step-up path rather than to the error.
//
...
// THE EXACT DOMException Chromium threw in the sweep, name and message reproduced verbatim.
...
async function main(): Promise<void> {
  ...
  console.log("\nDIFFERENTIAL CONTROL: the SAME class of error down the REGISTER path, which ace89505 already fixed --");
  {
