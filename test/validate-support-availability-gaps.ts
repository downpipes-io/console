// Validate the AVAILABILITY support-pack gaps from the diagnostic-coverage audit. Each of these is a case
// where the console is UP and something in it is dead, and the pack afterwards holds nothing that says so: an
// applied console update that is not the build being served, an identity report that never resolved (so an
// Owner sees a console with every control greyed out), a bring-up step that failed silently, a restore Apply
// that will never arm however many approvers sign, an engine wire value the console quietly coerced, and a
// screen frozen on skeleton rows.
//
// THE RULE THE SUITE DEFENDS: a gap is not closed until its evidence is IN THE BUNDLE, and the evidence
// answers the question the gap asks. So every assertion below drives the REAL fault site (never a
...
//
// Coverage:
//   the post-apply console build check, and the rollback it offers, reach the pack with a class that
//         says WHICH way the check failed (wrong build served / not JSON / unreachable / no version stamp)
//   a caller-identity read that FAILED is told apart from a genuinely low role
//   a swallowed lazy-chunk preload, and a navigation bridge that was never installed
//   a restore Apply that will not arm says WHY, and a plan genuinely awaiting its approver says nothing
//   an engine wire value the console could not use is recorded by field class and anomaly, never valued
//   an uncaught fault carries its error class and the channel it arrived on
//
...
// ============================================================================================
section("1. the console build check: WHICH way it failed, not just 'not confirmed'");
// ============================================================================================
//
...
// ============================================================================================
section("2. an identity report that FAILED, told apart from a genuinely low role");
// ============================================================================================
//
...
// ============================================================================================
section("3. the two silent bring-up failures");
// ============================================================================================
...
// The discrimination test: a failed preload and a dead bridge must not be one row.
...
// ============================================================================================
section("4. why Apply did not arm");
// ============================================================================================
...
// expiresAt IS RELATIVE TO NOW, AND IT HAS TO BE. A wall-clock literal drifts into the past as the calendar
// catches up to it, and a lapsed approval is excluded as a precondition of gateBlockClassFor (correctly, and
// in lockstep with findUsableApproval, which is that function's whole contract), so a fixture pinned to an
// absolute instant would eventually become a lapsed record whose asserted class stops being reachable, and it
// would fail long after anything explained why. The lapsed case is not lost by using a relative deadline: it
// is asserted deliberately below, from a deadline in the past by construction rather than by the calendar
// catching up.
...
// ============================================================================================
section("5. engine wire values the console silently coerced");
// ============================================================================================
...
// THE DISCRIMINATION TEST: the six field classes are six rows, not one bucket.
resetRing();
setActiveScreen("/runs");
...
// ============================================================================================
section("6. an uncaught fault says WHAT class, and on WHICH channel");
// ============================================================================================
