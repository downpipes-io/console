// RG3 (account-tier attach half): the multi-source attach on the Sources account tier, driven through the
// REAL accountTier.
//
...
// with `bound` naming what the engine already has. My first fixture put kv[] at the top level, so the tier
// rendered no controls at all and the test failed for a fixture reason rather than a product one.
...
function build(names: string[], result: () => Promise<unknown>): {
  ...
  return {
    ...
    // finds it before the first tick and loses it immediately after. That is what made my first attempt
    // read as "ticking does not enable the button" when ticking worked perfectly.
