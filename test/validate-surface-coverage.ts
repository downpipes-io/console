...
// ---------------------------------------------------------------------------
// A FAILED source-discovery read must say so, and say why.
//
// surfaceCoverage alone cannot: it discards the settled discovery ERROR and answers "unknown" for
// every surface, so the hero would show nine mute "Coverage unknown" tiles, the same picture an
// operator sees if their account is simply unreadable for benign reasons. The overwhelmingly common
// cause (an expired, revoked or rescoped Cloudflare discovery token) would go unnamed, leaving the
// operator nothing to act on.
//
// discoveryFailureClass/discoveryFailureNote close that gap: they classify the failure through the
// console's OWN closed error taxonomy (never the message text, so no engine or Cloudflare API string
// reaches the copy) and render one honest sentence naming the control that fixes it. A SUCCESSFUL read
// yields null: no note, no cry of wolf.
// ---------------------------------------------------------------------------
{
...
// ---------------------------------------------------------------------------
// NO DISCOVERY TOKEN is the DEFAULT state for an engine predating the account tier.
//
// Every disc() helper above defaults tokenPresent:true. Without a discovery token, a healthy current
// estate would otherwise show four surfaces its engine supports as "not on this engine" rather than as
// simply lacking a token. estate-build-drift verifies this against a live estate: the Overview summary
// chip reads "9 surfaces · 2 protected · 1 added · 1 not added · 5 not on this engine" when the token is
// missing.
...
// ---------------------------------------------------------------------------
// THE SUMMARY CHIP is the at-a-glance line above the fold, and its arithmetic must hold under three
// separate failure modes.
//
// Unknown must be named whenever ANY surface is unknown, not only when every surface is: a mix must
// never render the unread ones as "0 added · 0 not added", since "0 not added" reads as no gaps. The word
// "protected" must never describe a surface that merely has a downpipe, since that is coverage, not
// health, and the tiles below never claim it either. And the surface total in the chip must be COUNTED
// from the grid actually rendered, never a literal beside it, the same class of error as the "about 195
// surfaces" string elsewhere in this codebase.
