...
// maths, the sort comparator, and the bulk-action rejection handling. Extracted
// from validate-components.ts (finding console-test-003-01) with no change to
// any assertion.
//
// Gap audit reference: downpipes-AUDIT.md, CON-L19.
...
// ===========================================================================
// 8. DATA TABLE -- BULK ACTION REJECTION HANDLING (CON-L19)
// ===========================================================================
...
console.log("\n-- data-table bulk action rejection handling (CON-L19) --");
...
// 8a. A rejected run() surfaces the error and does NOT clear the selection.
{
  ...
  ok("CON-L19: rejected run does NOT clear selection", result.selectionCleared === false);
  ok("CON-L19: rejected run records the error (error surfaced)", result.errorLogged === true);
  // Negative control: if the rejection were swallowed, errorLogged would be false.
  ok("CON-L19 negative: errorLogged is NOT false (error was not swallowed)", result.errorLogged !== false);
}
...
// 8b. A rejected run() with a bare-string throw still surfaces correctly.
{
  ...
  ok("CON-L19: bare-string rejection records the error", result.errorLogged === true);
  ok("CON-L19: bare-string rejection does NOT clear selection", result.selectionCleared === false);
}
...
// log an error.
{
  ...
  ok("CON-L19: resolved run (keepSelection=false) clears selection", result.selectionCleared === true);
  ok("CON-L19: resolved run does NOT log an error", result.errorLogged === false);
  // Negative control: a spurious error on success would make errorLogged true.
  ok("CON-L19 negative: errorLogged is NOT true on success", result.errorLogged !== true);
}
...
// 8d. A resolved run() with keepSelection=true does NOT clear the selection.
{
  ...
  ok("CON-L19: resolved run (keepSelection=true) does NOT clear selection", result.selectionCleared === false);
  ok("CON-L19: keepSelection=true + success => no error logged", result.errorLogged === false);
}
