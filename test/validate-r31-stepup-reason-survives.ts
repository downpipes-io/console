// R-31 / SU-DETECT-01: client-attest.ts's createAttestSession (and every sibling that shares the same
...
async function testReadErrorReasonUnit(): Promise<void> {
...

  {
    ...
    ok("R-31 FIX: a 401 { stepUpRequired:true, error:'step-up required' } reads as null (was: 'step-up required')", reason === null);
  }
  {
...
async function expectMarkerSurvives(site: Site): Promise<void> {
  ...
  ok(`${site.name}: isUnauthorised(err) is FALSE (the R-31 sign-out defect)`, !isUnauthorised(caught));
}
...
// EngineClient/Transport/client-attest.ts stack over a scripted global fetch, exactly as the R-28 test
...
async function testEndToEndAttendStart(): Promise<void> {
  ...
  ok("R-31 FIX: goSignedOut() was NOT called -- the valid session survives", !signedOut);
  const bodyText = textOf(root);
  ok("R-31 FIX: 'Verify your identity' renders on the Start action's own screen", bodyText.includes("Verify your identity"));
