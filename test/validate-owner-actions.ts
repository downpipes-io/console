...
// no fetch). Split out of main() so each section is its own named function under the 50-line
// flag (console-test-006-02); the bodies are unchanged.
function testPureHelpers(): void {
  ...
  console.log("\n-- PURE: ownerActionKindLabel --");
    ...
    // idp-conn-cert (the zero-downtime SAML signing-cert rollover, engine src/admin/owner-action.ts) is
    // present in both OwnerActionKind and OWNER_ACTION_KIND, so a live rollover proposal renders its
    // approval-queue card heading as a plain-English description of what the approver is authorising,
    // rather than the raw string "idp-conn-cert".
...
// test202Handling covers the contract: a deferred (202) owner mutation resolves to a queued
// result, never a false applied success; a 200 stays the applied path; a non-2xx still throws.
async function test202Handling(): Promise<void> {
  ...
  // ------------------------------------------------------------------------
  {
    ...
    // The governance-lie class, the same class parseJsonOrPending's own decoder guards against, verified
    // by the write-fault capability on addDownpipe's sibling decoder: a 202 whose body does NOT
    // carry the OwnerActionQueued shape must throw an honest error, never resolve to a fabricated "result"
    // value. `{}` is the same shape the write-fault probe used against parseJsonOrPending; falling through
    // to the ordinary 2xx path would resolve it as { status:"result", value:{} } -- a false "saved"
    // for a destination repoint, an IdP connection, or the account-browsing token, none of which moved.
    {
