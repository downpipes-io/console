...
// exactly as it does in our gates.
//
// HOW IT GRADES, and this is the part that makes it different from the 235 assertion-driven specs beside it.
// It does not assert that a particular string appears. It renders the REAL production renderer over a family
// of labelled states, canonicalises what the operator would read (the visible text plus the status-dot HUES,
// because half of a tile's sentence is its colour), and looks for a COLLISION: two states of different truth
// class that produce a BYTE-IDENTICAL answer. Two states that answer identically is the signature: a
// destroyed version and a version that never existed can answer in the same few bytes, and that collision is
...
//
// THE FOUR STATES THIS PINS, each graded on the real renderer:
//   overview.updates-tile        versionSkew "uncomparable" must not answer exactly as "current" does, in ok green
//   overview.needs-me            a refused read must not answer exactly as a healthy fleet / a brand-new estate does
//   security.recovery-code-count an unreadable count must not earn the ok green a healthy eight earns
//   overview.config-tile         a status payload naming no gap must not still print "Missing: . Open to fix."
//
...
// figures, the recovery-posture read tiles). If a change here ever starts crying wolf on a healthy estate,
// or narrows one of those, this file goes red on the control rather than on the finding.
//
...
// ---- surfaces beyond overview/security/licence (see below) ----------------------------
...
interface Surface {
  ...
  // grading axis and it exists because the byte-identity axis alone can miss a real defect.
  //
  // A recovery-code count of NaN can render "NaN unused codes remaining" beside an OK GREEN dot. Against a
  // healthy eight the full answer differs (the digits differ), so byte-identity alone would score it as
  // told apart even though the hue vector is identical, and the hue is what an operator reads at a GLANCE,
  // before any word. The honesty rule that stat-tiles.ts states in its own header, "a tile NEVER shows
  // green for unknown", is a rule about exactly this projection, so the census grades it as its own axis.
...
// ---- fixtures for the additional surfaces -----------------------------------
...
const SURFACES: Surface[] = [
  ...
  },
  {
    ...
    cells: [
      ...
      // This state is enrolled because a gap list that comes back empty must still produce a real sentence,
      // not a promise with nothing named. Caught by the third pass below, not by the collision passes:
      // "Missing: . Open to fix." collides with nothing, so it is simply not a sentence.
  ...
  // ==========================================================================
  // ADDITIONAL SURFACES: customer-visible health verdicts beyond Overview, Security Centre and Licence.
  // ==========================================================================
  //
  // Nine surfaces are enrolled above, on Overview, Security Centre and Licence. The rest of the console
  // reads a health verdict off routes those do not walk: the run pickers, the map drawer, the reports
  // panel, the canary, the notification trail, the key vintages and the restore recency. Everything below
  // enrols one of those surfaces.
  //
  // THE SIBLING RULE IS WHY SEVERAL OF THESE ARE HERE. A numeric band comparison (`>=`) is FALSE for NaN
  // and TRUE-by-coercion for null and for a numeric string, so an unreadable count can fall through to the
  // healthy arm unless a readability guard sits in front of it. Several functions on the tree run the
  // identical comparison against a different number, and a guard on one of them does not imply a guard on
  // the rest, so each sibling is enrolled here individually.
  {
  ...
  },
  {
    ...
    // grading scoreBadge alone cannot see a defect that only shows up in this tile: a check that cannot
    // see the failure it grades is not a check. The tile re-derives `Math.round(report.score) / 100`
    // itself, so it is a separate answer on the same screen.
    //
    // NO GLANCE PROJECTION HERE. The score band is a whole card: one badge plus four stat tiles, and the
    // four tiles report check counts, which have nothing to do with the score. Their ok hues would sit in
    // the hue vector alongside the badge's, so a reassuring-hue test over the whole card can match on the
    // tile counts while the score's own honest warn is doing its job correctly, which would blunt the axis
    // for every other surface it grades correctly. The score's OWN hue is graded on security.posture-score
    // above, where the projection is the badge alone and nothing else can drown it; here the byte-identity
    // axis tells all five faulted states apart on the text, which is what this surface exists to check.
  ...
  },
  {
    ...
    // eight lines below it, doing the identical thing to a different closed set. Checking the siblings of
    // every finding matters here because the cheapest place a sibling hides is the next function down.
  ...
  },
  {
    ...
    cells: [
      ...
      // A FlowStatus this build does not know is not enrolled here, and the reason matters: statusPresent
      // (map/panels.ts:122) is an exhaustive switch with no default, so an unknown value returns undefined
      // and freshnessLine dereferences .tone off it, a real crash. It is NOT REACHABLE, though: FlowStatus
      // is not a wire value, it is DERIVED console-side by deriveStatus (map/data.ts:419) out of a closed
      // set, and the only wire enum behind it already goes through classifyFreshness's unknown-enum guard.
      // Enrolling an unreachable state here would cry wolf on a state the product cannot be in, the same
      // wolf this file declines to cry when it does not grade `neutral` as reassurance. The surface stays
      // enrolled on its real states, where it is a CONTROL: it already tells a never-run pipe from an
      // unreadable history.
    ],
  },
  {
  ...
  // ---- FURTHER CONTROLS: surfaces that already discriminate --------------
  {
...
// THIRD PASS: IS THE REFUSAL HONEST? A collision detector cannot see this class: "Missing: . Open to fix."
// is byte-identical to nothing at all, so it never collides with a healthy answer. It is not a lie about
// health, it is a sentence that promises to name something and then names nothing, which is a refusal an
// operator cannot act on, so it needs its own pass.
//
...
// nearer relative, and it is here because a real defect sits just outside the third pass's reach. The restore
...
// It carries no colon and no "Missing", so the third pass cannot see it.
//
...
// is something prose does on purpose. IT DOES NOT CRY WOLF: it runs over every answer this file renders,
// across every surface enrolled here, and finds nothing there.
...
// FIFTH PASS: IS A JAVASCRIPT NON-VALUE BEING READ AS A FIGURE?
//
// With a readability guard in front of scoreStatus, an unreadable score already wears a different HUE from
// every healthy one, so the collision axis alone tells the states apart no matter what figure the tile
// prints beside them. A tile could go on printing the literal "NaN / 100" and the collision axis would
// still call it told apart, and it would still be a lie: "NaN" is not a score, and an operator reading it
// has been shown the console's internals where a number belongs.
//
...
//
// `null` IS DELIBERATELY NOT IN IT: it reddens four cells on licence.estate-figures, and all four are
// measurement artefacts of this file rather than real defects. That surface's projection is a
// JSON.stringify of a structure, and a function honestly returning null (the line is not rendered at all)
// encodes as the four characters "null" inside this file's own debug encoding rather than on any screen.
// `null` is legitimate JSON output; NaN and undefined are not, so narrowing to those two drops the false
// positives without dropping a real defect: every defect this pass exists for prints one of the two, "NaN /
// 100" on the posture tile and "NaN unused codes remaining" on the break-glass card.
console.log("\n-- non-values on screen (a JavaScript nothing printed where a value belongs) --");
// NO LEADING \b: written as /\b(?:NaN|undefined)\b/ the pass reads clean over a tree with the defect
// present, because answerOf canonicalises DOM textContent and adjacent nodes concatenate with no separator:
// the posture tile renders "ScoreNaN / 100", the character before the N is a word character, and the word
// boundary never matches. A trailing lookahead is enough to keep it from firing inside a longer word.
