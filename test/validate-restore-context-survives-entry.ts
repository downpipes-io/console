...
//
// Four measured symptoms, each captured as both request bodies:
...
//
// THREE MORE ENTRIES INTO THE SAME DEFECT, found by sweeping the console for its shape and each measured the
// same way, by capturing request bodies through the real components:
...
async function main(): Promise<void> {
  ...
  // reads it back and ADDS a scope the operator did not type here. Measured before the fix: a landing
  // request of {"runId":"..."} re-mounted into {"runId":"...","include":["session:"]}, and the narrowed
  // plan is the one that painted. Narrower is the less destructive direction, but it is still a plan the
  // operator did not build, so it must not claim a URL either.
  ...
  // three things at once. Measured before the fix: one request, {"runId":"..."}, painted, with every field
  // on the re-mounted screen empty.
  ...
  // run id alone is what a fresh mount would rebuild and only the draft makes it disagree. Measured before
  // the fix: {"runId":"...","include":["session:"]}, a scope the operator never typed on this screen.
  ...
  // field is absent from the plan the approver arms against. Measured before the fix: POST 1 carried
  // target.binding and POST 2 was {"runId":"...","recordName":"session:42"}, so one record went back over
  // its LIVE ORIGINAL binding moments after the operator reviewed a plan showing the redirect on every row.
