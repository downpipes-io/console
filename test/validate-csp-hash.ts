// Validate that worker.ts's CSP_SCRIPT_THEME_HASH matches the ACTUAL bytes of the single
// sanctioned inline pre-paint script in public/index.html. The hash and the script live in
// different files, so nothing else catches drift between them: growing the inline script (the
// a11y pre-paint block) without regenerating the hash makes the strict CSP silently BLOCK the
// pre-paint script for every visitor - no theme or a11y attributes before first paint, and a CSP
// violation in every browser console - while every other suite stays green. Only a cross-file
// check can pin them together. Run with `node test/validate-csp-hash.ts`.
...
// blocking the inline script under every hash value. A correct hash value alone does not prove
// the quoting is right, and the browser surfaces the break only as a console warning, so assert
// the policy template interpolates the constant inside single quotes directly.
