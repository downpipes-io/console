...
export async function runOverlays(h: Harness, ctx: Ctx): Promise<void> {
  ...
  {
    // openOverlay's KEY guard: app.ts's identity-resolved quiet re-render repaints the current screen
    // via a fresh screen.render() -> a fresh load() -- a path that runs OUTSIDE the router, so
    // installAfterEach's closeAllOverlays never fires for it. A route-driven "open the deep-linked
    // drawer" side effect then re-fires while the first drawer is still mounted. A caller that names a
    // `key` gets an idempotent open: a second call with the SAME key, while the first is still open, is
    // a no-op that hands back the EXISTING overlay's handle instead of stacking a duplicate,
    // visually-covering surface on top of it.
  ...
  // =========================================================================
  // The drawer has no tabset: no caller passes `tabs` (see the component's own header for the
  // reasoning). The no-tablist assertion below guards that: it fails if a tabset is reintroduced
  // without a caller.
  console.log("\n-- detail-drawer --");
  {
    // On a holder: a `let` written only from inside a callback keeps the narrowing from its
    ...
    // THE RULE IS "NOT HOVER-ONLY". The drawer action goes through the shared refuseWithReason
    // (lib/dom.ts), which sets the title deliberately for a mouse user; the title is not the thing
    // forbidden. design-system.md 6.15 forbids a tooltip being the ONLY place essential information
    // lives, not tooltips themselves, and every control refused through that helper carries one.
    //
    // So the assertion checks the property that matters: the reason reaches a keyboard and touch user
    // WITHOUT hover, as real DOM text and as the control's accessible description. In Chromium this
    // button computes name "Delete downpipe" with description "<the reason>".
  ...
  {
    // openDetailDrawer's `key` end to end: reproduces the real
    // production shape, not just the raw openOverlay primitive. sources-downpipes.ts's load()
    // calls openDetail (which calls openDetailDrawer with key: `downpipes-detail:${dp.id}`) once
    // on the deep-linked first render, then AGAIN when app.ts's identity-resolved quiet re-render
    // repaints the screen (a second, independent load() that still matches the same
    // /downpipes/:id route). The second call must not stack a second drawer over the first,
    // which is what covered the Edit button (Playwright: "subtree intercepts pointer events";
    // overlayCount=2/dialogCount=2 at the failure instant).
