...
// available headless, so the create/get functions are not invoked here; we test the pure,
...
console.log("\n-- enrolmentGuidance: honest, house-style copy --");
{
  ...
  console.log("\n-- enrolSecurityKey / deriveWrappingKeyFromAssertion: worth-attempting path --");
  {
    ...
    // Leave the globals as we found them so the process exits without a lingering stub.
