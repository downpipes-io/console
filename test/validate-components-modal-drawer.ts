...
// contract. Extracted from validate-components.ts (finding console-test-003-01)
// with no change to any assertion.
//
// It also carried CON-L17, the detail drawer's onTabChange initial-open suppression,
// until the drawer tabset was removed for having had no call site in
// the whole history of src/ (see src/components/detail-drawer.ts's own header). The
// section went with the code it tested rather than being left asserting over a seam.
//
// Gap audit reference: downpipes-AUDIT.md, CON-M12.
...
// ===========================================================================
// 6. MODAL -- ACTION ERROR SURFACING (CON-M12)
// ===========================================================================
...
//
// Negative controls confirm the error is NOT swallowed (notifyError must fire
// on rejection), matching the CON-M12 finding that previously the error was
// silently discarded.

console.log("\n-- modal action error surfacing (CON-M12) --");
...
// 6c. _testRunNonBusyAction: a rejected onClick is NOT swallowed; notifyError is called.
{
  ...
  ok("CON-M12: rejected non-busy action returns false (modal stays open)", result === false);
  ok("CON-M12: rejected non-busy action calls notifyError (not swallowed)", surfaced !== null);
  ok("CON-M12: surfaced message matches the thrown Error.message", surfaced === "engine unreachable");
  // Negative control: if the error were swallowed, surfaced would still be null.
  ok("CON-M12 negative: surfaced is NOT null (error was not swallowed)", surfaced !== null);
}
...
// 6d. _testRunNonBusyAction: a non-Error rejection (bare string) is surfaced.
{
  ...
  ok("CON-M12: bare-string rejection surfaced correctly", surfaced === "forbidden");
}
...
// 6g. BUSY path (CON-M12 / console-src-005-11): the spinner branch must, on a
...
console.log("\n-- modal busy-path control recovery (console-src-005-11) --");
