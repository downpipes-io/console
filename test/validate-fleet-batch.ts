// RG23: the fleet-wide bulk loop, driven through the REAL runBatch.
//
...
// classifyError reads the status off the MESSAGE, not off a property (learned in RG9), so these are the
...
async function main(): Promise<void> {
  ...
  console.log("\n(2) a per-item failure is NAMED, never folded into a blanket success");
  {
    ...
    // the action that raised them, so an absence check here would fail for an unrelated reason. This is the
    // third time that trap has appeared in Phase 5, hence the explicit measurement.
