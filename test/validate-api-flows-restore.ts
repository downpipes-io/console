...
//       media restore shows the operator
//
...
export async function runRestoreFlows(
  ...
  // requestRestore payload, reading isLatest/plannedWrites/bytes from the dry-run plan
  // and attaching them when the plan carries a real value. Dropping them would show the
  // approver inbox fabricated zeros and flag every request as a non-latest restore.
  //
  ...
  // dropping it leaves the engine approval keyed on a planHash that never matches the hash
  // the operator was shown, so the dual-control gate sticks. This
  ...
  // 6h2b. HALF-FILLED cf-config context, THE OTHER HALF: an account id typed with the edit
  ...
  // gate, even though the engine applies them. The role-gated banner (the "permission to apply a restore"
  // phrase) only renders once the gate is PASSED (an Operator reaching renderRoleGated), so its presence
  // proves the short-circuit did not fire; "writes nothing" is the short-circuit copy. A regression back to
  // keying on plannedWrites alone flips both.
  {
    ...
    await driveRequestPanel(plan, { runId: "RUN-B57C" }, "config-only restore");
  ...
  }
  {
    ...
    await driveRequestPanel(plan, { runId: "RUN-B57M" }, "media-only restore");
  ...
  }
  {
    ...
    await driveRequestPanel(plan, { runId: "RUN-B57E" }, "empty plan");
  ...
  // 6i2. HALF-FILLED D1 subset: tables without a database (or the reverse) would otherwise fall through
  ...
  // 6j. MEDIA RESTORE request (the media restore UI): fill the real
  ...
  // mediaRestore { token, accountId } (media restore is API-only before this UI existed), (3) the
  ...
  // 6j2. HALF-FILLED media-restore context: an account id typed with the edit token left
  ...
  // failures", AND no record content is shown. The sentinel value/name are never echoed.
  {
    ...
    // The crystal-clear NO-data assurance copy is present. The no-data fact is stated ONCE, by the
    // card's standing line + badge; the verdict does not restate it, so the assertion targets the
    // standing line.
  ...
  //
  // A role-NAME gate on renderConfirm's apply/request panel -- canDo("approver"), a cumulative role
  // RANK (common.ts hasRole) -- disagrees with the engine, which grants restore.request +
  // restore.apply to the restore-operator role, sitting OFF that ladder (roleRank 0). A disposable
  // restore-operator's dry-run is accepted by the engine (HTTP 200), so the console must not offer it
  // NO request path and NO apply path: the authorised recovery-only DR role has to be able to
  // complete a restore. The panel gates by the capability the engine enforces (canCap("restore.apply")
  // for the apply path, canCap("restore.request") for the request path), so the role's real
  // capability set drives what it is offered.
  //
  ...
  console.log("\n-- 6R: restore role-vs-capability gate (real renderConfirm) --");
  {
    ...
    const PLAN_HASH = "sha384:rdiv-1-fixed-plan-hash";
  ...
  //
  // The single-run confirm/apply panel gates by capability (section 6R); this section covers the
  // other four restore surfaces, which must not gate by role rank and so hide restore from the
  // recovery-only restore-operator
  ...
  // restore-operator. Reverting any gate below to a role-rank check turns the restore-operator
  // "sees" assertions red (a default-fail check).
  // ==========================================================================
  ...
  async function driveApprovals(role: Role): Promise<{ text: string; approveBtns: number; rejectBtns: number }> {
    ...
    //
    // expiresAt IS DERIVED FROM NOW AND MUST STAY THAT WAY. A frozen literal timestamp eventually falls
    // into the past, and nothing else reads the field to catch it, so a lapsed approval would silently
    // drive these capability assertions instead of a live one. The approval card names its own window
    // and withdraws Approve and Reject once it has closed, so a frozen deadline makes this fixture a
    // lapsed record and these vectors stop being about the capability gate at all. This test is about
    // restore.approve, so the record has to be LIVE for the gate to be the
  ...
  // disabled-with-reason when the caller lacks drill.run (design-system.md: disabled-with-reason, not
