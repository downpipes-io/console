...
interface RecoveryEngineState {
  ...
  status: StatusReport;
  // confirmCalls counts confirmRecoveryCodes() invocations; optional (defaults to 0 in the stub) so
  // the many pre-existing literals here need no update.
...
async function testRunRecovery(): Promise<void> {
  ...
  {
    // THE FIXTURE USES A BROWSER'S OWN WORDING, WHICH IS THE POINT RATHER THAN A CONVENIENCE: a message
    // such as `new Error("network down")` is not wording any browser produces. classifyError treats only
    // a message that reads as a fetch failure as `network`; everything else becomes `console-fault`,
    // precisely so a TypeError from the console's own render code does not read as an unreachable engine.
    // A TypeError saying "Failed to fetch" is what Chromium actually rejects with, so this drives the
...
async function testRunRegister(): Promise<void> {
  ...
  // recoveryCodesPending:true (a self-add re-enrolment over an existing live set -- the forced re-enrolment
  // after a recovery-code sign-in is the common case) means the fresh codes shown are STAGED, not yet live.
  // Confirming the panel must tell the engine before the boot proceeds, or the old set would be abandoned
  // as dead without the console ever having said so.
...
async function testRenderRecoveryAccess(): Promise<void> {
  console.log("\n-- renderRecoveryAccess: shows the count, offers Regenerate, gates retire on Owner --");
  {
    ...
    // Disabled-with-reason (UIA-33), not the native `disabled` attribute: the button stays
...
async function testConfigApprovalControl(): Promise<void> {
  console.log("\n-- configApprovalControl: the switch starts disabled; a non-owner stays read-only --");
  {
    ...
    // the state line says Owner only. Disabled-with-reason (UIA-33), not the native `disabled`
...
// SECTION 7: signInContextControl (Security Centre) - the unusual-location notify switch. It
...
async function testSignInContextControl(): Promise<void> {
  console.log("\n-- signInContextControl: owner-gated switch bound to the signin-context policy --");
  {
    ...
    // Disabled-with-reason (UIA-33), not the native `disabled` attribute: aria-disabled, so the
