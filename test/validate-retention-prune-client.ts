// Drives the REAL client-retention-prune.ts functions (R-62) via a real EngineClient against a
// fetch-stubbed transport, the same pattern test/validate-api-client.ts uses: no network, no DOM, the
// production request/response code runs exactly as it does from the panel. This file had no direct test
// at all before now (only reachable indirectly via test/validate-break-glass-prune.ts, which stubs
// engine.retentionPrune* wholesale rather than calling through the real transport functions), which is
// why R-62 dropped this file under coverage-per-file-gate.mjs's 50% REST_FLOOR as a new gap.
//
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("retentionPruneApply: previewOnly forwarding, the not-approved 403 (data, not a throw), and an ordinary 403 (still a throw)");
    {
      ...
      // 403" (403 keeps its own failResponse marker rather than the B31 reason-fold; see parseJsonOrReason's
