...
// (a non-normalised Math.round carry).
...
//
// The drawer's verbs are Drill this run and Build a restore plan, both of them things to do with an
// archive that exists. The commonest day-two action on a run that FAILED is to fix the cause and run it
// again, and Run lives on the downpipe, not here, so the drawer needs its own route there: the downpipe's
// name must not sit in the title as inert text with the only navigations being /runs and /restore. A
// customer who has to already know where to go is looking at a dead end.
