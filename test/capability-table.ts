// The role-by-capability table, transcribed ONCE for the whole console test suite.
//
// WHY THIS FILE EXISTS. A hand-typed copy of this table duplicated elsewhere is one oracle, not two,
// and one opportunity to type it differently from the engine's own answer, which is the enforcement
// authority and the only place the answer is actually decided. A table pinned only against the
// console's own ROLE_CAPABILITIES proves only that the console agrees with itself.
//
// So there is ONE transcription, here, and it is CHECKED against the engine's own source rather than
