// The in-console break-glass restore panel: the custody-critical behaviours, driven against the REAL panel
// code under the shared DOM shim (no jsdom, no network; the engine is a recording stub). This drives the
// panel, populates BOTH the break-glass private and the recovered per-run master, tears the panel down, and
// asserts BOTH closures are nulled (and the master zeroed in place) DIRECTLY on the panel's own state, never
// via a different closure -- the direct proof against a vacuous oracle, since asserting through a different
// closure would pass even if teardown left the real one intact. It also proves the load-bearing custody
// contract: the break-glass PRIVATE never appears in ANY request, and the recovered 32-byte MASTER crosses
// ONLY on the header (opts.restoreMasterB64), never in the restore body. A wrong key degrades to a plain
// "this key does not match this archive".
//
// The decap is REAL: the panel recovers the master with keydecap.ts openCapsule from the SAME pinned
// {identity, capsule, keyCommitment} -> master vector validate-keydecap.ts uses, so a genuine master is
// populated (not a stub), which is what makes the teardown-wipe assertion meaningful.
//
...
async function main(): Promise<void> {
  ...
  console.log("-- teardown-wipe + custody --");
  ...
  // Teardown zeros the master IN PLACE and nulls BOTH closures, asserted on the panel's own state.
  ...
  // A successful preview must not dead-end: the standard restore flow (confirm.ts) needs a parameter to
  // carry the master, or nothing ever completes. This drives the REAL panel end to end: preview, then the
  // REAL renderConfirm block it mounts, through a REAL Apply click, and asserts the master that opened the
  // PREVIEW is the SAME master that rides the APPLY call -- proof that apply plumbs through the same
  // recovered master rather than a relabelled preview.
  console.log("\n-- preview -> the real dual-control confirm+apply block -> Apply carries the SAME master --");
  {
    // An OWNER holds restore.apply so renderConfirm's role gate does not short-circuit to the read-only
    ...
    ok("the panel never shows the dead-end 'raise it from the standard restore flow' copy", !textOf(panel3.el).includes("raise it from the standard restore flow"));
