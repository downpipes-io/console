...
//
// The assertions are deliberately on TEXT and on whether the send/post happened, not on structure. A
// check that asserts a control exists while ignoring what it says passes on the broken tree too; a check
// of that shape guards nothing, so each block below states the value driven in, and reads back the sentence
// the operator
// gets and the count of requests built.
//
...

{
  ...
  // THE ASSERTION THAT FAILS BEFORE THE FIX: the send went ahead with all 121 characters.
...

{
  ...
  for (const kind of ["certificate", "licence"]) {
    ...
    // THE ASSERTIONS THAT FAIL BEFORE THE FIX: the box was offered, tickable, on every kind, and
    // switching to a kind the engine refuses left the tick standing.
...

{
  ...
  for (const [id, label] of [["github", "GitHub"], ["generic-oauth2", "Generic OAuth2"]] as const) {
    ...
    // THE ASSERTION THAT FAILS BEFORE THE FIX: the tick was offered on every tile.
...
//
// Driven against the engine's PRODUCTION handleAdmin, POST /admin/destination:
...
// The report of this defect named new URL() for the first of those; the pattern refuses it before new URL()
// is ever reached, which is why the values that DO reach new URL() are driven here as well.

{
  ...
  // THE ASSERTIONS THAT FAIL BEFORE THE FIX: startsWith("https://") accepted all three.
...
//
// Driven against the engine's POST /admin/licence route. The engine's shape gate is
// engine/src/admin/router-updates.ts:221, and it runs BEFORE any verify:
...

{
  ...
  // THE ASSERTIONS THAT FAIL BEFORE THE FIX: the rule was v.length >= 1, so every one of these passed.
...
//
// Driven against the engine's live behaviour with the FLAT preset body the console really posts (an earlier
// drive using an invented {connection:...} body produced a 500 and proved nothing): POST
// /admin/idp/connections twice with the same id answers 200 {ok:true} and then 200
// {ok:false,reason:'a connection with id "okta-prod" already exists'}, from
// engine/src/admin/idpconn.ts:59. Both add forms PRE-FILL the id, so "Add another Okta" opens a form
// already holding the taken value.

{
  ...
  // THE ASSERTION THAT FAILS BEFORE THE FIX: the form opened on the taken id and said nothing.
...

{
  ...
  // THE ASSERTION THAT FAILS BEFORE THE FIX. onInput wrote `state.name = v.trim().toLowerCase()`, and the
  // Save gate composes its proposal from `state`, never from this control, so the builder was already
  // holding a DIFFERENT name from the one on screen and would have saved it.
  //
...

{
  ...
  // here at the reader, because that list is the whole discriminator and it is what submit branches on.
  ...
  ok("an eaten rate and a negative rate both surface as coerced, which submit refuses on", coerced.includes("dest-price-storage") && coerced.includes("dest-price-egress"));
  ...
  // The contract matters here, not just the branch: if submitDestination saved the PRESET after readPricing
  // reported a coercion and told the operator "Destination verified and saved", every cost estimate from then
  // on would run on a rate the customer did not choose -- stored under `source: "operator"`, which asserts it
  // IS their contracted rate. Submit instead runs the console's own rule on these four controls and refuses.
  // The states, the customer-visible text and the evidence that replaces a silently-coerced row are driven
  // end to end in validate-support-posture-gaps-5.ts, which says what it asserts and why.
  //
