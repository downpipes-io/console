// The dual-control restore-approval inbox (/restore/approvals). The approver must
...
//     or a thrown listDownpipes -- never a throw out of the join, never a fabricated name (this path has
...
// waitFor drains until the OUTCOME holds rather than for a fixed number of rounds.
//
// A fixed round count can pass locally and flake on a loaded CI runner: flushAsync's budget is counted
// in ROUNDS, and a loaded shared runner interleaves a promise chain across more turns than a quiet
// laptop does, so an assertion could read a DOM that had not finished building. Raising the round count
// only moves the goalpost. Every step below instead waits for the specific thing it is about to assert,
// which removes the race outright.
//
...
// This is the generic form for "the render has finished", where there is no single outcome to name:
// a presence check like "has some text" is WRONG here, because a heading paints before the cards do
// and such a check would return before the inbox has actually rendered. Quiescence is the property
// actually wanted, and unlike a round count it does not care how fast the machine is.
async function waitForStable(read: () => string, stableRounds = 12, timeoutMs = 10000): Promise<void> {
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    // perfectly stable for as many rounds as that promise stays pending, easily three rounds under
    // contention.
...
async function main(): Promise<void> {
  ...
  async function renderInbox(
    ...
    // header painted" from "the inbox rendered": the root is non-empty and perfectly stable from the
    // moment the header lands.
    await listCalled;
  ...
  // ------------------------------------------------------------------------
  {
    ...
    await waitForStable(() => textOf(docBody()));
    // The confirm is the reason picker. There is no bare "Reject" confirm to press, deliberately: a
    // rejection with no reason attached anywhere would leave a decision no one can explain, and the only
    // way to reach the engine is to choose one of the closed members.
    ...
    // DISCRIMINATION: two rejections of the same plan for DIFFERENT reasons must reach the engine as
    // different reasons. Without a reason field on the wire, "wrong target" and "against policy" would be
    // the identical request, the identical record and the identical audit event, and the requester's "why
    // was my restore rejected?" would be unanswerable by every part of the system at once.
    docBody().replaceChildren();
