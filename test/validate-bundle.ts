...
// can ship under two names, and this guard's file-name comparison reads "STALE" against a bundle that is
// not stale. That is exactly what happened at e4849f8, where the failure was misread as an operating-system
// difference: a macOS build of that commit in a clean checkout reproduces CI's Linux bytes exactly, all
// four artefacts. scripts/stamp-build.mjs pins the working directory and refuses a symlinked node_modules
// for this reason, and the cwd-independence check below is the regression teeth for that pinning.
//
...
// THE FRESH BUILD IS OF THE WORKING TREE, WHICH IS NOT ALWAYS THE TREE THAT WAS COMMITTED.
//
// MEASURED, console b1f2a80c. This repo is shared: more than one pass edits src/ at a time.
// hooks/pre-commit rebuilds the bundle and stages the whole emitted set, and it builds from the WORKING
// DIRECTORY. So a commit made while ANY uncommitted source sits in the tree bundles that work. It
// happened: the lazily-loaded chunk in that commit carries strings that exist in no committed source, and
// the entry file changed only by its chunk-hash reference, which is why looking at app.js alone missed it.
//
// This guard PASSED throughout, all seven checks, including "a build run from another working directory is
// byte-identical". It had to: it compared the committed artefacts against a fresh build taken from the same
// dirty tree, so the hook and the guard agreed with each other and both disagreed with HEAD.
//
// SO WHEN src/ IS DIRTY, THE COMPARISON COPY IS BUILT FROM A CLEAN CHECKOUT OF HEAD instead. A detached
// worktree gives one without disturbing anybody's working tree, which matters precisely because the failure
// is about shared-tree state. node_modules is HARDLINKED rather than symlinked: stamp-build.mjs refuses a
// symlink out of the repo, because esbuild resolves it to its real path and renames chunk-<hash>.js without
// changing a byte. A hardlink copy leaves real directories, so the refusal is satisfied honestly.
//
// Proven on the commit that motivated it: a HEAD build emits demo-fetch-M6IOARMJ.js where the committed
// bundle carries demo-fetch-23OUAJZP.js, so the drift is named rather than absorbed.
//
...
if (DIRTY_SRC.length > 0) {
  ...
  try {
    ...
    verdictCannotCheck(
      `src/ is dirty and a clean checkout of HEAD could not be prepared (${e instanceof Error ? e.message : String(e)}), so the only build available here would be a build of a tree nobody has landed. ` +
        "Comparing against that would answer a different question than this guard asks. Commit or revert the listed files, or fix the checkout, and run it again.",
    );
  }
}
...
if (failures === 0) {
  try {
    ...
    // cost this workspace two investigations. Absolute paths throughout, because cwd is the thing under test.
