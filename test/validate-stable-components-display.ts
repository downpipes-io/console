...
export async function runDisplay(h: Harness, ctx: Ctx): Promise<void> {
  ...
  console.log("\n-- error-view --");
  {
    ...
    // reach the engine" copy: a malformed-json GET /admin/history response still means the request
    // completed and the engine was reached; only the body was unreadable.
  ...
  console.log("\n-- gated primary re-lock (prereqs card) --");
  {
    ...
    // WHAT THE REQUIRED ITEMS ACTUALLY ASK FOR. downpipes writes to four kinds of store and the engine's
    // own R2 binding is optional and commented out in its wrangler.toml, so a customer archiving to Amazon
    // S3, Google Cloud or Azure could not finish first run if the prerequisite named only R2: it would
    // require ticking a box that is false for them, or teach them that the product writes only to R2. The
    // prerequisite names every store the product writes to, so it stays true for all four. It is still
    // REQUIRED, because somewhere for the archives to go is the one prerequisite a backup product cannot
    // proceed without, and only the customer can answer it.
