...
//       "network" (a malformed 2xx body, e.g. on GET /admin/history, is not a reachability failure)
...
//
// THE FALL-THROUGH IS `console-fault`, NOT `network`: classifyError never asserts a reachability it
// cannot establish, so only a message reading as a browser's own fetch-failure wording keeps `network`,
// and "redirect: 302" is not one. The assertion here is the NEGATIVE pair below it: a 3xx is neither a
// server status nor an answered-but-unreadable 2xx.
...
// which have their own markers and are recognised earlier). Falling through extractStatus's 400-599
// filter to the generic network default would misdiagnose this: the block-error card would read "Could
// not reach the engine" for a request that plainly reached the engine and completed with a 200.
// answer-unreadable is the honest, distinct kind that avoids that misdiagnosis.
...
// cases above: `console-fault`, because "something went wrong" establishes nothing about reachability, and
// a `network` classification here would send an operator to check a network that is fine. The pair below
// is deliberately a POSITIVE and a NEGATIVE: the negative is what this line is really guarding, and it is
// the one that would catch the branch collapsing back into a single answer.
