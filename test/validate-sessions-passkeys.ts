...
// ML-03 regression below) installs the shared dom-shim first, like validate-destinations.ts.
//
// Coverage:
...
//       route (ASVS V7.5.1, ML-34: it used to bypass gatedFetch and misread the 401 as a sign-out)
...
//   ML-03 regression (REAL render over the dom-shim, ASVS V7.4.5): the "Sign out everyone" break-glass
...
// response), this is needed for the ML-34 regression below: it records each call's headers so the
...
async function main(): Promise<void> {
  ...
  // ------------------------------------------------------------------------
  console.log("\n-- CSRF ECHO (ML-02): the double-submit header the terminate-* calls attach --");
  // ------------------------------------------------------------------------
  {
  ...
  // V7.5.2: listSessions is a plain GET of the caller's own sessions; terminateSession POSTs one sid through
  // gatedFetch with the CSRF echo, and a step-up 401 is retried once with the minted token.
  {
    const { cap, restore } = captureFetch(200, { sessions: [{ sid: "abc", method: "passkey", connId: null, iat: 1, exp: 2, lastSeen: 1, clientFamily: "Firefox", networkPrefix: "203.0.113.0/24", current: true }] });
    const res = await engine.listSessions().finally(restore);
    ok("listSessions GETs /admin/sessions", cap.path.endsWith("/admin/sessions") && cap.method === "GET");
    ok("listSessions parses the rows", res.sessions.length === 1 && res.sessions[0]?.sid === "abc" && res.sessions[0]?.current === true);
  }
  {
    const { cap, restore } = captureFetch(200, { ok: true });
    const res = await engine.terminateSession("abc").finally(restore);
    ok("terminateSession POSTs /admin/sessions/terminate with the sid, and only the sid", cap.path.endsWith("/admin/sessions/terminate") && cap.method === "POST" && JSON.stringify(cap.body) === JSON.stringify({ sid: "abc" }));
    ok("terminateSession parses { ok }", res.ok === true);
  }
  {
    engine.onStepUpRequired = async () => "stepup-tok-1";
    const { calls, restore } = scriptFetch({ status: 401, body: { stepUpRequired: true } }, { status: 200, body: { ok: true } });
    await engine.terminateSession("abc").finally(restore);
    ok("terminateSession retries a step-up 401 once with the minted token", calls.length === 2 && calls[1]?.headers["x-downpipes-stepup"] === "stepup-tok-1");
    engine.onStepUpRequired = undefined;
  }
  {
  ...
  {
    // ML-34 regression (V7.5.1): deletePasskeyCredential must route through gatedFetch like every other
    // STEPUP_SUBS route. Before the fix it used a bare fetch(), so a stale-but-valid session's step-up
    // 401 fell through to classifyError as a generic "unauthorised", and the passkeys screen bounced the
    // operator to sign-in instead of completing the WebAuthn ceremony -- revoking nothing on the click that
    // matters most (a suspected-compromised device's key). Here a wired ceremony must complete the retry
    // and the credential must actually be revoked on the first call.
    const engine = new EngineClient("https://engine.test");
    let ceremonyRuns = 0;
  ...
  // ------------------------------------------------------------------------
  console.log("\n-- ML-03 regression (V7.4.5): 'Sign out everyone' success calls the REAL signOut() --");
  // ------------------------------------------------------------------------
  {
    ...
    const fakeEngine = {
      // passkeysCard's and selfSessionsCard's mount-time load() call these; empty lists keep those paths no-ops.
    ...
    // B56: terminate-all rotates the session signing key, which is ALSO the recovery-code HMAC key (engine
    ...
    ok("B56: the break-glass warning states recovery codes are invalidated", textOf(section).toLowerCase().includes("recovery cod"));
    ...
    ok("B56: the confirm modal states recovery codes are invalidated (not only the session sign-out)", surface !== null && textOf(surface).toLowerCase().includes("recovery cod"));
    ...
    // fleet data resident in the tab (the exploit this finding describes).
    ...
    // B56: the success toast must state recovery codes are now invalid and must be regenerated, so the
    ...
    ok("B56: the success toast states recovery codes are invalid and must be regenerated", bodyText.includes("recovery cod") && bodyText.includes("regenerate"));
  }

  // ------------------------------------------------------------------------
  console.log("\n-- DENY3: the admin danger block is BUILT collapsed, and refused (not merely hidden) for a viewer --");
  // ------------------------------------------------------------------------
  {
    // WHY THIS EXISTS. The live fan-out (harness spec/journeys/p2-session-auth-m2m-fanout.spec.ts, cell
    // DENY3-admin-collapsed) grades this on a real browser against a real estate, and it reported
    // the block STANDING EXPANDED for a viewer. It was reading the screen it had just left: the shell swaps
    // the main region inside a View Transition update callback that the browser runs asynchronously
    // (shell/app-shell.ts setMain), so the cell before it had expanded the very <details> the cell then read.
    // The harness navigation now waits the swap out. This is the half of that claim which needs no estate and
    // no browser: what the screen BUILDS, graded straight off the constructed DOM, so a regression is caught
    // here rather than by a live cell that can only run when an estate is current.
    //
    ...
    installDomShim();
    const listEngine = { async listPasskeyCredentials() { return { credentials: [] }; }, async listSessions() { return { sessions: [] }; } } as unknown as EngineClient;
    ...
    for (const role of ["viewer", "access-admin", "owner"] as const) {
      ...
      // THE DENY3 ASSERTION. `open` is an attribute the constructor either sets or does not; collapsedSection
