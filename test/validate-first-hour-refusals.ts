...
//
// THE DEFECT THIS GUARDS AGAINST: setup is Destinations, then Sources, then Downpipes, and each of
// those three screens ends in a save that the engine can refuse. Left unguarded, all three render
// the TRANSPORT'S THROW straight into the one sentence the customer gets. The engine's own wording
// is good; the plumbing around it is not, and unchecked would read like:
//
...
// THE CONSOLE'S OWN INTERNAL TOKENS MUST NEVER SURFACE; THE ENGINE'S OWN WORDS MAY. The rule lives in
// components/error-view.ts refusalText / engineReason, and the destination form and the downpipe
// editor go through it.
//
// THE SOURCE-TOKEN SITE IS COVERED TOO, and the DRIVEN section at the foot of this file is what
// covers it. screens/sources/shared.ts errMsg and screens/sources-downpipes/helpers.ts errMsg both go
// through refusalText, so the three shapes never reach the field as "...Read scopes.: 400",
// "discovery token: 400" or the bare "forbidden-class=not-engine-body: 403".
//
// THE CALLERS THAT WRAP IT ARE PART OF THE SAME RULE, not a separate tidy-up. The no-reason branch
// returns a whole reviewed sentence carrying its own advice, so "Could not start a run (<that
// sentence>)" would put a sentence inside a parenthesis inside a sentence and give the customer two
// instructions in one line. Eleven toast and inline-error sites read "<what failed>. <why>".
//
...
verdictReached(failures, checks); // the verdict is DECLARED, so a silent exit 0 cannot pass as green
