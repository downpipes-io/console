...
// named function under the 50-line flag (console-test-006-01); bodies are unchanged.
...
async function testStepUpFlow(fx: PasskeyFixtures): Promise<void> {
  ...
  console.log("\n-- runStepUpOutcome: the three outcomes are DISTINGUISHABLE, and the fourth is merged on purpose --");
  {
    ...
    const messages = (["begin-refused", "no-passkey", "no-assertion", "finish-refused"] as const).map((r) => stepUpFailureMessage(r));
    ok("the no-passkey ending tells the operator to enrol, not to retry (V6.8.4)", stepUpFailureMessage("no-passkey").includes("Enrol a passkey") && !stepUpFailureMessage("no-passkey").includes("Try again in a moment"));
    ok("each outcome has its OWN sentence", new Set(messages).size === 4);
  ...
  console.log("\n-- the bootstrap banner, checked against the engine's own gate list --");
  {
    ...
    if (p === undefined) {
      ...
      if (process.env.REQUIRE_ENGINE === "1") {
        verdictCannotCheck(
          "validate-passkey-login: REFUSED, REQUIRE_ENGINE=1 and no engine checkout is reachable.\n" +
...
// the engine had answered and discarding it. Same class as the malformed-2xx correction in errors.ts (the
// chaos-visual finding,).
//
