...
// src/components/custody-step.ts, the wrapping-key integrity checksum, and the
// security-key credential-id-rides-with-the-ciphertext behaviour. Run with
// `node test/validate-custody-files.ts`.
//
// These are the exact round-trips the adversarial review found UNTESTED. They compose the
// verified primitives (envelope.ts, shamir.ts, webauthn-prf.ts) the way the component does:
//
// Coverage:
//   identityPlaintext:
...
async function checkShareRoundTrip(): Promise<void> {
  console.log("\n-- custody-files: share files round-trip, combine, and verify --");
  {
    ...
    // A single BAD/mis-transcribed share yields a wrong key that the checksum
...
async function checkNoSecretToSheet(): Promise<void> {
  console.log("\n-- custody-files: only PUBLIC metadata reaches the recovery sheet --");
  {
    ...
    // A Tier 1/2 metadata with the security-key credential id (the public handle). The
