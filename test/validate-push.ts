...
// The refusal rests on the parser argument that no SIEM auto-parses CEF out of a bucket, not on any
// claim that the s3 drop is limited to NDJSON: the drop writes whichever format is chosen.
...
// ---------------------------------------------------------------------------
// THE RISK THIS GUARDS AGAINST. The syslog-tls sink's record builder reads "LEEF if leef, else CEF", so
// `sink: "syslog-tls"` with `format: "splunk-hec"` would fall into the CEF arm and SILENTLY SHIP CEF: a
// Splunk HEC parser would receive ArcSight CEF. A combination that validates and then sends a different
// wire than the operator chose is worse than one that is refused, because nothing anywhere records the
// substitution.
//
// The engine refuses the same pair at both boundaries and its sender takes a narrowed two-member type. This
// is the console half. The formats are enumerated against the closed union rather than spot-checked, so a
// ninth format cannot slip through untested.
{
...
{
  // The state line makes no delivery claim for this combination: nothing is delivered at all for such a
  // destination (not "(sent as CEF)", which would describe the silent substitution the guard above prevents).
...
// CrowdStrike Falcon Next-Gen SIEM (both splunk-hec over http). The console records WHICH vendor it is.
{
...
ok("canTogglePush treats an absent sink as http (true)", canTogglePush({}));
{
  ...
  //
  // canTogglePush gates on the SINK only, so an http view whose redacted endpoint is absent must not sail
  // straight through: defaulting the missing endpoint to "" and submitting it would post an endpoint the
  // operator never typed, and the engine either refuses it (Enable reads as an unexplained failure,
  // recorded nowhere) or stores it, silently ending the audit-log drain while the panel still shows a
  // configured destination. Refuse instead.
...
// the console is the only place that code can become something an operator can act on: interpolating the slug
// straight in would leave the trail reading "failed sending a test event: network-error."
//
// A concrete case this must cover: a Splunk Cloud stack serves its HEC ingest port with Splunk's stock
// self-signed certificate while 443 carries a proper one, so strict TLS refuses the ingest port before any
// HTTP happens. The engine names that network-tls, and this is where it becomes readable.
...
ok("raw-json label names the Downpipes wrapper, not NDJSON (a separate, distinct format)", pushFormatLabel("raw-json").includes("Downpipes") && !pushFormatLabel("raw-json").includes("NDJSON"));
