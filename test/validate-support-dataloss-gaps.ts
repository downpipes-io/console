// Validate the DATA-LOSS support-pack gaps from the diagnostic-coverage audit. Each of these is a case where
// the customer loses something they cannot get back (their recovery material, a copy of their data, or the
// truth about a restore) and is never told, and where the support pack afterwards holds no evidence that it
// happened.
//
// THE RULE THE SUITE DEFENDS: a gap is not closed until its evidence is IN THE BUNDLE. So every assertion
// below drives the REAL fault site (never a re-implementation), and then asserts the row that lands in the
// console-diagnostics ring, which is what the support screen POSTs into the pack the customer generates
// (screens/settings/support.ts -> api.getSupportBundle(packPayload())). A recorder with no caller, or a
// caller whose row never reaches the payload, is a failure mode this suite watches for directly.
//
// AND THE OTHER RULE: no-custody is structural. The payload assertions below re-check that no sentinel value
// (a file name, an error message, a record name) can be found anywhere in the JSON the console would send.
//
// Coverage:
//   the destination fan-out read fails: the operator is TOLD before a batch silently loses its copies
//   the key-ceremony intent marker is taken at the ATTEMPT, so a failed ceremony != no ceremony
//   a browser capability that FAILS in a key or recovery ceremony says so, and reaches the pack as a
//         row that names the capability, the ceremony and whether the capability was absent or refused
//   a live apply is correlatable: half-applied, applied-nothing and answered-wrong are distinct rows
//
...
// download in this suite is REFUSED by default, which is exactly the fault under test here. grantDownloads
...
// ===========================================================================
// a browser capability failure in a key or recovery ceremony is TOLD to the operator, and lands in the
// pack as a row a support engineer can ACT on.
//
// THE BAR THIS SECTION HOLDS THE CODE TO: working plumbing is not enough. Routing the refusal through the
// window-fault seam would make a refused identity.key download and an unrelated TypeError on the same screen
// produce BYTE-IDENTICAL rows, and on one screen would coalesce a refused recovery-codes download, a dead
// clipboard and a console bug into a single row with a count of three. So the assertions below are not "a row
// was recorded". They are: the row SEPARATES the outcomes a support engineer must separate, it never lands in
// the console-DEFECT counter, and the discriminators survive the coalescing tuple all the way onto the wire.
// ===========================================================================
...
// THE DISCRIMINATION, MADE INTO A TEST. A refused identity.key download and an unrelated null dereference on
// the same screen must not read as the same row, character for character. They must be two rows that no one
// could confuse, and the console bug must be the only thing in the DEFECT bucket.
{
  resetRing();
  setActiveScreen("/keys");
  refuseDownloads();
...
// THE OTHER HALF OF THE DISCRIMINATION: THE COALESCING TUPLE. A field that discriminates two outcomes but is
// left OUT of the coalescing key discriminates NOTHING. On the access screen, a refused recovery-codes
// download, a dead clipboard and an unrelated console bug must not collapse into ONE row with a count of three.
{
  resetRing();
...
// legitimate configuration and not a console defect. Recording it as `unhandled` would inflate the counter that
// is defined to mean "a defect that reached no catch site", so it is its own outcome, nowhere near the defect
// bucket.
{
  const realNavigator = (globalThis as { navigator?: unknown }).navigator;
  resetRing();
...
// A KEY CEREMONY THAT PRODUCES NO KEYS. keys/posture and keys/rotation catch the ceremony's failure and render
// err.message into a field__error the customer alone sees; that alone would report NOTHING. A locked-down
// browser that cannot generate a key pair must not leave ZERO pack evidence of the single most consequential
// thing that can go wrong in the console.
{
  resetRing();
  setActiveScreen("/keys");
  grantDownloads();
  ...
  // what it claims the strict posture costs is load-bearing rather than decorative. Claiming "every proof is
  // offline" would be false: verify-at-seal and the canary run on the run's own per-run key, so a
  // break-glass-only engine reaches the same keyed tier and runs the canary with no skipped checks, both
  // in-account. Overstating the cost pushes a customer away from the stricter posture for a reason that
  // does not hold.
  //
  // These assert the CLAIMS, not the exact sentences, so the copy can be improved without breaking them,
  // while a regression to the overstatement fails.
  //
...
// THE RECOVERY CODES. The engine shows them ONCE, and on the regenerate path it has ALREADY invalidated the
// previous set. A browser that refuses the download must not take a silent early return or throw uncaught out
// of the click handler: that would leave the operator pressing a button, nothing arriving, the panel saying
// nothing, and a later lost passkey locking them out permanently.
{
  resetRing();
...
// failed ceremony by throwing into the ceremony's catch and saying the keys could not be generated.
{
  resetRing();
  refuseDownloads();
  setCeremony(null);
...
// ===========================================================================
// an unreadable destination list never silently costs the operator their copies.
// ===========================================================================
console.log("\n-- a failed destination read cannot silently degrade a batch to one copy --");
{
  ...
  // THE DISCRIMINATOR. The generic engine-call row above does not answer the question that matters here: it
  // says a read on the sources screen failed, which is also what a discovery blip says, and it cannot say
  // whether the operator then went ahead and created the batch anyway.
  ...
  // TUPLE COLLISION, the other half of the discrimination: a 500 from discoverSources on the SAME screen must
  // not coalesce into the same row as a lost fan-out. Different kinds, so different tuples, so two rows.
...
// both states recorded the same evidence the discriminator would be worthless.
{
  resetRing();
  setActiveScreen("/sources");
  let bulkCalls = 0;
...
// ===========================================================================
// a live apply is correlatable with what the engine actually did.
// ===========================================================================
console.log("\n-- EVERY apply ending is recorded, and silence never means 'clean' --");
{
  resetRing();
  ...
  // THE FOURTH ENDING. An apply with no failures must not record NOTHING: nothing is also what a restore that
  // never ran leaves behind, and the pack must be able to tell a restore that worked from a restore that
  // reported success and moved no data.
