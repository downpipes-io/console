...
function main(): void {
  ...
  // The regression guard: disabled and unknown must NOT both be the
  // info glyph, and must differ from each other by glyph (shape), so the
  ...
  // ONE destination. Without de-collision, every edge would terminate at the SAME point
  // (leftEdgeX, destY) and every join glyph + label would stamp at the SAME X with compressed
  // Ys, so the pipes would collapse into one bundle and the labels' legibility plates would
  // paint over each other. Build that exact case (5 sources, 1 shared destination) and assert
  // the de-collision:
  ...
  // not all equal to the single destination node's centre (a collapsed layout).
  ...
  // than a compressed midY band. Adjacent marker anchors must be at least one plate
