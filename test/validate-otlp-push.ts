...
// biome-ignore lint/complexity/noUselessLoneBlockStatements: the bare block is this validator's section marker, not dead scope
{
  ...
  //
  // Defaulting a missing endpoint to "" and submitting it would be a set the operator never made. The
  // engine either refuses it (so Enable reads as an unexplained failure with nothing recorded
  // anywhere) or accepts it and overwrites a working endpoint with an empty one, at which point the
  // metrics push silently stops going anywhere and the console still renders the destination as
  // configured. Refusing to reconstruct is the honest answer: the panel says the view is incomplete
  // and routes the operator to Replace.
